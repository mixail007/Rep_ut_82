...
функция ПолучитьМассивДляЗамены()
	...
	рез.Вставить("[СпособОплаты]", СсылкаНаОбъект.СпособОплаты);
...
СуммаУтвержденная = таблБонусы.Итог("Сумма") - таблБонусы.Итог("СуммаОбрПродаж");//30.01.2018
	...
	//ИтогоКоличество   = Цел(ИтогоКоличество/100)*100;   //округление до 100-шт. -30.01.2018
   ...
   Если ТекПокупатель.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо Тогда
	...
	//=============================================================
		...
		рез.Вставить("[Сумма]",Строка(Формат(СуммаУтвержденная * 1.18,"ЧДЦ=0"))); //+++ 22.01.2018  
		рез.Вставить("[СуммаБезНДС]",Строка(Формат(СуммаУтвержденная,"ЧДЦ=2")));  // = сумма / 1.18 (НДС включён в стоимость)
		рез.Вставить("[СуммаБонуса]",Строка(Формат(цел(таблБонусы.Итог("СуммаБонуса")),"ЧДЦ=0")));
		процент = 100*таблБонусы.Итог("СуммаБонуса")/СуммаУтвержденная;
		рез.Вставить("[Процент]",формат(процент,"ЧДЦ=1") );
