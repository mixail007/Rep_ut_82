...
функция ПолучитьМассивДляЗамены()
   ...
   Если ТекПокупатель.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо Тогда
	...
	КонецЕсли;
		...
		рез.Вставить("[СписокПроизводителей]",ПроизводителиСтрока);
		
		рез.Вставить("[ОкончаниеПериода]", Формат(СсылкаНаОбъект.ДатаПо,"ДЛФ=DD"));
	
		ПроцентСтавка = СсылкаНаОбъект.СтавкиБонуса.Выгрузить();
		если ПроцентСтавка.Количество()>0 тогда   
			// средне - арифметический Процент за год по всем поставщикам!
			чПроцент = ПроцентСтавка.Итог("Ставка") / ПроцентСтавка.Количество();
			Если чПроцент = Цел(чПроцент) тогда
				Процент = строка(чПроцент);
			Иначе	
				Процент = формат( чПроцент, "ЧДЦ=1"); 
			КонецЕсли;	
		иначе
			Процент = "___";
		КонецЕсли;
		рез.Вставить("[Процент]", Процент);

		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("Контрагент",СсылкаНаОбъект.Контрагент);
		Запрос.УстановитьПараметр("Вид",Справочники.ВидыКонтактнойИнформации.НайтиПоКоду("00018"));
		запрос.Текст = "ВЫБРАТЬ КИ.Представление
		               |ИЗ
		               |	РегистрСведений.КонтактнаяИнформация КАК КИ
		               |ГДЕ
		               |	КИ.Объект = &Контрагент
		               |	И КИ.Вид = &Вид";
		рез1 = Запрос.Выполнить().Выгрузить();
		Если Рез1.Количество() > 0 Тогда
			Сайт = рез1[0].Представление;
		Иначе Сайт = "_______________";
		КонецЕсли;
		рез.Вставить("[Сайт]", Сайт);
