...
Функция ПечатьРеализации( Регистратор) Экспорт
	...
	Запрос.УстановитьПараметр("ПроизводительТД", Справочники.Производители.НайтиПоКоду("3333"));
	...
	Запрос.Текст="ВЫБРАТЬ РАЗЛИЧНЫЕ
	             ...
	             |					И (Номенклатура.Наименование ПОДОБНО ""%LegeArtis%"" или Номенклатура.Производитель=&ПроизводительТД)
	             ...
	             |", "ИЗ
					 ...
					 |		И (ЗаказыОстатки.Номенклатура.Наименование ПОДОБНО ""%LegeArtis%"" или Номенклатура.Производитель=&ПроизводительТД)
			     ...
			     |			ГДЕ
				 |				Аналоги.Заменяемое.ВидТовара = ЗНАЧЕНИЕ(Перечисление.ВидыТоваров.Диски)
				 |				И Аналоги.Заменитель В ИЕРАРХИИ (&Крышки)
				 |				И (Аналоги.Заменяемое.Наименование ПОДОБНО ""%LegeArtis%"" или Аналоги.Заменяемое.Производитель=&ПроизводительТД)) КАК КомплектующиеНоменклатуры
...
ТабДок=Новый ТабличныйДокумент;
	...
	Если ТаблицаДискиБезКрышек.Количество()>0 Тогда
		...
		Для каждого Выборка ИЗ ТаблицаДискиБезКрышек Цикл
			...
			ТабДок.Вывести(ОбластьДетальныхЗаписей);
		КонецЦикла;	
		КоличествоИтогДискиБезКрышек=ТаблицаДискиБезКрышек.Итог("Количество");
		ОбластьИтог=Макет.ПолучитьОбласть("ПодвалТаблицыДискиБезКрышек");
		ОбластьИтог.Параметры.Количество=КоличествоИтогДискиБезКрышек;
