...
Процедура КнопкаВыполнитьНажатие(Элемент)
	...
	Если ТипЗнч(СсылкаНаОбъект)=Тип("ДокументСсылка.РеализацияТоваровУслуг") И Не ПолучитьЗначениеПоУмолчанию(ПараметрыСеанса.ТекущийПользователь, "УчетТолькоПоПодразделениюПользователя") Тогда
		НапечататьРаспискуИДискиПоКрышкам();
	КонецЕсли;
...
Функция ПечатьТТН()
	...
	ОбластьМакета.Параметры.Номер                     = ПолучитьНомерНаПечать(СсылкаНаОбъект);
...
Функция ПечатьТТН_отвХран()
	...
	Шапка = ПолучитьШапку_ОтвХран(); //23.08.2018
	...
	|	ГДЕ
	|		РеализацияТоваровУслуг.Ссылка В (&ДокументСписок)
	|
	|) КАК ВложенныйЗапрос
	|
	|УПОРЯДОЧИТЬ ПО Метка ВОЗР, НомерСтроки ВОЗР
...
Процедура ПриОткрытии()
	...
	ЭлементыФормы.Перевозчик1.Значение = Справочники.Контрагенты.ПустаяСсылка();
	ЭлементыФормы.Водитель1.Значение = Справочники.ФизическиеЛица.ПустаяСсылка();
	ЭлементыФормы.МаркаАвтомобиля1.Значение = "";
...
процедура ЗаполнитьДанныеПоДокументу() Экспорт
	...
	Если ЗначениеЗаполнено(СсылкаНаОбъект) Тогда
		...
		попытка
			...
			//			СсылкаНаОбъектЗакПок = СсылкаНаОбъект.ВнутреннийЗаказ.ДокументОснование;
			//		иначе
			//			сообщить("Не заполнен документ основание в поле ""Внутренний заказ"" документа ""Перемещение товаров""!", СтатусСообщения.Внимание);
			//		КонецЕсли;	
			//	иначе
			//		сообщить("Не заполнен Внутренний заказ в документе! Данные формы заполняются по документам основания перемещения: Внутренний заказ и Заказ покупателя!", СтатусСообщения.Внимание);
			//	КонецЕсли;
			//КонецЕсли;
			
			
			Если ТипЗнч(СсылкаНаОбъект) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
				или ТипЗнч(СсылкаНаОбъект) = Тип("ДокументСсылка.ПеремещениеТоваров")
				...
				Сообщить(СсылкаНаОбъект.Водитель);
				ЭтотОбъект.Водитель             	 = СсылкаНаОбъект.Водитель;
				Сообщить(ЭтотОбъект.Водитель);
				...
				Если ТипЗнч(СсылкаНаОбъект) <> Тип("ДокументСсылка.ПеремещениеТоваров") тогда //+++ 24.02.2012
					
					ЭтотОбъект.СрокДоставки = СсылкаНаОбъект.СрокДоставки;
					ЭтотОбъект.ВидПеревозки = ?(СокрЛП(СсылкаНаОбъект.ВидПеревозки)="", "Автомобильным транспортом", СсылкаНаОбъект.ВидПеревозки);//+++ 27/07/2011
					
					//ЭтотОбъект.ПунктПогрузки = ПолучитьАдресИзКонтактнойИнформации_Лок(СсылкаНаОбъект.Организация,"Складской");
					ЭтотОбъект.ПунктПогрузки		= СсылкаНаОбъект.ПунктПогрузки;
					Если ТипЗнч(СсылкаНаОбъект) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
						Если СокрЛП(СсылкаНаОбъект.Сделка.АдресДляТТН)<>""  Тогда
							ЭтотОбъект.ПунктРазгрузки      	= СокрЛП(СсылкаНаОбъект.Сделка.АдресДляТТН);
						иначе
							ЭтотОбъект.ПунктРазгрузки      	= СсылкаНаОбъект.АдресДоставки;
						конецЕсли;	
					иначе
						...
						КонецЕсли;	
						ЭтотОбъект.ПунктРазгрузки      	= ?(СокрЛП(СсылкаНаОбъект.АдресДоставки)="", СокрЛП(СсылкаНаОбъект.ПунктРазгрузки), СокрЛП(СсылкаНаОбъект.АдресДоставки) ); //+++ 28.10.2011
					конецЕсли;
					ЭтотОбъект.СрокДоставки         = СсылкаНаОбъект.СрокДоставки; //=Дата(1,1,1), СсылкаНаОбъект.Дата, СсылкаНаОбъект.СрокДоставки);
					ЭтотОбъект.ВидПеревозки        	= ?(СсылкаНаОбъект.ВидПеревозки="", "Автомобильным транспортом", СсылкаНаОбъект.ВидПеревозки);
					//02.08.11	
				...
				КонецЕсли; // не перемещение
				
			КонецЕсли; //только по нужным документам
			...
			флЮрЛицо = (Шапка.Грузоотправитель.ЮрФизЛицо = перечисления.ЮрФизЛицо.ЮрЛицо);
			...
			//+++ 2-ой этап
			Если УсловиеМосква и ОтветМосква = КодВозвратаДиалога.Да  тогда //+++ 10.05.2012
				ЭтотОбъект.ПунктРазгрузки = СведенияОГрузополучателе.ФактическийАдрес;
			иначе	
				//16.08.18 Смирнов, для франчайзи в перемещениях адрес ттн из заказа
				Если ТипЗнч(СсылкаНаОбъект)=Тип("ДокументСсылка.ПеремещениеТоваров") и (СсылкаНаобъект.СкладПолучатель.Родитель=Справочники.Склады.НайтиПоКоду("02337"))тогда
					Если ЗначениеЗаполнено(СсылкаНаОбъект.ВнутреннийЗаказ) тогда
						Если ЗначениеЗаполнено(СсылкаНаОбъект.ВнутреннийЗаказ.ДокументОснование) тогда
							ЭтотОбъект.ПунктРазгрузки = СокрЛП(СсылкаНаОбъект.ВнутреннийЗаказ.ДокументОснование.АдресДляТТН);
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
				
				ЭтотОбъект.ПунктРазгрузки = ?(СокрЛП(ЭтотОбъект.ПунктРазгрузки)="", ?(ГрузополучательПредставление<>"",СведенияОГрузополучателе.ФактическийАдрес,""), СокрЛП(ЭтотОбъект.ПунктРазгрузки));
			КонецЕсли;	
			...
			Если ТипЗнч(ЭтотОбъект.СсылкаНаОбъект) = Тип("ДокументСсылка.ПеремещениеТоваров") тогда	//+++
				dt = 0;
			иначе
				dt = ЭтотОбъект.СсылкаНаОбъект.ДоговорКонтрагента.ДопустимоеЧислоДнейЗадолженности;
			КонецЕсли;	
			...
			КонецЕсли;
			ЭлементыФормы.Пункт5_3Ведомость.Значение = "стоимость: "+СформироватьСуммуПрописью(ПолучитьСуммуДокументов(), валюта1);
			// = "стоимость: "+Формат( ПолучитьСуммуДокументов() , "ЧДЦ=2; ЧН=0.00")+" р.";
			
			
			Если ЭтотОбъект.Реализации.Количество()=0 тогда	
				ЭлементыФормы.ПереченьДопДокументов.Значение = "Накладная №"+Строка(ПолучитьНомерНаПечать(ЭтотОбъект.СсылкаНаОбъект))+" от "+Формат(ЭтотОбъект.СсылкаНаОбъект.Дата,"ДЛФ=DD");
			...
			//+++( 02.08.2011		
				...
				Для каждого Стр из СсылкаНаОбъект.Товары цикл
					...
					иначе		
						...
						Пункт3_1 = Пункт3_1 + " - "// - количеств каждого
						+?(Стр.КоличествоМест=0, строка(Стр.Количество)+" шт.", 
													строка(Стр.КоличествоМест)+" мест ("+ строка(Стр.Количество)+" шт.)")+"; ";
					КонецЕсли;
			...
			КонецЕсли;	
				...
				пока i<=ЭтотОбъект.Реализации.Количество() цикл
					...
					Если первый тогда
						...
						Иначе
							ПереченьДопДокументов =?(Реализации.Количество()=1,"Накладная №","Накладные №№")+ ПолучитьНомерНаПечать(ЭтотОбъект.Реализации[i-1].Реализация);
						конецЕсли;
					...
					иначе 
						...
						иначе	
							ПереченьДопДокументов = ПереченьДопДокументов +", "+ПолучитьНомерНаПечать(ЭтотОбъект.Реализации[i-1].Реализация);
						конецЕсли;
					КонецЕсли;
...
Процедура ДействиеЗаполнитьНажатие(Элемент)	
	...
	Если Не ЗначениеНеЗаполнено(СсылкаНаОбъект) Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если первый тогда
				ПереченьДопДокументов =?(Выборка.Количество()=1,"Накладная №","Накладные №№")+ПолучитьНомерНаПечать(Выборка.Ссылка);
				дата1 = " от "+Формат(Выборка.Ссылка.Дата,"ДЛФ=DD");
				первый = ложь;
			иначе 
				ПереченьДопДокументов = ПереченьДопДокументов +", "+ПолучитьНомерНаПечать(Выборка.Ссылка);
			КонецЕсли;
