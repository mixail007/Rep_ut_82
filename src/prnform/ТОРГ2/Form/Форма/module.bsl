...
Процедура ПолучитьДанныеПоДокументу()
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Организация", ВыбраннаяОрганизация.Ссылка);
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	ЭтаФорма.МестоПриемкиТовара 		= "Склад " + Строка(СсылкаНаОбъект.СкладПолучатель);//ФактАдресОрганизации; 
	...
	ЗапросНоменклатура.УстановитьПараметр("Перемещение",СсылкаНаОбъект);
	ЗапросНоменклатура.Текст = "ВЫБРАТЬ
	                           ...
	                           |		ВЫБОР
	                           |			КОГДА ПеремещениеТоваров.КоличествоФакт <> 0
	                           |				ТОГДА ПеремещениеТоваров.Количество - ПеремещениеТоваров.КоличествоФакт
	                           |			ИНАЧЕ 0
	                           |		КОНЕЦ КАК Расхождение,
	                           |		ПеремещениеТоваров.Количество КАК Количество,
	                           |		ПеремещениеТоваров.КоличествоФакт КАК КоличествоФакт
	                           |	ИЗ
	                           ...
	                           |		ПеремещениеТоваров.КоличествоФакт) КАК А
	                           |ГДЕ
	                           |	А.Расхождение > 0";
	...
	Для Каждого Стр ИЗ Рез Цикл
		  ...
		  НовСтр.КоличествоДок 		= Стр.Количество;
		  НовСтр.КоличествоФакт 	= Стр.КоличествоФакт;
	КонецЦикла;
	...
	ПолучитьРуководителя.УстановитьПараметр("СтруктурнаяЕдиница", СсылкаНаОбъект.Организация.Ссылка);
...
Процедура КнопкаВыполнитьНажатие(Кнопка)
	...
	ВыбраннаяОрганизация = СсылкаНаОбъект.Организация;
	ВыбранныйКонтрагент = СсылкаНаОбъект.Организация;
...
//=================================================	
	...
	ОблМакетаСтраница1Шапка.Параметры.Поставщик = "Склад " + Строка(СсылкаНаОбъект.СкладОтправитель);//ВыбранныйКонтрагент.НаименованиеПолное+", "+ЮрАдресКонтрагента+", "+ТелефонКонтрагента;
