...
//31.08.2018 - единая функция! 3 раза!
  ...
  //  ОбластьМакета.Параметры.СписокНомеровПортальный = СписокНомеровПортальный;
	...
	Пока Результат.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Результат.Заказ) тогда
			Заказчик = СделатьВсеЗамены(Результат.Заказ.Комментарий);//+++ из первоначального!
		иначе 
			Заказчик = "";
		КонецЕсли;
		ОбластьСтрока.Параметры.Заказчик = Заказчик;
...
Функция ПечатьЗаказаПокупателя()
	...
	Если СписокЗаказов.Количество()>0 тогда
		...
		Для каждого СтрТЧ ИЗ СсылкаНаОбъект.Заказы Цикл
			...
			ОбластьМакета.Параметры.Заказчик = СделатьВсеЗамены(СтрТЧ.ЗаказПокупателя.Комментарий);
	...
	Иначе
		...
		ОбластьМакета.Параметры.Заказчик = СделатьВсеЗамены(СсылкаНаОбъект.Комментарий);
		Если Найти(врег(СсылкаНаОбъект.Комментарий), "ПЕРЕМЕЩЕНИЕ ИЗ ЯРОСЛАВЛЯ") = 0 Тогда
			ОбластьМакета.Параметры.Поставщик = "Яршинторг (Шинтрейд)";
		Иначе
			ОбластьМакета.Параметры.Поставщик = "Яршинторг (Ярославль)";
...
Функция Печать() Экспорт
	...
	Если Найти(СсылкаНаОбъект.Комментарий, "объединенный заказ")>0 
		или Найти(СсылкаНаОбъект.Комментарий, "Общий заказ")>0 
		или Найти(СсылкаНаОбъект.Комментарий, "Общ.заказ")>0 тогда   //31.08.2018
		СписокЗаказов = НайтиПервоначальныеЗаказы();
		Если СписокЗаказов.Количество()>0 тогда 
