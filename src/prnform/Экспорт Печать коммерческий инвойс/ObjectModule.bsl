...
Функция ПечатьДокумента(СсылкаНаОбъект)
	...
	Если ДваЯзыка Тогда
		...
		Иначе
			...
			//ОбластьШапка.Параметры.ФактАдресПродавца = "YAROSLAVL 150049,RUSSIA, LISITSINA STR., 7 / "+ФактАдресПродавца;
			//ОбластьШапка.Параметры.ФактАдресПродавца = "YAROSLAVL 150014,RUSSIA, SALTYKOVA-SHCHEDRINA STR., 21, OFFICE 507 / "+ФактАдресПродавца;
	...
	Для Каждого СтрокаЗаголовок Из ТЗГруппТоваров1 Цикл	
		...
		Если СсылкаНаОбъект.ДоговорКонтрагента.ВалютаВзаиморасчетов = Справочники.Валюты.НайтиПоКоду("840") Тогда
			...
			ОбластьСтрокаИтог.Параметры.СуммаСтрока = ТЗИтогСтрока.Итог("Сумма");
