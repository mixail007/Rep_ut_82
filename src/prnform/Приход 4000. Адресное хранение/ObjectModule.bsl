...
//ЗаказПоставщику
Функция ПечатьЗаказаПоставщику()
   ...
   Макет = ПолучитьМакет("Заказ");
	...
	ОбластьМакета = Макет.ПолучитьОбласть("Заголовок");
	ОбластьМакета.Параметры.ТекстЗаголовка = СформироватьЗаголовокДокумента(СсылкаНаОбъект,"Заказ поставщику" );
	//+++ Штрих-Код номера документа - февраль 2012
	// требуется установленная компонента 1CBarCode.exe
    	ОбШтрихКодСообщение = глТорговоеОборудование.ПолучитьШтрихКодПоДокументу(СсылкаНаОбъект);
		попытка
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ЗаказПоставщику", СсылкаНаОбъект);
	
//			   
Запрос.Текст = "ВЫБРАТЬ
               |	ЕСТЬNULL(ДокЗаказПоставщику.НомерСтроки, 1000) КАК НомерСтроки,
               ...
               |		ГДЕ
               |			ЗаказПоставщикуТовары.Ссылка = &ЗаказПоставщику) КАК ДокЗаказПоставщику
               |		ПО ЗаказыПоставщикамОстатки.Номенклатура = ДокЗаказПоставщику.Номенклатура
               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыАдресноеХранение.Остатки(, Склад = &Склад5000) КАК ТоварыАдресноеХранениеОстатки
               |			ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ РАЗЛИЧНЫЕ
               |				РНАдреса.АдресХранения КАК АдресХранения,
               |				РНАдреса.КоличествоОстаток КАК КоличествоОстаток
               ...
               |				РегистрНакопления.ТоварыАдресноеХранение.Остатки(, Склад = &Склад5000) КАК РНАдреса) КАК ТоварыАдресноеХранениеОстатки1
               |			ПО ТоварыАдресноеХранениеОстатки.АдресХранения = ТоварыАдресноеХранениеОстатки1.АдресХранения
               |		ПО ЗаказыПоставщикамОстатки.Номенклатура = ТоварыАдресноеХранениеОстатки.Номенклатура
               |
               |УПОРЯДОЧИТЬ ПО
               |	ТоварыАдресноеХранениеОстатки.АдресХранения.Порядок
               |ИТОГИ
...
Функция ПечатьВозвратаТоваровОтПокупателя()
	возврат неопределено;
КонецФункции
...
Функция ПечатьПеремещенияТоваров()
   ...
   Макет = ПолучитьМакет("Заказ");
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыАдресноеХранение.Остатки(,Склад = &Склад5000
...
функция Склад5000()
	...
	?(СсылкаНаОбъект.Подразделение=справочники.Подразделения.НайтиПоКоду("00106"), Справочники.Склады.НайтиПоКоду("00791"),
