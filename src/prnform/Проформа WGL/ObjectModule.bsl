Функция Печать(НомерДока = "") Экспорт
	...
	Если ЗначениеЗаполнено(СсылкаНаОбъект.ДоговорКонтрагента) и СсылкаНаОбъект.ДоговорКонтрагента.ТипДоговора <> Справочники.ТипыДоговоров.WheelsGroupLimited Тогда
		Сообщить("Печатная форма только для клиентов Wheels Group!!!", СтатусСообщения.Внимание);
...
Функция ПечатьДокумента(СсылкаНаОбъект2,сКартинкой=ложь)
	...
	ОрганизацияWG=Справочники.Организации.НайтиПоКоду("00005");
	
	
	
	форма = ПолучитьФорму("Форма1");
	форма.орг = ОрганизацияWG;
	
	БанкСчетWG =форма.ОткрытьМодально();
	
	Если БанкСчетWG = Неопределено Тогда 
		возврат ТабДокумент;
	КонецЕсли; 
	...
	СведенияОWG     = СведенияОЮрФизЛице(ОрганизацияWG,  СсылкаНаОбъект2.Дата,,БанкСчетWG);
	...
	ОбластьШапка.Параметры.Seller 		 = ВРег(СведенияОWG.ПолноеНаименование);
	ОбластьШапка.Параметры.SellerAdress  = СведенияОWG.ЮридическийАдрес;
	ОбластьШапка.Параметры.Bank			 = СведенияОWG.Банк;
	ОбластьШапка.Параметры.BankAdress	 = СведенияОWG.АдресБанка;
	ОбластьШапка.Параметры.AccountNumbers= СведенияОWG.НомерСчета;
	ОбластьШапка.Параметры.SWIFT		 = СведенияОWG.Банк.СвифтКод;
	ОбластьШапка.Параметры.iban		 	 = БанкСчетWG.ТекстКорреспондента;
	...
	ОбластьИтог.Параметры.СуммаПрописью = ЧислоПрописью(ТЗТоваров.Итог("Сумма"),"Л=en_US;ДП=истина");
