...
Функция Печать() Экспорт
	...
	ДокументРезультат = новый ТабличныйДокумент;
    Макет = ПолучитьМакет("МакетА");
	ДокументРезультат.Очистить();
	
	Область   = Макет.ПолучитьОбласть("ШапкаЗаголовок");
	Область.Параметры.Заполнить(СсылкаНаОбъект);
	область.Параметры.Поставщик = СсылканаОбъект.ДоговорКонтрагента.Поставщик;
	...
	|ГДЕ
	|	ПровереннаяНагрузкаДисков.Дата >= &Дата2Месяца
	|	И ПровереннаяНагрузкаДисков.Дубль = ЛОЖЬ
	|	И ПровереннаяНагрузкаДисков.Номенклатура В
	|			(ВЫБРАТЬ
	|				ВТ.Номенклатура
	|			ИЗ
	|				ВТ КАК ВТ)
	...
	|	ВЫБОР
	|		КОГДА Проверка.Номенклатура ЕСТЬ NULL 
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	|		ПО ВТ.Номенклатура = Проверка.Номенклатура
	|ИТОГИ ПО
	|	НаПроверку";
	
	Запрос.УстановитьПараметр("Дата2Месяца",Добавитьмесяц(СсылканаОбъект.Дата,-2));
	Запрос.УстановитьПараметр("Таб", СсылкаНаОбъект.Товары);
	Результат = Запрос.Выполнить();
	
	ВыборкаНаПроверку = Результат.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	
	Пока ВыборкаНаПроверку.Следующий() Цикл
		Если    ВыборкаНаПроверку.НаПроверку = Истина тогда
			ОбластьЗаголовок   = Макет.ПолучитьОбласть("ШапкаПроверка");
		иначе
			ОбластьЗаголовок   = Макет.ПолучитьОбласть("ШапкаПартии");
		конецЕсли;
		ДокументРезультат.Вывести(ОбластьЗаголовок);
		ОбластьШапка   = Макет.ПолучитьОбласть("ШапкаТаблица");
		ДокументРезультат.Вывести(ОбластьШапка);
		ОбластьСтрока   = Макет.ПолучитьОбласть("Строка");
		
		ВыборкаДетальныеЗаписи = ВыборкаНаПроверку.Выбрать();
