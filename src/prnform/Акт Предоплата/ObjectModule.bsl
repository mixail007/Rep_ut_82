...
функция ПолучитьМассивДляЗамены()
	...
	//ИтогоКоличество   = Цел(ИтогоКоличество/100)*100;   //обрезание до 100-ен - 30.01.2018
   ...
   Если ТекПокупатель.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо Тогда
	...
	//=============================================================
		...
		рез.Вставить("[Сумма]",Строка(Формат(СуммаУтвержденная * 1.18,"ЧДЦ=0"))); //+++ 30.01.2018  
		рез.Вставить("[СуммаБезНДС]",Строка(Формат(СуммаУтвержденная,"ЧДЦ=2")));  // (НДС включён в стоимость)
