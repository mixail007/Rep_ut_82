...
Функция ПечатьДокумента(СтруктураШапки,ДокументОприходования,КоличествоКопий=1,НаПринтер=Истина);
	...
	| Док.Код Код,
	| Док.ЕдиницаИзмерения ЕдиницаИзмерения,
	| Док.КодЕдиницы КодЕдиницы,
	| Док.Количество Количество,
	| ЕстьNULL(ТоварыНаОтветственномХранении.Цена,1) Цена,
	| Док.Количество*ЕстьNULL(ТоварыНаОтветственномХранении.Цена,1) Сумма ИЗ
	|(Выбрать
	...
	|//			1 Как Цена,
	|//			1 * Количество как Сумма
	|			ИЗ Документ.ОперацияПоОтветственномуХранению.Товары 
	|			Где Ссылка = &Ссылка) как Док
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|	(ВЫБРАТЬ
	|	Номенклатура,
	|	Склад,
	|	ВЫБОР КОГДА Количество>0 ТОГДА
	|	ВЫРАЗИТЬ (Стоимость/Количество КАК ЧИСЛО (15,2))
	|	ИНАЧЕ
	|	0
	|	КОНЕЦ Цена,
	|	Стоимость	Сумма
