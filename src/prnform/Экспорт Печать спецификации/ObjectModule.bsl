...
Функция ПечатьДокумента(СсылкаНаОбъект)
	...
	Если СсылкаНаОбъект.ДокументОснование.Контрагент.ДвуязычныйЭкспорт Тогда
		ДваЯзыка = Истина;
	...
	Если НомерНаПечать = "" Тогда
		...
		Иначе
			Если Найти(СсылкаНаОбъект.Номер, "/") > 0 Тогда 
				//это повторная спецификация
				i = 0;
				Пока i < СтрДлина(СсылкаНаОбъект.Номер) Цикл 
					i = i + 1;
					Если Прав(Лев(СсылкаНаОбъект.Номер,i),1) = "0" Тогда
						Продолжить;
					Иначе
						НомерДокумента = Прав(СсылкаНаОбъект.Номер, СтрДлина(СсылкаНаОбъект.Номер) - (i-1)); 
						Прервать;
					КонецЕсли;
				КонецЦикла;
			Иначе
	...
	//ОбШтрихКод=ОбластьЗаголовок.Рисунки.Штрихкод.Объект;
	...
	Покупатель  = СсылкаНаОбъект.ДокументОснование.Контрагент;
	СведенияОПокупателе     = СведенияОЮрФизЛице(Покупатель,  СсылкаНаОбъект.Дата);
    НаимКонтракта = "между ООО ""ШинТрейд Ярославль"" и "+СведенияОПокупателе.ПолноеНаименование;
	Если ДваЯзыка Тогда
		НаимКонтракта = "between ShinTrade Yaroslavl Ltd and "+СведенияОПокупателе.ПолноеНаименование+Символы.ПС+НаимКонтракта;
	КонецЕсли;
	...
	//Шапка
	ВалютаДок = СсылкаНаОбъект.ДокументОснование.ВалютаДокумента;
	Если ДваЯзыка Тогда
	...
	Для Каждого СтрокаЗаголовок Из ТЗГруппТоваров1 Цикл	
		...
		Если СсылкаНаОбъект.ДоговорКонтрагента.ВалютаВзаиморасчетов = Справочники.Валюты.НайтиПоКоду("840") Тогда
			...
			ОбластьСтрокаИтог.Параметры.СтоимостьСтрока = ТЗИтогСтрока.Итог("Сумма");
	...
	Если СокрЛП(СсылкаНаОбъект.ДокументОснование.Сделка.АдресДоставки)= "" Тогда
	...
	Иначе
		ФактАдресГП = СокрЛП(СсылкаНаОбъект.ДокументОснование.Сделка.АдресДоставки);
	КонецЕсли;
	...
	ОбластьПодвал.Параметры.ДатаОплаты = Формат(СсылкаНаОбъект.ДокументОснование.Сделка.ДатаОплаты,"ДФ=dd.MM.yyyy");
	...
	поз = Найти(Города,"/");
	ОбластьПодвал.Параметры.УсловиеПоставки =""+ СсылкаНаОбъект.ДокументОснование.Сделка.УсловиеПоставкиНаЭкспорт+" "+Сред(Города,поз+1) ;
	попытка
	ОбластьПодвал.Параметры.УсловиеПоставкиАнгл =""+ СсылкаНаОбъект.ДокументОснование.Сделка.УсловиеПоставкиНаЭкспорт+" "+ Лев(Города,поз-1);
	Исключение
    конецпопытки;
