...
Функция Печать(НомерДокЛок = Неопределено) Экспорт
	...
	иначе
		...
		Если (СсылкаНаОбъект.Подразделение = справочники.Подразделения.НайтиПоКоду("00107")
			...
			Или СсылкаНаОбъект.Дата < Дата("20130401000000") )
			и СсылкаНаОбъект.ДоговорКонтрагента.ТипДоговора <> Справочники.ТипыДоговоров.ШинтрейдЯрославль Тогда
...
Если Найти(СсылкаНаОбъект.ДоговорКонтрагента.Наименование,"*")=0 тогда//16-24.04.2018
		...
		Если СокрЛП(Шапка.АдресДоставки) <> "" Тогда
			Если Найти(СсылкаНаОбъект.ДоговорКонтрагента.Наименование,"*")>0 тогда//16.04.2018
				ПредставлениеГрузополучателяДоАдреса    = ОписаниеОрганизации(СведенияОГрузополучателе, "ПолноеНаименование,");
		...
		Иначе
			Если Найти(СсылкаНаОбъект.ДоговорКонтрагента.Наименование,"*")>0 тогда//16.04.2018
			ОбластьМакета.Параметры.ПредставлениеГрузополучателя = ОписаниеОрганизации(СведенияОГрузополучателе, "ПолноеНаименование,");
			иначе
		...
		Если СсылкаНаОбъект.ДоговорКонтрагента.ТипДоговора = Справочники.ТипыДоговоров.НайтиПоКоду("Я0004") Тогда //ФормулаАвто
			...
			Если Результат.Количество() > 0 Тогда 
				сведенияФА.ПолноеНаименование = сведенияФА.ПолноеНаименование + " , л/с " + Результат.ЛС;
			КонецЕсли;
		...
		ИначеЕсли СсылкаНаОбъект.ДоговорКонтрагента.ТипДоговора = Справочники.ТипыДоговоров.ФормулаАвтоПлюс Тогда //ФормулаАвто
			...
			Если Результат.Количество()>0 Тогда
				сведенияФАПлюс.ПолноеНаименование = сведенияФАПлюс.ПолноеНаименование + " , л/с " + Результат[0].ЛС;
			КонецЕсли;
			Если Найти(СсылкаНаОбъект.ДоговорКонтрагента.Наименование,"*")>0 тогда//16.04.2018
		...
		ИначеЕсли СсылкаНаОбъект.ДоговорКонтрагента.ТипДоговора = Справочники.ТипыДоговоров.ШинтрейдЯрославль Тогда //Шинтрейд Ярославль
			...
			Иначе
				...
				иначе
					Если СсылкаНаОбъект.Дата>=Дата(2018,05,01)  тогда
						Областьмакета.Параметры.ПредставлениеПоставщика = "Общество с ограниченной ответственностью ""ШинТрейд Ярославль"", ИНН 7604090169, 150014, Ярославская обл, Ярославль г, Салтыкова-Щедрина ул, дом № 21, офис 507, р/с 40702810477030004491, в банке Калужское отделение № 8608 ПАО Сбербанк, БИК 042908612, к/с 30101810100000000612";
					иначе
						Областьмакета.Параметры.ПредставлениеОрганизации = СтрЗаменить(Областьмакета.Параметры.ПредставлениеОрганизации,"Калужское отделение № 8608 ПАО Сбербанк, БИК 042908612, к/с 30101810100000000612","СЕВЕРНЫЙ БАНК СБЕРБАНКА РОССИИ, БИК 047888670, к/с 30101810500000000670");
