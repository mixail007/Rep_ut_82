...
функция ПолучитьМассивДляЗамены()// для Установки Бонусов
   ...
   Если ТекПокупатель.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо Тогда
	...
	КонецЕсли;
		...
		СтрокаСтавокБонуса1 = "";
		СтрокаСтавокБонуса2 = "";
		//Если СтрДлина(СтрокаСтавокБонуса) > 250 Тогда
		//	СтрокаСтавокБонуса1 = Лев(СтрокаСтавокБонуса,250);
		//	СтрокаСтавокБонуса2 = Сред(СтрокаСтавокБонуса,251,СтрДлина(СтрокаСтавокБонуса)-250);	
		//иначе
...
Функция Печать() Экспорт
	...
	//+++ 
	Если СсылкаНаОбъект.КоличествоПлан=0 тогда
		#Если Клиент тогда
		Предупреждение("Соглашение по Лояльности нужно заполнить плановое количество
					   |или создать и рассчитать Выплату бонусов Лояльности!",60);
		#КонецЕсли
	...
	массивДляЗамены	= ПолучитьМассивДляЗамены(); 
		...
		//Сообщить("Идёт заполнение колонтитула Word...");
