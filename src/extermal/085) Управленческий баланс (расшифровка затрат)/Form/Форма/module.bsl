...
Перем ТекстЗапросаД;
Перем ТекстЗапросаНаценкаНаТовар;
Перем ТекстЗапросаПени;
Перем ТекстЗапросаДоговорД;
Перем ТекстЗапросаПрочиеУслуги;
...
Процедура ЗаполнитьНачальныеНастройки()	
	...
	ЗАполнитьТЕкстЗАпросаД();
	ЗаполнитьТекстЗапросаНаценкаНаТовар();
	ЗаполнитьТекстЗапросаПени();
	ЗаполнитьТекстЗапросаДоговорД();
	ЗаполнитьТекстЗапросаПрочиеУслуги();
...
Процедура ЗаполнитьТекстЗапросаД()
	...
	Если Не ПоВсем Тогда 	
		...
		|	Из 
		|	РегистрНакопления.ВзаиморасчетыСКонтрагентами.Остатки(&ДатаКон,ДоговорКонтрагента.ТипДоговора В (&ДоговорД) и ДоговорКонтрагента.Организация = &Организация)
		|) как ПЗ";
	Иначе 
		ТекстЗапросаД = "
		|Выбрать Сумма(СуммаВзаиморасчетовОстаток) Как Сумма Из
		...
		|	Выбрать ДоговорКонтрагента, СуммаВзаиморасчетовОстаток 
		|	Из 
		|	РегистрНакопления.ВзаиморасчетыСКонтрагентами.Остатки(&ДатаКон,ДоговорКонтрагента.ТипДоговора В (&ДоговорД))
		|) как ПЗ";		
    КонецЕсли;
...
Процедура ЗаполнитьТекстЗапросаНаценкаНаТовар()
Если Не ПоВсем Тогда
	ТекстЗапросаНаценкаНаТовар =
		"ВЫБРАТЬ
		|	СУММА(ПЗП.Стоимость) КАК Стоимость,
		|	СУММА(ПЗП.ВаловаяПрибыль) КАК ВП
		|ИЗ
		...
		|			Продажи.Период МЕЖДУ &ДатаНач И &ДатаКон
		|			И Продажи.Регистратор.Организация = &Организация
		|		    И Продажи.Номенклатура.Услуга=Ложь
		|		СГРУППИРОВАТЬ ПО
		|			Продажи.Регистратор) КАК ТаблицаРегистра
		|			ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|				ПродажиСебестоимость.Регистратор КАК Регистратор,
...
Процедура ЗаполнитьТекстЗапросаПени()
Если Не ПоВсем Тогда
	ТекстЗапросаПени =
		"ВЫБРАТЬ
		|	СУММА(ПЗП.Стоимость) КАК Стоимость,
		...
		|			Продажи.Период МЕЖДУ &ДатаНач И &ДатаКон
		|			И Продажи.Регистратор.Организация = &Организация
		|		    И Продажи.Регистратор.ДоговорКонтрагента.ТипДоговора = &Пени
		|		    И Продажи.Номенклатура.Услуга=Истина
		|		СГРУППИРОВАТЬ ПО
		|			Продажи.Регистратор) КАК ТаблицаРегистра
		|			ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|				ПродажиСебестоимость.Регистратор КАК Регистратор,
...
Процедура ЗаполнитьТекстЗапросаДоговорД()
Если Не ПоВсем Тогда
	ТекстЗапросаДоговорД =
		"ВЫБРАТЬ
		|	СУММА(ПЗП.Стоимость) КАК Стоимость,
		|	СУММА(ПЗП.ВаловаяПрибыль) КАК ВП
		|ИЗ
		...
		|			Продажи.Период МЕЖДУ &ДатаНач И &ДатаКон
		|			И Продажи.Регистратор.Организация = &Организация
		|		    И Продажи.Регистратор.ДоговорКонтрагента.ТипДоговора = &ДоговорД
		//|		    И Продажи.Номенклатура.Услуга=Истина
		|		СГРУППИРОВАТЬ ПО
		|			Продажи.Регистратор) КАК ТаблицаРегистра
		|			ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
...
Процедура ЗаполнитьТекстЗапросаПрочиеУслуги()
Если Не ПоВсем Тогда
	ТекстЗапросаПрочиеУслуги =
		"ВЫБРАТЬ
		|	СУММА(ПЗП.Стоимость) КАК Стоимость,
		|	СУММА(ПЗП.ВаловаяПрибыль) КАК ВП
		|ИЗ
		...
		|			И Продажи.Номенклатура.Услуга = ИСТИНА
		|			И Продажи.Регистратор.Организация = &Организация
		|			И Продажи.Регистратор.ДоговорКонтрагента.ТипДоговора не в (&СписокДоговоров)
		|		
		|		СГРУППИРОВАТЬ ПО
		|			Продажи.Регистратор) КАК ТаблицаРегистра
		|			ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		...
		|			ГДЕ
		|				ПродажиСебестоимость.Период МЕЖДУ &ДатаНач И &ДатаКон
		|				И ПродажиСебестоимость.Номенклатура.Услуга = ИСТИНА
		|				И ПродажиСебестоимость.Регистратор.Организация = &Организация
		|				И ПродажиСебестоимость.Регистратор.ДоговорКонтрагента.ТипДоговора не в (&СписокДоговоров)
		|			
		|			СГРУППИРОВАТЬ ПО
		|				ПродажиСебестоимость.Регистратор) КАК ТаблицаРегистраСебестоимость
		|			ПО ТаблицаРегистра.Регистратор = ТаблицаРегистраСебестоимость.Регистратор) КАК ПЗП";
...
Процедура КоманднаяПанельФормыОбновить(Кнопка)
	...
	ИнвестицииИКредит.Добавить(Справочники.ТипыДоговоров.Д);
	...
	ЗапросДоговорДН = новый Запрос;
	ЗапросДоговорДК = новый Запрос;
	ЗапросНаценкаНаТовар = новый Запрос;
	ЗапросПени = новый Запрос;
	ЗапросДоговорД = новый Запрос;
	ЗапросПрочиеУслуги = новый Запрос;
...
//	ЗапросСКН.УстановитьПараметр("ДатаНач",ВернутьмоментВремениСклад(ДатаНач));						
	...
	ДоговорД = новый СписокЗначений;
	ДоговорД.Добавить(Справочники.ТипыДоговоров.Д);
	...
	ЗапросДоговорДН.Текст = ТекстЗапросаД;
	ЗапросДоговорДН.УстановитьПараметр("ДатаКон",Новый Граница(ДатаНач, ВидГраницы.Включая));
	ЗапросДоговорДН.УстановитьПараметр("ДоговорД",ДоговорД);
	ЗапросДоговорДН.УстановитьПараметр("Организация",Организация);
	РезультатДоговорДН = ЗапросДоговорДН.Выполнить();
	ВыборкаДоговорДН = РезультатДоговорДН.Выбрать();
	
	ЗапросДоговорДК.Текст = ТекстЗапросаД;
	ЗапросДоговорДК.УстановитьПараметр("ДатаКон",Новый Граница(ДатаКон+1, ВидГраницы.Включая));
	ЗапросДоговорДК.УстановитьПараметр("ДоговорД",ДоговорД);
	ЗапросДоговорДК.УстановитьПараметр("Организация",Организация);
	РезультатДоговорДК = ЗапросДоговорДК.Выполнить();
	ВыборкаДоговорДК = РезультатДоговорДК.Выбрать();
	...
	ЗапросНаценкаНаТовар.Текст = ТекстЗапросаНаценкаНаТовар;
	ЗапросНаценкаНаТовар.УстановитьПараметр("ДатаНач",ДатаНач);
	ЗапросНаценкаНаТовар.УстановитьПараметр("ДатаКон",ДатаКон);
	ЗапросНаценкаНаТовар.УстановитьПараметр("Организация",Организация);
	РезультатНаценкаНаТовар = ЗапросНаценкаНаТовар.Выполнить();
	ВыборкаНаценкаНаТовар = РезультатНаценкаНаТовар.Выбрать();

	ЗапросПени.Текст = ТекстЗапросаПени;
	ЗапросПени.УстановитьПараметр("ДатаНач",ДатаНач);
	ЗапросПени.УстановитьПараметр("ДатаКон",ДатаКон);
	ЗапросПени.УстановитьПараметр("Организация",Организация);	
	ЗапросПени.УстановитьПараметр("Пени",Справочники.ТипыДоговоров.Пени);	
	РезультатПени = ЗапросПени.Выполнить();
	ВыборкаПени = РезультатПени.Выбрать();

	ЗапросДоговорД.Текст = ТекстЗапросаДоговорД;
	ЗапросДоговорД.УстановитьПараметр("ДатаНач",ДатаНач);
	ЗапросДоговорД.УстановитьПараметр("ДатаКон",ДатаКон);
	ЗапросДоговорД.УстановитьПараметр("Организация",Организация);	
	ЗапросДоговорД.УстановитьПараметр("ДоговорД",Справочники.ТипыДоговоров.Д);	
	РезультатДоговорД = ЗапросДоговорД.Выполнить();
	ВыборкаДоговорД = РезультатДоговорД.Выбрать();

	ЗапросДоговорД.Текст = ТекстЗапросаДоговорД;
	ЗапросДоговорД.УстановитьПараметр("ДатаНач",ДатаНач);
	ЗапросДоговорД.УстановитьПараметр("ДатаКон",ДатаКон);
	ЗапросДоговорД.УстановитьПараметр("Организация",Организация);	
	ЗапросДоговорД.УстановитьПараметр("Пени",Справочники.ТипыДоговоров.Д);	
	РезультатДоговорД = ЗапросДоговорД.Выполнить();
	ВыборкаДоговорД = РезультатДоговорД.Выбрать();

	СписокДоговоровИзНаценки = новый СписокЗначений;
	СписокДоговоровИзНаценки.Добавить(Справочники.ТипыДоговоров.Д);
	СписокДоговоровИзНаценки.Добавить(Справочники.ТипыДоговоров.Пени);
	ЗапросПрочиеУслуги.Текст = ТекстЗапросаПрочиеУслуги;
	ЗапросПрочиеУслуги.УстановитьПараметр("ДатаНач",ДатаНач);
	ЗапросПрочиеУслуги.УстановитьПараметр("ДатаКон",ДатаКон);
	ЗапросПрочиеУслуги.УстановитьПараметр("Организация",Организация);	
	ЗапросПрочиеУслуги.УстановитьПараметр("СписокДоговоров",СписокДоговоровИзНаценки);	
	РезультатПрочиеУслуги = ЗапросПрочиеУслуги.Выполнить();
	ВыборкаПрочиеУслуги = РезультатПрочиеУслуги.Выбрать();
	...
	Если (ВыборкаДоговорДН.Следующий()) Тогда 
		СуммаДоговорДН = ?(ВыборкаДоговорДН.Сумма=неопределено,0,ВыборкаДоговорДН.Сумма);
	КонецЕсли;
	
	Если (ВыборкаДоговорДК.Следующий()) Тогда 
		СуммаДоговорДК = ?(ВыборкаДоговорДК.Сумма=неопределено,0,ВыборкаДоговорДК.Сумма);
	КонецЕсли;
	Область.Параметры.ДоговорДНач	= СуммаДоговорДН;
    Область.Параметры.ДоговорДКон	= СуммаДоговорДК;	
	...
	Область.Параметры.ДоговорДНачР = "ДоговорДН";
	Область.Параметры.ДоговорДКонР = "ДоговорДК";
	
	
	ТабДок.Вывести(Область);
	
	Область = Макет.ПолучитьОбласть("Итого");
	ИтогоНач1 = ?(СуммаДЗН=NULL,0,СуммаДЗН) - ?(СуммаКЗН=NULL,0,СуммаКЗН) + ?(СуммаДСН=NULL,0,СуммаДСН) + ?(СуммаСКН=NULL,0,СуммаСКН) + ?(СуммаПОН=NULL,0,СуммаПОН) + ?(СуммаИНВН=NULL,0,СуммаИНВН) - ?(СуммаКредН=NULL,0,СуммаКредН) + ?(СуммаОСН=null,0,СуммаОСН) - ?(СуммаОХН=null,0,СуммаОХН) + ?(СуммаДоговорДН=null,0,СуммаДоговорДН);
	ИтогоКон1 = ?(СуммаДЗК=NULL,0,СуммаДЗК) - ?(СуммаКЗК=NULL,0,СуммаКЗК) + ?(СуммаДСК=NULL,0,СуммаДСК) + ?(СуммаСКК=NULL,0,СуммаСКК) + ?(СуммаПОК=NULL,0,СуммаПОК) + ?(СуммаИНВК=NULL,0,СуммаИНВК) - ?(СуммаКредК=NULL,0,СуммаКредК) + ?(СуммаОСК=null,0,СуммаОСК) - ?(СуммаОХК=null,0,СуммаОХК) + ?(СуммаДоговорДК=null,0,СуммаДоговорДК);
	...
	СуммаНаценкаНаТовар = 0; 
	Если (ВыборкаНаценкаНаТовар.Следующий()) Тогда 
		СуммаНаценкаНаТовар = ВыборкаНаценкаНаТовар.ВП;
	КонецЕсли;
	
	СуммаПени = 0; 
	Если (ВыборкаПени.Следующий()) Тогда 
		СуммаПени = ВыборкаПени.ВП;
	КонецЕсли;
	
	СуммаД = 0; 
	Если (ВыборкаДоговорД.Следующий()) Тогда 
		СуммаД = ВыборкаДоговорД.ВП;
	КонецЕсли;
	
	СуммаПрочиеУслуги = 0; 
	Если (ВыборкаПрочиеУслуги.Следующий()) Тогда 
		СуммаПрочиеУслуги = ВыборкаПрочиеУслуги.ВП;
	КонецЕсли;
	...
	Область.Параметры.Наценка = ?(СуммаНаценкаНаТовар=null,0,СуммаНаценкаНаТовар);
	Область.Параметры.Пени = ?(СуммаПени=null,0,СуммаПени);
	Область.Параметры.ДоговорД = ?(СуммаД=null,0,СуммаД);
	Область.Параметры.ПрочиеУслуги = ?(СуммаПрочиеУслуги=null,0,СуммаПрочиеУслуги);
...
Процедура РезультатыОтчетаОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	ИначеЕсли (Расшифровка = "ДЗН") Тогда 	
		...
		СписокТипов.Добавить(Справочники.ТипыДоговоров.Д);
	...
	ИначеЕсли (Расшифровка = "ДоговорДН") Тогда 	
		// Отчет по ДЗН
		ЭлементСправочника = Справочники.ВнешниеОбработки.НайтиПоНаименованию("Задолженность по контрагентам");
		ХранилищеВО = ЭлементСправочника.ХранилищеВнешнейОбработки.Получить();
		//Попытка
	...
	ИначеЕсли (Расшифровка = "ДоговорДК") Тогда 	
		ЭлементСправочника = Справочники.ВнешниеОбработки.НайтиПоНаименованию("Задолженность по контрагентам");
		ХранилищеВО = ЭлементСправочника.ХранилищеВнешнейОбработки.Получить();
		//Попытка
		ИмяФайла = КаталогВременныхФайлов()+"PrnForm.tmp";
		...
		ОбработкаДЗ = ВнешниеОбработки.Создать(ИмяФайла);
		ОбработкаДЗ.ЗаполнитьНачальныеНастройки();
		ОбработкаДЗ.ОбщийОтчет.ДатаКон = ДатаКон-1;
		ОбработкаДЗ.ОбщийОтчет.ДатаНач = ДатаКон-1;
		ОбработкаДЗ.ПереключательЗадолженность = 3;
		Для Каждого СтрОтб из ОбработкаДЗ.ОбщийОтчет.ПостроительОтчета.Отбор Цикл
			СтрОтб.Использование = Ложь;	
		КонецЦикла;
	...
	ИначеЕсли (Расшифровка = "Наценка") Тогда 	
		// Отчет по ДЗН
		ЭлементСправочника = Справочники.ВнешниеОбработки.НайтиПоНаименованию("Валовая прибыль");
		ХранилищеВО = ЭлементСправочника.ХранилищеВнешнейОбработки.Получить();
		ИмяФайла = КаталогВременныхФайлов()+"PrnForm.tmp";
		...
		Для Каждого СтрОтб из ОбработкаДЗ.ОбщийОтчет.ПостроительОтчета.Отбор Цикл
			СтрОтб.Использование = Ложь;	
		КонецЦикла;
		СтрОтб = ОбработкаДЗ.ОбщийОтчет.ПостроительОтчета.Отбор.Добавить("Номенклатура.Услуга");
	...
	ИначеЕсли (Расшифровка = "Пени") Тогда 	
		// Отчет по ДЗН
		ЭлементСправочника = Справочники.ВнешниеОбработки.НайтиПоНаименованию("Валовая прибыль");
		ХранилищеВО = ЭлементСправочника.ХранилищеВнешнейОбработки.Получить();
		ИмяФайла = КаталогВременныхФайлов()+"PrnForm.tmp";
		...
		СтрОтб = ОбработкаДЗ.ОбщийОтчет.ПостроительОтчета.Отбор.Добавить("ДоговорКонтрагента.ТипДоговора");
		СтрОтб.ВидСравнения = ВидСравнения.Равно;
		СтрОтб.Значение = Справочники.ТипыДоговоров.Пени;
		СтрОтб.Использование = Истина;	
	...
	ИначеЕсли (Расшифровка = "ДоговорД") Тогда 	
		// Отчет по ДЗН
		ЭлементСправочника = Справочники.ВнешниеОбработки.НайтиПоНаименованию("Валовая прибыль");
		ХранилищеВО = ЭлементСправочника.ХранилищеВнешнейОбработки.Получить();
		ИмяФайла = КаталогВременныхФайлов()+"PrnForm.tmp";
		...
		СтрОтб = ОбработкаДЗ.ОбщийОтчет.ПостроительОтчета.Отбор.Добавить("ДоговорКонтрагента.ТипДоговора");
		СтрОтб.ВидСравнения = ВидСравнения.Равно;
		СтрОтб.Значение = Справочники.ТипыДоговоров.Д;
		СтрОтб.Использование = Истина;	
	...
	ИначеЕсли (Расшифровка = "ПрочиеУслуги") Тогда 	
		// Отчет по ДЗН
		ЭлементСправочника = Справочники.ВнешниеОбработки.НайтиПоНаименованию("Валовая прибыль");
		ХранилищеВО = ЭлементСправочника.ХранилищеВнешнейОбработки.Получить();
		ИмяФайла = КаталогВременныхФайлов()+"PrnForm.tmp";
		...
		СписокДогворовДляВП = новый СписокЗначений;
		СписокДогворовДляВП.Добавить(Справочники.ТипыДоговоров.Пени);
		СписокДогворовДляВП.Добавить(Справочники.ТипыДоговоров.Д);
		СтрОтб = ОбработкаДЗ.ОбщийОтчет.ПостроительОтчета.Отбор.Добавить("ДоговорКонтрагента.ТипДоговора");
		СтрОтб.ВидСравнения = ВидСравнения.НеВСписке;
		СтрОтб.Значение = СписокДогворовДляВП;
		СтрОтб.Использование = Истина;	
