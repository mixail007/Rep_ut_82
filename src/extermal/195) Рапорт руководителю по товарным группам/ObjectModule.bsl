...
Процедура СформироватьОтчет(СтрДействие, Параметры = 0) Экспорт
	...
	Если НадоОбработатьПоказатель(СтрДействие, "ТоварныеЗапасы", Параметры) Тогда
		ТоварныеЗапасы(СтрДействие, Параметры);
	КонецЕсли;
	
	Если НадоОбработатьПоказатель(СтрДействие, "ДеньгиВТоварныхГруппах", Параметры) Тогда
		ДеньгиВТоварныхГруппах(СтрДействие, Параметры);
	КонецЕсли;
...
Процедура Продажи_ПродажиИПоступленияИнициализация()
	...
	// Параметр "ШтукиРубли"
	НоваяСтрока = ТабПарам.Добавить();
	НоваяСтрока.Имя                    = "ЕдИзм";
	НоваяСтрока.ПредставлениеПараметра = "Единица измерения";
	НоваяСтрока.ТипЗначения            = СоздатьТип("СписокЗначений");
	
	НоваяСтрока.Список = Новый СписокЗначений;
	НоваяСтрока.Список.Добавить("Штуки");
	НоваяСтрока.Список.Добавить("Рубли");
...
Процедура Продажи_ПродажиИПоступленияФормирование()
	...
	Запрос.Текст = " ВЫБРАТЬ ТипПериода,  Период,  СуммаПродажи ИЗ
		...
		|	ИЗ
		|		РегистрНакопления.Продажи.Обороты(&ДатаНач, &ДатаКон, Месяц, НЕ Номенклатура.Услуга //ОтборТГ //ОтборМ
		|) КАК ПродажиОбороты 
		|	ОБЪЕДИНИТЬ
		...
		|		ПродажиОбороты.СтоимостьОборот КАК СуммаПродажи
		|	ИЗ
		|		РегистрНакопления.Продажи.Обороты(&НачПредПериода, &КонПредПериода, Месяц, НЕ Номенклатура.Услуга //ОтборТГ //ОтборМ
		|) КАК ПродажиОбороты 
		|  ОБЪЕДИНИТЬ
		...
		|		ПродажиОбороты.СтоимостьОборот КАК СуммаПродажи
		|	ИЗ
		|		РегистрНакопления.Продажи.Обороты(&НачАналогПериода, &КонАналогПериода, Месяц, НЕ Номенклатура.Услуга //ОтборТГ //ОтборМ
		|) КАК ПродажиОбороты )А
	...
	Если мСтруктураСохраненныеНастройки.Свойство("СписокТГ") Тогда
		Запрос.УстановитьПараметр("СписокТГ",мСтруктураСохраненныеНастройки.СписокТГ );
		//Запрос.Текст=СтрЗаменить(Запрос.Текст,"//ОтборТГ"," И Номенклатура.НоменклатурнаяГруппа В (&СписокТГ)");
		Если ТипЗнч(мСтруктураСохраненныеНастройки.СписокТГ[0].Значение)= Тип("СправочникСсылка.НоменклатурныеГруппы") тогда
			Запрос.Текст=СтрЗаменить(Запрос.Текст,"//ОтборТГ"," и Номенклатура.НоменклатурнаяГруппа В (&СписокТГ)");
		иначе
			Запрос.Текст=СтрЗаменить(Запрос.Текст,"//ОтборТГ"," и Номенклатура.ВидТовара  В (&СписокТГ)");
		КонецЕсли;
	КонецЕсли;
	
	Если мСтруктураСохраненныеНастройки.Свойство("СписокМенеджеров") И мСтруктураСохраненныеНастройки.СписокМенеджеров.Количество()>0 Тогда
		Запрос.УстановитьПараметр("СписокМенеджеров",мСтруктураСохраненныеНастройки
		.СписокМенеджеров );
		Запрос.Текст=СтрЗаменить(Запрос.Текст,"//ОтборМ"," И ДоговорКонтрагента.ОтветственноеЛицо В (&СписокМенеджеров)");
	КонецЕсли;


	Если ПолучитьПараметрПоказателя("Продажи_ПродажиИПоступления", "ЕдИзм")="Штуки" Тогда
		Запрос.Текст=СтрЗаменить(Запрос.Текст,"СтоимостьОборот","КоличествоОборот");
	КонецЕсли;	
...
Процедура Продажи_ОбъемОтгрузкиФормирование()
	...
	Запрос.Текст = "
		...
		|	ПродажиОбороты.Период КАК Период
		|ИЗ
		|	РегистрНакопления.Продажи.Обороты(&НачПериода, &КонПериода, " + Периодичность + ", Не Номенклатура.Услуга //ОтборТГ  //ОтборМ
		|		) КАК ПродажиОбороты";
	...
	Если ФлагПредПериод Тогда
		...
		Запрос.Текст = Запрос.Текст + "
			...
			|	ПродажиОбороты.Период
			|ИЗ
			|	РегистрНакопления.Продажи.Обороты(&НачПредПериода, &КонПредПериода, " + Периодичность + ", Не Номенклатура.Услуга //ОтборТГ //ОтборМ
			|) КАК ПродажиОбороты";
	КонецЕсли;
	Если ФлагАналогПериод Тогда
		...
		Запрос.Текст = Запрос.Текст + "
			...
			|	ПродажиОбороты.Период
			|ИЗ
			|	РегистрНакопления.Продажи.Обороты(&НачАналогПериода, &КонАналогПериода, " + Периодичность + ",Не Номенклатура.Услуга //ОтборТГ //ОтборМ
			|) КАК ПродажиОбороты";
	КонецЕсли;
...
.СписокМенеджеров );
	...
	Если ПолучитьПараметрПоказателя("Продажи_ОбъемОтгрузки", "ЕдИзм")="Штуки" Тогда
		Запрос.Текст=СтрЗаменить(Запрос.Текст,"СтоимостьОборот","КоличествоОборот");
	КонецЕсли;	
...
Процедура Продажи_ВаловаяПрибыльФормирование()
	...
	Запрос.Текст = "
		...
		|	ПродажиОбороты.Период КАК Период
		|ИЗ
		|	РегистрНакопления.Продажи.Обороты(&НачПериода, &КонПериода, " + Периодичность + ",//ОтборТГ //ОтборМ 
		|) КАК ПродажиОбороты
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ПродажиСебестоимость.Обороты(&НачПериода, &КонПериода, " + Периодичность + ", //ОтборТГ) КАК СебестоимостьПродажОбороты
	...
	Если ФлагПредПериод Тогда
		...
		Запрос.Текст = Запрос.Текст + "
			...
			|	ПродажиОбороты.Период
			|ИЗ
			|	РегистрНакопления.Продажи.Обороты(&НачПредПериода, &КонПредПериода, " + Периодичность + ", //ОтборТГ //ОтборМ 
			|) КАК ПродажиОбороты
			|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ПродажиСебестоимость.Обороты(&НачПредПериода, &КонПредПериода, " + Периодичность + ", //ОтборТГ) КАК СебестоимостьПродажОбороты
			|	ПО ПродажиОбороты.Номенклатура = СебестоимостьПродажОбороты.Номенклатура
			|	И ПродажиОбороты.ХарактеристикаНоменклатуры = СебестоимостьПродажОбороты.ХарактеристикаНоменклатуры
			|	И ПродажиОбороты.ЗаказПокупателя = СебестоимостьПродажОбороты.ЗаказПокупателя
			|	И ПродажиОбороты.Подразделение = СебестоимостьПродажОбороты.Подразделение
	...
	Если ФлагАналогПериод Тогда
		...
		Запрос.Текст = Запрос.Текст + "
			...
			|	ПродажиОбороты.Период
			|ИЗ
			|	РегистрНакопления.Продажи.Обороты(&НачАналогПериода, &КонАналогПериода, " + Периодичность + ", //ОтборТГ //ОтборМ 
			|) КАК ПродажиОбороты
			|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ПродажиСебестоимость.Обороты(&НачАналогПериода, &КонАналогПериода, " + Периодичность + ", //ОтборТГ) КАК СебестоимостьПродажОбороты
			|	ПО ПродажиОбороты.Номенклатура = СебестоимостьПродажОбороты.Номенклатура
			|	И ПродажиОбороты.ХарактеристикаНоменклатуры = СебестоимостьПродажОбороты.ХарактеристикаНоменклатуры
			|	И ПродажиОбороты.ЗаказПокупателя = СебестоимостьПродажОбороты.ЗаказПокупателя
			|	И ПродажиОбороты.Подразделение = СебестоимостьПродажОбороты.Подразделение
	...
	Если мСтруктураСохраненныеНастройки.Свойство("СписокТГ") Тогда
		...
		//Запрос.Текст=СтрЗаменить(Запрос.Текст,"//ОтборТГ"," И Номенклатура.НоменклатурнаяГруппа В (&СписокТГ)");
		Если ТипЗнч(мСтруктураСохраненныеНастройки.СписокТГ[0].Значение)= Тип("СправочникСсылка.НоменклатурныеГруппы") тогда
			Запрос.Текст=СтрЗаменить(Запрос.Текст,"//ОтборТГ","  Номенклатура.НоменклатурнаяГруппа В (&СписокТГ)");
		иначе
			Запрос.Текст=СтрЗаменить(Запрос.Текст,"//ОтборТГ","  Номенклатура.ВидТовара  В (&СписокТГ)");
		КонецЕсли;
	КонецЕсли;
	
		Если мСтруктураСохраненныеНастройки.Свойство("СписокМенеджеров") И мСтруктураСохраненныеНастройки.СписокМенеджеров.Количество()>0 Тогда
		Запрос.УстановитьПараметр("СписокМенеджеров",мСтруктураСохраненныеНастройки.СписокМенеджеров );
		Запрос.Текст=СтрЗаменить(Запрос.Текст,"//ОтборМ"," И ДоговорКонтрагента.ОтветственноеЛицо В (&СписокМенеджеров)");
	КонецЕсли;
...
Процедура СебестоимостьТоварныхЗапасовФормирование()
	...
	Пока счДатаНачалаПериода<ДатаКонцаПериода Цикл
		...
		|ИЗ
		|	РегистрНакопления.ПартииТоваровНаСкладах.Остатки(&НачПериода,//ОтборТГ )";
...
Процедура ДеньгиВТоварныхГруппах(СтрДействий, Параметры = 0)

	Если СтрДействий = "Инициализация" Тогда
		
		ДеньгиВТоварныхГруппахИнициализация();
		
	ИначеЕсли СтрДействий = "Формирование" Тогда
		
		ДеньгиВТоварныхГруппахФормирование();
	Иначе // Данное действие не обрабатывается
...
Процедура ДеньгиВТоварныхГруппахИнициализация()
	
	ДобавитьВДеревоПоказателей("ДеньгиВТоварныхГруппах", "Деньги в товарных группах", "");
	...
	НоваяСтрока.Значение = НоваяСтрока.Список[3].Значение;
	...
	// Считать сохранные значения
	ПолучитьСохраненныеЗначения("ДеньгиВТоварныхГруппах", ТабПарам);

	// Добавить данные в структуру параметров показателей
	СтруктураПараметровПоказателей.Вставить("ДеньгиВТоварныхГруппах", ТабПарам);
	
КонецПроцедуры // ДТГИнициализация()


Процедура ДеньгиВТоварныхГруппахФормирование()
	
	Периодичность = ПолучитьПараметрПоказателя( "ДеньгиВТоварныхГруппах", "Периодичность");
	ПериодАнализа = ПолучитьПараметрПоказателя( "ДеньгиВТоварныхГруппах", "ПериодАнализа");
	АналогПериод  = ПолучитьПараметрПоказателя( "ДеньгиВТоварныхГруппах", "АналогПериод");
	
	// Проверим корректность значений параметров показателей
	Если СтруктураПараметровПоказателей.Свойство("ДеньгиВТоварныхГруппах") Тогда
		
		ТабПарам = СтруктураПараметровПоказателей["ДеньгиВТоварныхГруппах"];
		Строка   = ТабПарам.Найти("Периодичность", "Имя");
		Список1  = Строка.Список;
		Строка   = ТабПарам.Найти("ПериодАнализа", "Имя");
		Список2  = Строка.Список;
		...
		Если Поз1 > Поз2 Тогда
			Сообщить("Показатель ""Деньги в товарных группах"" значение периодичности не может превышать период анализа", СтатусСообщения.БезСтатуса);
			Сообщить("Показатель не сформирован!", СтатусСообщения.Внимание);
			Возврат;
		КонецЕсли;
	...
	// Расчет дат предыдущего периода
	ФлагПредПериод = ПолучитьПараметрПоказателя( "ДеньгиВТоварныхГруппах", "ПредПериод") = "Да";
	Если ФлагПредПериод Тогда
		НачалоПредПериода = 0;
		КонецПредПериода  = 0;
		СмещениеПредИнтервала = ПрошлыйПериодПоНеделям(
		...
		// Расчет дат аналог периода
	ФлагАналогПериод = Не ПолучитьПараметрПоказателя( "ДеньгиВТоварныхГруппах", "АналогПериод") = "Не выводить";
	//НачалоАналогПериода = ДобавитьМесяц( ДатаНачалаПериода, -24);
	Если ФлагАналогПериод Тогда
		НачалоАналогПериода = 0;
		КонецАналогПериода  = 0;
	...
	Пока счДатаНачалаПериода<ДатаКонцаПериода Цикл
		...
		Запрос.Текст="ВЫБРАТЬ
		             ...
		             |ИЗ
		             |	(ВЫБРАТЬ
		             |		ВзаимозачетНоменклатурныеГруппы.НоменклатурнаяГруппаПриход КАК НоменклатурнаяГруппа,
		             |		ВзаимозачетНоменклатурныеГруппы.ПроизводительПриход КАК Производитель,
		             |		СУММА(ВзаимозачетНоменклатурныеГруппы.СуммаДокумента) КАК Сумма
		             |	ИЗ
		             |		Документ.ВзаимозачетНоменклатурныеГруппы КАК ВзаимозачетНоменклатурныеГруппы
		             |	ГДЕ
		             |		ВзаимозачетНоменклатурныеГруппы.Проведен
		             |		И ВзаимозачетНоменклатурныеГруппы.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидОбменаМеждуТоварнымиГруппами.ОбменТоварами)
		             |		И ВзаимозачетНоменклатурныеГруппы.ДатаНачала <= &КонДата
		             |		И (ВзаимозачетНоменклатурныеГруппы.ДатаОкончания = ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
		             |				ИЛИ ВзаимозачетНоменклатурныеГруппы.ДатаОкончания >= &КонДата)
		             |	
		             |	СГРУППИРОВАТЬ ПО
		             |		ВзаимозачетНоменклатурныеГруппы.НоменклатурнаяГруппаПриход,
		             |		ВзаимозачетНоменклатурныеГруппы.ПроизводительПриход
		             |	
		             |	ОБЪЕДИНИТЬ
		             |	
		             |	ВЫБРАТЬ
		             |		ВзаимозачетНоменклатурныеГруппы.НоменклатурнаяГруппаРасход,
		             |		ВзаимозачетНоменклатурныеГруппы.ПроизводительРасход,
		             |		-СУММА(ВзаимозачетНоменклатурныеГруппы.СуммаДокумента)
		             |	ИЗ
		             |		Документ.ВзаимозачетНоменклатурныеГруппы КАК ВзаимозачетНоменклатурныеГруппы
		             |	ГДЕ
		             |		ВзаимозачетНоменклатурныеГруппы.Проведен
		             ...
		             |	СГРУППИРОВАТЬ ПО
		             |		ВзаимозачетНоменклатурныеГруппы.НоменклатурнаяГруппаРасход,
		             |		ВзаимозачетНоменклатурныеГруппы.ПроизводительРасход) КАК А
		             |;
		             |
		             |////////////////////////////////////////////////////////////////////////////////
		             |ВЫБРАТЬ
		             |	&КонДата КАК Период,
		             |	0 КАК ТипПериода,
		             |	СУММА(ЕСТЬNULL(Б.Сумма, 0)) КАК Долг
		             |ИЗ
		             |	(ВЫБРАТЬ
		             |		ДТГ.Вид КАК Вид,
		             ...
		             |	ВЫБРАТЬ
		             |		ВТ_ОбменМеждуТГ.Вид,
		             |		ВТ_ОбменМеждуТГ.НоменклатурнаяГруппа,
		             |		ВТ_ОбменМеждуТГ.Производитель,
		             |		ВТ_ОбменМеждуТГ.Контрагент,
		             |		ВТ_ОбменМеждуТГ.ДоговорКонтрагента,
		             |		ВТ_ОбменМеждуТГ.Сделка,
		             |		ВТ_ОбменМеждуТГ.Сумма,
		             |		""""
		             |	ИЗ
		             |		ВТ_ОбменМеждуТГ КАК ВТ_ОбменМеждуТГ
		             |	
		             |	ОБЪЕДИНИТЬ
		             |	
		             |	ВЫБРАТЬ
		             ...
		             |						И ДоговорКонтрагента.НоменклатурнаяГруппа <> &ПустаяНГ) КАК ВзаиморасчетыУслуги
		             |		
		             |		СГРУППИРОВАТЬ ПО
		             |			ВзаиморасчетыУслуги.ДоговорКонтрагента.НоменклатурнаяГруппа,
		             |			ВзаиморасчетыУслуги.ДоговорКонтрагента.Владелец,
		             |			ВзаиморасчетыУслуги.ДоговорКонтрагента) КАК ВзаиморасчетыУслуги) КАК Б  //ОтборТГ  ";
		
		//Если мСтруктураСохраненныеНастройки.Свойство("СписокТГ") Тогда
		//	Запрос.УстановитьПараметр("СписокТГ",мСтруктураСохраненныеНастройки.СписокТГ );
		//	Запрос.Текст=СтрЗаменить(Запрос.Текст,"//ОтборТГ"," ГДЕ НоменклатурнаяГруппа В (&СписокТГ)");
		//КонецЕсли;
		Если мСтруктураСохраненныеНастройки.Свойство("СписокТГ") Тогда
			Запрос.УстановитьПараметр("СписокТГ",мСтруктураСохраненныеНастройки.СписокТГ );
			//Запрос.Текст=СтрЗаменить(Запрос.Текст,"//ОтборТГ"," И Номенклатура.НоменклатурнаяГруппа В (&СписокТГ)");
			Если ТипЗнч(мСтруктураСохраненныеНастройки.СписокТГ[0].Значение)= Тип("СправочникСсылка.НоменклатурныеГруппы") тогда
				Запрос.Текст=СтрЗаменить(Запрос.Текст,"//ОтборТГ","  где НоменклатурнаяГруппа В (&СписокТГ)");
			иначе
				Запрос.Текст=СтрЗаменить(Запрос.Текст,"//ОтборТГ","  где ВидТовара  В (&СписокТГ)");
			КонецЕсли;
		КонецЕсли;
	
		Запрос.УстановитьПараметр("КонДата",счДатаНачалаПериода);
	...
	КонецЦикла;	
		...
		Если ФлагПредПериод Тогда
			...
			Пока счДатаНачалаПериода<КонецПредПериода Цикл
				...
				НоваяСтрока = ТабДанных.Добавить();
				//НоваяСтрока.Дата      = НачалоДня( Выборка.Период + СмещениеПредИнтервала);
				НоваяСтрока.Дата      = НачалоМесяца( Выборка.Период + СмещениеПредИнтервала);
				Если Выборка.Период< КонецМесяца(ДатаКон)+2 Тогда
					НоваяСтрока.ПредПериод = Выборка.Долг;
				Иначе
					НоваяСтрока.ПредПериод = 0;
				КонецЕсли;
			КонецЦикла;	
		КонецЕсли;
	//Аналог период
	Если ФлагАналогПериод Тогда
		...
		Пока счДатаНачалаПериода<КонецАналогПериода Цикл
			...
			НоваяСтрока = ТабДанных.Добавить();
			//НоваяСтрока.Дата      = НачалоДня( Выборка.Период + СмещениеАналогИнтервала);
			НоваяСтрока.Дата      = НачалоМесяца( Выборка.Период + СмещениеАналогИнтервала);

			Если Выборка.Период< КонецМесяца(ДатаКон)+2 Тогда
				НоваяСтрока.АналогПериод = Выборка.Долг;
			Иначе
				НоваяСтрока.АналогПериод = 0;
			КонецЕсли;
		КонецЦикла;	
	КонецЕсли;
	...
	Если НачалоДня(ДатаНачалаПериода) = НачалоДня(ДатаКонцаПериода) Тогда
		СтрПериод = "Деньги в товарных группах за " + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	Иначе
		СтрПериод = "Деньги в товарных группах за период: " + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	КонецЕсли;
	...
	ТабДок.Вывести(Секция);
	мСтруктТабДок.Вставить("ДеньгиВТоварныхГруппах", ТабДок);
	
КонецПроцедуры // ДеньгиВТоварныхГруппахФормирование()

//ТоварныеЗапасыГрафик
Процедура ТоварныеЗапасы(СтрДействий, Параметры = 0)

	Если СтрДействий = "Инициализация" Тогда
		
		ТоварныеЗапасыИнициализация();
		
	ИначеЕсли СтрДействий = "Формирование" Тогда
		
		ТоварныеЗапасыФормирование();
...
Процедура ТоварныеЗапасыИнициализация()
	
	ДобавитьВДеревоПоказателей("ТоварныеЗапасы", "Товарные запасы (в ед.изм. по месяцам)", "");
	...
	//
	// Считать сохранные значения
	ПолучитьСохраненныеЗначения("ТоварныеЗапасы", ТабПарам);

	// Добавить данные в структуру параметров показателей
	СтруктураПараметровПоказателей.Вставить("ТоварныеЗапасы", ТабПарам);
	
КонецПроцедуры 

Процедура ТоварныеЗапасыФормирование()

	ТабДок = Новый ТабличныйДокумент;
	ДатаНач = ПолучитьДатуНачалаПериода(ДатаКон,
		ПолучитьПараметрПоказателя("ТоварныеЗапасы", "ПериодАнализа"));
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   //|			И НЕ Склад.Транзитный 
	               |					//ОтборТГ
				   |) КАК ТоварыНаСкладахОстаткиИОбороты) КАК А
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	А.ТипПериода,
	               |	А.Период" ;
	...
	ДатаНач=НачалоГода(ДатаКон);
	Запрос.УстановитьПараметр("ДатаНач", ДатаНач);
	Запрос.УстановитьПараметр("ДатаКон", КонецГода(ДатаКон)+1 );
	...
	Запрос.УстановитьПараметр("НачПредПериода", НачПредПериода);
	Запрос.УстановитьПараметр("КонПредПериода", КонПредПериода+1);
	
	НачАналогПериода=НачалоГода(НачПредПериода-1);
	КонАналогПериода=КонецГода(НачАналогПериода);
	
	Запрос.УстановитьПараметр("НачАналогПериода", НачАналогПериода);
	Запрос.УстановитьПараметр("КонАналогПериода", КонАналогПериода+1);
	...
	Пока Обход.Следующий() Цикл
		...
		Если Обход.ТипПериода = 0 Тогда // Текущий период
			...
			НоваяСтрока.Дата      = Обход.Период;
			НоваяСтрока.ТекПериод = Обход.Количество;
		...
		ИначеЕсли Обход.ТипПериода = 1 Тогда // Предыдущий период
			...
			НоваяСтрока.Дата       = ДобавитьМесяц( Обход.Период,12);
			//НоваяСтрока.Дата       = НачалоДня( Обход.Период + СмещениеПредИнтревала);
			НоваяСтрока.ПредПериод = Обход.Количество;
			
		ИначеЕсли Обход.ТипПериода = 2 Тогда // Аналогичной период
			СмещениеАналогИнтервала =  ДатаНач-НачАналогПериода;
			НоваяСтрока.Дата       = ДобавитьМесяц( Обход.Период,24);
			//НоваяСтрока.Дата         = НачалоДня( Обход.Период + СмещениеАналогИнтервала);
			НоваяСтрока.АналогПериод = Обход.Количество;
	...
	Для каждого строка ИЗ ТабДанных Цикл
		...
		ИтогПериод=строка.ТекПериод;//ИтогПериод+строка.ТекПериод;
		ИтогПредПериод=строка.ПредПериод;//ИтогПредПериод+строка.ПредПериод;
		ИтогАналогПериод=строка.АналогПериод;//ИтогАналогПериод+строка.АналогПериод;
	...
	Макет  = мМакет.ПолучитьОбласть("ПродажиИПоступления");
	Секция = Макет.ПолучитьОбласть("ПродажиИПоступления");
	Секция.Параметры.Заголовок = "Товарные запасы";
	Диаграмма = Секция.Рисунки.ГрафикМесяц.Объект;
	Диаграмма.Очистить();
	...
	Для Каждого Строка Из ТабДанных Цикл
		...
		НомСерии = 0;
		Диаграмма.УстановитьЗначение(НомСтроки, НомСерии, ?(Строка.ТекПериодНакоп = 0,ИтогПериод,Строка.ТекПериодНакоп));    //кривая застывает на текущем уровне до конца года
		НомСерии = НомСерии + 1;
		Диаграмма.УстановитьЗначение(НомСтроки, НомСерии, Строка.ПредПериодНакоп);
		НомСерии = НомСерии + 1;
		Диаграмма.УстановитьЗначение(НомСтроки, НомСерии, Строка.АналогПериодНакоп);
	...
	//ТабДок.Вывести(Секция);
	мСтруктТабДок.Вставить("ТоварныеЗапасы", ТабДок);
