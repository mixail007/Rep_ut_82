...
Процедура ПриЗакрытии()
	...
	СохранитьЗначение("НастройкаВнешниеОтчетыОтчетПоБюджетуЗатратФактОтчет_32368574-c0b2-44fe-8096-f904eb4dbece", ПостроительОтчетаОтчет.ПолучитьНастройки());
...
Процедура ОтчетВывести(ТДок=неопределено,ДатаНач=Неопределено,ДатаКон=Неопределено) Экспорт 
	...
	ПостроительОтчетаОтчет.Параметры.Вставить("дата1", НачалоМесяца(НачПериода));
	ПостроительОтчетаОтчет.Параметры.Вставить("дата11", КонецМесяца(НачПериода));
	
	ПостроительОтчетаОтчет.Параметры.Вставить("дата2", НачалоМесяца(ДобавитьМесяц(НачПериода,1)));
	ПостроительОтчетаОтчет.Параметры.Вставить("дата22", КонецМесяца(ДобавитьМесяц(НачПериода,1)));
	
	ПостроительОтчетаОтчет.Параметры.Вставить("дата3", НачалоМесяца(ДобавитьМесяц(НачПериода,2)));
	ПостроительОтчетаОтчет.Параметры.Вставить("дата33", КонецМесяца(ДобавитьМесяц(НачПериода,2)));
	
	ПостроительОтчетаОтчет.Параметры.Вставить("дата4", НачалоМесяца(ДобавитьМесяц(НачПериода,3)));
	ПостроительОтчетаОтчет.Параметры.Вставить("дата44", КонецМесяца(ДобавитьМесяц(НачПериода,3)));
	
	ПостроительОтчетаОтчет.Параметры.Вставить("дата5", НачалоМесяца(ДобавитьМесяц(НачПериода,4)));
	ПостроительОтчетаОтчет.Параметры.Вставить("дата55", КонецМесяца(ДобавитьМесяц(НачПериода,4)));
...
Процедура ОтчетИнициализация() ЭКспорт
	...
	|	ВЫБОР
	|		КОГДА ЗатратыОбороты.Период МЕЖДУ &дата1 И &Дата11
	|			ТОГДА &Месяц1
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ЗатратыОбороты.Период МЕЖДУ &дата2 И &Дата22
	|					ТОГДА &Месяц2
	|				ИНАЧЕ ВЫБОР
	|						КОГДА ЗатратыОбороты.Период МЕЖДУ &дата3 И &Дата33
	|							ТОГДА &Месяц3
	|						ИНАЧЕ ВЫБОР
	|								КОГДА ЗатратыОбороты.Период МЕЖДУ &дата4 И &Дата44
	|									ТОГДА &Месяц4
	|								ИНАЧЕ ВЫБОР
	|										КОГДА ЗатратыОбороты.Период МЕЖДУ &дата5 И &Дата55
	|											ТОГДА &Месяц5
	|										ИНАЧЕ &Месяц6
	|									КОНЕЦ
	|							КОНЕЦ
	...
	Настройка = ВосстановитьЗначение("НастройкаВнешниеОтчетыОтчетПоБюджетуЗатратФактОтчет_32368574-c0b2-44fe-8096-f904eb4dbece");
