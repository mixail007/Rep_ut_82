...
Функция Показать(Кэш,СписокОтмеченныхДокументов)Экспорт
	...
	фрм = сбисПолучитьФорму("ФормаГлавноеОкно").сбисНайтиФормуФункции("ПолучитьВыборкуОтправляемыхПакетов",,"ФормаПересылкаПакетов",МестныйКэш);
	 ...
	 Для Каждого оФайл из МассивВложений Цикл
		...
		Для Каждого оПодпись из МассивПодписей Цикл
			Если Найти(оПодпись.ИмяБезРасширения,оФайл.Имя)>0 Тогда
				 Подпись.Добавить(Новый Структура("Файл",Новый Структура("Имя,ПолноеИмяФайла",оПодпись.Имя,оПодпись.ПолноеИмя)));
			КонецЕсли;	
		КонецЦикла;	
...
Функция СохранитьВложенияНаДиск(оДокумент) Экспорт
	...
	Для Каждого Вложение из оДокумент.Вложение Цикл
		//перебираем массив файлов для каждого вложения
		//Для Каждого ФайлВложение из Вложение.Файл Цикл
