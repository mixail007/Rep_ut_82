...
Процедура Маршрут1ПриИзменении(Элемент)
...
Процедура ОбновитьМ(Обнов1 = истина,Обнов2 = Истина)
	...
	если обнов1 тогда
		...
		запрос.УстановитьПараметр("Подразделения",ЭтаФорма.СписокПодразделений);
		запрос.Текст = "ВЫБРАТЬ
		               ...
		               |				ТОГДА ЗаказыМаршруты.ЗаказПокупателя.Подразделение  В (&Подразделения)
		               |			ИНАЧЕ ЗаказыМаршруты.Маршрут.Подразделение В (&Подразделения)
		               |		КОНЕЦ
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	Дата";
...
Процедура Маршрут1НачалоВыбора(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = ЛОЖЬ;
	
	формаСпр = справочники.НовМаршруты.ПолучитьФормуВыбора();
	формаСпр.СправочникСписок.Отбор.Подразделение.ВидСравнения = ВидСравнения.ВСписке;
	формаСпр.СправочникСписок.Отбор.Подразделение.Значение = ЭтаФорма.СписокПодразделений;
	формаСпр.СправочникСписок.Отбор.Подразделение.Использование = Истина;
	Маршрут1 = формаСпр.ОткрытьМодально();
	
	Маршрут1ПриИзменении(Элемент);
КонецПроцедуры

Процедура Маршрут2НачалоВыбора(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = ЛОЖЬ;
	...
	формаСпр.СправочникСписок.Отбор.Подразделение.Значение = ЭтаФорма.СписокПодразделений;
	формаСпр.СправочникСписок.Отбор.Подразделение.Использование = Истина;
	Маршрут2 = формаСпр.ОткрытьМодально();
	
	Маршрут2ПриИзменении(Элемент);
