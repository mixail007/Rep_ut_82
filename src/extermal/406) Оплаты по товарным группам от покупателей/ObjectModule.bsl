...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	Текст= "ВЫБРАТЬ
		   ...
		   Текст ="
|		   ВЫБРАТЬ    Номенклатура,Регистратор,ДоговорКонтрагента,Сделка,СуммаОплаты
|	{ВЫБРАТЬ
|	Период,
|	Номенклатура.*,
|	ДоговорКонтрагента.*,
...
| ИЗ
|		  ( ВЫБРАТЬ
| ЕстьNULL(ЗаказПокупателяТовары.Номенклатура, &НоменклатураНеИспользовать)  Номенклатура,
...
|  (НАЧАЛОПЕРИОДА(Период, МЕСЯЦ)) КАК Период,
|  ВЫБОР КОГДА  (ЗаказПокупателяТовары.Ссылка Есть NULL ) или ЗаказПокупателяТовары.Ссылка.СуммаДокумента =0 ТОГДА
|   СуммаУпрРасход
|   ИНАЧЕ
...
|				Регистратор,
|				ДоговорКонтрагента.Владелец.Покупатель  И ДоговорКонтрагента.ВедениеВзаиморасчетов = &ПоЗаказам) КАК ВзаиморасчетыСКонтрагентамиОбороты
|	ГДЕ
...
|		ПО ВзаиморасчетыСКонтрагентамиОбороты.Сделка = ЗаказПокупателяТовары.Ссылка   ) А
|{ГДЕ
|	Период,
...
//	ОбщийОтчет.ЗаполнитьПоказатели("Сумма", "Сумма заказа", Ложь, "ЧЦ=15; ЧДЦ=2");
	...
	ОбщийОтчет.мНазваниеОтчета = "Оплаты от покупателей по товарным группам";
