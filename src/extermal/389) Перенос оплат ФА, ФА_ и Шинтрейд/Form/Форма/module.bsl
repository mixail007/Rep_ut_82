...
|				ТОГДА ВЫРАЗИТЬ((КорректировкаРеализацииТовары.Сумма - КорректировкаРеализацииТовары.СуммаДоИзменения) * (1 - &N / 100) КАК ЧИСЛО(15, 0))
|			КОГДА ВзаиморасчетыСКонтрагентамиОбороты.Регистратор ССЫЛКА Документ.ВозвратТоваровОтПокупателя
|				ТОГДА ВЫРАЗИТЬ(ВозвратТоваровОтПокупателяТовары.Сумма * (1 - &N / 100) КАК ЧИСЛО(15, 0))
|			ИНАЧЕ ВЫРАЗИТЬ((ВЫРАЗИТЬ(РеализацияТоваровУслугТовары.Цена * 1.18 * (1 - &N / 100) * (1 - (ЕСТЬNULL(ТранспортныеПоРеализациям.Транспортные, 0) + РеализацияТоваровУслугТовары.Ссылка.Сделка.СуммаДопРасходовЭкспорт) / РеализацияТоваровУслугТовары.Ссылка.СуммаДокумента) КАК ЧИСЛО(15, 0))) * РеализацияТоваровУслугТовары.Количество / 118 * 100 КАК ЧИСЛО(15, 0))
...
|			КОГДА ВзаиморасчетыСКонтрагентамиОбороты.Регистратор ССЫЛКА Документ.ВозвратТоваровОтПокупателя
|				ТОГДА ВЫРАЗИТЬ(ВозвратТоваровОтПокупателяТовары.Сумма * (1 - &N / 100) КАК ЧИСЛО(15, 0))
|			ИНАЧЕ ВЫРАЗИТЬ((ВЫРАЗИТЬ(РеализацияТоваровУслугТовары.Цена * (1 - &N / 100) КАК ЧИСЛО(15, 0))) * РеализацияТоваровУслугТовары.Количество * (1 - ЕСТЬNULL(ТранспортныеПоРеализациям.Транспортные, 0) / (ВЫРАЗИТЬ(РеализацияТоваровУслугТовары.Ссылка.СуммаДокумента КАК ЧИСЛО(15, 0)))) КАК ЧИСЛО(15, 0))
|		КОНЕЦ) КАК СуммаПослеПерерасчета,
|	ВзаиморасчетыСКонтрагентамиОбороты.СуммаУпрПриход,
|	ВзаиморасчетыСКонтрагентамиОбороты.Регистратор,
|	ВзаиморасчетыСКонтрагентамиОбороты.Сделка,
...
конецЕсли;	
	...
	Запрос.УстановитьПараметр("N", 0.5);
