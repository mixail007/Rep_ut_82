...
//+++( 25.10.2018 --- Задача № 57245 ----
	флРазрешенСтопЛист = ЛОЖЬ;
		i=1; N = ТабличноеПоле1.Количество(); 
		пока i<N цикл
	...
	//+++ С.Селин
	ВремяНачала = Вычислить("ТекущаяУниверсальнаяДатаВМиллисекундах()")/1000.0;
	...
	Если Результат.Количество()>1 Тогда 
		Если Результат.НайтиСтроки(Отбор).Количество() > 0 Тогда 
			Сообщить("Нельзя объединять Заказ Интернет магазина и Заказы терминала в один заказ");
			стоп = Истина;
		КонецЕсли;
	КонецЕсли;
	Если стоп тогда
	...
	Если Флажок1 тогда
		...
		Если ВремяВыполнения>0 тогда
			...
			КонецЦикла;
			Время1 = Вычислить("ТекущаяУниверсальнаяДатаВМиллисекундах()")/1000.0 - ВремяНачала;
			ЗаписатьВЖурналИзмененийВремяВыполненияВнешнейОбработки( 352, ВремяВыполнения, "Объединение "+строка(КолЗаказов)+" заказов по точкам 1 клиенту: "+строка(Контрагент1)+" ("+строка(Время1)+" Сек.)" ); //!!!!
		КонецЕсли;	
	КонецЕсли;
...
Процедура ОбъединитьЗаказыПокупателя( Контрагент, ТолькоЧтениеЗаказов=ЛОЖЬ, Подразделение=неопределено, Предупреждение=истина, ОбъединятьРазныеЦены = истина) ЭКСПОРТ
	...
	|
		...
		i=1; N = табСписокТочек.Количество(); 
		если N>0 тогда
			...
			пока ОбщийЗаказТовары.Следующий() цикл
				Если НЕ(ОбщийЗаказТовары.Номенклатура.Услуга) Тогда
					...
					Если КоличествоТовара>0 или стрБонусов = неопределено тогда
						...
						//31.03.2017 - чтобы копейки не выскакивали в реализациях!
