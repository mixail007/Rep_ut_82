...
Процедура Сформировать(Кнопка)
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ДесятьМесДоТекущейДаты", ДобавитьМесяц(НачалоДня(ДатаФормирования),-10));
	Запрос.УстановитьПараметр("КонецСледМесяца", КонецМесяца(ДобавитьМесяц(ДатаФормирования,1)));
	...
	Для каждого СтрокаТЗПоМенеджерам ИЗ ТаблицаПоМенеджерам Цикл
		...
		//#Если Сервер Тогда
					...
					Пока ТаблицаТоварыВПутиДляВывода.Следующий() Цикл
						...
						Пока ВыборкаПоПризнакуПоступленияВПорт.Следующий() Цикл
							...
							Если ВыборкаПоПризнакуПоступленияВПорт.ПризнакПоступленияВПорт = 0 Тогда
								ОбластьСтрокаЗаголовокПриходТоваров.Параметры.ПризнакПоступленияВПорт = "Поступление в порт после "+Лев(КонецМесяца(ДатаФормирования)+20*86400,10);
							ИначеЕсли ВыборкаПоПризнакуПоступленияВПорт.ПризнакПоступленияВПорт = 1 Тогда
								ОбластьСтрокаЗаголовокПриходТоваров.Параметры.ПризнакПоступленияВПорт = "Поступление в порт до "+Лев(КонецМесяца(ДатаФормирования)+20*86400,10)+" включительно";
							КонецЕсли;
			...
			Если ОтправлятьНаПочту Тогда 
				...
				ТекстПисьма = "Анализ выполнен по всем сезонным заказам покупателей, у которых дата действия больше " + Строка(КонецМесяца(ДобавитьМесяц(ДатаФормирования,1))) + ". " + Символы.ПС + "Учитываются товары в пути со сроком прихода на склад по " + Строка(КонецМесяца(ДатаФормирования)+20*86400);
...
Функция ПолучитьТоварыВПроизводстве()
	...
	Запрос.УстановитьПараметр("ТекущаяДата", НачалоДня(ДатаФормирования));
	Запрос.Текст = "ВЫБРАТЬ
	               |	ВЫБОР
...
Функция ПолучитьТоварыВПути()
	...
	Запрос.УстановитьПараметр("ДвадцатоеЧислоСледМесяца", КонецМесяца(ДатаФормирования)+20*86400);
	Запрос.УстановитьПараметр("ПустаяСсылка", Справочники.Контрагенты.ПустаяСсылка());
	Запрос.Текст = "ВЫБРАТЬ
