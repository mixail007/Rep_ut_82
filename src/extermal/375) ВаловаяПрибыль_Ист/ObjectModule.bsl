Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	Текст = "ВЫБРАТЬ
	        ...
	        |	МАКСИМУМ(ПартииТоваровНаСкладахОбороты.ДокументОприходования.ВалютаДокумента) КАК ДокументОприходованияВалютаДокумента,
	        ...
	        |	ВЫБОР
	        |		КОГДА ТаблицаРегистра.Номенклатура.ВидТовара = ЗНАЧЕНИЕ(Перечисление.ВидыТоваров.Диски)
	        |				И ТаблицаРегистра.Номенклатура.НоменклатурнаяГруппа.Код = ""00049""
	        |				И ВТ.ДокументОприходованияВалютаДокумента.Код = ""643""
	        |			ТОГДА ""Россия""
	        |		ИНАЧЕ """"
	        ...
	        |		ПО ТаблицаРегистра.Регистратор = ВТ.Регистратор
	        |			И ТаблицаРегистра.Номенклатура = ВТ.Номенклатура
	        |{ГДЕ
	        |	ТаблицаРегистра.ДоговорКонтрагента.Владелец.* КАК Покупатель,
	        |	ТаблицаРегистра.ДоговорКонтрагента.* КАК ДоговорКонтрагента,
	        |	ТаблицаРегистра.ЗаказПокупателя.* КАК ЗаказПокупателя,
