...
КонецЕсли;
  ...
  Если МенятьАдрес тогда
	 ...
	 Иначе	//+++ определение Вида адреса из адреса заказа покупателя
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ
	...
	//	Запрос.УстановитьПараметр("Представление", стр1.Заказ.АдресДоставки);
		...
		ВидАдреса1 = "";  Координаты1 = "";
		Пока Выборка.Следующий() Цикл
			знач1 =  Выборка.Представление+?(Выборка.Комментарий="","","~"+Выборка.Комментарий);
			
			Если сокрЛП(знач1)=сокрЛП(стр1.Заказ.АдресДоставки) тогда
				вид1 = строка(Выборка.Вид);
				ВидАдреса1 = ?(Выборка.Вид=Справочники.ВидыКонтактнойИнформации.ФактАдресКонтрагента, "Ф:",
						  	   ?(Выборка.Вид=Справочники.ВидыКонтактнойИнформации.ЮрАдресКонтрагента, "Ю:",
								?( Найти(Вид1,"Адрес доставки")>0, "Д" + прав(Вид1, стрДлина(Вид1)-14)+":",
									?( Найти(Вид1,"Адрес грузополучателя")>0, "Г" + прав(Вид1, стрДлина(Вид1)-21)+":",
								   строка(Выборка.Вид)+":"
						      ) ))   );
				Координаты1 = СокрЛП(выборка.Координаты);			  
				прервать; // дальше не ищем!
			КонецЕсли;			  
		КонецЦикла;
		
		Если ВидАдреса1="" тогда // не нашли???
			если стр1.Заказ.НомерТорговойТочкиКонтрагента>0 тогда
				точка = справочники.Точки.НайтиПоРеквизиту("Номер", стр1.Заказ.НомерТорговойТочкиКонтрагента);
				если точка <> неопределено тогда 
				знач1 =  СокрЛП(точка.Адрес);
				знач2 =  СокрЛП(точка.Адрес)   //новое определение адреса из Терминала!
				+?(сокрЛП(точка.ИнформацияДляВодителя)="", "", "~"+сокрЛП(точка.ИнформацияДляВодителя)+" "+сокрЛП(точка.КонтактноеЛицо));
				 	Если знач1=сокрЛП(стр1.Заказ.АдресДоставки) 
					 или знач2=сокрЛП(стр1.Заказ.АдресДоставки) тогда
					 	ВидАдреса1="Т"+формат( стр1.Заказ.НомерТорговойТочкиКонтрагента,"ЧГ=0")+":";
					 	Координаты1 = СокрЛП(точка.Координаты);			  
			 		КонецЕсли;
			    конецЕсли;
			 КонецЕсли;
		КонецЕсли;
		 
		стр1.АдресДоставки = стр1.Заказ.АдресДоставки; //ВидАдреса1  
		стр1.Координаты    = ?(Координаты1<>"", Координаты1, сокрЛП(Стр1.заказ.КоординатыДоставки));
	КонецЕсли;
...
//+++ 16.04.2016 - Адрес ТОЧКИ терминала - Координаты Сразу!
