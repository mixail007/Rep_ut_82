 ...
 //?(ВидОтчета = 0, "",
	...
	Если НЕ USD и не ТолькоБрак Тогда
		Если НеВыводитьДискиПодЗаказ=ЛОЖЬ Тогда // VISSOL    
			...
			|		ИЗ
			//+++ 13.03.2018 по задаче № 42166
			//|			РегистрСведений.НоменклатураПодЗаказ КАК НоменклатураПодЗаказ
			//|			РегистрСведений.СезонныйАссортимент  КАК НоменклатураПодЗаказ
			...
			ТекстВыводаВМакет14 = ?(USD, "Under the order. Delivery date of 30 days.", "Под заказ. Срок доставки 30 дней."); //18.08 - 30.11.2016
			ТекстВыводаВМакет7 = ?(USD, "Under the order. Delivery date of 7 days.", "Под заказ. Срок доставки 7 дней."); //30.07.2018
...
//======================================================================
	...
	Пока ВыборкаДетали.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ВыборкаДетали.Номенклатура) 
		Тогда
		...
		КонецЕсли;			
		ОбластьДетальныхЗаписей.Параметры.ЦенаМРИЦ = ценаМРЦ / ?(USD, курсUSD, 1);
		//+++)
		
			//рез = таблРаспродажи.Найти( СокрЛП(ВыборкаДетали.Номенклатура.Код), "Код" );
			рез = соотвРаспродажи.Получить( ВыборкаДетали.НоменклатураКод ); //12.07.2016 - соответствие ищет быстрее
			Если рез = Неопределено тогда
				ОбластьДетальныхЗаписей.Параметры.СвободнаяЦена = "";
			Иначе
				ОбластьДетальныхЗаписей.Параметры.СвободнаяЦена = СвЦена; //распродажа!
			КонецЕсли;
		...
		//21.02.2018 ------------------------------------------------------------------------------------
			...
			//+++ 18.08.2016 - соответствие ищет быстрее чем в таблице!
			рез2 = соотвЗавод.Получить( ВыборкаДетали.НоменклатураКод ); 
			Если рез2 = Неопределено тогда
				ОбластьДетальныхЗаписей.Параметры.Завод = "";
			Иначе
				ОбластьДетальныхЗаписей.Параметры.Завод = рез2;
			КонецЕсли;
			
			папкаПроизводителя = нрег(ВыборкаДетали.Производитель); //12.07.2016 номенклатура.Производитель
			...
			ФайлКартинки=нрег(""+СокрЛП(ВыборкаДетали.Модель)+"_"+СокрЛП(ВыборкаДетали.Цвет))+".png";
			
			//Если СписокФайловФТПдиски.НайтиПоЗначению(ФайлКартинки)=Неопределено тогда   //+++ 12.07.16
			...
			НоменклатураНаименование=СокрЛП(ВыборкаДетали.Номенклатура); 
...
Процедура ДействияФормыСформироватьТолькоШины(Кнопка)
	...
	//+++ 09.10.2014 - отдельная колонка Склад2
	//Если Константы.ИспользоватьОстаткиСтороннихПоставщиков.Получить() тогда  //05.02.2018 !!!
...
//==========================28.07.2016===================================	
	...
	|	ВТ_Остатки.Номенклатура.Типоразмер КАК Размер,
	|	ВТ_Остатки.Номенклатура.Производитель КАК Производитель,
	|	ВЫБОР
	...
	//СписокПоставщиковОТХ.Добавить(Справочники.Контрагенты.НайтиПоКоду("93827")); //ШинПромТорг с 12.09.2014
	...
	Пока ВыборкаДетали.Следующий() Цикл
		...
		Если ВидОтчета =0 И ВыборкаДетали.ТипЗаписи() = ТипЗаписиЗапроса.ИтогПоГруппировке Тогда // для клиентов
			...
			//Если Константы.ИспользоватьОстаткиСтороннихПоставщиков.Получить() тогда //+++ 09.10.2014 - 05.02.2018
				//Если ВыборкаДетали.ОстатокПоставщиков>20 Тогда 
				//	ОбластьДетальныхЗаписей.Параметры.ОстатокПоставщиков = ">20" ;
				//КонецЕсли;	
			//КонецЕсли;
...
функция ВыгрузитьДанные(Excel, номСтр=1, НазвСтр="Ярославль")
	...
	//ТабДок.ТекущаяОбласть = ТабДок.Область(11,1,N,M);
	...
	Если не (назвСтр="Диски" или назвСтр="Wheels" или назвСтр="Лист1") тогда 
...
Процедура ДействияФормыВыгрузитьВExcel(Кнопка, путь0="" ) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Нет Тогда
		Возврат;
	КонецЕсли;
	Если USD тогда нПерекМакс=0; // только для клиентов!
	Иначе нПерекМакс=1;
	КонецЕсли;	
	
	для нПерек=0 по нПерекМакс цикл  //сначала для клиентов, потом для менеджеров
		Переключатель1 = нПерек;
		...
		Сообщить(строка(ТекущаяДата())+"======== начало выгрузки данных по Ярославлю в файл: "+путь+ИмяФайла+" ==============");
		...
		Если USD тогда  //+++ 21.08.2015
			нМакс=1;
		иначе
			// 02.06.2016 - закладка Комплектующие
			нМакс=6; // 24.06.2016 - еще Грузовые диски  ТОЛЬКО для клиентов = 0
		КонецЕсли;
		...
		исключение
			ИмяФайла = стрЗаменить(ИмяФайла,".xls", "2.xls");
			Excel.SaveAs(путь+ИмяФайла); //чистый c закладками
		...
		//=============================================================================================
		Если не USD тогда
			Сообщить(строка(ТекущаяДата())+" --- начало формирования данных Шины Ярославль ---");
			...
			если Переключатель1=0 и не USD тогда
				...
				Сообщить(строка(ТекущаяДата())+" >>> Шины Ярославль выгружены на сайт---");
			КонецЕсли;	
			если ВыгрузитьДанные(Excel, 1, "Шины")<0 тогда возврат; КонецЕсли;
		КонецЕсли;	
		
		
		Сообщить(строка(ТекущаяДата())+" --- начало формирования данных "+?(USD,"Wheels","Диски")+" Ярославль ---");
		ДействияФормыСформироватьТолькоДиски(неопределено);
		если Переключатель1=0 и не USD  тогда
			ДействияФормыСохранитьНаСайте(неопределено);
			Сообщить(строка(ТекущаяДата())+" >>> Диски Ярославль выгружены на сайт---");
		КонецЕсли;	
		...
		иначе	
			НеВыводитьДискиПодЗаказ=ЛОЖЬ;	 //21 день  >> до 45
			если ВыгрузитьДанные(Excel, 2, "Диски" )<0 тогда возврат; КонецЕсли;
			
			//04.07.2016 --- раскраска ---
			ExcelЛист = Excel.WorkSheets(2);
			i=11; Nmax = 4200; //4150 строк 10/09/2018!
			пока i<Nmax цикл	
				ОбработкаПрерыванияПользователя();
					
			 Если СокрЛП(ExcelЛист.cells(i,24).Text)<>"" тогда // 28.03.2018 - 23>>24,  25>>26
				 ExcelЛист.Range( ExcelЛист.cells(i,1), ExcelЛист.cells(i,26) ).Select();
		...
		Если не USD тогда
			...
			Сообщить(строка(ТекущаяДата())+" --- начало формирования данных АКБ Ярославль ---");
			ДействияФормыСформироватьТолькоАКБ(неопределено);
			если Переключатель1=0  и не USD  тогда
				ДействияФормыСохранитьНаСайте(неопределено);
				Сообщить(строка(ТекущаяДата())+" >>> АКБ Ярославль выгружены на сайт---");
			КонецЕсли;	
			если ВыгрузитьДанные(Excel, 3, ?(USD,"Batteries", "АКБ") )<0 тогда возврат; КонецЕсли;
			
			Сообщить(строка(ТекущаяДата())+" --- начало формирования данных Аксессуары Ярославль ---");
			ДействияФормыСформироватьТолькоАксессуары(неопределено);
			если Переключатель1=0  и не USD  тогда
				ДействияФормыСохранитьНаСайте(неопределено);
				Сообщить(строка(ТекущаяДата())+" >>> Аксессуары Ярославль выгружены на сайт---");
			КонецЕсли;	
			если ВыгрузитьДанные(Excel, 4, ?(USD,"Accessories","Аксессуары") )<0 тогда возврат; КонецЕсли;
		
		//02.06.2016 добавлена выгрузка крепежа
			Сообщить(строка(ТекущаяДата())+" --- начало формирования данных Крепёж Ярославль ---");
			ДействияФормыСформироватьТолькоКрепёж(неопределено);
			если Переключатель1=0  и не USD  тогда
				ДействияФормыСохранитьНаСайте(неопределено);
				Сообщить(строка(ТекущаяДата())+" >>> Крепёж Ярославль выгружены на сайт---");
			КонецЕсли;	
		...
		Попытка
			...
			Сообщить(строка(ТекущаяДата())+" ==== Данные сохранены в файл: "+путь+ИмяФайла+" =============", СтатусСообщения.Информация);
		Исключение
			Excel.SaveAs(путь+ИмяФайла); //чистый c закладками
...
КонецПроцедуры
 ...
 процедура СохранитьВExcel_ДискиБрак(путь)
	...
	Попытка
		...
		Сообщить(строка(ТекущаяДата())+" ==== Данные сохранены в файл: "+путь+ИмяФайла1+" =============", СтатусСообщения.Информация);
	Исключение
		Excel1.Application.WorkBooks.Close();
		Excel1 = 0;
...
Для ПереклНом = 0 по 1 цикл
	...
	Переключатель1 = 1-ПереклНом; //наоборот!
	...
	ДействияФормыСформироватьТолькоДиски(неопределено);
 	иначе //27.08.2018 --- прямо в форме!
		сообщить(строка(ТекущаяДата())+" Обработка того же файла");
		N = ЭлементыФормы.ПолеТабличногоДокумента.ВысотаТаблицы;
		для СтрНом=11 по N цикл
			для КолНом=15 по 19 цикл
				числоТ = ЭлементыФормы.ПолеТабличногоДокумента.Область(стрНом, КолНом,стрНом, КолНом).Текст;
				Если числоТ<>"" тогда
				попытка
					число1= число(числоТ);
					Если число1>50 тогда
						ЭлементыФормы.ПолеТабличногоДокумента.Область(стрНом, КолНом,стрНом, КолНом).Текст = ">50";
					КонецЕсли;	
				исключение	
				КонецПопытки;	
				КонецЕсли;
			КонецЦикла;	
		КонецЦикла;	
		сообщить(строка(ТекущаяДата())+" Обработаны данные "+строка(N)+" строк >50");
	КонецЕсли;
	...
	если ВыгрузитьДанные(Excel1, 1, "Лист1")<0 тогда возврат; КонецЕсли;
	
	//04.07.2016 --- раскраска ---
		ExcelЛист = Excel1.WorkSheets(1);
		
//+++(23.03.2018 - удаляем 25 столбец Y комментариев и без раскраски
//28.03 - 08.10.2018 - добавлен ещё столбец - поэтому 26 столбец удаляем Z
		ExcelЛист.Columns("Z:Z").Select();
