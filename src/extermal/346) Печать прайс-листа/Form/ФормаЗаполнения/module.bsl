...
Процедура ОбновитьОтчет() Экспорт
	...
	Если ВВалютеОтчета И НЕ ЗначениеЗаполнено(ВалютаОтчета) Тогда
		СообщитьОбОшибке("Не указана валюта, в которой должен формироваться прайс-лист.", Отказ);
	КонецЕсли;

	Если ФлажокЦеныНеИзменилисьС И НЕ ЗначениеЗаполнено(ДатаЦеныНеИзменились) Тогда
		СообщитьОбОшибке("Не указана дата, с которой нужно проверять изменение цен.", Отказ);
	КонецЕсли;

	Если ФлажокЦеныНеИзменилисьС И ДатаЦеныНеИзменились > ДатаПрайса Тогда
		СообщитьОбОшибке("Дата, с которой нужно проверять изменение цен, больше даты отчета.", Отказ);
	КонецЕсли;
	...
	Если ВВалютеОтчета И ЗначениеЗаполнено(ВалютаОтчета) Тогда
		...
		Если КурсВалюты.Курс = 0 тогда
			СообщитьОбОшибке("Курс валюты отчета """ + ВалютаОтчета.Наименование+""" на дату " +
			               строка(ДатаКурса)+" не определен или нулевой. Формирование отчета невозможно.", Отказ);
		КонецЕсли;

		Если КурсВалюты.Кратность = 0 тогда
			СообщитьОбОшибке("Кратность курса валюты отчета """ + ВалютаОтчета.Наименование+""" на дату " +
...
Процедура УстановитьСвязьПолейБыстрогоОтбораНаФормеЛокально()
	...
	ЭлементыФормы.ПолеВидаСравненияНоменклатура.СписокВыбора = ПолучитьСписокВидовСравненияПоТипу(ТипОтбора);
	...
	ЭлементыФормы.ПолеВидаСравненияТипЦен.СписокВыбора = ПолучитьСписокВидовСравненияПоТипу(ТипОтбора);
	...
	//ЭлементыФормы.ПолеВидаСравненияЦеноваяГруппа.СписокВыбора = ПолучитьСписокВидовСравненияПоТипу(ТипОтбора);
...
Процедура ПриОткрытии()
	...
	//	Организация = ПолучитьЗначениеПоУмолчанию(глЗначениеПеременной("глТекущийПользователь"), "ОсновнаяОрганизация");
	//КонецЕсли;
