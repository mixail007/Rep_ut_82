...
//	Если НачалоПериода <> Дата(1,1,1) Тогда	
	Если ЗначениеЗаполнено(ПериодПланирования) Тогда	
		...
		если ЗаполнялиДиски тогда
			...
			НаборЗаписей.Отбор.МесяцПланирования.Установить(ПериодПланирования.ДатаНачала);
			НаборЗаписей.Отбор.периодПланирования.Установить(ПериодПланирования);
	 ...
	 Для каждого стр из Строка.Строки Цикл
		 Если Утвердить= Истина Тогда
			 ...
			 Если стр.ТоварнаяГруппа = Null и стр.направлениеПродаж <> Null  Тогда
				 ...
				 Если не Проверка.утвержден тогда //если еще не был утвержден
					 ...
					 Запись.МесяцПланирования = ПериодПланирования.ДатаНачала;
		 ...
		 если ЗначениеЗаполнено(стр.НаправлениеПродаж) и ЗначениеЗаполнено(Стр.товарнаягруппа) и ЗначениеЗаполнено(стр.НоменклатурнаяГруппа) Тогда
			 Если Стр.ТоварнаяГруппа.ЭтоГруппа =Ложь  Тогда
				 Если Утвердить=Истина или стр.Пометка = Истина  тогда
					 Запись = РегистрыСведений.ПолугодовойПланДепертаментаПродаж.СоздатьМенеджерЗаписи();
					 Запись.МесяцПланирования = ПериодПланирования.ДатаНачала;
					 Запись.ПериодПланирования = ПериодПланирования;
...
Процедура КоманднаяПанель1Действие9(Кнопка)
  //проверим, есть ли уже план
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ПолугодовойПланДепертаментаПродаж.ТоварнаяГруппа
		|ИЗ
	...
	Если Результат.Количество()>0 тогда
		Ответ = Вопрос("Уже есть записанный план. Вы уверены, что хотите перезаполнить?", РежимДиалогаВопрос.ДаНет, 0);
	...
	Если не РольДоступна("ПолныеПрава") или ЗначениеЗаполнено(НаправлениеПродаж)  Тогда
		...
		конецЕсли;
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "99 = 99","Основа.НаправлениеПродаж В (&НаправлениеПродаж )");
		Запрос.УстановитьПараметр("НаправлениеПродаж",СписокНаправлениеПродаж);
	иначе
		//Если глТекущийПользователь = Справочники.Пользователи.НайтиПоКоду("Серков")тогда  //ДП
		//  Запрос.Текст = СтрЗаменить(Запрос.Текст, "99 = 99","Основа.НаправлениеПродаж В Иерархии (&НаправлениеПродаж )");
		//  Запрос.УстановитьПараметр("НаправлениеПродаж",Справочники.НаправленияПродаж.НайтиПоКоду("26"));
		//  Запрос.УстановитьПараметр("ДепартаментПродаж",Справочники.НаправленияПродаж.НайтиПоКоду("26"));
	    //Иначе
		Если глТекущийПользователь = Справочники.Пользователи.НайтиПоКоду("Малышев Егор")тогда  //Диски
		  Запрос.Текст = СтрЗаменить(Запрос.Текст, "99 = 99","Основа.НаправлениеПродаж В Иерархии (&НаправлениеПродаж )");
		  Запрос.УстановитьПараметр("НаправлениеПродаж",Справочники.НаправленияПродаж.НайтиПоКоду("23"));
          Запрос.УстановитьПараметр("ДепартаментПродаж",Справочники.НаправленияПродаж.НайтиПоКоду("23"));
		ИначеЕсли глТекущийПользователь = Справочники.Пользователи.НайтиПоКоду("Смирнов А")или глТекущийПользователь = Справочники.Пользователи.НайтиПоКоду("Федунов")  тогда // колеса тут
		  Запрос.Текст = СтрЗаменить(Запрос.Текст, "99 = 99","Основа.НаправлениеПродаж В Иерархии (&НаправлениеПродаж )");
		  Запрос.УстановитьПараметр("НаправлениеПродаж",Справочники.НаправленияПродаж.НайтиПоКоду("24"));
          Запрос.УстановитьПараметр("ДепартаментПродаж",Справочники.НаправленияПродаж.НайтиПоКоду("24"));
		ИначеЕсли глТекущийПользователь = Справочники.Пользователи.НайтиПоКоду("Голубева В.С.")тогда //Аксы
		  Запрос.Текст = СтрЗаменить(Запрос.Текст, "99 = 99","Основа.НаправлениеПродаж В Иерархии (&НаправлениеПродаж )");
		  Запрос.УстановитьПараметр("НаправлениеПродаж",Справочники.НаправленияПродаж.НайтиПоКоду("28"));
          Запрос.УстановитьПараметр("ДепартаментПродаж",Справочники.НаправленияПродаж.НайтиПоКоду("28"));
		ИначеЕсли глТекущийПользователь = Справочники.Пользователи.НайтиПоКоду("Вострилов А.В.")тогда //contyre
		  Запрос.Текст = СтрЗаменить(Запрос.Текст, "99 = 99","Основа.НаправлениеПродаж В Иерархии (&НаправлениеПродаж )");
		  Запрос.УстановитьПараметр("НаправлениеПродаж",Справочники.НаправленияПродаж.НайтиПоКоду("10"));
          Запрос.УстановитьПараметр("ДепартаментПродаж",Справочники.НаправленияПродаж.НайтиПоКоду("10"));
		конецЕсли;  
	конецЕсли;	
	...
	//заполним свод
	 	Запрос.Текст = 
	  				...
	  				|			И ПродажиОбороты.ЗаказПокупателя = ПродажиСебестоимостьОбороты.ЗаказПокупателя
					//|ГДЕ
					//|	ПродажиОбороты.ДоговорКонтрагента.ОтветственноеЛицо.НаправлениеПродаж В Иерархии (&НаправлениеПродаж )
	  				|
	  				|СГРУППИРОВАТЬ ПО
	  				|	ПродажиОбороты.Номенклатура.НоменклатурнаяГруппа
	  				|
		...
		//Если глТекущийПользователь = Справочники.Пользователи.НайтиПоКоду("Серков")тогда  //ДП
		//  Запрос.Текст = СтрЗаменить(Запрос.Текст, "99 = 99","ПродажиОбороты.ДоговорКонтрагента.ОтветственноеЛицо.НаправлениеПродаж В Иерархии (&НаправлениеПродаж )");
		//  Запрос.УстановитьПараметр("НаправлениеПродаж",Справочники.НаправленияПродаж.НайтиПоКоду("26"));
		//  Запрос.УстановитьПараметр("ДепартаментПродаж",Справочники.НаправленияПродаж.НайтиПоКоду("26"));
		//ИначеЕсли глТекущийПользователь = Справочники.Пользователи.НайтиПоКоду("Малышев Егор")тогда  //Диски
		//  Запрос.Текст = СтрЗаменить(Запрос.Текст, "99 = 99","ПродажиОбороты.ДоговорКонтрагента.ОтветственноеЛицо.НаправлениеПродаж В Иерархии (&НаправлениеПродаж )");
		//  Запрос.УстановитьПараметр("НаправлениеПродаж",Справочники.НаправленияПродаж.НайтиПоКоду("23"));
		//  Запрос.УстановитьПараметр("ДепартаментПродаж",Справочники.НаправленияПродаж.НайтиПоКоду("23"));
		//ИначеЕсли глТекущийПользователь = Справочники.Пользователи.НайтиПоКоду("Смирнов А")или глТекущийПользователь = Справочники.Пользователи.НайтиПоКоду("Федунов")  тогда // колеса тут
...
Процедура КоманднаяПанель1ЗаполнитьИзПлана(Кнопка)
	...
	Запрос.Текст = 
		...
		|	И ПолугодовойПланДепертаментаПродаж.ПериодПланирования = &ПериодПланирования
		...
		|	ПолугодовойПланДепертаментаПродаж.ТоварнаяГруппа.Наименование
		|ИТОГИ
		|	СУММА(Количество),
		|	СУММА(Сумма),
		|	ВЫБОР
	...
	Запрос.УстановитьПараметр("МесяцПланирования", ПериодПланирования.ДатаНачала);
	Запрос.УстановитьПараметр("ПериодПланирования", ПериодПланирования);
	
	Если (не РольДоступна("ПолныеПрава") или ЗначениеЗаполнено(НаправлениеПродаж) или ( ((глТекущийПользователь =Справочники.Пользователи.НайтиПоКоду("Марешева И.Г.")или глТекущийПользователь = Справочники.Пользователи.НайтиПоКоду("Бондаренко Е.Д. (снабжение)"))))) и Кнопка.Имя<>"ПосмотретьВсе"  Тогда
		Если ЗначениеЗаполнено(НаправлениеПродаж) Тогда
	    	СписокНаправлениеПродаж = Новый СписокЗначений;
			СписокНаправлениеПродаж.Добавить(НаправлениеПродаж);
		иначе
			...
			СписокНаправлениеПродаж = РезультатНП.ВыгрузитьКолонку("Ссылка");
		конецЕсли;
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "99 = 99","ПолугодовойПланДепертаментаПродаж.НаправлениеПродаж В (&НаправлениеПродаж )");
		Запрос.УстановитьПараметр("НаправлениеПродаж",СписокНаправлениеПродаж);
		
	иначеЕсли Кнопка.Имя<>"ПосмотретьВсе" тогда
		Если глТекущийПользователь = Справочники.Пользователи.НайтиПоКоду("Серков") или (Кнопка.Имя="ПосмотретьВсе")  тогда  //ДП
		  Запрос.Текст = СтрЗаменить(Запрос.Текст, "99 = 99","ПолугодовойПланДепертаментаПродаж.НаправлениеПродаж В Иерархии (&НаправлениеПродаж )");
		  Запрос.УстановитьПараметр("НаправлениеПродаж",Справочники.НаправленияПродаж.НайтиПоКоду("26"));
		  //Запрос.УстановитьПараметр("ДепартаментПродаж",Справочники.НаправленияПродаж.НайтиПоКоду("26"));
		ИначеЕсли глТекущийПользователь = Справочники.Пользователи.НайтиПоКоду("Малышев Егор")тогда  //Диски
		  Запрос.Текст = СтрЗаменить(Запрос.Текст, "99 = 99","ПолугодовойПланДепертаментаПродаж.НаправлениеПродаж В Иерархии (&НаправлениеПродаж )");
	...
	Для каждого строка из Элементыформы.ДеревоПланирования.Значение.Строки Цикл
		...
		Для каждого строчка из Строка.Строки Цикл
			...
			Для каждого стр из строчка.Строки Цикл
				Если стр.товарнаяГруппа = Справочники.ТоварныеГруппыДляПланирования.НайтиПоКоду("000000037") тогда
					стр.ПриростПродажПлан = стр.ПриростПродажПланНаправлениеПродаж;
					Для каждого ст из Стр.Строки цикл
						ст.ПриростПродажПлан = ст.ПриростПродажПланНаправлениеПродаж;
                    конецЦикла;
				конецЕсли;	
			конецЦикла;	
		конецЦикла;	
	...
	конецЕсли;	
	  ...
	  Элементыформы.ДеревоСвод.Значение = Результат.Выгрузить(ОбходРезультатаЗапроса.ПоГруппировкамСИерархией);
	 	...
	 	Для каждого строка из Элементыформы.ДеревоСвод.Значение.Строки Цикл
		   	строка.ПриростПродажПлан = строка.ПриростПродажПланНаправлениеПродаж;
			Для каждого стр из строка.Строки Цикл
				Если стр.товарнаяГруппа = Справочники.ТоварныеГруппыДляПланирования.НайтиПоКоду("000000037") тогда
					стр.ПриростПродажПлан = стр.ПриростПродажПланНаправлениеПродаж;
	...
	конецЦикла;	
	  ...
	  Если Кнопка.Имя="ПосмотретьВсе" тогда
	  ...
	  иначе
		  ...
		  ЭлементыФормы.ДеревоПланирования.ТолькоПросмотр = ложь;
...
Процедура ПриОткрытии()
	...
	СписокДоступа.добавить(Справочники.Пользователи.НайтиПоКоду("Серков"));
	//СписокДоступа.добавить(Справочники.Пользователи.НайтиПоКоду("Малышев Егор"));
	//СписокДоступа.добавить(Справочники.Пользователи.НайтиПоКоду("Смирнов А"));
	//СписокДоступа.добавить(Справочники.Пользователи.НайтиПоКоду("Федунов"));
	//СписокДоступа.добавить(Справочники.Пользователи.НайтиПоКоду("Голубева В.С."));
	//СписокДоступа.добавить(Справочники.Пользователи.НайтиПоКоду("Вострилов А.В."));
	ЭлементыФормы.Панель.Страницы.Свод.Видимость = Истина;
	
//	Если   СписокДоступа.НайтиПоЗначению(глТекущийПользователь)<> Неопределено Тогда
	Если  РольДоступна("ПравоЗавершенияРаботыПользователей") или СписокДоступа.НайтиПоЗначению(глТекущийПользователь)<> Неопределено Тогда
		...
		//ЭлементыФормы.Панель.Страницы.Свод.толькоПросмотр = Истина;
		ЭлементыФормы.ДеревоСвод.ТолькоПросмотр = Истина;
		Элементыформы.ДеревоПланирования.Колонки.ПриростСреднейЦены.Доступность = Истина;
        Элементыформы.КоманднаяПанель1.Кнопки.кнРазбитьПоМесяцам.Доступность = Истина;
    конецЕсли;
...
Процедура КоманднаяПанель1кнРазбитьПоМесяцам(Кнопка)
	Ответ = Вопрос("Все месячные планы за период будут очищены. Продолжить?",РежимДиалогаВопрос.ДаНет);
	Если Ответ = КодВозвратаДиалога.Нет Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос.Текст = 
				...
				|	СУММА(ЕСТЬNULL(ПродажиОбороты.СтоимостьОборот, 0)) КАК Стоимость,
				|	ПродажиОбороты.ДоговорКонтрагента.ОтветственноеЛицо.НаправлениеПродаж
				|ПОМЕСТИТЬ ПродажиЗаПериодПланированияПрошлогоГода
				|ИЗ
				|	РегистрНакопления.Продажи.Обороты(&НачПППрошлогоГода, &КонПППрошлогоГода, , ) КАК ПродажиОбороты
				|
				|СГРУППИРОВАТЬ ПО
				|	ПродажиОбороты.Номенклатура.НоменклатурнаяГруппа,
				|	ПродажиОбороты.ДоговорКонтрагента.ОтветственноеЛицо.НаправлениеПродаж
				...
				|	0 КАК ПриростПродажПлан,
				|	СУММА(ВЫБОР
				|			КОГДА ЕСТЬNULL(ПродажиЗаПериодПланированияПрошлогоГода.Количество, 0) <> 0
				|				ТОГДА втДанныеПланаНаПолугодие.КоличествоПлан * ПродажиПрошлогоГода.Количество / ЕСТЬNULL(ПродажиЗаПериодПланированияПрошлогоГода.Количество, 0)
				|			ИНАЧЕ 0
				|		КОНЕЦ) КАК КоличествоПлан,
				|	СУММА(ВЫБОР
				|			КОГДА ЕСТЬNULL(ПродажиЗаПериодПланированияПрошлогоГода.Стоимость, 0) <> 0
				|				ТОГДА втДанныеПланаНаПолугодие.СуммаПлан * ПродажиПрошлогоГода.Стоимость / ЕСТЬNULL(ПродажиЗаПериодПланированияПрошлогоГода.Стоимость, 0)
				|			ИНАЧЕ 0
				|		КОНЕЦ) КАК СуммаПлан,
				|	0 КАК ПриростСреднейЦены,
				|	СУММА(ВЫБОР
				...
				|		ЛЕВОЕ СОЕДИНЕНИЕ ПродажиЗаПериодПланированияПрошлогоГода КАК ПродажиЗаПериодПланированияПрошлогоГода
				|		ПО Основа.НаправлениеПродаж = ПродажиЗаПериодПланированияПрошлогоГода.ДоговорКонтрагентаОтветственноеЛицоНаправлениеПродаж
				|			И Основа.НоменклатурнаяГруппа = ПродажиЗаПериодПланированияПрошлогоГода.НоменклатурнаяГруппа
				|ГДЕ
	...
	Запрос.УстановитьПараметр("НаправлениеПродажУбрать",СписокНаправлениеПродажУбрать);
	Запрос.УстановитьПараметр("ПериодПланирования",ПериодПланирования);
	Запрос.УстановитьПараметр("НачПППрошлогоГода",ДобавитьМесяц(ПериодПланирования.ДатаНачала,-12));
	Запрос.УстановитьПараметр("КонПППрошлогоГода",КонецДня(ДобавитьМесяц(ПериодПланирования.ДатаКонца,-12)));
	...
	к=0;
	КоличествоМесяцевПланирования = ПолучитьКоличествоМесяцев(ПериодПланирования.ДатаНачала, КонецМесяца(ПериодПланирования.ДатаКонца)+1);
	пока к <КоличествоМесяцевПланирования цикл
		//Запрос.УстановитьПараметр("НачПериода", ДобавитьМесяц(НачалоПериода,-12+к));
		Запрос.УстановитьПараметр("НачПериода", ДобавитьМесяц(ПериодПланирования.ДатаНачала,-12+к));
		...
		ЕстьПланыНаМесяц = ЕстьПланыНаМесяц(ДобавитьМесяц(Запрос.Параметры.НачПериода,12));
		Если не ЕстьПланыНаМесяц тогда
			...
			Сообщить("Записан план на: " + ДобавитьМесяц(Запрос.Параметры.НачПериода,12));
		Иначе
			Сообщить("НЕ записан план на: " + ДобавитьМесяц(Запрос.Параметры.НачПериода,12)+" т.к. есть ежемесячные планы!");
		КонецЕсли;
...
Функция ЕстьПланыНаМесяц(месяц)
	Запрос = новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   Запрос.УстановитьПараметр("МесяцПланирования", НачалоМесяца(месяц));
...
Функция ПолучитьКоличествоМесяцев(Дата1, Дата2)
	Запрос = новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	РАЗНОСТЬДАТ(&Дата1, &Дата2, МЕСЯЦ) КАК ЧислоМесяцев";
	Запрос.УстановитьПараметр("Дата1",Дата1);
	Запрос.УстановитьПараметр("Дата2",Дата2);
	Рез = Запрос.Выполнить().Выбрать();
	
	Пока Рез.Следующий() Цикл
		Возврат Рез.ЧислоМесяцев;
	КонецЦикла;
	Возврат 0 ;
КонецФункции

Процедура ДеревоПланированияПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
			//стр = ЭлементыФормы.ДеревоПланирования.текущиеДанные;
			//стр.Пометка = Истина;
КонецПроцедуры
