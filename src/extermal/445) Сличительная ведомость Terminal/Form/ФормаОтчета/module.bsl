Процедура КнопкаСформироватьНажатие(Кнопка)
	...
	Результат = Прочитать(НачПериода,КонПериода);
	НомерПП=0;
	...
	Для каждого строка ИЗ Результат Цикл
		...
		Если (строка.Статус <> строка.СтатусTerminal) или ((строка.СуммаДокумента - строка.СуммаTerminal)>1 или (строка.СуммаДокумента - строка.СуммаTerminal)<-1) Тогда	
			НомерПП=НомерПП+1;
			...
			Если  строка.Статус <> строка.СтатусTerminal Тогда
				ОбластьСтрокаДокумент.Области.Статусы.ЦветФона=WebЦвета.БледноЗеленый;
				постатусу = постатусу+1; 
			конецЕсли;
			Если  строка.СуммаДокумента <> строка.СуммаTerminal Тогда
...
иначеЕсли ЭлементыФормы.Заказы.Значение = 1 Тогда
    ...
    //ТабДок.ТолькоПросмотр=Истина;
    ЭлементыФормы.ПолеТабличногоДокумента.ТолькоПросмотр=Истина;
    ЭлементыФормы.Инфо.Заголовок = "Всего реализаций "+ИтогоЗаказов+".Расхождения по сумме у "+посумме+" реализаций ("+ +Окр(посумме/ИтогоЗаказов*100,2)+"%)."; 
	
иначеЕсли ЭлементыФормы.Заказы.Значение = 2 Тогда
	Результат = ПрочитатьСезонныеЗаказы();
	НомерПП=0;
	ТабДок = ЭлементыФормы.ПолеТабличногоДокумента;
	ТабДок.Очистить();
	Макет = ПолучитьМакет("МакетСезонныйЗаказ");
	ОбластьЗаголовок = Макет.ПолучитьОбласть("Заголовок");
    ...
    //ТабДок.ТолькоПросмотр=Истина;
    ЭлементыФормы.ПолеТабличногоДокумента.ТолькоПросмотр=Истина;
    ЭлементыФормы.Инфо.Заголовок = "Всего сезонных заказов "+ИтогоЗаказов+".Расхождения по сумме у "+посумме+" сезонных заказов ("+ +Окр(посумме/ИтогоЗаказов*100,2)+"%)."; 
...
КонецЦикла;	
   ...
   // ОбменСУТИнтернетМагазин.ВключитьРегистрациюОбъектаОИМ(ДокументОбъект,Истина);
	...
	ПланыОбмена.ЗарегистрироватьИзменения(ПланыОбмена.ПланОбмена_ОИМ.НайтиПоКоду("sim"), ТекущаяСтрока.Значение);
	ПроверитьВложенныеЗаказыДляРегистрации(ТекущаяСтрока.Значение);
...
процедура 	ПроверитьВложенныеЗаказыДляРегистрации(ДокументОбъект)
	Для каждого стр из ДокументОбъект.Заказы Цикл
...
Процедура ПриОткрытии()
	//пред.день
	НачПериода = НачалоДня(ТекущаяДата()) - 1*86400;
