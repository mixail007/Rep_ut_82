Процедура СкомпоноватьОтчет(ТабДок,ДатаНач,ДатаКон) Экспорт
	...
	Запрос.Текст = 
		...
		|	ВЫБОР
		|		КОГДА Маршруты.РасстояниеВозвратВГараж = 0
		|			ТОГДА Маршруты.Километров + Маршруты.Километров
		|		ИНАЧЕ Маршруты.РасстояниеВозвратВГараж + Маршруты.Километров
		...
		|	ВТ_Четвертая.ИтогоПробег КАК ИтогоПробег,
		...
		|	ВТ_Фин.Дата,
		|	ЕСТЬNULL(ВТ_Фин.Километров, 0) КАК Километров,
		|	ЕСТЬNULL(ВТ_Фин.РасстояниеВозвратВГараж, 0) КАК РасстояниеВозвратВГараж,
		...
		|	ЕСТЬNULL(ВТ_Фин.ИтогоПробег, 0) КАК ИтогоПробег
		|ИЗ
		|	ВТ_Фин КАК ВТ_Фин";
...
ЗапросБП = БазаИсточник.NewObject("Запрос");
	...
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|	Максимум(ЗаданиеНаПеревозкуГрузовПрямой.Вес) КАК ВесПрямой,
		...
		|	Максимум(ЗаданиеНаПеревозкуГрузовОбратный.Вес) КАК ВесОбратный,
	...
	ЗапросБП_УТ.Текст ="ВЫБРАТЬ
	                   ...
	                   |			И ВТ_2.Дата >= ТарифыНаПеревозкиСрезПоследних.Период
	                   |;
	                   |
	                   |////////////////////////////////////////////////////////////////////////////////
	                   |ВЫБРАТЬ РАЗЛИЧНЫЕ
	                   ...
	                   |			И ВТ_Тариф.Дата = ТаблицаБП.Дата";
...
процедура ПереименоватьМаркуАвтомобиля(Строка)	Экспорт
...
Функция  МассивНаименованийМарокАвто() Экспорт
	Рез = "";
...
//		"Сборочный лист, новый склад", // Представление команды в пользовательском интерфейсе
//		"РасходнаяНакладнаяВнешняя",				// Уникальный идентификатор команды
//	);
