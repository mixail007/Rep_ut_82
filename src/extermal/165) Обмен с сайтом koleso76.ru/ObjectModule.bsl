...
Перем СвойстваНоменклатуры Экспорт;
...
Процедура НастроитьПостроительОтчета(ПостроительОбъект) Экспорт
	...
	ПостроительОбъект.Текст = "ВЫБРАТЬ
							  ...
							  |	ИЗ
							  |		РегистрНакопления.ТоварыНаСкладах.Остатки(&ДатаОтчета, НЕ Склад.ЗапретитьИспользование И НЕ Склад.Транзитный {(Склад).* , (Номенклатура).*}) КАК ТоварыНаСкладахОстатки
...
Функция ЗаполнитьСписокЗначенийПредложения(СписокЗначенийCML, СтрокаТовара)
	...
	Если СтрокаТовара.ТипЦен<>NULL Тогда
		...
		ИдТипаЦены = Строка(СтрокаТовара.ТипЦен.УникальныйИдентификатор());
...
Функция ВыгрузитьПроизводителейВКаталогНаДиске(СтруктураИзменений, КаталогНаДиске) 
	...
	ИмяФайлаКаталога = КаталогНаДиске + "\import_man.xml";
	...
	ВыгрузитьКлассификатор(ОбъектCMLКаталог, МассивПроизводителей, ИдКаталога, СвойстваПроизводителей);
...
Функция ВыгрузитьМоделиВКаталогНаДиске(СтруктураИзменений, КаталогНаДиске) 
	...
	ИмяФайлаКаталога = КаталогНаДиске + "\import_mod.xml";
...
Функция ВыгрузитьТоварыВКаталогНаДиске(СтруктураИзменений, КаталогНаДиске) 
	...
	ИмяФайлаКаталога = КаталогНаДиске + "\import_nom.xml";
	...
	НаименованиеКаталогаТоваровCML = "Номенклатура";
	...
	ВыгрузитьКлассификатор(ОбъектCMLКаталог, МассивНоменклатуры, ИдКаталога, СвойстваНоменклатуры);
	
	ОтобразитьСостояние("Подготовка данных...");
	
	СтруктураРезультата = СформироватьCMLКаталогаПредложений_Товары(СтруктураИзменений, ОбъектCMLКаталог, ОбъектCMLПакетПредложений, ИдКаталога, 
...
Функция СформироватьCMLКаталогаПредложений_Товары(СтруктураИзменений, ОбъектCMLКаталог, ОбъектCMLПакетПредложений, ИдКаталога, 
	ПостроительЗапросаРезультат, КаталогНаДиске)
	...
	Если Всего > 0 Тогда
		...
		Для Каждого СтрокаГруппировкиСсылка Из ТаблицаРезультатаЗапроса  Цикл
			...
			СписокЗначенийРеквизитов.Добавить("model",СтрокаГруппировкиСсылка.МодельСсылка.УникальныйИдентификатор());
			...
			Если СтрокаГруппировкиСсылка.НоменклатураСсылка.ВидТовара = Перечисления.ВидыТоваров.Шины Тогда
				...
				Если СтруктураРазмеров<>Неопределено Тогда
					СписокЗначенийРеквизитов.Добавить("widthshina",СтруктураРазмеров.Ширина);
				...
				Иначе
					...
					СписокЗначенийРеквизитов.Добавить("widthshina",0);
					СписокЗначенийРеквизитов.Добавить("height",0);
					СписокЗначенийРеквизитов.Добавить("diam",0);
					СписокЗначенийРеквизитов.Добавить("diag_razm",СтрокаГруппировкиСсылка.НоменклатураСсылка.Типоразмер.Наименование);
				КонецЕсли;
				
			ИначеЕсли СтрокаГруппировкиСсылка.НоменклатураСсылка.ВидТовара = Перечисления.ВидыТоваров.Диски Тогда
				СписокЗначенийРеквизитов.Добавить("typedisk",СтрокаГруппировкиСсылка.НоменклатураСсылка.Родитель.УникальныйИдентификатор());
				...
				Если СтруктураРазмеров<>Неопределено Тогда
					...
					СписокЗначенийРеквизитов.Добавить("diamdisk",СтруктураРазмеров.Диаметр);
					...
					СписокЗначенийРеквизитов.Добавить("dia",СтруктураРазмеров.Ступица);
				Иначе
					Сообщить("Ошибка при разборе размера '"+СтрокаГруппировкиСсылка.НоменклатураСсылка+"'",СтатусСообщения.Важное);
				КонецЕсли;
...
СвойстваНоменклатуры  = Новый СписокЗначений;
...
СвойстваНоменклатуры.Добавить("type","ВидТовара");
СвойстваНоменклатуры.Добавить("model","Модель");
СвойстваНоменклатуры.Добавить("widthshina","Ширина");
СвойстваНоменклатуры.Добавить("height","Высота");
СвойстваНоменклатуры.Добавить("diam","Диаметр");
СвойстваНоменклатуры.Добавить("diag_razm","Диагональный размер");
СвойстваНоменклатуры.Добавить("typedisk","Тип");
СвойстваНоменклатуры.Добавить("width","Ширина обода");
СвойстваНоменклатуры.Добавить("diamdisk","Диаметр диска");
СвойстваНоменклатуры.Добавить("pcd","Cверловка");
СвойстваНоменклатуры.Добавить("et","ET- вылет");
СвойстваНоменклатуры.Добавить("dia","Ступица");
