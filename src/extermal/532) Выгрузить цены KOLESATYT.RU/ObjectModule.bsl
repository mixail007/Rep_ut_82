...
Функция ВыгрузитьЦены(ВыгружатьНаСайт=истина) экспорт
	...
	//		|				И (НАЧАЛОПЕРИОДА(втПродажи.Период, МЕСЯЦ) >= втБонусы.Период)
	//		|	
	//		|	СГРУППИРОВАТЬ ПО
	//		|		втПродажи.Период,
	...
	//		|	МАКСИМУМ(ВЫРАЗИТЬ(втОстатки.СтоимостьОстаток / втОстатки.КоличествоОстаток - втОстатки.СтоимостьОстаток / втОстатки.КоличествоОстаток / 1.18 * ЕСТЬNULL(втБонусыПоНоменклатуре.Бонус, 0) / 100 КАК ЧИСЛО(15, 0))) КАК СебестоимостьСУчетомБонуса,
	//		|	МИНИМУМ(ЕСТЬNULL(втБонусыПоНоменклатуре.Бонус, 0)) КАК Бонус
	//		|ПОМЕСТИТЬ втСебестоимость
	//		|ИЗ
	//		|	втОстатки КАК втОстатки
	//		|		ЛЕВОЕ СОЕДИНЕНИЕ втБонусыПоНоменклатуре КАК втБонусыПоНоменклатуре
	//		|		ПО втОстатки.Номенклатура = втБонусыПоНоменклатуре.Номенклатура
	//		|ГДЕ
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
				...
				ЗаписьXML.ЗаписатьНачалоЭлемента("Цены");
				 ...
				 Для каждого стр из Рез цикл
					...
					//Если стр.Себестоимость<>стр.СебестоимостьСУчетомБонуса тогда
