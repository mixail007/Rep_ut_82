...
Процедура Отчет(ТабДок) Экспорт
	...
	|   Б.ПремияЗаНаценку * Б.КоэффициентОплаты * Б.Квалификация КОНЕЦ КАК БонусЗаНаценку,
	|	ВЫБОР КОГДА Б.Контрагент.ВходитВХолдинг  ИЛИ Б.Контрагент =&Старк ТОГДА
	|	Б.СуммаОплаты * &КоэффициентЗаПродажиФФ * Б.Квалификация
	|   ИНАЧЕ
	|   Б.СуммаОплаты * &КоэффициентЗаПродажи * Б.Квалификация
	|   КОНЕЦ
	|	КАК ПремияЗаПродажи,
	|	0 КАК ПремияИтого
	|ИЗ
	...
	|       ВЫБОР
	|		КОГДА &ФлагКвалификация_1 ТОГДА 1
	|		КОГДА ОплатыОтПокупателей.ОтветственноеЛицо.Квалификация = 0 ТОГДА 1
	...
	|)
	//16.03.2016 - убираем АЭ для ВСЕХ менеджеров
	|И ((НЕ (ДоговорКонтрагента.Владелец.КонтрагентДляРезерваИМ = &КонтрРезерваИМ)) ИЛИ ДоговорКонтрагента.Владелец В (&КтргИсключения))

	|И (ДоговорКонтрагента.ОтветственноеЛицо В (&СписокМенеджер) ИЛИ НЕ &ОтбиратьПоМенеджеру) ) КАК ВзаиморасчетыСКонтрагентамиОбороты
	|			ГДЕ
	...
	|		ОБЪЕДИНИТЬ
	|
//*** 02.2016 До этого было сделано по табличной части документа Взаимозачет, переделала по регистарм
|ВЫБРАТЬ
...
|	СУММА(ВзаиморасчетыСКонтрагентамиОбороты.СуммаУпрРасход) КАК СуммаУпр,
|	СУММА(ВзаиморасчетыСКонтрагентамиОбороты.СуммаУпрРасход) КАК СуммаОплатыДляРасчетаПремии
|ИЗ
|	РегистрНакопления.ВзаиморасчетыСКонтрагентами.Обороты(&НачДата,&КонДата, Регистратор,
|(ДоговорКонтрагента.ВедениеВзаиморасчетов = &ПоЗаказам 
//13.02.2018
...
|	ВзаиморасчетыСКонтрагентамиОбороты.ДоговорКонтрагента.Владелец	
|
//*** конец Взаимозачеты по регистрам

|) КАК ОплатыОтПокупателей
|
...
|
//+++ 18.07.2013  КоэффициентОтсечения разделился на КоэффициентОтсеченияШ и КоэффициентОтсеченияД
|  ВЫБОР КОГДА Номенклатура.ВидТовара=Значение(Перечисление.ВидыТоваров.Шины) И Наценка>0 И (СуммаПродажи>Себестоимость*(1+&КоэффициентОтсеченияШ))  ТОГДА 
|    (СуммаПродажи-Себестоимость*(1+&КоэффициентОтсеченияШ))
| * &КоэффициентЗаНаценку
|
| КОГДА  Номенклатура.ВидТовара=Значение(Перечисление.ВидыТоваров.Диски) И Наценка >0 И (СуммаПродажи>Себестоимость*(1+&КоэффициентОтсеченияД))  ТОГДА 
|    (СуммаПродажи-Себестоимость*(1+&КоэффициентОтсеченияД))
| * &КоэффициентЗаНаценкуДиски // Для дисков другой коэффициент
//***2015.02.24 АКБ
|
| КОГДА  Номенклатура.ВидТовара=Значение(Перечисление.ВидыТоваров.АКБ) И Наценка >0 И (СуммаПродажи>Себестоимость*(1+&КоэффициентОтсеченияАКБ))  ТОГДА 
|    (СуммаПродажи-Себестоимость*(1+&КоэффициентОтсеченияАКБ))
| * &КоэффициентЗаНаценкуАКБ // Для АКБ другой коэффициент          
//***
|
| КОГДА  Номенклатура.ВидТовара=Значение(Перечисление.ВидыТоваров.Аксессуары) И Наценка >0 И (СуммаПродажи>Себестоимость)  ТОГДА 
|    (СуммаПродажи-Себестоимость)
| * &КоэффициентЗаНаценкуАксессуары // Для аксессуаров другой коэффициент
...
|ДоговорКонтрагента.ОтветственноеЛицо В (&СписокМенеджер) ИЛИ НЕ &ОтбиратьПоМенеджеру 

//16.03.2016 - убираем АЭ для ВСЕХ менеджеров
|И ((НЕ (ДоговорКонтрагента.Владелец.КонтрагентДляРезерваИМ = &КонтрРезерваИМ)) ИЛИ ДоговорКонтрагента.Владелец В (&КтргИсключения))

|										) КАК ПродажиОбороты
|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ПродажиСебестоимость.Обороты(&НачалоПериода, , , 

//16.03.2016 - убираем АЭ для ВСЕХ менеджеров  +++
|((НЕ ЗаказПокупателя.Контрагент.КонтрагентДляРезерваИМ = &КонтрРезерваИМ) ИЛИ ЗаказПокупателя.Контрагент В (&КтргИсключения))
...
Запрос.Текст="ВЫБРАТЬ Менеджер ОтветственноеЛицо, //Контрагент, ДоговорКонтрагента, 
		...
		|	  НЕ ДоговорКонтрагента.Владелец.ВходитВХолдинг И НЕ ДоговорКонтрагента.Владелец =&Старк
