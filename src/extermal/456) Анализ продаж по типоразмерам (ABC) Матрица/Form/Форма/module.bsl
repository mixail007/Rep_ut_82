...
процедура ЗаполнитьНачальныеДанные()
	...
	ФлФилиал=ЛОЖЬ;
	ПроцентФ = 10;
	Филиал = масФилиалов[2]; //СПб  5, 106, 112, 133, 138, 172...
		...
		//---------------- по товарам типоразмера------------------------
			для каждого стр22 из стр11.строки цикл
				...
				Если стр00<>неопределено тогда
					...
					стр22.Количество = ?(стр00.Количество>0 и стр00.Количество<2, 4, Окр(стр00.Количество/4)*4 ); // 1,2 >> 4
...
//		стр2.Количество = стр1.Количество 				* стр0.КолC/100; 
	...
	//---------------Сразу округлять НЕ будем!-----------------------------
		Если Фл4 тогда
			Для каждого стр2 из стр1.Строки цикл
				   стр2.КоличествоПлан = окр(стр2.КоличествоПлан/4,0)*4;
				стр2.КоличествоМатрица = окр(стр2.КоличествоМатрица/4,0)*4;
			КонецЦикла;	
				...
				Для каждого стр22 из стр11.Строки цикл
					...
					Если Фл4 тогда //-------округляем до 4х------------
						...
						Если стр3.КоличествоПлан=0 тогда //16.10.2018 
							стр3.КоличествоПлан=4;
						КонецЕсли;	
						стр3.КоличествоМатрица = Окр(стр3.КоличествоМатрица/4 +ОкрВниз)*4;
			   ...
			   КолМатрицаКласса = стр2.КоличествоМатрица;
				 ...
				 Для каждого стр3 из стр2.Строки цикл
					...
					Если Фл4 тогда //-------округляем до 4х------------
						...
						Если КоличествоПлан=0 тогда //16.10.2018 
							КоличествоПлан=4;
						КонецЕсли;	
...
//	Дерево1.Строки.Сортировать("Типоразмер");
		...
		Для каждого стр2 из стр1.Строки цикл
			Для каждого стр3 из стр2.Строки цикл
				Если стр3.КоличествоПлан>0 тогда
					...
					стрЗак.СтавкаНДС = перечисления.СтавкиНДС.НДС18;
					стрЗак.СуммаНДС = стрЗак.Сумма*18/118;
				КонецЕсли;	
		    КонецЦикла;
	    КонецЦикла;
    КонецЦикла;
	...
	//---------------подсказка для выбора аналогов----------------------------------------	
		Если Элемент.Имя = "ДеревоПеремещений" тогда
			Если стр1.ДанныеСтроки.Уровень()=2 тогда //ВСЕ строки с остатками Ярославля!
				...
				Если  услРекомендуем(стр1.ДанныеСтроки) тогда //есть в Ярославле
				стр1.ЦветФона = webЦвета.БледноЗеленый;    //---------> рекомендуемое... 
				
				ИначеЕсли стр1.ДанныеСтроки.КоличествоФил>стр1.ДанныеСтроки.КоличествоПлан тогда// уже есть БОЛЬШЕ чем надо!
				стр1.ЦветФона = webЦвета.СветлоРозовый;
