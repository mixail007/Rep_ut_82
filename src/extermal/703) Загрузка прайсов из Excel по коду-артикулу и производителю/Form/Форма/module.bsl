...
Функция мПрочитатьТабличныйДокументИзExcel( ИмяФайла, НомерЛистаExcel = 1) Экспорт
	...
	таб.Колонки.Добавить("ПредполагаемыйКод", Новый ОписаниеТипов("Строка"	, , Новый КвалификаторыСтроки(50, ДопустимаяДлина.Переменная)));
	таб.Колонки.Добавить("НомерСтроки", Новый ОписаниеТипов("Число"	, Новый КвалификаторыЧисла(6, 0)));
	таб.Колонки.Добавить("Производитель", Новый ОписаниеТипов("Строка"	, , Новый КвалификаторыСтроки(200, ДопустимаяДлина.Переменная)));
	...
	Для Row = НачалоЗахваченногоНомера По RowCount Цикл
		...
		ПредполагаемыйПроизводитель=Формат(ExcelЛист.Cells(Row,НомерКолонкиСПроизводителем).Value,"ЧГ=''");
		Если ЗначениеЗаполнено(СокрЛП(ПредполагаемыйКод)) Тогда
			нстр = таб.Добавить();
			нстр.ПредполагаемыйКод = СокрЛП(ПредполагаемыйКод);
			нстр.Производитель = СокрЛП(ПредполагаемыйПроизводитель);
	...
	Запрос.Текст="ВЫБРАТЬ РАЗЛИЧНЫЕ
	             ...
	             |////////////////////////////////////////////////////////////////////////////////
	             |ВЫБРАТЬ
	             |	вт.ПредполагаемыйКод,
	             |	вт.НомерСтроки,
	             |	втПроизводители.Ссылка КАК ПроизводительСсылка,
	             |	вт.Производитель КАК ПроизводительСтр
	             ...
	             |		ГДЕ
	             |			99 = 99) КАК втПроизводители
	             |		ПО (втПроизводители.Наименование ПОДОБНО вт.Производитель)
	             |;
	             |
	             |////////////////////////////////////////////////////////////////////////////////
	             |ВЫБРАТЬ
	             |	втПроизводители.ПредполагаемыйКод,
	             |	втПроизводители.НомерСтроки,
	             |	Номенклатура.Ссылка КАК Номенклатура,
	             |	втПроизводители.ПроизводительСсылка КАК Производитель,
	             |	втПроизводители.ПроизводительСтр
	             |ИЗ
	             |	втПроизводители КАК втПроизводители
	             |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК Номенклатура
	             |		ПО (Номенклатура.Артикул ПОДОБНО ""%"" + втПроизводители.ПредполагаемыйКод + ""%"")
	             |			И втПроизводители.ПроизводительСсылка = Номенклатура.Производитель";
				 Запрос.УстановитьПараметр("таб",Таб);
				 ИсключаемыеРодители = новый СписокЗначений;
				 Если ЗначениеЗаполнено(ВидТовара) тогда
				 	Запрос.Текст = СтрЗаменить(Запрос.Текст,"99 = 99","не Производители.Родитель в (&ИсключаемыйРодитель)");
					Если ВидТовара = Перечисления.ВидыТоваров.Шины тогда
						ИсключаемыеРодители.Добавить(Справочники.Производители.НайтиПоКоду("208"));//аксы
						ИсключаемыеРодители.Добавить(Справочники.Производители.НайтиПоКоду("763"));//диски
						Запрос.УстановитьПараметр("ИсключаемыйРодитель",ИсключаемыеРодители);
					Иначе
						Запрос.УстановитьПараметр("ИсключаемыйРодитель",Справочники.Производители.НайтиПоКоду("762")); //шины
					КонецЕсли;
				 КонецЕсли;
				 ...
				 Для каждого стр из Рез цикл
					 ...
					 //ПредполагаемыйКод=Формат(ExcelЛист.Cells(стр.НомерСтроки,НомерКолонкиСКодом).Value,"ЧГ=''");
					 ПредполагаемыйКод = стр.ПредполагаемыйКод;
					 Если ЗначениеНеЗаполнено(СокрЛП(ПредполагаемыйКод)) Тогда
						 Сообщить("--- Значение не заполнено в ячейке Кода №" + Строка(НомерКолонкиСКодом)+", в строке №" +Строка(стр.НомерСтроки), СтатусСообщения.Внимание);
						 Продолжить;
					 КонецЕсли;	
					 ...
					 //   Номенклатура = НоменклатураНайтиПоКодуИАртикулу(ПредполагаемыйКод, Производитель); 
					 //КонецЕсли;//НоменклатураНайтиПоКодуИАртикулу(ПредполагаемыйКод, Производитель);   //долго ищет
					 ...
					 Если ЗначениеНеЗаполнено(Номенклатура) Тогда
						 Сообщить("ххх Не найдена номенклатура по коду или артикулу в колонке №" + Строка(НомерКолонкиСКодом)+", в строке №" +Строка(стр.НомерСтроки)+", код/артикул: "+ПредполагаемыйКод+", производитель: "+стр.ПроизводительСтр, СтатусСообщения.Внимание);
						 Продолжить;
					 КонецЕсли;
					 //КонецЕсли;
					 ...
					 Для сч=0 по ИспользуемыеТипыЦен.Количество()-1 Цикл
						 Попытка 
							 СтрокаЗаказа["ТипЦен"+Число(сч+1)] = Число(Строка(ExcelЛист.Cells(стр.НомерСтроки,ИспользуемыеТипыЦен[сч].НомерКолонки).Value));
							 
							 //ЦенаТипЦен5=Число(Строка(ExcelЛист.Cells(Row,НомерКолонкиБазовая).Value));
						 Исключение
							 Сообщить("В строке №" +Строка(стр.НомерСтроки)+"содержится неверный формат числа.");
							 Продолжить;
						 КонецПопытки;
					 КонецЦикла;
