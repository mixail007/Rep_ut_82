...
//+++16.10.2014
	ОбщийОтчет.ПостроительОтчета.Параметры.Вставить("ОтборНомГрупп", (СписокНомГрупп.Количество()>0) );
	ОбщийОтчет.ПостроительОтчета.Параметры.Вставить("СписокНомГрупп", СписокНомГрупп);

//+++28.11.2014
	ОбщийОтчет.ПостроительОтчета.Параметры.Вставить("ОтборПоПодразделению", НЕ мПодразделение.Пустая() );
	//подразделение складов Ярославля - пустое!
	ОбщийОтчет.ПостроительОтчета.Параметры.Вставить("Подразделение", ?(мПодразделение.Код="00005", справочники.Подразделения.ПустаяСсылка(),мПодразделение) );
    ОбщийОтчет.ПостроительОтчета.Параметры.Вставить("Подразделение2", мПодразделение);
	
	ОбщийОтчет.ПостроительОтчета.Параметры.Вставить("ПустоеПодразделение", Справочники.Подразделения.ПустаяСсылка());
...
Процедура ПриОткрытии()
	...
	Если СписокНомГрупп.Количество()=0 тогда //+++ 16.10.2014 автоопределение полного списка
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               ...
		               |ГДЕ
		               |	НоменклатурныеГруппы.Ответственный = &Ответственный
		...
		Если табл.Количество()>0 тогда
			СписокНомГрупп.ЗагрузитьЗначения( табл.ВыгрузитьКолонку("Ссылка") );
		КонецЕсли;	
	КонецЕсли;
	
	Если ОбщийОтчет.ДатаНач='00010101' тогда
		ОбщийОтчет.ДатаНач = ДобавитьМесяц( НачалоМесяца(ТекущаяДата()), -1);
	КонецЕсли;	
	Если ОбщийОтчет.ДатаКон='00010101' тогда
		ОбщийОтчет.ДатаКон = КонецДня(ТекущаяДата())+1;
	КонецЕсли;
...
Процедура ДокументРезультатОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	если ТипЗнч(Расшифровка)=ТИП("Структура") тогда
		СтандартнаяРасшифровка = ложь;
		//+++ 28.11.2014
		если СписокНомГрупп.Количество()>0 тогда
			Расшифровка.Вставить("НоменклатураНоменклатурнаяГруппа", СписокНомГрупп);
		КонецЕсли;
		если не мПодразделение.Пустая() тогда
			Расшифровка.Вставить("Подразделение", мПодразделение);
		КонецЕсли;
		ЭтотОтчет.ОбработкаРасшифровки(Расшифровка, Элемент, ВысотаЗаголовка, СтандартнаяОбработка);
   	Иначе
		СтандартнаяРасшифровка = истина;
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Ответ = Вопрос("Завершить работу с отчетом?", Режим, 0);
