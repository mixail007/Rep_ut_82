...
Процедура УправлениеПараметрамиОтображенияЭлементовФормы()
	...
	Если флУточнитьВремяОтчета Тогда
		ЭлементыФормы.ДатаНач.Формат = "";
		ЭлементыФормы.ДатаКон.Формат = "";
	Иначе
		ЭлементыФормы.ДатаНач.Формат = "ДФ=dd.MM.yyyy";
		ЭлементыФормы.ДатаКон.Формат = "ДФ=dd.MM.yyyy";
	КонецЕсли;
...
Процедура ОбновитьОтчет() Экспорт
	...
	ЭтотОтчет.СформироватьОтчет(ЭлементыФормы.ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка,,флУточнитьВремяОтчета);
...
Процедура КоманднаяПанельФормыОбновить(Кнопка)
	...
	Если ВремяВыполнения>0 тогда
		ЗаписатьВЖурналИзмененийВремяВыполненияВнешнейОбработки( 101, ВремяВыполнения, "Кнопка [Сформировать] - "+парамЗапроса+" ("+строка(ВремяВыполнения)+" Сек.)" ); //!!!!
	КонецЕсли;	
	//+++)   18.05.2012
...
Процедура флУточнитьВремяОтчетаПриИзменении(Элемент)
