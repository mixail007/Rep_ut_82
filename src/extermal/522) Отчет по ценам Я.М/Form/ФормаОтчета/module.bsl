...
Процедура кнОбновитьКампанииНажатие(Элемент)
	...
	WinHttp.setRequestHeader("Authorization", "OAuth oauth_token=1f94b0cfc82d42e9b9efdeb5f3609a79,oauth_login=kolesoyst,oauth_client_id=b85a2f83710f46249a2786b6243167f3");
	...
	Иначе	
		...
		Для каждого эл из Регионы Цикл
			Если Эл.Пометка тогда
				...
				ВывестиОтчетНарушениеМРЦ(Эл.Представление, контакты);
...
Функция ВывестиОтчетНарушениеМРЦ(Регион, Контакты = неопределено)
	...
	Иначе
		...
		Если НашаЦена=Справочники.ТипыЦенНоменклатуры.ПустаяСсылка() тогда
			...
			МРЦ = ОбменСУТИнтернетМагазин.ПолучитьЦеныДляКонтрагента_РегСв(Справочники.Контрагенты.НайтиПоКоду("11011"),СписокНом);
			...
			Запрос.Текст="ВЫБРАТЬ РАЗЛИЧНЫЕ
			             ...
			             |ГДЕ
			             |	НЕ втОснова.МРЦ = втОснова.price
			             |
			             |УПОРЯДОЧИТЬ ПО
			             |	Ответственный,
			             |	ИнтернетМагазин,
		...
		ОбластьРегион.Параметры.Регион = "Расхождения МРЦ "+Регион;
