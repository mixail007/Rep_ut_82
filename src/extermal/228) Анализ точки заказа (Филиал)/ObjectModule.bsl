...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	ПостроительОтчета.Текст = ТекстЗапросаСОтборомПоПодразделению;
	...
	Если ОбщийОтчет.Показатели.Найти("КоличествоЗаказано", "Имя") = Неопределено Тогда
		...
		ОбщийОтчет.ЗаполнитьПоказатели("КоличествоЗаказано", "Количество заказано", истина, "ЧЦ=15; ЧДЦ=3");
...
КонецПроцедуры
 ...
 // для Филиалов
    ТекстЗапросаСОтборомПоПодразделению = "ВЫБРАТЬ
	 ...
	 |   		ИЗ
	 |   		РегистрСведений.ЗначенияТочкиЗаказа.СрезПоследних(&ДатаКон, Подразделение <> &ПустоеПодразделение {Подразделение.* ,Номенклатура.* } ) КАК ЗначенияТочкиЗаказа
	 |   				
	 |   			ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыНаСкладах.Остатки(&ДатаКон, {Склад.Подразделение.* Подразделение, Номенклатура.*} Склад.Транзитный ) КАК ТоварыНаСкладах
	 ...
	 |  ЗаказПокупателя.Проверен И  ЗаказПокупателя.Транзит {ЗаказПокупателя.Подразделение.* Подразделение, Номенклатура.*} 
	 ...
	 |   		РегистрСведений.ЗначенияТочкиЗаказа.СрезПоследних(&ДатаКон, Подразделение =&ПустоеПодразделение {Подразделение.* ,Номенклатура.* } ) КАК ЗначенияТочкиЗаказа
	 |   				
	 |   			ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыНаСкладах.Остатки(&ДатаКон, {Склад.Подразделение.* Подразделение, Номенклатура.*} НЕ Склад.Транзитный ) КАК ТоварыНаСкладах
	 |   			ПО ЗначенияТочкиЗаказа.Номенклатура = ТоварыНаСкладах.Номенклатура 
	 |  ГДЕ  ЗначенияТочкиЗаказа.СпособОпределенияЗначенияТочкиЗаказа = &Фиксированная
	 |   		) КАК ВложенныйЗапрос
	 |   			ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗаказыПокупателей.Остатки(&ДатаКон, 
	 |  ЗаказПокупателя.Проверен И  НЕ ЗаказПокупателя.Транзит {ЗаказПокупателя.Подразделение.* Подразделение, Номенклатура.*} 
	 |) КАК ЗаказыПокупателей
	 |   			ПО ВложенныйЗапрос.Номенклатура = ЗаказыПокупателей.Номенклатура
	 |				ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗаказыПоставщикам.Остатки(&ДатаКон,НЕ ЗаказПоставщику.Транзит {ЗаказПоставщику.Подразделение.* Подразделение}) КАК ЗаказыПоставщикам
