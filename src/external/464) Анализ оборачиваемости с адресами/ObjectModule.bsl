...
Перем СтруктураОтбораПоКоэффициенту Экспорт;

Перем ТЗПартии Экспорт;

Перем ТЗПервоеПоступление;
...
Процедура РассчитатьПоказатели(РезультатЗапроса,ПоследняяГруппировка,КОборачиваемости=0,СрокХранения=0)
Если (ПоследняяГруппировка) Тогда	
		...
		КОборачиваемости = ?(Остаток = 0, ?(Расход>0,1000,0), Расход / Остаток);
...
Процедура ВывестиПоказатели(РезультатЗапроса, КоличествоПоказателей, ДокументРезультат, МакетПоказатель, ПоследняяГруппировка,МакетИзмерение)
	Перем Расход, Остаток, КОборачиваемости, СрокХранения, ПролежитЛет;
	//МакетСтроки = МакетПоказатель;
	Если (ПоследняяГруппировка) Тогда
		Если Периодичность <> Перечисления.Периодичность.ПустаяСсылка() Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				КонечныйОстаток = Выборка.КонечныйОстаток;
                СтоимостьОстаток=Выборка.СтоимостьОстаток;
				Остаток = Остаток + КонечныйОстаток;
				КоличествоКонтрольныхТочек = КоличествоКонтрольныхТочек + 1;
				ПрибавитьПериод(ТочкаОтсчета);
			КонецЦикла;
		...
		Для Каждого Показатель Из Показатели Цикл
			Если Показатель.Использование Тогда
				...
				ИначеЕсли Показатель.Имя = "КонечныйОстаток" Тогда
					Если НЕ РезультатЗапроса.Номенклатура.ЭтоГруппа Тогда	
						НайдСтрока= ТЗПартии.Найти(РезультатЗапроса.Номенклатура,"Номенклатура");
						Если НайдСтрока<>Неопределено Тогда
							МакетПоказатель.Параметры.КонечныйОстаток= НайдСтрока.КоличествоОстаток ;
						Иначе
							МакетПоказатель.Параметры.КонечныйОстаток= 0;
						КонецЕсли;
					Иначе
						МакетПоказатель.Параметры.КонечныйОстаток= 0;
					КонецЕсли;
				ИначеЕсли Показатель.Имя = "СтоимостьОстаток" Тогда
					Если НЕ РезультатЗапроса.Номенклатура.ЭтоГруппа Тогда	
						НайдСтрока= ТЗПартии.Найти(РезультатЗапроса.Номенклатура,"Номенклатура");
						Если НайдСтрока<>Неопределено Тогда
							МакетПоказатель.Параметры.СтоимостьОстаток= НайдСтрока.СтоимостьОстаток ;
						Иначе
							МакетПоказатель.Параметры.СтоимостьОстаток= 0;
						КонецЕсли;
					Иначе
						МакетПоказатель.Параметры.СтоимостьОстаток= 0;
					КонецЕсли;
				ИначеЕсли Показатель.Имя = "ПролежитЛет" Тогда
					Если Расход >= 0 и НайдСтрока <> Неопределено Тогда
						Если НайдСтрока.КоличествоОстаток > 0 и Расход >0 Тогда
						КоличествоДнейПериода = (ДатаКонца-ДатаНачала)/86400;
						ГодовойОборот = Расход * 365 / КоличествоДнейПериода;
					    ПролежитЛет = НайдСтрока.КоличествоОстаток / ГодовойОборот;
					    МакетПоказатель.Параметры.ПролежитЛет = ПролежитЛет;
					    ИначеЕсли НайдСтрока.КоличествоОстаток > 0 и Расход =0 Тогда
						МакетПоказатель.Параметры.ПролежитЛет = 100;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		...
		отбор.Вставить("Подразделение",Справочники.Подразделения.ПустаяСсылка());
		
		пам = мАдреса.Скопировать(Отбор);
		
		
		//МакетСтроки.Параметры.Очистить();
		
		если пам.Количество() > 0 Тогда
			ДокументРезультат.Вывести(МакетИзмерение, Выборка.Уровень()-1);
			
			ОбластьОформления = ДокументРезультат.Присоединить(МакетПоказатель);
			
		    ДокументРезультат.Область(ОбластьОформления.Верх, 2, ОбластьОформления.Низ, 11).ЦветФона = WebЦвета.Голубой;
		Для каждого стр из пам Цикл
		    МакетСтрока = ЭтотОбъект.ПолучитьМакет("Макет").ПолучитьОбласть("СтрокаПроизвольная");
		//ДокументРезультат.Вывести(МакетИзмерение, Выборка.Уровень());
		    МакетСтрока.Параметры.Адрес = стр.Склад.Наименование + ", АДРЕС : " + строка(стр.Адрес);
			МакетСтрока.Параметры.Количество = стр.Количество;
			ДокументРезультат.Вывести(МакетСтрока, Выборка.Уровень());
		КонецЦикла;
		    
		//Иначе
		//	ОбластьОформления = ДокументРезультат.Присоединить(МакетПоказатель);
		//	ОбластьОформления2 = ДокументРезультат.Область(ОбластьОформления.Верх, 1, ОбластьОформления.Низ, 11);
		//	Смещать = ТипСмещенияТабличногоДокумента.ПоВертикали;
		//	ДокументРезультат.УдалитьОбласть(ОбластьОформления,Смещать);
		КонецЕсли;
		// Макет показателей
...
Процедура ВывестиСтроку(РезультатЗапроса, Знач Индекс, КоличествоКолонок, КоличествоПоказателей, ДокументРезультат)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ТипЗаписи() = ТипЗаписиЗапроса.ИтогПоГруппировке Тогда
			МакетСтроки.Параметры.Код = МакетСтроки.Параметры.Номенклатура.Код;
			
			
			СтрокаПервоеПоступление = ТЗПервоеПоступление.Найти(МакетСтроки.Параметры.Номенклатура,"Номенклатура");
			Если (СтрокаПервоеПоступление<>Неопределено ) Тогда
				МакетСтроки.Параметры.ДатаПервойЗакупки = Формат(СтрокаПервоеПоступление.ДатаПервойЗакупки,"ДФ=dd.MM.yyyy");
			КонецЕсли;		
		КонецЕсли;
		//Орлов --
		
		МакетСтроки.Параметры.КонечныйОстаток=0;
		МакетСтроки.Параметры.СтоимостьОстаток=0;

		МакетИзмерение = МакетСтроки.ПолучитьОбласть(1,1,МакетСтроки.ВысотаТаблицы,1+КоличествоКолонок);

		Если НЕ ЗначениеНеЗаполнено(СтруктураОтбораПоКоэффициенту.Использование) Тогда  Кобор=0;ЧислоДней=0; КонецЕсли;

		Если  ПоследняяГруппировка Тогда
			РассчитатьПоказатели(Выборка,ПоследняяГруппировка,Кобор,ЧислоДней);
		КонецЕсли;	
		
		 Если ЗначениеНеЗаполнено(СтруктураОтбораПоКоэффициенту.Использование) ИЛИ НЕ ПоследняяГруппировка ИЛИ Выборка.Номенклатура.ЭтоГруппа Тогда
				...
				ВывестиПоказатели(Выборка, КоличествоПоказателей, ДокументРезультат, МакетПоказатель, ПоследняяГруппировка,МакетИзмерение);
				
			Иначе
				Если СтруктураОтбораПоКоэффициенту.Использование Тогда
				Если (СтруктураОтбораПоКоэффициенту.ВидСравнения=ВидСравнения.Меньше И Кобор<СтруктураОтбораПоКоэффициенту.Значение ) ИЛИ
				(СтруктураОтбораПоКоэффициенту.ВидСравнения=ВидСравнения.МеньшеИлиРавно И Кобор<=СтруктураОтбораПоКоэффициенту.Значение ) ИЛИ
				(СтруктураОтбораПоКоэффициенту.ВидСравнения=ВидСравнения.Больше И  Кобор>СтруктураОтбораПоКоэффициенту.Значение) ИЛИ
				(СтруктураОтбораПоКоэффициенту.ВидСравнения=ВидСравнения.БольшеИлиРавно И  Кобор>=СтруктураОтбораПоКоэффициенту.Значение ) ИЛИ
				(СтруктураОтбораПоКоэффициенту.ВидСравнения=ВидСравнения.Интервал И Кобор>СтруктураОтбораПоКоэффициенту.ЗначениеС И Кобор<СтруктураОтбораПоКоэффициенту.ЗначениеПо) ИЛИ
				(СтруктураОтбораПоКоэффициенту.ВидСравнения=ВидСравнения.ИнтервалВключаяГраницы И  Кобор>=СтруктураОтбораПоКоэффициенту.ЗначениеС И Кобор<=СтруктураОтбораПоКоэффициенту.ЗначениеПо) ИЛИ
				(СтруктураОтбораПоКоэффициенту.ВидСравнения=ВидСравнения.ИнтервалВключаяНачало И Кобор>=СтруктураОтбораПоКоэффициенту.ЗначениеС И Кобор<СтруктураОтбораПоКоэффициенту.ЗначениеПо) ИЛИ
				(СтруктураОтбораПоКоэффициенту.ВидСравнения=ВидСравнения.ИнтервалВключаяОкончание И  Кобор>СтруктураОтбораПоКоэффициенту.ЗначениеС И Кобор<=СтруктураОтбораПоКоэффициенту.ЗначениеПо)  Тогда			
				 //ДокументРезультат.Вывести(МакетИзмерение, Выборка.Уровень());
...
Процедура УстановитьТекстЗапросаПостроителяОтчета() Экспорт
	...
	|	ВложенныйЗапрос.СтоимостьОстаток КАК СтоимостьОстаток,
	|	ВЫБОР
	|		КОГДА ВложенныйЗапрос.Расход ЕСТЬ NULL 
	|				ИЛИ ВложенныйЗапрос.Расход = 0
	|			ТОГДА 0
	|		ИНАЧЕ ВложенныйЗапрос.КонечныйОстаток / ВложенныйЗапрос.Расход * (365 / (ДЕНЬГОДА(&ДатаКонца) - ДЕНЬГОДА(&ДатаНачала)))
	|	КОНЕЦ КАК ПролежитЛет
	|{ВЫБРАТЬ
	|	Склад.*,
	|	Номенклатура.*,
	...
	|		0 КАК Расход,
	|		ПартииТоваровНаСкладахОстаткиИОбороты.СтоимостьКонечныйОстаток КАК СтоимостьОстаток
	|	ИЗ
	...
	|										РегистрНакопления.ТоварыНаСкладах.Номенклатура
	|						ИМЕЮЩИЕ
	|							МИНИМУМ(РегистрНакопления.ТоварыНаСкладах.Период) < &ДатаПервойЗакупки) {(Склад).* КАК Склад, (Номенклатура).* КАК Номенклатура, (ХарактеристикаНоменклатуры).* КАК ХарактеристикаНоменклатуры}) КАК ПартииТоваровНаСкладахОстаткиИОбороты
	|	ГДЕ
	|		(&ВсеТовары = ИСТИНА
	...
	|	ГДЕ
	|		(&НеФлагОтборЗакупка
	|				ИЛИ ПартииТоваровНаСкладах.Номенклатура В
	|					(ВЫБРАТЬ
	|						РегистрНакопления.ТоварыНаСкладах.Номенклатура
	|					ИЗ
	|						РегистрНакопления.ТоварыНаСкладах
	...
	|									РегистрНакопления.ТоварыНаСкладах.Номенклатура
	|					ИМЕЮЩИЕ
	|						МИНИМУМ(РегистрНакопления.ТоварыНаСкладах.Период) < &ДатаПервойЗакупки))
	|		И (&ВсеТовары = ИСТИНА
	|				ИЛИ &ТоварыНаСкладах = ИСТИНА)
	...
	|		ПартииТоваровНаСкладах.ХарактеристикаНоменклатуры.* КАК ХарактеристикаНоменклатуры}) КАК ВложенныйЗапрос
	|ГДЕ
	|	ИСТИНА
	|{ГДЕ
	|	ВложенныйЗапрос.Склад.* КАК Склад,
	...
	//	ТекстКатегорий = "";
	//	
	//	ДобавитьВТекстСвойстваИКатегории(ТаблицаПолей, ТекстЗапроса, СтруктураПредставлениеПолей, мСоответствиеНазначений, 
	...
	//	ДобавитьВТекстСвойстваОбщие(ТекстЗапроса, ТекстСвойств, "//ОБЩИЕСВОЙСТВА");
	//	
	//КонецЕсли;
	...
	//Если ИспользоватьСвойстваИКатегории Тогда
	//	УстановитьТипыЗначенийСвойствИКатегорийДляОтбора(ПостроительОтчета, ТекстКатегорий, ТекстСвойств, мСоответствиеНазначений, СтруктураПредставлениеПолей);
	//КонецЕсли;	
...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	//Орлов ++
	ЗаполнитьПоказатели("Код",          "Код",
	                    ИСТИНА,   "");
	ЗаполнитьПоказатели("ДатаПервойЗакупки",  "Дата первой закупки",
	                    ИСТИНА,   "");
	...
	ЗаполнитьПоказатели("СрокХранения",     "Средний срок хранения в днях",
	                    ИСТИНА,   "ЧЦ = 15; ЧДЦ = 2");
	ЗаполнитьПоказатели("КонечныйОстаток",     "Конечный остаток",
						ИСТИНА,   "ЧЦ = 15; ЧДЦ = 0");
	ЗаполнитьПоказатели("СтоимостьОстаток",     "Стоимость",
						ИСТИНА,   "ЧЦ = 15; ЧДЦ = 0");
	ЗаполнитьПоказатели("ПролежитЛет",     "Срок хранения остатков(в годах)",
						ИСТИНА,   "ЧЦ=4; ЧДЦ=1");
	...
	Поле = ДоступныеПоляОтбора.Добавить("КоэффициентОборачиваемости", "Коэффициент оборачиваемости", 
	Новый ОписаниеТипов("Число"));
	Поле.Отбор = Истина;
	
	
	Поле = ДоступныеПоляОтбора.Добавить("ДатаПервойЗакупки", "ДатаПервойЗакупки", 
...
запрос.Текст = "ВЫБРАТЬ
               ...
               |	ТоварыАдресноеХранениеОстатки.АдресХранения КАК Адрес,
               |	ТоварыАдресноеХранениеОстатки.КоличествоОстаток КАК Количество,
               |	ТоварыАдресноеХранениеОстатки.Склад.Подразделение КАК Подразделение,
               |	ТоварыАдресноеХранениеОстатки.Склад
               |ИЗ
               |	РегистрНакопления.ТоварыАдресноеХранение.Остатки(, Склад В ИЕРАРХИИ (&Склад)) КАК ТоварыАдресноеХранениеОстатки";
мАдреса = запрос.Выполнить().Выгрузить();
	
	
ПОтчета=Новый ПостроительОтчета;

ПОтчета.Текст="ВЫБРАТЬ Номенклатура КАК Номенклатура,
	|		КоличествоОстаток КАК КоличествоОстаток,
	|		СтоимостьОстаток КАК СтоимостьОстаток
	|	ИЗ
	|		РегистрНакопления.ПартииТоваровНаСкладах.Остатки(&ДатаКонца, {Склад.*, Номенклатура.*})";	
ПОтчета.Параметры.Вставить("ДатаКонца",?(ПоОстаткамНаНачало, НачалоДня(ДатаНачала), КонецДня(ДатаКонца))   ); //+++ 28.09.2015

Для каждого Элемент ИЗ ПостроительОтчета.Отбор Цикл
	Если 	Элемент.ПутьКДанным<>"КоэффициентОборачиваемости"  И Элемент.ПутьКДанным<>"ДатаПервойЗакупки" Тогда
		СтрокаОтбора=ПОтчета.Отбор.Добавить(Элемент.ПутьКДанным);	
		СтрокаОтбора.Использование = Элемент.Использование;
		СтрокаОтбора.ВидСравнения = Элемент.ВидСравнения;
		СтрокаОтбора.Значение = Элемент.Значение;
	КонецЕсли;	
КонецЦикла;	

 ЗапросСтоимостьТоваров = ПОтчета.ПолучитьЗапрос();
 ТЗПартии=ЗапросСтоимостьТоваров.Выполнить().Выгрузить();
 
 //---
 
 ЗапросПервоеПоступление = Новый Запрос();
 
 ЗапросПервоеПоступление.Текст ="ВЫБРАТЬ Номенклатура, МИНИМУМ(Период) ДатаПервойЗакупки  ИЗ РегистрНакопления.ТоварыНаСкладах СГРУППИРОВАТЬ ПО Номенклатура" ;
 
 ТЗПервоеПоступление =  ЗапросПервоеПоступление.Выполнить().Выгрузить();
//ТЗПервоеПоступление=ЗапросПервоеПоступление.Выполнить().Выгрузить();

//ТЗПартии=ЗапросПартии.Выполнить().Выгрузить();

	
	
СтруктураОтбораПоКоэффициенту=Новый Структура("Использование,ВидСравнения,Значение,ЗначениеС,ЗначениеПо");	
	...
	Для Сч=0 По ПостроительОтчета.Отбор.Количество()-1 Цикл
		Если ПостроительОтчета.Отбор[Сч].Использование Тогда
			...
			Если ПостроительОтчета.Отбор[Сч].Представление="Коэффициент оборачиваемости" Тогда
				СтруктураОтбораПоКоэффициенту.Использование=Истина;
				ЗаполнитьЗначенияСвойств(СтруктураОтбораПоКоэффициенту,ПостроительОтчета.Отбор[Сч]);
			КОнецЕсли;	
		КонецЕсли;
	КонецЦикла; 
	...
	Запрос.УстановитьПараметр("ТоварыУКомиссионеров", СтатусТоваров = "ТоварыУКомиссионеров");
	Запрос.УстановитьПараметр("ДатаПервойЗакупки",ТекущаяДата()); // заглушка
	
	НеФлагОтборЗакупка = Истина;
// --- Дата первой закупки	
	Для сч =0 по  ПостроительОтчета.Отбор.Количество()-1 Цикл
		
		УсловиеОтбора =ПостроительОтчета.Отбор[сч];
		Если ( УсловиеОтбора.ПутьКДанным = "ДатаПервойЗакупки" )  И УсловиеОтбора.Использование  Тогда
	    //ПостроительОтчета.Параметры.Вставить("ДатаПервойЗакупки" ,УсловиеОтбора.Значение);
		Запрос.УстановитьПараметр("ДатаПервойЗакупки",УсловиеОтбора.Значение);
		НеФлагОтборЗакупка = Ложь;
		Прервать;
		КонецЕсли;	
	КонецЦикла;	

	Запрос.УстановитьПараметр("НеФлагОтборЗакупка", НеФлагОтборЗакупка);
	...
	Для Сч = 0 По ПостроительОтчета.ИзмеренияСтроки.Количество()-1 Цикл
		...
		//		ИмяПоля    = ПостроительОтчета.ВыбранныеПоля[Инд].Имя;
		//		ШаблонПоля = "["+ИмяПоля+"]";
		//		Макет.Область(ОбластьСтрока.Верх,1+КоличествоКолонок+1+Инд+6).Текст = ИмяПоля;
...
мКодыОперацийПродажи.Добавить(Перечисления.КодыОперацийПартииТоваров.Комплектация);
