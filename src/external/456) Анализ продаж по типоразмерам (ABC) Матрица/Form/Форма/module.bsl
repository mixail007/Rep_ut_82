...
//============из 4 закладки=============================================	
Если ЗначениеЗаполнено(ЗаказПлан) или ЗначениеЗаполнено(ЗаказМатрица) тогда
...
//===========по Плану Закупок!=============================================	
Если НЕ ЗначениеЗаполнено(ЗаказПлан) или НЕ ЗначениеЗаполнено(ЗаказМатрица) тогда
...
//		стр2.Количество = стр1.Количество 				* стр0.КолC/100; 
		...
		Если Фл4 тогда   //14.01.2019 --- ТУТ НЕ НАДО !!!
			Для каждого стр2 из стр1.Строки цикл
		...
		Если Дерево1.Строки.Итог("КоличествоМатрица") >= Емкость тогда //14.01.2019
			Сообщить("Достигнут максимум загрузки склада по Емкости!");
			продолжить;
		КонецЕсли;	
	
		//+++( 25.10.2018 - Задача № 42972 - п.16 -------------------------------------
		Если ФлБезПересчета тогда 	
			КолМин = 4 / (ПроцентМатрицы/100);  //+++ 17.12.2018 - чтобы на матрицу хватало!
			Для каждого стр2 из стр1.Строки цикл
				...
				Для каждого стр22 из стр11.Строки цикл
					...
					КонецЕсли;
				КонецЦикла;
				стр2.КоличествоПлан    = стр2.строки.Итог("КоличествоПлан");
				стр2.КоличествоМатрица = стр2.строки.Итог("КоличествоМатрица");
				стр1.КоличествоПлан    = стр1.строки.Итог("КоличествоПлан");
				стр1.КоличествоМатрица = стр1.строки.Итог("КоличествоМатрица");
				
				Если Фл4 тогда //+++( 14.01.2019 обрезание по количеству Класса!
					КолПланКласса    = стр2.КоличествоПлан;
					КолМатрицаКласса = стр2.КоличествоМатрица;
					
			    	Если КолПланКласса>стр2.Строки.Итог("КоличествоПлан") тогда
			    		дельтаПлан = КолПланКласса - стр2.Строки.Итог("КоличествоПлан");
			    		стр2.Строки.Сортировать("КоличествоПлан убыв");
			    		Для каждого стр3 из стр2.Строки цикл
			    			Если дельтаПлан>=12 и стр3.КоличествоПлан>=12 тогда
			    				стр3.КоличествоПлан = стр3.КоличествоПлан - 12;
			    				дельтаПлан = дельтаПлан - 12;
			    			ИначеЕсли дельтаПлан>=8 и стр3.КоличествоПлан>=8 тогда
			    				стр3.КоличествоПлан = стр3.КоличествоПлан - 8;
			    				дельтаПлан = дельтаПлан - 8;
			    			иначеЕсли дельтаПлан>=4 и стр3.КоличествоПлан>=4 тогда
			    				стр3.КоличествоПлан = стр3.КоличествоПлан - 4;
			    				дельтаПлан = дельтаПлан - 4;
			    			КонецЕсли;	
			    		КонецЦикла;
			    	КонецЕсли;//План	
			    	
			    	Если КолМатрицаКласса>стр2.Строки.Итог("КоличествоМатрица") тогда
			    		дельтаМатрица = КолМатрицаКласса - стр2.Строки.Итог("КоличествоМатрица");
			    		стр2.Строки.Сортировать("КоличествоМатрица убыв");
			    		Для каждого стр3 из стр2.Строки цикл
			    			Если дельтаМатрица>=12 и стр3.КоличествоМатрица>=12 тогда
			    				стр3.КоличествоМатрица = стр3.КоличествоМатрица - 12;
			    				дельтаМатрица = дельтаМатрица - 12;
			    			ИначеЕсли дельтаПлан>=8 и стр3.КоличествоМатрица>=8 тогда
			    				стр3.КоличествоМатрица = стр3.КоличествоМатрица - 8;
			    				дельтаМатрица = дельтаМатрица - 8;
			    			иначеЕсли дельтаПлан>=4 и стр3.КоличествоМатрица>=4 тогда
			    				стр3.КоличествоМатрица = стр3.КоличествоМатрица - 4;
			    				дельтаМатрица = дельтаМатрица - 4;
			    			КонецЕсли;	
			    		КонецЦикла;
			    	КонецЕсли;//План	
			   КонецЕсли;
...
функция ПолучитьПланПродажПоСезоннымЗаказам()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |		Документ.ЗаказПокупателяСезонный.Товары КАК ЗаказПлан
	               |	ГДЕ
	               |		ЗаказПлан.Ссылка В (&ЗаказМатрица) ) КАК ВсеЗаказы
	               |
	               |СГРУППИРОВАТЬ ПО
...
Процедура Кнопка1Нажатие(Элемент)
	 ...
	 |	И ЗаказПокупателяСезонный.Дата > &Дата1
	 |	И ЗаказПокупателяСезонный.Комментарий ПОДОБНО (&Комментарий)
	 |";
	 Если НЕ Филиал.Пустая() тогда
		Запрос.Текст = Запрос.Текст + "И ЗаказПокупателяСезонный.Контрагент = &Контр";
	    Запрос.УстановитьПараметр("Контр", Филиал.Контрагент);
	 КонецЕсли;	
	 Запрос.УстановитьПараметр("Дата1", Дата1);
	 Запрос.УстановитьПараметр("Комментарий", "# Формирование Плана на сезон (456 отчет): %");
	 
	 ЗаказПлан.Очистить();
	 Результат = Запрос.Выполнить();
	 табл = Результат.Выгрузить();
	 ЗаказПлан.ЗагрузитьЗначения( табл.ВыгрузитьКолонку("Ссылка") );
	 
	ЗаказМатрица.Очистить();
	Запрос.УстановитьПараметр("Комментарий", "# Формирование МАТРИЦЫ мин.остатков (456 отчет): %");
	 Результат = Запрос.Выполнить();
	 табл = Результат.Выгрузить();
	 ЗаказМатрица.ЗагрузитьЗначения( табл.ВыгрузитьКолонку("Ссылка") );
