...
//======================================================================
	...
	Пока ВыборкаДетали.Следующий() Цикл
		...
		//Если стрМРЦ=неопределено тогда ценаМРЦ = 0;
		...
		КонецЕсли;
			...
			//+++ 14.12.2018 - давно уже из свойства!
			//НоменклатураНаименование=СокрЛП(ВыборкаДетали.НоменклатураНаименование); 
...
функция ВыгрузитьДанные(Excel, номСтр=1, НазвСтр="Ярославль")
	...
	Иначе	
		...
		ExcelЛист.cells(7,2).value = "Дата формирования: "+формат(ТекущаяДата(),"ДФ='dd MMMM yyyy ""г."" (HH:mm)'");
	КонецЕсли;	
...
Процедура ДействияФормыВыгрузитьВExcel(Кнопка, путь0=неопределено ) Экспорт
	...
	для нПерек=0 по нПерекМакс цикл  //сначала для клиентов, потом для менеджеров
		...
		иначе	
			...
			i=11; Nmax = 3232; //3232 строк 14/12/2018!
			пока i<Nmax цикл	
				ОбработкаПрерыванияПользователя();
