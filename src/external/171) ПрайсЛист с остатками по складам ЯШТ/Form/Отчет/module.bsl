...
Процедура Отчет(ТабДок, ТолькоБрак = ЛОЖЬ, флАртикул=ЛОЖЬ) Экспорт
	...
	//+++ 05.04.2019
	флПокупатель = НЕ ТолькоБрак И НЕ ВыводитьПрименяемость И НЕ Покупатель.Пустая();
	
	Если флПокупатель тогда // защита от неограниченного просмотра продаж
		Если НачДата<'20160101' тогда
			Предупреждение("Период продаж не должен быть ранее 01.01.2016г.",30);
			возврат;
		КонецЕсли;	
		Если КонДата<НачДата тогда
			КонДата=НачДата;
		КонецЕсли;	
		Если КонДата<'20160101' тогда
			Предупреждение("Период продаж не должен быть ранее 01.01.2016г.",30);
			возврат;
		КонецЕсли;	
	КонецЕсли;
...
КонецЕсли;
	Если НЕ ВыводитьПрименяемость тогда	
		...
		|///////////////////////////////////////////////////////////////
		|", "")+
	//+++ 05.04.2019 - прямо в запрос! 	
		?(флПокупатель,"ВЫБРАТЬ
         |	ПродажиОбороты.Номенклатура как НоменклатураПродано,
         |	ПродажиОбороты.КоличествоОборот как КоличествоПродано,
         |	ПродажиОбороты.СтоимостьОборот как СтоимостьПродано
		 |ПОМЕСТИТЬ ВТ_Продажи
         |ИЗ
         |	РегистрНакопления.Продажи.Обороты(&НачДата, &КонДата, , ДоговорКонтрагента.Владелец = &Покупатель) КАК ПродажиОбороты		
		 |;
		|///////////////////////////////////////////////////////////////
		|","")+
		...
		//+++ 05.04.2019 - прямо в запрос! 	
		+?(флПокупатель, "
		|	ЕстьNull(прод.КоличествоПродано,0) как КоличествоПродано, 
		|	ЕстьNull(прод.СтоимостьПродано,0) как СтоимостьПродано,
		|","")
	...
	// --- 11.10.2018 - список уже 3х договоров	---
		...
		//+++ 05.04.2019 - прямо в запрос! 	
		|"+?(флПокупатель," ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Продажи как Прод
		|	ПО Б.Номенклатура = Прод.НоменклатураПродано", "")+"
...
//============ 09.02.2017 = (доп.закладки "Грузовые диски" - только для Менеджеров (и в доп.закладку Груз.Диски!) ==============		   
 ...
 //?(ВидОтчета = 0, "",
	...
	Если USD и не ВыводитьПрименяемость тогда
		...
		Если НЕ Покупатель.Пустая() тогда //+++ 05.04.2019
			Запрос1.Параметры.Вставить("НачДата", НачДата);
			Запрос1.Параметры.Вставить("КонДата", КонецДня(КонДата) );
			Запрос1.Параметры.Вставить("Покупатель", Покупатель);
		КонецЕсли;	
...
//+++)
	...
	Иначе //=========================================================================================================================
		...
		Если НЕ Покупатель.Пустая() тогда //+++ 05.04.2019
			...
			ПостроительОтчетаОтчет.Параметры.Вставить("Покупатель", Покупатель);
		КонецЕсли;	
	...
	Если флДиски и не флПокупатель тогда //+++ 05.04.2019 без последних столбцов
	...
	Если флДиски и не флПокупатель тогда //+++ 05.04.2019 без последних столбцов
		ОбластьДетальныхЗаписей = Макет.ПолучитьОбласть("Детали|Основная");
	иначе//как было 
		ОбластьДетальныхЗаписей = Макет.ПолучитьОбласть("Детали");
	КонецЕсли;
...
Процедура ПриОткрытии()
	...
	//+++ 05.04.2019 - продажи за ТЕК.месяц
...
//+++ 17.03.2015 - через буфер
	...
	Excel.Application.Visible = 0;//04.04.2019
		//ExcelЛист.Range( ExcelЛист.cells(11,1), ExcelЛист.cells(11,1) ).Select();
		ExcelЛист.Range( ExcelЛист.cells(11,1), ExcelЛист.cells(N-k,M) ).Select();
		...
		Если назвСтр="Аксессуары" тогда стФото=11;    // 6400    200/сек + 32 сек
		  ИначеЕсли назвСтр="АКБ" тогда стФото=15;
		  ИначеЕсли назвСтр="Диски" тогда стФото=24; // 15400   + 75 сек
		  иначе стФото=0;
		КонецЕсли;
		i=10; Сообщить("Идёт добавление гиперСсылок на фото...");
		пока стФото>0 и i<N цикл //04.04.2019
			i=i+1; ОбработкаПрерыванияПользователя();
			добавитьГиперСсылку(ExcelЛист, i, стФото);
			если i%200=0 тогда
				состояние("Идёт создание гиперссылки для фото "+строка(i)+" / "+строка(N-10)+" ("+формат(100*i/(N-10),"ЧДЦ=1")+"%) строк...");
			КонецЕсли;	
		КонецЦикла;
...
процедура добавитьГиперСсылку(ExcelЛист, i=1,стФото=1)
		ссылкаНаФото = ExcelЛист.cells(i,стФото).Text; 
		//ExcelЛист.Range( ExcelЛист.cells(i,стФото),ExcelЛист.cells(i,стФото) ).Select();
			// Add( _Anchor_ , _Address_ , _SubAddress_ , _ScreenTip_ , _TextToDisplay_ , _Target_ )
		Anchor = ExcelЛист.cells(i,стФото); //Excel.Application.Selection;
		href = ExcelЛист.Hyperlinks.Add(Anchor, ссылкаНаФото, , "Фото", ссылкаНаФото);
КонецПроцедуры
	...
	для нПерек=0 по 1 цикл  
		...
		иначе	
			...
			Если НеВыводитьДискиПодЗаказ=ЛОЖЬ тогда 
				i=11; k=0; Nmax = 14600; //до 14500 строк 04.04.2019!
				Состояние("Идёт раскраска дисков под заказ...");
				стФото = 24;
				пока i<Nmax цикл	
					ОбработкаПрерыванияПользователя();
					Если ExcelЛист.cells(i,25).Text<>"" тогда//есть под заказ 7 или 30 дн.
					 	k=k+1;
						ExcelЛист.Range( ExcelЛист.cells(i,1), ExcelЛист.cells(i,28) ).Select();
						Excel.Application.Selection.Interior.ColorIndex = 38; // 3; //красный  38 - розовый
						Excel.Application.Selection.Interior.Pattern = 1; //xlSolid;
						
						//добавитьГиперСсылку(ExcelЛист, i, стФото); //ТОЛЬКО раскрашенные 3500 -все 15,5 тыс
					...
					КонецЕсли;
					 ...
					 Если i%200=0 тогда 
						 проц = цел(100*i/Nmax);
						 Состояние("Идёт раскраска дисков под заказ. Пройдено: "+строка(i)+" / "+строка(Nmax)+" ("+формат(проц,"ЧДЦ=1")+"%) строк. Раскрашено: "+строка(k)+" строк ("+формат(k*100/i,"ЧДЦ=0")+"%)",проц);
					 КонецЕсли;
				КонецЦикла;
			КонецЕсли;
...
Для ПереклНом = 0 по 1 цикл
	...
	//04.07.2016 --- раскраска ---
		...
		Если НеВыводитьДискиПодЗаказ тогда 
			Колонка1 = "Y";
		КонецЕсли;	
		//Если НЕ ВыводитьДискиСтопЛист тогда  //+++ 04.04.2019  УДАЛЯЕМ признак! Егор так сказал
			Колонка2="AA";
		//КонецЕсли;
...
Процедура ПокупательПриИзменении(Элемент)
	...
	ЕстьПокупатель = НЕ Покупатель.Пустая();
	
	Если ЕстьПокупатель тогда
		если НачДата<'20160101' тогда
			Предупреждение("Продажи нельзя смотреть ранее 2016г.", 30);
			НачДата='20160101';
		КонецЕсли;	
		
		Если КонДата<НачДата тогда
			Предупреждение("Дата окончания Продаж не может быть ранее даты начала...", 30);
			КонДата=НачДата;
		КонецЕсли;	
	...
	ЭлементыФормы.НачДата.Доступность = ЕстьПокупатель;
	ЭлементыФормы.КонДата.Доступность = ЕстьПокупатель;
	ЭлементыФормы.КнопкаВыбораПериода.Доступность = ЕстьПокупатель;
	
	// другой макет - не изменен!
	Если ВыводитьПрименяемость и ЕстьПокупатель тогда
		ВыводитьПрименяемость = НЕ ЕстьПокупатель; //выключаем
	КонецЕсли;	
    ЭлементыФормы.ВыводитьПрименяемость.Доступность = НЕ ЕстьПокупатель; 
	
КонецПроцедуры

Процедура ПокупательОчистка(Элемент, СтандартнаяОбработка)
	ПокупательПриИзменении(неопределено);
КонецПроцедуры
