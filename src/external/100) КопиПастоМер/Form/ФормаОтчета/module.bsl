Процедура КнопкаСформироватьНажатие(Кнопка)
	...
	КолСтрок = Повторы.Итог("Рост");
	ЭтаФорма.Заголовок = "КопиПастаМер - " + формат(ТочноеТекущееВремя() - Старт,"ЧДЦ=3") + " Cек, "  +", повтор: " + строка(СреднийРост)+" раз, "+формат(КолСтрок,"ЧГ=0")+"/" + формат(ВсегоСтрок,"ЧГ=0") + "("+формат(КолСтрок*100/ВсегоСтрок,"ЧДЦ=1")+"%), различных: " + строка(РазныхСтрок);
