...
Процедура КнопкаВыполнитьНажатие(Кнопка)
	...
	Если НайденныеФайлы<>Неопределено тогда
		...
		для каждого файл из НайденныеФайлы цикл
			...
			чтение.ОткрытьФайл(сокрЛП(файл.Значение.ПолноеИмя));
			...
			пока мРодитель<>неопределено цикл  		
				...
				//	мОбход=мРезультатЗапроса.ПолучитьСледующий();
				//конеццикла;                     //мРодитель.Атрибуты
				
				Атрибуты = мРодитель.Атрибуты;
				АтрибутыРодитель = мРодитель.РодительскийУзел.Атрибуты;
				
				Если Найти(Атрибуты.ПолучитьИменованныйЭлемент ("s").Значение,"Телеф")>0 тогда
					НовСтр = ТЗ.Добавить();
					НовСтр.Дата	 				= Атрибуты.ПолучитьИменованныйЭлемент ("d").Значение;  
					НовСтр.НомерСотрудника 		= СтрЗаменить(Прав( АтрибутыРодитель.ПолучитьИменованныйЭлемент ("n").Значение,10),"+7","8"); 		
					НовСтр.ДлительностьЗвонка 	= ПолучитьДлительностьИзСтроки( Атрибуты.ПолучитьИменованныйЭлемент ("du").Значение) ;
					НовСтр.Исход_Вход 			= ?(Найти(Атрибуты.ПолучитьИменованныйЭлемент ("n").Значение,"<--")>0,"Входящий","Исходящий");
					НовСтр.НомерАбонента 		= СтрЗаменить(Прав( Атрибуты.ПолучитьИменованныйЭлемент ("n").Значение,10),"+7","8"); 	
				КонецЕсли;
				мРодитель=мРезультат.ПолучитьСледующий();
			конеццикла;
	...
	иначе
		Сообщить("Нет файлов к загрузке!");
		Возврат;
...
Функция ПолучитьДлительностьИзСтроки(значение)
		Если ТипЗнч(значение) = Тип("Строка") Тогда
			массивДлит = РазложитьСтрокуВМассивПодстрок(значение,":");
			ЕСли массивДлит.Количество()=2 тогда
				Длит =число( массивДлит[0]) * 60 + число( массивДлит[1]); //)+ТекЛист.Cells(Ячейка,4).Value*86400;
			иначе
				Длит =число( массивДлит[0]) * 60 ; //)+ТекЛист.Cells(Ячейка,4).Value*86400;
...
Процедура ПутьКФайлуНачалоВыбора(Элемент, СтандартнаяОбработка)
	ЭлементыФормы.Найдено.Заголовок = "Найдено "; 
    ...
    Если ДиалогОткрытияФайла.Выбрать() Тогда 
		...
		НашлиФайлы = НайтиФайлы(ПутьКФайлу, "*.xml");
		Если ЗначениеЗаполнено( НайденныеФайлы) тогда
			НайденныеФайлы.Очистить();
		КонецЕсли;
		ЭлементыФормы.Найдено.Заголовок =ЭлементыФормы.Найдено.Заголовок + " " + НашлиФайлы.Количество(); 
		для каждого стр из НашлиФайлы цикл
			НайденныеФайлы.Добавить(стр);			
		КонецЦикла;
