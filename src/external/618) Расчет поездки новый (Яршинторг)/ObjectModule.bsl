...
// для Контрагента -> по адресу доставки, фактич. или юридич.адресу - в рс.КонтактнаяИнформация -> поле10
	 Если ЗначениеЗаполнено(стр1.Контрагент) и стр1.Координаты="" тогда
	  ...
	  Попытка
		...
		иначе
			стр1.Координаты = "";
			Сообщить("В стр."+строка(стр1.НомерСтроки)+"  - не заполнено поле ""Координаты"" по адресу - "+адрес1, СтатусСообщения.Внимание);
		КонецЕсли;
	  исключение
			рс = неопределено;
	  КонецПопытки;
	Иначе
		Сообщить("В стр."+строка(стр1.НомерСтроки)+" "+?(стр1.Координаты=""," - не заполнено поле ""Контрагент"" !"," Координаты точки: "+стр1.Координаты), СтатусСообщения.Внимание);
	КонецЕсли;	
