...
Процедура ЗаказыКоординатыОткрытие(Элемент, СтандартнаяОбработка)
	...
	попытка
     ...
     IE.Navigate("http://maps.google.com/maps?f=q&source=s_q&hl=ru&q="+ОбработанныйАдрес);  //+стр1.АдресДоставки+
	 IE.Visible = Истина;
	 
  	исключение
	  Предупреждение("Ошибка получения карты по адресу: "+ОбработанныйАдрес+"!
	  |"+ОписаниеОшибки() ,60);
	КонецПопытки;	
...
Процедура ОсновныеДействияФормыЗаданиеНаПеремещение(Кнопка)
	...
	Если НЕ (КолесаТУТКарточки или КолесаТУТ или КолесаТУТПИТСТОП или Диски или Тендеры или Маркировка или ПЭК) тогда //13.03.2019
