...
АдресИзИнтернета = ЛОЖЬ; //+++ 04.12.2018 не работает!
...
Функция ПолучитьТекстЗапросаБрак()
   ЗапросТекст = "ВЫБРАТЬ
                ...
                // |	ИтогПоВсем.ДоговорКонтрагента,
                 ...
                 |
   	//+++ 04.12.2018 -> сразу в запросе!  РезБрак.свернуть("Заявка,Контрагент,Адрес", "Объем, Вес");
                 |СГРУППИРОВАТЬ ПО
...
Процедура ДатаФормыПриИзменении(Элемент)
	...
	ЗапросБрак.УстановитьПараметр("Отклонить",Перечисления.СостоянияЗаявкиНаБрак.Отклонить);
