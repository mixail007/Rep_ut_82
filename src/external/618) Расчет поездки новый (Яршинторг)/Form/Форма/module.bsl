...
//+++ 26.03.2018 АдресИзИнтернета = Истина;

	Состояние("Идет обновление адресов...");
	ОбновитьАдресаДоставки(); //+++ 21.12.2017

	Заказы.Сортировать("Точка, Контрагент"); //автоматом!
 			...
 			если стр1.Точка>номТочки тогда
				...
				если СокрЛП(стр1.Координаты)<>"" тогда
 				...
 				иначе // по адресу
					...
					если Найти(Адрес2,":")>0 тогда
						Адрес2 = прав(Адрес2, стрДлина(Адрес2) - Найти(Адрес2,":"));
					КонецЕсли;
					рез = получитьМаршрут(Адрес1,Адрес2);
...
Процедура ДатаФормыПриИзменении(Элемент)
	...
	иначе  //неотгруженные всегда!
		...
		Запрос.Текст = "ВЫБРАТЬ
				...
				//21.08.2018	   
				//	    |	ЗакОст.ЗаказПокупателя.СуммаДокумента КАК СуммаДокумента,
		               ...
		               |				ГДЕ
		               |					Категории.Категория В (&списокисключений))
		               |;
		               |
		               |////////////////////////////////////////////////////////////////////////////////
		               |ВЫБРАТЬ
	...
	СписокКатегорийИсключений.Добавить(Справочники.КатегорииОбъектов.НайтиПоКоду("00043"));
	СписокКатегорийИсключений.Добавить(Справочники.КатегорииОбъектов.НайтиПоКоду("00045"));
	СписокКатегорийИсключений.Добавить(Справочники.КатегорииОбъектов.НайтиПоКоду("00055"));
	СписокКатегорийИсключений.Добавить(Справочники.КатегорииОбъектов.НайтиПоКоду("00025"));
	СписокКатегорийИсключений.Добавить(Справочники.КатегорииОбъектов.НайтиПоКоду("00018"));
	//СписокКатегорийИсключений.Добавить(Справочники.КатегорииОбъектов.НайтиПоКоду("00044"));    Тендеры собираем по галке, а отгружаем по маршруту
	СписокКатегорийИсключений.Добавить(Справочники.КатегорииОбъектов.НайтиПоКоду("00015"));  
	СписокКатегорийИсключений.Добавить(Справочники.КатегорииОбъектов.НайтиПоКоду("00054"));
	
	//16.10.18 Смирнов
	СписокКатегорийИсключений.Добавить(Справочники.КатегорииОбъектов.НайтиПоКоду("00057"));
	...
	если Диски Тогда
	    СписокКатегорийИсключенийМожно.Добавить(Справочники.КатегорииОбъектов.НайтиПоКоду("00025"));
	КонецЕсли;
	если Тендеры Тогда
	    СписокКатегорийИсключенийМожно.Добавить(Справочники.КатегорииОбъектов.НайтиПоКоду("00044"));
	КонецЕсли;   
	если ПЭК Тогда
	    СписокКатегорийИсключенийМожно.Добавить(Справочники.КатегорииОбъектов.НайтиПоКоду("00018"));
	КонецЕсли;  
	если Маркировка Тогда
	    СписокКатегорийИсключенийМожно.Добавить(Справочники.КатегорииОбъектов.НайтиПоКоду("00054"));
	КонецЕсли;
	если КолесаТУТ Тогда
	    СписокКатегорийИсключенийМожно.Добавить(Справочники.КатегорииОбъектов.НайтиПоКоду("00043"));
	КонецЕсли;
	если КолесаТУТКарточки Тогда //05.10.2018
	    СписокКатегорийИсключенийМожно.Добавить(Справочники.КатегорииОбъектов.НайтиПоКоду("00045"));
	КонецЕсли;
	//16.10.18 Смирнов
	если КолесаТУТПИТСТОП Тогда //16.10.2018
	    СписокКатегорийИсключенийМожно.Добавить(Справочники.КатегорииОбъектов.НайтиПоКоду("00057"));
	КонецЕсли;
	...
	Если ВремяВыполнения>0 Тогда
		ЗаписатьВЖурналИзмененийВремяВыполненияВнешнейОбработки( 618, ВремяВыполнения, "Кнопка [Обновить] - ВыполнитьЗапрос - "+парамЗапроса+" ("+строка(ВремяВыполнения)+" Сек.)" ); //!!!!
	КонецЕсли;	
	//+++)   16.05.2012
	...
	Если  Io=0 Тогда
		...
		Для Каждого стр1 из Заказы Цикл  
			...
			стр1.ДатаОтгрузки = стр1.Заказ.ДатаОтгрузки; //получитьПоследнююДатуУстановкиКатегории(стр1.Заказ);
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	И ТоварыНаОтветственномХраненииОбороты.Регистратор.ДокументОснование.ДокументОснование.СостояниеЗаявки = &Отклонить
				    //+++ 16.08.2018 - отклонить и заявка Согласована! по задаче № 35734 
...
ЗаполненныВсеКоординаты = ОбновитьАдресаДоставки();
	Если не ЗаполненныВсеКоординаты.Признак Тогда 		
		Сообщить("В строке номер " + ЗаполненныВсеКоординаты.НомерСтроки + " не указано расстояние. Нажмите ""Пересчет"" или введите Расстояние от предыдущей точки...");
		Отказ = Истина;
	КонецЕсли;	
//30.06.2016 
...
Процедура ОсновныеДействияФормыЗаданиеНаОтгрузку(Кнопка)
	...
	Если Обратки.Количество()>0 тогда
	  Если Вопрос("Уже заполнены Обратки. Перезаполнить Заявки на брак?", РежимДиалогаВопрос.ДаНет, 0)= КодВозвратаДиалога.Да Тогда
...
//=====================проверка точек!===========================
	...
	Иначе
		...
		//	Предупреждение("У Вас недостаточно прав для создания Задания на отгрузку!");
		//	возврат;
		//КонецЕсли;
...
КонецЕсли;
	...
	Для каждого Стр1 из Заказы Цикл
	  ...
	  //08.08.2017  ЕДИНАЯ ФУНКЦИЯ!  Вид, Адрес, Регион, Город, Улица, Дом, Координаты
	   ...
	   КонецЕсли;
       стр1.АдресДоставки =  сАдрес.Адрес; //ОБНОВЛЯЕМ ВСЕГДА!
...
Процедура КоманднаяПанель1Действие2(Кнопка)
	...
	Заказы.Сортировать("Расстояние Возр");
	...
	для каждого стр из заказы Цикл
		...
		если стр.Расстояние <> пред тогда
