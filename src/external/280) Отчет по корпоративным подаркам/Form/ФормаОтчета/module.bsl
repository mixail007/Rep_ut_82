...
Процедура ДействияФормыСводПоСертификатам(Кнопка)
	...
	Иначе
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	ПодаркиКлиентамСрезПоследних.Контрагент.НаименованиеПолное КАК Контрагент,
		               |	ВЫБОР
		               |		КОГДА ПодаркиКлиентамСрезПоследних.БрендыСертификатаДиски ПОДОБНО ""%0%""
		               |			ТОГДА """"
		               |		ИНАЧЕ ПодаркиКлиентамСрезПоследних.БрендыСертификатаДиски
		               |	КОНЕЦ КАК БрендыСертификатаДиски,
