...
Процедура КоманднаяПанельФормыПрибыльИЗатраты(Кнопка)
	
	ИмяФайла = КаталогВременныхФайлов()+"ВнешнийОтчет356.epf";
//	ИмяФайла = ПолучитьИмяВременногоФайла();
	
	файл = новый файл(ИмяФайла);
		обр = справочники.ВнешниеОбработки.НайтиПоКоду(356); 
		ДвоичныеДанные = обр.ХранилищеВнешнейОбработки.Получить();
		ДвоичныеДанные.Записать(ИмяФайла);
	Отчет = ВнешниеОтчеты.Создать(ИмяФайла);
	
	Форма = Отчет.ПолучитьФорму();
	Отчет.мУстановитьИнтервалВПараметры(ДатаНач, ДатаКон );
	Форма.Открыть();
	
	Форма.ДатаНач = ДатаНач;
	Форма.ДатаКон = ДатаКон;

	ТабДок = Форма.ЭлементыФормы.Результат;
