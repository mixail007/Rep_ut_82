...
Процедура Кнопка1Нажатие(Элемент)
	...
	////ПараметрСКД = СхемаКомпоновкиДанных3.Параметры.НачПериодаП;
	////ПараметрСКД.Использование = ИспользованиеПараметраКомпоновкиДанных.Всегда;
	////ПараметрСКД.Значение  = НачПериодаП;
	
	ПараметрСКД = СхемаКомпоновкиДанных3.Параметры.КонПериодаП;
	ПараметрСКД.Использование = ИспользованиеПараметраКомпоновкиДанных.Всегда;
	ПараметрСКД.Значение  = КонПериодаП;
...
Процедура ВыбПериодНажатие1(Элемент)
	...
	//НастройкаПериода.РедактироватьКакПериод = Истина;
	//НастройкаПериода.ВариантНастройки = ВариантНастройкиПериода.Период;
	//НастройкаПериода.УстановитьПериод(НачПериодаП, ?(КонПериодаП='0001-01-01', КонПериодаП, КонецДня(КонПериодаП)));
	//Если НастройкаПериода.Редактировать() Тогда
	//	НачПериодаП = НастройкаПериода.ПолучитьДатуНачала();
	//	КонПериодаП = НастройкаПериода.ПолучитьДатуОкончания();
	//КонецЕсли;
КонецПроцедуры


Процедура РезультатПереносыОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
    Перем ВыполненноеДействие;
    СтандартнаяОбработка=Ложь;
		СхемаКомпоновкиДанных3 = ПолучитьМакет("Переносы");

    ОбработкаРасшифровки3=Новый ОбработкаРасшифровкиКомпоновкиДанных(ДанныеРасшифровки3,Новый ИсточникДоступныхНастроекКомпоновкиДанных(СхемаКомпоновкиДанных3));
    ДоступныеДействия=Новый Массив; // заполняем доступные действия, если параметр не указан, то будут доступны все действия
    ДоступныеДействия.Добавить(ДействиеОбработкиРасшифровкиКомпоновкиДанных.ОткрытьЗначение);
	Настройки=ОбработкаРасшифровки3.Выполнить(Расшифровка,ВыполненноеДействие,,);
	   Если ВыполненноеДействие = "Открыть значение" Тогда
   	 ЭлементРасшифровкиДанных = ДанныеРасшифровки1.Элементы[Расшифровка];
   	 ЭлементРасшифровкиДанныхПоля = ЭлементРасшифровкиДанных.ПолучитьПоля()[0];
   	 ОбработкаРасшифровки3.ОткрытьЗначение(ЭлементРасшифровкиДанных.Идентификатор, новый ПолеКомпоновкиДанных(ЭлементРасшифровкиДанныхПоля.Поле));
    КонецЕсли;       
КонецПроцедуры
