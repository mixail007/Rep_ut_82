...
функция получитьНашиКоординатыПодр() экспорт
попытка
	подр = СписокПодразделений[0].Значение;
	
	Если подр.Код = "00106" тогда      // РнД
		нашиКоординаты = "39.590228,47.245316"; //ДШ   Ростовская обл., г. Ростов-на-Дону, ул. Доватора, д. 154/5
		
	ИначеЕсли подр.Код = "00112" тогда // СПб
		нашиКоординаты = "30.4261098,59.8424869";// ДШ
		
	ИначеЕсли подр.Код = "00138" тогда 
		нашиКоординаты = "60.6057025,56.8389261";// новый адрес!  - Екатеринбург г, Жуковского пл, дом № 1-Я
		
	ИначеЕсли подр.Код = "00133" или  подр.Код ="00184" тогда 
		нашиКоординаты = "37.692681,55.706624";  // новый адрес!  Москва г, Южнопортовая ул, дом № 7, корпус 1
		
	ИначеЕсли подр.Код = "00172" тогда //Тольятти - свой адрес!  Тольятти г, Северная ул, дом № 18
		нашиКоординаты = "49.286984,53.581501"; //
		
	Иначе
		нашиКоординаты = "39.8380129,57.6596170";
	КонецЕсли;
исключение
	нашиКоординаты = "39.8380129,57.6596170";
Конецпопытки;
...
функция получитьГородПодразделения()
попытка
	подр = СписокПодразделений[0].Значение;
	Если подр.Код = "00106" тогда      // РнД
		Город1 = "Ростов-на-Дону"; 
	ИначеЕсли подр.Код = "00112" тогда // СПб
		Город1 = "Санкт Петербург"; 
	ИначеЕсли подр.Код = "00138" или подр.Код ="00122" тогда // ЕКТ
	 	Город1 = "Екатеринбург"; 
	ИначеЕсли подр.Код = "00133" или подр.Код ="00184" тогда // ЕКТ
	 	Город1 = "Москва"; 
		
	ИначеЕсли подр.Код = "00172" тогда //Тольятти - пока через Ростов!
		Город1 = "Ростов-на-Дону"; //"Тольятти";
		
	Иначе //Если подр.Код = "00005" тогда
		Город1 = "Ярославль";
	КонецЕсли;
исключение
	Город1 = "Ярославль";
...
Если ЗначениеЗаполнено(стр1.Заказ) тогда 
	...
	Если Не неОтгружено тогда
		...
		сАдрес = ПолучитьСтруктуруАдреса(стр1.Заказ);  //+++ 06.04.2018
...
Если МенятьАдрес тогда
	...
	нашиКоординаты = получитьНашиКоординатыПодр(); //так же как в Модуле!
...
функция получитьРасстояние(к1,к2) Экспорт
	...
	результат = 0;
	попытка
		//ВСЕ координаты Широта,Долгота !!!
	д1 = Число(Лев(к1, Найти(к1, ",")-1)); ш1 = Число( Прав(к1, стрДлина(к1) - Найти(к1, ",")) );
	д2 = Число(Лев(к2, Найти(к2, ",")-1)); ш2 = Число( Прав(к2, стрДлина(к2) - Найти(к2, ",")) );
...
процедура  РассчитатьРасстояние() Экспорт
	...
	город1 = получитьГородПодразделения();
	i=1; 
