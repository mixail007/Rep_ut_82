...
Процедура КнопкаВыполнитьНажатиеНовый(Кнопка)
	...
	|ИЗ
	|	РегистрСведений.ФотоВНаличии КАК ВременныеФото";
	...
	Запрос.УстановитьПараметр("ДатаНач", ДобавитьМесяц( НачалоГода(ДатаКон), -7*12) ); //7 лет
	...
	Если не  Результат.Пустой() тогда    // получаем файл с именами фоток
		...
		//ОбластьСтрокаВсе.Параметры.Наличие		= ?(выборка.Наличие>0 , "в наличии", "нет");					
		//ОбластьСтрокаВсе.Параметры.ДатаПрихода	= ?(ДатаПриходаНаСклад =  Дата(1,1,1) , "", Лев(ДатаПриходаНаСклад,10));
		...
		//--- вывод всех в отдельный ТабДок
			...
			Если HTTPОтвет.КодСостояния = 200 Тогда  		//если коннект удачный
				...
				СтрокиНаличие = РезультатВр.НайтиСтроки(Новый Структура("ИмяФото",ИмяФото));
				
				
				ЕСли не ЕстьФото тогда      //нет фото
				...
				иначе              //даа  - есть фото
					...
					Если СтрокиНаличие.Количество()=0 Тогда      //нет галки
						ВрСтрока = ТабВрФото.Добавить();
						ВрСтрока.Производитель = выборка.Производитель;
						ВрСтрока.Моедль = СокрЛП(выборка.Модель);
						ВрСтрока.Цвет = СокрЛП(выборка.Цвет);
