...
ТекстЗапроса = 	
	...
	|		ТоварыНаОтветственномХранении.Период КАК Период,
	|		ТоварыНаОтветственномХранении.КоличествоНачальныйОстаток КАК НачальныйОстаток,
	|		ТоварыНаОтветственномХранении.КоличествоКонечныйОстаток КАК КонечныйОстаток,
	|		0 КАК Расход
	|	ИЗ
	|		РегистрНакопления.ТоварыНаОтветственномХранении.ОстаткиИОбороты(&ДатаНачала, &ДатаКонца, Месяц, , { (Контрагент).* КАК Контрагент, (Склад).* КАК Склад, (Номенклатура).* КАК Номенклатура  } ) КАК ТоварыНаОтветственномХранении
	|	{ГДЕ
	|		ТоварыНаОтветственномХранении.Склад.* КАК Склад,
	|		ТоварыНаОтветственномХранении.Номенклатура.* КАК Номенклатура
	|	}
	...
	|	
	|	ВЫБРАТЬ
	|		""Товары на отв. хранении"",
	|		ТоварыНаОтветственномХранении.Склад,
	|		ТоварыНаОтветственномХранении.Номенклатура,
	|		NULL,
	|		0,
	|		0,
	|		ТоварыНаОтветственномХранении.Количество
	|	ИЗ
	|		РегистрНакопления.ТоварыНаОтветственномХранении КАК ТоварыНаОтветственномХранении
	|	ГДЕ  ТоварыНаОтветственномХранении.ВидДвижения = Значение(ВидДвиженияНакопления.Расход)
	|		 И ТоварыНаОтветственномХранении.Период >= &ДатаНачала
	|		И ТоварыНаОтветственномХранении.Период <= &ДатаКонца
	|	{ГДЕ
	|		ТоварыНаОтветственномХранении.Контрагент.* КАК Контрагент,
	|		ТоварыНаОтветственномХранении.Склад.* КАК Склад,
	|		ТоварыНаОтветственномХранении.Номенклатура.* КАК Номенклатура
	|}
	|) КАК ВложенныйЗапрос
...
ПОтчета.Текст="ВЫБРАТЬ Номенклатура КАК Номенклатура,
	|		КоличествоОстаток КАК КоличествоОстаток
	|	ИЗ
	|		РегистрНакопления.ТоварыНаОтветственномХранении.Остатки(&ДатаКонца, {Контрагент.*, Склад.*, Номенклатура.*})";		
ПОтчета.Параметры.Вставить("ДатаКонца",КонецДня(ДатаКонца));

Для каждого Элемент ИЗ ПостроительОтчета.Отбор Цикл
	Если 	Элемент.ПутьКДанным<>"КоэффициентОборачиваемости" Тогда
