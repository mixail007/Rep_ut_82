...
Функция СоздатьНужныйДоговор(Владелец, ТекОрганизация,Плательщик,НазначениеПлатежа = "") Экспорт
	...
	КонецЕсли;
	//21.08.17 Смирнов, Договоры 8 исключаем
	//ТаблицаДоговоровКонтрагента = УбратьДоговорыЗвезда(ТаблицаДоговоровКонтрагента);
	
	Если не Владелец.Покупатель и не Владелец.поставщик Тогда
		Возврат Владелец.ОсновнойДоговорКонтрагента;
	КонецЕсли;
	
	Если ТаблицаДоговоровКонтрагента.Количество()>0 и Плательщик = Ложь Тогда //Плотников+++, для поставщиков как было, 
		Возврат ТаблицаДоговоровКонтрагента[0].ДоговорКонтрагента;        //а для покупателей теперь только Основной договор 17.05.2016
	КонецЕсли;
	
	Если Владелец.Покупатель и Владелец.поставщик и Плательщик и Владелец.ОсновнойДоговорКонтрагента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СПоставщиком Тогда
		ТаблицаДоговоровКонтрагента=ВернутьТаблицуДоговоровКонтрагента(ТекОрганизация,Владелец,Перечисления.ВидыДоговоровКонтрагентов.СПокупателем);
		//21.08.17 Смирнов, Договоры 8 исключаем
		//ТаблицаДоговоровКонтрагента = УбратьДоговорыЗвезда(ТаблицаДоговоровКонтрагента);
		Если ТаблицаДоговоровКонтрагента.Количество() > 0 Тогда   //*** 2018.03.13 добавила условие, т.к. выпадало в ошибку при пустой таблице договоров
	...
	// + Сакулина. Если НазначениеПлатежа сожержит ИМ00, тогда выбираем договор, который в наименовании содержит строку " ИМ". иначе пустая ссылка
	Если Найти(НазначениеПлатежа,"ИМ00") <> 0 Тогда
		рез = НайтиДоговорПоИМ(ТекОрганизация,Владелец);
	//Плотников, договор франчайзи
	//08.05.18 Смирнов по франчайзи типовой алгоритм Задача № 47340
	//ИначеЕсли Найти(НазначениеПлатежа,"Франчайзи") <> 0 Тогда
	//	рез = НайтиДоговорФранчайзи(ТекОрганизация,Владелец);
	Иначе
		
		Если Найти(НазначениеПлатежа,"факторинг")<> 0  тогда 
			
			Дог = Справочники.ДоговорыКонтрагентов.ПустаяСсылка();
			Для каждого стр11 из ТаблицаДоговоровКонтрагента Цикл
				Если Найти(Стр11.ДоговорКонтрагента.наименование,"ДЕНЬГИ") и не Стр11.ДоговорКонтрагента.ПометкаУдаления Тогда
					Дог = Стр11.ДоговорКонтрагента;
				КонецЕсли;	
			КонецЦикла;
		...
		иначе
			...
			если оснДог.ВидДоговора = перечисления.ВидыДоговоровКонтрагентов.СПокупателем
				...
				рез = оснДог;
				//21.08.17 Смирнов
				Если найти(оснДог.Наименование,"*")>0 тогда
					//берем первый попавшийся с покупателем без *
				    Запрос = новый Запрос;
					Запрос.Текст="ВЫБРАТЬ
					             ...
					             |		Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
					             |	ГДЕ
					             |		ДоговорыКонтрагентов.Ссылка = &Ссылка) КАК А
					             |
					             |УПОРЯДОЧИТЬ ПО
					             |	Приоритет
					             |АВТОУПОРЯДОЧИВАНИЕ";
								 Запрос.УстановитьПараметр("Владелец",оснДог.Владелец);
								 Запрос.УстановитьПараметр("Организация",оснДог.Организация);
								 Запрос.УстановитьПараметр("ВидДоговора",оснДог.ВидДоговора);
								 Запрос.УстановитьПараметр("ТипДоговора",оснДог.ТипДоговора);
								 Запрос.УстановитьПараметр("Ссылка",оснДог);
								 Рез =Запрос.Выполнить().Выгрузить()[0].ДоговорКонтрагента;
...
// + Сакулина
Функция НайтиДоговорПоИМ(ТекОрганизация, Владелец)
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ДоговорыКонтрагентов.Ссылка,
	...
	Пока Выборка.Следующий() Цикл
		Если Найти(Выборка.Наименование," ИМ") = 0  Тогда
			Продолжить;
		Иначе
...
Функция НайтиДоговорФранчайзи(ТекОрганизация, Владелец)
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ДоговорыКонтрагентов.Ссылка,
	...
	Договор = Справочники.ДоговорыКонтрагентов.ПустаяСсылка();
	Пока Выборка.Следующий() Цикл
		Если Найти(Выборка.Наименование," Договор предоплаты франчайзи") = 0  Тогда
			Продолжить;
		Иначе
			Договор = Выборка.Ссылка;
			Прервать;
...
Процедура СформироватьСоответствияНеПустыхПриИмпорте()
	...
	"ПоказательПериода,ПоказательНомера,ПоказательДаты");
...
Процедура СформироватьСтруктуруТаблицыИмпорта(Таб, ТабПоле) Экспорт
	...
	Т_Контрагент = Новый ОписаниеТипов("СправочникСсылка.Контрагенты");
	Т_БанкСчет   = Новый ОписаниеТипов("СправочникСсылка.БанковскиеСчета");
	Т_Реализация   = Новый ОписаниеТипов("ДокументСсылка.РеализацияТоваровУслуг");
	...
	СоздатьКолонкуВТаблице(Таб, ТабПоле, "Код",			         Т_Строка,    "Код",				        НеВидимая);
	СоздатьКолонкуВТаблице(Таб, ТабПоле, "ДокументРеализации",	 Т_Реализация,"Документ реализации",		НеВидимая);
	
	//***2019.01.21
	СоздатьКолонкуВТаблице(Таб, ТабПоле, "ПлатежФизическогоЛица",Т_Булево,"Платеж от физ.лица",		Видимая);
...
Процедура ЗаполнитьДокументыНаЭкспорт(ДокументыНаЭкспорт) Экспорт
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |	ВЫБОР
		           |          КОГДА ПлатежноеПоручениеИсходящее.ВидОперации = ЗНАЧЕНИЕ(перечисление.ВидыоперацийППИсходящее.ОплатаПоставщикуСКодомКБК)
			       |               ТОГДА ЗНАЧЕНИЕ(перечисление.ВидыоперацийППИсходящее.ПеречислениеНалога)
		           |          ИНАЧЕ ПлатежноеПоручениеИсходящее.ВидОперации
...
Функция ПолучитьСекциюДокумент(СтруктураДокумента, КоллекцияРеквизитов)
	...
	Для каждого ВыгружаемыйРеквизит из КоллекцияРеквизитов Цикл
		Если Экспорт_Выгружаемые[ВыгружаемыйРеквизит.Имя] = Истина Тогда
			...
			ИначеЕсли ВыгружаемыйРеквизит.Имя = "ПолучательИНН" Тогда
				ДобавитьВСтроку(Буфер, ВыгружаемыйРеквизит.Имя + "=" + "000000000000");
			КонецЕсли;
...
Процедура ВыгрузитьВалютные(ДокументыНаЭкспорт) Экспорт
	
	ПотокВыгрузки = Новый ТекстовыйДокумент();
	
	// Формируем заголовок
	//ПотокВыгрузки.ДобавитьСтроку("ISO_код_валюты_перевода	ISO_код_валюты_списания	OUR	ISO код страны банка бенефициара	ISO код страны плательщика	SWIFT_код_банка_бенефициара	Адрес банка бенефициара	Валюта счета бенефициара	Дата документа	Дата справки о валютных операциях	ИНН плательщика	Код валюты перевода	Код страны банка бенефициара	Код страны плательщика	Международное наименование плательщика	Международный адрес плательщика	Назначение платежа	Наименование банка бенефициара	Наименование бенефициара	Номер документа	Платеж внутри/вне СБРФ	Сумма перевода	Счет бенефициара	Счет плательщика	Сумма списания	Страна плательщика	Страна бенефициара	Код страны бенефициара");
	ПотокВыгрузки.ДобавитьСтроку("ISO_код_валюты_перевода	ISO_код_валюты_списания	OUR	ISO код страны банка бенефициара	ISO код страны плательщика	SWIFT_код_банка_бенефициара	Валюта счета бенефициара	Дата документа	Дата справки о валютных операциях	ИНН плательщика	Код валюты перевода	Код страны банка бенефициара	Код страны плательщика	Международное наименование плательщика	Международный адрес плательщика	Назначение платежа	Наименование банка бенефициара	Наименование бенефициара	Номер документа	Платеж внутри/вне СБРФ	Сумма перевода	Счет бенефициара	Счет плательщика	Сумма списания	Страна плательщика	Страна бенефициара	Код страны бенефициара	Местонахождение плательщика	Местонахождение бенефициара	Адрес бенефициара	Опция 50a");
	...
	Для каждого СтрокаСекции из ДокументыНаЭкспорт Цикл
		...
		КонецЕсли;
		стр1="";
		//ISO_код_валюты_перевода
		ДобавитьЭлемент(стр1,СтрокаСекции.Документ.СчетОрганизации.ВалютаДенежныхСредств.Наименование);
		//ДобавитьЭлемент(стр1,СтрокаСекции.Документ.СчетОрганизации.ВалютаДенежныхСредств.Код);
		//ISO_код_валюты_списания
		//ДобавитьЭлемент(стр1,СтрокаСекции.Документ.СчетОрганизации.ВалютаДенежныхСредств.Код);
		ДобавитьЭлемент(стр1,СтрокаСекции.Документ.СчетОрганизации.ВалютаДенежныхСредств.Наименование);
		//OUR
		ДобавитьЭлемент(стр1,"1");
		//ISO код страны банка бенефициара
		ДобавитьЭлемент(стр1,СтрокаСекции.Документ.СчетКонтрагента.Банк.Страна.КодСтраны);
		//ДобавитьЭлемент(стр1,"CN");
		//ISO код страны плательщика
		ДобавитьЭлемент(стр1,"643");
		//SWIFT_код_банка_бенефициара
		ДобавитьЭлемент(стр1,СтрокаСекции.Документ.СчетКонтрагента.Банк.СвифтКод);
		//Адрес банка бенефициара
		//ДобавитьЭлемент(стр1,СтрокаСекции.Документ.СчетКонтрагента.Банк.Адрес);
		//Валюта счета бенефициара
		ДобавитьЭлемент(стр1,СтрокаСекции.Документ.СчетОрганизации.ВалютаДенежныхСредств.Код);
		//Дата документа
		ДобавитьЭлемент(стр1,"");
		//Дата справки о валютных операциях
		ДобавитьЭлемент(стр1,"");
		//ИНН плательщика
		ДобавитьЭлемент(стр1,СтрокаСекции.Документ.Организация.ИНН);
		//Код валюты перевода
		//ДобавитьЭлемент(стр1,СтрокаСекции.Документ.СчетОрганизации.ВалютаДенежныхСредств.Код);
		ДобавитьЭлемент(стр1,СтрокаСекции.Документ.СчетОрганизации.ВалютаДенежныхСредств.НАИМЕНОВАНИЕ);
		//Код страны банка бенефициара
		ДобавитьЭлемент(стр1,СтрокаСекции.Документ.СчетКонтрагента.Банк.Страна.Код);
		//ДобавитьЭлемент(стр1,"CN");
		//Код страны плательщика
		ДобавитьЭлемент(стр1,"643");
		
		//Международное наименование плательщика
		ДобавитьЭлемент(стр1,"ZAO TK YARSHINTORG");
		//Международный адрес плательщика
		ДобавитьЭлемент(стр1,"BAZOVAYA STREET 3 BUILDING 2 YAROSLAVL, RU");
		//Назначение платежа
		ДобавитьЭлемент(стр1,СтрЗаменить(СтрокаСекции.Документ.НазначениеПлатежа,Символы.ПС," "));
		//Наименование банка бенефициара
		ДобавитьЭлемент(стр1,СтрокаСекции.Документ.СчетКонтрагента.Банк.Наименование);
		//Наименование бенефициара
		ДобавитьЭлемент(стр1,СтрокаСекции.Документ.Контрагент.Наименование);
		//Номер документа
		ДобавитьЭлемент(стр1,"");
		//Платеж внутри/вне СБРФ
		ДобавитьЭлемент(стр1,"0");
		//Сумма перевода
		ДобавитьЭлемент(стр1,Формат(СтрокаСекции.Сумма,"ЧГ=0"));
		//Счет бенефициара
		ДобавитьЭлемент(стр1,СтрокаСекции.НомерСчетаКонтрагента);
		//Счет плательщика
		ДобавитьЭлемент(стр1,СтрокаСекции.НомерСчетаОрганизации);
		//Сумма списания
		ДобавитьЭлемент(стр1,Формат(СтрокаСекции.Сумма,"ЧГ=0"));
		//Страна плательщика
		ДобавитьЭлемент(стр1,"RU");
		//Страна бенефициара
		//ДобавитьЭлемент(стр1,СтрокаСекции.Документ.СчетКонтрагента.Банк.КодСтраны);
		ДобавитьЭлемент(стр1,СтрокаСекции.Документ.СчетКонтрагента.Банк.Страна.Код);
		//Код страны бенефициара
		//ДобавитьЭлемент(стр1,СтрокаСекции.Документ.СчетКонтрагента.Банк.КодСтраны);
		ДобавитьЭлемент(стр1,СтрокаСекции.Документ.СчетКонтрагента.Банк.Страна.Код);
		//Местонахождение плательщика
		ДобавитьЭлемент(стр1,"YAROSLAVL");
		...
		Запрос.УстановитьПараметр("Объект",СтрокаСекции.Документ.Контрагент);
		Запрос.УстановитьПараметр("Свойство",ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоКоду("90271"));
		Рез = Запрос.Выполнить().Выбрать();
		Пока Рез.Следующий() Цикл
			ГородПолучателя = Рез.Город;
		КонецЦикла;
		ДобавитьЭлемент(стр1,ВРЕГ(ГородПолучателя));
		//Адрес бенефициара
		Адрес = "";
		Запрос.Текст="ВЫБРАТЬ
		...
		ДобавитьЭлемент(стр1,ВРЕГ(Адрес));
		
		ДобавитьЭлемент(стр1,"K"); //Опция 50a
		ПотокВыгрузки.ДобавитьСтроку(стр1);	
	КонецЦикла;
...
Процедура ДобавитьЭлемент(строка,Элемент)
	Строка = Строка + ?(Строка = "","",Символы.Таб) + Элемент;
КонецПроцедуры
////////////// ПОГОТОВКА ЗАГРУЗКИ //////////////////////////////////////////////////
...
Процедура РаспознатьДанныеВСтрокеДокумента(СтрокаДокумента, РеквизитИмпорта)
	...
	Если РеквизитИмпорта = "Документ" Тогда
		...
		Если ВидДокумента = "ПлатежноеПоручениеВходящее"
		 ИЛИ ВидДокумента = "АккредитивПолученный"
		 ИЛИ ВидДокумента = "ПлатежныйОрдерПоступлениеДенежныхСредств"  // 07.07.2016   Банковский ордер
		 ИЛИ ВидДокумента = "ПлатежноеТребованиеПолученное"
		 ИЛИ ВидДокумента = "ИнкассовоеПоручениеПолученное" Тогда
		...
		Если ЗначениеНеЗаполнено(СтрокаДокумента.Документ) Тогда
			// 07.07.2016 более подробное описание где и что не нашел
			//текст1 = "Документ не нейден в информационной базе!";
			текст1 = "Документ '"+ВидДокумента+"' не найден по реквизиту "+РеквизитНомера +" = "+строка(НомерДок)+" в период: "+формат(ДатаДок,"ДФ=dd.MM.yyyy")+"!";
			ДобавитьЗамечание(СтрокаДокумента, 1, текст1);
		КонецЕсли;
	...
	ИначеЕсли РеквизитИмпорта = "ТипДокумента" Тогда
		...
		ИначеЕсли ВидДокумента = "ПлатежныйОрдерСписаниеДенежныхСредств" Тогда
			СтрокаДокумента.ТипДокумента = 1;            //Плотников+++
			СтрокаДокумента.ТипОперации = "Исходящий";
	...
	ИначеЕсли РеквизитИмпорта = "Контрагент" Тогда
		...
		Если СокрЛП(Организация.ИНН) <> СокрЛП(ИННОрганизации) Тогда
			Если  (ИННОрганизации<>"7604090169") и (ИННОрганизации<>"7602088206")  тогда
				ДобавитьЗамечание(СтрокаДокумента, 3, "В документе указан неверный ИНН организации: " + ИННОрганизации + " <> "+СокрЛП(Организация.ИНН) );
			КонецЕсли;
		конецЕсли;
		...
		//Плотников 01.06.2017 по задаче Веселовой +++++++++++++++++++++++++++++++
		
		Если Кандидат = Справочники.Контрагенты.НайтиПоКоду("94121") Тогда
			
			Если Найти(СтрокаДокумента.НазначениеПлатежа,"Оплата по отчету") > 0 Тогда
				
				Кандидат = Справочники.Контрагенты.НайтиПоКоду("94247");
				
			КонецЕсли;
			
		ИначеЕсли Кандидат = Неопределено и СтрокаДокумента.ПолучательИНН = "7726594300" Тогда
			
			Кандидат = Справочники.Контрагенты.НайтиПоКоду("П005342");
			
		ИначеЕсли	СтрокаДокумента.ПолучательИНН = "7709654253" Тогда
			
			Кандидат = Справочники.Контрагенты.НайтиПоКоду("90151");
			
		ИначеЕсли	СтрокаДокумента.ПолучательИНН = "7715669410" Тогда
			
			Кандидат = Справочники.Контрагенты.НайтиПоКоду("П003046");
			
		ИначеЕсли	Кандидат = Справочники.Контрагенты.НайтиПоКоду("П013340") Тогда  //надо другой евротранс
			
			Кандидат = Справочники.Контрагенты.НайтиПоКоду("93273");
	...
	ИначеЕсли РеквизитИмпорта = "СчетКонтрагента" Тогда
		...
		Пока ВыборкаПоиска.Следующий() Цикл
			...
			//09.01.18 Смирнов, 2 евротранса
			Если ВыборкаПоиска.Владелец.Код = "П013340" тогда //не тот евротранс, продолжаем
				Продолжить;
			КонецЕсли;
...
Процедура ПроверитьНаПустоеЗначениеИмпорта(СтрокаИмпорта, ИмяСвойства, ПредставлениеСвойства)
	...
	Если Импорт_НеПустые[ВариантНеПустых][ИмяСвойства] = Истина Тогда
		Если ЗначениеНеЗаполнено(СтрокаИмпорта[ИмяСвойства]) Тогда
			Если СтрокаИмпорта.ПолучательИНН = "7604090169" и имяСвойства = "ПлательщикИНН" тогда
			//	продолжить;
			иначе	
	...
	Пока Импорт_ТекущаяСтрока <= Импорт_КоличествоСтрок Цикл
		...
		ИначеЕсли Лев(Врег(СокрЛП(Стр)), 10)="КОНЕЦФАЙЛА" Тогда
			...
			Для каждого СтрокаДокумента Из ДокументыДляИмпорта Цикл
				...
				//***2019.01.21
				Если СтрокаДокумента.Операция = "Платежное поручение" Тогда
					Если Лев(СтрокаДокумента.ПлательщикСчет, 5) = "30233" ИЛИ Лев(СтрокаДокумента.ПлательщикСчет, 5) = "40817" Тогда
						СтрокаДокумента.ПлатежФизическогоЛица = Истина;
					КонецЕсли;
				КонецЕсли;
			...
			Для каждого стр из ДокументыДляИмпорта  цикл
				Если стр.Операция = "Банковский ордер" тогда
					Если Найти(стр.назначениеПлатежа,"факторинг")<> 0  тогда
						стр.Операция = "Платежное поручение";
						//( N РД001463/1 от 13.03.2017, дебитор - ИП Резницкий Юрий Леонидович).
						позицияНомера = Найти(стр.назначениеПлатежа,"N ");
						ОстатокСтроки = Прав(стр.назначениеПлатежа, СтрДлина(стр.назначениеПлатежа)-позицияНомера-1);
						позицияПробела =Найти(ОстатокСтроки," ");
						Номер = Лев(ОстатокСтроки,позицияПробела-1);
						...
						конецЕсли;
						позицияОТ = Найти(ОстатокСтроки,"от ");
						ОстатокСтрокиДата = Прав(ОстатокСтроки, СтрДлина(ОстатокСтроки)-позицияОТ-2);
						ДатаСтрокой =Лев(ОстатокСтрокиДата,10);
						День = Лев(ДатаСтрокой,2);
						Месяц = Сред(ДатаСтрокой,4,2);
						год = Прав(ДатаСтрокой,4);
						ДатаРеализации = Дата(Число(Год),Число(месяц),Число(День));
						
						НайденныйДок = Документы.РеализацияТоваровУслуг.НайтиПоНомеру(номер,ДатаРеализации);
						Если НачалоДня(НайденныйДок.Дата)= ДатаРеализации тогда
							стр.ДокументРеализации = НайденныйДок;
							стр.плательщик1 = НайденныйДок.Контрагент;
							стр.Контрагент = НайденныйДок.Контрагент;
							
							стр.ПлательщикИНН = НайденныйДок.Контрагент.ИНН;
							стр.ПлательщикКПП = НайденныйДок.Контрагент.КПП;
							
							банковскийСчет =  НайденныйДок.Контрагент.ОсновнойБанковскийСчет.Банк;
							
							стр.ПлательщикБанк1 = БанковскийСчет.Наименование;
							стр.ПлательщикБИК =   БанковскийСчет.Код;
							стр.ПлательщикКорсчет =  БанковскийСчет.КоррСчет;
						иначе
							Сообщить("Не найдена реализация № "+номер+" от "+ДатаРеализации+". Загрузка платежа не возможна!");	
							стр.Операция = "Банковский ордер";	
						конецЕсли;
					конецЕсли;
				конецЕсли;
			конецЦикла;
...
Процедура ЗаписатьОбъект(ОбъектДляЗаписи)
	...
	Попытка
		Если ОбъектДляЗаписи.ЭтоНовый() и Найти(НазваниеОбъекта,"Входящее") и не ЗначениеЗаполнено(ОбъектДляЗаписи.СчетОрганизации.Подразделение)  Тогда
			ОбъектДляЗаписи.ОтражатьВБухгалтерскомУчете = Истина;              //Плотников+++
			ОбъектДляЗаписи.ОтражатьВНалоговомУчете = Истина;
			ОбъектДляЗаписи.ОтраженоВОперУчете = Истина;                       //Плотников
		    ОбъектДляЗаписи.Записать(РежимЗаписиДокумента.Проведение);     ///Плотников, проводить новые
			Если ОбъектДляЗаписи.Контрагент.Инн<>"7602088206" или ОбъектДляЗаписи.Контрагент.Инн<>"7604090169" тогда

			
			конецесли;
		Иначе
			//Если ОбъектДляЗаписи.ЭтоНовый() и ОбъектДляЗаписи.СчетОрганизации = Справочники.БанковскиеСчета.НайтиПоКоду("15912") тогда
			//	 ОбъектДляЗаписи.УстановитьНовыйНомер("ШЯ");
			//	 НазваниеОбъекта = ПолучитьПредставлениеОбъекта(ОбъектДляЗаписи);
			//иначеЕсли ОбъектДляЗаписи.ЭтоНовый() и ОбъектДляЗаписи.СчетОрганизации = Справочники.БанковскиеСчета.НайтиПоКоду("15918") тогда
			//	 ОбъектДляЗаписи.УстановитьНовыйНомер("ФА");
			//	 НазваниеОбъекта = ПолучитьПредставлениеОбъекта(ОбъектДляЗаписи);
			//конецесли;	 
		...
		Если ОбъектДляЗаписи.ЭтоНовый() и Найти(НазваниеОбъекта,"Входящее") Тогда
	...
	Исключение
		...
		Если ОбъектДляЗаписи.ЭтоНовый() и Найти(НазваниеОбъекта,"Входящее") Тогда
			Сообщить(Врег(Лев(НазваниеОбъекта, 1)) + Сред(НазваниеОбъекта, 2) + " не создан! " + "Контрагент " + ОбъектДляЗаписи.Контрагент.Наименование + ". Произошли ошибки при записи! "+ОписаниеОшибки(), СтатусСообщения.Важное);
		
		Иначе
			Сообщить(Врег(Лев(НазваниеОбъекта, 1)) + Сред(НазваниеОбъекта, 2) + " не записан! " + "Контрагент " + ОбъектДляЗаписи.Контрагент.Наименование + ". Произошли ошибки при записи! "+ОписаниеОшибки(), СтатусСообщения.Важное);
...
Функция ПолучитьВидДокумента(ПлатежнаяОперация, Списано, Поступило)
	...
	ИначеЕсли Списали Тогда
		...
		Иначе
			...
			ВидДокумента = "ПлатежноеПоручениеИсходящее";//Плотников+++
	...
	Иначе
		...
		Иначе
			ВидДокумента = "ПлатежноеПоручениеИсходящее";    //"ПлатежныйОрдерСписаниеДенежныхСредств" - НЕ используется !!!
...
Процедура ЗаполнитьРеквизитПлатежногоПорученияИсходящее(ОбъектДокумента, ИсходныеДанные, ИмяРеквизита)
	...
	Если ИмяРеквизита = "Организация" Тогда
		...
		Если ОбъектДокумента.ЭтоНовый() Тогда
			...
			//ОбъектДокумента.ОтражатьВБухгалтерскомУчете = Истина;              //Плотников+++
			//ОбъектДокумента.ОтражатьВНалоговомУчете = Истина;
			//ОбъектДокумента.ОтраженоВОперУчете = Истина;                       //Плотников
			...
			Попытка
				...
				Если ИсходныеДанные.Операция = "Банковский ордер"	тогда	
					ОбъектДокумента.НомерОрдера = Формат(ОкончаниеНомера, "ЧЦ=" + ДлинаОкончания + "; ЧВН=; ЧГ=0");  
				Иначе 
		...
		Если ОбъектДокумента.ЭтоНовый() Тогда
			...
			Если ОбъектДокумента.РасшифровкаПлатежа.Количество() = 0 Тогда
				...
				Если ВсеСНДС18 Тогда
					СтрокаРасшифровки.СтавкаНДС = СтавкаНДСнаДату(ТекущаяДата()); //+++ 01.02.2019 Перечисления.СтавкиНДС.НДС18;
			...
			ИначеЕсли ОбъектДокумента.РасшифровкаПлатежа.Количество() = 1 Тогда
				...
				Если ВсеСНДС18 Тогда
					...
					ОбъектДокумента.РасшифровкаПлатежа[0].СтавкаНДС = СтавкаНДСнаДату(ТекущаяДата()); //+++ 01.02.2019 Перечисления.СтавкиНДС.НДС18;
...
Процедура ЗаполнитьРеквизитПлатежногоПорученияВходящее(ОбъектДокумента, ИсходныеДанные, ИмяРеквизита)
	...
	ИначеЕсли ИмяРеквизита = "Контрагент" Тогда
		...
		Попытка
			СтрокаРасшифровки.ДокументРеализации = ИсходныеДанные.ДокументРеализации;
		Исключение
		КонецПопытки;
		...
		Если ЗначениеНеЗаполнено(ОбъектДокумента.ДоговорКонтрагента) Тогда
			Если Не ЗначениеНеЗаполнено(ОбъектДокумента.Контрагент) Тогда
				ОбъектДокумента.ДоговорКонтрагента =СоздатьНужныйДоговор(ОбъектДокумента.Контрагент, Организация, Истина, ИсходныеДанные.НазначениеПлатежа);
	...
	//ИначеЕсли ИмяРеквизита = "ДокументРеализации" Тогда
	//	УстановитьСвойство(ОбъектДокумента, "ДокументРеализации", ИсходныеДанные.ДокументРеализации);
...
Процедура Загрузить(ДокументыКИмпорту) Экспорт
	...
	Для каждого СтрокаСекции из ДокументыКИмпорту Цикл
		Если СтрокаСекции.Загрузить Тогда
			...
			Если ПустаяСтрока(РезультатПроверки) Тогда
				...
				//***2018.03.11
				Если ОбъектДокумента.ЭтоНовый() И ЗначениеЗаполнено(ОбъектДокумента.НазначениеПлатежа) И ТипЗНЧ(ОбъектДокумента) = Тип("ДокументОбъект.ПлатежноеПоручениеВходящее") Тогда
					ЗаполнитьРасшифровкуПлатежаПоНомерамУказаннымВНазначенииПлатежа(ОбъектДокумента); 
				КонецЕсли;
	...
	КонецЦикла;
	//
	//ДокументыНаУдаление = Новый Запрос("ВЫБРАТЬ                                                //Плотников 23.10.2017 Убираем удаление исходящих платежек, не нужно 
	//								   |	ПлатежноеПоручениеИсходящее.Ссылка,
	//								   |	ПлатежноеПоручениеИсходящее.Дата,
...
Функция УбратьДоговорыЗвезда(ТабДог)
	Запрос = Новый Запрос;
	Запрос.Текст="ВЫБРАТЬ
	|	ДоговорыКонтрагентов.Ссылка КАК ДоговорКонтрагента
	|ИЗ
	...
	|	И НЕ ДоговорыКонтрагентов.Наименование ПОДОБНО ""%*%""";
	Запрос.УстановитьПараметр("Договоры",ТабДог);
...
//***
Процедура ЗаполнитьРасшифровкуПлатежаПоНомерамУказаннымВНазначенииПлатежа(ОбъектДокумента)
	
	Если ОбъектДокумента.ВидОперации = Перечисления.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.ОплатаПокупателя И ОбъектДокумента.РасшифровкаПлатежа.Количество() = 1 Тогда
		...
		СтрокаНазначениеПлатежа = НРег(ОбъектДокумента.НазначениеПлатежа);
		
		Пока Найти(СтрокаНазначениеПлатежа, "№")>0 Цикл
			НомерСимвола = Найти(СтрокаНазначениеПлатежа, "№");
			
			НаборЦифр = "";
			СчитаноСимволовПослеЗнакаN = 0;
			Для i=НомерСимвола По СтрДлина(СтрокаНазначениеПлатежа) Цикл
				Символ = Сред(СтрокаНазначениеПлатежа, i, 1);
				СчитаноСимволовПослеЗнакаN = СчитаноСимволовПослеЗнакаN + 1;
				Если Найти("0123456789", Символ) = 0 Тогда
					//отработка возможных ситуаций, когда между символом № и самим номером ставят пробелы
					Если Найти("№", Символ) > 0 Тогда
						Если НаборЦифр = "" Тогда
							Продолжить;
						КонецЕсли;
					КонецЕсли;
					Если Найти(" ", Символ) > 0 И НаборЦифр = "" Тогда
						Если НаборЦифр = "" Тогда
							Продолжить;
						КонецЕсли;
					КонецЕсли;
					Если Найти("т", Символ) > 0 Тогда
						Если НаборЦифр = "" Тогда
							Продолжить;
						КонецЕсли;
					КонецЕсли;
					Если Найти("к", Символ) > 0 Тогда
						Если НаборЦифр = "" Тогда
							Продолжить;
						КонецЕсли;
					КонецЕсли;
					Если Найти("с", Символ) > 0 Тогда
						Если НаборЦифр = "" Тогда
							Продолжить;
						КонецЕсли;
					КонецЕсли;
					Если Найти("п", Символ) > 0 Тогда
						Если НаборЦифр = "" Тогда
							Продолжить;
						КонецЕсли;
					КонецЕсли;
					Если НЕ НаборЦифр = "" Тогда
						Если СтрДлина(НаборЦифр) > 2 Тогда
							СтрТЗ = ТаблицаНомеров.Добавить();
							СтрТЗ.НомерДокумента = НаборЦифр;
							Сообщить(НаборЦифр);
						КонецЕсли;
						СтрокаНазначениеПлатежа = Прав(СтрокаНазначениеПлатежа, СтрДлина(СтрокаНазначениеПлатежа) - (НомерСимвола));
						НаборЦифр = ""; //после добавления обнуляется
						СчитаноСимволовПослеЗнакаN = 0;
						Прервать;
					Иначе
						СтрокаНазначениеПлатежа = Прав(СтрокаНазначениеПлатежа, СтрДлина(СтрокаНазначениеПлатежа) - (НомерСимвола));
		...
		Если ТаблицаНомеров.Количество() > 0 Тогда
			Для каждого СтрТЗ Из ТаблицаНомеров Цикл
				Запрос = Новый Запрос;
				Запрос.УстановитьПараметр("Дата", ДобавитьМесяц(ОбъектДокумента.Дата, -12));
				...
				Иначе
					Запрос.Текст = "ВЫБРАТЬ
					               ...
					               |	И НЕ ЗаказПокупателя.ДоговорКонтрагента.Наименование ПОДОБНО ""%*%""";
					Результат = Запрос.Выполнить().Выбрать();
					Если Результат.Следующий() Тогда
			...
			ОстатокДенегПоДокументу = ОбъектДокумента.СуммаДокумента;
			...
			Для каждого СтрТЗ Из ТаблицаНомеров Цикл
				...
				Результат = Запрос.Выполнить().Выбрать();
				Если Результат.Следующий() Тогда
					Если Результат.СуммаДолга > 0 Тогда
						Если (ТипЗНЧ(СтрТЗ.ЗаказПокупателя) = Тип("ДокументСсылка.ЗаказПокупателя")) И ОстатокДенегПоДокументу > 0 Тогда
							СтрТЧ = ОбъектДокумента.РасшифровкаПлатежа.Добавить();
							СтрТЧ.ДоговорКонтрагента = СтрТЗ.ЗаказПокупателя.ДоговорКонтрагента;
							СтрТЧ.Сделка = СтрТЗ.ЗаказПокупателя;
							СтрТЧ.СуммаПлатежа = ?(ОстатокДенегПоДокументу > Результат.СуммаДолга, Результат.СуммаДолга, ОстатокДенегПоДокументу);
							...
							СтрТЧ.СтавкаНДС = ОбъектДокумента.РасшифровкаПлатежа[0].СтавкаНДС;
							ПересчитатьСуммуНДС(СтрТЧ);
							ОстатокДенегПоДокументу = ОстатокДенегПоДокументу - СтрТЧ.СуммаПлатежа;
							...
							ОбъектДокумента.Комментарий = ОбъектДокумента.Комментарий + "Сумма " + Строка(СтрТЧ.СуммаПлатежа) + " по документу №" + Строка(СтрТЗ.НомерДокумента) + " ";
						ИначеЕсли ТипЗНЧ(СтрТЗ.ЗаказПокупателя) = Тип("ДокументСсылка.НачислениеПени") И ОстатокДенегПоДокументу > 0 Тогда
							Для каждого СтрТЧУслуги ИЗ СтрТЗ.Реализация.Услуги Цикл
								СтрТЧ = ОбъектДокумента.РасшифровкаПлатежа.Добавить();
								СтрТЧ.ДоговорКонтрагента = СтрТЧУслуги.ДоговорПени;
								СтрТЧ.Сделка = СтрТЗ.ЗаказПокупателя;
								СтрТЧ.СуммаПлатежа = ?(ОстатокДенегПоДокументу > СтрТЧУслуги.Сумма, СтрТЧУслуги.Сумма, ОстатокДенегПоДокументу);
								СтрТЧ.СтатьяДвиженияДенежныхСредств = Справочники.СтатьиДвиженияДенежныхСредств.ОплатаПокупателя;
								СтрТЧ.КурсВзаиморасчетов = 1;
								СтрТЧ.СуммаВзаиморасчетов = СтрТЧ.СуммаПлатежа;
								СтрТЧ.СтавкаНДС = СтрТЧУслуги.СтавкаНДС;
								ПересчитатьСуммуНДС(СтрТЧ);
								ОстатокДенегПоДокументу = ОстатокДенегПоДокументу - СтрТЧУслуги.Сумма;
								СтрТЧ.ДокументРеализации = ?(ТипЗНЧ(СтрТЗ.Реализация)=Тип("ДокументСсылка.РеализацияТоваровУслуг"), СтрТЗ.Реализация, Документы.РеализацияТоваровУслуг.ПустаяСсылка());
								БылоРаспределение = Истина;
								ОбъектДокумента.Комментарий = ОбъектДокумента.Комментарий + "Сумма " + Строка(СтрТЧ.СуммаПлатежа) + " по документу №" + Строка(СтрТЗ.НомерДокумента) + " ";
							КонецЦикла;
						КонецЕсли;
					Иначе
						//проверим, что это предоплата
						Запрос = Новый Запрос;
						Запрос.УстановитьПараметр("Сделка", СтрТЗ.ЗаказПокупателя);
						Запрос.Текст = "ВЫБРАТЬ
						               |	ВзаиморасчетыСКонтрагентамиОстаткиИОбороты.СуммаВзаиморасчетовРасход
						               |ИЗ
						               |	РегистрНакопления.ВзаиморасчетыСКонтрагентами.ОстаткиИОбороты(, , , , Сделка = &Сделка) КАК ВзаиморасчетыСКонтрагентамиОстаткиИОбороты";
						Результат = Запрос.Выполнить().Выбрать();
						Если Результат.Следующий() Тогда
						Иначе
							Если (ТипЗНЧ(СтрТЗ.ЗаказПокупателя) = Тип("ДокументСсылка.ЗаказПокупателя")) И ОстатокДенегПоДокументу > 0 Тогда
								...
								СтрТЧ.ДоговорКонтрагента = СтрТЗ.ЗаказПокупателя.ДоговорКонтрагента;
								СтрТЧ.Сделка = СтрТЗ.ЗаказПокупателя;
								СтрТЧ.СуммаПлатежа = ОстатокДенегПоДокументу;
								СтрТЧ.СтатьяДвиженияДенежныхСредств = Справочники.СтатьиДвиженияДенежныхСредств.ОплатаПокупателя;
								СтрТЧ.КурсВзаиморасчетов = 1;
								СтрТЧ.СуммаВзаиморасчетов = СтрТЧ.СуммаПлатежа;
								СтрТЧ.СтавкаНДС = ОбъектДокумента.РасшифровкаПлатежа[0].СтавкаНДС;
			...
			Если БылоРаспределение Тогда
				Если ОстатокДенегПоДокументу > 0 Тогда
					ОбъектДокумента.РасшифровкаПлатежа[0].СуммаПлатежа = ОстатокДенегПоДокументу;
					ОбъектДокумента.РасшифровкаПлатежа[0].СуммаВзаиморасчетов = ОстатокДенегПоДокументу;
					ПересчитатьСуммуНДС(ОбъектДокумента.РасшифровкаПлатежа[0]);
				Иначе
					ОбъектДокумента.РасшифровкаПлатежа.Удалить(0);
				КонецЕсли;
				ОбъектДокумента.Комментарий = ОбъектДокумента.Комментарий + " - автоматическая привязка денег";
			КонецЕсли;
