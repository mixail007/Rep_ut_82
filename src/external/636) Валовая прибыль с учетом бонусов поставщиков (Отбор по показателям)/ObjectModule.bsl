Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	Текст = ТексЗапросаБонусы+ " 
			...
			|(ВЫБРАТЬ
				...
				|	СУММА(ЕстьNull(втБонусыПоНоменклатуре2.Бонус,0) * СебестоимостьОбороты.КоличествоОборот) как БонусПоставщика,
				|	СУММА(СебестоимостьОбороты.СтоимостьОборот) 
				|		- СУММА( ЕстьNull(втБонусыПоНоменклатуре2.Бонус,0)*СебестоимостьОбороты.КоличествоОборот ) КАК СтоимостьОборотБонус
				
			    |ИЗ
			    |	РегистрНакопления.ПродажиСебестоимость.Обороты(&ДатаНач, &ДатаКон, Регистратор, ) КАК СебестоимостьОбороты
				//==============================бонусы по поступлениям!====================================
			    |		ЛЕВОЕ СОЕДИНЕНИЕ втБонусыПоНоменклатуре2 КАК втБонусыПоНоменклатуре2
			    |		ПО СебестоимостьОбороты.Номенклатура = втБонусыПоНоменклатуре2.Номенклатура
			    |			И СебестоимостьОбороты.ДокументОприходования = втБонусыПоНоменклатуре2.ДокументПоступления
			    |
			    |СГРУППИРОВАТЬ ПО
			    |	СебестоимостьОбороты.Номенклатура,
			    |	СебестоимостьОбороты.ЗаказПокупателя,
			    |	СебестоимостьОбороты.Подразделение,
			    |	СебестоимостьОбороты.Регистратор) КАК ТаблицаРегистраСебестоимость
			//+++)	
