Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	|;"+	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	    	...
	    	|	СУММА(ЕСТЬNULL(ТаблицаРегистраСебестоимость.СтоимостьОборот, 0) - ЕСТЬNULL(ТаблицаРегистраСебестоимость.СтоимостьОборот, 0) / 1.18 * ЕСТЬNULL(втБонусыПоНоменклатуре.Бонус, 0) / 100) КАК СебестоимостьСУчетомБонуса,
	    	|	МАКСИМУМ(ЕСТЬNULL(втБонусыПоНоменклатуре.Бонус, 0)) КАК ПроцентБонуса,
	    	|	ВЫРАЗИТЬ(ВЫБОР
	    	|			КОГДА СУММА(ЕСТЬNULL(ТаблицаРегистраСебестоимость.СтоимостьОборот, 0) - ЕСТЬNULL(ТаблицаРегистраСебестоимость.СтоимостьОборот, 0) / 1.18 * ЕСТЬNULL(втБонусыПоНоменклатуре.Бонус, 0) / 100) <> 0
	    	|				ТОГДА 100 * (СУММА(ТаблицаРегистра.СтоимостьОборот) - СУММА(ЕСТЬNULL(ТаблицаРегистраСебестоимость.СтоимостьОборот, 0) - ЕСТЬNULL(ТаблицаРегистраСебестоимость.СтоимостьОборот, 0) / 1.18 * ЕСТЬNULL(втБонусыПоНоменклатуре.Бонус, 0) / 100)) / СУММА(ТаблицаРегистраСебестоимость.СтоимостьОборот - ТаблицаРегистраСебестоимость.СтоимостьОборот / 1.18 * ЕСТЬNULL(втБонусыПоНоменклатуре.Бонус, 0) / 100)
	    	|			ИНАЧЕ 0
	    	...
	    	|		ИНАЧЕ СУММА(ТаблицаРегистра.СтоимостьОборот)
	    	|	КОНЕЦ - ВЫБОР
	    	|		КОГДА СУММА(ЕСТЬNULL(ТаблицаРегистраСебестоимость.СтоимостьОборот, 0) - ЕСТЬNULL(ТаблицаРегистраСебестоимость.СтоимостьОборот, 0) / 1.18 * ЕСТЬNULL(втБонусыПоНоменклатуре.Бонус, 0) / 100) ЕСТЬ NULL 
	    	|			ТОГДА 0
	    	|		ИНАЧЕ СУММА(ЕСТЬNULL(ТаблицаРегистраСебестоимость.СтоимостьОборот, 0) - ЕСТЬNULL(ТаблицаРегистраСебестоимость.СтоимостьОборот, 0) / 1.18 * ЕСТЬNULL(втБонусыПоНоменклатуре.Бонус, 0) / 100)
	    	|	КОНЕЦ КАК ВаловаяПрибыльСУчетомБонуса
	    	|{ВЫБРАТЬ
	    	|	ТаблицаРегистра.ДоговорКонтрагента.Владелец.* КАК Покупатель,
	    	...
	    	|	МАКСИМУМ(ПроцентБонуса),
	    	|	ВЫРАЗИТЬ(ВЫБОР
	    	|			КОГДА СУММА(СебестоимостьСУчетомБонуса) <> 0
	    	|				ТОГДА 100 * (СУММА(Стоимость) - СУММА(СебестоимостьСУчетомБонуса)) / СУММА(СебестоимостьСУчетомБонуса)
	    	|			ИНАЧЕ 0
	    	|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК ПроцентНаценкиСУчетомБонуса,
