		...
		Для Каждого СтрокаДокумента Из НайденныеСтроки Цикл
			...
			Если НадоСоздатьДокумент Тогда
				...
				Если НовДок.РасшифровкаПлатежа.Количество()>0 Тогда
					//Для Юниверсал логистик сумму НДС считаем по каждому счету (как сумма счета/118*18) и складываем
...
Функция РассчитатьСуммуНДСпоСчетам(НовДок)
	...
	Для Каждого времТЗстр из времТЗ Цикл
		ТекСуммаНДС = ТекСуммаНДС+Окр(времТЗстр.СуммаПлатежа/118*18,2,1);
	КонецЦикла;
	возврат ТекСуммаНДС;
...
Процедура ПриОткрытии()
	...
	ОтветственныйЗаДоговор = Справочники.Пользователи.НайтиПоКоду("Новикова С.");
