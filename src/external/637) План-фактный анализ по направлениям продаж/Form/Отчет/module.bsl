...
Процедура Отчет(ТабДок, ДатаКон, ДатаНач) Экспорт
	...
	КонецЦикла;
	ТабДок.Область("КПО").Текст = Окр(КоэффициентЭффективностиОбъекта, 2);
	...
	Пока Результат.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Если НЕ Выборка.ПоказательЭффективности.ТекстЗапроса = "" Тогда
				...
				Если Выборка.ПараметрПоказателя <> Справочники.ПараметрыПоказателейЭффективности.ПустаяСсылка() Тогда
					...
					//	ИначеЕсли СтрокаТЗ.Тип = Тип("СправочникСсылка.МоделиТоваров") Тогда
					//		ТекстЗапроса = Лев(ТекстЗапроса, СтрДлина(ТекстЗапроса)-1)+ " И Номенклатура.Модель В(&СписокПараметров))";
					//	ИначеЕсли СтрокаТЗ.Тип = Тип("ПеречислениеСсылка.ВидыТоваров") Тогда
					//		ТекстЗапроса = Лев(ТекстЗапроса, СтрДлина(ТекстЗапроса)-1)+ " И Номенклатура.ВидТовара В(&СписокПараметров))";																	
					//	КонецЕсли;
					//КонецЦикла;
					//// Сакулина
		...
		Пока РезультатПоМенеджерам.Следующий() Цикл
			...
			Пока Выборка.Следующий() Цикл
				...
				ТабДок.Вывести(ОбластьПоказательЭффективности, Выборка.Уровень()+1);
			КонецЦикла;
			ТабДок.Область("КПМ").Текст = Окр(КоэффициентЭффективностиМенеджера, 2); 
			КоэффициентЭффективностиМенеджера = 0;
		КонецЦикла;
