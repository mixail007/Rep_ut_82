...
Функция ЗаполнитьНастройкиЗапросаИнвестиции(ДатаКон, ДатаНач) //Инвестиции
	...
	Если Не ПоВсем Тогда 	
		...
		ЗапросИнв.Текст = "ВЫБРАТЬ
		                  ...
		                  |		NULL,
						  |		ВЫБОР
						  |			КОГДА &Организация = &WG
						  |					ИЛИ &Организация = &Турция
						  |				ТОГДА ВЫБОР
						  |						КОГДА ЗатратыОбороты.Подразделение = &Подразделение
						  |								И ЗатратыОбороты.Регистратор.Организация <> &Организация
						  ...
						  |		КОНЕЦ
		                  |	ИЗ
		                  |		РегистрНакопления.Затраты.Обороты(&ДатаНач, &ДатаКон, Регистратор, ) КАК ЗатратыОбороты") + ") КАК ПЗ";
