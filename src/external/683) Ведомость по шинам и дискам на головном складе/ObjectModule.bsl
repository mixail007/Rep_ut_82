Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	ОбщийОтчет.мНазваниеОтчета = "Ведомость по товарам на складах (шины, диски)";
	...
	//+++ Шарафутдинов 08.02.2018
	//видТовара
	СЗ = новый СписокЗначений;
	СЗ.Добавить(Перечисления.ВидыТоваров.Шины);
	СЗ.Добавить(Перечисления.ВидыТоваров.Диски);
	
	ОбщийОтчет.ПостроительОтчета.Отбор.НоменклатураВидТовара.ВидСравнения = ВидСравнения.ВСписке;
	ОбщийОтчет.ПостроительОтчета.Отбор.НоменклатураВидТовара.Использование= Истина; 
	ОбщийОтчет.ПостроительОтчета.Отбор.НоменклатураВидТовара.Значение 	  = СЗ; 
	
	//Склады   	
	Подразделения = новый СписокЗначений;
	Подразделения.Добавить(Справочники.Подразделения.ПустаяСсылка());
	Подразделения.Добавить(Справочники.Подразделения.НайтиПоКоду("00005")); //Головное подразделение Ярославль
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	НЕ Склады.Транзитный
		|	И Склады.Подразделение В(&Подразделения)		
		|	И НЕ Склады.ПометкаУдаления";

	Запрос.УстановитьПараметр("Подразделения", Подразделения);

	СЗ_Склады = новый СписокЗначений;
	СЗ_Склады.ЗагрузитьЗначения(Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка"));  	
	
	
	ОбщийОтчет.ПостроительОтчета.Отбор.Склад.ВидСравнения 				= ВидСравнения.ВСписке;
	ОбщийОтчет.ПостроительОтчета.Отбор.Склад.Использование 	  			= Истина;
	ОбщийОтчет.ПостроительОтчета.Отбор.Склад.Значение 					= СЗ_Склады;   
