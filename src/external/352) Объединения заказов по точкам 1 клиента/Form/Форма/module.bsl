...
функция УжеЕстьЗадача(Заказ1, Тема1)
	...
	Запрос.УстановитьПараметр("Объект1", Заказ1);
	...
	|
		...
		Пока i<N цикл
			...
			Если НельзяОбъединятьНеПодтвержденныйЗаказВКоторомЕстьТоварыСтопЛиста(стр1ЗаказПокупателя) тогда
				...
				//31.10.2018 - 11.02.2019 была задача-уведомление!
				//ПослатьЗадачу()
				Если УжеЕстьЗадача(стр1ЗаказПокупателя, Тема1) тогда
					#Если Клиент тогда
					Сообщить("Уже есть задача на согласование Стоп-листа по данному заказу.", СтатусСообщения.Информация);	
					#КонецЕсли
				Иначе	//сразу создаём... не дожидаясь регл.задания!
					ОбменСУТИнтернетМагазин.СоздатьЗадачуОповещениеПоСтопЛисту(стр1ЗаказПокупателя, Исполнитель1, Тема1); //11.02.2019
				КонецЕсли;
...
//+++( 30.10.2018 --- Задача № 57855  ----
		...
		флРазрешенСтопЛист = ЛОЖЬ;
		Исполнитель1 = справочники.Пользователи.НайтиПоКоду("Смирнов А");
		Тема1 		 = "Надо РАЗРЕШИТЬ Стоп-лист по Заказу ИМ !"; // фикс!
		пока i<N цикл
			стр1ЗаказПокупателя = ТабличноеПоле1[i].ЗаказПокупателя;
			Если НельзяОбъединятьНеПодтвержденныйЗаказВКоторомЕстьТоварыСтопЛиста(стр1ЗаказПокупателя) тогда
				#Если Клиент тогда
