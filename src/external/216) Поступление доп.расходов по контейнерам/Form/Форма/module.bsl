...
Процедура КоманднаяПанельФормыОбновить(Кнопка)
	Если ДатаНач='00010101' и ДатаКон='00010101' тогда
		ДатаНач=НачалоМесяца( ТекущаяДата() ) ; // с 1-го числа текущего месяца
		ДатаКон=ДобавитьМесяц(ДатаНач, 6); // + полгода
	КонецЕсли;	
	ОбновитьОтчет();
...
Процедура ПолеНастройкиСсылкаПриИзменении(Элемент)
	
	//+++ 27.04.2012  после изменения в форме - теперь меняется и в запросе
	ПостроительОтчета.Параметры.КонтрагентТоваров  =  ?(ЗначениеЗаполнено(ЭтотОтчет.СсылкаНаОбъект),ЭтотОтчет.СсылкаНаОбъект, Справочники.Контрагенты.ПустаяСсылка()) ; //+++ 27.04.2012
	
	ПостроительОтчета.Отбор.ПоставщикТоваров.видСравнения =видСравнения.равно;
	ПостроительОтчета.Отбор.ПоставщикТоваров.Значение = ПостроительОтчета.Параметры.КонтрагентТоваров;
	ПостроительОтчета.Отбор.ПоставщикТоваров.использование = (ЭтотОтчет.СсылкаНаОбъект <> Справочники.Контрагенты.ПустаяСсылка()  );
КонецПроцедуры
