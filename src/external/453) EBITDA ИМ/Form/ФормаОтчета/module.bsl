...
Функция ПолучитьПоказатели()
	...
	Пока Рез.Следующий() цикл
		...
		Показатели.НДСЗакупки = рез.Себестоимость*чНДС/(100+чНДС);//28.12.2018
	КонецЦикла;
...
Процедура КнопкаСформироватьНажатие(Кнопка)
 ...
 КонецЦикла;
	...
	чНДС = ПолучитьСтавкуНДС( СтавкаНДС_2019 );	
	кНДС = чНДС/(100+чНДС);
	Область.Параметры.Вход_ндс_АЭ=ПоказателиАЭ.Себестоимость*кНДС+ПоказателиАЭ.НДСУслуги;//-ПоказателиАЭ.НДСПродажи;
	Область.Параметры.Исх_ндс_АЭ =(ПоказателиАЭ.Стоимость)  *кНДС;//28.12.2018
	...
	Область.Параметры.Вход_ндс_ЯШТ=СебестоимостьЯШТ*кНДС+НДСВходЯШТУслуги;
	Область.Параметры.Исх_ндс_ЯШТ =     (ОборотЯШТ)*кНДС;//28.12.2018
	...
	кНалогНаПрибыль = 0.2; //20%
	Область.Параметры.НалогНаПрибыль_ЯШТ=0;//((ВаловыйДоходЯШТ) * кНДС - ЗатратыБезНалоговЯШТ)*кНалогНаПрибыль;
	Область.Параметры.НалогНаПрибыль_АЭ =0;//((ПоказателиАЭ.Наценка)* кНДС - ПоказателиАЭ.Затраты)*кНалогНаПрибыль;
	
	СуммаНалогаНаПрибыль = (Область.Параметры.EBITDA_P_Итого-Область.Параметры.НДС_Итого)*кНалогНаПрибыль; // 20%
	Область.Параметры.НалогНаПрибыль_Итого=?(СуммаНалогаНаПрибыль<0,0,СуммаНалогаНаПрибыль);
