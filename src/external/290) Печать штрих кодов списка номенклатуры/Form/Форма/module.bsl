...
Процедура КнопкаВыполнитьНажатие(Кнопка)
	...
	ОбластьСтрока128=Макет.ПолучитьОбласть("Строка2|Столбец");// Code-128
	...
	если ТипЗнч(НоменклатураСписок)=Тип("СправочникСсылка.Номенклатура") Тогда
		...
		Для сч=1 по КоличествоШтрихКодов Цикл
			...
			конецЕсли;
		Иначе //+++ 22.01.2019 ++++++++++++++++++++++++++++++++++++++++++++	 
		ОбластьСтрока128.Параметры.Наименование2 = Наименование;
			ОбШтрихКод=ОбластьСтрока128.Рисунки.ШК2.Объект;
			...
			Если ПринтерЭтикеток Тогда
					ТабДок.ВывестиГоризонтальныйРазделительСтраниц();			
				ТабДок.Вывести(ОбластьСтрока128);
			иначе	
				Если сч%2=1 Тогда
					Если Не ТабДок.ПроверитьВывод(ОбластьСтрока128) Тогда
						ТабДок.ВывестиГоризонтальныйРазделительСтраниц();			
					КонецЕсли;
					ТабДок.Вывести(ОбластьСтрока128);
				Иначе
					ТабДок.Присоединить(ОбластьСтрока128);
				КонецЕсли;
			конецЕсли;
