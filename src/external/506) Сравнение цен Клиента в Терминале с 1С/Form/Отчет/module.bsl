...
Процедура ДействияФормыОтчетСформировать(Кнопка)
	...
	Если N=0 тогда
		Предупреждение("Нет различий в ценах!",60);
	иначе
		Предупреждение("Обнаружено "+строка(N)+" различий в ценах!",60);
	КонецЕсли;	
...
N = -1;	
	...
	Если ФлагСтандарт1С тогда
		...
		сообщить(строка(ТекущаяДата())+" Начало операции сравнения данных для клиента: "+строка(Клиент));
...
Процедура ДействияФормыЦены1С(Кнопка) 
	...
	КонецЕсли;	
	сообщить("Получен список из "+строка(списТов.Количество())+" товаров для клиента: "+строка(Клиент));
