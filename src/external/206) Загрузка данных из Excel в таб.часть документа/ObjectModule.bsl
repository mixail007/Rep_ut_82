...
//
Функция ЗагрузитьДанные(ТабличныйДокумент, Индикатор, Производитель = неопределено) Экспорт
	...
	Если Вопрос("Загрузить "+КоличествоЭлементов  + ТекстВопросаИсточника, РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да Тогда
		...
		Если РежимЗагрузки = 0 Тогда
			...
			Если Не СтрокиПоиска.Количество() = 0 Тогда
				...
				Если МетаданныеИсточника.Имя="Номенклатура" и ЗначениеЗаполнено(Производитель) тогда
					ТекстЗапроса = ТекстЗапроса+" и Справочник.Номенклатура.Производитель = &Производитель "
				КонецЕсли;	
			...
			КонецЕсли;
		ИначеЕсли РежимЗагрузки = 1 Тогда
			//09.09.14 Смирнов спрашиваем очищать ли ТЧ
		...
		Для К = ПерваяСтрокаДанныхТабличногоДокумента По ТабличныйДокумент.ВысотаТаблицы Цикл
			...
			Отказ = Ложь;
			ТекущаяСтрока = КонтрольЗаполненияСтроки(ТабличныйДокумент, К, ТекстыЯчеек, ,производитель);
			Если РежимЗагрузки =0 Тогда
			...
			//+++( авто установка Ед.изм, коэффициента и Ставки НДС - 16.04.2012
			попытка
				Объект.единицаизмерения = Объект.номенклатура.единицаХраненияОстатков; 
				Объект.коэффициент = 1;
				Если НЕ ЗначениеЗаполнено(Объект.СтавкаНДС) и  НЕ ЗначениеЗаполнено(Объект.СуммаНДС) тогда
...
//
Функция КонтрольЗаполненияСтроки(ТабличныйДокумент, НомерСтроки, ТекстыЯчеек = Неопределено, КоличествоОшибок = 0, Производитель = неопределено)
	...
	Для каждого КлючИЗначение Из Колонки Цикл
		...
		Если Колонка.Пометка Тогда
			...
			ИначеЕсли Не Колонка.НомерКолонки = 0 Тогда
				...
				Если Не ОбработатьОбласть(ТабличныйДокумент.Область("R"+Формат(НомерСтроки,"ЧГ=")+"C"+Формат(Колонка.НомерКолонки,"ЧГ=")), Колонка, ТекущаяСтрока, ТекстыЯчеек,Производитель) Тогда
...
//  ТекстыЯчеек    - массив текстов ячеек строки
//
Функция ОбработатьОбласть(Область, Колонка, ТекущиеДанные, ТекстыЯчеек, Производитель = неопределено)
	...
	ИначеЕсли ПустаяСтрока(Представление) Тогда
		Результат = Неопределено;
	Иначе
		НайденныеЗначения = ПолучитьВозможныеЗначения(Колонка, Представление, Примечание, ТекущиеДанные, производитель);
...
//
Функция ПолучитьВозможныеЗначения(Колонка, Представление, Примечание, ТекущиеДанные, производитель=неопределено)
	...
	Для каждого ТипРеквизита Из Тип.Типы() Цикл
		...
		Иначе
			...
			ИначеЕсли Не МетаданныеТипа = Неопределено Тогда
				...
				//31.01.17 Смирнов, отбор по производителю
				Если МетаданныеТипа.ПолноеИмя()="Справочник.Номенклатура" и ЗначениеЗаполнено(Производитель) тогда
					Запрос.Текст = Запрос.Текст + "
				|  и _Таблица.Производитель = &Производитель";
				Запрос.УстановитьПараметр("Производитель", производитель);
				КонецЕсли;
				...
				//+++Шарафутдинов 13.09.2018
				Если МетаданныеТипа.ПолноеИмя()="Справочник.Номенклатура" и ИскатьПо ="Артикул" и ЕстьНеЦифры(Представление) тогда
					Запрос.Текст = СтрЗаменить(Запрос.Текст," = &Представление"," ПОДОБНО &Представление");
					Запрос.УстановитьПараметр("Представление","%" + Представление + "%");
				КонецЕсли;
				//---Шарафутдинов 13.09.2018
...
Функция мПолучитьНастройки() Экспорт
	...
	Для каждого ЗагружаемыйРеквизит Из ТаблицаЗагружаемыхРеквизитов Цикл
		...
		ОбластьСтроки.Параметры.НомерСтроки = ЗагружаемыйРеквизит.НомерКолонки;//+++ 16.07.2013 - добавлен в макет и здесь!
		...
		Попытка
			...
			Пока Не ПустаяСтрока(ИмяРеквизита) Цикл
				...
				КонецЕсли;
					...
					СтруктураУмолчанияРеквизита.Вставить("НомерКолонки", мПривестиКЧислу(Настройки.Область(ТекущаяСтрокаОбласти + "C11").Текст));//+++ 16.07.2013
		...
		Для каждого КлючИЗначение Из СтруктураУмолчаний Цикл
			...
			Если Не ЗагружаемыйРеквизит = Неопределено Тогда
				...
				Если Индекс >= НомерКолонкиОформления Тогда
					...
					ТаблицаЗагружаемыхРеквизитов.Сдвинуть(ЗагружаемыйРеквизит, НомерКолонкиОформления - Индекс);
					Если Колонка.Пометка Тогда
						ЗагружаемыйРеквизит.НомерКолонки = Колонка.НомерКолонки; // = НомерКолонки; //+++ 15.07.2013
						НомерКолонки = НомерКолонки + 1;
					КонецЕсли;
					НомерКолонкиОформления = НомерКолонкиОформления + 1;
