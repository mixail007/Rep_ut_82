...
Процедура ВычислитьЦикломатическуюСложность(Знач Текст, УзелДереваРезультатов) Экспорт
	...
	Пока Текст <> "" Цикл
		...
		Попытка
			...
			ЦикломатическаяСложность = 1 + ЧислоВхожденийСлова(ТекстМетода, "Тогда") + ЧислоВхожденийСлова(ТекстМетода, "Then") + ЧислоВхожденийСлова(ТекстМетода, "Цикл") + ЧислоВхожденийСлова(ТекстМетода, "Do") + ?(ОбрабатыватьУсловныйОператор, ЧислоВхожденийСлова(ТекстМетода, "?"), 0);
...
Процедура ВычислитьСложностьТекстаМодуля() Экспорт
	Состояние("Анализ текста модуля...");
...
Процедура ВычислитьСложностьМодулейВКаталоге(ПутьКаталога, МаскаФайлов = "", ВключатьПодкаталоги = Ложь) Экспорт
	...
	Если МассивФайлов.Количество() = 0 Тогда
		Предупреждение("Подходящий файлов не найдено!");
		Возврат;
	КонецЕсли; 
	...
	Для каждого ТекущийФайл Из МассивФайлов Цикл
		Состояние("Анализ файла " + ТекущийФайл.ПолноеИмя + "...");
		ОбработкаПрерыванияПользователя();
	...
	Состояние("");
	Предупреждение("Анализ завершен!");
КонецПроцедуры

ДеревоРезультатовАнализа.Колонки.Добавить("Путь", Новый ОписаниеТипов("Строка"),, 20);
ДеревоРезультатовАнализа.Колонки.Добавить("Файл", Новый ОписаниеТипов("Строка"),, 20);
ДеревоРезультатовАнализа.Колонки.Добавить("ИмяМетода", Новый ОписаниеТипов("Строка"), "Имя метода", 30);
ДеревоРезультатовАнализа.Колонки.Добавить("ЦикломатическаяСложность", Новый ОписаниеТипов("Число"), "Цикломатическая сложность", 10);