...
Процедура Отчет(ТабДок) Экспорт   // диски
  ...
  // АКБ=Справочники.Номенклатура.НайтиПоКоду("0080021"); // аккумуляторы
  // Nmax = 50;
...
Запрос.Текст= "ВЫБРАТЬ 
           ...
           //   |	Б.Номенклатура.Артикул КАК Артикул,
              ...
              |	Б.Производитель КАК Производитель,
...
//============================================================================================
			...
			|	РегистрСведений.НоменклатураИмпорт КАК НоменклатураИмпорт)
					...
					//|И НЕ Б.Номенклатура.Производитель В (&списокПроизводителей)
					//|И ВЫБОР
					//|КОГДА Б.Номенклатура.Производитель = &алкаста
					//|	ТОГДА Б.Номенклатура.Модель В (&списокМоделей)
					//|ИНАЧЕ ИСТИНА
					//|КОНЕЦ
					//Плотников 26.05.17 запрет на отгрузку моделей временный-----------------------------------------
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++			
   ...
   Запрос.УстановитьПараметр("ТекущаяДатаСезЗак", ДобавитьМесяц(ТекущаяДата(),-1) ); //07.12.2016
	...
	//Запрос.УстановитьПараметр("ГрузовыеДиски", Справочники.Номенклатура.НайтиПоКоду("9004163")); // грузовыедиски
...
i=0; N=ВыборкаДетали.Количество(); проц = макс(10, мин(100, цел(N/100)));
	...
	Если ЗначениеЗаполнено(ВыборкаДетали.Номенклатура)
		//и ВыборкаДетали.Номенклатура.ВидТовара = Перечисления.ВидыТоваров.Диски 
		Тогда
		
		//+++( 28.03.2018  -- добавлен 22 столбец
		...
		//+++)
			...
			//28.09.2015
			//папкаПроизводителя = нрег(ВыборкаДетали.Производитель); //ТАК быстрее!
			...
			//	КонецЕсли;
			//КонецЕсли;
			путьККартинкамТов = ПутьКДискам ; //21.08.2018 БЕЗ производителя! + папкаПроизводителя+"/"
	...
	КонецЦикла;
    Состояние("Идет автогруппировка...");
...
|////////////////////////////////////////////////////////////////////////////////
|
|ВЫБРАТЬ Б.Номенклатура, Б.Номенклатура.Родитель Родитель, Б.Номенклатура.Код Код, Б.Номенклатура.ТипоРазмер Размер, 
| Б.Номенклатура.Производитель Производитель, Б.ОстатокДляКлиента,
| Б.СвободныйОстатокДляКлиента, 

 //+++ 04.09.2014 - через Объединение запросов
| Б.СвободныйОстатокЯШТ как СвободныйОстатокЯШТ,
...
//+++ 17.12.2013 вывод ">50"		
	Если ВидОтчета=0 и ВыборкаДетали.ТипЗаписи() = ТипЗаписиЗапроса.ИтогПоГруппировке тогда  //+++ вывод >50
		...
		Если ВыборкаДетали.ОстатокДляКлиента>Nmax тогда
			ОбластьДетальныхЗаписей.Параметры.ОстатокДляКлиента 		= ">"+строка(Nmax);
		иначеЕсли ВыборкаДетали.СвободныйОстатокДляКлиента<0 тогда
			ОбластьДетальныхЗаписей.Параметры.ОстатокДляКлиента 		= 0;
		КонецЕсли;
		
		Если ВыборкаДетали.СвободныйОстатокДляКлиента>Nmax тогда
			ОбластьДетальныхЗаписей.Параметры.СвободныйОстатокДляКлиента = ">"+строка(Nmax);
		иначеЕсли ВыборкаДетали.СвободныйОстатокДляКлиента<0 тогда
			ОбластьДетальныхЗаписей.Параметры.СвободныйОстатокДляКлиента = 0;
		КонецЕсли;	
		//+++ 04.09.2014
		Если ВыборкаДетали.СвободныйОстатокЯШТ>Nmax тогда
			ОбластьДетальныхЗаписей.Параметры.СвободныйОстатокЯШТ = ">"+строка(Nmax);
		...
		Если ВыборкаДетали.КоличествоВПутиИзЯрославля>Nmax тогда
			ОбластьДетальныхЗаписей.Параметры.КоличествоВПутиИзЯрославля = ">"+строка(Nmax);
...
Процедура ДействияФормыВыгрузитьВExcel(Кнопка, путь0=неопределено, МинНомФил=1)  Экспорт
...
стр1= таблФилиалов.Добавить();
стр1.Регион ="Екатеринбург";  	
стр1.НазвАнгл = "ekaterinburg";
...
стр1.Регион ="Тольятти";        
стр1.НазвАнгл = "tolyatti";
стр1.Подразделение = справочники.Подразделения.НайтиПоКоду("00172");    
стр1.Номер = 5;
КонецЕсли;

//Если Флажок2 тогда нПерекМакс=1; КонецЕсли;
Флажок1 = Истина; //всегда с остатками ЯШТ!

таблВидов = новый ТаблицаЗначений;
таблВидов.Колонки.Добавить("Вид");
таблВидов.Колонки.Добавить("ВидАнгл");
таблВидов.Колонки.Добавить("КолНач");
таблВидов.Колонки.Добавить("КолКон");
...
Сообщить(строка(ТекущаяДата())+" === выгрузки данных "+Филиал.Регион+" ("+?(Переключатель1=0,"для клиентов","для менеджеров")+") в файл: "+ИмяФайла+" ===");
	...
	Если Переключатель1=0 тогда
		...
		//	Excel.Application.Visible = false; //29.01.2018
...
Для каждого стрВид из таблВидов цикл
	...
	//Переключатель1 = 0 тогда: 1-4 (вид)
	//Переключатель1 = 1 тогда  с 6-ой закладки - Шины, Ростов:  6=1+1+4, 10=1+1+8, 14=1+1+12...
