...
Процедура мСообщитьОбОшибке(ТекстСообщения, Отказ = Ложь, Заголовок = "") Экспорт
	...
	#Иначе
		...
		Если НЕ мЗначениеНеЗаполнено(Заголовок) Тогда
			Сообщить(Заголовок);
			Заголовок = "";
		КонецЕсли;
...
Функция ЗаписатьОбъект(Объект, ТекстыЯчеек = Неопределено)
	...
	Если Не Отказ  Тогда
		Попытка
			Объект.Записать(ЗамещатьСуществующиеЗаписи); //+++
		Исключение
			Отказ = Истина;
			ОписаниеОшибки = ОписаниеОшибки();
		КонецПопытки;
...
Функция мПрочитатьТабличныйДокументИзExcel(ТабличныйДокумент, ИмяФайла, НомерЛистаExcel = 1) Экспорт
	...
	Для Row = 1 По RowCount Цикл
		...
		Для Column = 1 По ColumnCount Цикл
			ТабличныйДокумент.Область("R" + Формат(Row, "ЧГ=") +"C" + Формат(Column, "ЧГ=")).Текст = ExcelЛист.Cells(Row,Column).text;//+++ 17.09.2014 текст а не значение!
		КонецЦикла;
...
Функция ЗагрузитьДанные(ТабличныйДокумент, Индикатор) Экспорт
	...
	Если НоменклатураКонтрагентов тогда
		ТекущиеДанные = ТаблицаЗагружаемыхРеквизитов.Найти("Номенклатура", "ПредставлениеРеквизита");
		Контрагент = ?(ТекущиеДанные=неопределено, неопределено, ТекущиеДанные["ЗначениеПоУмолчанию"]);
		
		Если ТекущиеДанные<>неопределено и Контрагент = Справочники.Контрагенты.ПустаяСсылка() тогда
			сигнал();
			Предупреждение("Не выбран Контрагент для установки!
	...
	Если Вопрос("Загрузить "+КоличествоЭлементов  + ТекстВопросаИсточника, РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да Тогда
		...
		Загружено = 0;  i=0; СписокЗначенийКонтр = новый СписокЗначений;	
		...
		Для К = ПерваяСтрокаДанныхТабличногоДокумента По ТабличныйДокумент.ВысотаТаблицы Цикл
			...
			ИначеЕсли РежимЗагрузки = 2 Тогда
				Объект = РегистрыСведений[МетаданныеИсточника.Имя].СоздатьМенеджерЗаписи();
							...
							ТекущиеДанные1 = ТаблицаЗагружаемыхРеквизитов.Найти("Контрагент", "ПредставлениеРеквизита");
							Если ТекущиеДанные1 <> неопределено тогда
								Контр1 = ТекущиеДанные1["ЗначениеПоУмолчанию"];
								Объект.Контрагент = Контр1;
							КонецЕсли;	
				...
				Для каждого КлючИЗначение Из ТекущаяСтрока Цикл
					...
					Если ИзмеренияРегистра.Свойство(КлючИЗначение.Ключ) Тогда
						//+++(
						Если НоменклатураКонтрагентов и КлючИЗначение.Ключ="Номенклатура" тогда 
							
							Запрос = Новый Запрос;
							Запрос.УстановитьПараметр("Контрагент", Объект.Контрагент);
							
							ТекущиеДанные = ТаблицаЗагружаемыхРеквизитов.Найти("Номенклатура", "ПредставлениеРеквизита");
							имяПоля = ?(ТекущиеДанные=неопределено, "Наименование", ТекущиеДанные["ИскатьПо"]);
							
							Nном = число(ТекущиеДанные["НомерКолонки"]);
							
							Запрос.УстановитьПараметр("ЗначениеПоля", СокрЛП(ТекстыЯчеек[Nном])  );
							...
							Запрос.Текст = "ВЫБРАТЬ
							               ...
							               |	И Выразить(НоменклатураКонтрагентов."+имяПоля+"НоменклатурыКонтрагента как Строка(250)) = &ЗначениеПоля
							               |
							               |УПОРЯДОЧИТЬ ПО
							               |	Номенклатура
							               |АВТОУПОРЯДОЧИВАНИЕ";
							...
							Если Выборка.Следующий() тогда
								Номенклатура2 = Выборка.Номенклатура;
								Объект[КлючИЗначение.Ключ] = Номенклатура2;
								Если Выборка.Количество()>1 тогда
									сообщить("Неоднозначное соответствие ("+строка(Выборка.Количество())+" эл.) для '"+СокрЛП(ТекстыЯчеек[Nном])+"' в рег.свед. ""НоменклатураКонтрагентов"" !", СтатусСообщения.Информация);
								КонецЕсли;	
								
							иначе
								Номенклатура2 = справочники.Номенклатура.ПустаяСсылка();
								Объект[КлючИЗначение.Ключ] = Номенклатура2;  //нельзя цену или Остаток БЕЗ номенклатуры!
								  СписокЗначенийКонтр.Добавить( СокрЛП(ТекстыЯчеек[Nном]) );
								//Если СокрЛП(ТекстыЯчеек[Nном])<>"" тогда
								//	
								//	Если НоменклатураКонтрагентов тогда
								//	регСведНомКонтр = РегистрыСведений.НоменклатураКонтрагентов.СоздатьМенеджерЗаписи();
								//	регСведНомКонтр.Контрагент = Объект.Контрагент;  // не пустой
								//	регСведНомКонтр.Номенклатура = справочники.Номенклатура.ПустаяСсылка();
								//	регСведНомКонтр.ХарактеристикаНоменклатуры = справочники.ХарактеристикиНоменклатуры.ПустаяСсылка();
								//	регСведНомКонтр[имяПоля+"НоменклатурыКонтрагента"] = СокрЛП(ТекстыЯчеек[Nном]);
								//	регСведНомКонтр.Прочитать();
								//	
								//	регСведНомКонтр.Контрагент = Объект.Контрагент;  // не пустой
								//	регСведНомКонтр.Номенклатура = справочники.Номенклатура.ПустаяСсылка();
								//	регСведНомКонтр.ХарактеристикаНоменклатуры = справочники.ХарактеристикиНоменклатуры.ПустаяСсылка();
								//	
								//	регСведНомКонтр[имяПоля+"НоменклатурыКонтрагента"] = СокрЛП(ТекстыЯчеек[Nном]);
								//	регСведНомКонтр.ЕдиницаНоменклатурыКонтрагента = справочники.КлассификаторЕдиницИзмерения.НайтиПоКоду("796"); //шт.
								//	
								//		попытка
								//		регСведНомКонтр.Записать(); // НЕ перезаписывать!!!
										сообщить("---> Не нейдена Номенклатура в рег.свед. ""Номенклатура контрагентов"" для: "+СокрЛП(ТекстыЯчеек[Nном]), СтатусСообщения.Информация);
								//		Исключение
								//		Сообщить("для '"+СокрЛП(ТекстыЯчеек[Nном])+"' : "+ОписаниеОшибки(), СтатусСообщения.Внимание);
								//		КонецПопытки;
								//	КонецЕсли;
								//	
								//КонецЕсли;
			...
			Для каждого КлючИЗначение Из ТекущаяСтрока Цикл
				...
				Если Не ОбъектНайден или Колонки[КлючИЗначение.Ключ].Пометка  Тогда
					Попытка
						//+++( вставляем ранее найденную номенклатуру
						Если НЕ (НоменклатураКонтрагентов и КлючИЗначение.Ключ="Номенклатура") тогда 
							Объект[КлючИЗначение.Ключ] = КлючИЗначение.Значение;
						иначе
							Объект[КлючИЗначение.Ключ] = Номенклатура2;
						КонецЕсли;//+++)	
			...
			ИначеЕсли РежимЗагрузки = 2 Тогда
				...
				Если Не Отказ и НЕ (Объект.Номенклатура=справочники.номенклатура.ПустаяСсылка()) тогда //+++
					Если ЗаписатьОбъект(Объект, ТекстыЯчеек) тогда
						Сообщить(?(ОбъектНайден,"Изменена","Добавлена") + " запись № " + НомерТекущейСтроки + ".", СтатусСообщения.Информация);
						Загружено = Загружено + 1;
					Иначе
						Сообщить("Запись не " + ?(ОбъектНайден,"изменена","загружена") + ". № записи: " + НомерТекущейСтроки + ".", СтатусСообщения.Информация);
					КонецЕсли;	
				КонецЕсли;
				
			КонецЕсли; //РежимЗагрузки = 2
...
Функция ОбработатьОбласть(Область, Колонка, ТекущиеДанные, ТекстыЯчеек)
	...
	Иначе
		...
		ИначеЕсли НайденныеЗначения.Количество() = 1 Тогда
			Примечание = "";  //+++ 27.07.2012
			Результат = НайденныеЗначения[0];
			
		Иначе
			
			Примечание = "Несколько вариантов: "+НайденныеЗначения.Количество()+?(Примечание = "","", Символы.ПС+Примечание);
...
Функция мПолучитьНастройки() Экспорт
	...
	ОбластьШапки.Параметры.НоменклатураКонтрагентов                  = Формат(НоменклатураКонтрагентов,"БЛ=Ложь; БИ=Истина");   //+++
...
Процедура ЗаполнитьНастройкиКолонок(Настройки) Экспорт
	...
	Если ТипЗнч(Настройки) = Тип("ТабличныйДокумент") Тогда
		...
		Попытка
			...
			Пока Не ПустаяСтрока(ИмяРеквизита) Цикл
				...
				Иначе
					...
					СтруктураУмолчанияРеквизита.Вставить("НомерКолонки", мПривестиКЧислу(Настройки.Область(ТекущаяСтрокаОбласти + "C11").Текст));  //+++
				КонецЕсли;
		...
		НоменклатураКонтрагентов = (Настройки.Область( ?(ВерсияОбработки = "1.1","R5","R8") + "C5" ).Текст = "Истина");//+++
