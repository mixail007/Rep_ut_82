...
//-------------Определение автомобиля---------------------------------
	...
	ОбластьМакета.Параметры.Автомобиль = "Автомобиль: "+ТаблЗнач1[0].Автомобиль+"  "+ТаблЗнач1[0].ГосНомерАвтомобиля;
...
Процедура КоманднаяПанель1Пересчет(Кнопка)
	...
	Если АдресИзИнтернета Тогда
		...
		СведенияОфилиале  = СведенияОЮрФизЛице(Контр, ТекущаяДата() );
	//	если нашиКоординаты="" тогда
	        Адрес0 = ОписаниеОрганизации(СведенияОфилиале, "ФактическийАдрес,");
			нашиКоординаты = ОбменСУТИнтернетМагазин.получитьКоординаты(Адрес0);
	//	КонецЕсли;
...
//------------------------------------------------------------------
 ...
 Процедура ОсновныеДействияФормыМаршрут(Кнопка)
    ...
    КонецЦикла; 
    rtext =  rtext+"~"+rtext0;
	срКоорд =формат(срКоордШ,"ЧРД=.")+"%2C"+формат(срКоордД,"ЧРД=."); //НАОБОРОТ!	
...
иначе  //неотгруженные всегда!
		 ...
		 Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		                ...
		                |	ЗаказыПокупателей.ЗаказПокупателя.ДоговорКонтрагента.ОтветственноеЛицо КАК Ответственный,
		                |	ЕСТЬNULL(ЗаказыПокупателей.КоличествоОстаток, 0) КАК Количество,
		                |	ВЫБОР
		                |		КОГДА ЗаказыПокупателей.Номенклатура В ИЕРАРХИИ (&Камеры)
		                |			ТОГДА ЕСТЬNULL(ЗаказыПокупателей.КоличествоОстаток, 0)
		                |		ИНАЧЕ 0
		                |	КОНЕЦ КАК КолКамеры,
		                |	ВЫБОР
		                |		КОГДА ЗаказыПокупателей.Номенклатура В ИЕРАРХИИ (&Камеры)
		                |				ИЛИ ЗаказыПокупателей.Номенклатура В ИЕРАРХИИ (&Крепеж)
		                |			ТОГДА 0
		                |		ИНАЧЕ ВЫБОР
		                |				КОГДА ЕСТЬNULL(НормыЗагрузки.НормаЗагрузки, 0) = 0
		                |					ТОГДА ЗаказыПокупателей.Номенклатура.ЕдиницаХраненияОстатков.Объем / 0.033
		                |				ИНАЧЕ НормыЗагрузки.НормаЗагрузки
		                |			КОНЕЦ
		                |	КОНЕЦ * ЕСТЬNULL(ЗаказыПокупателей.КоличествоОстаток, 0) КАК Кубатура,
						
		                |	ЗаказыПокупателей.Номенклатура.ЕдиницаХраненияОстатков.Вес * ЕСТЬNULL(ЗаказыПокупателей.КоличествоОстаток, 0) КАК Вес,
						
						|	ЗаказыПокупателей.Номенклатура.ЕдиницаХраненияОстатков.Объем * ЕСТЬNULL(ЗаказыПокупателей.КоличествоОстаток, 0) 
						//+++ 30% прибавки только для дисков!
		                |		* выбор когда ЗаказыПокупателей.Номенклатура.ВидТовара = Значение(Перечисление.ВидыТоваров.Диски) 
						|		тогда 1.29
						|		иначе 1 Конец КАК Объем,
		                ...
		                |				И ЗаказПокупателя.Подразделение В (&СписокПодразделений)) КАК ЗаказыПокупателей
		                |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Заказы КАК ЗначенияСвойствОбъектов
		                |		ПО ЗаказыПокупателей.ЗаказПокупателя = ЗначенияСвойствОбъектов.Объект
		                |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НормыЗагрузки КАК НормыЗагрузки
		                |		ПО ЗаказыПокупателей.Номенклатура.Типоразмер = НормыЗагрузки.Типоразмер
		                |			И ЗаказыПокупателей.Номенклатура.ВидТовара = НормыЗагрузки.ВидПродукции
		                ...
		                |		ПО ЗаказыПокупателей.ЗаказПокупателя = ВложенныйЗапрос.Заказ
		                |ГДЕ
...
если НЕ ЗначениеЗаполнено(НаправлениеОтгрузки) тогда
	Предупреждение("Выберите одно или несколько направлений из списка!", 10);
	возврат;
КонецЕсли;

Запрос.УстановитьПараметр("Значение", НаправлениеОтгрузки);  //можно сделать список...
...
СписокПодразделений.Очистить(); //+++ Это список подразделений  
 ...
 КонецЕсли;
	...
	Если глТекущийПользователь.ОсновноеПодразделение.Код = "00106" тогда      // РнД
		нашиКоординаты = "47.245319,39.59017";
	ИначеЕсли глТекущийПользователь.ОсновноеПодразделение.Код = "00112" тогда // СПб
		нашиКоординаты = "59.8424869,30.4261098";// 
	Иначе   // глТекущийПользователь.ОсновноеПодразделение.Код = "00138"
...
Если СокрЛП(ИмяНастройки = "Основная") Тогда
	...
	+строка(СписокПодразделений[0].Значение.ПрефиксИБ)+")" //?(ЭлементыФормы.Неотгруженные.Значение," Неотгруженные","")
...
//-------------------------------------------------------------------------------------------
	...
	//+++ 09.08.2017 ----------- нумерация точек	
	k=0; флАвтоНумТочек = (ТЗЗаказов.Итог("Точка")=0);	
	 Если флАвтоНумТочек тогда
	 	N = ТЗЗаказов.Количество();
	 Иначе //уже есть номера точек
		N = 0;
		для каждого стр1 из ТЗЗаказов цикл
			если стр1.Точка > N тогда
				N= стр1.Точка;
			КонецЕсли;
		КонецЦикла;	
	 КонецЕсли;
	...
	Для Каждого СтрокаЗаказа Из ТЗЗаказов Цикл
		...
		k=k+1;
		если флАвтоНумТочек тогда //+++ 09.08.2017
			СтрокаЗаказа.Точка = k;
		КонецЕсли;
		
		СтрокаЗадания = Задание.ЗаказыПокупателей.Добавить();
		СтрокаЗадания.НомерОчереди = N + 1 - СтрокаЗаказа.Точка;//+++ 09.08.2017  - сразу в обратном порядке!
		...
		КонецЦикла;
	КонецЦикла;
	Задание.ЗаказыПокупателей.Сортировать("НомерОчереди");//+++ 09.08.2017 !!!
	...
	//+++ 12.09.2017 >> отгрузка с группы складов сборки ИМ если есть транспортная компания
	Если ЗначениеЗаполнено(Задание.ЗаказыПокупателей[0].ЗаказПокупателя.ТранспортнаяКомпания) тогда  
		Задание.Исключать = 1; //включать
		СписокГрупп1 = новый СписокЗначений;
		стр11 = Задание.СписокГрупп.Добавить();
		стр11.Флаг   = истина;
		стр11.Группа = Задание.Подразделение.СкладСборкиИМ.ГруппаСкладов;
	КонецЕсли;
...
КонецЦикла;
	 НадписьКатегорийВсе = 	Формат(ДатаФормы,"ДФ=MM")+" "+Формат(ДатаФормы,"ДФ=MMMM")+"\"+Формат(ДатаФормы,"ДФ=dd.MM.yyyy")
	+?(НаправлениеОтгрузки.Количество()=0,""," "+НаправлениеОтгрузкиТекст)+?(ЗначениеЗаполнено(Водитель)," - "+ФамилияИнициалыФизЛица(Водитель), "");
...
попытка
	подр = СписокПодразделений[0].Значение;
	
	Если подр.Код = "00106" тогда      // РнД
		нашиКоординаты = "47.245319,39.59017";
	ИначеЕсли подр.Код = "00112" тогда // СПб
		нашиКоординаты = "59.8424869,30.4261098";// 
	Иначе 
		нашиКоординаты = "56.781591,60.64839";// ЕКТ новый
	КонецЕсли;
...
Процедура ПолеВвода1ПриИзменении(Элемент)
	...
	Если значениеЗаполнено(ПолеВвода1) Тогда
		Водитель = ПолеВвода1.Наименование;
		ТелефонВодителя = ПолучитьКонтактнуюИнформацию(ПолеВвода1,"Телефон");
		Автомобиль = ПолучитьКонтактнуюИнформацию(ПолеВвода1, "Другое");
	Иначе
...
Процедура НаправлениеОтгрузкиНачалоВыбора(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = ЛОЖЬ;
	форма  = ЭтотОбъект.ПолучитьФорму("ФормаКатегорий");
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ВЫБОР
	               |		КОГДА ЗначенияСвойствОбъектов.Ссылка В (&Список)
	               |			ТОГДА ИСТИНА
	               |		ИНАЧЕ ЛОЖЬ
	               |	КОНЕЦ КАК Пометка,
		...
		Запрос.УстановитьПараметр("Список", НаправлениеОтгрузки);
	Запрос.УстановитьПараметр("Владелец", ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоКоду("90175") );
	
	Результат = Запрос.Выполнить();
	Выборка = Результат.Выбрать();
	
	НаправлениеОтгрузки.Очистить();
	пока Выборка.Следующий() цикл
		НаправлениеОтгрузки.Добавить( выборка.Ссылка, строка(выборка.Ссылка), выборка.Пометка); 
	КонецЦикла;
	форма.ОткрытьМодально();
	
	i=0;
	пока i<НаправлениеОтгрузки.Количество() цикл
		Если НЕ НаправлениеОтгрузки[i].Пометка тогда
			НаправлениеОтгрузки.Удалить(i);
		иначе i=i+1;
		КонецЕсли;
	КонецЦикла;
...
//-------------------------------------------------------------------------------------------
	...
	//Задание.СкладПолучатель = Задание.Подразделение.Склад;
	  Задание.СкладПолучатель = Задание.Подразделение.СкладСборкиИМ;//15.08.2017
...
мМассивПользователейКоторымРазрешеноСохранять.Добавить(Справочники.Пользователи.НайтиПоКоду("Румянцева Н.В.",истина)); //06.03.18 Смирнов
