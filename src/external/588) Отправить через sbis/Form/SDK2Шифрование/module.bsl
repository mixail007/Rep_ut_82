...
// Авторизуется на online.sbis.ru по логину/паролю	
	МестныйКэш = Кэш;
	Возврат Кэш.ВИ.АвторизоватьсяПоЛогинуПаролю(Кэш,Логин,Пароль,ТекстОшибки);
КонецФункции	
&НаКлиенте
Функция АвторизоватьсяПоСертификату(Кэш,Сертификат,ТекстОшибки) Экспорт
// Авторизуется на online.sbis.ru по сертификату		
	МестныйКэш = Кэш;
	Возврат Кэш.ВИ.АвторизоватьсяПоСертификату(Кэш,Сертификат,ТекстОшибки);
КонецФункции
&НаКлиенте
...
// Получает структуру документа СБИС	
	Возврат Кэш.ВИ.ПрочитатьДокумент(Кэш,ИдДок);
КонецФункции
&НаКлиенте
...
// Используется при просмотре документов из реестров продаж (1С)
	Возврат Кэш.ВИ.ПолучитьHTMLПоXML(Кэш, ТекстXML);
КонецФункции
&НаКлиенте
...
Функция сбисВыполнитьДействие(Кэш, СоставПакета, Этап, Действие, Комментарий, ПредставлениеПакета) Экспорт
	...
	Для Каждого Вложение Из СоставПакета.Вложение Цикл
		Если Вложение.Свойство("Тип") и Вложение.Свойство("ВерсияФормата") Тогда
			ГлавноеОкно = сбисПолучитьФорму("ФормаГлавноеОкно");
			ИмяФункции = Действие.Название+"_"+СтрЗаменить(Вложение.Тип, " ", "")+"_"+СтрЗаменить(СтрЗаменить(Вложение.ВерсияФормата, ".", "_"), " ", "");
			фрм = ГлавноеОкно.сбисНайтиФормуФункции(ИмяФункции,"SDK2Шифрование","",Кэш);
			Если фрм<>Ложь Тогда
				Попытка
					Выполнить("фрм."+ИмяФункции+"(Кэш, СоставПакета, Вложение, Действие);");
				Исключение
				КонецПопытки;
			КонецЕсли;
		КонецЕсли;
...
КонецФункции
&НаКлиенте
Функция Утвердить_ЭДОНакл_5_01(Кэш, СоставПакета, Вложение, Действие) Экспорт
...
КонецФункции
&НаКлиенте
Функция Утвердить_АктВР_5_01(Кэш, СоставПакета, Вложение, Действие) Экспорт
	НовоеВложение = Новый Структура;
...
Функция сбисСформироватьТитулПолучателяАктаНакладной_5_01(Кэш, СоставПакета, Вложение, НовоеВложение, Действие, ИмяXSLT, ПрефиксФайла) Экспорт
	...
	XMLДокумента = Кэш.ОбщиеФункции.сбисПолучитьXMLФайлаИзСтруктурыПоИмениXSLT(СтруктураФайла,Кэш.XSLT, ИмяXSLT);
