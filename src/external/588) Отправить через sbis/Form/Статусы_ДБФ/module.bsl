...
КонецФункции
Функция ПолучитьСоединениеСтатусыДБФ(ИниКонфигурация,КаталогНастроек) Экспорт
// получает com-соединение с ДБФ статусов при открытии обработки, чтобы при последующих операциях со статусами сразу обращаться к соединению, а не получать заново	
...
// проставляет статусы по идентификаторам пакетов при получении списка изменений	
    БД = сбисПолучитьФорму("ФормаГлавноеОкно").СоединениеСтатусыДБФ;
...
// добавляет свойства для документа 1С (при сопоставлении и загрузке документов)	
	...
	//КонецЕсли;
	БД = сбисПолучитьФорму("ФормаГлавноеОкно").СоединениеСтатусыДБФ;
	Если БД = Неопределено тогда    // при отправке по кнопке Печать в форме документа 1С
		БД = ПолучитьСоединениеСтатусыДБФ(Ини,КаталогНастроек);
	КонецЕсли;
