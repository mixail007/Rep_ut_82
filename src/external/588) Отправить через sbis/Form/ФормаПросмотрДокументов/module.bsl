...
Процедура ОтправитьНажатие(Элемент)
	...
	Для Каждого СтрокаТаблицы из СписокПакетов Цикл
		Если СтрокаТаблицы.Отмечен Тогда
			МассивПакетовСтроки.Добавить(СтрокаТаблицы.пакет[0].значение);
				КонецЕсли;
	КонецЦикла;
		...
		Если ПолныйСоставПакета.Свойство("Вложение") Тогда
			Для Каждого Элемент Из ПолныйСоставПакета.Вложение Цикл
				Если ЗначениеЗаполнено(Элемент.XMLДокумента) Тогда
					ТекстHTML = МестныйКэш.Интеграция.ПолучитьHTMLПоXML(МестныйКэш, Элемент.XMLДокумента);
				Иначе
					ТекстHTML = "";
				КонецЕсли;
...
Процедура ОтметитьВсеПриИзменении(Элемент)
	Для Каждого Пакет из СписокПакетов Цикл
		Пакет.Отмечен= Элемент.значение;
	конеццикла;
КонецПроцедуры
&НаКлиенте
