	...
	Попытка	
		...
		Для Каждого Параметр Из Контекст.ФайлДанные.мТаблДок Цикл
			...
			Для Каждого ДокОсн Из ТабЧасть Цикл
				...
				Попытка
					Если Кэш.Ини[Кэш.Текущий.ТипДок].Свойство(ИниДокумента) Тогда
						ИниДокумента = Кэш.ОбщиеФункции.РассчитатьЗначение(ИмяДокумента,Кэш.Ини[Кэш.Текущий.ТипДок],Кэш);
					КонецЕсли;
				Исключение
				КонецПопытки;
				...
				Если ДокументДанные = Неопределено Тогда
					...
					ДокументДанные = Кэш.ОбщиеФункции.ПолучитьДанныеДокумента1С(Кэш.ини[ИниДокумента], ДокОснование, Кэш.КэшЗначенийИни);
				КонецЕсли;
				...
				фрм = сбисПолучитьФорму("ФормаГлавноеОкно").сбисНайтиФормуФункции("ПрочитатьТабличнуюЧасть","Документ_"+ИмяДокумента,"Документ_Шаблон",Кэш);
				...
				Для Каждого Строка Из ВременныйКонтекст.ТаблДок.СтрТабл Цикл
					Если Контекст.ФайлДанные.Свойство("СвернутьКолонкиГруппировок") и Контекст.ФайлДанные.Свойство("СвернутьКолонкиСуммирования") Тогда
						...
						МассивКолонок = Кэш.ОбщиеФункции.РазбитьСтрокуВМассив(Контекст.ФайлДанные.СвернутьКолонкиГруппировок, ",");
						ИдСтроки = "";
						...
						Иначе
							...
							МассивКолонок = Кэш.ОбщиеФункции.РазбитьСтрокуВМассив(Контекст.ФайлДанные.СвернутьКолонкиСуммирования, ",");
							Для каждого ИмяКолонки из МассивКолонок Цикл
								ПозТочки = Найти(ИмяКолонки, ".");
								Если ПозТочки > 0 Тогда
				...
				Если Не Контекст.ФайлДанные.мСторона.Свойство("Грузоотправитель") Тогда // берем грузоотправителя и грузополучателя с первого основания
					Если ВременныйКонтекст.ДокументДанные.Свойство("мФайл")	Тогда
						Для Каждого Элемент Из ВременныйКонтекст.ДокументДанные.мФайл Цикл
							Если Элемент.Значение.Свойство("мТаблДок") Тогда
								Если Элемент.Значение.мТаблДок.Свойство("Товары") и Элемент.Значение.мТаблДок.Товары.Количество()>0 Тогда
									Если Элемент.Значение.Свойство("мСторона") и Элемент.Значение.мСторона.Свойство("Грузоотправитель") Тогда
										...
										Кэш.ОбщиеФункции.сбисСкопироватьСтруктуру(Контекст.ФайлДанные.мСторона.Грузоотправитель,Элемент.Значение.мСторона.Грузоотправитель);
									КонецЕсли;
									Если Элемент.Значение.Свойство("мСторона") и Элемент.Значение.мСторона.Свойство("Грузополучатель") Тогда
										...
										Кэш.ОбщиеФункции.сбисСкопироватьСтруктуру(Контекст.ФайлДанные.мСторона.Грузополучатель,Элемент.Значение.мСторона.Грузополучатель);
									КонецЕсли;
									Прервать;
								КонецЕсли;
							КонецЕсли;
