...
Функция ПолучитьДанныеИзДокумента1С(Кэш,Контекст) Экспорт
	Попытка	
		...
		ПолучитьТабличнуюЧастьДокумента1С_Детал(Кэш,Контекст);
		...
		Док.Файл.Документ.Вставить("Сумма",формат(Контекст.Документ.СуммаВознаграждения,"ЧЦ=17; ЧДЦ=2; ЧРД=.; ЧГ=0; ЧН=0.00"));
		...
		НазваниеВложения = ?(Док.Файл.Документ.Свойство("Название"), Док.Файл.Документ.Название, "");
		Док.Файл.Документ.Вставить("ТаблДок", Контекст.ТаблДок);
		Док.Файл.Документ.Вставить("ТаблДетал", Контекст.ТаблДетал);
		
		Контекст.СоставПакета.Вложение.Добавить(Новый Структура("СтруктураДокумента,Отправитель,Получатель,Ответственный,Подразделение,Регламент,ДокументОснование, Документ1С, Название", Док,ОтправительРоль,ПолучательРоль,ОтветственныйСтруктура,ПодразделениеСтруктура,РегламентСтруктура,ОснованияМассив, Контекст.Документ, НазваниеВложения));
...
Функция ПолучитьТабличнуюЧастьДокумента1С_Детал(Кэш,Контекст) Экспорт
...
Функция ПолучитьТабличнуюЧастьДокумента1С(Кэш,Контекст) Экспорт
	...
	//						КонецЦикла;
	//					Иначе
	//						Кэш.ОбщиеФункции.сбисСкопироватьСтруктуру(Стр,Параметр.Значение);
	//						Основание = Новый Структура();
