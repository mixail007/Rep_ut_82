...
&НаКлиенте
Функция ЗаполнитьИзФайла(СтрокаТаблДок, СоставПакета) Экспорт
// Функция заполняет ревизиты формы данными из файла (ИНН, КПП, контактная информация, банковские реквизиты)	
	...
	Если СтруктураФайла.Файл.Документ.Свойство(РольКонтрагента) Тогда
		СтруктураКонтрагента = СтруктураФайла.Файл.Документ[РольКонтрагента];
		ИниКонтрагента = СтруктураИниФайла.мСторона[РольКонтрагента];
		Если СтруктураКонтрагента.Свойство("СвЮЛ") Тогда
			Наименование = СтруктураКонтрагента.СвЮЛ.Название;
			ИНН = СтруктураКонтрагента.СвЮЛ.ИНН;
			КПП = СтруктураКонтрагента.СвЮЛ.КПП;
		Иначе
			
		КонецЕсли;
		Если СтруктураКонтрагента.Адрес.Свойство("АдрРФ") Тогда
			АдресПредставление = "";
...
// Функция возвращает банк по БИК	
	Банк = Справочники.КлассификаторБанковРФ.НайтиПоКоду(БИК);
	Возврат Банк;
КонецФункции
