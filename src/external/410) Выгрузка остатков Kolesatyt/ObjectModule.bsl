...
Функция получитьТекстЗапроса()
	текстЗапроса ="ВЫБРАТЬ
				   ...
				   |	) 
				   | И НЕ Номенклатура.Ссылка В (ВЫБРАТЬ
				   ...
				   // - тормозит до 70 сек ! 
				   //|индексировать по НоменклатураВидТовара
				   |;
//02.04.2019 --------вынесен в ВТ_Остатки ---------------------------				   
	               ...
	               |				Остатки3.Номенклатура.ВидТовара,
	...
	//02.04.2019 --------вынесен в ВТ_ОстаткиТр ---------------------------				   
			   |////////////////////////////////////////////////////////////////////////////////
				   ...
				   |			И Склад.Подразделение В (&СписокФилиалов)
				     //+++ 04.02.2019 условие или
				   |			И ( (Склад.Подразделение = &ПодразделениеМСК и Склад = &СкладМоскваИМ)
				   |				или (Склад.Подразделение <> &ПодразделениеМСК) )
	               ...
	               |								И Подразделение В (&СписокФилиалов) ) КАК РезервДляИМОстатки) КАК ТоварыНаСкладахТр
	               |			
	               |			СГРУППИРОВАТЬ ПО
	               |				ТоварыНаСкладахТр.Подразделение,
	               |				ТоварыНаСкладахТр.Номенклатура,
				   |				ТоварыНаСкладахТр.Номенклатура.ВидТовара
				   |;
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ЕСТЬNULL(ВЫБОР
	               ...
	               |	Табл1.Номенклатура.Код КАК НоменклатураКод,
	               |	Табл1.НоменклатураВидТовара КАК НоменклатураВидТовара,
	              ...
	              |			МАКСИМУМ(остаткиТр.КоличествоОстаток),
	               ...
	               |							И ЗаказПокупателя.Проверен
				   |							И ЗаказПокупателя.Подразделение В (&СписокФилиалов)
				   |							) КАК ЗаказыПокупателейОстатки
	               |				
	               |				СГРУППИРОВАТЬ ПО
				   ...
				   //+++ 1 раз внутренне соединение, а не 6 раз в каждом запросе
...
//
функция АЭ_ВыгрузитьОстаткиТоваров(флВыгружатьНаFTP=Истина) Экспорт 
...
дНачала0 = ТекущаяУниверсальнаяДатаВМиллисекундах();
	...
	Запрос.УстановитьПараметр("МинОстаток", -2001); // ???	
	...
	СписокФилиалов.ЗагрузитьЗначения( яштПоставщики.ПолучитьМассивФилиалов() ); //все действующие филиалы!
	Запрос.УстановитьПараметр("Ярославль",		 СписокФилиалов[0].Значение ); //Яр.
	Запрос.УстановитьПараметр("ПодрЕКБ",  		 СписокФилиалов[4].Значение ); //ЕКБ !
	Запрос.УстановитьПараметр("ПодразделениеМСК",СписокФилиалов[3].Значение ); //Мск !
	Запрос.УстановитьПараметр("СкладМоскваИМ", СписокФилиалов[3].Значение.СкладСборкиИМ); //  "02136" - Москва Интернет-Магазин
	...
	СписокГоловное.Добавить(СписокФилиалов[0].Значение); //Яр.
	Запрос.УстановитьПараметр("СписокГоловное",СписокГоловное);
		
	//СписокФилиалов.Удалить(4);//ЕКБ
	 СписокФилиалов.Удалить(0);//Ярославль
