...
Процедура ДеревоСделкаНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	РеализацияТоваровУслуг.Сделка В (ВЫБРАТЬ Остатки.Сделка
		...
		|	И НЕ ЗаказПокупателя.Ссылка В  (ВЫБРАТЬ Остатки.Сделка
		|									ИЗ Остатки КАК Остатки)
	...
	Запрос.УстановитьПараметр("Дата", НачалоДня(Начпериода)-14 * 86400 );
	...
	Если ВыбЗначение<> Неопределено тогда
		Если  ТипЗнч(ЭлементыФормы.Дерево.ТекущиеДанные.Родитель.Документ)=Тип("ДокументСсылка.ПлатежноеПоручениеВходящее") и Найти(ВыбЗначение.Договорконтрагента,"*")<>0 Тогда
			Предупреждение("Нельзя в платежное поручение подбирать заказ с договором со ""*""",30);
		ИначеЕсли  ТипЗнч(ЭлементыФормы.Дерево.ТекущиеДанные.Родитель.Документ)=Тип("ДокументСсылка.ПриходныйКассовыйОрдер") и Найти(ВыбЗначение.Договорконтрагента,"*")=0 Тогда
			Предупреждение("Нельзя в ПКО подбирать заказ с договором без ""*""",30);
		Иначе	
...
Процедура ИзменитьДокумент()
	...
	СтавкаНДС_2019 = СтавкаНДСнаДату(Документ.Дата ); //28.12.2018
	чНДС = ПолучитьСтавкуНДС( СтавкаНДС_2019 );
	статьяДДС =  справочники.СтатьиДвиженияДенежныхСредств.ОплатаПокупателя; // 00002 
	Для каждого стр из  ЭлементыФормы.Дерево.ТекущиеДанные.родитель.строки Цикл 
		...
		НовСтр.СуммаНДС  = Окр(Стр.СуммаПлатежа * чНДС/(100+чНДС),2);
	...
	иначе
		Сообщить("Не удалось изменить документ. Сумма документа не совпадает с суммой расшифровки!", СтатусСообщения.Внимание);
	КонецЕсли;
КонецПроцедуры	