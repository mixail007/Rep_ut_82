...
Процедура КоманднаяПанель1Сформировать(Кнопка)
	...
	Если Дата < Дата('20180101000000') Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	СУММА(ПродажиОбороты.СтоимостьОборот) КАК СуммаПродаж,
	               |	ЕСТЬNULL(Важность.Значение, 0) КАК Важность,
	               |	ЕСТЬNULL(ПодаркиКлиентамСрезПоследних.СертификатШины,0) КАК СертификатШины,
	               |	ЕСТЬNULL(ПодаркиКлиентамСрезПоследних.СертификатДиски,0) КАК СертификатДиски,
	               |	ЕСТЬNULL(ПодаркиКлиентамСрезПоследних.СертификатАксессуары,0) КАК СертификатАКС,
	               |	ЕСТЬNULL(ПодаркиКлиентамСрезПоследних.СертификатШиныVIP,0) КАК СертификатШиныVIP,
	               |	ЕСТЬNULL(ПодаркиКлиентамСрезПоследних.СертификатДискиVIP,0) КАК СертификатДискиVIP,
	               |	ЕСТЬNULL(ПодаркиКлиентамСрезПоследних.СертификатАксессуарыVIP,0) КАК СертификатАКСVIP,
				   |	ЕСТЬNULL(ПодаркиКлиентамСрезПоследних.БрендыСертификатаДиски,0) КАК БрендыСертификатаДиски,
