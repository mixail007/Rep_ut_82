...
//Оставляем тут ВСЕГДА для полного архива!
	попытка
	КопироватьФайл(путь1+дата1+ИмяФайлаUSD, путь2+"Все диски\"+дата1+ИмяФайлаUSD);
	КопироватьФайл(путь1+ИмяАрхиваUSD,      путь2+"Все диски\"+ИмяАрхиваUSD);
...
//===========================выгрузка прайсов на сайт===============================
	...
	ИмяОбщАрхива_FTP = "Price_YST.zip";
	врФТП.Записать(путь1+ИмяОбщАрхива , "public_html/data/"+ИмяОбщАрхива_FTP);
	Сообщить(строка(ТекущаяДата())+" - Файл: "+ИмяОбщАрхива_FTP+" - успешно выгружен на сайт.", СтатусСообщения.Информация);
	Сообщить("Прямая cсылка для скачивания:   http://www.yst.ru/data/"+ИмяОбщАрхива_FTP, СтатусСообщения.Информация);
	
	
	Состояние("Идет копирование файла USD на сайт...");
	ИмяАрхиваUSD_FTP="Price_USD_YST.zip";
	врФТП.Записать(путь1+ИмяАрхиваUSD , "public_html/data/"+ИмяАрхиваUSD_FTP);
	Сообщить(строка(ТекущаяДата())+" - Файл: "+ИмяАрхиваUSD_FTP+" - успешно выгружен на сайт.", СтатусСообщения.Информация);
	Сообщить("Прямая cсылка для скачивания:   http://www.yst.ru/data/"+ИмяАрхиваUSD_FTP, СтатусСообщения.Информация);
	
	Исключение
		Сообщить("Ошибка при выгрузке на FTP файлов: "+ИмяОбщАрхива+" или "+ИмяАрхиваUSD+" : "+ОписаниеОшибки(), СтатусСообщения.Информация);
	КонецПопытки;
