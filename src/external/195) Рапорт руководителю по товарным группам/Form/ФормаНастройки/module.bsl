...
Процедура ДоступностьЭлементовФормы()
	...
	ЭлементыФормы.КоманднаяПанельВремяПубликации.Доступность = Истина;
	ЭлементыФормы.ТабличноеПолеВремяПубликации  .Доступность = Истина;
...
Процедура СохранитьДанные()
	...
	мСтруктураСохраненныеНастройки.Вставить("СписокТГ",           СписокТГ);
	мСтруктураСохраненныеНастройки.Вставить("СписокМенеджеров",           СписокМ);
...
Процедура ПриОткрытии()
	...
	//ВремяПубликаций.Колонки[0].Имя="НоменклатурнаяГруппа" ;
	...
	Если мСтруктураСохраненныеНастройки.Свойство("СписокТГ") Тогда
		СписокТГ=мСтруктураСохраненныеНастройки.СписокТГ;
		Для каждого ЗначениеСписка ИЗ СписокТГ Цикл
				НоваяСтрока = ВремяПубликаций.Добавить();
				НоваяСтрока.Колонка1 = ЗначениеСписка.Значение;
		КонецЦикла;
	КонецЕсли;
	
		Если мСтруктураСохраненныеНастройки.Свойство("СписокМенеджеров") Тогда
		СписокМ=мСтруктураСохраненныеНастройки.СписокМенеджеров;
		Для каждого ЗначениеСписка ИЗ СписокМ Цикл
...
Процедура Заполнить(Кнопка)
	ВыборкаНомГруппа=Справочники.НоменклатурныеГруппы.Выбрать();
	Пока ВыборкаНомГруппа.Следующий() Цикл
		Если Найти (ВыборкаНомГруппа.Ссылка.Наименование,"я_")=0 Тогда // служебные ном. группы не включаем
				НоваяСтрока = ВремяПубликаций.Добавить();
				НоваяСтрока.Колонка1 = ВыборкаНомГруппа.Ссылка;
		КонецЕсли;		
	КонецЦикла;
