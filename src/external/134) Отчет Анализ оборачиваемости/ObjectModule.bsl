...
Процедура УстановитьТекстЗапросаПостроителяОтчета() Экспорт
	...
	|	ИЗ
	|		РегистрНакопления.ПартииТоваровНаСкладах.ОстаткиИОбороты(&ДатаНачала, &ДатаКонца, Месяц, , {(Склад).* КАК Склад, (Номенклатура).* КАК Номенклатура, (ХарактеристикаНоменклатуры).* КАК ХарактеристикаНоменклатуры}
	|  (&НеФлагОтборЗакупка ИЛИ Номенклатура В  (ВЫБРАТЬ Номенклатура  ИЗ РегистрНакопления.ТоварыНаСкладах СГРУППИРОВАТЬ ПО Номенклатура  ИМЕЮЩИЕ МИНИМУМ(Период) < &ДатаПервойЗакупки ))
	...
	|	ВЫБРАТЬ
	|		""Товары на складах"",
	|		ПартииТоваровНаСкладах.Склад Склад,
