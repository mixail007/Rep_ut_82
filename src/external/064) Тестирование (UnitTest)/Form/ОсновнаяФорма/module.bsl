...
Процедура СохранитьТест(ЗапрашиватьИмяФайла=Ложь)
	...
	Если Найти(Файл.Расширение,"gtest") > 0 Тогда
		...
		сообщить(строка(ТекущаяДата())+" Сохранен файл результата тестирования и настроек 64 обработки в текущий файл.", СтатусСообщения.Информация);
	Иначе
...
Процедура ВывестиПодвал(Шаблон, Отчет, Результат)
	...
	Результат.Время = ?(Часов>0, Формат(Часов,"ЧЦ=2; ЧДЦ=0; ЧН=00; ЧВН=") + " ч. ","") + ?(Минут>0,Формат(Минут,"ЧЦ=2; ЧДЦ=0; ЧН=00; ЧВН=") + " мин. ","") + Формат(Секунд,"ЧЦ=2; ЧДЦ=0; ЧН=00; ЧВН=") + " сек.";
...
Процедура КнопкаВыполнитьНажатие(Кнопка)
	...
	РезультатОтчет.ПоказатьУровеньГруппировокСтрок(1);//01.02.2019 - до 1 уровня
...
Процедура ТестыПриАктивизацииСтроки(Элемент)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ЭлементыФормы.ОписаниеТеста.ТолькоПросмотр = ЛОЖЬ;//Сразу можно редактировать
		ЭлементыФормы.ОписаниеТеста.УстановитьТекст(ТекущиеДанные.Описание);
		Если ТекущиеДанные.Родитель = Неопределено тогда // у "папок" - нет описания!
...
Процедура ПриОткрытии()
	...
	ЦветБезОшибок = WebЦвета.БледноЗеленый;
	...
	Если Найти(ПараметрЗапуска, ".gtest") > 0 Тогда
		...
		КонецПопытки;
	иначе
		ТекущийФайл = "\\Terminal\ОБМЕН\Огородников С\Тестирование\Тесты_ЯШТ_УТ.gtest";  //+++ 01.02.2019
		Тест = ЗначениеИзФайла(ТекущийФайл);
		ЗаполнитьИзСтруктуры(Тест);
		//ТекущийФайл = "";
		...
		Для каждого стр1 из Тесты.Строки цикл//01.02.2019 
			ЭлементыФормы.Тесты.Развернуть(стр1, Истина);
		КонецЦикла;
...
Процедура ТестыПриИзмененииФлажка(Элемент, Колонка)
	...
	Если ТекущиеДанные<>Неопределено тогда
		стрРодитель = ТекущиеДанные.Родитель;  //+++ 01.02.2019 - ВКЛючение галки у родителя!
		Если стрРодитель <> Неопределено тогда
			если ТекущиеДанные[Колонка.ДанныеФлажка] и НЕ стрРодитель[Колонка.ДанныеФлажка] тогда
				стрРодитель[Колонка.ДанныеФлажка] = Истина;
			иначеЕсли не ТекущиеДанные[Колонка.ДанныеФлажка] и стрРодитель[Колонка.ДанныеФлажка] тогда
				всеВыкл = Истина;         //+++ 01.02.2019 - ВЫКЛючение галки у родителя!
				для каждого стр1 из стрРодитель.строки цикл
					Если стр1[Колонка.ДанныеФлажка] тогда
						всеВыкл = ЛОЖЬ;
						Прервать;
					КонецЕсли;	
				КонецЦикла;	
				Если всеВыкл тогда
					стрРодитель[Колонка.ДанныеФлажка] = ЛОЖЬ;
				КонецЕсли;	
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;	 
