...
Процедура ВыполнитьПроизвольныйКод(Тест, ГлобальныеПараметры)
	...
	Параметры = ТаблицуВСтруктуру(Тест.Параметры, ГлобальныеПараметры, Новый Структура, Неопределено);
	//+++ 17.01.2019 ДобавитьОписание("Код 1С:", Тест.Код,,0); 
	РезультатТеста="";
	Попытка
		Выполнить(Тест.Код);
		ДобавитьОписание("Результат", ?(РезультатТеста="","Тест пройден успешно.",РезультатТеста),,0);//+++ 17.01.2019
	Исключение
		...
		ИначеЕсли РезультатТеста<>"" тогда//+++ 17.01.2019	
			ДобавитьОшибку("ЕСТЬ ОШИБКИ", РезультатТеста,,5);
		иначе
