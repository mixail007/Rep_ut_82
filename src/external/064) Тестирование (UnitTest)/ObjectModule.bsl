...
Функция ВыполнитьТест(Тест, ГлобальныеПараметры, ТАлгоритмы, ТФайлы, Обработчики) Экспорт
	...
	Если Тест.ФормаРедактирования = "Справочник_Запись" Тогда
	...
	ИначеЕсли Тест.ФормаРедактирования = "Форма_ПроизвольныйКод" Тогда
...
Процедура ВыполнитьПроизвольныйКод(Тест, ГлобальныеПараметры)
	...
	Попытка
		Выполнить(Тест.Код);
	Исключение
		ДобавитьЗамечание("При выполнении кода произошла ошибка.", ОписаниеОшибки(),,3);
	КонецПопытки;
КонецПроцедуры
