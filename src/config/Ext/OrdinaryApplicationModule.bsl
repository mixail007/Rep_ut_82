...
процедура СменаИнтерфейсаОгр() Экспорт
	Если ПолучитьЗначениеПоУмолчанию(глТекущийПользователь, "ОграниченныйИнтерфейс") = Истина Тогда
		...
		иначе   //03.09.18 Смирнов, для роли Формула, свой интерфейс
			ГлавныйИнтерфейс.ПереключитьИнтерфейс("СотрудникФормулы");
		КонецЕсли;
...
//	СформироватьОтчеты();
  ...
  //+++ 01.02.2012  - нужно для авто-подключения сканера ШтрихКода
	...
	Если СокрЛП(глТекущийПользователь.Код)    = "Малышев" 
		...
		или СокрЛП(глТекущийПользователь.Код) = "Смирнов А.А."
		Тогда
...
//--------------Корпоративный портал в 1С?! -----------------------------------		
	...
	Попытка 
		...
		КонецЕсли;
	Исключение
	    Сообщить("Не удалось запустить внешнюю обработку. "+ОписаниеОшибки(),СтатусСообщения.Важное);
	Конецпопытки;
   ...
   //+++ 01.02.2012  - нужно для авто-подключения сканера ШтрихКода
	...
	Если 
		глТекущийПользователь =  Справочники.Пользователи.НайтиПоКоду("Паращенко") 		
		//или глТекущийПользователь =  Справочники.Пользователи.НайтиПоКоду("Смирнов А.А.") 
		или глТекущийПользователь =  Справочники.Пользователи.НайтиПоКоду("Федунов")
		или глТекущийПользователь =  Справочники.Пользователи.НайтиПоКоду("Паращенко А.")
		или глТекущийПользователь =  Справочники.Пользователи.НайтиПоКоду("Смирнов А",истина)
		Тогда
		ТекСсылка=Справочники.ВнешниеОбработки.НайтиПоКоду("392"); 
		ИмяФайла = ПолучитьИмяВременногоФайла(); 
		ДвоичныеДанные = ТекСсылка.ХранилищеВнешнейОбработки.Получить(); 
		ДвоичныеДанные.Записать(ИмяФайла); 
	    ...
	    Форма.Открыть();
		попытка
			Форма.ДействияФормыДействие6(неопределено);
		исключение
		КонецПопытки;
...
Процедура ПроверитьПодключениеОбработчикаОжидания(ПроверятьДеньРождения = Ложь) Экспорт
	...
	Если глТекущийПользователь <> Неопределено
		 ...
		 //+++( 05.10.2018 - перс.время напоминаний +++
		 персИнтервал = ПолучитьЗначениеПоУмолчанию(глТекущийПользователь, "ИнтервалНапоминанияДоНачалаСобытия") * 60; // в минутах до 99!
		 Если персИнтервал>0 тогда
			ИнтервалПроверкиНапоминанийВСекундах = персИнтервал;
		 КонецЕсли;
         //+++)
