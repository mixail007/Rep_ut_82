...
Процедура КонтрольОстатков(ДокументОбъект, ИмяТабличнойЧасти, СтруктураШапкиДокумента, Отказ, Заголовок, флНеоперативно=ЛОЖЬ ) Экспорт
	...
	Если ЕстьЗаказПокупателяВШапке Тогда
		...
		////исключаем резерв по заказу из самой реализации - //26.04.2017
...
//14.12.2016 задача №16950 все резервы БЕЗ этого заказа! =====================================================
	...
	//23.03.2017 - только + резерв!	
	|(выбрать рез.Номенклатура, рез.Склад, рез.КоличествоОстаток из
	
	|	РегистрНакопления.РезервНаСкладах.Остатки( "+?(флНеоперативно,"&ДатаСреза","")+" ,
	|		Номенклатура В (" + ТекстЗапросаСписокНоменклатуры + ")
	//15.12.2016 - НАДО именно с отбором по 1 или нескольким складам!
	|"+?(не ЕстьСкладВТабЧасти, " И Склад = &Склад", "")+"
	...
	+ ?(ЕстьСкладВТабЧасти, "
	| И Док.Склад                       = РезервыНаСкладах.Склад ","")
	+"
//=========================================================================================================
	|
	|ЛЕВОЕ СОЕДИНЕНИЕ
	...
	|		Номенклатура В (" + ТекстЗапросаСписокНоменклатуры + ")
	|		И Склад = &Склад  ) КАК ТоварыКПередаче
	|ПО 
	...
	|		Номенклатура В (" + ТекстЗапросаСписокНоменклатуры + ")
	|		И Склад = &Склад
	|" + ?(ЕстьЗаказПокупателяВШапке, "		И ДокументПередачи = &ДокументСсылка", "") + ") КАК ТоварыКПередачеПоДокументу
	|ПО 
	|	Док.Номенклатура                = ТоварыКПередачеПоДокументу.Номенклатура"
	+ ?(ЕстьХарактеристика, "
	| И Док.ХарактеристикаНоменклатуры  = ТоварыКПередачеПоДокументу.ХарактеристикаНоменклатуры"
	,"")
	+ ?(ЕстьДокументПередачи, "
	| И &ДокументПередачи = ТоварыКПередачеПоДокументу.ДокументПередачи",
	?(ЕстьДокументРезерва,"
...
//+++ 15.11.2017 - НЕТ ордерной схемы! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	Пока Выборка.Следующий() Цикл
		...
		КоличествоВРезерве             = ?(Выборка.РезервыКоличество = NULL, 0, Выборка.РезервыКоличество);//+++ 14.12.2016 тут 
		...
		Если ЕстьСерия Тогда
			...
			Если КоличествоНаСкладе < Выборка.ДокументКоличество Тогда
				...
				" с качеством """ + СокрЛП(Выборка.Качество) + """ на складе """ + СокрЛП(Выборка.Склад) +
				""" недостаточно.";
			...
			Если КоличествоНаСкладеБезСерии - (КоличествоВРезерве - РезервыПоДокументуКоличество + КПередачеКоличество + КПолучениюКоличество - КПередачеПоДокументуКоличество) < ДокументКоличествоБезСерии Тогда
			...
			КонецЕсли;
		Иначе
			Если КоличествоНаСкладе - (КоличествоВРезерве - РезервыПоДокументуКоличество + КПередачеКоличество + КПолучениюКоличество - КПередачеПоДокументуКоличество ) < Выборка.ДокументКоличество Тогда
