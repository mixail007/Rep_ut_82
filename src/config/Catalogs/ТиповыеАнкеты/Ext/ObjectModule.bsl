...
Процедура ЗаписатьВопрос(ОбъектXML, Выборка, ФлагКолонки)
	...
	Иначе
		...
		Для каждого СтрокаТаблицы Из Ответы Цикл
			Если ЗначениеНеЗаполнено(СтрокаТаблицы.Ссылка) и Параметры.Представление <> "СписокВыбора" тогда
			...
			Если ВыборкаОтветовИзОпроса <> Неопределено тогда
				Пока ВыборкаОтветовИзОпроса.НайтиСледующий(ВопросПоиска) Цикл
					Если ВыборкаОтветовИзОпроса.Уровень() = 0 тогда
						Продолжить;
					КонецЕсли;
					Если ТипВопроса = Перечисления.ТипВопросаАнкеты.Дата ИЛИ
						ТипВопроса = Перечисления.ТипВопросаАнкеты.Число тогда
						Если ЗначениеНеЗаполнено(ВыборкаОтветовИзОпроса.ТиповойОтвет) тогда
...
Функция ВыгрузитьАнкетуВXML(Анкета, ИмяФайла = "", ОпрашиваемоеЛицо, ДокРассылки, ДокОпрос = Неопределено) Экспорт
	...
	ДобавитьПоле(XML,"ДатаЗаполненияАнкеты", 	XMLСтрока(?(ЗначениеНеЗаполнено(ДокОпрос), "", ДокОпрос.Дата)));
...
Функция СформироватьВложение(ОпрашиваемоеЛицо = Неопределено, ЭлектронныйАдресОтветов = "", ДокРассылки = Неопределено, ДокОпрос = Неопределено, ИмяФайла = "", ФормироватьXSL = Ложь) Экспорт
	...
	Если ФормироватьXSL тогда
		...
		Если ЭлектронныйАдресОтветов = "e-mail ответов" И ЗначениеНеЗаполнено(ДокОпрос) тогда
		...
		// в нее помещаем только кнопку "Сохранить в файл"
			...
			ТекстXSL.УдалитьСтроку(ТекстXSL.КоличествоСтрок());
			СтрокаПреобразования = СтрокаПреобразования + ТекстXSL.ПолучитьТекст();
		ИначеЕсли ЭлектронныйАдресОтветов <> "e-mail ответов" И ЗначениеНеЗаполнено(ДокОпрос) тогда
		...
		// в нее помещаем обе кнопки
			...
			ТекстXSL.УдалитьСтроку(ТекстXSL.КоличествоСтрок());
			СтрокаПреобразования = СтрокаПреобразования + ТекстXSL.ПолучитьТекст();
		ИначеЕсли НЕ ЗначениеНеЗаполнено(ДокОпрос) тогда
			ТекстXSL = XSLМакет.ПолучитьОбласть("ДоКнопок");
			ТекстXSL.УдалитьСтроку(1);
			ТекстXSL.УдалитьСтроку(ТекстXSL.КоличествоСтрок());
			СтрокаПреобразования = ТекстXSL.ПолучитьТекст();
...
Функция СформироватьМакет(ДокументМакет) Экспорт
	...
	//ДокументМакет.ВставитьОбласть(МакетВариантаОтвета.Область(), 	ДокументМакет.Область(), ТипСмещенияТабличногоДокумента.ПоГоризонтали, Ложь);
...
Процедура Печать() Экспорт
	...
	НапечататьДокумент(ПечатныйДокумент);
