...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ ОбменДанными.Загрузка И НЕ ЗначениеНеЗаполнено(БазовыйТипЦен) Тогда
		// Если цена рассчетная и введена на основании рассчетной- это неправильно - записывать нельзя
		Если БазовыйТипЦен.Рассчитывается Тогда
			СообщитьОбОшибке("Базовый тип цен не может быть динамическим!", Отказ);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ОбменДанными.Загрузка И Не ЭтоГруппа Тогда
		Если Рассчитывается Тогда
			// Если цена рассчетная и на основании её введена уже рассчетная - это неправильно - записывать нельзя
			Запрос = Новый Запрос;
			...
			Если ВыборкаЦен.Следующий() Тогда
				СообщитьОбОшибке("Этот тип цен уже используется как базовый, он уже не может быть динамическим!", Отказ);
				Возврат;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
