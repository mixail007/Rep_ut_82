...
Процедура ПриКопировании(ОбъектКопирования)
	...
	Если НЕ ЭтотОбъект.ЭтоГруппа Тогда
		...
		ЭтотОбъект.РабочаяГруппа = Справочники.Контрагенты.ПустаяСсылка();
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ)
	...
	//07.02.19 Смирнов
	Если ЭтоНовый() и ЭтоГруппа = ложь и ЗначениеЗаполнено(РабочаяГруппа)=ложь тогда
		ЭтотОбъект.УстановитьСсылкуНового(Справочники.Контрагенты.ПолучитьСсылку(Новый УникальныйИдентификатор));
		РабочаяГруппа = ЭтотОбъект.ПолучитьСсылкуНового();
	КонецЕсли;
КонецПроцедуры
...
Процедура ПриЗаписи(Отказ)
	...
	//07.02.19 Смирнов
	Если ЗначениеЗаполнено(РабочаяГруппа) И РабочаяГруппа <> Ссылка Тогда
		Если ЗначениеЗаполнено(РабочаяГруппа.РабочаяГруппа) И РабочаяГруппа.РабочаяГруппа <> РабочаяГруппа Тогда
			СообщитьОбОшибке("Контрагент "+СокрЛП(РабочаяГруппа)+" не может быть выбран в качестве рабочей группы, 
			|так как для него уже была назначена рабочая группа "+СокрЛП(РабочаяГруппа.РабочаяГруппа)+"!");
			Отказ = Истина;
			Возврат;
		КонецЕсли;
		...
		МенеджерЗаписи.Свойство = ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоКоду("90230");
		МенеджерЗаписи.Значение = РабочаяГруппа;
