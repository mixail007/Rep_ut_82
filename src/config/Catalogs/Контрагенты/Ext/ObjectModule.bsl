...
Процедура ПередЗаписью(Отказ)
	...
	Если не ЭтоНовый() тогда
		...
		//+++ 13.08.2018 --- задача № 52399 - разрешено для всех?!		
			//иначеЕсли 
			//	не ( РольДоступна("МенеджерПоПродажамИМ") 
			//		// 21.03.2017  по задаче № 23723 - проверки только на клиенте!
			...
			//			)
			//		)
			//	и ЭтотОбъект.Родитель <> справочники.Контрагенты.ПотенциальныеПокупатели 
			//	и Ссылка.Родитель = справочники.Контрагенты.ПотенциальныеПокупатели тогда
			//	
		...
		//+++ 13.08.2018 --- задача № 52399 - нет лимита 0р.----------------------
		
			// 21.03.2017  по задаче № 23723  - проверки только на клиенте!
			//Если ЭтотОбъект.Родитель <> справочники.Контрагенты.ПотенциальныеПокупатели 
			...
			//	   и глТекущийПользователь = глТекущийПользователь.ОсновноеПодразделение.Руководитель ) тогда
			//	КонтролироватьСуммуЗадолженности = истина;
			//	ДопустимаяСуммаЗадолженности = 0; 
			//	сообщить("Клиент '"+строка(ЭтотОбъект.Наименование)+"' перенесен в группу '"+строка(ЭтотОбъект.Родитель)+"' и установлен лимит = 0р.");
			//	
			////задача №23723
			////2.Просьба, при переносе контрагента из потенциальных в покупатели, автоматически создавать договор предоплаты и договор предоплаты* 
...
//+++ 13.08.2018 --- задача № 52399 --- можно!
	// нельзя обратно в потенциальные покупатели
	//Если не ЭтоНовый() И ссылка.Родитель <> справочники.Контрагенты.ПотенциальныеПокупатели И
	//	ЭтотОбъект.Родитель = справочники.Контрагенты.ПотенциальныеПокупатели Тогда
	//	
	//	Если РольДоступна("ПолныеПрава")  //21.03.2017  по задаче № 23723  - проверки только на клиенте!
	//		или  (ПолучитьЗначениеПоУмолчанию(глТекущийПользователь, "УчетТолькоПоПодразделениюПользователя") 
	//			 и глТекущийПользователь = глТекущийПользователь.ОсновноеПодразделение.Руководитель ) тогда  
	//	  	//+++ 10.07.2014 - иногда надо обратно 
	//		Сообщить("Не рекомендуется переносить Контрагента обратно в группу потенц.покупатели !", СтатусСообщения.Важное);
	//	Иначе
	//		Сообщить("Вам не разрешено переносить Контрагента  обратно в группу потенц.покупатели !", СтатусСообщения.Важное);
	//		Отказ = Истина;
	//	КонецЕсли;
	//	
	//КонецЕсли;	
	...
	Если НЕ ЭтотОбъект.ЭтоГруппа тогда
	Если НЕ ЭтотОбъект.Экспорт 	// для экспортных - нет проверки!
		//+++ 01.08.2017 НЕЛЬЗЯ ТАК! Всегда создаётся в Потенциальных! и проверка никогда не работает!
		//и Родитель<>Справочники.Контрагенты.НайтиПоКоду("П000001") 
		тогда
	//+++ Поиск дублей +++++++++++++++++++++++++++++++++++++++++++++++++++
	Усл = "";
	Запрос = Новый Запрос;
	...
	Если НЕ ЭтотОбъект.ЭтоГруппа тогда //+++ 09.02.2016
		Если Усл = "" тогда
			#Если Клиент тогда
