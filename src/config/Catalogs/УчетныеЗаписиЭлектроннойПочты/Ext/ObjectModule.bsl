...
Процедура ПроверитьОтветственныхЗаТранспортЭлектронныхПисем(Отказ, СтрокаОшибки)
	...
	Для каждого СтрокаТЧ Из ДоступКУчетнойЗаписи Цикл
		Если СтрокаТЧ.Транспорт И НЕ СтрокаТЧ.Отправка Тогда
			СообщитьОбОшибке(("Пользователю " + СтрокаТЧ.Пользователь + " предоставлено право осуществлять транспорт писем, но нет права их редактировать. Данное сочетание недопустимо."),, СтрокаОшибки);
			Отказ = Истина;
		КонецЕсли; 
	КонецЦикла;
...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ ОбменДанными.Загрузка Тогда
		...
		Если НЕ Константы.ИспользованиеВстроенногоПочтовогоКлиента.Получить() Тогда
			СообщитьОбОшибке("В настройках учета не установлено использование встроенного почтового клиента.",, СтрокаОшибки);
			Отказ = Истина;
			Возврат;
		КонецЕсли;
		
		Если ЗначениеНеЗаполнено(Наименование) Тогда
			СообщитьОбОшибке("Не указано представление учетной записи.",, СтрокаОшибки);
			Отказ = Истина;
			Возврат;
		КонецЕсли;
		
		Если НЕ ЗначениеНеЗаполнено(ГруппаУдаленные) И ЕстьСсылки(ГруппаУдаленные) Тогда
			СообщитьОбОшибке(("В группе """ + ГруппаУдаленные + """ есть электронные письма. Данную группу нельзя устанавливать в качестве группы удаленных писем."),, СтрокаОшибки);
			Отказ = Истина;
			Возврат;
		КонецЕсли; 
		
		Если НЕ ЗначениеНеЗаполнено(ГруппаЧерновики) И ЕстьСсылки(ГруппаЧерновики) Тогда
			СообщитьОбОшибке(("В группе """ + ГруппаЧерновики + """ есть электронные письма. Данную группу нельзя устанавливать в качестве группы черновиков."),, СтрокаОшибки);
			Отказ = Истина;
			Возврат;
		КонецЕсли;
		...
		Если НЕ ЗначениеНеЗаполнено(ГруппаВходящие) Тогда
			НоваяСтрока = ТаблицаПроверкиГрупп.Добавить();
			НоваяСтрока.Группа     = ГруппаВходящие;
			НоваяСтрока.Количество = 1;
		КонецЕсли; 
		Если НЕ ЗначениеНеЗаполнено(ГруппаИсходящие) Тогда
			НоваяСтрока = ТаблицаПроверкиГрупп.Добавить();
			НоваяСтрока.Группа     = ГруппаИсходящие;
			НоваяСтрока.Количество = 1;
		КонецЕсли; 
		Если НЕ ЗначениеНеЗаполнено(ГруппаУдаленные) Тогда
			НоваяСтрока = ТаблицаПроверкиГрупп.Добавить();
			НоваяСтрока.Группа     = ГруппаУдаленные;
			НоваяСтрока.Количество = 1;
		КонецЕсли; 
		Если НЕ ЗначениеНеЗаполнено(ГруппаЧерновики) Тогда
			НоваяСтрока = ТаблицаПроверкиГрупп.Добавить();
		...
		Если ТаблицаПроверкиГрупп.Количество() > 0 Тогда
			...
			Для каждого СтрокаТаблицы Из ТаблицаПроверкиГрупп Цикл
				Если СтрокаТаблицы.Количество > 1 Тогда
					СообщитьОбОшибке("У предопределенных групп Входящие, Исходящие, Удаленные и Черновики не должно быть повторяющихся значений.",, СтрокаОшибки);
					Отказ = Истина;
					Возврат;
				КонецЕсли; 
			КонецЦикла; 
		...
		Если АвтоПолучениеОтправкаСообщений Тогда
			Если ЗначениеНеЗаполнено(ОтветственныйЗаАвтоПолучениеОтправкуСообщений) Тогда
				СообщитьОбОшибке("Не указан ответственный за автополучение/отправку писем.",, СтрокаОшибки);
				Отказ = Истина;
			ИначеЕсли ЗначениеНеЗаполнено(ИнтервалАвтоПолученияОтправкиСообщений) Тогда
				СообщитьОбОшибке("Не указан интервал автополучения/отправки писем.",, СтрокаОшибки);
				Отказ = Истина;
			Иначе
				...
				КонецЕсли; 
				Если Отказ Тогда
					СообщитьОбОшибке("У ответственного за автополучение/отправку нет прав на транспорт писем.",, СтрокаОшибки);
				КонецЕсли; 
			КонецЕсли; 
		КонецЕсли; 
		...
		Если НЕ ПометкаУдаления Тогда
			...
			Если НЕ ЗначениеНеЗаполнено(Проверка) Тогда
				СообщитьОбОшибке(Проверка,, СтрокаОшибки);
				Отказ = Истина;
			КонецЕсли; 
		КонецЕсли; 
