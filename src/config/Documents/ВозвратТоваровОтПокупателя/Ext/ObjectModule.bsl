...
Процедура ЗаполнитьТоварыПоОрдеруУпр(ДокументОснование, ТабличнаяЧасть) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТабличнаяЧасть = Товары Тогда
			...
			СтрокаТабличнойЧасти.СтавкаНДС                  = СтавкаНДСнаДату( ДокументОснование.Дата );//+++ 28.12.2018 //СтрокаТабличнойЧасти.Номенклатура.СтавкаНДС;
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоТаре, Отказ, Заголовок);
	...
	//+++( 27.08.2014 - Здесь контроля не надо!?
		...
		Если СтруктураШапкиДокумента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СПокупателем Тогда
			...
			ТаблицаПродаж.Колонки.СтоимостьПродажи.Имя = "Стоимость";
				...
				Для каждого стр из ТаблицаПоТоварамБ Цикл
					Если ТипЗнч(стр.ДокументПродажи)<> Тип("ДокументСсылка.ОтчетОРозничныхПродажах") тогда //нет заказа
						Если ЗначениеЗаполнено(стр.ДокументПродажи.сделка) тогда
		...
		Если СтруктураШапкиДокумента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомиссионером или СтруктураШапкиДокумента.ДоговорКонтрагента.ТипДоговора = Справочники.ТипыДоговоров.ПеремещениеОТХ Тогда
            //01.11.18 Смирнов сторно резерва Товары переданные
			СторноРезерваПоЗаявкамНаБрак(СтруктураШапкиДокумента);
...
Процедура СторноРезерваПоЗаявкамНаБрак(СтруктураШапкиДокумента)
	Если СтруктураШапкиДокумента.ДоговорКонтрагента.ТипДоговора = Справочники.ТипыДоговоров.ПеремещениеОТХ и значениеЗаполнено(ЗаявкаОснование) тогда
		ТабДок = Товары.Выгрузить(,"Номенклатура,Количество");
		...
		Запрос.Текст="ВЫБРАТЬ
		             ...
		             |////////////////////////////////////////////////////////////////////////////////
		             |ВЫБРАТЬ
		             |	ТоварыПереданныеОбороты.Сделка КАК Сделка,
		             |	ТоварыПереданныеОбороты.Номенклатура,
		             |	СУММА(ТоварыПереданныеОбороты.КоличествоРасход) КАК Количество,
		             |	СУММА(ТоварыПереданныеОбороты.СуммаВзаиморасчетовРасход) КАК СуммаВзаиморасчетов,
		             |	ТоварыПереданныеОбороты.СтатусПередачи,
		             ...
		             |						вт.Номенклатура
		             |					ИЗ
		             |						вт)) КАК ТоварыПереданныеОбороты
		             |ГДЕ
		             |	ТоварыПереданныеОбороты.Регистратор = &ЗаявкаНаВозвратТоваров
		             |
		             |СГРУППИРОВАТЬ ПО
		             ...
		             |	вт2.Сделка,
		             |	вт2.Номенклатура,
		             |	вт2.СуммаВзаиморасчетов / вт2.Количество * вт.Количество КАК СуммаВзаиморасчетов,
		             |	вт2.СтатусПередачи,
		...
		Запрос.УстановитьПараметр("ЗаявкаНаВозвратТоваров", ЗаявкаОснование);
		Запрос.УстановитьПараметр("НачПериода", ЗаявкаОснование.Дата-1);
		Запрос.УстановитьПараметр("КонПериода", ЗаявкаОснование.Дата+1);
...
Процедура ОбработкаЗаполнения(Основание)
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваров") Тогда 
		...
		для каждого стр1 из  Основание.Товары цикл //29.12.2015
			...
			стр2.СтавкаНДС 		= СтавкаНДСнаДату(Основание.Дата); //+++ 28.12.2018 -> ПО ОСНОВАНИЮ!  перечисления.СтавкиНДС.НДС18;
			РассчитатьСуммуНДСТабЧасти(стр2, ЭтотОбъект); // с учетом / без НДС
		КонецЦикла;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если НЕ Отказ Тогда
		...
		//+++ 08.04.2019 --- Задача № 65874 --- автозаполнение грузчиков
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//***2019.01.24 проверка превышения количества в заявке на возврат
	#Если Клиент тогда
		Если ЗначениеЗаполнено(ЗаявкаОснование) Тогда
			//Отказ = ПроверитьПревышениеКоличестваТовараПоЗаявкеНаВозврат();
		КонецЕсли;
	#КонецЕсли
	
	// Движения по документу
	ЭтотОбъектСделка = ЭтотОбъект.Сделка; // 13.11.2018 - 1 а не 6 раз
	Если Не Отказ Тогда
		...
		Если ЗначениеЗаполнено(ЭтотОбъектСделка) и ТипЗнч(ЭтотОбъектСделка)=Тип("ДокументСсылка.ЗаказПокупателя") тогда  //+++ 20.05.2013
			ОбменСУТИнтернетМагазин.ВключитьРегистрациюОбъектаОИМ( ЭтотОбъектСделка ); // 01.11.2016     13.11.2018 - не надо в МиМ! , Контрагент.ПриниматьЗаказыЧерезСайт
			//+++( 14.02.2019 ---- на дату отгрузки ---
				ДатаСреза = ЭтотОбъектСделка.ДатаОтгрузки; 
	...
	//Миронычев
		...
		|ГДЕ
		|	СостояниеЗаказовИМ.Номенклатура В(&Номенклатура)
		...
		|					ИЛИ &ЗаказПервичныйТип = 10
		|				ТОГДА СостояниеЗаказовИМ.ЗаказПервичный = &ЗаказПервичный
		|			ИНАЧЕ СостояниеЗаказовИМ.ЗаказОбщий = &ЗаказПервичный
		|		КОНЕЦ";
		
		Запрос.УстановитьПараметр("Terminal", ЭтотОбъектСделка.TerminalОтгрузкаТранспортнойКомпанией);
    	Запрос.УстановитьПараметр("ЗаказПервичныйТип", ЭтотОбъектСделка.ТипЗаказа);
		Запрос.УстановитьПараметр("ЗаказПервичный", ЭтотОбъектСделка);
...
//----------" - " сумма бонуса по документу партии / Сделка(Реализация) -----------------
...
процедура ИзменитьРегСведДанныеПоДокументам() 
	...
	ЭтотОбъектСделка = ЭтотОбъект.Сделка; //13.11.2018 - 1 а не 6 раз
	Если НЕ ЗначениеЗаполнено(ЭтотОбъектСделка) тогда
		возврат;
	КонецЕсли;
	
	Если ТипЗнч(ЭтотОбъектСделка)<>Тип("ДокументСсылка.ЗаказПокупателя") тогда
		возврат;
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("СсылкаЭтотДок", Ссылка);
	Запрос.УстановитьПараметр("Сделка", ЭтотОбъектСделка);
	Запрос.УстановитьПараметр("Дата", НачалоДня(ЭтотОбъектСделка.Дата) );  // реализация не раньше Заказа покупателя!
	Запрос.УстановитьПараметр("Дата1", КонецДня(Макс(ЭтотОбъект.Дата,ЭтотОбъектСделка.Дата))  ); // 13.11.2018 реализация не позже Возврата от покупателя!?
	
	Результат = Запрос.Выполнить();
	Если Результат.Пустой() тогда
		сообщить("Не найдено ни одного проведенного документа ""Реализация товаров и услуг"" по сделке: "+строка(ЭтотОбъектСделка));
	КонецЕсли;	
...
Функция ПроверитьПревышениеКоличестваТовараПоЗаявкеНаВозврат()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	СУММА(ЕСТЬNULL(ЗаявкаНаВозвратТоваровТовары.Количество, 0)) КАК КоличествоВЗаявке,
	               |	ВозвратТоваровОтПокупателяТовары.Ссылка.ЗаявкаОснование
	               |ПОМЕСТИТЬ ВТ_ТекущийВозврат
	               |ИЗ
	               |	Документ.ВозвратТоваровОтПокупателя.Товары КАК ВозвратТоваровОтПокупателяТовары
	               ...
	               |			ДокументПродажи В
	               |				(ВЫБРАТЬ
	               |					ВТ_ТекущийВозврат.ДокументПартии
	               |				ИЗ
	               |					ВТ_ТекущийВозврат)) КАК ПродажиОбороты
	               |ГДЕ
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ВТ_ТекущийВозврат.Номенклатура,
	               |	СУММА(ВТ_ТекущийВозврат.Количество) КАК Количество,
	               |	СУММА(ВТ_ТекущийВозврат.КоличествоВЗаявке) КАК КоличествоВЗаявке,
	               |	СУММА(ЕСТЬNULL(ВТ_Возвраты.КоличествоОборот, 0) * -1) КАК КоличествоВВозвратах
	               |ИЗ
	               |	ВТ_ТекущийВозврат КАК ВТ_ТекущийВозврат
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Возвраты КАК ВТ_Возвраты
	               |		ПО ВТ_ТекущийВозврат.Номенклатура = ВТ_Возвраты.Номенклатура
	               |			И ВТ_ТекущийВозврат.ДокументПартии = ВТ_Возвраты.ДокументПродажи
	               |			И ВТ_ТекущийВозврат.ЗаявкаОснование = ВТ_Возвраты.РегистраторЗаявкаОснование
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ВТ_ТекущийВозврат.Номенклатура";
	...
	Пока Результат.Следующий() Цикл
		Если (Результат.КоличествоВЗаявке - Результат.КоличествоВВозвратах) < Результат.Количество Тогда
			Отказ = Истина;
			Сообщить("Превышено количество в рамках одной заявки на возврат! По номенклатуре " + Строка(Результат.Номенклатура) + " уже проведены возвраты по заявке " + Строка(ЗаявкаОснование) + " на общее количество " + Строка(Результат.КоличествоВВозвратах) + "шт.");
		КонецЕсли;
	КонецЦикла;
