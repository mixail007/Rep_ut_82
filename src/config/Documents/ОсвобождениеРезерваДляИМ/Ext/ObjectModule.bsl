...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если Не Отказ Тогда //29.05.2018 отказ - значит всем отказ!
...
Процедура ДвижениеПоРегистрам(РежимПроведения,Отказ)
	...
	Если НЕ Отказ тогда
		ОтказЗак    = Отказ;
		ОтказРезерв = Отказ;
		ДвижениеПоЗаказамПокупателей(РежимПроведения,ОтказЗак);
		ДвижениеПоРезервДляИМ(РежимПроведения,ОтказРезерв);
		Отказ = ОтказРезерв или ОтказЗак;  //29.05.2018
	КонецЕсли;
