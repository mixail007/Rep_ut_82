	...
	Если РежимЗаписи  <> РежимЗаписиДокумента.ОтменаПроведения Тогда
		...
		//конецЕсли;	
			Если ЗначениеЗаполнено(стр.Сделка) и ТипЗнч(стр.Сделка)=Тип("ДокументСсылка.ЗаказПоставщику") и стр.ВидЗадолженности = перечисления.ВидыЗадолженности.Кредиторская и стр.ДоговорКонтрагента.ВалютаВзаиморасчетов <> константы.ВалютаРегламентированногоУчета.Получить() Тогда
				...
				Если стр.Сумма > МаксПредоплата.МаксОстаток Тогда
					#Если Клиент тогда
						...
						Если Ответ = КодВозвратаДиалога.Да Тогда
							стр.Сумма =МаксПредоплата.МаксОстаток;
	...
	#Если клиент тогда
		...
		Если НЕ РольДоступна("МенеджерПоВзаимозачетам") И Константы.ОплатыРаспределяютМенеджеры.Получить() 
			...
			Для каждого строкаДолга Из СуммыДолга Цикл
				...
				ИначеЕсли не (ПолучитьЗначениеПоУмолчанию(глТекущийПользователь, "РазрешитьВзаимозачетПоРазнымДоговорамПродажи") и ПользователиГруппы.Найти(глТекущийПользователь)<> неопределено ) 
					...
					Если ПолучитьЗначениеПоУмолчанию(глТекущийПользователь, "ЖурналыОтборПоРегиону")  Тогда
						...
						ЕСли СЗКонтрагенты.НайтиПоЗначению(строкаДолга.ДоговорКонтрагента.Владелец) = Неопределено тогда //не нашли в разрешенных
							Сообщить("В строке №"+Строка(строкаДолга.НомерСтроки)+" указан договор контрагента, по которому вы НЕ являетесь лицом ответственным за контрагента.");
							Отказ=Истина;
						КонецЕсли;
			...
			Если СписокДоговоровВДокументе.Количество()>1 Тогда // зачеты между несколькими договорами не допускаются
				Если  ЕстьПоставка = истина и ЕстьПокупка = истина тогда 
					Сообщить("Вы не имеете права производить зачеты между несколькими договорами с разными типами. Обратитесь за разъяснениями в финансовую службу.",СтатусСообщения.Важное);
					Отказ=Истина;
				//+++( 04.02.2014 только для Ани Серебренниковой - можно вести зачет по разным договорам 1 контрагента
				ИначеЕсли ПолучитьЗначениеПоУмолчанию(глТекущийПользователь, "РазрешитьВзаимозачетПоРазнымДоговорам") 
