...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
    ...
    //11.03.19 Смирнов удалим заказы
	СписокЗаказов = ТаблицаДвиженийПоЗаказам.ВыгрузитьКолонку("ЗаказПокупателяСезонный");

	Запрос = новый Запрос;
	Запрос.Текст="ВЫБРАТЬ
	             |	ЗаказПокупателяСезонный.Ссылка
	             |ПОМЕСТИТЬ втЗаказПокупателяСезонные
	             |ИЗ
	             |	Документ.ЗаказПокупателяСезонный КАК ЗаказПокупателяСезонный
	             |ГДЕ
	             |	ЗаказПокупателяСезонный.БудетПредоплата = истина
	             |	И ЗаказПокупателяСезонный.Ссылка В(&СписокЗаказов)
	             |;
	             |
	             |////////////////////////////////////////////////////////////////////////////////
	             |ВЫБРАТЬ
	             |	ЗаказПокупателя.Ссылка КАК ЗаказПокупателяДляПредоплаты
	             |ИЗ
	             |	Документ.ЗаказПокупателя КАК ЗаказПокупателя
	             |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втЗаказПокупателяСезонные КАК втЗаказПокупателяСезонные
	             |		ПО ЗаказПокупателя.ДокументОснование = втЗаказПокупателяСезонные.Ссылка
	             |ГДЕ
	             |	ЗаказПокупателя.Номер ПОДОБНО ""СЗ%""
				 ...
				 Пока Рез.Следующий() Цикл
					 ЗаказОб = Рез.ЗаказПокупателяДляПредоплаты.ПолучитьОбъект();
					 ЗаказОб.УстановитьПометкуУдаления(истина);
					 ЗаказОб.Комментарий = ЗаказОб.Комментарий+ "; удален, т.к. документ основния был закрыт";
					 Попытка
						 ЗаказОб.Записать();
					 Исключение
					 КонецПопытки;
