...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, 
	...
	//01.04.2019 ДвиженияПоРасчетамСКонтрагентами( СтруктураШапкиДокумента, Отказ);
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//Для кованных дисков проверим обязательоне наличие заготовки
	...
	Запрос.Текст = 
		...
		|	&Таб КАК Таб
		//|ГДЕ
		//|	Таб.Номенклатура.НоменклатурнаяГруппа = &НоменклатурнаяГруппа
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ВТ.Номенклатура,
		|	Заготовки.Комплектующая КАК Комплектующая
		|ИЗ
		|	ВТ КАК ВТ
		|		ЛЕВОЕ СОЕДИНЕНИЕ Заготовки КАК Заготовки
		|		ПО ВТ.Номенклатура = Заготовки.Номенклатура
		|ГДЕ
		|	Заготовки.Комплектующая ЕСТЬ NULL
		|	и Вт.Номенклатура.НоменклатурнаяГруппа = &НоменклатурнаяГруппа";
	...
	Пока Выборка.Следующий() Цикл
		Отказ = Истина;
		Сообщить("Для номенклатуры "+Выборка.Номенклатура.код+" "+Выборка.Номенклатура+" не задана заготовка! ");
	КонецЦикла;
		...
		Для Каждого ТекСтрока Из Заготовки Цикл
					Движение = Движения.РезервЗаготовокПодЗаказы.Добавить();
					Движение.Диск = ТекСтрока.Диск;
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	Если не Результат.Пустой() Тогда  //отличается количество по регистру и по таб. части документа хоть по 1 позиции
		...
		#конецесли
		Отказ=Истина;
	иначеЕсли БудетПредоплата тогда
		ЗаказДляПредоплаты = ПолучитьЗаказПокупателяДляПредоплаты(Ссылка);
		Если ЗначениеЗаполнено(ЗаказДляПредоплаты) тогда
			ЗаказДляПредоплатыОб = ЗаказДляПредоплаты.ПолучитьОбъект();
			ЗаказДляПредоплатыОб.УстановитьПометкуУдаления(истина);
			ЗаказДляПредоплатыОб.Комментарий = ЗаказДляПредоплатыОб.Комментарий+"; удален, т.к. документ основния был удален";
			Попытка
				ЗаказДляПредоплатыОб.Записать();
			Исключение
			КонецПопытки;
		КонецЕсли;
	конецЕсли;
...
Функция ПолучитьЗаказПокупателяДляПредоплаты(Сс) экспорт
  Запрос = новый Запрос;
  Запрос.Текст="ВЫБРАТЬ
               ...
               |	И ЗаказПокупателя.Номер ПОДОБНО &ПрефиксСЗ
               |	И ЗаказПокупателя.ПометкаУдаления = ЛОЖЬ";
			   Запрос.УстановитьПараметр("ДокументОснование",Сс);
			   Запрос.УстановитьПараметр("ПрефиксСЗ","СЗ%");
			   Рез = Запрос.Выполнить().Выбрать();
			   Пока Рез.Следующий() Цикл
				   Возврат Рез.ЗаказПокупателя;
			   КонецЦикла;
