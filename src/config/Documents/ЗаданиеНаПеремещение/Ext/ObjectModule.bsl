	...
	Пока выборкаЗаказ.Следующий() Цикл
		...
		колЗак = 0; // чтобы не дублилось для разных складов и адресов...
			...
			Если выборкаТовар.КоличествоЗаказано=выборкаТовар.Количество или (колЗак=0) тогда
				...
				Если ЗначениеЗаполнено(выборкаТовар.Номенклатура) И выборкаТовар.Номенклатура.ВидТовара = перечисления.ВидыТоваров.Аксессуары Тогда
		...
		//------------обнуление, так как это продолжение отгрузки с другого склада---------------
			...
			Иначе	
				...
				//+++( 18.07.2017 - для Аксессуаров нужен Артикул!
				Если выборкаТовар.Номенклатура.ВидТовара = перечисления.ВидыТоваров.Аксессуары 	Тогда
...
//+++ 05.08.2018 --- для 8000 и Ангаров	-- 2 раза проходим 
	флРазделятьПоВидамТоваров = ложь; флРазделятьДиски=ложь; колШин=0; колДиски=0;
	...
	и (ВыборкаГруппаСкладов.ГруппаСкладов.Код = "08"
		...
		Если ВыборкаГруппаСкладов.ГруппаСкладов.Код <> "12"	 
			и ( СокрЛП(Ассортимент)= ""   //Ангар-И
			 или ( СокрЛП(Ассортимент)<>"" и Найти(Ассортимент, "Шины")>0 и Найти(Ассортимент, ",")>0 )
			  ) тогда   // есть шины и не только они
	     флРазделятьПоВидамТоваров = Истина;
	 	 флРазделятьДиски = ЛОЖЬ; // сначала только "Шины",а потов всё остальное!
	  КонецЕсли;
...
~111:	Если  СокрЛП(МаркаАвтомобиля)<>"" Тогда
	...
	ОбластьМакета.Параметры.ТекстЗаголовка = СформироватьЗаголовокДокумента(Ссылка, "Задание на перемещение");
	#КонецЕсли
	ОбластьМакета.Параметры.СловоВажный = ?(ЭтотОбъект.Важность = перечисления.Важность.Высокая, "!!!",	"")
	...
	Пока ВыборкаНоменклатура.Следующий() Цикл   //======================================================
		...
		ОбластьНоменклатура.Параметры.Количество = ?(ВыборкаНоменклатура.КоличествоЗаказано=колТов, "", 
														?(ВыборкаНоменклатура.КоличествоЗаказано<колТов,"", //15.08.2018 - убран знак <
														     ?(колТов>0,"!!! ","") ) )
												   + формат(колТов,"ЧН=' '"); 
	...
	Если НЕ этотОбъект.ЗаказыПокупателей[0].ЗаказПокупателя.ТранспортнаяКомпания.Пустая() тогда
		ОбластьПримечение.Параметры.комментарий = СокрЛП( ЭтотОбъект.Комментарий );
	Иначе
		ОбластьПримечение.Параметры.комментарий = ?(флРазделятьПоВидамТоваров, ?(флРазделятьДиски,"Только Диски!","Только Шины!"),"") //+++ 05.08.2018 Задача 52016 Фролов
		+ ""; //+++ 25.10.2017 ФРОЛОВ
	КонецЕсли;
	...
	Если флРазделятьПоВидамТоваров тогда //+++ 05.08.2018
		Если НЕ флРазделятьДиски и колДиски>0 тогда
			флРазделятьДиски = Истина;
