...
//+++ 05.08.2018 --- для 8000 и Ангаров	-- 2 раза проходим 
	флРазделятьПоВидамТоваров = ложь; ФлРазделятьШины=ложь; колШин=0; 	
	...
	и (ВыборкаГруппаСкладов.ГруппаСкладов.Код = "08"
		...
		Если ВыборкаГруппаСкладов.ГруппаСкладов.Код <> "12"	 
	     ...
	     флРазделятьПоВидамТоваров = Истина;
	 	 ФлРазделятьШины = Истина; // сначала только Шин",потов всё остальное!
	  КонецЕсли;
...
~111:	Если  СокрЛП(МаркаАвтомобиля)<>"" Тогда
	...
	ОбластьМакета.Параметры.СловоВажный = ?(ЭтотОбъект.Важность = перечисления.Важность.Высокая, "!!!",	"")
				+?(флРазделятьПоВидамТоваров, ?(ФлРазделятьШины,"д","ш"),""); //+++ 05.08.2018
	...
	Если НЕ этотОбъект.ЗаказыПокупателей[0].ЗаказПокупателя.ТранспортнаяКомпания.Пустая() тогда
		ОбластьПримечение.Параметры.комментарий = СокрЛП( ЭтотОбъект.Комментарий );
	Иначе
		ОбластьПримечение.Параметры.комментарий = ?(флРазделятьПоВидамТоваров, ?(флРазделятьШины,"Только Диски!","Только Шины!"),"") //+++ 05.08.2018 Задача 52016 Фролов
		+ ""; //+++ 25.10.2017 ФРОЛОВ
	КонецЕсли;
	...
	Если флРазделятьПоВидамТоваров тогда //+++ 05.08.2018
		Если флРазделятьШины и колШин>0 тогда
			флРазделятьШины = ЛОЖЬ;
