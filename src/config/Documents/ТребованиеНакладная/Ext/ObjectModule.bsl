...
Процедура ДополнитьСуммуПлановойСебестоимости(Табл)
	Запрос = Новый Запрос;
	Запрос.Текст = "Выбрать табл.Номенклатура, табл.Количество 
	|Поместить ВТ_Товары
	|из &Табл как табл;
	|//////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	вт.Номенклатура,
	|	вт.Количество,
	|	вт.Количество * ЕстьNull(ЦеныПлан.Цена,0) как Сумма

	|ИЗ вт_Товары как вт
	|	левое соединение РегистрСведений.ЦеныНоменклатуры.СрезПоследних(&ДатаДок,
	|			Номенклатура В (ВЫБРАТЬ тч.Номенклатура
	|					ИЗ вт_Товары КАК тч)
	|				И ТипЦен = &ТипЦенПлан				) КАК ЦеныПлан
