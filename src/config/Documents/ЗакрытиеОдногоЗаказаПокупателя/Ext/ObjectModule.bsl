	...
	Если не РольДоступна("ПолныеПрава") тогда
		...
		ИначеЕсли РежимЗаписи=РежимЗаписиДокумента.Проведение
			и не (ПараметрыСеанса.ТекущийПользователь=ЗаказПокупателя.ДоговорКонтрагента.ОтветственноеЛицо
				...
				ЗакрепленныеОператорПомощник = ПолучитьЗакрпленныхЗаРегиономОператораПомощника(ЗаказПокупателя.Контрагент.Регион, ЗаказПокупателя.Контрагент);
				Если ЗакрепленныеОператорПомощник.Помощник <> ПараметрыСеанса.ТекущийПользователь тогда 				
...
//+++( 18.01.2016 ===========РЕЗЕРВ ИМ===================================================================================================
	Если ЗаказПокупателя.ДоговорКонтрагента.КонтрагентДляРезерваИМ = справочники.Контрагенты.НайтиПоКоду("П004703") тогда //Резерв для ИМ
		ТаблицаПоРезервамИМ = ПодготовитьТаблицуПоРезервамИМ(); 	 //по-старому!
	иначе //07.11.2018 -------------------новая схема через рс.РезервРазрешения--------------------
		ТаблицаПоРезервамИМ = ПодготовитьТаблицуПоРезервамИМ_new(); 	 
	КонецЕсли;
...
//07.11.2018 - как в Закрытии заказов, только на 1 заказ
функция ПодготовитьТаблицуПоРезервамИМ_new()
	 
	Запрос = Новый Запрос;
	Запрос.Текст ="ВЫБРАТЬ
	              |	ЗаказыПокупателейОстатки.ЗаказПокупателя.Подразделение КАК Подразделение,
	              |	МАКСИМУМ(РезервРазрешенияСрезПоследних.КонтрагентРезерв) КАК КонтрагентДляРезерваИМ,
	              |	ЗаказыПокупателейОстатки.Номенклатура,
	              |	СУММА(ЗаказыПокупателейОстатки.КоличествоОстаток) КАК Количество,
	              |	ЗаказыПокупателейОстатки.ЗаказПокупателя
	              |ПОМЕСТИТЬ ВТ_ОстаткиТоваровПоЗаказам
	              |ИЗ
	              |	РегистрНакопления.ЗаказыПокупателей.Остатки(&МоментДокумента, ЗаказПокупателя = &Зак1) КАК ЗаказыПокупателейОстатки
	              |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РезервРазрешения.СрезПоследних(&МоментЗаказа, Контрагент = &Контрагент) КАК РезервРазрешенияСрезПоследних
	              |		ПО ЗаказыПокупателейОстатки.ЗаказПокупателя.Подразделение = РезервРазрешенияСрезПоследних.Подразделение
	...
	Запрос.УстановитьПараметр("МоментЗаказа", новый Граница(ЗаказПокупателя.Дата, видГраницы.Включая)  ); // ДатаЗаказа - для определения разрешений !
	
	Запрос.УстановитьПараметр("МоментДокумента", Ссылка.Дата  ); // дата Этого документа!
	...
	Запрос.УстановитьПараметр("Зак1", ЗаказПокупателя);// 1 заказ
	Запрос.УстановитьПараметр("Контрагент",ЗаказПокупателя.Контрагент); //07.11.2018 - 1 контрагент!
