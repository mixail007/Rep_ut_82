...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если видЗаказа = 0 Тогда
		...
		// Отказ = проверитьВесМаксНагрузка(); //+++ 21.06.2016 - убрана проверка по просьбе Новиковой (по согласованию с Егором)
		 ...
		 //для белорусских заказов обязательно должен быть указан порт поступления
		 Если не ЗначениеЗаполнено(ПортПоступления) и контрагент.код = "95728" и дата>Дата('20181201') тогда
		  Сообщить("Не указан порт поступления!");	 
		  Отказ = истина;
	     конецЕсли;
		 
		 //проверим доп.расходы
		 если контрагент.код = "95728" и ПлановыеДопРасходы.количество()>0 тогда
			 ...
			 |ВЫБРАТЬ
			 |	ВТ.Номенклатура,
			 |	КонтрольДопРасходовСрезПоследних.Значение КАК Значение
			 |ИЗ
			 |	ВТ КАК ВТ
			 |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КонтрольДопРасходов.СрезПоследних(&Период, ) КАК КонтрольДопРасходовСрезПоследних
			 |		ПО ВТ.Номенклатура = КонтрольДопРасходовСрезПоследних.ДопРасход
			 |ГДЕ
			 |	ВЫБОР
			 |			КОГДА КонтрольДопРасходовСрезПоследних.Значение ЕСТЬ NULL 
			 |				ТОГДА ВТ.СуммаФакт
			 |			ИНАЧЕ КонтрольДопРасходовСрезПоследних.Значение
			 |		КОНЕЦ - ВТ.СуммаФакт < 0";
			 
			 Запрос.УстановитьПараметр("Период", Дата);
			 Запрос.УстановитьПараметр("ВТ", ПлановыеДопРасходы);
			 Результат = Запрос.Выполнить();
			 
			 Выборка = Результат.Выбрать();
			 
			 Пока Выборка.Следующий() Цикл
				Сообщить("Для доп.расхода "+выборка.Номенклатура + "допустимое значение не больше "+ выборка.значение);
				отказ = Истина;
			 КонецЦикла;
		 конецЕсли;
...
Процедура ОбработкаЗаполнения(Основание) Экспорт
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаказПоОтветственномуХранению") Тогда 
		...
		СтавкаНДС = СтавкаНДСнаДату( ТекущаяДата() ); //+++ 09.01.2019 = перечисления.СтавкиНДС.НДС18;
		табЗнач.ЗаполнитьЗначения(СтавкаНДС, "СтавкаНДС");
...
//			КонецЕсли; 			
	...
	//	ДоговорКонтрагента 					= Реализация0.ДоговорКонтрагента;
		...
		СтавкаНДС_2019 = СтавкаНДСнаДату( ТекущаяДата() ); //+++ 09.01.2019 = перечисления.СтавкиНДС.НДС18;
		...
		Для Каждого ТекСтрокаТовары Из Выб Цикл
			...
			НоваяСтрока.СтавкаНДС	   = СтавкаНДС_2019; //+++ 09.01.2019 
	...
	иначе
		...
		Если ЗаказПоОТХ<>неопределено тогда
			...
			ЗапросОстаткиОТХ.Текст="ВЫБРАТЬ
						 ...
						 ЕстьСкладАдресногоХранения = ЛОЖЬ;//12.11.2018 - анализ адресов!
						 Для каждого стр из  ТоварыОТХ цикл
							 ...
							 Для каждого эл из МассивСтр цикл
								 Если осталосьСписать>0 тогда
									 ...
									 Если эл.Склад.АдресноеХранение тогда    //12.11.2018 - проверка
									 ЕстьСкладАдресногоХранения=Истина;
								 	 КонецЕсли;	
						 ...
						 КонецЦикла;
					 	 ...
					 	 //+++( 12.11.2018 - Заполнение адресов!
						 Если ЕстьСкладАдресногоХранения тогда 
							Сообщить("Списание с ОТХ с ячеек Адресного хранения! Пытаемся списать c тех же ячеек, куда было сделано Поступление ОТХ по этому клиенту!");
						 	таблАдр = ПолучитьАдресаОТХ( ЗаказПоОТХ, ТоварыДляСписания ); 
							для каждого стрАдр из таблАдр цикл
								стрТов = ОперацияОб.ТоварыАдресноеХранение.Добавить();
							    ЗаполнитьЗначенияСвойств(стрТов, стрАдр);
								Сообщить(строка(стрТов.Номенклатура.Код)+") "+строка(стрТов.Номенклатура)+" -> "+строка(стрТов.АдресХранения)+" : "+строка(стрТов.Количество)+" шт.", СтатусСообщения.Информация);
						    КонецЦикла;	
						 КонецЕсли;	 
						 //+++)
						 ...
						 если нехватает=Ложь и не ЕстьПоступлениеПоЗаказу тогда
							 ...
							 Если ЕстьСкладАдресногоХранения тогда //12.11.2018 - Заполнение адресов!
								 //таблАдр - уже получен выше!
								Сообщить("Поступление Товаров на те же ячейки, откуда списали ОТХ!");
						 		для каждого стрАдр из таблАдр цикл
									стрТов = ПоступлениеТиУ.ТоварыАдресноеХранение.Добавить();
								    ЗаполнитьЗначенияСвойств(стрТов, стрАдр);
								КонецЦикла;	
							 КонецЕсли;	 
							...
							Исключение
								 //ПоступлениеТиУ.Записать(РежимЗаписиДокумента.Проведение);
								 Сообщить("ОШИБКА Записи/проведения Поступления Товаров и Услуг: "+ОписаниеОшибки(), СтатусСообщения.Внимание);
...
//+++ 12.11.2018 - надо найти адреса, на которые положили ОТХ... 
//		если оно есть ещё есть там, инача с любых других мест!
функция ПолучитьАдресаОТХ( ЗаказПоОТХ, таблОТХ )
	Запрос=Новый Запрос;
	Запрос.Текст="ВЫБРАТЬ
	             |	ТаблТовСклад.Номенклатура,
	             |	ТаблТовСклад.Склад,
	             |	ТаблТовСклад.Количество
	             |ПОМЕСТИТЬ ВТ_Товары
	             |ИЗ
	             |	&ТаблТовСклад КАК ТаблТовСклад
	             |;
	             |
	             |////////////////////////////////////////////////////////////////////////////////
	             |ВЫБРАТЬ
	             ...
	             |	ВТ.Склад,
	             |	ВТ.Количество,
	             |	ТоварыАХ.АдресХранения,
	             |	ТоварыАХ.Количество КАК КоличествоАдр
				 //|	, ТоварыАХ.Ссылка КАК ОперацияОТХ,
				 //|	ТоварыАХ.СсылкаДокументОснование КАК ЗаказОТХ
	             |ИЗ
	             |	ВТ_Товары КАК ВТ
	             |		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	             |			товАХ.Ссылка КАК Ссылка,
				 |			товАХ.Ссылка.ДокументОснование КАК СсылкаДокументОснование,
	             |			товАХ.Номенклатура КАК Номенклатура,
	             |			товАХ.Склад КАК Склад,
	             |			товАХ.АдресХранения КАК АдресХранения,
	             |			товАХ.Количество КАК Количество
	             |		ИЗ
	             |			Документ.ОперацияПоОтветственномуХранению.ТоварыАдресноеХранение КАК товАХ
	             |		ГДЕ
	             |			(товАХ.Номенклатура, товАХ.Склад) В	(ВЫБРАТЬ вт.Номенклатура, вт.Склад ИЗ ВТ_Товары КАК вт)
				//------------------------ последние 10 приходов -------------------------------------
	             |			И товАХ.Ссылка В (ВЫБРАТЬ ПЕРВЫЕ 10
	             |						ОперацияОТХ.Ссылка КАК Ссылка
	             |					ИЗ
	             |						Документ.ОперацияПоОтветственномуХранению КАК ОперацияОТХ
	             |					ГДЕ
	             |						ОперацияОТХ.Проведен
	             |						И ОперацияОТХ.Контрагент = &Контрагент
	             |						И ОперацияОТХ.ДоговорКонтрагента = &ДоговорКонтрагента
	             |						И ОперацияОТХ.ВидОперации = Значение(Перечисление.ВидыОперацийПоОтветственномуХранению.Поступление)
				 |					УПОРЯДОЧИТЬ ПО ОперацияОТХ.Дата УБЫВ)
				 |						) КАК ТоварыАХ
	             |		ПО ВТ.Номенклатура = ТоварыАХ.Номенклатура
	             |			И ВТ.Склад = ТоварыАХ.Склад
				 |; 
				 |////////////////////////////////////////////////////////////////////////////////
				 |Уничтожить ВТ_Товары";
	 
	 Запрос.УстановитьПараметр("Контрагент",ЗаказПоОТХ.Контрагент);
	 Запрос.УстановитьПараметр("ДоговорКонтрагента",ЗаказПоОТХ.ДоговорКонтрагента);
	 
	 ТаблТовСклад = таблОТХ.Скопировать(); //ТоварыДляСписания = Операция.Товары.Выгрузить();
	 i=0; N = ТаблТовСклад.Количество();
	 пока i<N цикл
		 Если ТаблТовСклад[i].Склад.АдресноеХранение Тогда i=i+1;
		 	 Иначе ТаблТовСклад.Удалить(i); N=N-1;
		КонецЕсли;	 
	 КонецЦикла;	 
	 Запрос.УстановитьПараметр("ТаблТовСклад", ТаблТовСклад );
	 
	 результат  = запрос.Выполнить();
	 табл1 = результат.Выгрузить();  
	 
//---постОбработка - списываем Количество, по разным ячейкам------
тов1 = Неопределено; Кол1=0;
	Для каждого стр1 из табл1 цикл 
		Если тов1 = неопределено 
			или тов1<>стр1.Номенклатура тогда
			тов1 = стр1.Номенклатура;
			кол1 = стр1.Количество;
		КонецЕсли;	
		колСписать = мин(кол1, стр1.Количество, стр1.КоличествоАдр);
		колСписать = ?(колСписать<0, 0, колСписать); //списание в 1 сторону!
		Кол1 = Кол1 - колСписать;
		Если стр1.Количество > колСписать тогда
			стр1.Количество = колСписать;
		КонецЕсли;
	КонецЦикла;
//--------------------удалям строки с 0 количеством---------------	
	 i=0; N = табл1.Количество();
	 Пока i<N цикл
		Если табл1[i].Количество>0 тогда i=i+1;
			Иначе табл1.Удалить(i); N=N-1;
		КонецЕсли;
	 КонецЦикла;		
//----------------------------------------------------------------	
