			...
			Если ПустаяСтрока(ИзФормулы) Тогда
				...
				Если НЕ РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения Тогда
					...
					Запрос.Текст = "ВЫБРАТЬ
					               ...
					               |	А.Номенклатура,
					               |	А.Реализация,
					               |	СУММА(А.КоличествоВТекущейЗаявке) КАК КоличествоВТекущейЗаявке,
					               |	СУММА(ЕСТЬNULL(А.КоличествоВЗаявкахНаБрак, 0)) КАК КоличествоВЗаявкахНаБрак,
					               |	СУММА(РеализацияТоваровУслугТовары.Количество) КАК КоличествоВРеализации
					               |ИЗ
					               |	(ВЫБРАТЬ
					               ...
					               |		ПО А.Реализация = РеализацияТоваровУслугТовары.Ссылка
					               |			И А.Номенклатура = РеализацияТоваровУслугТовары.Номенклатура
					               |
					               |СГРУППИРОВАТЬ ПО
					               |	А.Номенклатура,
					               |	А.Реализация";
