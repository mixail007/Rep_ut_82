...
Процедура ДействияФормыСоздатьПретензию()
	...
	задача.Наименование 	= "Выставить претензию на перевозчика связанную с порчей товара"; 
	...
	Попытка
			задача.Записать();
			Сообщить(" + Сформирована задача по выставлению претензии о порче товара для сотрудника: "+строка(задача.Исполнитель), СтатусСообщения.Информация );

	исключение
		Сообщить(" - Не удалось сформировать задачу по выставлению претензии о порче товара для сотрудника: "+строка(задача.Исполнитель), СтатусСообщения.Внимание );	
	КонецПопытки;
