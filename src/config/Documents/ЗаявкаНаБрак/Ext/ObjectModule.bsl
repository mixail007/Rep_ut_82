		...
		Если ПустаяСтрока(ИзФормулы) Тогда
			...
			Если (Расшифровка.Количество()=0) и не БракОбнаруженНаСкладе Тогда 
				...
				иначеЕсли Реализация.Дата < ДобавитьМесяц(Дата,-N) и ИмМожно.НайтиПоЗначению(глТекущийПользователь)=Неопределено 
					...
					//***2018.10.30 заремлено, т.к. надо давать редактировать операторам состояние заявки 
...
Функция ПечатьРекламационнойКарточки()	
	...
	Если ВидТовара = Перечисления.ВидыТоваров.Диски Тогда
		Область.Параметры.ВидТовара =	"дисков" ;
	ИначеЕсли ВидТовара = Перечисления.ВидыТоваров.Шины Тогда
		Область.Параметры.ВидТовара =	"шин" ;
	ИначеЕсли ВидТовара = Перечисления.ВидыТоваров.Аксессуары Тогда
		Область.Параметры.ВидТовара =	"аксессуаров" ;
	ИначеЕсли  ВидТовара = Перечисления.ВидыТоваров.АКБ Тогда
		Область.Параметры.ВидТовара =	"аккумуляторов" ;
	иначе
		Область.Параметры.ВидТовара =	"" ;
	КонецЕсли;
		...
		Если не БракОбнаруженНаСкладе Тогда	
			...
			Если ПустаяСтрока(ИзФормулы) Тогда
				...
				Для Каждого СтрРасшифровка из Расшифровка Цикл
					Если ЗначениеЗаполнено(СтрРасшифровка.Реализация) и ТипЗнч(СтрРасшифровка.Реализация) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
						Если  СтрРасшифровка.Реализация.Контрагент <>  Контрагент  
							...
							ОтказВсе = Истина;
						КонецЕсли;
					ИначеЕсли ЗначениеЗаполнено(СтрРасшифровка.Реализация) и ТипЗнч(СтрРасшифровка.Реализация) = Тип("ДокументСсылка.ОтчетОРозничныхПродажах") Тогда
					Иначе //Если НЕ ЗначениеЗаполнено(СтрРасшифровка.Реализация) тогда	
						Сообщить("Необходимо выбрать документ продажи в строке №: " + СтрРасшифровка.НомерСтроки, СтатусСообщения.Внимание);
						ОтказВсе = Истина;
					КонецЕсли;
				КонецЦикла;
				...
				Запрос.Текст = "ВЫБРАТЬ
				               ...
				               |			И ДокНоменклатура.Реализация = ТЧРеализации.ДокументПродажи";
				РезультатЗапроса = Запрос.Выполнить();
				Выборка = РезультатЗапроса.Выбрать();
				Пока Выборка.Следующий() Цикл
					Если Выборка.Количество > Выборка.КоличествоИзРеализации Тогда
						Сообщить("Номенклатуры " + Выборка.Номенклатура + " по реализации №" + ?(Выборка.Реализация<>Неопределено,Выборка.Реализация.Номер,"") + " продано меньше, чем указано в заявке.", СтатусСообщения.Внимание);
						ОтказВсе = Истина;
					КонецЕсли;
				КонецЦикла;
				...
				Если НЕ РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения Тогда
					...
					Запрос.Текст = "ВЫБРАТЬ
					               ...
					               |	А.Номенклатура,
					               |	А.Реализация,
					               |	СУММА(ЕСТЬNULL(А.КоличествоВТекущейЗаявке, 0)) КАК КоличествоВТекущейЗаявке,
					               |	СУММА(ЕСТЬNULL(А.КоличествоВЗаявкахНаБрак, 0)) КАК КоличествоВЗаявкахНаБрак,
					               |	СУММА(ЕСТЬNULL(Б.Количество, 0)) КАК КоличествоВРеализации
					               |ИЗ
					               |	(ВЫБРАТЬ
					               |		ТЧНоменклатура.Номенклатура КАК Номенклатура,
					               |		ТЧНоменклатура.Реализация КАК Реализация,
					               |		МАКСИМУМ(ТЧНоменклатура.Количество) КАК КоличествоВТекущейЗаявке,
					               |		СУММА(ЗаявкаНаБракРасшифровка.Количество) КАК КоличествоВЗаявкахНаБрак
					               |	ИЗ
					               |		ВТ_Номенклатура КАК ТЧНоменклатура
					               |			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаявкаНаБрак.Расшифровка КАК ЗаявкаНаБракРасшифровка
					               |			ПО ТЧНоменклатура.Номенклатура = ЗаявкаНаБракРасшифровка.Номенклатура
					               |				И ТЧНоменклатура.Реализация = ЗаявкаНаБракРасшифровка.Реализация
					               |				И (ЗаявкаНаБракРасшифровка.Ссылка.Контрагент = &Контрагент)
					               |				И (ЗаявкаНаБракРасшифровка.Ссылка.Проведен)
					               |				И (НЕ ЗаявкаНаБракРасшифровка.Ссылка = &Ссылка)
					               |	
					               |	СГРУППИРОВАТЬ ПО
					               |		ТЧНоменклатура.Номенклатура,
					               |		ТЧНоменклатура.Реализация) КАК А
					               ...
					               |		СГРУППИРОВАТЬ ПО
					               |			ОтчетОРозничныхПродажахТовары.Номенклатура,
					               |			ОтчетОРозничныхПродажахТовары.Ссылка) КАК Б
					               |		ПО А.Реализация = Б.Ссылка
					               |			И А.Номенклатура = Б.Номенклатура
					               |
					               |СГРУППИРОВАТЬ ПО
					               |	А.Номенклатура,
					               |	А.Реализация";
					...
					Пока Результат.Следующий() Цикл
						Если Результат.КоличествоВРеализации < Результат.КоличествоВТекущейЗаявке + Результат.КоличествоВЗаявкахНаБрак Тогда
							Сообщить("По товару " + Строка(Результат.Номенклатура) + ", купленному по документу " + Строка(Результат.Реализация) + ", уже введены заявки на брак.");
							ОтказВсе = Истина;
						КонецЕсли;
					КонецЦикла;
				КонецЕсли;
...
Процедура СоздатьЗадачуНаПретензию()
	...
	Исполнитель1 = Справочники.Пользователи.НайтиПоКоду("Брызгалова А.");
...
//--- Шарафутдинов 
	...
	//***заремлено, т.к. непонятно, зачем всем видеть это сообщение, если виновник не отдел логистики
