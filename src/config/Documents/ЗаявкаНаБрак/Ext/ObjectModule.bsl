...
Процедура ДействияФормыСоздатьПретензию()
	...
	//+++ Щарафутдинов 01.02.2018     ищем уже созданные задачи от заявки на брак
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
	...
	Если НЕ Результат.Пустой() тогда		
		Возврат; 						// Если есть уже задачи от заявки - ничего не создаем
	КонецЕсли;
	...
	Задача = Задачи.ЗадачиПользователя.СоздатьЗадачу();
	Задача.Исполнитель 		= Справочники.Пользователи.НайтиПоКоду("Захарова Светлана");
...
Функция ПолучитьЦену(Номенклатура, Параметр) 
