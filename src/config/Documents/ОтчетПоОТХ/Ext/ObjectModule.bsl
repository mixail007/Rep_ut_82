...
Процедура РазобратьПоFIFO() экспорт
	...
	Запрос.УстановитьПараметр("ГоловноеПодразделение", Подразделение);
	иначе	
...
Функция СоздатьПакетДокументов(ДоговорРеализации = неопределено) экспорт
	Если ЗначениеЗаполнено(Подразделение) тогда
	...
	иначе
		...
		Если 	ГоловноеПодразделение = Справочники.Подразделения.НайтиПоКоду("00005") тогда
			...
			конецЕсли;
		иначе 
			нужныйСклад = справочники.Склады.НайтиПоНаименованию(""+ СокрЛП(Контрагент.код)+" (ОТХ) "+ГоловноеПодразделение.Код);
			Если НужныйСклад = Справочники.Склады.ПустаяСсылка() тогда
				НужныйСклад = справочники.Склады.СоздатьЭлемент();
				НужныйСклад.Наименование =""+ СокрЛП(Контрагент.код)+" (ОТХ) "+ГоловноеПодразделение.Код;
				НужныйСклад.Транзитный = Истина;
				НужныйСклад.Подразделение = Подразделение;
				НужныйСклад.Комментарий = Контрагент.Наименование;
				НужныйСклад.ВидСклада = Перечисления.ВидыСкладов.Оптовый;
				НужныйСклад.Записать();
			конецЕсли;
		...
		Заказы = ТоварыПоЗаказам.Выгрузить(,"Заказ,ДатаОтгрузки");
		Заказы.свернуть("Заказ,ДатаОтгрузки");
		для каждого стр из Заказы цикл
			...
			иначе
				ДокВозврат.Дата = стр.ДатаОтгрузки+61;
			конецесли;
		...
		иначе
			...
			Для каждого стр из Договоры Цикл
				...
				|	И 99 = 99";
				Запрос.УстановитьПараметр("ЗаказКолесаТут",ложь);
				Если ДоговорРеализации<>неопределено тогда
					...
					Запрос.УстановитьПараметр("ЗаказКолесаТут",истина);
				КонецЕсли;
				...
				Заказ.ВыписатьСпецификацию = ложь;
				...
				Заказ.ЧислоДнейСМоментаОтгрузки = (Заказ.ДатаОплаты-Заказ.ДатаОплаты)/(24*60*60);
				...
				Для каждого стр1 из строки цикл
					...
					ст.СтавкаНДС = Перечисления.СтавкиНДС.НДС20;
					ст.Сумма = стр1.Сумма;
		...
		Попытка 
			...
			Для каждого зак из списокЗаказов цикл
				...
				иначе
					Реализация.Дата = ?(ЗначениеЗаполнено(Заказ.ДатаОтгрузки),КонецДня(Заказ.ДатаОтгрузки)-61,Заказ.Дата);
				конецесли;
				СтрокаПрефикс = "ТК";
...
//		
//КонецПроцедуры
Процедура СписатьсОТХВФормуле() экспорт
	URL = "http://ServiceFormula";
	Прокси = WSссылки.WSFormula.СоздатьWSПрокси(URL, "ServiceFormula", "ServiceFormulaSoap12");
	...
	тзРеал = СформированныеДокументы.Выгрузить();
	//тзРеал.Свернуть("ДатаОтгрузки,НомерОтгрузки");
	...
	Для каждого стр из тзРеал цикл
		Если ТипЗнч(стр.Документ) = Тип("ДокументСсылка.РеализацияТоваровУслуг") тогда
			тТовары = стр.Документ.Товары.Выгрузить(,"Номенклатура,Количество,Сумма");
			...
			Для каждого найденнаяСтрока из тТовары цикл
				Товар = Фабрика.Создать(Фабрика.Тип("http://Formula", "Product"));
				Товар.Code.Добавить(прав(СокрЛП(найденнаяСтрока.Номенклатура.Код),7));
				Товар.Quantity.Добавить(найденнаяСтрока.Количество);
				Товар.Price = Окр(найденнаяСтрока.Сумма/найденнаяСтрока.Количество,0);
				...
				Товар.Point.Добавить(КодСклада);
				МассивТовары.Products.Добавить(Товар);
				зЗаказ = стр.Документ.Сделка;
			КонецЦикла;
			
			Попытка
				дДата = стр.Документ.Дата;
				//2015-08-01T00:00:00
				дДата = Формат(дДата,"ДФ=yyyy-MM-ddThh:mm:ss");
				Рез = Прокси.ReturnFromResponsibleStorage(МассивТовары, стр.Документ.Номер, дДата, зЗаказ.Номер, ""+ДокументОснование.КоординатыДоставки, "");
				Если Рез.Success = ложь тогда
					Комментарий = Комментарий+" # "+Рез.Error;
					GuidСтороннейОрганизации = Рез.OrderNumber;
					Записать(РежимЗаписиДокумента.Проведение);
...
Процедура ОтправитьДоки() экспорт
	Для каждого стрДок из СформированныеДокументы цикл
		если ТипЗнч(стрДок.Документ) = Тип("ДокументСсылка.РеализацияТоваровУслуг") тогда
			...
			ПеренестиВКопию = истина;
			текДок.ПолучитьОбъект().ПеренестиДокиВБухгалтерию(ПеренестиВКопию);
			
			//  УПД
			имя = "ТОРГ-12 (Яршинторг)";
			ПФСсылка = Справочники.ДополнительныеПечатныеФормы.НайтиПоНаименованию(имя);
			
			
			ИмяФайла = ПолучитьИмяВременногоФайла(); 
			ДвоичныеДанные = ПФСсылка.ХранилищеВнешнейОбработки.Получить(); 
			ДвоичныеДанные.Записать(ИмяФайла); 
			
			Обработка = ВнешниеОбработки.Создать(ИмяФайла,ложь); 
			Обработка.СсылкаНаОбъект = текДок;
