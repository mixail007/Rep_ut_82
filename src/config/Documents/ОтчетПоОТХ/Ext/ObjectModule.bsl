...
Процедура СписатьсОТХВФормуле()
	...
	Для каждого стр из тзРеал цикл
		...
		Попытка
			дДата = Формат(стр.ДатаОтгрузки, "ДФ=yyyy-MM-ddThh:mm:ss");
			
			Рез = Прокси.ReturnFromResponsibleStorage(МассивТовары, стр.НомерОтгрузки, дДата, зЗаказ.Номер, ""+зЗаказ.УникальныйИдентификатор(), ДокументОснование.КоординатыДоставки);
