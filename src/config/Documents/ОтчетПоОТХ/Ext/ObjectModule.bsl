...
Процедура СписатьсОТХВФормуле()
	...
	Для каждого стр из тзРеал цикл
		...
		Попытка
			Рез = Прокси.ReturnFromResponsibleStorage(МассивТовары, стр.НомерОтгрузки, стр.ДатаОтгрузки, зЗаказ.Номер, зЗаказ.УникальныйИдентификатор(), ДокументОснование.НомерВходящегоДокумента, ДокументОснование.КоординатыДоставки);
