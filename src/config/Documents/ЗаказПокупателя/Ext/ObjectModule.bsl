					...
					Если ТаблПоставщиков[0].АвтоПодтверждение и Статус = перечисления.СтатусыЗаказов.Подтвержден Тогда
						...
						Для каждого стр1 из ТаблПоставщиков цикл
							стр2 = Товары.Найти(стр1.Номенклатура,"Номенклатура");
						 	стр2.Размещение = справочники.Склады.НайтиПоКоду("02332"); //Ангар-Ремонт Тест (запрещенный!)
						КонецЦикла;
						
					иначеЕсли  нетЗаказов И Статус = перечисления.СтатусыЗаказов.Подтвержден Тогда //19.12.2016 если подтвержден поставщиком и есть остатки сторонних поставщиков....
