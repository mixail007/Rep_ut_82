...
//+++ 28.06.2018 Константы.ИспользоватьОстаткиСтороннихПоставщиков.Получить() = ЛОЖЬ ВСЕГДА!
	...
	Если ДатаОплаты<>Дата(1,1,1)и СрокиОплаты.Количество()=0 тогда
	...
	Если ДатаОплаты <> СрокиОплаты[СрокиОплаты.Количество()-1].ДатаОплаты тогда
	     ДатаОплаты = СрокиОплаты[СрокиОплаты.Количество()-1].ДатаОплаты;
	конецЕсли;	
	
	
	Если СрокиОплаты.Итог("Процент")<> 100-?(ДоговорКонтрагента.ТипДоговора = Справочники.ТипыДоговоров.ПеремещениеОТХ,ПроцентОТХ,0) тогда
		#Если Клиент Тогда
