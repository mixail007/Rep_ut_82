...
процедура  ДвиженияПоЗаказыПокупателяСезонные(СтруктураШапкиДокумента, ТаблицаПоТоварам)
	...
	|						ТОГДА ЗаказПокупателяСезонный.Транзит = ЛОЖЬ
...
процедура ДвижениеПоРезервДляИМ( СтруктураШапкиДокумента, ТаблицаПоТоварам )
	...
	Если КлиентИМ = Контрагент Тогда
		...
		#Если Клиент Тогда
			Сообщить("Создан ""Резерв Для ИМ"" Для "+строка(ТаблицаДвижений.Количество())
			+" товаров ("+строка(ТаблицаДвижений.Итог("Количество"))+"шт.) по клиенту: "+строка(КлиентИМ), СтатусСообщения.Информация);
		#КонецЕсли	
	Иначе
		Движения.РезервДляИМ.ВыполнитьРасход();
		#Если Клиент Тогда
			Сообщить("Списан ""Резерв Для ИМ"" Для "+строка(ТаблицаДвижений.Количество())
			+" товаров ("+строка(ТаблицаДвижений.Итог("Количество"))+"шт.) по клиенту: "+строка(КлиентИМ), СтатусСообщения.Информация);
		#КонецЕсли	
		...
		Если ЗначениеЗаполнено(СтруктураВыбора) Тогда
			...
			Иначе	
				...
				Иначе
					...
					//|	ЗаказыПокупателейСезонныйТовары.Цена КАК Цена,
					...
					//|		Ссылка КАК Ссылка
					//|	ИЗ
					//|		Документ.ЗаказПокупателяСезонный.Товары 
					//|	ГДЕ  Ссылка = &Ссылка
					////И Ссылка.Контрагент = &Контрагент
					//|) КАК ЗаказыПокупателейСезонныйТовары
					//|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗаказыПокупателей.Остатки(,ЗаказПокупателя.Проверен
					//
					////список  05.11.2015
					//|//ОтборПоОдномуДоговору И ДоговорКонтрагента В (&ДоговорКонтрагентаСписок)
					//|					И ДоговорКонтрагента.Владелец В (&КонтрагентСписок)
					//
					...
					//
					//|и ЗаказПокупателя.ДокументОснование =&Ссылка) КАК ЗаказыКонтрагента
					//|		ПО ЗаказыПокупателейСезонныйТовары.Номенклатура = ЗаказыКонтрагента.Номенклатура
					//|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗаказыПокупателей.Остатки(, 	
					////+++ 14.07.2014 -вычитаем проделенные заказы по 1 подразделению!!!---------------------------------------
					//|  ЗаказПокупателя.Проверен
					//|					И (    (&Транзит
					//|							И ЗаказПокупателя.Транзит
					//|							И ЗаказПокупателя.Подразделение = &Подразделение
					...
					//|
					//|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ Номенклатура, КоличествоОборот
					//|		ИЗ
					...
					//| ДоговорКонтрагента.Владелец В (&КонтрагентСписок) и ЗаказПокупателя.ДокументОснование =&Ссылка
					//|//ОтборПоОдномуДоговору И ДоговорКонтрагента В (&ДоговорКонтрагентаСписок)
					//
					////-------------------------------вычитаем ВСЕ приходы по всем подразделениям!!!---------------------------------------
					////+++ продажи только по этому подразделению!
					////|		И (  (&Транзит и Подразделение = &Подразделение) 
					////|		 или ( НЕ &Транзит
					////|		  )	 ) 		     
					//|						 )                    ) КАК ПродажиОбороты
				...
				Если СтруктураВыбора.ВидОстатков>=2 Тогда  // по приходам (товарам в пути!)  //+++ 19.08.2014
					...
					|								СЗТовары.Номенклатура
					|							ИЗ
					|								СЗТовары) И ЕСТЬNULL(Подразделение.ВЭД, ЛОЖЬ) = ЛОЖЬ
					|						И ЗаказПоставщику В (ВЫБРАТЬ РАЗЛИЧНЫЕ СписЗак.Заказ ИЗ ВТ_СписЗак КАК СписЗак)) КАК ЗаказыПоставщикам) КАК ЗаказыПоставщикамОстатки
					...
					Если СтруктураВыбора.ВидСклада>0 Тогда
						запрос1.УстановитьПараметр("Транзит", Истина);
					Иначе
					...
					Если СтруктураВыбора.ВидОстатков = 3 
						...
						запрос1.текст = стрЗаменить(запрос1.текст, "ЗаказПоставщику В (ВЫБРАТЬ РАЗЛИЧНЫЕ СписЗак.Заказ ИЗ ВТ_СписЗак КАК СписЗак)",
						"(ЗаказПоставщику.ДатаПриходаВПортИлиНаТаможнюETA >= &Дата1 И ЗаказПоставщику.ДатаПриходаВПортИлиНаТаможнюETA <= &Дата2) 
						| И ЗаказПоставщику.Транзит = &Транзит 
						| И Выбор когда &Транзит Тогда ЗаказПоставщику.Подразделение = &Подразделение
						|			Иначе Истина Конец ");
					Иначе
