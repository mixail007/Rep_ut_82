...
#Если Клиент Тогда
	...
	Функция ПечатьСчетаЗаказаКорректировки(Тип, СКартинкой = ложь) Экспорт //+++ 18.06.2015
		...
		Если Шапка.ДоговорКонтрагента.ТипДоговора = Справочники.ТипыДоговоров.ШинтрейдЯрославль Тогда //Для экспорта
			Если НЕ ОрганизацияОтгрузки.Пустая() Тогда
				Если Найти(Шапка.ДоговорКонтрагента.Наименование,"*")>0 тогда  //16.04.2018 по задаче № 44778
					ОбластьМакета.Параметры.ПредставлениеПоставщика = ОписаниеОрганизации(СведенияОЮрФизЛице(ОрганизацияОтгрузки, Шапка.Дата), "ПолноеНаименование,");
				иначе	
		...
		Если (Тип = "Счет" или Тип = "СчетАртикул") и ЗначениеЗаполнено(ЭтотОбъект.Грузоотправитель) Тогда 
			ОбластьМакета = Макет.ПолучитьОбласть("Грузоотправитель");
				Если Найти(ДоговорКонтрагента.Наименование,"*")>0 тогда  //16.04.2018 по задаче № 44778
					ОбластьМакета.Параметры.ПредставлениеГрузоотправителя = ОписаниеОрганизации(СведенияОЮрФизЛице(ЭтотОбъект.Грузоотправитель, Шапка.Дата), "ПолноеНаименование,");
				иначе
		...
		Если (Тип = "Счет" или Тип = "СчетАртикул") и ЗначениеЗаполнено(ЭтотОбъект.Грузополучатель) Тогда 
			...
			Если Найти(ДоговорКонтрагента.Наименование,"*")>0 тогда  //16.04.2018 по задаче № 44778
				ОбластьМакета.Параметры.ПредставлениеГрузополучателя = ОписаниеОрганизации(СведенияОЮрФизЛице(ЭтотОбъект.Грузополучатель, Шапка.Дата), "ПолноеНаименование,");
			иначе
...
//+++)
	...
	Если Найти(ДоговорКонтрагента.Наименование,"*")>0 тогда
		...
		i=0; 
		импортЗима = справочники.НоменклатурныеГруппы.НайтиПоКоду("00016");
		импортЛето = справочники.НоменклатурныеГруппы.НайтиПоКоду("00017");
		пока i<таблТов.Количество() цикл
			Если таблТов[i].Номенклатура.НоменклатурнаяГруппа = импортЗима
			 или таблТов[i].Номенклатура.НоменклатурнаяГруппа = импортЛето тогда
				i=i+1;
			иначе 
				таблТОв.Удалить( i );
			КонецЕсли;
		КонецЦикла;	
		Если таблТов.Количество()>0 тогда
			спис1 = таблТов.ВыгрузитьКолонку("Номенклатура");
...
//16.04.2018
Функция получитьЦеныB2B(списТов)
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
...
Процедура изменитьМинЦенуНаB2B(табл1, табл2)
	...
	Для каждого стр1 из табл1 цикл
		Для каждого стр2 из табл2 цикл
			...
			ценаB2B = стр2.Цена * (1 + процB2B/100); // по задаче № 44778 - согласование на Доколина B2B  -2%
			 
			Если стр1.Номенклатура = стр2.Номенклатура
				и стр1.ЦенаОбщ>ценаB2B тогда   //более высокий порог - снижается!
				#Если Клиент тогда
					сообщить(">> Цена общ. по товару "+строка(стр1.Номенклатура.Код)+" "+строка(стр1.Номенклатура)+" - не может быть меньше цены B2B ("+строка(процB2B)+"%): "+строка(ценаB2B)+"р.");
				#КонецЕсли	
