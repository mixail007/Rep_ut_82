...
//+++ 28.06.2018 Константы.ИспользоватьОстаткиСтороннихПоставщиков.Получить() = ЛОЖЬ ВСЕГДА!
	...
	Если НЕ (  (ЭтоНовый()                  // --- новые заказы НЕ проверяются и НЕ Согласуются! --- 
		...
		или (ЭтотОбъект.БонусПрименен <> Ссылка.БонусПрименен)    //***2018.06.21 после применения бонуса за * цены не согласовываются, т.к. эта скидка при проведении реализации относится на затраты 
		) Тогда
