...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ЭтоНовый() И Контрагент.ПечататьСпецификацию и Найти(ДоговорКонтрагента.Наименование,"*")=0 тогда  //04.04.2018 - только для Б/нал
...
//+++ 28.06.2018 Константы.ИспользоватьОстаткиСтороннихПоставщиков.Получить() = ЛОЖЬ ВСЕГДА!
	...
	Если Константы.ИспользоватьПодаркиЗаНаборы.Получить() Тогда
		Если (ЗначениеНеЗаполнено(ДоговорКонтрагента.КонтрагентДляРезерваИМ) или ДоговорКонтрагента.ОтветственноеЛицо.НаправлениеПродаж = Справочники.НаправленияПродаж.НайтиПоКоду("16")) и (ЭтоНовый() или РежимПроведения = РежимПроведенияДокумента.Оперативный) 
					...
					Если  нетЗаказов И Статус = перечисления.СтатусыЗаказов.Подтвержден Тогда //19.12.2016 если подтвержден поставщиком и есть остатки сторонних поставщиков....
						...
						Для каждого пост1 из табл1 Цикл  //по каждому поставщику - своя задача! только на его товар!
							...
							Для каждого стр2 из закПост.Товары Цикл
								...
								Иначе
									стр2.СтавкаНДС = перечисления.СтавкиНДС.БезНДС;
									стр2.СуммаНДС  = 0;
								КонецЕсли;	
...
//	Запрос1.УстановитьПараметр("Подразделение",  Подразделение);
//	результат_Подарки = Запрос1.Выполнить();  //Подарки
//	
//	Если НЕ результат_Подарки.Пустой() тогда 	
//		
...
//		Запрос.Текст = "ВЫБРАТЬ
//					   |	ТаблПодарки.Подарок,
//					   |	ТаблПодарки.Количество КАК Количество,
//					   |	ТаблПодарки.КоличествоПодарка
//					   |ПОМЕСТИТЬ Подарки
//					   |ИЗ
...
//					   |			ТОГДА ЕСТЬNULL(Остатки.ОстатокПодарковНаСкладе, 0) + Подарки.Количество * Подарки.КоличествоПодарка
...
//				  |ВЫБРАТЬ РАЗЛИЧНЫЕ
//				  |	вт2.НаборДляПодарка
//				  |ПОМЕСТИТЬ втНаборДляПодаркаИсключить
//				  |ИЗ
//				  |	вт2 КАК вт2
//				  |ГДЕ
...
//				  |ВЫБРАТЬ
//				  |	а.НаборДляПодарка,
//				  |	МИНИМУМ(а.КоличествоПодарков) КАК КоличествоПодарков,
//				  |	а.ЗаказИМ
//				  |ПОМЕСТИТЬ вт
//				  |ИЗ
//				  |	(ВЫБРАТЬ
//				  |		вт2.НаборДляПодарка КАК НаборДляПодарка,
//				  |		вт2.Номенклатура КАК Номенклатура,
//				  |		вт2.КоличествоНоменклатурыПодарка КАК КоличествоНоменклатурыПодарка,
//				  |		вт2.КоличестовоВЗаказеДляПодарка КАК КоличестовоВЗаказеДляПодарка,
//				  |		вт2.КоличествоПодарков КАК КоличествоПодарков,
//				  |		вт2.ЗаказИМ КАК ЗаказИМ
//				  |	ИЗ
//				  |		вт2 КАК вт2) КАК а
//				  |ГДЕ
//				  |	НЕ а.НаборДляПодарка В
//				  |				(ВЫБРАТЬ
//				  |					втНаборДляПодаркаИсключить.НаборДляПодарка
//				  |				ИЗ
//				  |					втНаборДляПодаркаИсключить)
//				  |
//				  |СГРУППИРОВАТЬ ПО
//				  |	а.НаборДляПодарка,
//				  |	а.ЗаказИМ
//				  |
//				  |ИМЕЮЩИЕ
//				  |	МИНИМУМ(а.КоличествоПодарков) > 0
//				  |;
//				  |
//				  |////////////////////////////////////////////////////////////////////////////////
//				  |ВЫБРАТЬ РАЗЛИЧНЫЕ
//				  |	НаборыДляПодарков.Подарок,
//				  |	НаборыДляПодарков.КоличествоПодарка * вт.КоличествоПодарков КАК Количество
//				  |ИЗ
//				  |	вт КАК вт
//				  |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.НаборыДляПодарков КАК НаборыДляПодарков
//				  |		ПО вт.НаборДляПодарка = НаборыДляПодарков.Номенклатура
//				  |			И вт.ЗаказИМ = НаборыДляПодарков.ПодарокДляKolesatyt
//				  |ГДЕ
//				  |	НаборыДляПодарков.Подразделение = &Подразделение
//				  |;
//				  |
...
//	Результат_ПодаркиРассчитанные = Запрос2.Выполнить(); //  29.03.18 Смирнов Наборы для подарков
//	
//	Если НЕ Результат_ПодаркиРассчитанные.Пустой() тогда //только если есть подарки... 
//		Запрос = Новый Запрос;
//		таблПодаркиРассчитанные = Результат_ПодаркиРассчитанные.Выгрузить();   // предыдущий запрос!
//		Запрос.УстановитьПараметр("таблПодаркиРассчитанные", таблПодаркиРассчитанные);
//		
//		Если Подразделение = Справочники.Подразделения.ПустаяСсылка() или Подразделение = Справочники.Подразделения.НайтиПоКоду("00005") тогда
//			СкладПодразделения = ложь;
//		Иначе
...
//					 |	ПодаркиРассчитанные.Количество
...
//					 |ВЫБРАТЬ
//					 |	втПодаркиРассчитанные.Подарок,
//					 |	втПодаркиРассчитанные.Количество,
...
//				Нов = Бонусы.Добавить();
//				Нов.Номенклатура = стр.Подарок;
//				Нов.Количество = мин(стр.Количество, стр.СвободныйОстаток);
//				Нов.СтатьяЗатрат = Справочники.СтатьиЗатрат.НайтиПоКоду("О0215");
//				Нов.Бонус = истина;
//			КонецЕсли;
//		КонецЦикла;
...
Процедура ЗаполнитьТабЧастьБонусы(БратьИзИсходныхЗаказов = ложь)
	...
	Запрос2.Текст="ВЫБРАТЬ
				  ...
				  |ВЫБРАТЬ РАЗЛИЧНЫЕ
				  |	вт.Подарок,
				  |	СУММА(вт.НеобходимоПодарить) КАК НеобходимоПодарить
				  |ИЗ
				  |	вт КАК вт
				  |
				  |СГРУППИРОВАТЬ ПО
