...
Перем СогласовыватьРекламу, флБылиОтгрузки;
...
//+++ 28.06.2018 Константы.ИспользоватьОстаткиСтороннихПоставщиков.Получить() = ЛОЖЬ  ВСЕГДА! другая система рабтоает!
		...
		Если Найти(Шапка.ДоговорКонтрагента.Наименование,"*")=0 тогда  //24.04.2018 по задаче № 44778
		...
		ОбластьМакета.Параметры.Заполнить(Шапка);
		Если Найти(ДоговорКонтрагента.Наименование, "*")>0 тогда //16.04.2018 - по задаче 44778
		ОбластьМакета.Параметры.ПредставлениеПоставщика =""; //24.04.2018
		иначе	
		...
		Иначе
			...
			Если Найти(ДоговорКонтрагента.Наименование, "*")>0 тогда //16.04.2018 - по задаче 44778
				ОбластьМакета.Параметры.ПредставлениеПолучателя = ""; //24.04.2018
			иначе	
	...
	Функция ПечатьСчетаЗаказаКорректировки(Тип, СКартинкой = ложь) Экспорт //+++ 18.06.2015
		...
		|АВТОУПОРЯДОЧИВАНИЕ";
		//+++ 30.06.2014
//+++ 28.06.2018 ИспользоватьОстаткиСтороннихПоставщиков =НЕТ ВСЕГДА!		
		//Если Константы.ИспользоватьОстаткиСтороннихПоставщиков.Получить()
		//	и НЕ Поставщик.пустая() Тогда // 13.08.2015 только Если есть поставщик!
		//	Запрос.Текст = стрЗаменить(Запрос.Текст, "//СтатусНеОтменен", " 
		//	| И  ЗаказПокупателя.Статус <> Значение(перечисление.СтатусыСтрокЗаказа.Отменен)");
		...
		Если Тип = "Счет" или Тип = "СчетАртикул" Тогда
			...
			КонецЕсли;	 
			//Конец ОЛ
			Если Найти(Шапка.ДоговорКонтрагента.Наименование,"*")=0 тогда //24.04.2018
			ОбластьМакета.Параметры.Заполнить(Шапка);
			ОбластьМакета.Параметры.ИНН = СведенияОПоставщике.ИНН;
			ОбластьМакета.Параметры.КПП = СведенияОПоставщике.КПП;
			КонецЕсли;
			...
			Если ТипЗнч(СтруктурнаяЕдиница) = Тип("СправочникСсылка.БанковскиеСчета") и Найти(Шапка.ДоговорКонтрагента.Наименование,"*")=0  Тогда
				Банк       = СтруктурнаяЕдиница.Банк;
				БИК        = Банк.Код;
				КоррСчет   = Банк.КоррСчет;
				НомерСчета = СтруктурнаяЕдиница.НомерСчета;
				...
				ОбластьМакета.Параметры.ПредставлениеПоставщика          = СтруктурнаяЕдиница.ТекстКорреспондента;
			КонецЕсли;
			Если ЗначениеНеЗаполнено(ОбластьМакета.Параметры.ПредставлениеПоставщика) и Найти(Шапка.ДоговорКонтрагента.Наименование,"*")=0 тогда //24.04.2018
				ОбластьМакета.Параметры.ПредставлениеПоставщика = ОписаниеОрганизации(СведенияОПоставщике, "ПолноеНаименование,");
			КонецЕсли;
			...
			КонецЕсли;
			Если Не ОрганизацияОтгрузки.Пустая() и Найти(Шапка.ДоговорКонтрагента.Наименование,"*")=0 тогда //24.04.2018
		...
		ОбластьМакета = Макет.ПолучитьОбласть("Поставщик");
		ОбластьМакета.Параметры.Заполнить(Шапка);
       	Если Найти(Шапка.ДоговорКонтрагента.Наименование,"*")>0 тогда  //16.04.2018 по задаче № 44778
			ОбластьМакета.Параметры.ПредставлениеПоставщика = ""; //24.04.2018
		Иначе
			ОбластьМакета.Параметры.ПредставлениеПоставщика = ОписаниеОрганизации(СведенияОЮрФизЛице(Шапка.Организация, Шапка.Дата), "ИНН,КПП,ПолноеНаименование,ЮридическийАдрес,Телефоны,");
		КонецЕсли;
		...
		Если (Тип = "Счет" или Тип = "СчетАртикул") и ЗначениеЗаполнено(ЭтотОбъект.Грузоотправитель) Тогда 
			ОбластьМакета = Макет.ПолучитьОбласть("Грузоотправитель");
				Если Найти(ДоговорКонтрагента.Наименование,"*")>0 тогда  //16.04.2018 по задаче № 44778
					ОбластьМакета.Параметры.ПредставлениеГрузоотправителя = ""; //24.04.2018
				иначе
		...
		Если (Тип = "Счет" или Тип = "СчетАртикул") и ЗначениеЗаполнено(ЭтотОбъект.Грузополучатель) Тогда 
			...
			Если Найти(ДоговорКонтрагента.Наименование,"*")>0 тогда  //16.04.2018 по задаче № 44778
				ОбластьМакета.Параметры.ПредставлениеГрузополучателя = ""; //24.04.2018
			иначе
		...
		// Сакулина
		Если Тип ="Счет" и Найти(Шапка.ДоговорКонтрагента.Наименование,"*")=0 Тогда
			//Миронычев
			Если Шапка.ДоговорКонтрагента.ТипДоговора = Справочники.ТипыДоговоров.ПредоплатаПоСчетам Тогда
		...
		// Вывести подписи
		Если (Тип = "Счет" или Тип = "СчетАртикул") и Найти(Шапка.ДоговорКонтрагента.Наименование,"*")=0 Тогда  //24.04.2018
			...
			Иначе
				Если глТекущийПользователь = Справочники.Пользователи.НайтиПоКоду("Невежина И.Н.") 
					или глТекущийПользователь = Справочники.Пользователи.НайтиПоКоду("Пименова Н.И.") тогда
	...
	Функция СформироватьНазначениеПлатежа(ЗапросТовары)
		...
		Для Каждого СтрокаНДС ИЗ ТабНДС цикл
			...
			ТекстСумма = ТекстСумма + "НДС(" + ?(ЗначениеНеЗаполнено(СтрокаНДС.СтавкаНДС), "без налога", СтрокаНДС.СтавкаНДС) + ") "
			+ Формат(СтрокаНДС.СуммаНДС, "ЧЦ=15;ЧДЦ=2;ЧРД=-;ЧН=0-00")+Символы.ПС;
		КонецЦикла;
...
//+++ 28.06.2018 -> Константы.ОбязательнаяУстановкаПодразделений.Получить() = ЛОЖЬ ВСЕГДА!
...
//+++ 28.06.2018	Константы.ОплатыРаспределяютМенеджеры.Получить() = Истина ВСЕГДА!
...
Процедура ПроверитьПовторяющиесяТовары(Отказ)
	...
	Если не Отказ Тогда
		...
		Если ТабБезДублей.Количество()<Товары.Количество() Тогда // Есть дубли! 
			...
			//+++ 24.07.2018  Отказ = Истина; //НЕ РольДоступна("ПравоЗавершенияРаботыПользователей"); // НИКОМУ НЕ ДОСТУПНО!
			...
			Для каждого стр1 из ТабБезДублей Цикл
				...
				#Если Клиент тогда
					...
					Отказ = Истина;
				#Иначе
			//+++ 24.07.2018 на сервере - удаляем дубли строк ! +++
					для i=1 по стр2.Количество()-1 цикл
					  Товары.Удалить( стр2[i] );
					КонецЦикла;
				#КонецЕсли
				КонецЕсли;
			КонецЦикла;
...
Процедура ПроверитьНулевыеЦены(Отказ) экспорт
	ЦенаФикс = 25000; //единая (максимальная) цена для всех ТОВАРОВ!
	ТемаНулевыхЦен = "НУЛЕВЫЕ ЦЕНЫ на ТОВАРЫ по Заказу "+строка(ссылка.Номер)+" Покупателя: "+строка(ссылка.Контрагент);
	
	//04.10.2017 - надо брать данные из ОБЪЕКТА!
	таблИзЗаказа = ЭтотОбъект.Товары.Выгрузить(,"Номенклатура, Цена, Количество, Статус, НомерСтроки");
	
	Запрос = новый Запрос;
	//+++( 04.06.2018 - прямой запрос в том же запросе, а не 2хN раз в базе!
	масТов = таблИзЗаказа.ВыгрузитьКолонку("Номенклатура");
	Запрос.УстановитьПараметр("масТов",масТов);
	Запрос.УстановитьПараметр("ЦенаФикс", ЦенаФикс);
	//+++)
	
	//---------таблица Для согласования--------------------	
	Запрос.Текст="ВЫБРАТЬ
	             |	ТЗ.НомерСтроки как НомерСтрокиОснования,
	             |	ТЗ.Цена,
				  |	ТЗ.Количество,
				 |	ТЗ.Статус,
	             |	ТЗ.Номенклатура
	             ...
	             |////////////////////////////////////////////////////////////////////////////////
	             |ВЫБРАТЬ
	              |	спрТов.ПользовательУтверждающий,
				  |	ЗаказПокупателяТовары.Статус,
				 |	ЗаказПокупателяТовары.Номенклатура КАК Номенклатура,
				 |	спрТов.Код КАК НоменклатураКод,
	             |	ВЫБОР
	             |		КОГДА спрТов.ВидТовара = ЗНАЧЕНИЕ(Перечисление.ВидыТоваров.Шины)
	             |				И ЗаказПокупателяТовары.Цена < &минШины
	             |			ТОГДА &минШины
	             |		КОГДА спрТов.ВидТовара = ЗНАЧЕНИЕ(Перечисление.ВидыТоваров.Диски)
	             |				И ЗаказПокупателяТовары.Цена < &минДиски
	             |			ТОГДА &минДиски
	             |		КОГДА спрТов.ВидТовара = ЗНАЧЕНИЕ(Перечисление.ВидыТоваров.АКБ)
	             |				И ЗаказПокупателяТовары.Цена < &минАКБ
	             |			ТОГДА &минАКБ
	             |		ИНАЧЕ ВЫБОР
	             |				КОГДА ЗаказПокупателяТовары.Цена < 1
	             |					ТОГДА 1
	             |				КОГДА ЗаказПокупателяТовары.Цена > 1000000
	             |					ТОГДА 1000000
				 ...
				 //===========ТА ЦЕНА, КОТОРАЯ УСТАНОВЛЕНА В ЗАКАЗ!=================
				 |	&ЦенаФикс КАК ЦенаМин,
				 
	              |	ВЫБОР
	             |		КОГДА спрТов.ВидТовара = ЗНАЧЕНИЕ(Перечисление.ВидыТоваров.Шины) ТОГДА &минШины
	             |		КОГДА спрТов.ВидТовара = ЗНАЧЕНИЕ(Перечисление.ВидыТоваров.Диски)ТОГДА &минДиски
	             |		КОГДА спрТов.ВидТовара = ЗНАЧЕНИЕ(Перечисление.ВидыТоваров.АКБ)  ТОГДА &минАКБ
	             |		ИНАЧЕ 1 Конец как ЦенаПоТипуЦен,
				 //--------кр.опт тоже отсутсвует!------------------------
				 |	2000 как Приоритет,
				 
	             |	ЗаказПокупателяТовары.НомерСтрокиОснования
	             |ИЗ
	             |	ВТ_Товары КАК ЗаказПокупателяТовары
				 |	Левое соединение(выбрать 
				 |		спр.Код, 
				 |		спр.ВидТовара, 
				 |		выбор когда спр.НоменклатурнаяГруппа = Значение(Справочник.НоменклатурныеГруппы.ПустаяСсылка) тогда &ИсполнительЗадачи
				 |		иначе спр.НоменклатурнаяГруппа.Ответственный Конец как ПользовательУтверждающий,  
				 |		спр.Ссылка как Ссылка1 
				 |		из справочник.Номенклатура как спр 
				 |		где спр.Ссылка в (&МасТов) ) как спрТов
				 |		по ЗаказПокупателяТовары.Номенклатура = спрТов.Ссылка1
	             |ГДЕ
	             |	ЗаказПокупателяТовары.Цена <= 1
	             |	ИЛИ ЗаказПокупателяТовары.Цена >= 1000000
	             |	ИЛИ (спрТов.ВидТовара = ЗНАЧЕНИЕ(Перечисление.ВидыТоваров.Шины) И ЗаказПокупателяТовары.Цена < &минШины)
	             |	ИЛИ (спрТов.ВидТовара = ЗНАЧЕНИЕ(Перечисление.ВидыТоваров.Диски) И ЗаказПокупателяТовары.Цена < &минДиски)
	             |	ИЛИ (спрТов.ВидТовара = ЗНАЧЕНИЕ(Перечисление.ВидыТоваров.АКБ)И ЗаказПокупателяТовары.Цена < &минАКБ)
				 |
	             |УПОРЯДОЧИТЬ ПО
	...
	ИсполнительЗадачи = справочники.Подразделения.НайтиПоКоду("00005").Руководитель; // Бонд
	Запрос.УстановитьПараметр("ИсполнительЗадачи", ИсполнительЗадачи);	
	...
	таблНулЦен = Результат.Выгрузить();	
	
	Если таблНулЦен.Количество()=0 Тогда
		возврат;
	КонецЕсли;	
	
	ЭтоРобот = ( Найти(строка(ссылка.Ответственный),"Робот")>0 );    
	УстанавливатьМинЦену = ЛОЖЬ; //10.04.2019 - НИКОГДА и НИКТО не может установить?!  = ЭтоРобот или (НЕ Контрагент.НеКонтроллироватьЦеныПродажи);
	
  ЭтоПредоплата = (ДоговорКонтрагента.ТипДоговора = справочники.ТипыДоговоров.НайтиПоКоду("00001") 
  			   или ДоговорКонтрагента.ТипДоговора = справочники.ТипыДоговоров.ПредоплатаПоСчетам //+++14.03.2018 
		       или ДоговорКонтрагента.ТипДоговора = справочники.ТипыДоговоров.ФакторингПредоплата //  16.05.2017
                   ); 
				   
	флНадоДелатьЗадачи = ЛОЖЬ;
   
	Для i=0 по таблНулЦен.Количество()-1 Цикл
		Выборка = таблНулЦен[i];
			
		//Если РольДоступна("ПравоЗавершенияРаботыПользователей") Тогда  // ТОЛЬКО Для IT-отдела можно делать любую цену!			
		//	#Если Клиент Тогда
		...
		//МЕНЯЕМ ТОВАРЫ!
		Если ЗначениеНеЗаполнено(Выборка.Статус) тогда // 1 раз отправляем!
			стр1 = Товары.Найти(выборка.НомерСтрокиОснования, "НомерСтроки");
			флНадоДелатьЗадачи = Истина;
			Если УстанавливатьМинЦену тогда
				#Если Клиент Тогда
					сообщить("В строке № "+строка(выборка.НомерСтрокиОснования)+" Товар: "+СокрЛП(выборка.НоменклатураКод)+" - "+СокрЛП(выборка.Номенклатура)+" - Цена: "+формат(выборка.Цена,"ЧДЦ=2")+"р. изменена до минимально-допустимую: "+формат(выборка.ЦенаМин,"ЧДЦ=2")+"р.", СтатусСообщения.Внимание);
				#КонецЕсли	
				стр1.Цена   = Выборка.Цена;
				стр1.Статус = Перечисления.СтатусыСтрокЗаказа.НаСогласовании;  //отправляем на согласование!
			Иначе
				#Если Клиент Тогда
					сообщить("В строке № "+строка(выборка.НомерСтрокиОснования)+" Товар: "+СокрЛП(выборка.НоменклатураКод)+" - "+СокрЛП(выборка.Номенклатура)+" - Цена: "+формат(выборка.Цена,"ЧДЦ=2")+"р. изменена но ФИКСИРОВАННУЮ: "+формат(ЦенаФикс,"ЧДЦ=2")+"р.", СтатусСообщения.Внимание);
				#КонецЕсли	
				стр1.Цена = ЦенаФикс;
	...
	КонецЦикла;	
	Если флНадоДелатьЗадачи тогда
		ЭтотОбъект.ОбменДанными.Загрузка = истина;
		ЭтотОбъект.Записать();
		ЭтотОбъект.ОбменДанными.Загрузка = ЛОЖЬ;
	КонецЕсли;	
	Если флНадоДелатьЗадачи тогда // 1 раз только отправляем задачи!
		таблРук = таблНулЦен.Скопировать(,"ПользовательУтверждающий");
		таблРук.свернуть("ПользовательУтверждающий");
		для i=0 по таблРук.Количество()-1 цикл
			ИсполнительЗадачи = таблРук[i].ПользовательУтверждающий;
			СоздатьЗадачуНаСогласование(ИсполнительЗадачи, таблНулЦен, ТемаНулевыхЦен, ЭтоПредоплата);
		КонецЦикла;	
	КонецЕсли;
...
Процедура СоздатьЗадачуНаСогласование(ПользовательУтверждающий, таблСогл, Тема="", ЭтоПредоплата=ЛОЖЬ)
	
	задача1 = НайтиЗадачуНаСогласованиеПоЗаказу(ПользовательУтверждающий, Тема);
	
	Если задача1 = неопределено Тогда 
		
		задачаОб = Задачи.ЗадачиПользователя.СоздатьЗадачу();
		
		задачаОб.Исполнитель = ПользовательУтверждающий;
	...
	Иначе //------из Задачи - записываем Заказ, а из заказа - Задачу...
		#Если Клиент Тогда
			сообщить("Уже есть Задача на согласование от "+строка(задача1.Дата)+" Для пользователя: "+строка(ПользовательУтверждающий), СтатусСообщения.Информация);
		#КонецЕсли
	...
	КонецЕсли;	
	строки1 =  таблСогл.НайтиСтроки( новый Структура("ПользовательУтверждающий", ПользовательУтверждающий));
	...
	Если строки1.Количество()>0 Тогда
		...
		Для i=0 по строки1.Количество()-1 Цикл
			...
			|";
		КонецЦикла;	   
		задачаОб.Наименование = Тема; //10.04.2019
		
		задачаОб.Описание = ?(задачаОб.Выполнена, "Согласованы", "Требуется согласовать")+" следующие цены:
		|"+Тема+"
		|Отгрузка: "+ ?(ссылка.ОтгрузкаСПодразделенияСклада, строка(Склад.Подразделение)+" (с доп.перемещением!)",строка(Подразделение))+"
		//22.08.2016
		|Менеджер: "+строка(ДоговорКонтрагента.ОтветственноеЛицо)
		+?(Подразделение<>ДоговорКонтрагента.ОтветственноеЛицо.ОсновноеПодразделение, "("+строка(ДоговорКонтрагента.ОтветственноеЛицо.ОсновноеПодразделение)+")","")
		+"
		//08.02.2017
		|Адрес покупателя: "+?(строка(Контрагент.Регион)<>"", "
		|РЕГИОН : "+врег(строка(Контрагент.Регион)),"")+"
		...
		задачаОб.ОбменДанными.Загрузка = ЛОЖЬ;
		#Если Клиент Тогда
			сообщить("Записана Задача на согласование Для пользователя: "+строка(ПользовательУтверждающий), СтатусСообщения.Информация);
		#КонецЕсли	
	КонецЕсли;
КонецПроцедуры
...
Процедура ПроверитьЗаполнениеТабличнойЧастиТовары(ТаблицаПоТоварам, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	//+++ 04.06.2018 - НЕТ учёта по характеристикам! уже давно!
...
Процедура  ДвиженияПоЗаказыПокупателяСезонные(СтруктураШапкиДокумента, ТаблицаПоТоварам)
	...
	табл.Свернуть("Номенклатура", "Количество"); //без дублей
	...
	|ГДЕ
	|	АналогиДисков.Номенклатура В (ВЫБРАТЬ ВТ_Товары.Номенклатура ИЗ ВТ_Товары)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	РегистрСведений.АналогиДисков КАК АналогиДисков
	|ГДЕ
	|	АналогиДисков.Аналог В (ВЫБРАТЬ ВТ_Товары.Номенклатура ИЗ ВТ_Товары)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ИЗ
	//+++ 16.07.2018 сначала отбор, а потом соединение
	|(выбрать * из	РегистрСведений.АналогиДисков КАК АналогиДисков0
	|ГДЕ
	|	АналогиДисков0.Аналог В (ВЫБРАТЬ ВТ_Товары.Номенклатура ИЗ ВТ_Товары) ) как АналогиДисков
	...
	|ГДЕ
	|	АналогиДисков.Аналог <> ЕСТЬNULL(АналогиДисков1.Аналог, АналогиДисков.Аналог)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	(выбрать * из РегистрСведений.АналогиДисков КАК АналогиДисков0
	|ГДЕ
	|	АналогиДисков0.Номенклатура В (ВЫБРАТЬ ВТ_Товары.Номенклатура ИЗ ВТ_Товары) ) как АналогиДисков
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналогиДисков КАК АналогиДисков1
	|		ПО АналогиДисков.Аналог = АналогиДисков1.Аналог
	|ГДЕ
	|	АналогиДисков.Номенклатура <> ЕСТЬNULL(АналогиДисков1.Номенклатура, АналогиДисков.Номенклатура)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ГДЕ
	|	АналогиДисков.Номенклатура <> ЕСТЬNULL(АналогиДисков1.Аналог, АналогиДисков.Номенклатура)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ГДЕ
	|	АналогиДисков.Аналог <> ЕСТЬNULL(АналогиДисков1.Номенклатура, АналогиДисков.Аналог)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ 
	...
	|			И Номенклатура В (ВЫБРАТЬ ВТ.Аналог ИЗ НоменклатураЗаказаСАналогами КАК ВТ)
	|		И (ЗаказПокупателяСезонный.Транзит = ЛОЖЬ 
	|//ИЛИДок	или ЗаказПокупателяСезонный = &ДокОснование 
	|		  )
//-------------------наоборот - проверка полей на условие &Дата	--------------------------
	//|			И (&Дата МЕЖДУ ЗаказПокупателяСезонный.Дата И ЗаказПокупателяСезонный.ДатаДействияПо)  
	|		И ЗаказПокупателяСезонный.Дата<=&Дата И ЗаказПокупателяСезонный.ДатаДействияПо>=&Дата   
	...
	|	ОстаткиСезонных.ЗаказПокупателяСезонныйМоментВремени как ЗаказПокупателяСезонныйМоментВремени
	|ИЗ
	...
	Если ДокументОснование.Транзит тогда // ещё условие ИЛИ
		Запрос.Текст = стрЗаменить(Запрос.Текст,"//ИЛИДок","");
		Запрос.УстановитьПараметр("ДокОснование",ДокументОснование);
	КонецЕсли;	
	Результат = Запрос.Выполнить();  //ДвиженияПоЗаказыПокупателяСезонные()
...
процедура ДвижениеПоРезервДляИМ( СтруктураШапкиДокумента, ТаблицаПоТоварам )
	...
	Если КлиентИМ = СтруктураШапкиДокумента.Контрагент Тогда
		...
		ТаблицаДвижений.ЗаполнитьЗначения(КлиентИМ,"КонтрагентДляРезерваИМ");//07.11.2018 - фикс
		
	ИначеЕсли КлиентИМ = справочники.Контрагенты.НайтиПоКоду("П004703") тогда //Резерв для ИМ
		//ТОЛЬКО в "-" по резервуДляИМ и ТОЛЬКО по "+" ОстаткамПоРезервуИМ
		таблРезерва = найтиПоложительныеОстаткиРезерваИМ_старая(СтруктураШапкиДокумента, ТаблицаПоТоварам);	
		ЗагрузитьВТаблицуЗначений(таблРезерва, ТаблицаДвижений); //номенклатура, количество
		ТаблицаДвижений.ЗаполнитьЗначения(КлиентИМ,"КонтрагентДляРезерваИМ");//07.11.2018 - фикс
	Иначе
		//ТОЛЬКО в "-" по резервуДляИМ и ТОЛЬКО по "+" ОстаткамПоРезервуИМ
		таблРезерва = найтиПоложительныеОстаткиРезерваИМ_new(СтруктураШапкиДокумента, ТаблицаПоТоварам);
		если таблРезерва.Количество()=0 тогда
			возврат;
		КонецЕсли;	
		ЗагрузитьВТаблицуЗначений(таблРезерва, ТаблицаДвижений); //номенклатура, количество
	КонецЕсли;
	...
	//07.11.2018 - ТаблицаДвижений.ЗаполнитьЗначения(КлиентИМ,"КонтрагентДляРезерваИМ");
	...
	Иначе
		...
		#Если Клиент Тогда
			...
			Сообщить("По регистру ""Резерв Для ИМ"" списано "+строка(ТаблицаДвижений.Количество())+" товаров ("+строка(ТаблицаДвижений.Итог("Количество"))+"шт.)", СтатусСообщения.Информация);
			КонецЕсли;
		#КонецЕсли	
		
		//==== и сразу по ЗаказыПокупателей по этим заказам надо сторнировать!======================
		//      по подразделению и клиентуИМ по всем заказам с + остатками
	Если КлиентИМ = справочники.Контрагенты.НайтиПоКоду("П004703") тогда //Резерв для ИМ
		СторнироватьЗаказПокупателяКлиентаИМ_старая(СтруктураШапкиДокумента, таблРезерва);//как было!
	иначе
		СторнироватьЗаказПокупателяКлиентаИМ_new(СтруктураШапкиДокумента, таблРезерва);//06.11.2018 - НОВАЯ !!!
	КонецЕсли;	
...
функция найтиПоложительныеОстаткиРезерваИМ_старая( Шапка, Табл1 )
...
//+++ 06.11.2018 - 25.03.2019 - через разрешения - если есть св. остатки - сначала с них (резерв НЕ списывается!)
функция найтиПоложительныеОстаткиРезерваИМ_new( Шапка, Табл1 )
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	Табл1.Номенклатура,
	               |	Табл1.Количество
	               ...
	               |ВЫБРАТЬ
	               |	РезервДляИМОстатки.Номенклатура,
		 //25.03.2019 - если есть св.остаток
	               |	ВЫБОР КОГДА Остатки.Остаток - Остатки.Заказано > 0
				   // и товаров в заказе меньше, чем в св.остатке - тогда ненужно резервировать!
	               |			ТОГДА ВЫБОР КОГДА Остатки.Остаток - Остатки.Заказано >= ВТ1.Количество
	               |						ТОГДА 0
	               // и товары в заказе частично есть в св.остатке - тогда нужно резервировать, но меньше!
	               |						ИНАЧЕ ВЫБОР КОГДА РезервДляИМОстатки.КоличествоОстаток > ВТ1.Количество - (Остатки.Остаток - Остатки.Заказано)
	               |									ТОГДА ВТ1.Количество - (Остатки.Остаток - Остатки.Заказано)
	               |									ИНАЧЕ РезервДляИМОстатки.КоличествоОстаток
	               |							  КОНЕЦ
	               |				  КОНЕЦ
	               |			ИНАЧЕ ВЫБОР КОГДА РезервДляИМОстатки.КоличествоОстаток > ВТ1.Количество
	               |						ТОГДА ВТ1.Количество
	               |						ИНАЧЕ РезервДляИМОстатки.КоличествоОстаток
	               |				  КОНЕЦ
	               |	КОНЕЦ КАК Количество,
	               |	РезервРазрешенияСрезПоследних.КонтрагентРезерв КАК КонтрагентДляРезерваИМ
	               |ИЗ
	               ...
	               |				Номенклатура В (ВЫБРАТЬ вт.Номенклатура ИЗ ВТ_Табл1 КАК вт)
	               |					И Подразделение = &Подразделение) КАК РезервДляИМОстатки
	               |			ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	               |				остОТХ.Номенклатура КАК Номенклатура,
	               |				СУММА(ЕСТЬNULL(остОТХ.КоличествоОстаток, 0)) КАК Остаток,
	               ...
	               |								И Склад.ЗапретитьИспользование = ЛОЖЬ) КАК ост
	               |				ГДЕ
	               ...
	               |				И СУММА(остОТХ.КоличествоОстаток) >= 0) КАК Остатки
	               |			ПО РезервДляИМОстатки.Номенклатура = Остатки.Номенклатура
	               |		ПО РезервРазрешенияСрезПоследних.КонтрагентРезерв = РезервДляИМОстатки.КонтрагентДляРезерваИМ
	               |			И РезервРазрешенияСрезПоследних.ВидТовара = РезервДляИМОстатки.Номенклатура.ВидТовара
	               |			И (РезервРазрешенияСрезПоследних.НоменклатурнаяГруппа = РезервДляИМОстатки.Номенклатура.НоменклатурнаяГруппа
	               ...
	               |ГДЕ
	               |	РезервДляИМОстатки.КоличествоОстаток > 0
	               |	И РезервРазрешенияСрезПоследних.ДатаОкончания = ДАТАВРЕМЯ(1, 1, 1)";
	Запрос.УстановитьПараметр("Дата",    новый Граница(Шапка.Дата, ВидГраницы.Исключая) ); // на долю секунды раньше даты документа!
	Запрос.УстановитьПараметр("Подразделение",         Шапка.Подразделение);
	Запрос.УстановитьПараметр("Контрагент",            Шапка.Контрагент);
	
	флТранзит = (Шапка.Подразделение.Код <> "00005");  //25.03.2019
	Запрос.УстановитьПараметр("Транзит",флТранзит);
	Запрос.УстановитьПараметр("ПодразделениеСклада",   ?(флТранзит, Шапка.Подразделение, справочники.Подразделения.ПустаяСсылка()) ); 
	...
	i=0; //+++ 25.03.2019 +++  удаляем строки, по которым есть св.остатки
	пока i<резТабл.Количество() цикл
		если резТабл[i].Количество<=0 тогда
			#Если Клиент тогда
				сообщить("Товар "+строка(резТабл[i].Номенклатура)+" - есть в св.остатках и не требует снятия с резерва!");
			#КонецЕсли
			резТабл.Удалить(i);
		иначе	
			i=i+1;
...
процедура СторнироватьЗаказПокупателяКлиентаИМ_старая(Шапка, Табл1)
	...
	//+++ 04.06.2018 для большого числа заказов
	|"+?(Табл1.Количество()>=100,"Индексировать по Номенклатура","")+"
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
//+++ 04.06.2018
|ЗакПок.ЗаказПокупателя.Дата как ДатаЗаказа,
	...
	|	КОНЕЦ КАК Количество,
	|	ВЫРАЗИТЬ(ЗакПок.СуммаВзаиморасчетовОстаток * 
//+++ 04.06.2018---------------------------------------
	|	ВЫБОР КОГДА ЗакПок.КоличествоОстаток > ВТ1.Количество
	|		ТОГДА ВТ1.Количество / ЗакПок.КоличествоОстаток
	|		ИНАЧЕ 1
	|	КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаВзаиморасчетов,
	|	ВЫРАЗИТЬ(ЗакПок.СуммаУпрОстаток * 
//+++ 04.06.2018---------------------------------------
	|	ВЫБОР	КОГДА ЗакПок.КоличествоОстаток > ВТ1.Количество
	|		ТОГДА ВТ1.Количество / ЗакПок.КоличествоОстаток
	|		ИНАЧЕ 1
	...
	|				И Номенклатура В (ВЫБРАТЬ вт.Номенклатура ИЗ ВТ_Табл1 КАК вт)
	|											  ) КАК ЗакПок
	|ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Табл1 как ВТ1
...
|; УНИЧТОЖИТЬ ВТ_Табл1";
	...
	//+++ 04.06.2018 - так быстрее чем в запросе почти в 2 раза с 0,47 до 0,26 сек?!
	таблРез.Сортировать("ДатаЗаказа"); 
...
КонецПроцедуры
//+++ 06.11.2018 - через разрешения
процедура СторнироватьЗаказПокупателяКлиентаИМ_new(Шапка, Табл1)
	...
	//			   |"+?(Табл1.Количество()>=100,"Индексировать по Номенклатура","")+"
	               ...
	               |ИЗ
	               |	РегистрСведений.РезервРазрешения.СрезПоследних( &Дата,
	               |			Контрагент = &Контрагент И Подразделение = &Подразделение) КАК РезервРазрешенияСрезПоследних
				   
				   // резервые заказы! по контрагентам сам себе резерв! на дату заказа!
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗаказыПокупателей.Остатки( &Дата,
				   
				   //знаем точно какие резервы нам надо!
	               |	ДоговорКонтрагента.Владелец в (выбрать разрешения.КонтрагентРезерв из  
				   |			РегистрСведений.РезервРазрешения.СрезПоследних( &Дата,
	               |			Контрагент = &Контрагент И Подразделение = &Подразделение)как разрешения )
				   
				   |					И ЗаказПокупателя.Подразделение = &Подразделение
	               |					И ЗаказПокупателя.Проверен
	               |					И Номенклатура В (ВЫБРАТЬ вт.Номенклатура ИЗ ВТ_Табл1 КАК вт)  ) КАК ЗакПок
				   
	               |			ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Табл1 КАК ВТ1
	               |			ПО ЗакПок.Номенклатура = ВТ1.Номенклатура
				   
	               |		ПО  ЗакПок.ДоговорКонтрагента.Владелец = РезервРазрешенияСрезПоследних.КонтрагентРезерв
				   |			И  ЗакПок.Номенклатура.ВидТовара = РезервРазрешенияСрезПоследних.ВидТовара
	               |			И (ЗакПок.Номенклатура.НоменклатурнаяГруппа = РезервРазрешенияСрезПоследних.НоменклатурнаяГруппа
				   |				или РезервРазрешенияСрезПоследних.НоменклатурнаяГруппа = Значение(Справочник.НоменклатурныеГруппы.ПустаяСсылка) )
	               |ГДЕ
	               |	ЗакПок.КоличествоОстаток > 0
	               |	И РезервРазрешенияСрезПоследних.ДатаОкончания = ДАТАВРЕМЯ(1, 1, 1)
	...
	//+++ 07.11.2018 --------------->  РезервРазрешенияСрезПоследних.КонтрагентРезерв  как КонтрагентДляРезерваИМ
	
	Запрос.УстановитьПараметр("Контрагент",Шапка.Контрагент); //+++ 06.11.2018
	Запрос.УстановитьПараметр("Табл1", Табл1 );
	...
	Если таблЗак.Количество()>1 Тогда
		#Если Клиент Тогда
			//Сообщить("Существует "+строка(таблЗак.Количество())+" резервных заказов на "+строка(Шапка.Дата)+" по клиенту ИМ: "+строка(Шапка.Контрагент.КонтрагентДляРезерваИМ), СтатусСообщения.Внимание); //123заменить
			Сообщить("Существует "+строка(таблЗак.Количество())+" резервных заказов на "+строка(Шапка.Дата)+" по клиенту ИМ!", СтатусСообщения.Внимание);
		#КонецЕсли	
...
функция ЕстьОстаткиПоСезоннымЗаказамКлиента()
	...
	|			,
	|			Контрагент = &Контрагент
				// 16.07.2018 только 1 клиент и 1 заказ DISTRI CASH ACCESSOIRES, WG000078
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, 
	...
	Если Не Отказ Тогда
		...
		Если // (ДатаОтсчета <> Дата(1,1,1) и ДатаОтсчета <=Дата) и
			НеУчитыватьВСезонномЗаказе=ЛОЖЬ и Проверен тогда
	...
	//		КонецЕсли;
		...
		Если (не ЭтотОбъект.ДоговорКонтрагента.КонтрагентДляРезерваИМ.Пустая() 
			или ЕстьРезервИМ(ЭтотОбъект) //+++ 06.11.2018
			  )	и ЭтотОбъект.Проверен Тогда
...
//+++ 01.04.2019 - ВЫКЛЮЧЕНО ---		
	...
	//+++ 28.06.2018 Константы.ИспользоватьОстаткиСтороннихПоставщиков.Получить() = ЛОЖЬ ВСЕГДА!
				...
				//И СтрокаТаблицы.Статус = перечисления.СтатусыСтрокЗаказа.Отменен )
				Тогда
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаказПокупателяСезонный") Тогда
		...
		Если ЗначениеЗаполнено(СтруктураВыбора) Тогда
			...
			Иначе	
				...
				Если СтруктураВыбора.СУчетомАналогов Тогда
					...
					|		РегистрСведений.АналогиДисков КАК АналогиДисков
					|	ГДЕ
					|		АналогиДисков.Номенклатура В (ВЫБРАТЬ СЗТовары.Номенклатура ИЗ СЗТовары КАК СЗТовары)
					|	
					|	ОБЪЕДИНИТЬ ВСЕ
					|	
					|	ВЫБРАТЬ
					...
					|		РегистрСведений.АналогиДисков КАК АналогиДисков
					|	ГДЕ
					|		АналогиДисков.Аналог В (ВЫБРАТЬ СЗТовары.Номенклатура ИЗ СЗТовары КАК СЗТовары)
					|	
					|	ОБЪЕДИНИТЬ ВСЕ
					|	
					|	ВЫБРАТЬ
					...
					|		АналогиДисков1.Аналог
					|	ИЗ
					//+++ 16.07.2018 - сначала отбор, потом соединение
					//|		РегистрСведений.АналогиДисков КАК АналогиДисков
					|	(Выбрать * из РегистрСведений.АналогиДисков КАК АналогиДисков0
					|		ГДЕ
					|		АналогиДисков0.Аналог В (ВЫБРАТЬ СЗТовары.Номенклатура ИЗ СЗТовары КАК СЗТовары) ) как АналогиДисков
					|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналогиДисков КАК АналогиДисков1
					|			ПО АналогиДисков.Номенклатура = АналогиДисков1.Номенклатура
					|	ГДЕ
					//|		АналогиДисков.Аналог В (ВЫБРАТЬ СЗТовары.Номенклатура ИЗ СЗТовары КАК СЗТовары) И
					|		АналогиДисков.Аналог <> ЕСТЬNULL(АналогиДисков1.Аналог, АналогиДисков.Аналог)
					|	
					|	ОБЪЕДИНИТЬ ВСЕ
					|	
					...
					|	(Выбрать * из РегистрСведений.АналогиДисков КАК АналогиДисков0
					|		ГДЕ
					|		АналогиДисков0.Номенклатура В (ВЫБРАТЬ СЗТовары.Номенклатура ИЗ СЗТовары КАК СЗТовары) ) как АналогиДисков
 				|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналогиДисков КАК АналогиДисков1
					|			ПО АналогиДисков.Аналог = АналогиДисков1.Аналог
					|	ГДЕ
					//|		АналогиДисков.Номенклатура В (ВЫБРАТЬ СЗТовары.Номенклатура ИЗ СЗТовары КАК СЗТовары) И
					|		 АналогиДисков.Номенклатура <> ЕСТЬNULL(АналогиДисков1.Номенклатура, АналогиДисков.Номенклатура)
					|	
					|	ОБЪЕДИНИТЬ ВСЕ
					|	
		...
		//		|		РегистрСведений.АналогиДисков КАК АналогиДисков
			...
			|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналогиДисков КАК АналогиДисков1
					...
					|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗаказыПокупателей.Остатки( ,
					|					ЗаказПокупателя.Проверен И ЗаказПокупателя.Проведен
					...
					Если Основание.Транзит Тогда // транзитный и автокомплектация >> Для перемещения в филиал
						...
						Если СокрЛП(Основание.Контрагент.Код) = "93187" 
							...
							|	И ЗакПокТов.Ссылка.ПометкаУдаления = ЛОЖЬ
							|	И ЗакПокТов.Ссылка.транзит = ЛОЖЬ
				...
				Если СтруктураВыбора.ВидОстатков>=2 Тогда  // по приходам (товарам в пути!)  //+++ 19.08.2014
					...
					Если СтруктураВыбора.ВидОстатков = 3 
						...
						Дата1 = ДобавитьМесяц(НачалоДня(ТекущаяДата()),-6);
						запрос1.УстановитьПараметр("Дата1", Дата1);
						Если СтруктураВыбора.ВидОстатков = 3 Тогда
							запрос1.УстановитьПараметр("Дата2", Дата1 + 15*86400); //"ДОБАВИТЬКДАТЕ(&Дата1, ДЕНЬ, 15 )");   //15 т.к. на начало дня
						ИначеЕсли СтруктураВыбора.ВидОстатков = 4 Тогда
							запрос1.УстановитьПараметр("Дата2", Дата1 + 46*86400); //"ДОБАВИТЬКДАТЕ(&Дата1, ДЕНЬ, 46)");    //46 т.к. на начало дня
					...
					Иначе
						...
						|	И СписокЗаказовИзЗаданияНаПриход.ЗаказПоставщику.Транзит "+?(СкладТранзит, "И СписокЗаказовИзЗаданияНаПриход.ЗаказПоставщику.Подразделение = &Подразделение","=ЛОЖЬ")+"
				...
				Если СтруктураВыбора.СУчетомАналогов и структураВыбора.ВидОстатков<2 Тогда
					...
					Пока  i<N Цикл 
						...
						ставкаНДС_2019 = СтавкаНДСнаДату( ЭтотОбъект.Дата ); //28.12.2018 - 1 раз перед циклом
						//+++ Задача № 62563 - всегда 0%  и для $
						Если Контрагент.Экспорт //и ДоговорКонтрагента.ВалютаВзаиморасчетов = справочники.Валюты.НайтиПоКоду("643")
						и найти(врег(ДоговорКонтрагента.Наименование),"ЭКСПОРТ")>0    
							тогда ставкаНДС_2019 = перечисления.СтавкиНДС.НДС0; 
						КонецЕсли;
						чНДС = ПолучитьСтавкуНДС(ставкаНДС_2019); //18 или 20
						...
						Для каждого стр из НайденныеСтроки Цикл
							...
							Если КоличествоОсталосьЗаказать>0 Тогда
								...
								Если ЭтотОбъект.УчитыватьНДС тогда//+++ 28.12.2018 - универсальные функции
									НоваяСтрока.СтавкаНДС= ставкаНДС_2019;
				 					НоваяСтрока.СуммаНДС = РассчитатьСуммуНДС(НоваяСтрока.Сумма, УчитыватьНДС, СуммаВключаетНДС, чНДС);
								Иначе
									НоваяСтрока.СтавкаНДС = перечисления.СтавкиНДС.БезНДС; // 0%
				...
				Иначе
					...
					ставкаНДС_2019 = СтавкаНДСнаДату( этотОбъект.Дата ); //28.12.2018
					//+++ Задача № 62563 - всегда 0%  и для $
					Если Контрагент.Экспорт //и ДоговорКонтрагента.ВалютаВзаиморасчетов = справочники.Валюты.НайтиПоКоду("643")
					и найти(врег(ДоговорКонтрагента.Наименование),"ЭКСПОРТ")>0    
						тогда ставкаНДС_2019 = перечисления.СтавкиНДС.НДС0; 
					...
					Пока  i<N Цикл 
						...
						Если КоличествоОсталосьЗаказать>0 Тогда
							...
							Если ЭтотОбъект.УчитыватьНДС  тогда //+++ 28.12.2018
								НоваяСтрока.СтавкаНДС= ставкаНДС_2019;
			 					НоваяСтрока.СуммаНДС = РассчитатьСуммуНДС(НоваяСтрока.Сумма, УчитыватьНДС, СуммаВключаетНДС, чНДС);
							Иначе
								НоваяСтрока.СтавкаНДС = перечисления.СтавкиНДС.БезНДС; // 0%
    ...
    РеквизитыЗаказаТК.Очистить();
	СтатусДПД = Перечисления.СтатусыЗаказовДПД.ПустаяСсылка();
	СтатусПеревозчика = "";
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//+++ 29.03.2018 - ВСЕГДА и для ВСЕХ клиентов по галке из карточки клиента! 
	Если ЭтоНовый() И Контрагент.ПечататьСпецификацию и Найти(ДоговорКонтрагента.Наименование,"*")=0  //04.04.2018 - только для Б/нал
		и ДоговорКонтрагента.ТипДоговора<>Справочники.ТипыДоговоров.ОтсрочкаОТХ и ДоговорКонтрагента.ТипДоговора<>Справочники.ТипыДоговоров.ПеремещениеОТХ  //22.02.19 Смирнов - ОТХ тоже не надо
		тогда
		ВыписатьСпецификацию = Истина;
		#Если Клиент тогда
			сообщить("Автоматически включена [v] Выписывать Спецификацию", СтатусСообщения.Информация);
		#КонецЕсли	
	КонецЕсли;	
...
//изменение номера при переносе документа в Новый год
//+++ 28.06 -28.12.2018 - X - блокирока!  ТОЛЬКО на Январь! нужно !!!
	Если не ЭтоНовый() и Год(ссылка.Дата)<> Год(Дата) тогда
		Номер = "";
	...
	Если ((ВводитьТолькоПеремещение = Истина 
		...
		ИЛИ (ДоговорКонтрагента.ТипДоговора = Справочники.ТипыДоговоров.ПеремещениеОТХ И НЕ Контрагент = Справочники.Контрагенты.НайтиПоКоду("П017979")))  //*** с 23.08.2018 не только заказы на филиалы, но и перемещения на отх (задача 52068) исключение - Скрипченко
		и ТипЗаказа <> 13 
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение 
		и мПроведениеИзФормы = ЛОЖЬ Тогда	 
	...
	#Если Клиент Тогда
		...
		Если Контрагент = Контрагент.КонтрагентДляРезерваИМ //+++ 19.07.2018 - сначала проверяем Клиента... крайне редко такое бывает!  
			//или Контрагент = ДоговорКонтрагента.КонтрагентДляРезерваИМ
			Тогда
			
			Если РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения тогда //19.07.2018  ссылка.Проведен  - избыточная проверка и X-блокирока!
	...
	Если Услуги.Количество() > 0  И ДоговорКонтрагента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомиссионером Тогда
		Услуги.Очистить();
	КонецЕсли;
	//Проверим некомплекты по литым не отечественным
	Если не РазрешитьНекомплект тогда
	проверитьНекомплекты();
	конецЕсли;
	...
	Если ТипЗаказа = 12 Тогда
		Если ЕстьНетРекламнаяПродукция(Товары, ЛОЖЬ) тогда
		Отказ = Истина;
			#Если Клиент Тогда
	...
	ИначеЕсли ВводитьТолькоПеремещение = Ложь Тогда
		Если ЕстьНетРекламнаяПродукция(Товары, Истина) тогда
		Отказ = Истина;
			#Если Клиент Тогда
	...
	// --------------- 19.12.2016 -----------------------------------------------------
	//Если константы.ИспользоватьДележкуЗаказовВМинус.Получить() тогда    // ДА!
	...
	флБылиОтгрузки=неопределено; //+++ 28.06.2018 - здесь уже определяется, но всегда!
	...
	Если НЕ (  (ЭтоНовый()                  // --- новые заказы НЕ проверяются и НЕ Согласуются! --- 
		...
		или Контрагент.НеКонтроллироватьЦеныПродажи  //+++ VIP клиенты.... любая (ручная) цена
		или (ЭтотОбъект.БонусПрименен И ЭтотОбъект.БонусПрименен <> Ссылка.БонусПрименен)    //***2018.06.21 после применения бонуса за * цены не согласовываются, т.к. эта скидка при проведении реализации относится на затраты 
		) Тогда
	...
	ЗаполнитьКодТНВЭД(Товары); //+++ 04.06.2018 - 0,5 сек
	
	//Адиянов>>>
	
	//+++ 08.09.2016 - Ответственный = Менеджер из Договора! по задаче от Лапенкова №16925
	//Если ЭтоНовый() Тогда //03.04.2018
	Если этотОбъект.ДоговорКонтрагента.ОтветственноеЛицо <> этотОбъект.Ответственный Тогда
		этотОбъект.Ответственный = этотОбъект.ДоговорКонтрагента.ОтветственноеЛицо; //МЕНЯЕТСЯ ВСЕГДА по договору!
	КонецЕсли;	
	//КонецЕсли;	
		
	Если Константы.ИспользоватьПодаркиЗаНаборы.Получить() и Бонусы.Количество() = 0 Тогда
		Если (ЗначениеНеЗаполнено(ДоговорКонтрагента.КонтрагентДляРезерваИМ) или ДоговорКонтрагента.ОтветственноеЛицо.НаправлениеПродаж = Справочники.НаправленияПродаж.НайтиПоКоду("16")) 
			   и (ЭтоНовый() или РежимПроведения = РежимПроведенияДокумента.Оперативный) 
			   и Контрагент <> Справочники.Контрагенты.НайтиПоКоду("36092")  //Яршинторг ТК
			   и Контрагент <> Справочники.Контрагенты.НайтиПоКоду("П004703") //резерв для им
			   и Контрагент <> Справочники.Контрагенты.НайтиПоКоду("00367") //(Д)Автоснаб
			   //01.02.19 Смирнов
			   и Контрагент.КатегорияПокупателя <> Справочники.КатегорииПокупателей.ИнтернетМагазин
			   Тогда
			Если ДоговорКонтрагента.ОтветственноеЛицо.НаправлениеПродаж <> Справочники.НаправленияПродаж.НайтиПоКоду("16") и ПроверитьНаличиеАксессуаров() тогда
			...
			ИначеЕсли ДоговорКонтрагента.ОтветственноеЛицо.НаправлениеПродаж = Справочники.НаправленияПродаж.НайтиПоКоду("16") тогда //Колеса тут
				Если ТипЗаказа = 10 или ТипЗаказа = 0 тогда //новый
					Для Индекс = -Бонусы.Количество() По -1 Цикл          //чистим от бонусов, хотя их и так нет
						уд = Бонусы.Получить(-Индекс-1);
						Если уд.Бонус Тогда
							Бонусы.Удалить(-Индекс-1);
						КонецЕсли;
				...
				Иначе
					Для Индекс = -Бонусы.Количество() По -1 Цикл          //чистим от бонусов
						...
						КонецЕсли;
					КонецЦикла;
					ЗаполнитьТабЧастьБонусы(истина);    //бонусы подгружаем из исходных заказов
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	//26.03.2019 Синхронизация ДатыОплаты и Сроков оплаты
	N = СрокиОплаты.Количество();
	Если N=0 тогда
		Если ДатаОплаты<>Дата(1,1,1) тогда
			стр = СрокиОплаты.Добавить();
			стр.ДатаОплаты = ДатаОплаты;
			стр.Процент    = 100;
		Конецесли;
	Иначе 	//+++ 27.03.2019 - bugFix:  N=0 при заказе без даты оплаты!
		СрокиОплаты.Сортировать("ДатаОплаты");
		Если ДатаОплаты <> СрокиОплаты[N-1].ДатаОплаты тогда
			СрокиОплаты[N-1].ДатаОплаты = ДатаОплаты;
			Для каждого ст из СрокиОплаты Цикл
				Если ст.ДатаОплаты> ДатаОплаты тогда
		         ст.ДатаОплаты = ДатаОплаты;
			    конецЕсли;
		    конецЦикла;
		    СрокиОплаты.Свернуть("ДатаОплаты","Процент,Сумма");
		КонецЕсли;	
		процОТХ = ?(ДоговорКонтрагента.ТипДоговора = Справочники.ТипыДоговоров.ПеремещениеОТХ, ПроцентОТХ, 0); 
		Если СрокиОплаты.Итог("Процент")<> 100-процОТХ тогда
			#Если Клиент Тогда
			СообщитьОбОшибке("Сроки оплаты распределены не правильно!!! Общая сумма процентов на равна 100%"
				        		+?(процОТХ=0,"", "(Процент ОТХ: "+строка(процОТХ)+"%)") //+++ чтобы понять почему не 100%
							);
			#КонецЕсли
			Отказ=Истина;
		КонецЕсли;
...
процедура проверитьНекомплекты()
ТабТ= товары.ВыгрузитьКолонки("Номенклатура,количество");

	

конецПроцедуры

//одним запросом а не N строк!
процедура ЗаполнитьКодТНВЭД(Товары)
Запрос = Новый Запрос;
табл1 = Товары.Выгрузить(,"НомерСтроки, Номенклатура, КодТНВЭД");
Запрос.УстановитьПараметр("табл1", табл1);
Запрос.Текст = "ВЫБРАТЬ
               |	тов.НомерСтроки,
               |	тов.Номенклатура,
               ...
               |	Тов.Номенклатура,
               |	ВЫБОР
               |		КОГДА Тов.КодТНВЭД = ЗНАЧЕНИЕ(Справочник.КлассификаторТНВЭД.ПустаяСсылка)
               |			ТОГДА спрНоменклатура.КодТНВЭД
               |		ИНАЧЕ Тов.КодТНВЭД
               |	КОНЕЦ КАК КодТНВЭД
               |ИЗ
               |	ВТ_Тов КАК Тов
               ...
               |		ИЗ Справочник.Номенклатура КАК спр
               |		ГДЕ
               |			спр.Ссылка В(ВЫБРАТЬ ВТ.Номенклатура
               |					ИЗ ВТ_Тов КАК ВТ) ) КАК спрНоменклатура
               |		ПО Тов.Номенклатура = спрНоменклатура.Ссылка1
               |
               |УПОРЯДОЧИТЬ ПО
               |	НомерСтроки";
Результат = Запрос.Выполнить();
Выборка = Результат.Выбрать();
	Для Каждого Стр Из Товары Цикл // номер строки всегда отсортирован!
		Выборка.следующий();
		Стр.КодТНВЭД = Выборка.КодТНВЭД;
	КонецЦикла;	
	
КонецПроцедуры

//+++ 04.06.2018 - один запрос к базе, а на N строк к товарам!
функция ЕстьНетРекламнаяПродукция(Товары, Есть=Истина)
рез = ложь;
 Запрос = Новый Запрос;
 Запрос.Текст = "выбрать
 |тов.ссылка
 |из справочник.Номенклатура как тов
 |ГДЕ тов.Ссылка В (&масТов)
 |	и тов.ВидТовара "+?(Есть,"=","<>")+" Значение(Перечисление.ВидыТоваров.РекламнаяПродукция)";
 масТов= Товары.ВыгрузитьКолонку("Номенклатура");
 Запрос.УстановитьПараметр("масТов", масТов);
 Результат = Запрос.Выполнить();
...
процедура ЗаполнитьТаблСогласования()
	...
	//+++ 10.08.2018 - при включении бонуса - не надо отправлять на согласование!
	Если этотОбъект.БонусПрименен и этотОбъект.ПроцентСкидкиНал<>0 тогда
		КоэфБонусНал = (1 - этотОбъект.ПроцентСкидкиНал/100);
	иначе
	    КоэфБонусНал = 1;
	КонецЕсли;
	...
	Для каждого стрОб из ЭтотОбъект.Товары Цикл
		...
		ИначеЕсли мПроведениеИзФормы Тогда
			...
			//ПРИ уменьшении цены ниже Бонуса?! для строк, которые ещё на согласовании - меняем задачу на Согласование?!
			Если стрОб.Цена/КоэфБонусНал < стрСсылка.Цена 
				...
				#Если Клиент Тогда
					сообщить(строка(стрОб.НомерСтроки)+")  для товара "+стрОб.Номенклатура.Код+" цена: "+формат(стрСсылка.Цена,"ЧДЦ=2")+"р. УЖЕ на согласовании! и Ещё УМЕНЬШЕНА до: "+формат(стрОб.Цена/КоэфБонусНал,"ЧДЦ=2")+"р. Требуется Повторное согласование!", СтатусСообщения.Внимание);
				#КонецЕсли	
			ИначеЕсли стрОб.Цена/КоэфБонусНал < стрСсылка.Цена //+++ 10.08.2018 - цена уменьшена!
				и стрОб.Статус <> перечисления.СтатусыСтрокЗаказа.ПустаяСсылка() //	Подтвержден, Согласован, Отменен...   //+++ 05.12.2018 по задаче № 60274 - любой другой статус!
				Тогда
				ЭтотОбъект.Согласован = ложь; //только Для согласования
				стрОб.Статус = перечисления.СтатусыСтрокЗаказа.НаСогласовании;
				#Если Клиент Тогда
					сообщить(строка(стрОб.НомерСтроки)+") для товара "+стрОб.Номенклатура.Код+" цена: "+формат(стрСсылка.Цена,"ЧДЦ=2")+"р. УМЕНЬШЕНА до: "+формат(стрОб.Цена/КоэфБонусНал,"ЧДЦ=2")+"р. Требуется Новое согласования!", СтатусСообщения.Внимание);
				#КонецЕсли	
			КонецЕсли
...
//+++( 14.08.2017 - 14.03.2018 предоплата по счетам - не считается?!
//	догПредоплаты = (ДоговорКонтрагента.ТипДоговора.Код="00001" или ДоговорКонтрагента.ТипДоговора.Код="00005") ;
  догПредоплаты = (ДоговорКонтрагента.ТипДоговора = справочники.ТипыДоговоров.НайтиПоКоду("00001") 
  			   или ДоговорКонтрагента.ТипДоговора = справочники.ТипыДоговоров.ПредоплатаПоСчетам //+++14.03.2018 
		       или ДоговорКонтрагента.ТипДоговора = справочники.ТипыДоговоров.ФакторингПредоплата //  16.05.2017
                   ); 
//+++)
...
//10.08.2018 - бонусы?! отменяют мин.цену = B2B
Если КоэфБонусНал<>1 тогда
	//+++( 16.04.2018 --- цены B2B ---------
	Если Найти(ДоговорКонтрагента.Наименование,"*")>0 
		и ДоговорКонтрагента.ТипДоговора = Справочники.ТипыДоговоров.НайтиПоКоду("00001") тогда  //+++ 22.05.2018 - только Предоплата!
		спис1 = товары.ВыгрузитьКолонку("Номенклатура");
		таблB2B = ОбменСУТИнтернетМагазин.ПолучитьЦеныB2B(спис1);
		Если таблB2B.Количество()>0 тогда
			изменитьМинЦенуНаB2B(таблТовары, таблB2B); // прямо в мин. цену!
		КонецЕсли;	
	КонецЕсли;
	//+++)
КонецЕсли;
	...
	Для каждого стр1 из таблТовары Цикл
		...
		Если ценыПоПравилам
			и Стр1.Цена >= стр1.ЦенаОбщ Тогда  // для B2B тоже не согласовывается!
			продолжить;
		КонецЕсли;
		
		//+++( 10.08.2018 с учётом бонуса "уменьшаем" мин.цены! и копейка вниз
		стр1.ЦенаМин = Цел(стр1.ЦенаМин * КоэфБонусНал);      //VIP
		стр1.ЦенаМинРук = Цел(стр1.ЦенаМинРук * КоэфБонусНал);//1
		//+++)
		...
		Если Стр1.Цена < стр1.ЦенаМин 
			...
			Если Стр1.Статус<>перечисления.СтатусыСтрокЗаказа.Подтвержден Тогда //уже подтверждена низкая цена!
					...
					Иначе 
						...
						Если ЗначениеНеЗаполнено(менТГ) Тогда
							...
							Если текСтр.ВидТовара = перечисления.ВидыТоваров.Диски Тогда
								менТГ = справочники.НоменклатурныеГруппы.НайтиПоКоду("00026").Ответственный; // литые - "Малышев Егор";
							ИначеЕсли текСтр.ВидТовара = перечисления.ВидыТоваров.АКБ Тогда
								менТГ =  справочники.НоменклатурныеГруппы.НайтиПоКоду("00060").Ответственный; //Голубева В.С.");
							ИначеЕсли текСтр.ВидТовара = перечисления.ВидыТоваров.Аксессуары Тогда
								менТГ = справочники.НоменклатурныеГруппы.НайтиПоКоду("00073").Ответственный; //Голубева В.С.";
							ИначеЕсли текСтр.ВидТовара = перечисления.ВидыТоваров.Шины Тогда //все шины - Горохову
								...
								ИначеЕсли СокрЛП(текСтр.Производитель.Код)=39
									...
									менТГ = справочники.Пользователи.НайтиПоКоду("Коширенков");
				...
				СтрЗаказа.СуммаНДС = РассчитатьСуммуНДС(СтрЗаказа.Сумма, УчитыватьНДС, СуммаВключаетНДС, ПолучитьСтавкуНДС(СтрЗаказа.СтавкаНДС));
			КонецЕсли;
...
процедура ЗаполнитьМинЦены( таблТовары, ЭтоПредоплата=ложь, ЦеныПоПравилам=ложь)
	...
	Для jj=1 по N Цикл
		...
		ИначеЕсли массТов.Количество()>0 Тогда
			...
			//12.12.2016 - ВОЗВРАТА НЕ БУДЕТ!    ЦенообразованиеПоКатегориям = ДА ВСЕГДА!
	...
	Если НЕ типЦенМин.Пустая()
		и Контрагент.НеКонтроллироватьЦеныПродажи=ЛОЖЬ
		Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ
		|	тов.Номенклатура,
		...
		Если НЕ Результат.Пустой() тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				Если ЭтотОбъект.УчитыватьНДС тогда //+++ 28.12.2018 - универсальная!
					стр1.СтавкаНДС= ставкаНДС_2019;
				 	стр1.СуммаНДС = РассчитатьСуммуНДС(стр1.Сумма, УчитыватьНДС, СуммаВключаетНДС, чНДС);
				Иначе
					стр1.СтавкаНДС = перечисления.СтавкиНДС.БезНДС; // 0%
					стр1.СуммаНДС  = 0;
				КонецЕсли;	
...
//16.04.2018
Процедура изменитьМинЦенуНаB2B(табл1, табл2)
	
	процB2B = -3; //до -3% не нужно согласование!
	
	Для каждого стр1 из табл1 цикл
		Для каждого стр2 из табл2 цикл
			
			ценаB2B = Цел( стр2.Цена * (1 + процB2B/100) ); // по задаче № 44778 - согласование на Доколина B2B  -3% (+ уменьшение вниз!)
			 
			Если стр1.Номенклатура = стр2.Номенклатура
				и стр1.ЦенаОбщ>ценаB2B тогда   //более высокий порог - снижается и НЕ согласовывается!
				стр1.ЦенаОбщ = ценаB2B;
				стр1.ЦенаМин = ?(стр1.ЦенаМин<ценаB2B, ценаB2B, стр1.ЦенаМин);
				стр1.Приоритет = "3 000"; //2000 - это кр.опт, +3000 - B2B
			ИначеЕсли стр1.Номенклатура = стр2.Номенклатура
				и ( стр1.ЦенаМин<ценаB2B) тогда
				#Если Клиент тогда
					сообщить(">>>> Цена товара "+строка(стр1.Номенклатура.Код)+" "+строка(стр1.Номенклатура)+" - не может быть меньше цены B2B ("+строка(процB2B)+"%): "+строка(ценаB2B)+"р.");
				#КонецЕсли	
				стр1.ЦенаМин = ценаB2B;
				стр1.Приоритет = "3000"; //2000 - это кр.опт, +3000 - B2B
			КонецЕсли;
...
процедура ЗаполнитьЗадачиНаСогласование() экспорт
	...
	Если ТипЗаказа = 12 и СогласовыватьРекламу и Проверен Тогда   //Реклама
		...
		Голубева = Справочники.Пользователи.НайтиПоКоду("Голубева В.С.");
		СписокОтветственных = Новый СписокЗначений;
		СписокОтветственных.Добавить(Лаврова);
		СписокОтветственных.Добавить(Голубева);  //***Задача № 45248
		...
		Для каждого ОтветственноеЛицо ИЗ СписокОтветственных Цикл
			
			Задача = Задачи.ЗадачиПользователя.СоздатьЗадачу();
			Задача.Дата				 = ТекущаяДата();
			Задача.Постановщик		 = ОтветственноеЛицо.Значение;
			Задача.Наименование		 = "Согласование РЕКЛАМНОЙ продукции";
			Задача.Исполнитель		 = ОтветственноеЛицо.Значение;
			...
			Если Задача.Товары.Количество() > 0 Тогда
				Задача.Записать(); 
				#Если Клиент Тогда
					Сообщить("Создана заявка на согласование Для пользователя: "+строка(ОтветственноеЛицо.Значение)+". Только после согласования заказ будет [v]""К отгрузке"".");
				#КонецЕсли
			КонецЕсли;	
	...
	//ЭтоПредоплата = (ЭтотОбъект.ДоговорКонтрагента.ТипДоговора.Код="00001" или ЭтотОбъект.ДоговорКонтрагента.ТипДоговора.Код="00005"); //16.05.2017
	  ЭтоПредоплата = (ДоговорКонтрагента.ТипДоговора = справочники.ТипыДоговоров.НайтиПоКоду("00001") 
		...
		ТемаЗадачи = "Утвердите цены "+?(ЭтоПредоплата," ПО ПРЕДОПЛАТЕ","")+?(Найти(ДоговорКонтрагента.Наименование,"*")>0," - ПО ДОГОВОРУ ( * )","") //12.04.2018 по задаче № 44237
	    +?(этотОбъект.БонусПрименен и этотОбъект.ПроцентСкидкиНал<>0, " БОНУС "+строка(этотОбъект.ПроцентСкидкиНал)+"%", "") //+++10.08.2018
	  	+" по Заказу "+строка(ссылка.Номер)+" покупателя: "+строка(ссылка.Контрагент);
		...
		Для каждого стрСогл из 	таблСогл1 Цикл
			...
			//10.04.2019 - единая функция для создания Задач на согласование
			СоздатьЗадачуНаСогласование(стрСогл.ПользовательУтверждающий, таблСогл, ТемаЗадачи, ЭтоПредоплата);
...
функция НайтиЗадачуНаСогласованиеПоЗаказу(Исполнитель, Тема="")
	рез = Неопределено;
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ 
	...
	|
	| ГДЕ ЗадачиПользователяЗадачиПоИсполнителю.Наименование = &Тема
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Запрос.УстановитьПараметр("Ссылка", Ссылка );
	Запрос.УстановитьПараметр("Тема", ?(стрДлина(Тема)>100, лев(Тема,100), Тема) ); // в задаче больше НЕ бывает!
...
функция ПолучитьТаблСкладов()
	...
	//26-27.06.2018  - через ВТ - дольше!?
	|	и Склад.ЗапретитьИспользование = ЛОЖЬ
	|	И Склад.Транзитный "+?(Подразделение.ОбособленноеПодразделение, " И Склад.Подразделение = &Подразделение", " = ЛОЖЬ")+"
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//+++( 28.06.2018 - остатки проверяем ТОЛЬКО если заказ Проверен т.е. [v] К Отгрузке и в "нормальном" режиме...
	флПроверятьОстатки = ЭтотОбъект.Проверен И НЕ ЭтотОбъект.ОбменДанными.Загрузка;
	Если флПроверятьОстатки и НЕ ЭтоНовый() тогда 
		//По Задаче № 36008 -> не проверять остатки по отруженым и закрытым заказам
		//флПроверятьОстатки = НЕ ЗаказЗакрытИлиОтгруженПолностью(Ссылка, ЛОЖЬ); // частично-отгруженный заказ - НАДО проверять остатки?!
	//+++ 28.06.2018 - передЗаписью() УЖЕ всё проверили в модуле!
		Если флБылиОтгрузки=неопределено тогда
			флПроверятьОстатки =  НЕ яштПоЗаказуБылиОтгрузки(Ссылка);  
		иначе
			флПроверятьОстатки =  НЕ флБылиОтгрузки;
		КонецЕсли;	
	КонецЕсли;	
	//флаг контролирует ВСЕ проверки и уведомления на клиента! При включении Проверен
	//+++)
	

//+++( 16-17.07.2018 проверка св.остатка надо делать ПЕРЕД блокировкой - с включенной U-блокировкой ДЛЯ ИЗМЕНЕНИЯ в запросе!
//U-блокировка: будет установлена управляемая блокировка для всех комбинаций измерений в соответствии с записями набора записей. 
	//не мешает чтению, но мешает записи UхU с такими же измерениями! ОДНАКО движений тут НЕТ!
	Движения.ЗаказыПокупателей.БлокироватьДляИзменения = Истина;  
	Если флПроверятьОстатки 
		 и не Отказ и не ЭтотОбъект.ОбменДанными.Загрузка 
	Тогда  
		ТаблПоставщиков = неопределено;
		ПроверитьРезервы(ЭтотОбъект, Отказ, ТаблПоставщиков, ИСТИНА); // U-блокировкой других заказов в запросе! (4-ый параметр!)
	КонецЕсли;
...
//+++)
	...
	//26.06.2018  - X-блокировка ОБЯЗАТЕЛЬНА!
...
//+++ 16.07.2018 - нет движений по Товарам и ОТХ - блокировать здесь ИЗБЫТОЧНО !	
	...
	Если таблТоварСкладОТХ.Количество()>0 тогда
...
//+++)
	...
	//!!! проверки - предупреждения на Клиенте !!! =================
...
//01.09.2016 - надо проверять ТОЛЬКО на Клиенте... Предупреждение на Сервере всё равно никто не смотрит!		
	...
	#Если Клиент Тогда
			...
			Отказ1 = Отказ; //+++ 28.06.2018
		 
			//из-за этих проверок - возникает ошибка блокировки данных (20 сек ожидание!)
			//РОБОТЫ НИКАКИХ Уведомлений не делают! некуда их показывать!
			//28.06.2018 - как и остатки - уведомления только при [v] К отгрузке  и если заказ ещё не отгружен...
			Если Найти(сокрЛП(ПараметрыСеанса.ТекущийПользователь.код),"Робот (")=0  и НЕ флПроверятьОстатки Тогда  
			
				Если не Отказ1 Тогда  
					ПроверитьПревышениеОбщегоЛимитаДЗСУчетомЗаказа(ЭтотОбъект, Отказ1);
				КонецЕсли;
				
				Если не Отказ1 Тогда  //+++ 28.06.2018 - как и остатки 
				...
				КонецЕсли;
			 КонецЕсли;//на роботе или когда не надо проверять остатки - тут тоже ничего не проверяется!
			 
			 
//+++ 28.06.2018 - ВЫКЛЮЧЕН рудимент - только для АКБ и только по 1 производителю: Исток проверяется для всех заказов??? +++++			
...
//+++ 28.06.2018 - убрано навсегда, работает другая схема!	
			...
			Если ТаблПоставщиков<>неопределено Тогда
				...
				Если ТаблПоставщиков.Количество()>0  // Есть товары под заказ (стороннему поставщику)
					...
					//+++ поставщик должен быть обязательно!
						...
						Если ТаблПоставщиков[0].АвтоПодтверждение тогда //+++ 07.08.2018 Задача № 52100 --- автоматически сразу в Ожидание!
					...
					Если ЭтотОбъект.Статус = перечисления.Статусызаказов.Подтвержден и ЭтотОбъект.Проверен тогда//+++ 17.07.2017
						...
						КонецЦикла;
					КонецЕсли;// Подтвержден поставщиком и [v]Проверен
					
					//+++ 07.08.2018 - не создаём заказа поставщику, если [v]АвтоПодтверждение (без поставщика подтверждаем!)
					//, а в "размещение" пишем Склад чтобы строка "светилась" голубым
					Если ТаблПоставщиков[0].АвтоПодтверждение и Статус = перечисления.СтатусыЗаказов.Подтвержден Тогда
						
						//что-то надо написать в размещение...
						Для каждого стр1 из ТаблПоставщиков цикл
							стр2 = Товары.Найти(стр1.Номенклатура,"Номенклатура");
							Размещение0 = стр2.Размещение;
							Размещение  = справочники.Склады.НайтиПоКоду("02332");
							Если Размещение0<>Размещение тогда
						 		стр2.Размещение = Размещение; //Ангар-Ремонт Тест (запрещенный!)
								ДатаОтгрузки0 = ДатаОтгрузки + 86400;
							КонецЕсли;	
						КонецЦикла;
				...
				////-----------СОЗДАЁМ Заказы поставщикам при "включении" подтверждения и если нет АвтоПодтверждения--------------------
					нетЗаказов = истина;
					...
					Если  нетЗаказов И Статус = перечисления.СтатусыЗаказов.Подтвержден Тогда //19.12.2016 если подтвержден поставщиком и есть остатки сторонних поставщиков....
						...
						Для каждого пост1 из табл1 Цикл  //по каждому поставщику - своя задача! только на его товар!
							...
							Если Контрагент.Экспорт //и ДоговорКонтрагента.ВалютаВзаиморасчетов = справочники.Валюты.НайтиПоКоду("643")
								...
								тогда ставкаНДС_2019 = перечисления.СтавкиНДС.НДС0; 
							КонецЕсли;
 							чНДС = ПолучитьСтавкуНДС(ставкаНДС_2019); //18 или 20 или 0%
							...
							Для каждого стр2 из закПост.Товары Цикл
								...
								Если ЭтотОбъект.УчитыватьНДС тогда //+++ 28.12.2018 - универсальная!
									стр2.СтавкаНДС= ставкаНДС_2019;
				 					стр2.СуммаНДС = РассчитатьСуммуНДС(стр2.Сумма, УчитыватьНДС, СуммаВключаетНДС, чНДС);
								Иначе
									стр2.СтавкаНДС = перечисления.СтавкиНДС.БезНДС; // 0%
									стр2.СуммаНДС  = 0;
								КонецЕсли;	
					//			стр2.ЗаказПокупателя = ссылка; // ссылка на этот заказ покупателя (делает движение в "Размещение заказов")
							...
							попытка 
								...
								Если закПост.Контрагент = справочники.Контрагенты.НайтиПоКоду("00379") тогда //+++ 18.08.2017 - по задаче "Авторусь терминал"
									закПост.Записать();
								Иначе	
	...
	//+++ 08-09.04.2019 - [ ]Проверен - НЕ делаем движений task/1745
	//11.04.2019 Смирнов, решили откатить назад, слишком много исключений
	//Если Проверен 
	//	или НЕ Поставщик.Пустая() 
	//	или ДоговорКонтрагента.ТипДоговора = справочники.ТипыДоговоров.ОтсрочкаОТХ
	//	или ДоговорКонтрагента.ТипДоговора = справочники.ТипыДоговоров.ПеремещениеОТХ
	//	или ОтгрузкаСПодразделенияСклада     // надо для отгрузок через Филиал
	//	или Подразделение.ОбособленноеПодразделение //10.04.2019 филиальные заказы "собираются" и Объединятся ДО прихода товара....
	//Тогда 
		ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоУслугам, 
		ТаблицаПоТаре, Отказ, Заголовок);
	//Иначе
	//	#Если Клиент тогда
	// 		сообщить("Движения по заказу "+ЭтотОбъект.Номер+" - выключены, так как флаг [ ] В Резерв (Проверен) - выключен!");
	//	#КонецЕсли	
	//КонецЕсли;
	...
	//{Миронычев
	Если Не Отказ Тогда 
		Если не ДоговорКонтрагента.ТипДоговора = Справочники.ТипыДоговоров.ПеремещениеОТХ тогда //29.03.19 Смирнов для ОТХ не надо контролировать
			Если ТипЗаказа = 10 или TerminalОтгрузкаТранспортнойКомпанией Тогда 
				//28.03.19 Смирнов что б не перезаписывало вышестоящие статусы
				Если ЗаписыватьСтатусСоздан(ЭтотОбъект.Ссылка) тогда
...
Функция ЗаписыватьСтатусСоздан(ЗаказПервичный) экспорт
	Запрос = новый Запрос;
	Запрос.Текст="ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("ЗаказПервичный",ЗаказПервичный);
...
функция создатьЗаказСтороннемуПоставщику(НовЛайн)
	...
	Если НЕ регСвСроки.ОдинЗаказПоставщикуВДень или ЗакПост = неопределено Тогда   //	====================
		...
		ЗакПост.Ответственный = справочники.Пользователи.НайтиПоКоду("Робот (магазин)");
	...
	#Если Клиент Тогда
		Если НЕ РольДоступна("ПолныеПрава") Тогда // бухгалтерия или Серков/Горохов может!!!
			...
			Если НЕ ( Контрагент.ОсновнойМенеджерКонтрагента = ПараметрыСеанса.ТекущийПользователь 
				...
				//+++Шарафутдинов по задаче 47665 
				Если ПолучитьЗначениеПоУмолчанию(ПараметрыСеанса.ТекущийПользователь, "ЖурналыОтборПоРегиону")  тогда
					СЗКонтр = ПолучитьСписокКонтрагентовМенеджераПоРегиону(ПараметрыСеанса.ТекущийПользователь);
					Если СЗКонтр.НайтиПоЗначению(ДоговорКонтрагента.Владелец) = Неопределено тогда  //не нашли среди разрешенных
						Сообщить("Вы не имеете права отменить проведение чужого заказа! Не Ваш контрагент!
...
Процедура ПриЗаписи(Отказ)
	...
	Если не Отказ Тогда
		...
		Запрос1.УстановитьПараметр("Владелец", Контрагент);
		Результат = Запрос1.Выполнить();
		Если не Результат.Пустой() Тогда //ДЛЯ ВСЕХ КЛИЕНТОВ STORE и TERMINAL
			Попытка
				//18.07.2018 - в МИМ заказы не выгружаем уже давно!
				ОбменСУТИнтернетМагазин.ВключитьРегистрациюОбъектаОИМ(Ссылка, ЛОЖЬ ); //  по объекту
...
Процедура ПроверкаНаУстновку13Статуса()
	...
	|			И Вт_Товары.НоменклатурнаяГруппа = ЗапрещенныеТипоРазмеры.НоменклатурнаяГруппа
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	ТЗ.Колонки.Добавить("НоменклатурнаяГруппа",Новый ОписаниеТипов("СправочникСсылка.НоменклатурныеГруппы"));

	Для Каждого Стр из Товары Цикл
		НовСтрТЗ = ТЗ.Добавить();
		ЗаполнитьЗначенияСвойств(НовСтрТЗ,Стр);
		НовСтрТЗ.Типоразмер = Стр.Номенклатура.Типоразмер;
		НовСтрТЗ.НоменклатурнаяГруппа = Стр.Номенклатура.НоменклатурнаяГруппа;
	КонецЦикла;
...
Процедура ЗаполнитьТабЧастьБонусы(БратьИзИсходныхЗаказов = ложь)
	
	ТоварыТЗ = Товары.Выгрузить();
	ТоварыТЗ.Свернуть("Номенклатура","Количество");
	ТоварыТЗ.Колонки.Добавить("Видтовара",Новый ОписаниеТипов("ПеречислениеСсылка.ВидыТоваров"));
	ТоварыТЗ.Колонки.Добавить("Родитель", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));   //24.05.2017
	ТоварыТЗ.Колонки.Добавить("ЗаказИМ", Новый ОписаниеТипов("Булево"));
	
	ЗаказИМ = ДоговорКонтрагента.ОтветственноеЛицо.НаправлениеПродаж = Справочники.НаправленияПродаж.НайтиПоКоду("16");
	
	для каждого стр12 из ТоварыТЗ Цикл
		стр12.ВидТовара = стр12.Номенклатура.ВидТовара;	
		стр12.Родитель = стр12.Номенклатура.Родитель;
		стр12.ЗаказИМ = ЗаказИМ;
	КонецЦикла;
	Если не БратьИзИсходныхЗаказов тогда
		//+++( 19.07.2018 - запрос разбит на 2 части для исключения чтения регистра Заказов покупателей... в 99,9% случаев
		Запрос1 = новый Запрос;
		Запрос1.Текст = "ВЫБРАТЬ
						|	ТоварыТЗ.Номенклатура,
						|	ТоварыТЗ.Количество,
						...
						|			И Заказ.ЗаказИМ = НаборыДляПодарков.ПодарокДляKolesatyt
						|ГДЕ
						|	НаборыДляПодарков.Подразделение = &Подразделение
						|	И НаборыДляПодарков.Номенклатура ССЫЛКА Справочник.Номенклатура
						|;
						|
						|////////////////////////////////////////////////////////////////////////////////
						|ВЫБРАТЬ
						...
						|	(ВЫРАЗИТЬ(Итог.Количество / Итог.КоличествоКомплект - 0.5 КАК ЧИСЛО(15, 0))) * Итог.КоличествоПодарка КАК НеобходимоПодарить
						|ИЗ
						|	Итог КАК Итог
						|ГДЕ
						...
						|////////////////////////////////////////////////////////////////////////////////
						|УНИЧТОЖИТЬ Итог";
		запрос1.УстановитьПараметр("ТоварыТЗ",ТоварыТЗ);
		запрос1.УстановитьПараметр("Акс",Перечисления.ВидыТоваров.Аксессуары);
		Запрос1.УстановитьПараметр("Подразделение",  Подразделение);
		таблПодарки = Запрос1.Выполнить().Выгрузить();  //Подарки
	Иначе
		таблПодарки = ПолучитьПодаркиИзИсходныхЗаказов();
	КонецЕсли;
	
	Если таблПодарки.Количество()>0 тогда 	
		
		Если Подразделение = Справочники.Подразделения.ПустаяСсылка() или Подразделение = Справочники.Подразделения.НайтиПоКоду("00005") тогда
			СкладПодразделения = ложь;
		Иначе
			СкладПодразделения = истина;
		КонецЕсли;
		...
		запрос.УстановитьПараметр("СкладПодразделения",СкладПодразделения);
		
		//таблПодарки = результат_Подарки.Выгрузить();
		запрос.УстановитьПараметр("таблПодарки",таблПодарки); //+++ 19.07.2018
		
		Запрос.Текст = "ВЫБРАТЬ
					   |	ТаблПодарки.Подарок,
					   |	ТаблПодарки.НеобходимоПодарить
					   |ПОМЕСТИТЬ Подарки
					   |ИЗ
					   |	&таблПодарки КАК ТаблПодарки
					   |;
					   |
					   |////////////////////////////////////////////////////////////////////////////////
					   |ВЫБРАТЬ
					   |	ЕСТЬNULL(ЗаказыПокупателейОстатки.Номенклатура, ТоварыНаСкладахОстатки.Номенклатура) КАК Номенклатура,
					   |	СУММА(ЕСТЬNULL(ТоварыНаСкладахОстатки.КоличествоОстаток, 0) - ЕСТЬNULL(ЗаказыПокупателейОстатки.КоличествоОстаток, 0)) КАК ОстатокПодарковНаСкладе
					   |ПОМЕСТИТЬ Остатки
					   |ИЗ
					   |	РегистрНакопления.ТоварыНаСкладах.Остатки(
					   |			,
					   |			Склад.ЗапретитьИспользование = ЛОЖЬ
					   |				И Склад.Транзитный = &СкладПодразделения
					   |				И ВЫБОР
					   |					КОГДА &СкладПодразделения
					   |						ТОГДА Склад.Подразделение = &Подразделение
					   |					ИНАЧЕ ИСТИНА
					   |				КОНЕЦ
					   |				И Номенклатура В
					   |					(ВЫБРАТЬ
					   |						Подарки.Подарок
					   |					ИЗ
					   |						Подарки)) КАК ТоварыНаСкладахОстатки
					   |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗаказыПокупателей.Остатки(
					   |				,
					   |				ЗаказПокупателя.Проверен
					   |					И ЗаказПокупателя.Транзит = &СкладПодразделения
					   |					И ВЫБОР
					   |						КОГДА &СкладПодразделения = ЛОЖЬ
					   |							ТОГДА ЗаказПокупателя.Подразделение = &Подразделение
					   |						ИНАЧЕ ИСТИНА
					   |					КОНЕЦ
					   |					И Номенклатура В
					   |						(ВЫБРАТЬ
					   |							Подарки.Подарок
					   |						ИЗ
					   |							Подарки)) КАК ЗаказыПокупателейОстатки
					   |		ПО ТоварыНаСкладахОстатки.Номенклатура = ЗаказыПокупателейОстатки.Номенклатура
					   |			И (ВЫБОР
					   |				КОГДА &СкладПодразделения
					   |					ТОГДА ТоварыНаСкладахОстатки.Склад.Подразделение = ЗаказыПокупателейОстатки.ЗаказПокупателя.Подразделение
					   |				ИНАЧЕ ИСТИНА
					   |			КОНЕЦ)
					   |
					   |СГРУППИРОВАТЬ ПО
					   |	ЕСТЬNULL(ЗаказыПокупателейОстатки.Номенклатура, ТоварыНаСкладахОстатки.Номенклатура)
					   |;
					   |
					   |////////////////////////////////////////////////////////////////////////////////
					   |ВЫБРАТЬ
					   ...
					   |			ТОГДА ЕСТЬNULL(Остатки.ОстатокПодарковНаСкладе, 0) + Подарки.НеобходимоПодарить
					   |		ИНАЧЕ ЕСТЬNULL(Остатки.ОстатокПодарковНаСкладе, 0)
					   |	КОНЕЦ КАК Остаток,
					   |	Подарки.НеобходимоПодарить
					   |ИЗ
					   |	Подарки КАК Подарки
		...
		Рез = Запрос.Выполнить(); //Подарки по св. остаткам
		таблРез = Рез.Выгрузить();
		
		Для каждого стр из таблРез Цикл  //!!!
			Если стр.НеобходимоПодарить <= стр.Остаток тогда
				Нов = Бонусы.Добавить();
				Нов.Номенклатура = стр.Подарок;
				Нов.Количество = стр.НеобходимоПодарить;
				Нов.СтатьяЗатрат = Справочники.СтатьиЗатрат.НайтиПоКоду("О0215");
				Нов.Бонус = истина;
			ИначеЕсли стр.Остаток > 0 Тогда
				Нов = Бонусы.Добавить();
				Нов.Номенклатура = стр.Подарок;
				Нов.Количество = стр.Остаток;
				Нов.СтатьяЗатрат = Справочники.СтатьиЗатрат.НайтиПоКоду("О0215");
				Нов.Бонус = истина;
			КонецЕсли;
		КонецЦикла;
	...
	//+++) Конец Подарки
	Если не БратьИзИсходныхЗаказов тогда
	//+++ 19.07.2018 =============Наборы для подарков - так же на 2 части ля исключения чтения регистра Заказов покупателей... в 99,9% случаев=================
	Запрос2 = Новый Запрос;
	запрос2.УстановитьПараметр("ТоварыТЗ",ТоварыТЗ);
	запрос2.УстановитьПараметр("Подразделение",Подразделение);
	запрос2.УстановитьПараметр("ПодарокДляKolesatyt",  ЗаказИМ);

	Запрос2.Текст="ВЫБРАТЬ
	              |	ТоварыТЗ.Номенклатура,
	              |	ТоварыТЗ.Количество,
	              |	ТоварыТЗ.ЗаказИМ
	              |ПОМЕСТИТЬ втЗаказВрем
	              |ИЗ
	              |	&ТоварыТЗ КАК ТоварыТЗ
	              |;
	              |
	              |////////////////////////////////////////////////////////////////////////////////
	              |ВЫБРАТЬ
	              |	втЗаказВрем.Номенклатура,
	              |	втЗаказВрем.Количество,
	              |	втЗаказВрем.ЗаказИМ,
	              |	НоменклатураСпр.НоменклатурнаяГруппа,
	              |	НоменклатураСпр.ВидТовара
	              |ПОМЕСТИТЬ втЗаказ
	              |ИЗ
	              |	втЗаказВрем КАК втЗаказВрем
	              |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК НоменклатураСпр
	              |		ПО втЗаказВрем.Номенклатура = НоменклатураСпр.Ссылка
	              |;
	              |
	              |////////////////////////////////////////////////////////////////////////////////
	              |ВЫБРАТЬ
	              |	НаборыПодарковПодарки.Ссылка КАК НаборДляПодарка,
	              |	НаборыПодарковПодарки.Номенклатура,
	              |	СУММА(НаборыПодарковПодарки.Количество * НаборыДляПодарков.Количество) КАК КоличествоДляПодарка,
	              |	НаборыДляПодарков.Подарок,
	              |	НаборыДляПодарков.КоличествоПодарка КАК КоличествоПодарка,
	              |	НаборыДляПодарков.ПодарокДляKolesatyt
	              |ПОМЕСТИТЬ втНаборыДляПодарка
	              |ИЗ
	              |	РегистрСведений.НаборыДляПодарков КАК НаборыДляПодарков
	              |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.НаборыПодарков.Подарки КАК НаборыПодарковПодарки
	              |		ПО НаборыДляПодарков.Номенклатура = НаборыПодарковПодарки.Ссылка
	              |ГДЕ
	              |	НаборыДляПодарков.Подразделение = &Подразделение
	              |	И НаборыДляПодарков.ПодарокДляKolesatyt = &ПодарокДляKolesatyt
	              |
	              |СГРУППИРОВАТЬ ПО
	              |	НаборыДляПодарков.Подарок,
	              |	НаборыПодарковПодарки.Ссылка,
	              |	НаборыПодарковПодарки.Номенклатура,
	              |	НаборыДляПодарков.ПодарокДляKolesatyt,
	              |	НаборыДляПодарков.КоличествоПодарка
	              |;
	              |
	              |////////////////////////////////////////////////////////////////////////////////
	              |ВЫБРАТЬ
	              |	втНаборыДляПодарка.НаборДляПодарка КАК НаборДляПодарка,
	              |	втНаборыДляПодарка.Номенклатура КАК Номенклатура,
	              |	СУММА(втНаборыДляПодарка.КоличествоПодарка) КАК КоличествоНоменклатурыПодарка,
	              |	СУММА(ЕСТЬNULL(втЗаказ.Количество, 0)) КАК КоличестовоВЗаказеДляПодарка,
	              |	МИНИМУМ(ВЫРАЗИТЬ(ЕСТЬNULL(втЗаказ.Количество, 0) / втНаборыДляПодарка.КоличествоДляПодарка - 0.5 КАК ЧИСЛО(15, 0))) КАК КоличествоПодарков,
	              |	втЗаказ.ЗаказИМ КАК ЗаказИМ
	              |ПОМЕСТИТЬ вт2
	              |ИЗ
	              |	втНаборыДляПодарка КАК втНаборыДляПодарка
	              |		ЛЕВОЕ СОЕДИНЕНИЕ втЗаказ КАК втЗаказ
	              |		ПО втНаборыДляПодарка.Номенклатура = втЗаказ.Номенклатура
	              |			И (втЗаказ.ЗаказИМ = втНаборыДляПодарка.ПодарокДляKolesatyt)
	              |			И втНаборыДляПодарка.КоличествоДляПодарка <= втЗаказ.Количество
	              |
	              |СГРУППИРОВАТЬ ПО
	              |	втНаборыДляПодарка.НаборДляПодарка,
	              |	втНаборыДляПодарка.Номенклатура,
	              |	втЗаказ.ЗаказИМ
	              |
	              |ОБЪЕДИНИТЬ ВСЕ
	              |
	              ...
	              |	втНаборыДляПодарка.НаборДляПодарка,
	              |	втНаборыДляПодарка.Номенклатура,
	              |	СУММА(втНаборыДляПодарка.КоличествоПодарка),
	              |	СУММА(ЕСТЬNULL(взНомГр.Количество, 0)),
	              |	МИНИМУМ(ВЫРАЗИТЬ(ЕСТЬNULL(взНомГр.Количество, 0) / втНаборыДляПодарка.КоличествоДляПодарка - 0.5 КАК ЧИСЛО(15, 0))),
	              |	взНомГр.ЗаказИМ
	              |ИЗ
	              |	втНаборыДляПодарка КАК втНаборыДляПодарка
	              |		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	              |			втЗаказ.НоменклатурнаяГруппа КАК НоменклатурнаяГруппа,
	              ...
	              |		
	              |		СГРУППИРОВАТЬ ПО
	              |			втЗаказ.НоменклатурнаяГруппа,
	              |			втЗаказ.ЗаказИМ) КАК взНомГр
	              |		ПО (взНомГр.ЗаказИМ = втНаборыДляПодарка.ПодарокДляKolesatyt)
	              |			И втНаборыДляПодарка.КоличествоДляПодарка <= взНомГр.Количество
	              |			И втНаборыДляПодарка.Номенклатура = взНомГр.НоменклатурнаяГруппа
	              |
	              |СГРУППИРОВАТЬ ПО
	              |	втНаборыДляПодарка.НаборДляПодарка,
	              |	втНаборыДляПодарка.Номенклатура,
	              ...
	              |	втНаборыДляПодарка.Номенклатура,
	              |	СУММА(втНаборыДляПодарка.КоличествоПодарка),
	              |	СУММА(ЕСТЬNULL(взВидТовара.Количество, 0)),
	              |	МИНИМУМ(ВЫРАЗИТЬ(ЕСТЬNULL(взВидТовара.Количество, 0) / втНаборыДляПодарка.КоличествоДляПодарка - 0.5 КАК ЧИСЛО(15, 0))),
	              |	взВидТовара.ЗаказИМ
	              |ИЗ
	              |	втНаборыДляПодарка КАК втНаборыДляПодарка
	              |		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	              |			СУММА(втЗаказ.Количество) КАК Количество,
	              |			втЗаказ.ЗаказИМ КАК ЗаказИМ,
	              |			втЗаказ.ВидТовара КАК ВидТовара
	              |		ИЗ
	              |			втЗаказ КАК втЗаказ
	              |		
	              |		СГРУППИРОВАТЬ ПО
	              |			втЗаказ.ЗаказИМ,
	              |			втЗаказ.ВидТовара) КАК взВидТовара
	              |		ПО (взВидТовара.ЗаказИМ = втНаборыДляПодарка.ПодарокДляKolesatyt)
	              |			И втНаборыДляПодарка.КоличествоДляПодарка <= взВидТовара.Количество
	              |			И втНаборыДляПодарка.Номенклатура = взВидТовара.ВидТовара
	              |
	              |СГРУППИРОВАТЬ ПО
	              |	втНаборыДляПодарка.НаборДляПодарка,
	              |	втНаборыДляПодарка.Номенклатура,
	              ...
	              |	вт2.НаборДляПодарка,
	              |	вт2.Номенклатура,
	              |	вт2.КоличествоНоменклатурыПодарка,
	              |	вт2.КоличестовоВЗаказеДляПодарка,
	              |	вт2.КоличествоПодарков,
	              |	вт2.ЗаказИМ
	              |ПОМЕСТИТЬ вт4
	              |ИЗ
	              |	вт2 КАК вт2
	              |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ вт3 КАК вт3
	              |		ПО вт2.НаборДляПодарка = вт3.НаборДляПодарка
	              |			И вт2.Номенклатура = вт3.Номенклатура
	              |ГДЕ
	              |	вт2.КоличестовоВЗаказеДляПодарка > 0
	              |;
	              |
	              |////////////////////////////////////////////////////////////////////////////////
	              |ВЫБРАТЬ РАЗЛИЧНЫЕ
	              |	втНаборыДляПодарка.НаборДляПодарка,
	              |	втНаборыДляПодарка.Номенклатура,
	              |	втНаборыДляПодарка.КоличествоДляПодарка,
	              |	втНаборыДляПодарка.Подарок,
	              |	втНаборыДляПодарка.КоличествоПодарка,
	              |	втНаборыДляПодарка.ПодарокДляKolesatyt,
	              |	ЕСТЬNULL(вт4.КоличествоПодарков, 0) КАК КоличествоПодарков
	              |ПОМЕСТИТЬ втОснова
	              |ИЗ
	              |	втНаборыДляПодарка КАК втНаборыДляПодарка
	              |		ЛЕВОЕ СОЕДИНЕНИЕ вт4 КАК вт4
	              |		ПО втНаборыДляПодарка.Номенклатура = вт4.Номенклатура
	              |			И втНаборыДляПодарка.НаборДляПодарка = вт4.НаборДляПодарка
	              |;
	              |
	              |////////////////////////////////////////////////////////////////////////////////
	              |ВЫБРАТЬ
	              |	втОснова.НаборДляПодарка
	              |ПОМЕСТИТЬ втНаборИсключить
	              |ИЗ
	              |	втОснова КАК втОснова
	              |ГДЕ
	              |	втОснова.КоличествоПодарков <= 0
	              |;
	              |
	              |////////////////////////////////////////////////////////////////////////////////
	              |ВЫБРАТЬ РАЗЛИЧНЫЕ
	              ...
	              |	втОснова.КоличествоДляПодарка,
	              |	втОснова.Подарок,
	              |	втОснова.КоличествоПодарка,
	              |	втОснова.ПодарокДляKolesatyt,
	              |	втОснова.КоличествоПодарков,
	              |	втОснова.КоличествоПодарков * втОснова.КоличествоПодарка КАК НеобходимоПодарить
	              |ПОМЕСТИТЬ вт
	              |ИЗ
	              |	втОснова КАК втОснова
	              |ГДЕ
	              |	НЕ втОснова.НаборДляПодарка В
	              |				(ВЫБРАТЬ
	              |					втНаборИсключить.НаборДляПодарка
	              |				ИЗ
	              |					втНаборИсключить)
	              |;
	              |
	              ...
	              |	вт.НаборДляПодарка,
	              |	вт.Подарок,
	              |	МИНИМУМ(вт.НеобходимоПодарить) КАК НеобходимоПодарить
	              |ПОМЕСТИТЬ втОснова2
	              |ИЗ
	              |	вт КАК вт
	              |
	              ...
	              |ВЫБРАТЬ РАЗЛИЧНЫЕ
	              |	втОснова2.Подарок,
	              |	СУММА(втОснова2.НеобходимоПодарить) КАК НеобходимоПодарить
	              |ИЗ
	...
	таблПодарки = Запрос2.Выполнить().Выгрузить();  //Подарки
	Иначе
		таблПодарки = ПолучитьПодаркиИзИсходныхЗаказов();
	КонецЕсли;
	
	Если таблПодарки.Количество()>0 тогда //только если есть подарки...
		
		//Эта ветка приоритетнее, если по ней положены бонусы, то очищаем предыдущие
		Бонусы.Очистить();
		
		Запрос = Новый Запрос;
		//таблПодаркиРассчитанные = Результат_ПодаркиРассчитанные.Выгрузить();   // предыдущий запрос!
		Запрос.УстановитьПараметр("таблПодаркиРассчитанные", таблПодарки);
		...
		Запрос.Текст="ВЫБРАТЬ РАЗЛИЧНЫЕ
					 |	ПодаркиРассчитанные.Подарок,
					 |	ПодаркиРассчитанные.НеобходимоПодарить
					 |ПОМЕСТИТЬ втПодаркиРассчитанные
					 |ИЗ
					 |	&таблПодаркиРассчитанные КАК ПодаркиРассчитанные
					 |;
					 |
					 |////////////////////////////////////////////////////////////////////////////////
					 |ВЫБРАТЬ
					 ...
					 |				И Номенклатура В
					 |					(ВЫБРАТЬ
					 |						втПодаркиРассчитанные.Подарок
					 |					ИЗ
					 |						втПодаркиРассчитанные)) КАК ТоварыНаСкладахОстатки
					 |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗаказыПокупателей.Остатки(
					 |				,
					 |				ЗаказПокупателя.Проверен
					 |					И ЗаказПокупателя.Транзит = &СкладПодразделения
					 ...
					 |							втПодаркиРассчитанные.Подарок
					 |						ИЗ
					 |							втПодаркиРассчитанные)) КАК ЗаказыПокупателейОстатки
					 |		ПО ТоварыНаСкладахОстатки.Номенклатура = ЗаказыПокупателейОстатки.Номенклатура
					 |			И (ВЫБОР
					 |				КОГДА &СкладПодразделения
					 |					ТОГДА ТоварыНаСкладахОстатки.Склад.Подразделение = ЗаказыПокупателейОстатки.ЗаказПокупателя.Подразделение
					 ...
					 |ВЫБРАТЬ
					 |	втПодаркиРассчитанные.Подарок,
					 |	втПодаркиРассчитанные.НеобходимоПодарить,
					 |	ЕСТЬNULL(Остатки.СвободныйОстаток, 0) КАК СвободныйОстаток
					 |ИЗ
					 |	втПодаркиРассчитанные КАК втПодаркиРассчитанные
					 |		ЛЕВОЕ СОЕДИНЕНИЕ Остатки КАК Остатки
					 |		ПО втПодаркиРассчитанные.Подарок = Остатки.Номенклатура";
		Рез = Запрос.Выполнить().Выгрузить(); // наборы для подарков
		
		Для каждого стр из Рез Цикл
			Если стр.СвободныйОстаток>0 тогда
				Нов = Бонусы.Добавить();
				Нов.Номенклатура = стр.Подарок;
				Нов.Количество = мин(стр.НеобходимоПодарить, стр.СвободныйОстаток);
				Нов.СтатьяЗатрат = Справочники.СтатьиЗатрат.НайтиПоКоду("О0215");
				Нов.Бонус = истина;
			КонецЕсли;
		КонецЦикла;
...
Процедура ОтправитьНаСогласованиеПодарки() Экспорт	
	...
	Для каждого стр22 из ПоВидамТоваров Цикл
		...
		Если стр22.ВидТовара = Перечисления.ВидыТоваров.Диски Тогда
			СписокОтветственных.Добавить(Справочники.Пользователи.НайтиПоКоду("Малышев Егор"));
		Иначеесли стр22.ВидТовара = Перечисления.ВидыТоваров.Шины Тогда
			СписокОтветственных.Добавить(Справочники.Пользователи.НайтиПоКоду("Доколин")); 
		Иначеесли стр22.ВидТовара = Перечисления.ВидыТоваров.АКБ Тогда
			СписокОтветственных.Добавить(Справочники.Пользователи.НайтиПоКоду("Голубева В.С.")); 
		Иначеесли стр22.ВидТовара = Перечисления.ВидыТоваров.Аксессуары Тогда
			СписокОтветственных.Добавить(Справочники.Пользователи.НайтиПоКоду("Голубева В.С.")); 
		Иначеесли стр22.ВидТовара = Перечисления.ВидыТоваров.РекламнаяПродукция Тогда
			СписокОтветственных.Добавить(Справочники.Пользователи.НайтиПоКоду("Лаврова")); 
			СписокОтветственных.Добавить(Справочники.Пользователи.НайтиПоКоду("Голубева В.С.")); //***Задача № 45248
		иначе
			СписокОтветственных.Добавить(Справочники.Пользователи.НайтиПоКоду("Малышев Егор"));
		КонецЕсли;
		...
		Для каждого ОтветственноеЛицо ИЗ СписокОтветственных Цикл
			...
			НоваяЗадача.Исполнитель		  = ОтветственноеЛицо.Значение;
			...
			иначе
				Сообщить(">>>НЕ Создано задание на согласование для  пользователя, т.к. неопределен исполнитель!!!");
				Возврат;
				
			КонецЕсли;
			
			#Если Клиент Тогда
				Сообщить(">>> Создано задание на согласование для  пользователя " +  ОтветственноеЛицо.Значение.ПолноеНаименование()  + "!!!");
			#КонецЕсли
...
Функция ПолучитьПодаркиИзИсходныхЗаказов()
	СписокЗаказов = Заказы.Выгрузить(,"ЗаказПокупателя");
	Запрос = новый Запрос;
	Запрос.Текст="ВЫБРАТЬ
	             |	ЗаказПокупателяБонусы.Номенклатура КАК Подарок,
	             |	СУММА(ЗаказПокупателяБонусы.Количество) КАК НеобходимоПодарить
	             |ИЗ
