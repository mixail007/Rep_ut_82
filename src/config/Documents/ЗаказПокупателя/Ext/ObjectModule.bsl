...
процедура  ДвиженияПоЗаказыПокупателяСезонные(СтруктураШапкиДокумента, ТаблицаПоТоварам)
	...
	//16-17.07.2018 - блокирока нужна по товарам и их аналогам!
	//ЭлементБлокировки.ИсточникДанных = ЭтотОбъект.Товары; 
	//ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Номенклатура", "Номенклатура");  	  
	
	//! НЕЛЬЗЯ блокировать - подразделение из выборки - по сезонному заказу ! 
	...
	|ГДЕ
	//|	АналогиДисков.Аналог В (ВЫБРАТЬ ВТ_Товары.Номенклатура ИЗ ВТ_Товары) И
	...
	|ГДЕ
	//|	АналогиДисков.Номенклатура В (ВЫБРАТЬ ВТ_Товары.Номенклатура ИЗ ВТ_Товары) И
...
//	|	РегистрСведений.АналогиДисков КАК АналогиДисков
	...
	|
	|ВЫБРАТЬ 
	//различные  - излишнее условие, различные будут потом
...
//Подразделение не индексируется! приводит к сканированию ВСЕЙ таблицы!
//|			И ЕСТЬNULL(Подразделение.ВЭД, ЛОЖЬ) = ЛОЖЬ

////---------- ВЫБОР заменен на необязательное ИЛИ---------------------------------------- 
    //|				И ВЫБОР
...
процедура ЗаполнитьТаблСогласования()
	...
	Для каждого стрОб из ЭтотОбъект.Товары Цикл
		...
		ИначеЕсли мПроведениеИзФормы Тогда
			...
			Если стрОб.Статус = перечисления.СтатусыСтрокЗаказа.Подтвержден  // при измении строки - утверждать надо заново!
				и окр(стрОб.Цена/КоэфБонусНал) <  стрСсылка.Цена //+++ 10.08.2018
				Тогда
			...
			ИначеЕсли окр(стрОб.Цена/КоэфБонусНал) < стрСсылка.Цена 
