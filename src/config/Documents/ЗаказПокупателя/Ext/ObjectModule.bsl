...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, 
	...
	Если Не Отказ Тогда
		...
		Если // (ДатаОтсчета <> Дата(1,1,1) и ДатаОтсчета <=Дата) и
			не  НеУчитыватьВСезонномЗаказе и проверен тогда
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ЭтоНовый() И Контрагент.ПечататьСпецификацию и Найти(ДоговорКонтрагента.Наименование,"*")=0 тогда  //04.04.2018 - только для Б/нал
		ЭтотОбъект.ВыписатьСпецификацию = Истина;
		#Если Клиент тогда
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение 
		и мПроведениеИзФормы = ЛОЖЬ
		и этотОбъект.ТипЗаказа<>6 //+++ 04.06.2018 - Объединение заказов фиксируется в другом показателе!
		и этотОбъект.ТипЗаказа<>7 Тогда	 
...
функция ПолучитьТаблСкладов()
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ спр.Ссылка КАК Склад
	|ПОМЕСТИТЬ ВТ_Склады
	|ИЗ справочник.Склады как спр
	|	ГДЕ спр.ЭтоГруппа = ЛОЖЬ 
	| 	И спр.ПометкаУдаления = ЛОЖЬ
	| 	И спр.ЗапретитьИспользование = ЛОЖЬ
	|	И спр.Транзитный "+?(Подразделение.ОбособленноеПодразделение, " И спр.Подразделение = &Подразделение", " = ЛОЖЬ")+"
	|;
	|ВЫБРАТЬ 
	|	ТоварыНаСкладахОстатки.Номенклатура,
	...
	//26.06.2018  - 1 подзапрос вместо 3х
	|			И Склад в (выбрать вт1.Склад из ВТ_Склады как вт1)  ) КАК ТоварыНаСкладахОстатки";
...
функция ПолучитьТаблСкладовОТХ()
	...
	|			Номенклатура В (&СписТов)
	//26.06.2018  - 1 подзапрос вместо 3х
	|			И Склад в (выбрать вт1.Склад из ВТ_Склады как вт1)
	|			И Контрагент В (ВЫБРАТЬ СрокиДоставки.Поставщик
	|					ИЗ РегистрСведений.СрокиДоставки КАК СрокиДоставки
	|					ГДЕ СрокиДоставки.ОТХ = ИСТИНА) ) КАК ТоварыНаОТХ";
	Запрос.УстановитьПараметр("СписТов",      Товары.ВыгрузитьКолонку("Номенклатура") );
	Запрос.УстановитьПараметр("Подразделение", Подразделение);	
