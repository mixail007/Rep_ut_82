...
Процедура ПроверитьНулевыеЦены(Отказ) экспорт
	...
	Запрос.Текст="ВЫБРАТЬ
	             ...
	             |	ТЗ.Цена,
	             |	ТЗ.Номенклатура
				 //+++ 04.06.2018 из справочника прямо в запросе!
				 //|	, ТЗ.НоменклатураВидТовара,
				 //|	ТЗ.НоменклатураКод
	             |ПОМЕСТИТЬ ВТ_Товары
	             |ИЗ
	             |	&таблИзЗаказа КАК ТЗ;
				 ...
				 |	Левое соединение(выбрать спр.Код, спр.ВидТовара, спр.Ссылка как Ссылка1 
...
//+++ 28.06.2018 Константы.ИспользоватьОстаткиСтороннихПоставщиков.Получить() = ЛОЖЬ ВСЕГДА!
	...
	//Если ЗначениеНеЗаполнено(ДоговорКонтрагента.КонтрагентДляРезерваИМ) и (ЭтоНовый() или РежимПроведения = РежимПроведенияДокумента.Оперативный) и Контрагент <> Справочники.Контрагенты.НайтиПоКоду("36092") Тогда
	
	////05.10.18 старый метод
	//
	//Если (ЗначениеНеЗаполнено(ДоговорКонтрагента.КонтрагентДляРезерваИМ) или ДоговорКонтрагента.ОтветственноеЛицо.НаправлениеПродаж = Справочники.НаправленияПродаж.НайтиПоКоду("16")) и (ЭтоНовый() или РежимПроведения = РежимПроведенияДокумента.Оперативный) и Контрагент <> Справочники.Контрагенты.НайтиПоКоду("36092") Тогда
	...
	ИначеЕсли ТипЗаказа <> 12 Тогда
		...
		Для каждого стрСогл из 	таблСогл1 Цикл
			...
			//ЭтоПредоплата = (ЭтотОбъект.ДоговорКонтрагента.ТипДоговора.Код="00001" или ЭтотОбъект.ДоговорКонтрагента.ТипДоговора.Код="00005"); //16.05.2017
			  ЭтоПредоплата = (ЭтотОбъект.ДоговорКонтрагента.ТипДоговора = справочники.ТипыДоговоров.НайтиПоКоду("00001") 
			  			   или ЭтотОбъект.ДоговорКонтрагента.ТипДоговора = справочники.ТипыДоговоров.ПредоплатаПоСчетам //+++14.03.2018 
					       или ЭтотОбъект.ДоговорКонтрагента.ТипДоговора = справочники.ТипыДоговоров.ФакторингПредоплата //  16.05.2017
			                   ); 
			//+++)
				Для i=0 по строки1.Количество()-1 Цикл
					...
					|";
				КонецЦикла;	                                                                     
				задачаОб.Наименование = "Утвердите цены "+?(ЭтоПредоплата," ПО ПРЕДОПЛАТЕ","")+?(Найти(ДоговорКонтрагента.Наименование,"*")>0," - ПО ДОГОВОРУ ( * )","") //12.04.2018 по задаче № 44237
				+?(этотОбъект.БонусПрименен и этотОбъект.ПроцентСкидкиНал<>0, " БОНУС "+строка(этотОбъект.ПроцентСкидкиНал)+"%", "") //+++10.08.2018
 				+" Для "+строка(строки1.Количество())+"тов. по Заказу "+строка(ссылка.Номер)+" покупателя: "+строка(ссылка.Контрагент);
				
				задачаОб.Описание = ?(задачаОб.Выполнена, "Согласованы", "Требуется согласовать")+" следующие цены"+?(ЭтоПредоплата," ПО ПРЕДОПЛАТЕ","")
				+?(Найти(ДоговорКонтрагента.Наименование,"*")>0," - ПО ДОГОВОРУ ( * )","") //12.04.2018 по задаче № 44237 
				+?(этотОбъект.БонусПрименен и этотОбъект.ПроцентСкидкиНал<>0, " БОНУС "+строка(этотОбъект.ПроцентСкидкиНал)+"%", "") //+++10.08.2018
...
//+++ 28.06.2018 - убрано навсегда, работает другая схема!	
	...
	Если Не Отказ Тогда
		...
		ТаблицаПоТаре, Отказ, Заголовок);
		//28.03.19 Смирнов уже есть запись внизу
		//// миронычев
