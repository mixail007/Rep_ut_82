...
Функция ПечатьСчетаЗаказа(Тип)Экспорт
	...
	|	СуммаВключаетНДС
	|ИЗ
	|	Документ.ЗаказПоставщикуСезонный КАК ЗаказПокупателя
	|
	|ГДЕ
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ИЗ
	               |	(ВЫБРАТЬ
	               |		ЗаказПоставщикуСезонный.Номенклатура КАК Номенклатура,
	               |		ЗаказПоставщикуСезонный.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	               |		ЗаказПоставщикуСезонный.ПроцентСкидкиНаценки КАК ПроцентСкидкиНаценки,
	               |		ЗаказПоставщикуСезонный.ПроцентАвтоматическихСкидок КАК ПроцентАвтоматическихСкидок,
	               |		ЗаказПоставщикуСезонный.Цена КАК Цена,
	               |		СУММА(ЗаказПоставщикуСезонный.Количество) КАК Количество,
	               |		СУММА(ЗаказПоставщикуСезонный.Сумма) КАК Сумма,
	               |		СУММА(ЗаказПоставщикуСезонный.СуммаНДС) КАК СуммаНДС,
	               |		ЗаказПоставщикуСезонный.ХарактеристикаНоменклатуры КАК Характеристика,
	               |		МИНИМУМ(ЗаказПоставщикуСезонный.НомерСтроки) КАК НомерСтроки,
	               |		0 КАК Метка
	               |	ИЗ
	               |		Документ.ЗаказПоставщикуСезонный.Товары КАК ЗаказПоставщикуСезонный
	               |	ГДЕ
	               |		ЗаказПоставщикуСезонный.Ссылка = &ТекущийДокумент
	               |	
	               |	СГРУППИРОВАТЬ ПО
	               |		ЗаказПоставщикуСезонный.Номенклатура,
	               |		ЗаказПоставщикуСезонный.ЕдиницаИзмерения,
	               |		ЗаказПоставщикуСезонный.ПроцентСкидкиНаценки,
	               |		ЗаказПоставщикуСезонный.ПроцентАвтоматическихСкидок,
	               |		ЗаказПоставщикуСезонный.Цена,
	               |		ЗаказПоставщикуСезонный.ХарактеристикаНоменклатуры) КАК ВложенныйЗапрос
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	ВложенныйЗапрос.Метка,
	               |	ВложенныйЗапрос.НомерСтроки";
	...
	Иначе
		ОбластьМакета.Параметры.ТекстЗаголовка = СформироватьЗаголовокДокумента(Шапка, "Заказ поставщику сезонный");
	КонецЕсли;
	ТабДокумент.Вывести(ОбластьМакета);
...
Функция ПечатьСчетаЗаказаКорректировки(Тип)
	...
	ТабДокумент = Новый ТабличныйДокумент;
	ТабДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЗаказПокупателяСезонный_СчетЗаказ"; // одинаково!
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |		ЗаказПоставщикуСезонный.ПроцентСкидкиНаценки КАК ПроцентСкидкиНаценки,
	               |		ЗаказПоставщикуСезонный.ПроцентАвтоматическихСкидок КАК ПроцентАвтоматическихСкидок,
	               |		ЗаказПоставщикуСезонный.ХарактеристикаНоменклатуры КАК ХарактеристикаНоменклатуры,
	               |		ЗаказПоставщикуСезонный.СуммаНДС КАК СуммаНДС,
	               |		ЗаказПоставщикуСезонный.Сумма КАК Сумма,
	               |		ЗаказПоставщикуСезонный.Количество КАК Количество
	               |	ИЗ
	               |		Документ.ЗаказПоставщикуСезонный.Товары КАК ЗаказПоставщикуСезонный
	               |	ГДЕ
	               |		ЗаказПоставщикуСезонный.Ссылка = &ТекущийДокумент) КАК ВложенныйЗапрос
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ВложенныйЗапрос.Номенклатура,
...
Функция ПолучитьСписокПечатныхФорм() Экспорт
	...
	СписокМакетов.Добавить("ЗаказКорректировка", "Заказ поставщику (корректировка)");
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	#Если Клиент Тогда 
    			//сравним цены с ФОП
		если Этоновый()и валютаДокумента <> константы.ВалютаРегламентированногоУчета.Получить()и дата>Дата('20161001000000')и ПараметрыСеанса.ТекущийПользователь <> Справочники.Пользователи.НайтиПоКоду("Малышев Егор") Тогда
			сравнитьЦенысФОП();
		конецЕсли;
	#КонецЕсли
