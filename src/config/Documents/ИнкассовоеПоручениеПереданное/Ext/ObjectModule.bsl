...
Функция ПолучитьСписокПечатныхФорм() Экспорт
	...
	СписокМакетов.Добавить("ИнкассовоеПоручение","Инкассовое поручение");
		...
		Для Каждого СтрокаПлатеж ИЗ ДвиженияПоПланируемымПлатежам Цикл
			...
			Если НЕ СтрокаПлатеж.ДокументПланированияПлатежа.Пустая() Тогда
				...
				Если НЕ СтрокаПлатеж.ВключатьВПлатежныйКалендарь Тогда // Документ не был проведен по оперативным взаиморасчетам
					ЕстьРасчеты=Истина; //01.04.2019 - выключено движение По регистру "РасчетыСКонтрагентами"
				КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	ТаблицаПлатежейУпр=ПолучитьТаблицуПлатежейУпр(?(Оплачено,КонецДня(ДатаОплаты),Дата),ВалютаДокумента,Ссылка, "ИнкассовоеПоручениеПереданное");
