...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения)=ТипЗнч(Документы.ЗаказПокупателя.ПустаяСсылка()) Тогда 
		...
		АдресДоставки = ДанныеЗаполнения.АдресДоставки; //+++ 06.02.2019
	...
	иначеЕсли 	ТипЗнч(ДанныеЗаполнения)=Тип("ДокументСсылка.ЗаявкаНаБрак") Тогда 
	  ...
	  ВидДефектаДляУценки = ДанныеЗаполнения.ВидДефектаДляУценки;  //Адиянов<<< 20.06.2016
 	  АдресДоставки = ДанныеЗаполнения.АдресДоставки; //+++ 06.02.2019
...
//===============Печатные формы===================================
Процедура Печать(ТабДок=Неопределено, СразуПоказывать=Истина) Экспорт
	...
	//BugFix: ШК=EAN13 ТипКода=1 и меняется не нужно!
	ОбШтрихКод.ТипКода = 4; 
	ОбШтрихКод.Сообщение = СформироватьШКРеализации(Ссылка,"6"); 
	...
	Запрос.УстановитьПараметр("ТипДоговора",Справочники.ТипыДоговоров.НайтиПоКоду("Я0015")); //диагностирование повреждений
	Выборка = Запрос.Выполнить().Выбрать();
	Если выборка.Следующий() тогда
		ОбластьПодвал.Параметры.ТекстЭкспертиза = "*услуги за экспертизу";	
	КонецЕсли;	
	//////////////////////
	...
	Если СразуПоказывать тогда
		табДок.Показать();
	КонецЕсли;
конецпроцедуры	

Процедура ПечатьТорг2(ТабДок=неопределено, СразуПоказывать=Истина) Экспорт
		...
		СведенияОКонтрагенте = СведенияОЮрФизЛице(Контрагент, Ссылка.Дата);
		
		НомерАкта = ПолучитьНомерНаПечать(Ссылка); 
		ДатаАкта = Дата;
...
//			Область.Параметры.ЗаключениеКомиссии="При получении товара обнаружен брак в количестве";
		...
		иначе  //заказ не ИМ
			...
			ОбластьШапка.Параметры.Поставщик 				= ОписаниеОрганизации(СведенияОКонтрагенте, "ПолноеНаименование,ЮридическийАдрес,Телефоны,");
			ОбластьШапка.Параметры.Поставщик2 				= ОписаниеОрганизации(СведенияОКонтрагенте, "ПолноеНаименование,ЮридическийАдрес,Телефоны,");
			ОбластьШапка.Параметры.ДоговорНом 				= "";
			ОбластьШапка.Параметры.СфНом 					= "";
			ОбластьШапка.Параметры.СФД 						= "";
			ОбластьШапка.Параметры.СФМ 						= "";
			...
			ОбластьШапка.Параметры.ДатаТоварнойНакладной 	= "";
			ОбластьШапка.Параметры.СкладОтправителяТовара	= ОписаниеОрганизации(СведенияОКонтрагенте, "ПолноеНаименование,");
...
Функция ПечатьАКТ() Экспорт
	...
	Для Каждого стр из товары Цикл
		Если стр.Повреждение тогда  //на каждый товар?!
			ТабДок=Новый ТабличныйДокумент;
...
Функция ПечатьМХ() экспорт
        УстановитьЗначенияКоличестваСтрокНаЛисте();
		СтруктураШапки = ЗаполнитьРеквизитыШапки(Ссылка);
		Если СтруктураШапки<>Неопределено Тогда
			ПечатьДокумента(СтруктураШапки,Ссылка,1,Ложь);
		КонецЕсли; 		
КонецФункции
...
Функция ЗаполнитьРеквизитыШапки(ДокументСсылка)
	...
	Если Шапка.Следующий() Тогда
		...
		Валюта								= Орг.ОсновнойБанковскийСчет.ВалютаДенежныхСредств;
...
Процедура ПечатьДокумента(СтруктураШапки,Документ,КоличествоКопий=1,НаПринтер=Истина);
	
	ЗапросПоТоварам = Новый Запрос;
	ЗапросПоТоварам.УстановитьПараметр("Ссылка",Документ);
	...
	"ВЫБРАТЬ
	|	ЗаявкаНаВозвратТоваровТовары.Номенклатура.Наименование КАК Наименование,
	|	ЗаявкаНаВозвратТоваровТовары.Номенклатура.Код КАК Код,
	|	ЗаявкаНаВозвратТоваровТовары.Номенклатура.БазоваяЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|	ЗаявкаНаВозвратТоваровТовары.Номенклатура.БазоваяЕдиницаИзмерения.Код КАК КодЕдиницы,
	|	ЗаявкаНаВозвратТоваровТовары.Сумма КАК Сумма,
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ЗаявкаНаВозвратТоваровТовары.Количество > 0
	|				ТОГДА ЗаявкаНаВозвратТоваровТовары.Сумма / ЗаявкаНаВозвратТоваровТовары.Количество
	|			ИНАЧЕ ЗаявкаНаВозвратТоваровТовары.Сумма
	|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК Цена,
	|	ЗаявкаНаВозвратТоваровТовары.Количество КАК Количество
	|ИЗ
	|	Документ.ЗаявкаНаВозвратТоваров.Товары КАК ЗаявкаНаВозвратТоваровТовары
	|ГДЕ
	|	ЗаявкаНаВозвратТоваровТовары.Ссылка = &Ссылка";						  
	...
	Если (Выборка.Количество()>0) Тогда 
		Если (Выборка.Количество() + КоличествоСтрокШапки + КоличествоСтрокПодвала)> КоличествоСтрокНаЛисте Тогда	 		
			...
			// Первая страница
        	МакетОсновной 							= ПолучитьМакет("МакетПервойСтраницыМХ3");
			МакетОстальные 							= ПолучитьМакет("МакетВторойСтраницыМХ3");
			ШапкаОсновная							= МакетОсновной.ПолучитьОбласть("Шапка");
			
			
			ОбШтрихКод=ШапкаОсновная.Рисунки.Штрихкод.Объект ;
			ОбШтрихКод.ТипКода = 4; 
			ОбШтрихКод.Сообщение = СформироватьШКРеализации(Документ,"6"); 
			ОбШтрихКод.ОтображатьТекст = Ложь;
		...
		//КонецЕсли;
			...
			ШапкаОсновная.Параметры.Дата 			= Документ.Дата;	
			...
			Пока (Условие) и (НомерПозиции<=(КоличествоСтрокНаЛисте)) Цикл
				СтрокаОсновная.Параметры.Номер 			= 	НомерСтр;
			...
			Если (Условие) Тогда
				...
				Пока Условие Цикл								
					СтрокаОстальные.Параметры.Номер 			= 	НомерСтр;
		...
		Иначе
			...
			Макет 		= ПолучитьМакет("МакетПервойСтраницыМХ3");
			МакетВторой = ПолучитьМакет("МакетВторойСтраницыМХ3");
	...
	//	КонецЕсли;	
			...
			Пока Выборка.Следующий() Цикл
				Строка.Параметры.Номер 			= 	НомерСтр;
...
Процедура ПроверитьНоменклатуруВРеализации(стр) экспорт
	...
	Если ЗначениеНеЗаполнено(стр) или ЗначениеНеЗаполнено(стр.Реализация) тогда
		Возврат; //BugFix: Задача № 58871 п.9
	КонецЕсли;	
	...
	Если ТипЗнч(стр.Реализация) = ТИП("ДокументСсылка.РеализацияТоваровУслуг") ИЛИ ТипЗнч(стр.Реализация) = ТИП("ДокументСсылка.ОтчетОРозничныхПродажах") Тогда 
		Запрос.Текст = "ВЫБРАТЬ
	...
	ИначеЕсли ТипЗнч(стр.Реализация) = ТИП("ДокументСсылка.ПеремещениеТоваров") Тогда
		Запрос.Текст = "ВЫБРАТЬ
