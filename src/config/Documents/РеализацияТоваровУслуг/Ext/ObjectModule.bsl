...
//			Если РежимПроведения = РежимПроведенияДокумента.Оперативный Тогда
		...
		Если Не ЗначениеНеЗаполнено(Сделка) 
			...
			Если Не Отказ и ТипЗнч(Сделка) = Тип("ДокументСсылка.ЗаказПокупателя") и (ДатаОтсчета <> Дата(1,1,1) и ДатаОтсчета <=Дата) Тогда
...
процедура  ДвиженияПоЗаказыПокупателейСезонные()
	...
	Если ТипЗнч(ЭтотОбъект.Сделка) <> Тип("ДокументСсылка.ЗаказПокупателя") тогда  //14.11.2017
		возврат;
	КонецЕсли;
