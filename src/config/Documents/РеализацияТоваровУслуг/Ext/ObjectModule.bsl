...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, 
	...
	Если ВидПередачи = Перечисления.ВидыПередачиТоваров.СоСклада Тогда
		...
		Если ПроводитьПоПартиям Тогда
			...
			//Если Контрагент.КонтрагентДляРезерваИМ <> Справочники.Контрагенты.ПустаяСсылка()
			//	и ЭтотОбъект.Дата >= '20160401' Тогда
	...
	Если ОтражатьВУправленческомУчете Тогда
		Если СтруктураШапкиДокумента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СПокупателем 
		   И СуммаДокумента <> 0 Тогда
			...
			// провести по другому подразделению 25.03.2016
			//Если Контрагент.КонтрагентДляРезерваИМ <> Справочники.Контрагенты.ПустаяСсылка() 
			//	и Дата>'20160401' Тогда //28.04.2016
...
процедура  ДвиженияПоЗаказыПокупателейСезонныеРезерв(КлиентРезерв)
	...
	резервныйЗаказ = ПолучитьЗаказРезерв(КлиентРезерв, табл, СезонныйЗаказКлиентаРезерв); // должен быть 1... или список
...
функция ПолучитьСезонныйЗаказНаКлиентРезерв(КлиентРезерв)
	...
	если выборка1.Следующий() тогда
			рез = выборка1.ЗаказПокупателяСезонный;
	КонецЕсли;
...
функция ПолучитьЗаказРезерв(КонтрРезерв, табл, ЗакСезон)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |				И Номенклатура В(ВЫБРАТЬ ВТ_тов.Номенклатура
	               |					ИЗ ВТ_тов) ) КАК РезервДляИМОбороты
				   ...
				   |					И Номенклатура В (ВЫБРАТЬ ВТ_тов.Номенклатура
				   |						ИЗ ВТ_тов)) КАК ЗаказыПокупателейСезонныеОстатки
	               ...
	               |УНИЧТОЖИТЬ ВТ_Тов";
	Запрос.УстановитьПараметр("табл", табл);
	Запрос.УстановитьПараметр("НачДата", ЗакСезон.Дата); //раньше нет смысла смотреть!
	Запрос.УстановитьПараметр("КонДата", сделка.Дата+1); //зак.пок. - резервный заказ ОБЯЗАТЕЛЬНО раньше заказа конечного покупателя
	...
	Если выборка.Количество()>1 тогда //есть несколько заказов на резерв!
		#Если Клиент тогда
			Сообщить("ВНИМАНИЕ! Найдено "+строка(выборка.Количество())+" резервных заказов покупателей на клиента: "+строка(КонтрРезерв)+"
					|уменьшается только 1-ый ""резервный"" заказ: "+строка(рез), СтатусСообщения.Важное );
		#КонецЕсли 	
	КонецЕсли;
