...
Функция ПечатьДокумента()   Экспорт
	...
	Если Найти(ДоговорКонтрагента.Наименование,"*")=0 тогда//24.04.2018
...
КонецЕсли;
 ...
 Если Найти(ДоговорКонтрагента.Наименование,"*")=0 тогда //24.04.2018
		...
		Если Шапка.ДоговорКонтрагента.ТипДоговора = Справочники.ТипыДоговоров.ШинтрейдЯрославль Тогда
			Если НЕ ОрганизацияОтгрузки.Пустая() Тогда
				ОбластьМакета.Параметры.ФИОРуководителя  = "/Никитин М.И./"; //20.03.2019 << "/Захаров В.В./";
				ОбластьМакета.Параметры.ФИОБухгалтера    = "/Никитин М.И./";
...
Процедура ЗаполнитьТабличнуюЧастьПоЗаказуПокупателяУпр(ТабличнаяЧасть, ДокументОснование, ВариантВыводаФормыЗаполнения = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("ставкаНДС_2019", ставкаНДС_2019);
	Запрос.Текст ="
	|ВЫБРАТЬ
...
Процедура ЗаполнитьУслугиПоОстаткамУпр() Экспорт
	...
	Запрос.УстановитьПараметр("Договор",      ДоговорКонтрагента);
	Запрос.УстановитьПараметр("Сделка",       Сделка);
    Запрос.УстановитьПараметр("СтавкаНДС_2019", СтавкаНДСнаДату(ТекущаяДата()) );   //+++ 25.01.2019
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВложенныйЗапрос.Номенклатура                        КАК Номенклатура,
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, 
	...
	Если ОтражатьВУправленческомУчете Тогда
		Если СтруктураШапкиДокумента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СПокупателем 
			...
			//найдем коэф., если он задан для типа договора
			
			ЗапросК = Новый Запрос;
			ЗапросК.Текст = 
			"ВЫБРАТЬ
			|	КоэффициентПересчетаШТЯ_ФАПСрезПоследних.Коэффициент
			|ИЗ
			|	РегистрСведений.КоэффициентПересчетаШТЯ_ФАП.СрезПоследних(&Дата, ) КАК КоэффициентПересчетаШТЯ_ФАПСрезПоследних
			|ГДЕ
			|	КоэффициентПересчетаШТЯ_ФАПСрезПоследних.ТипДоговора = &ТипДоговора";
			
			ЗапросК.УстановитьПараметр("Дата", Дата);
			ЗапросК.УстановитьПараметр("ТипДоговора", ДоговорКонтрагента.типДоговора);
			
			РезультатК = ЗапросК.Выполнить();
			
			ВыборкаК = РезультатК.Выбрать();
			
			Если ВыборкаК.Следующий() тогда
				коэф = ВыборкаК.Коэффициент;
			иначе 
				коэф = 0;
			КонецЕсли;
			...
			Если ДоговорКонтрагента.типДоговора = Справочники.ТипыДоговоров.ШинтрейдЯрославль и Дата > '20150831235959' и Дата <= '20150930235959'Тогда  
				...
				Для каждого стр из ТаблицаПоТоварамШ Цикл
					...
					Если   Дата <= '20181231235959' тогда
					стр.Цена =  Окр(Стр.Сумма/стр.Количество*К1*1.18,0);
				    иначе
					стр.Цена =  Окр(Стр.Сумма/стр.Количество*К1*1.2,0);
					конецЕсли;
			...
			иначеЕсли ДоговорКонтрагента.типДоговора = Справочники.ТипыДоговоров.ШинтрейдЯрославль и  коэф>0 и Дата >  '20150930235959'Тогда
				...
				Для каждого стр из ТаблицаПоТоварамШ Цикл
				    ...
				    иначе
					стр.Цена =  Окр(Стр.Сумма/стр.Количество*К1*К2*1.2,0);
					конецЕсли;
				...
				чНДС = ПолучитьСтавкуНДС( СтавкаНДСнаДату(ЭтотОбъект.Дата) );//+++ 28.12.2018 18>>20%
				НДС  = ТаблицаПоТоварамШ.Итог("Стоимость")*чНДС/(100+чНДС);// /118*18;
				СтрокаДвижений.СуммаВзаиморасчетов   = Окр(НДС,2); //-> до копеек!
				СтрокаДвижений.СуммаУпр   			 = Окр(НДС,2);
				КНДС = 1+(НДС)/таблицаПоТоварам.Итог("стоимость"); 
				
				ТаблицаПоТоварамШ = таблицаПоТоварам.Скопировать();
				k = ТаблицаПоТоварамШ.Количество()-1;  макс = 0; i=0; //28.12.2018 
				Для каждого стр из ТаблицаПоТоварамШ Цикл 
					стр.Стоимость          = Окр(Стр.Стоимость*КНДС,2);
					стр.СтоимостьБезСкидок = стр.Стоимость;
					//28.12.2018 - копейки на макс.сумму!
					  Если макс<стр.Стоимость тогда
						макс = стр.Стоимость; 
						  k  = i;
					  КонецЕсли; 
					  i = i + 1;
				...
				Разница = ТаблицаПоТоварамШ.Итог("Стоимость") -( СуммаДокументаП + Окр(НДС,2) );
				Если Разница<>0 и k>=0 Тогда
					ТаблицаПоТоварамШ[k].стоимость 		    = ТаблицаПоТоварамШ[k].стоимость-Разница;
					ТаблицаПоТоварамШ[k].стоимостьБезСкидок = ТаблицаПоТоварамШ[k].стоимостьБезСкидок - Разница;
				КонецЕсли;	 
			КонецЕсли;	
			
			//с 09.06.18 измененная таблица для бонусов со *  (подготавливаем таблицу для продаж)
			Если  ТипЗнч(сделка) = тип("ДокументСсылка.ЗаказПокупателя") и сделка.БонусПрименен И Найти(Сделка.ДоговорКонтрагента.Наименование, "*")>0 тогда
				
				ТаблицаПоТоварамБ = таблицаПоТоварам.Скопировать();
				...
				Для каждого стр из ТаблицаПоТоварамБ Цикл 
					Цена = Окр((стр.Стоимость/стр.Количество)/((100-Сделка.ПроцентСкидкиНал)/100),0); 
			...
			///( таблица для расчета курсовых разниц   А.А. 20.08.18
			...
			Если ВалютаДокумента <> Константы.ВалютаРегламентированногоУчета.Получить() Тогда
				...
				|	Поступление.ДоговорКонтрагента КАК Договор
				|ИЗ
				|	Поступление КАК Поступление
				|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ВзаиморасчетыСКонтрагентами.Остатки(&МоментВремени, Сделка = &Сделка) КАК ВзаиморасчетыСКонтрагентамиОстатки
				|		ПО Поступление.Сделка = ВзаиморасчетыСКонтрагентамиОстатки.Сделка
				|			И Поступление.ДоговорКонтрагента = ВзаиморасчетыСКонтрагентамиОстатки.ДоговорКонтрагента
				|ГДЕ
			...
			Если ВалютаДокумента <> Константы.ВалютаРегламентированногоУчета.Получить()  Тогда	
				...
				конецЦикла;
			конецЕсли;
			НаборДвижений.мПериод            = Дата; //КонецДня(ДатаОплаты);
			НаборДвижений.мТаблицаДвижений   = ТаблицаДвижений;
		...
		//+++ 01.04.2019 ВЫКЛЮЧЕНО движение по регистру "РасчетыСКонтрагентами"	
			...
			Если не ДоговорКонтрагента.ВнутренниеВзаиморасчеты тогда
				...
				иначеЕсли ТипЗнч(сделка) = тип("ДокументСсылка.ЗаказПокупателя") и  Сделка.БонусПрименен И Найти(Сделка.ДоговорКонтрагента.Наименование, "*")>0 тогда
					// подсунем пересчитанную таблицу с бонусами
				...
				если договорконтрагента.типдоговора = справочники.ТипыДоговоров.Бонусы и дата>=дата('20180101')тогда
					...
					для каждого тр из таблицаБонусов цикл
						тр.стоимость= тр.сумма*курсВзаиморасчетов;  
						тр.стоимостьбезскидок= тр.сумма*курсВзаиморасчетов; 
					конеццикла;
				...
				Если  Дата>'20180101'и договорконтрагента.типдоговора <> справочники.ТипыДоговоров.Бонусы Тогда //28.04.2016
					ТаблицаДвижений.ЗаполнитьЗначения(ЭтотОбъект.ПодразделениеДохода,"Подразделение");
				иначеЕсли  Дата>'20180101'и договорконтрагента.типдоговора = справочники.ТипыДоговоров.Бонусы Тогда
				Иначе
					ТаблицаДвижений.ЗаполнитьЗначения(Подразделение,"Подразделение");
				КонецЕсли;
			...
			Иначе                                                                                                 //01.11.2018 -> флНеоперативно
				...
				Если ОТКАЗ тогда
					Если РежимПроведения = РежимПроведенияДокумента.Оперативный тогда
						...
						//оперативно - ТОЛЬКО  IT-Отдел!
						Если РольДоступна("ПравоЗавершенияРаботыПользователей") или РольДоступна("яштФинДиректор") тогда 
							Отказ = ложь; 
							#Если Клиент тогда
					...
					Иначе //неоперативно - может только фин.отдел делать!
						...
						//неоперативно - ФИНОТДЕЛ + IT!
						Если РольДоступна("ПравоЗавершенияРаботыПользователей") или РольДоступна("яштФинДиректор") тогда // финОтдел + IT + ЕгорМалышев, Бондаренко, Марешева?
							Отказ = ложь; 
						КонецЕсли;
		...
		Если СтруктураШапкиДокумента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомиссионером или СтруктураШапкиДокумента.ДоговорКонтрагента.ТипДоговора = Справочники.ТипыДоговоров.ПеремещениеОТХ  Тогда
		...
		Если Не ЗначениеНеЗаполнено(Сделка) 
			...
			// = 19.10.2016
			// ДатаОтсчета  = Константы.ДатаУчетаСЗППоРегистру.Получить();  // давно не используется!
			...
			//+++ 08.11.2018 --- проверяем резерв и по дате и по товарам! ---------------
				флНовСхема = ЛОЖЬ;				
				Если клиентРезерв.Пустая() тогда 
					флНовСхема = ЕстьРезервИМ(ссылка); 
				КонецЕсли;
				
				Если НЕ клиентРезерв.Пустая() тогда
					ДвиженияПоЗаказыПокупателейСезонныеРезерв(клиентРезерв, флНовСхема); // ЗАКРЫТИЕ обычного и "нескончаемого" резерва
				КонецЕсли;
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаказПокупателя")
		...
		Если ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаказПокупателя") и (не Основание.Транзит)
			...
			Если не РольДоступна("МенеджерФормулы") тогда
				#Если Клиент тогда
					Предупреждение("Ввод реализаций осуществляется только из документа ""Задание на отгрузку""!",30);
				#КонецЕсли  
			КонецЕсли;
			Если РольДоступна("ПолныеПрава") или РольДоступна("МенеджерФормулы") тогда
			иначе
				возврат;
			КонецЕсли;
		...
		//Формула не может вводить на основании заказа
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") Тогда
		...
		Если Основание.Подразделение.ВЭД Тогда //если это ВЭД, тогда заполняем по Сезонному заказу покупателю
			...
			Если Найти(Основание.Организация.Наименование,"Wheels Group") > 0 Тогда
				//+++Шарафутдинов 14.06.2018 по задаче 49245 
...
//			УслугаПени.СтавкаНДС = УслугаПени.Номенклатура.СтавкаНДС;
			УслугаПени.СтавкаНДС = СтавкаНДСнаДату(ЭтотОбъект.Дата);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	Если ЭтоНовый() тогда
		...
		ИначеЕсли Договорконтрагента.ТипДоговора = Справочники.ТипыДоговоров.TyreTechnology Тогда //турция
			подразделениеДохода =   справочники.Подразделения.НайтиПоКоду("00162");
		ИначеЕсли Договорконтрагента.ТипДоговора = Справочники.ТипыДоговоров.WheelsGroupLimited Тогда  //гонконг
			подразделениеДохода =   справочники.Подразделения.НайтиПоКоду("00166");
		ИначеЕсли Договорконтрагента.ТипДоговора = Справочники.ТипыДоговоров.ШинтрейдТранзит Тогда  //шинтрейд транзит
			подразделениеДохода =   справочники.Подразделения.НайтиПоКоду("00198");
		иначе
		...
		конецЕсли;
		Если Склад = Справочники.Склады.НайтиПоКоду("00196") тогда//склад услуг
			УстановитьНовыйНомер("У");
		КонецЕсли;	
	конецЕсли;	
...
//+++ 01.11.2018 ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++	
	Если не Отказ и ТипЗнч( ЭтотОбъект.Сделка ) = Тип("ДокументСсылка.ЗаказПокупателя") тогда
		Если не Этотобъект.Сделка.подразделение.ОбособленноеПодразделение тогда 
			Если Дата>=НачалоДня(ТекущаяДата()) //+++ 07.12.2018 - чтобы партии проводились... 
				и НельзяОбъединятьНеПодтвержденныйЗаказВКоторомЕстьТоварыСтопЛиста( ЭтотОбъект.Сделка ) тогда
				Отказ = Истина;
				#Если Клиент тогда
					Сообщить("НЕТ Разрешения на отгрузку СТОП-ЛИСТА в документе: "+строка(ЭтотОбъект.Сделка) , СтатусСообщения.Внимание);
				#КонецЕсли
			КонецЕсли;
		конецЕсли;
	КонецЕсли;
...
//			ПроверитьПревышениеОбщегоЛимитаДЗСУчетомЗаказа(ЭтотОбъект.Ссылка, Отказ); // ДЗ + сумма этой реализации
	...
	//+++( 14.02.2019
	ДатаСреза = мин(ДатаОтгрузки0, ДатаОтгрузки1); // на дату отгрузки!
	яштРезервыПоТоварам.ЗаполнитьДвиженияРН_ПрибыльОтПовышенияЦен(Движения, ЭтотОбъект , ДатаСреза ); 
	//+++)
...
Процедура ПриЗаписи(Отказ)
	...
	КонецЦикла;
       ...
       //04.10.18 Смирнов, если Франчайзи, то зарегистрируем для обмена
	   Запрос = новый Запрос;
	   Запрос.Текст = "ВЫБРАТЬ
	                  ...
	                  |	И СоответствиеКонтрагентовСкладамФРан.Контрагент = &Контрагент
	                  |	И СоответствиеКонтрагентовСкладамФРан.Договор = &Договор";
	   Запрос.УстановитьПараметр("Контрагент",Контрагент);
	   Запрос.УстановитьПараметр("Договор",ДоговорКонтрагента);
...
процедура  ДвиженияПоЗаказыПокупателейСезонные()
	...
	Если ТипЗнч(Сделка) = Тип("ДокументСсылка.ЗаказПокупателя") тогда 				//минимальная блокировка!
		ЭлементБлокировки.УстановитьЗначение("ЗаказПокупателя",ЭтотОбъект.Сделка);  //аналоги списываются ТОЛЬКО по Заказу!
	КонецЕсли;
	ЭлементБлокировки.УстановитьЗначение("Контрагент",Контрагент);
	Блокировка.Заблокировать();
	//---------------------ИЛИ все товары НАДО блокировать так же!------------------------------------
	ЭлементБлокировки = Блокировка.Добавить("РегистрНакопления.ЗаказыПокупателейСезонные");
	ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
	ЭлементБлокировки.ИсточникДанных = ЭтотОбъект.Товары; //16.07.2018 - для разделения по товарам!
	ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Номенклатура", "Номенклатура");  	  
	...
	Запрос.Текст = 
		...
		|ИЗ
		|РегистрНакопления.ЗаказыПокупателейСезонные.Остатки( , ЗаказПокупателя = &Заказ 
		//16.07.2018 - сразу сделаем отбор по Аналогам перед соединением
		|							И Аналог в (выбрать ВТ_Товары.Номенклатура из ВТ_Товары) ) КАК ЗаказыПокупателейСезонныеОстатки
		// ВНУТРЕННЕЕ ??? зачем ???
...
процедура  ДвиженияПоЗаказыПокупателейСезонныеРезерв(КлиентРезерв, новаяСхема=ЛОЖЬ)
	...
	Если новаяСхема тогда //+++ 08.11.2018
		табл = ЕстьРезервИМ(ЭтотОбъект, 2);
	иначе
	...
	Если масРезервныхЗаказов.Количество()>0 тогда
		...
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		ЭлементБлокировки.ИсточникДанных = ЭтотОбъект.Товары;
		ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Аналог", "Номенклатура"); 	//16.07.2018 - только по аналогам из этой реализации
		ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Номенклатура", "Номенклатура"); 	 // Аналог = Номенклатура
		ЭлементБлокировки.УстановитьЗначение("Подразделение", Подразделение);
		ЭлементБлокировки.УстановитьЗначение("Контрагент", КлиентРезерв);
		Блокировка.Заблокировать();
    КонецЕсли;
...
//	НаборДвижений.Очистить();
	...
	Запрос.Текст = 
		...
		//сразу отбор по аналогам перед соединением!
		|		и Аналог в (выбрать Номенклатура из ВТ_Товары)
		...
		|	ПО  ЗаказыПокупателейСезонныеОстатки.Аналог = ТоварыРеализации.Номенклатура
		|
		// 17.07.2018 =============U-блокировка по всем измерениям===========================
		|ДЛЯ ИЗМЕНЕНИЯ РегистрНакопления.ЗаказыПокупателейСезонные.Остатки
		|
		|УПОРЯДОЧИТЬ ПО
		|	ЗаказПокупателяСезонныйМоментВремени
		|ИТОГИ
...
функция ПолучитьСезонныйЗаказНаКлиентРезерв(КлиентРезерв)
	...
	Запрос1.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1 
	                |	ЗаказыПокупателейСезонныеОстатки.ЗаказПокупателяСезонный,
				//+++ 16.07.2018 сразу в выборке!
					|	ЗаказыПокупателейСезонныеОстатки.ЗаказПокупателяСезонный.Дата как ДатаДок
					|
	                |ИЗ
	                |	РегистрНакопления.ЗаказыПокупателейСезонные.Остатки( ,
	                |			Контрагент = &КлиентРезерв
				//+++( 16.07.2018 
				//----------- только 1 клиент и 1 заказ DISTRI CASH ACCESSOIRES, WG000078---------------------
				    |		И ЕСТЬNULL(ЗаказПокупателяСезонный.Подразделение.ВЭД, ЛОЖЬ) = ЛОЖЬ
				//------------сразу в отборе остатков!  для исключения взаимоблокировки по заказам!-----------------
					|		и ЗаказПокупателя =Значение(Документ.ЗаказПокупателя.ПустаяСсылка) 
					|		и ( Аналог в (&СписокТоваров) 
					|		или Номенклатура в (&СписокТоваров) )
					|) КАК ЗаказыПокупателейСезонныеОстатки
	               // |ГДЕ
				...
				//+++)   
	                ...
	                |УПОРЯДОЧИТЬ ПО
	                |	ДатаДок УБЫВ
	              //  |АВТОУПОРЯДОЧИВАНИЕ - лишнее
				    |";
	Запрос1.УстановитьПараметр("КлиентРезерв",    КлиентРезерв );
	Запрос1.УстановитьПараметр("СписокТоваров", Товары.ВыгрузитьКолонку("Номенклатура") );  //+++ 16.07.2018 только по товарам/аналогам из реализации !!!
	
	Результат1 = Запрос1.Выполнить(); //ПолучитьСезонныйЗаказНаКлиентРезерв()
...
функция ПолучитьТоварыРезерва(КлиентРезерв, Дата0, ЗакПок ) 
	...
	запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	РезервДляИМОбороты.КоличествоРасход КАК Количество
	               |ИЗ
				   ...
				   //23.05.2018
	               |	И РезервДляИМОбороты.КоличествоРасход <> 0
	               |";
	//запрос.УстановитьПараметр("НачДата", Дата0);/  //от начала сезонного заказа?
	запрос.УстановитьПараметр("НачДата", ЗакПок.Дата ); //дата заказа  //23.05.2018
	запрос.УстановитьПараметр("КонДата", ЗакПок.Дата+1 ); //дата заказа + 1 сек
...
функция ПолучитьЗаказРезерв(КонтрРезерв, табл, ЗакСезон)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ИЗ
	               |	РегистрНакопления.РезервДляИМ.Обороты( &НачДата,&КонДата, Регистратор,
	               |				КонтрагентДляРезерваИМ = &КонтрРезерв
	               |				И Номенклатура В (ВЫБРАТЬ ВТ_тов.Номенклатура ИЗ ВТ_тов)) КАК РезервДляИМОбороты
				   ...
				   |				И Номенклатура В (ВЫБРАТЬ ВТ_тов.Номенклатура ИЗ ВТ_тов)) КАК ЗаказыПокупателейСезонныеОстатки
	...
	Результат = Запрос.Выполнить(); //ПолучитьЗаказРезерв()
...
процедура  ДвиженияПоЗаказыПокупателейСезонныеВЭД()
	...
	ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
	ЭлементБлокировки.ИсточникДанных = ЭтотОбъект.Товары;
	ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Номенклатура", "Номенклатура"); //ТОЛЬКО по Номенклатура!
//	ЭлементБлокировки.УстановитьЗначение("Подразделение", Подразделение);			 //! подразделение из выборки, а не из документа !
	ЭлементБлокировки.УстановитьЗначение("Контрагент",Контрагент);
	Блокировка.Заблокировать();
	...
	Запрос.Текст = 
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ЗаказыПокупателейСезонные.Остатки(, ЗаказПокупателяСезонный = &Заказ
		//+++ 16.07.2018 - сразу отбор только по товарам из реализации!
		|			И Номенклатура в (выбрать ТоварыРеализации.Номенклатура из ТоварыРеализации)  ) КАК ЗаказыПокупателейСезонныеОстатки
...
Процедура СравнитьЦеныСЗаказомПокупателя(Отказ)
	Если ЗначениеЗаполнено(Сделка) и ТипЗнч(Сделка)=тип("ДокументСсылка.ЗаказПокупателя") //01.04.2019
		 Тогда
		Запрос = Новый Запрос;
		//14.11.2017 - сразу Табл документа
		...
		Запрос.Текст = "ВЫБРАТЬ
					   ...
					   //01.04.2019 --- выборка сразу по 2м измерениям
		               |					(ЗаказПокупателя,Номенклатура) В (ВЫБРАТЬ вт.ЗаказПокупателя, вт.Номенклатура
		               |														ИЗ	ВТ_Товары КАК вт)  
		...
		Пока Выборка.Следующий() Цикл
			Сообщить("В строке "+ Строка(Выборка.НомерСтроки)+  " товар " + СокрЛП(Выборка.НоменклатураПредставление) + " с ценой " +Строка(Выборка.Цена) +"р. не соответствует цене в заказе: "+Строка(Выборка.ЦенаВЗаказе)+"р.", СтатусСообщения.Важное);
...
Процедура ПеренестиДокиВБухгалтерию() Экспорт
	...
	удалитьфайлы(КаталогВременныхФайлов(),"*.fi");
	...
	Попытка		
		...
		//+++Шарафутдинов
		XML = Новый ХранилищеЗначения(Новый ДвоичныеДанные(КаталогВременныхфайлов()+Ссылка.Номер+"_fi.fi"));
		//---Шарафутдинов
