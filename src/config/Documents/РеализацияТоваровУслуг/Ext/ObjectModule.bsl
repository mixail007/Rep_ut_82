...
функция ПолучитьТоварыРезерва(КлиентРезерв, Дата0, ЗакПок ) 
	...
	запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	СУММА(ЕстьNUll(РезервДляИМОбороты.КоличествоРасход,0)) КАК Количество
	               |ИЗ
				   ...
				   //23.05.2018
	               |    И ЕстьNUll(РезервДляИМОбороты.КоличествоРасход,0)<>0
				   |
	               |СГРУППИРОВАТЬ ПО
	               |	РезервДляИМОбороты.Номенклатура";
	//запрос.УстановитьПараметр("НачДата", Дата0);/  //от начала сезонного заказа?
	запрос.УстановитьПараметр("КонДата", ЗакПок.Дата-1 ); //дата заказа - 1 сек //23.05.2018
