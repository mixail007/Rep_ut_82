...
Процедура ПечатьЗаявки() Экспорт
	...
	ЕстьРасчетыСКонтрагентами=ЛОЖЬ; //01.04.2019 ЕстьРасчетыСКонтрагентами(ВидОперации);
...
//+++ 01.04.2019 по регистру "РасчетыСКонтрагентами".>> "ВзаимоРасчетыСКонтрагентами"
// 
...
//+++ 01.04.2019 - выключено движение по регистру "РасчетыСКонтрагентами"	
	...
	если состояние <> Перечисления.СостоянияОбъектов.Отклонен тогда
		...
		Если валютаДокумента<> Константы.ВалютаРегламентированногоУчета.Получить() тогда	
			...
			ЗапросВ.УстановитьПараметр("Валюта", ВалютаДокумента);
			ЗапросВ.УстановитьПараметр("Дата", ДобавитьМесяц(НачалоМесяца(ДатаРасхода),-1)+25*24*60*60);
			
			РезультатВ = ЗапросВ.Выполнить();
			
			ВыборкаВ = РезультатВ.Выбрать();
			
			Если ВыборкаВ.Следующий() тогда
				курс = ВыборкаВ.Курс+3;
			иначе 
				курс=1;	
			КонецЕсли;
		иначе курс=1;	
		конецесли;	
			...
			пока выборка.Следующий() цикл
				...
				иначеЕсли Выборка.статьяРасхода.Ввалюте и ВалютаДокумента = Константы.ВалютаРегламентированногоУчета.Получить() тогда
					отказ = Истина;
					Сообщить("Выбрана валютная статья расходов при расчете в рублях!");
				иначе	
					...
					//	курс =1;
					//конецЕсли;	
					СтрокаПланРасход.Сумма=Выборка.суммаПлатежа*курс;
				конецЕсли;
			конецЦикла;
		конецЕсли;
...
ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.АвансовыйОтчет")
	...
	Если СпособЗаполнения = "По заказу" Тогда
     	ЗаполнитьПоЗаказу(Основание,СтрокаПлатеж); //+++ 01.04.2019 ВзаимоРасчеты
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если не отказ и ВидОперации = Перечисления.ВидыОперацийЗаявкиНаРасходование.ОплатаПоставщику Тогда
		...
		Для каждого стр из Расшифровкаплатежа Цикл
			Если Организация = Справочники.Организации.НайтиПоКоду("00001") и ЗначениеЗаполнено(стр.Сделка) и ТипЗнч(стр.Сделка)=Тип("ДокументСсылка.ЗаказПоставщику")и стр.ДоговорКонтрагента.ВалютаВзаиморасчетов<>Константы.ВалютаРегламентированногоУчета.Получить() и ЗначениеЗаполнено(стр.ДоговорКонтрагента) и Свойство.Значение<>Истина  Тогда
				...
				Если стр.СуммаВзаиморасчетов > МаксПредоплата.МаксОстаток Тогда
					Ответ=вопрос("По сделке "+стр.Сделка+" максимально допустимая сумма платежа "+МаксПредоплата.МаксОстаток+". Изменить на максимальную сумму?",РежимДиалогаВопрос.ДаНет,0);
					Если Ответ = КодВозвратаДиалога.Да Тогда
						стр.СуммаВзаиморасчетов =МаксПредоплата.МаксОстаток;
	...
	Если Подразделение = Справочники.Подразделения.НайтиПоКоду("00139") и состояние = Перечисления.СостоянияОбъектов.Подготовлен тогда  //Отдел интернет-продаж
		Если глТекущийПользователь = Справочники.Пользователи.НайтиПоКоду("Федунов") или
			...
			глТекущийПользователь = Справочники.Пользователи.НайтиПоКоду("Сафронов Р. В.") или
	...
	Если РасшифровкаПлатежа.Количество()>0 тогда
		ОтветственныйЗаПланированиеДС = РасшифровкаПлатежа[0].ОтветственныйЗаПланирование;
	КонецЕсли;
