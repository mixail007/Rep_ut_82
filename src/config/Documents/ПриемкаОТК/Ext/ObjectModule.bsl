...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Для Каждого ТекСтрокаТовары Из Товары Цикл
		// регистр ДанныеПоОТК 
		Движение = Движения.ДанныеПоОТК.Добавить();
		Движение.Период = Дата;
		
		Если ЗначениеНеЗаполнено(Сотрудник) Тогда
		//
			ОшибкаПриПроведении("Необходимо заполнить сотрудника !", Отказ);
			Прервать;
		//	
		КонецЕсли; 
		
		Если (ЗначениеНеЗаполнено(ТекСтрокаТовары.ЕдиницаИзмерения))
			 ИЛИ (ЗначениеНеЗаполнено(ТекСтрокаТовары.Количество)) 
			 //ИЛИ (ЗначениеНеЗаполнено(ТекСтрокаТовары.Принято)) 
			 ИЛИ (ЗначениеНеЗаполнено(ТекСтрокаТовары.Номенклатура)) 
			 ИЛИ (ЗначениеНеЗаполнено(ТекСтрокаТовары.Коэффициент)
			 ИЛИ (ЗначениеНеЗаполнено(ТекСтрокаТовары.Ошиповщик))) Тогда
		
			 ОшибкаПриПроведении("Не все данные заполнены! Строка (" + Строка(ТекСтрокаТовары.НомерСтроки) + ")", Отказ);
		...
		Движение.Осмотрено = ТекСтрокаТовары.Количество;
		Движение.Принято = ТекСтрокаТовары.Принято;
		Движение.Ошиповщик = ТекСтрокаТовары.Ошиповщик;
		Движение.КоличествоШипов = ТекСтрокаТовары.КоличествоШипов;
		
		Если (ТекСтрокаТовары.Забраковано > 0) И (ЗначениеНеЗаполнено(ТекСтрокаТовары.Причина)) Тогда
		
			ОшибкаПриПроведении("В случае брака необходимо заполнить причину брака. Строка (" + Строка(ТекСтрокаТовары.НомерСтроки) + ")", Отказ); 
...
Процедура ДвиженияРегистровГрузчикиИКладовщики(Отказ)
	...
	//// Грузчики
	//Набор = РегистрыНакопления.ДанныеПоГрузчикам.СоздатьНаборЗаписей();
	...
	//КонецЦикла;
	//
	//ТаблицаДанныхПоПогрузке.Свернуть("Сотрудник","Вес");
	//
	...
	//КонецЦикла;
	//
	//Набор = РегистрыНакопления.ДанныеПоКладовщикам.СоздатьНаборЗаписей();
	//Набор.Отбор.Регистратор.Значение = ЭтотОбъект.Ссылка;
	//
	//// Кладовщики
