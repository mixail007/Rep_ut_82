...
~111:	Если  СокрЛП(МаркаАвтомобиля)<>"" Тогда
	...
	#КонецЕсли	
	ОбластьМакета.Параметры.СловоВажный = ?(ЭтотОбъект.Важность = перечисления.Важность.Высокая, "!!!", ""); //+++ 13.09.2013
	ОбластьМакета.Параметры.НомерОчереди = ?(ЭтотОбъект.НомерОчереди=0,"",ЭтотОбъект.НомерОчереди) 
...
для каждого стр1 из ИнфоПоездки цикл
	...
	+"_"+СокрЛП(стр1.ЗаказПокупателя.АдресДоставки)+"-"+строка(стр1.НомерОчереди);//+++ 03.11.2017 с учетом очереди!
	...
	СтруктураОднойОчереди = новый Структура;
	СтруктураОднойОчереди.Вставить("НомерОчереди", Ном);
