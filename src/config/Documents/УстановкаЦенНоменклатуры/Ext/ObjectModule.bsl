...
//+++ 07.09.2017 - 23.10.2018 как рекоментация/ЗАПРЕТ! с 17-00...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение тогда
		
		Если НЕ (ТипыЦен.Количество()=1 и ТипыЦен[0].ТипЦен.Код = "00001") тогда // 24.10.2018  исключение для Закупочной цены
			...
			//до 8-00 проводятся автоматами!
			Если (  (ТекущееВремя1>8*3600 и ТекущееВремя1<16.5*3600) или ТекущееВремя1>18.34*3600 ) тогда //с 16:30:00 по 18:20:24
				Если ЭтоНеНовыеТовары() тогда //17.12.2018
					#Если Клиент тогда
				...
				Иначе
					#Если Клиент тогда
					Сообщить("Новые цены разрешено проводить в любое время!", СтатусСообщения.Информация);
				    #КонецЕсли
	            КонецЕсли;
			КонецЕсли;
		КонецЕсли;	
...
//+++ 17.12.2018 Новые товары, цен не было?!  проверка и в Форме и в Модуле!
функция ЭтоНеНовыеТовары() экспорт 
	Запрос = Новый Запрос;
	...
	|			Номенклатура В (&масТов)
	|				И ТипЦен В (&масЦен)) КАК ЦеныНоменклатурыСрезПоследних
	|ГДЕ
	|	ЦеныНоменклатурыСрезПоследних.Цена > 0";
	Запрос.УстановитьПараметр("ДатаДок", НачалоДня(Дата)-1); // за 1 сек до нач.документа
	 масТов = Товары.ВыгрузитьКолонку("Номенклатура");
	 Запрос.УстановитьПараметр("масТов", масТов);
	 масЦен = ТипыЦен.ВыгрузитьКолонку("ТипЦен");
	 Запрос.УстановитьПараметр("масЦен", масЦен);
