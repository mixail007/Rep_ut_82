		...
		Если не отказ Тогда
			...
			//		ц=0;
				//	Для Каждого Подразделение из Подразделения Цикл
					//	ц=ц+1;
						СуммаПодразделения = Стр["Месяц"+й+"Сумма"];
	...
	Иначе 
		...
		Если не отказ Тогда
			Для каждого стр из ЗатратыМесяц Цикл
				...
				//Для Каждого Подразделение из Подразделения Цикл
				//	ц=ц+1;
					СуммаПодразделения = Стр["Месяц1"+"СуммаПодразделение"];
					Если СуммаПодразделения<>0 Тогда 
	    ...
	    Если не отказ Тогда
			Для каждого стр из Затраты Цикл
				...
				Для й=1 по 6 Цикл
					...
					//Для Каждого Подразделение из Подразделения Цикл
					//	ц=ц+1;
						СуммаПодразделения = Стр["Месяц"+й+"СуммаПодразделение"];
						Если СуммаПодразделения<>0 Тогда 
							СтрокаДвижений = ТаблицаДвижений.Добавить();
							СтрокаДвижений.Подразделение = стр.Подразделение; 
...
Процедура ДвиженияПоРегистру()
	...
	Для каждого стр из Затраты Цикл
		...
		для ии = 1 по КоличествоМесяцев Цикл			
			СоздатьПроводку(ии,Датаии,стр,НаборДвижений,"Сумма",стр.Подразделение);
			//СоздатьПроводку(ии,Датаии,стр,НаборДвижений,"",Справочники.Подразделения.НайтиПоКоду("00133"));
			//СоздатьПроводку(ии,Датаии,стр,НаборДвижений,"",Справочники.Подразделения.НайтиПоКоду("00133"));
			//СоздатьПроводку(ии,Датаии,стр,НаборДвижений,"",Справочники.Подразделения.НайтиПоКоду("00133"));
			//СоздатьПроводку(ии,Датаии,стр,НаборДвижений,"",Справочники.Подразделения.НайтиПоКоду("00133"));
			Датаии = ДобавитьМесяц(Датаии,1);
