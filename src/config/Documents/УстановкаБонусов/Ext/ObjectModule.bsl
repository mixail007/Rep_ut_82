...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = ЛОЖЬ, Форма = Неопределено) Экспорт
	...
	//06.02.2018 ---- сразу на принтер - ничего тут нет ----
	Если параметрыСеанса.ТекущийПользователь = Ответственный.НаправлениеПродаж.Руководитель //рукНапр
		или параметрыСеанса.ТекущийПользователь = НоменклатурнаяГруппа.Ответственный 	   //рукТГ
		или РольДоступна("ПолныеПрава")                                                    //Бух
		или параметрыСеанса.ТекущийПользователь.ОсновноеПодразделение = справочники.Подразделения.НайтиПоКоду("00007") //Фин.отдел
		...
		иначе 
			Если НЕ (состояние = Перечисления.СостоянияОбъектов.Утвержден
				...
				ВОЗВРАТ;//17.12.2018 - запрет!
			иначе	//сразу на принтер! без редактирования!   //17.12.2018 - спросимм...
				Если Вопрос("Документ будет сразу отправлен на печать! Продолжить?", РежимДиалогаВопрос.ДаНет)<>КодВозвратаДиалога.Да тогда
		        	Возврат;
				КонецЕсли;
				НаПринтер = Истина;
			КонецЕсли;
	...
	Если Бонус = 1  тогда //ОТК ------------Движение на Дату документа!---------------------
		...
		Для Каждого стр из Бонусы Цикл
			Если стр.Статус <> перечисления.СтатусыСтрокЗаказа.Отменен Тогда
				...
				НовЗапись.Период = НачалоДня(Дата); // +++ 31.05.2018  - с начала дня должно быть (для выплат - это ВАЖНО!)
...
процедура ПроверитьУникальностьОТК(Отказ, Кол)
	...
	// |				И НоменклатурнаяГруппа = &НоменклатурнаяГруппа
	 ...
	 Запрос.УстановитьПараметр("ОТК", Истина );
...
процедура ПроверитьУникальностьБонуса(Отказ, Кол)
	...
	|				И ТипБонуса = &ТипБонуса) КАК БонусыКонтрагентовОстатки";
	 ...
	 Запрос.УстановитьПараметр("Контрагент", Контрагент);
	 Запрос.УстановитьПараметр("НоменклатурнаяГруппа", НоменклатурнаяГруппа);
	 Запрос.УстановитьПараметр("ОТК", ЛОЖЬ );
	...
	Если не Отказ тогда
		 ...
		 Если Выборка.Следующий() тогда
			 ...
			 #Если Клиент тогда
				 ...
				 +" ("+строка(ТипБонуса)+") только по договорам"+?(нал,"(*)","(б/н)") , СтатусСообщения.Внимание);
			#КонецЕсли	
 		 КонецЕсли;	
	КонецЕсли;
