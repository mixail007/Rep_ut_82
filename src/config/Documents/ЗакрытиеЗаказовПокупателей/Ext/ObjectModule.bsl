...
Процедура ЗаполнитьЗаказыПоОстаткамНаДатуУпр(Контр=неопределено) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   |//Контр ДоговорКонтрагента.Владелец = &Контр
...
Процедура ЗаполнитьЗаказыПоПросроченнымРезервамУпр() Экспорт
	...
	Запрос.УстановитьПараметр("Д14", НачалоДня(ТекущаяДата()) - 10 *86400);           //+++ 10.04.2019
...
//----------------своё закрытие от 1 до 99 дней и для этих клиентов >> СписокКонтрагентовБезЗакрытия ---------------------
	...
	Запрос7.Текст = "ВЫБРАТЬ
	   ...
	   |	вт.Контрагент,
	   //16.02.2018
	   | 	вт.ЧислоДней как ЧислоДнейДоЗакрытия,
	   ...
	   //16.02.2018 --- вырезает клиентов!
	   //|	И РазностьДат(ЗакПок.ЗаказПокупателя.Дата, &ТекДата, День) > вт.ЧислоДней
	   |";
		...
		для каждого стр7 из табл7 Цикл
			Если стр7.ЧислоДней > стр7.ЧислоДнейДоЗакрытия тогда //16.02.2018 --- закрываются только те, что превышают
			НоваяСтрока = Заказы.Добавить();
	...
	//клиенты по которым уже сделали своё закрытие - и заказы, которые НЕ надо закрывать - по списку Клиентов!
...
//+++)	
	...
	//+++( 14.02.2018 - Клиенты Резерва ИМ - резервные Заказы НЕ закрываются НИКОГДА!-----------------------
	...
	СписокКонтрагентовБезЗакрытия.Добавить(выборка1.ссылка);//Резерв для ИМ
	КонецЦикла;
    //+++)
	
	Запрос.УстановитьПараметр("СписокКонтрагентовБезЗакрытия", СписокКонтрагентовБезЗакрытия);  // оперативно текущей датой
	
	//12.10.18 Смирнов Задача 55059 //Заказы клиентов собранные через "Перемещение" не должны закрываться.
	СписокЗаказовИсключитьИзЗакрытия = ПолучитьСписокЗаказовСАксамиПоКоторымЕстьПеремещения();
...
//16.11.2016 --- для сторонних поставщиков определяем по статусу самого заказа------------
	...
	Запрос.УстановитьПараметр("СписокЗаказовИсключитьИзЗакрытия", СписокЗаказовИсключитьИзЗакрытия);
...
// 08.06.2017 - условия на Договоры и ВСЕ заказы - уже есть в ВТ_Заказы!
    //12.10.18 Смирнов Задача 55059 //Заказы клиентов собранные через "Перемещение" не должны закрываться.
	|		НЕ ЗаказПокупателя В (&СписокЗаказовИсключитьИзЗакрытия) и 
...
//+++ 11.11.2015 - закрываем ВСЕ заказы с неполной предоплатой!  по согласованию с Гороховым, Малышевым 
	...
	//  отменили по задаче Бондаренко 44976. 08.05.18  заказы типа 4 должны закрываються по общей схеме
	//+++ 10.04.2019 ВКЛЮЧИЛИ Задача № 66146 - Бондаренко (не более 4х раб.дней!?) в 4 утра субботы - закрываем понедельник!
	
	//*** По интернет-магазинам держим не более 3 рабочих дней
	Запрос = Новый Запрос;
	
	//держим заказы N рабочих дня
	Nдней = 4; // 3 >> 4 дня 10.04.2019 - БЕЗ учетавыходных!
	ЧислоДней = Nдней;	
	////----С учётом выходных - прибавляется 1 - 2 дня-----
	//НомерДня = ДеньНедели(ТекущаяДата());
	//Если НомерДня >=1 И НомерДня <= Nдней Тогда
	//	ЧислоДней = Nдней + 2;//с учётом 2 выходных
	//ИначеЕсли НомерДня >Nдней И НомерДня <=6 Тогда 
	//	ЧислоДней = Nдней;
	//ИначеЕсли НомерДня = 7 Тогда // 1 день на субботу
	//	ЧислоДней = Nдней+1;
	//Иначе // быть такого не может...тогда 7+2
	//	ЧислоДней = 9;
	//КонецЕсли;
	...
	//10.04.2019
	Запрос.УстановитьПараметр("Группа11",     справочники.КатегорииПокупателей.ИнтернетМагазин );
	...
	Запрос.Текст ="ВЫБРАТЬ
				...
				//10.04.2019 было  |				И ЗаказПокупателя.ТипЗаказа = 4
				   |				И ДоговорКонтрагента.Владелец.КатегорияПокупателя = &Группа11
				  ...
				  |				И ЗаказПокупателя.Подразделение = &ГоловноеПодразделение
...
Процедура ЗаполнитьЗаказыПоОтрицательнымОстаткам() Экспорт
   ...
   |			и НЕ ДоговорКонтрагента.Владелец в (&СписокКонтрагентовБезЗакрытия)  ) как ЗакПокОстатки
   | ГДЕ
 ...
 //| 	УПОРЯДОЧИТЬ ПО ЗаказПокупателя.Дата
   |";
   	...
   	//+++( 18.07.2018 ++++++++ исключение любых "+"  "движений" по резерву 11 группы - "увеличивается" более допустимого!
	СписокКонтрагентовБезЗакрытия = новый СписокЗначений;
	СписокКонтрагентовБезЗакрытия.Добавить( справочники.Контрагенты.НайтиПоКоду("П015036") );
	Запрос.УстановитьПараметр("СписокКонтрагентовБезЗакрытия", СписокКонтрагентовБезЗакрытия);
    //+++)
...
//13.01.2016  - 12.11.2018
функция ПодготовитьТаблицуПоРезервамИМ()
 	СписокЗаказов = Заказы.ВыгрузитьКолонку("ЗаказПокупателя");
	
	Запрос = Новый Запрос;
	//+++ 12.11.2018 - без повторов товара!
	...
	//++++++++++++++++++ СТАРАЯ СХЕМА - для клиентов ИМ +++++++++  
				   ...
				   |	ЗаказыПокупателейОстатки.ЗаказПокупателя
					   |ИЗ
				   |	РегистрНакопления.ЗаказыПокупателей.Остатки(&МоментДокумента,
	...
	//+++ 12.11.2018 -----НОВАЯ СХЕМА!----------------------------------------------	   
				   ...
				   |ВЫБРАТЬ
                       ...
                       |ИЗ
                       |	РегистрНакопления.ЗаказыПокупателей.Остатки(&МоментДокумента, ЗаказПокупателя В (&СписокЗаказов)) КАК ЗаказыПокупателейОстатки
                       |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.РезервРазрешения.СрезПоследних(&МоментДокумента, ) КАК РезервРазрешенияСрезПоследних
                       |		ПО ЗаказыПокупателейОстатки.ДоговорКонтрагента.Владелец = РезервРазрешенияСрезПоследних.Контрагент
                       |			И ЗаказыПокупателейОстатки.Номенклатура.ВидТовара = РезервРазрешенияСрезПоследних.ВидТовара
					   
                       |			И (ЗаказыПокупателейОстатки.Номенклатура.НоменклатурнаяГруппа = РезервРазрешенияСрезПоследних.НоменклатурнаяГруппа
	...
	Если ТаблицаПоЗаказамПокупателей.Количество() > 0 Тогда
		...
		Для Каждого СтрЗП из ТаблицаПоЗаказамПокупателей Цикл
		  //Если СтрЗП.ДоговорКонтрагента.Владелец  =  СтрЗП.ДоговорКонтрагента.Владелец.КонтрагентДляРезерваИМ и СтрЗП.ЗаказПокупателя.Проверен Тогда //123заменить
			Если СтрЗП.ДоговорКонтрагента.Владелец  =  СтрЗП.ДоговорКонтрагента.КонтрагентДляРезерваИМ и СтрЗП.ЗаказПокупателя.Проверен Тогда  //резервые заказы!
		...
		Движения.РезервДляИМ.ВыполнитьРасход();//Закрываем + резерв по резервным клиентам! 
	КонецЕсли;
...
Функция ПолучитьСписокЗаказовСАксамиПоКоторымЕстьПеремещения()
	Запрос = новый Запрос;
	Запрос.Текст="ВЫБРАТЬ
	             |	ЗаказыПокупателейОстатки.ЗаказПокупателя
	             |ПОМЕСТИТЬ втЗаказы
	             ...
	             |				И ЗаказПокупателя.Дата > &Месяц
	             |				И ЗаказПокупателя.Подразделение = &Головное) КАК ЗаказыПокупателейОстатки
	             |;
	             |
	             |////////////////////////////////////////////////////////////////////////////////
	             |ВЫБРАТЬ
	             ...
	             |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВнутреннийЗаказ КАК дВнутреннийЗаказ
	             |			ВНУТРЕННЕЕ СОЕДИНЕНИЕ втЗаказы КАК втЗаказы
	             |			ПО дВнутреннийЗаказ.ДокументОснование = втЗаказы.ЗаказПокупателя
	             |		ПО ПеремещениеТоваров.ВнутреннийЗаказ = дВнутреннийЗаказ.Ссылка";
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
    ...
    ТаблицаПоРезервамИМ =  Новый ТаблицаЗначений;//ничего назад в резерв не возвращаем ?!
