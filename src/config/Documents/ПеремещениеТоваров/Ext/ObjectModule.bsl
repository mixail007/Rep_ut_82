...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ОтчетПроизводстваЗаСмену") тогда
		...
		СкладОтправитель   = Основание.СкладГотовойПродукции;
		СкладПолучатель    = Справочники.Склады.НайтиПоКоду("12278");//Ангар-ковка готовые
		Если ЗначениеЗаполнено(Основание.Задание) тогда
		  Комментарий        = "Задание " + Основание.Задание.Номер+ " от " + Основание.Задание.Дата;
        КонецЕсли;
		Для Каждого СтрокаТЧ Из Основание.ГотоваяПродукция Цикл			
			СтрДоб = Товары.Добавить();
			СтрДоб.Номенклатура = СтрокаТЧ.Номенклатура;
			СтрДоб.Вес          = ПолучитьВесНетто(СтрокаТЧ.Номенклатура)*СтрокаТЧ.Количество;
			СтрДоб.Качество     = справочники.Качество.Новый;  
			СтрДоб.Склад        = Справочники.Склады.НайтиПоКоду("12278");
			СтрДоб.Количество   = СтрокаТЧ.Количество;
			СтрДоб.ЕдиницаИзмерения = СтрДоб.Номенклатура.ЕдиницаХраненияОстатков;
			СтрДоб.Коэффициент = 1;
			СтрДоб.Вес = СтрокаТЧ.Номенклатура.ЕдиницаХраненияОстатков.Вес * СтрокаТЧ.Количество;
