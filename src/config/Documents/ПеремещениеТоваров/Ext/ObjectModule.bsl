...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ОтказБрак = ПроверитьНаПеремещениеБрака(Отказ);
	
	// Движения по документу
	Если Не Отказ и не ОтказБрак Тогда
...
Процедура ПриЗаписи(Отказ)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ГДЕ
	               |	СоответствиеКонтрагентовСкладамФРан.Склад В(&СкладыПолучатели)";
				   Запрос.УстановитьПараметр("СкладыПолучатели",Товары.ВыгрузитьКолонку("Склад"));
				   Рез = Запрос.Выполнить().Выбрать();
	
	Если Рез.Количество()>0 и СкладОтправитель.ЗапретитьИспользование = ложь тогда
