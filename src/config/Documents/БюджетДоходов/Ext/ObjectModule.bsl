...
Процедура ДвиженияПоставкиВРегистр(НаборДвижений)
	...
	ПоступленияСвернуть.Свернуть("Менеджер,разрез1,разрез2,разрез3","месяц1Количество,Месяц1Сумма,месяц2Количество,Месяц2Сумма,
	...
	Для каждого стр из Дивиденды Цикл
		...
		для ии = 1 по КоличествоМесяцев Цикл    //считаем что продаем услуг столько же, сколько за них получаем денег,
			...
			Если стр.ВидДвижения = Перечисления.ВидыДвиженийПриходРасход.Расход тогда
				...
				СтрокаДвижений.СЧЕТКТ = ПланыСчетов.Хозрасчетный.РасчетныеСчета;
				СтрокаДвижений.СЧЕТДТ = ПланыСчетов.Хозрасчетный.ПрибылиИУбытки;
			...
			иначе
				...
				СтрокаДвижений.СЧЕТДТ = ПланыСчетов.Хозрасчетный.РасчетныеСчета;
				СтрокаДвижений.СЧЕТКТ = ПланыСчетов.Хозрасчетный.ПрибылиИУбытки;
...
Запрос = Новый запрос;
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ЕСТЬNULL(-ХозрасчетныйОбороты.СуммаОборот, 0) + ЕСТЬNULL(ХозрасчетныйОбороты1.СуммаОборот, 0) + ЕСТЬNULL(ХозрасчетныйОбороты2.СуммаОборот, 0) + ЕСТЬNULL(ХозрасчетныйОбороты3.СуммаОборот, 0) + ЕСТЬNULL(ХозрасчетныйОбороты4.СуммаОборот, 0) КАК Сумма,
	               ...
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.Обороты(, , Авто, Счет = &ОС, , ПериодПланирования = &ПериодПланирования, , ) КАК ХозрасчетныйОбороты4
	               |		ПО ХозрасчетныйОбороты.ПериодМесяц = ХозрасчетныйОбороты4.ПериодМесяц
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	Период";
