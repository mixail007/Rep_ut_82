...
//27.07.2018 Процедура выполняет заполниение табличной части по инвентаризации товаров.
//
// Параметры:
//  ДокументОснование - ссылка на документ основание (инвентаризация товаров).
//
Процедура ЗаполнитьТоварыПоИнвентаризацииТоваровУпрРаспределения(ДокументОснование)
...
#КонецЕсли
	...
	|	Док.Ссылка.Склад                              КАК Склад,
	// + потратили на производство, поэтому надо списать со складов!
	|	МАКСИМУМ(Док.Количество) КАК КоличествоОтклонение,
	|	ВЫБОР КОГДА СУММА(ВложенныйЗапрос.Количество) ЕСТЬ NULL 
	|		ТОГДА 0
	...
	|	Значение(Справочник.Качество.Новый) Качество
	|ИЗ
	...
	Если Товары.Количество() = 0 Тогда
		Сообщить("В документе """ + ДокументОснование.Метаданные().Представление() + """ № " + ДокументОснование.Номер +" отсутствуют Материалы для списания.");
	КонецЕсли;

	#КонецЕсли

КонецПроцедуры // ЗаполнитьТоварыПоИнвентаризацииТоваровУпрРаспределения()
...
Процедура ОбработкаЗаполнения(Основание)
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.РаспределениеЗатратНаВыпускГотовойПродукции") Тогда //27.07.2018

		ИнвентаризацияТоваровНаСкладе = Основание;  // добавлен тип
		
		ЗаполнитьШапкуДокументаПоОснованию(ЭтотОбъект, Основание); //стандартная

		ЗаполнитьТоварыПоИнвентаризацииТоваровУпрРаспределения(Основание); //измененная
		
		Комментарий = "+ доп.списание на производство";
