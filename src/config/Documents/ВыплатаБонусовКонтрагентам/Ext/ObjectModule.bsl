...
#Если Клиент тогда
	...
	Если НЕ (ЭтотОбъект.Состояние = перечисления.СостоянияОбъектов.Утвержден 
			или ЭтотОбъект.Состояние = перечисления.СостоянияОбъектов.Согласован   //07.02.2018
			ИЛИ РольДоступна("ПравоЗавершенияРаботыПользователей") ) тогда
		Предупреждение("Документ ещё не Утвержден! Нельзя печатать документ!", 30);
		Возврат;
	КонецЕсли; 
...
#КонецЕсли
	...
	ИначеЕсли ТипЗнч(ИмяМакета) = Тип("СправочникСсылка.ДополнительныеПечатныеФормы") Тогда
		...
		//============================Печать ещё и Доп.Соглашения по Лояльности=====================
		Если ЭтотОбъект.ТипБонуса = перечисления.ТипыБонусов.Лояльность и НЕ Основание.Пустая() тогда
			ИмяФайла = КаталогВременныхФайлов()+"PrnForm2.tmp";
			ОбъектВнешнейФормы = справочники.ДополнительныеПечатныеФормы.НайтиПоНаименованию("Соглашение Лояльность").ПолучитьОбъект();
					
			ДвоичныеДанные = ОбъектВнешнейФормы.ХранилищеВнешнейОбработки.Получить();
			ДвоичныеДанные.Записать(ИмяФайла);
			Обработка = ВнешниеОбработки.Создать(ИмяФайла);
			Обработка.СсылкаНаОбъект = Основание;  // Установка!
			ТабДокумент = Обработка.Печать();
		КонецЕсли;
