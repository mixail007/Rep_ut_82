...
Функция ПолучитьСтоимостьНоменклатурыВозврат(ТабТовары)
	...
	Запрос.Текст="ВЫБРАТЬ
			...
			//15.01.2016 ---- на возврат не более, чем есть "+" остаток
	             |	Выбор когда ЗаявкаНаВозвратТоваровТовары.Количество>ЕстьNull(Продажи.Количество,0)
...
N = СписокРеализаций.Количество();
	...
	|	КОНЕЦ КАК Цена
	|ИЗ
	|	РегистрНакопления.Продажи.Обороты(&НачДата, , ,
	|			ДокументПродажи В (&СписокРеализаций)
	|				И Номенклатура В (&СписокНом)) КАК ПродажиОбороты
	|
	|СГРУППИРОВАТЬ ПО
