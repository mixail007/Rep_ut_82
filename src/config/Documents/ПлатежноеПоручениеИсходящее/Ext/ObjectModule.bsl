...
Процедура ЗаполнитьПоЗаказуУпр(ДокументОснование, СтрокаПлатеж)
	...
	|ИЗ
	|	РегистрНакопления.ВзаимоРасчетыСКонтрагентами.Остатки(, Сделка = &Заказ) КАК ВзаимоРасчетыСКонтрагентамиОстатки";
...
Процедура ЗаполнитьПоЗаявкеЯШТ(ПерезаполнятьРасшифровкуПлатежа = Ложь)  Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ЕСТЬNULL(ЗаявкаНаРасходованиеСредствРасшифровкаПлатежа.СтатьяЗатрат, ЗНАЧЕНИЕ(справочник.статьиЗатрат.пустаяссылка)) КАК СтатьяЗатрат,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПеречислениеНалога 
			...
			//
			КПППлательщика		 = ?(ЗначениеЗаполнено(ДокументОснование.ПодразделениеКПП), ДокументОснование.ПодразделениеКПП.КПП, Организация.КПП);
		КонецЕсли;
		...
		СтрокаПлатеж.СуммаПлатежаПлан = Выборка.СуммаВзаиморасчетов; //Выборка.СуммаПлатежа;  ***в валютных платежках вставали рубли
	...
	Если ТЗсчетов.Количество() > 0 И СокрЛП(Контрагент.Код) = "93345" Тогда
		...
		Для Каждого ТЗсчетовСтр из ТЗсчетов Цикл
			ТЗсчетовСтр.СуммаСчетаНДС = Окр(ТЗсчетовСтр.СуммаСчета*чНДС/(100+чНДС),2,1);
		КонецЦикла;
		Если ТЗсчетов.Итог("СуммаСчетаНДС")<>РасшифровкаПлатежа.Итог("СуммаНДС") Тогда
			...
			Для Каждого РасшифровкаПлатежаСтр из РасшифровкаПлатежа Цикл
				Если ТипЗнч(РасшифровкаПлатежаСтр.Сделка)=Тип("ДокументСсылка.ПоступлениеДопРасходов") Тогда
					РасшифровкаПлатежаСтр.СтавкаНДС = ставкаНДС_2019; //28.12.2018
					ТекНомСчета = РасшифровкаПлатежаСтр.Сделка.НомерСчета;
					НайденнаяСтрока  = ТЗсчетов.Найти(ТекНомСчета,"НомерСчета");
					Если НайденнаяСтрока <> Неопределено Тогда
						РасшифровкаПлатежаСтр.СуммаНДС = макс(0,мин(Окр(РасшифровкаПлатежаСтр.СуммаПлатежа*чНДС/(100+чНДС),2,1), НайденнаяСтрока.СуммаСчетаНДС));
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, Отказ, Заголовок)
	...
	Если Оплачено И ОтраженоВОперУчете Тогда  // Проводим по фактическим взаиморасчетам
		...
		Если РасчетыСКонтрагентами Тогда
			...
			Для Каждого СтрокаПлатеж ИЗ ДвиженияПоКонтрагентам Цикл
				...
				СтрокаДвижений.СуммаУпр            = СтрокаПлатеж.СуммаВзаиморасчетов*КоэффициентСторно;
					иначе
				СтрокаДвижений.СуммаВзаиморасчетов = СтрокаПлатеж.СуммаВзаиморасчетов*КоэффициентСторно;
	...
	Если ОтражатьВЗатратах Тогда		
		...
		Если (ТаблицаЗатрат.Количество() > 0) И Контрагент2 =  Справочники.Контрагенты.ПустаяСсылка() Тогда 
			...
			КонецЦикла;		
		Иначе 
			Если (Подразделение <> Справочники.Подразделения.ПустаяСсылка() И Контрагент2 =  Справочники.Контрагенты.ПустаяСсылка()) и ВидОперации <> Перечисления.ВидыОперацийППИсходящее.ОплатаПоставщикуСКодомКБК Тогда 
				НаборДвижений = Движения.Затраты.Добавить();
