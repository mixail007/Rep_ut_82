			...
			Для Каждого СтрокаРазмещения Из СтрокиПоРазмещению Цикл
				...
				Если КолвоПоРазмещению > 0 Тогда
					...
					Если    СписатьПоРазмещению = КолвоПоЗаказу - ВсегоСписано Тогда   //последнее списание этой номенклатуры
						Если  СтрокаЗаказа.цена* СтрокаЗаказа.КоличествоОстатокПоЗаказу <> СтрокаЗаказа.сумма Тогда
							СтрокаТабличнойЧасти.Сумма = СтрокаТабличнойЧасти.Сумма+(СтрокаЗаказа.сумма-СтрокаЗаказа.цена* СтрокаЗаказа.КоличествоОстатокПоЗаказу);
						конецЕсли;
					КонецЕсли;
...
Процедура ОбработкаЗаполнения(Основание)
	...
	УчитыватьНДС = Истина; // для ОТК - да, для Бонусов - НЕТ !
	...
	статьяБонус = справочники.СтатьиЗатрат.НайтиПоКоду("О0215");
		...
		для каждого стр1 из табл1 цикл
			...
			стрУсл.СтавкаНДС = перечисления.СтавкиНДС.НДС18;
			стрУсл.СуммаНДС = Окр(стрУсл.Сумма *18/118, 2);
		КонецЦикла;	
...
//}}MRG[ <-> ]
	...
	//Литые от китайцев сразу помещаем в запрещенные
	Если не отказ и ЭтоНовый() и ДоговорКонтрагента.ВалютаВзаиморасчетов = Справочники.Валюты.НайтиПоКоду("840") и (Подразделение = Справочники.Подразделения.НайтиПоКоду("00005") или Подразделение = Справочники.Подразделения.ПустаяСсылка()) тогда
		Литые = Справочники.НоменклатурныеГруппы.НайтиПоКоду("00026");
		для каждого товар из Товары цикл
			Если товар.Номенклатура.НоменклатурнаяГруппа = Литые тогда
...
Функция СверкаЦенНаДискиСЗаказомПоставщику()
	...
	Если НЕ Результат.Найти(Контрагент) = Неопределено Тогда  //по контрагенту есть сезонныее заказы, значит, это китайский поставщик
		...
		Запрос.Текст = "ВЫБРАТЬ
		               |	ЗаказПоставщикуТовары.Цена - ПоступлениеТоваровУслугТовары.Цена КАК РазницаВЦене,
		               |	ЗаказПоставщикуТовары.Номенклатура
		               |ИЗ
		               |	Документ.ЗаказПоставщику.Товары КАК ЗаказПоставщикуТовары
		               ...
		               |	И ПоступлениеТоваровУслугТовары.Ссылка = &ПоступлениеТоваров";
		Результат = Запрос.Выполнить().Выбрать();
		Пока Результат.Следующий() Цикл
			Если Результат.РазницаВЦене <> 0 И Результат.Номенклатура.ВидТовара = Перечисления.ВидыТоваров.Диски Тогда
				Сообщить("Для номенклатуры " + Результат.Номенклатура.Код + " цена в Поступлении отличается от цены в Заказе поставщику");
