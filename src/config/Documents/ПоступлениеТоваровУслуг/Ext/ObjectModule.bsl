...
Функция СверкаЦенНаДискиСЗаказомПоставщику()
	...
	Если НЕ Результат.Найти(Контрагент) = Неопределено Тогда  //по контрагенту есть сезонныее заказы, значит, это китайский поставщик
		...
		//+++( 04.02.2019 - сразу ищет только разницу			   
					   |	И ЗаказПоставщикуТовары.Цена - ПоступлениеТоваровУслугТовары.Цена <> 0
					   |";
		...
		Пока Результат.Следующий() Цикл
			Если //Результат.РазницаВЦене <> 0 И   // уже в запросе есть отбор
				...
				Сообщить("Для товара с кодом: " + Результат.Номенклатура.Код + " - цена в Поступлении отличается от цены в Заказе поставщику на "+формат(Результат.РазницаВЦене,"ЧДЦ=2"), СтатусСообщения.Внимание);
