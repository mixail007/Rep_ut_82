...
//+++( 22.08.2018 - контроль копеек и граммов - "списываем" до 0
таблМатКонтроль = ЭтотОбъект.Материалы.Выгрузить();
таблМатКонтроль.Колонки.Добавить("Док");
таблМатКонтроль.Колонки.Добавить("Доля");
для каждого стр1 из таблМатКонтроль цикл
	стр1.Доля = 0;
КонецЦикла;	
таблЗатратКонтроль = ЭтотОбъект.ДопЗатраты.Выгрузить();
таблЗатратКонтроль.Колонки.Добавить("Док");
таблЗатратКонтроль.Колонки.Добавить("Доля");
для каждого стр1 из таблЗатратКонтроль цикл
	стр1.Доля = 0;
КонецЦикла;	
//+++)
...
Для каждого стр1 из таблДок цикл
	...
	Для каждого стрЗатрат из ЭтотОбъект.ДопЗатраты цикл
		...
		стрКонтроль = таблЗатратКонтроль.НайтиСтроки(новый Структура("СтатьяЗатрат, СтатьяЗатратУПР",стрЗатрат.СтатьяЗатрат, стрЗатрат.СтатьяЗатратУПР) );
	...
	//+++ 22.08.2018  запоминаем документ с максимальной долей!
		стрКонтроль[0].Сумма = стрКонтроль[0].Сумма - стрЗатрат1.Сумма; //контроль
		Если доля1>стрКонтроль[0].Доля тогда //запоминаем документ с максимальной долей!
			стрКонтроль[0].Доля = доля1;
			стрКонтроль[0].Док = док1;
		КонецЕсли;
...
// а уже внутри выпуска - по количеству в спецификации на все товары готовой продукции
	Для каждого стрМ из ЭтотОбъект.Материалы цикл
		...
		стрКонтроль = таблМатКонтроль.найтиСтроки(новый Структура("Номенклатура", стрМ.Номенклатура) );
		...
		стр1.Количество = ОКР(стрМ.Количество * доля1,3); //до Грамм
		стр1.Сумма      = ОКР(стрМ.Сумма * доля1,2); // до копеек
		
	//+++ 22.08.2018  запоминаем документ с максимальной долей!
		стрКонтроль[0].Сумма = стрКонтроль[0].Сумма - стр1.Сумма;
		стрКонтроль[0].Количество = стрКонтроль[0].Количество - стр1.Количество;
		Если доля1>стрКонтроль[0].Доля тогда 
			стрКонтроль[0].Доля = доля1;
			стрКонтроль[0].Док = док1;
		КонецЕсли;
	...
	ПровестиДок( док1 ); // ВыпускПродукции
	
КонецЦикла;	 //-------------------------------------


//+++ 22.08.2018 контроль копеек/грамм после распределения по каждому товару!
для каждого стрМ из таблМатКонтроль цикл
	Если стрМ.Количество<>0 или стрМ.Сумма<>0 тогда
		док1 = стрМ.Док;   //04.10.2018 - уже объект!
		строки1 = док1.Материалы.найтиСтроки(новый Структура("Номенклатура, Распределяется", стрМ.Номенклатура, Истина) ); 
		строки1[0].Количество = строки1[0].Количество + стрМ.Количество;
		строки1[0].Сумма      = строки1[0].Сумма + стрМ.Сумма;
	    ПровестиДок( док1 );
	КонецЕсли;
КонецЦикла;

//+++ контроль копеек распределения затрат по каждой строке!
для каждого стрЗатрат из таблЗатратКонтроль цикл
	Если стрЗатрат.Сумма<>0 тогда
		док1 = стрЗатрат.Док; //04.10.2018 - уже объект!
		строки1 = док1.ДопЗатраты.найтиСтроки(новый Структура("СтатьяЗатрат, СтатьяЗатратУПР",стрЗатрат.СтатьяЗатрат, стрЗатрат.СтатьяЗатратУПР) );
 		строки1[0].Сумма      = строки1[0].Сумма + стрЗатрат.Сумма;
     	ПровестиДок( док1 );
	КонецЕсли;
КонецЦикла;

//2) ----------перепроведение реализаций и Перемещений (готовой продукции)----------------------
...
функция ПолучитьРеализацииЗаПериодПоГотПродукции()
 мас = новый массив;
	 ...
	 Запрос.Текст = "ВЫБРАТЬ
	                ...
	                |	И РеализацияТоваровУслугТовары.Номенклатура В (ВЫБРАТЬ вт.Номенклатура ИЗ ВТ_Товары КАК вт)
	                |
	                |ОБЪЕДИНИТЬ ВСЕ
	                |
	                |ВЫБРАТЬ РАЗЛИЧНЫЕ
	                ...
	                |	И ПеремещениеТоваровТовары.Номенклатура В (ВЫБРАТЬ вт.Номенклатура ИЗ ВТ_Товары КАК вт)
					//16.08.2018
					|
	                |УПОРЯДОЧИТЬ ПО
	                |	Дата
