		...
		Если IsFormula = Ложь  Тогда
			...
			Если Success Тогда
				...
				Если СтрУ.Реализация <> Неопределено и СтрУ.Реализация <> Документы.РеализацияТоваровУслуг.ПустаяСсылка()
					...
					//***2018.11.29 Проверяем, не превышает ли количество в уже созданных заявках на брак количество в реализации
					Запрос = Новый Запрос;
					Запрос.УстановитьПараметр("Контрагент", Контрагент);
					Запрос.УстановитьПараметр("Реализация", СтрУ.Реализация);
					Запрос.УстановитьПараметр("Номенклатура", СтрУ.Номенклатура);
					Запрос.Текст = "ВЫБРАТЬ
					|	А.Номенклатура,
					|	А.Реализация,
					|	СУММА(РеализацияТоваровУслугТовары.Количество) КАК КоличествоПродано,
					...
					|		ЗаявкаНаБракРасшифровка.Реализация КАК Реализация
					|	ИЗ
					...
					|		И ЗаявкаНаБракРасшифровка.Реализация = &Реализация
					|		И ЗаявкаНаБракРасшифровка.Номенклатура = &Номенклатура
					|	
					|	СГРУППИРОВАТЬ ПО
					|		ЗаявкаНаБракРасшифровка.Номенклатура,
					|		ЗаявкаНаБракРасшифровка.Реализация) КАК А
					...
					Если Результат.Следующий() Тогда
						Если Результат.КоличествоПродано < Результат.КоличествоВЗаявкахНаБрак + СтрУ.Количество Тогда
							Success = Ложь;
							РезультатОперации.Error = Истина;
							РезультатОперации.NumberError = "По товару " + Строка(СтрУ.Номенклатура) + ", купленному по документу " + Строка(СтрУ.Реализация) + ", уже введены заявки на брак.";
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
