	...
	Для каждого СтрВремТЗ ИЗ ВремТЗ Цикл  //перебираем все подразделения заказа для проверки остатков
		...
		Если  ПодразделениеЗаказа<>"00000" Тогда
			...
			Если ПодразделениеЗаказа <> "00005" Тогда  //
				...
				Запрос.Текст = стрЗаменить(Запрос.Текст ,"Склад.Транзитный = ЛОЖЬ",  "Склад.Транзитный");
				Запрос.Текст = стрЗаменить(Запрос.Текст ,"ЗаказПокупателя.Транзит = ЛОЖЬ",  "ЗаказПокупателя.Транзит");
			Иначе	
