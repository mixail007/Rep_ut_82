...
функция СоздатьЗаказ(Partner, PointId, Goods, Comment, ShippingDay, isReserve, TransportCompany,NameOperation,Prepay,IsStar,WayOfDelivery,AddressId,TkId,DeliveryDate2,CaseForLogistik,IsSeasonAdjournment)
	...
	//Набор.Отбор.КодТочки.Установить(PointId);
	...
	Если Success Тогда
		...
		Для каждого СтрВремТЗ ИЗ ВремТЗД Цикл  //перебираем все подразделения заказа для проверки остатков
			...
			Если ПодразделениеЗаказа="00000" Тогда  //СезонныйЗаказПокупателя
				...
				//+++ 27.03.2018 - устанавливаем число дней отсрочки и дату оплаты от даты отгрузки
				Заказ.ЧислоДнейСМоментаОтгрузки = Заказ.ДоговорКонтрагента.ЧислоДнейРезерваБезОплаты; 
			...
			Если ПодразделениеЗаказа<>"00000" тогда // ЗаказПокупателя
				...
				Заказ.ЧислоДнейСМоментаОтгрузки = Заказ.ДоговорКонтрагента.ДопустимоеЧислоДнейЗадолженности;		//+++ 27.03.2018
				Заказ.ДатаОплаты = макс(ТекущаяДата(), Заказ.ДатаОтгрузки) + 86400 * Заказ.ДоговорКонтрагента.ДопустимоеЧислоДнейЗадолженности;
				//если  Заказ.ДоговорКонтрагента.ДопустимоеЧислоДнейЗадолженности <> Заказ.контрагент.ДопустимоеЧислоДнейЗадолженности тогда
				//	Заказ.ВыписатьСпецификацию = Истина;
				//конецЕсли;
					//WayOfDelivery =0 - мы везем, WayOfDelivery =1 самовывоз
                 Заказ.СпособОтгрузки = ?(WayOfDelivery =0,Справочники.СпособыОтгрузкиТовара.НайтиПоКоду("6"),Справочники.СпособыОтгрузкиТовара.НайтиПоКоду("10"));
