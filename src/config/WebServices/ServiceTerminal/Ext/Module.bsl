...
Функция ChengeOrders(DeliveryDate,WayOfDelivery,AddressId,TkId,DayOfWeekToDeliver,OrderGuids)
		URL = "http://37.1.84.50:8080/Terminal";
	Заказы = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(URL, "ArrayOfOrdersGuid"));
	...
	ЗаписьXML = Новый ЗаписьXML;
	ЗаписьXML.ОткрытьФайл(ПутьКФайлу);
	ФабрикаXDTO.ЗаписатьXML(ЗаписьXML, OrderGuids);
	ЗаписьXML.Закрыть();
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если ЧтениеXML.ТипУзла = ТипУзлаXML.КонецЭлемента Тогда
			Если ЧтениеXML.Имя = "OrderGuid" Тогда
				СписокЗаказов.Добавить(Запись.OrderGuid);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	ЧтениеXML.Закрыть();
	...
	для каждого стр из списокЗаказов цикл
			УИД = Новый УникальныйИдентификатор(СокрЛП(Стр.Значение));
	     ЗаказСсылка = Документы.ЗаказПокупателя.ПолучитьСсылку(УИД);
		...
		попытка
			...
			Если DayOfWeekToDeliver = 1 тогда
				...
				Запись.Записать(Истина);
			конецесли;
			 УдачныеЗаказы.Добавить(Стр.Значение);
