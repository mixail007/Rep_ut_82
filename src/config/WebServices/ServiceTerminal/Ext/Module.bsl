	...
	Для каждого СтрВремТЗ ИЗ ВремТЗ Цикл  //перебираем все подразделения заказа для проверки остатков
		...
		Если  ПодразделениеЗаказа<>"00000" Тогда
			...
			Запрос.Текст = "ВЫБРАТЬ
			               ...
			               |							ТОГДА НЕ Номенклатура В (ВЫБРАТЬ
			               ...
			               |						ИНАЧЕ НЕ Номенклатура В (ВЫБРАТЬ
			               |										НоменклатураИмпорт.Номенклатура
			               |									ИЗ	РегистрСведений.НоменклатураИмпорт КАК НоменклатураИмпорт)
			...
			Иначе //---------------------------------------------------------------------------------
				...
				Если НЕ КонтрРезерв.Пустая() тогда //СТАРАЯ СХЕМА!
					...
					результат_new = запрос_old.Выполнить();
					табл_New = результат_new.Выгрузить();
					масТов_new = табл_New.ВыгрузитьКолонку("Номенклатура");	
				КонецЕсли;
				...
				Если КонтрРезерв.Пустая() тогда 
					...
					запрос_new.УстановитьПараметр("СписокНоменклатуры", СписокНоменклатуры);
					результат_new = запрос_new.Выполнить();
				    Если НЕ результат_new.Пустой() тогда
				    	табл_New =результат_new.Выгрузить();
						КонтрРезерв = табл_New[0].КонтрагентРезерв;
						масТов_new = табл_New.ВыгрузитьКолонку("Номенклатура");
					Конецесли;
				КонецЕсли;	
			...
			//Плотников <<<<<
			Запрос.УстановитьПараметр("РазрешитьДопДиски",  Контрагент.Экспорт или Контрагент.ВыгружатьСерыйСписокВTerminal); //10.10.2018 --- серый список!
			//06.03.18 Смирнов
			Запрос.УстановитьПараметр("РазрешитьЧерныйЛист", Контрагент.Экспорт или Контрагент.ВыгружатьЧерныйСписокВTerminal); //10.10.2018 --- черный список
