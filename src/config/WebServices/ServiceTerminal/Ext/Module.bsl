...
функция СоздатьЗаказ(Partner, PointId, Goods, Comment, ShippingDay, isReserve, TransportCompany,NameOperation,Prepay,IsStar,WayOfDelivery,AddressId,TkId,DeliveryDate2,CaseForLogistik,IsSeasonAdjournment,DayOfWeekToDeliver)
	...
	Если СтрДлина(Comment) > 500 Тогда
		Success = Ложь;
		Ошибка = "Комментарий к заказу слишком большой ("+строка(СтрДлина(Comment))+"). Комментарий не должен превышать 500 символов.";
	КонецЕсли;
	...
	//+++( 14.11.2018 -- ошибка доступа к файлу: в один и тот же файл пишут разные клиенты!---
	мс = ТекущаяУниверсальнаяДатаВМиллисекундах();  
	мс = (мс/1000 - цел(мс/1000) )*1000; // до 0.001 сек !
	датавремя1 = формат(ТекущаяДата(),"ДФ='dd.MM.yyyy hh-mm-ss'; ДЛФ=")+"."+формат(мс,"ЧЦ=3; ЧВН=");
	ИмяФайла2 = "D:\1C\Terminal\"+Partner+"_Текущий"+датавремя1+".xml"; //+ код партнёра!
	//+++)
	ЗаписьXML.ОткрытьФайл(ИмяФайла2);
	ФабрикаXDTO.ЗаписатьXML(ЗаписьXML, Goods);
	ЗаписьXML.Закрыть();
	
	ЧтениеXML = Новый ЧтениеXML;
	ЧтениеXML.ОткрытьФайл(ИмяФайла2);
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если ЧтениеXML.ТипУзла = ТипУзлаXML.КонецЭлемента Тогда
			Если ЧтениеXML.Имя = "Products" Тогда
				...
				иначеЕсли Найти(Comment,"KOLESATYT") > 0 и 	Partner ="П000053"   Тогда
					СтрокаТЗ.Договор = Справочники.ДоговорыКонтрагентов.НайтиПоКоду("Е3431");
				Иначе	
					...
					Если  NameOperation = "CreateOrder2" или NameOperation = "CreateOrder3" тогда
						...
						//Смирнов
						//Костыль для Скрипченко Ярославль, всегда Договор перемещения ОТХ
						Если Partner = "П017979" и СтрокаТЗ.Подразделение ="00005" тогда //Скрипченко
							СтрокаТЗ.Договор = Справочники.ДоговорыКонтрагентов.НайтиПоКоду("Е8922");
						КонецЕсли;	
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
	...
	Для каждого СтрВремТЗ ИЗ ВремТЗ Цикл  //перебираем все подразделения заказа для проверки остатков
		...
		Если  ПодразделениеЗаказа<>"00000" Тогда
			...
			Запрос.Текст = "ВЫБРАТЬ
			               |	спр.Ссылка
			               |ПОМЕСТИТЬ ВТ_Товары_СтопЛист
			               |ИЗ 
						   |	Справочник.Номенклатура КАК спр
			               |ГДЕ 
						   |	спр.Код В (&СписокНоменклатуры)
			             //+++ 12.11.2018 СРАЗУ отбор по стоп-листу - 4 варианта!----------------------------
			               |//РазрешитьЧерныйСерыйСписки 
						   |;
			               |////////////////////////////////////////////////////////////////////////////////
			               |ВЫБРАТЬ
			               ...
			               |					И Номенклатура В (ВЫБРАТЬ ВТ.Ссылка ИЗ ВТ_Товары_СтопЛист как ВТ)       
						   ...
						   //а тут может быть!? черный / серый списки !?
					       |				И Номенклатура В (&СписокТоваров_РезервИМ)
			               |				И Подразделение = &Подразделение            ) КАК РезервДляИМОстатки
			               |		ПО ТоварыНаСкладахОстатки.Номенклатура = РезервДляИМОстатки.Номенклатура 
			               |;
			               |
			               |////////////////////////////////////////////////////////////////////////////////
			               |ВЫБРАТЬ
				 ...
				 //09.11.2018 -------всегда надо подразделение! ----------------------------------------
					...
					//	   |ГДЕ  ЕСТЬNULL(ЗаказыПокупателейОстатки.КоличествоОстаток, 0)>=0
			               |;
			               |////////////////////////////////////////////////////////////////////////////////
			               |УНИЧТОЖИТЬ ВТ_Товары_СтопЛист
			               |;
			               |////////////////////////////////////////////////////////////////////////////////
			               |УНИЧТОЖИТЬ СвободныеОстатки";			   		
			
			Запрос.УстановитьПараметр("Подразделение", Подразделение);//надо еще и для резервов
			флТранзит =( ПодразделениеЗаказа <> "00005");
			Запрос.УстановитьПараметр("Транзит", флТранзит);
			Если флТранзит Тогда  //для ФИЛИАЛОВ -  подразделение склада = подразделению!
				Запрос.УстановитьПараметр("ПодразделениеСклада",Подразделение);
			Иначе	//Ярославль - пустое!
			...
			Если  КонтрРезерв.Код="П004703" и Контрагент.НеВыгружать=ЛОЖЬ Тогда // для Интернет-магазина  НЕ добавляем резервы!?
				КонтрРезерв = Справочники.Контрагенты.ПустаяСсылка();	
			КонецЕсли;
			//) 23.01.2017 
			
	//+++( 09-12.11.2018 --- срочное/полное исправление! -----------------
		Если Найти(Comment,"OrderFR") > 0 Тогда 
			КонтрРезерв = ДоговорФР.КонтрагентДляРезерваИМ;
		КонецЕсли;				
		
		масТов_new  =новый массив;
		
		Если НЕ КонтрРезерв.Пустая() тогда //СТАРАЯ СХЕМА!
			запрос_old = новый Запрос;
			запрос_old.Текст = "ВЫБРАТЬ спр.Ссылка КАК Номенклатура
           |ИЗ  Справочник.Номенклатура КАК спр
           |ГДЕ спр.Код В (&СписокНоменклатуры)";
  			запрос_old.УстановитьПараметр("СписокНоменклатуры", СписокНоменклатуры);
			результат_old = запрос_old.Выполнить();
			ТаблТов_new = результат_old.Выгрузить();
			масТов_new = ТаблТов_new.ВыгрузитьКолонку("Номенклатура");	
		КонецЕсли;

	//+++( 12.11.2018 - ЕДИНАЯ ФУНКЦИЯ в модуле яштПрочее 
		Если КонтрРезерв.Пустая() тогда 
			ТаблТов_new = ПолучитьТаблТовРезерваИМ( Контрагент, СписокНоменклатуры, Подразделение);   
		    Если ТаблТов_new.Количество() > 0 тогда
		    	КонтрРезерв = ТаблТов_new[0].КонтрагентРезерв; // должен быть 1 резерв!
				масТов_new = ТаблТов_new.ВыгрузитьКолонку("Номенклатура");
			Конецесли;
		КонецЕсли;
 		Запрос.УстановитьПараметр("КонтрагентДляРезерваИМ", КонтрРезерв);
		Запрос.УстановитьПараметр("СписокТоваров_РезервИМ", масТов_new);
	//+++)
			
	//+++( 12.11.2018 --- 4 варианта --- сразу в выборе товаров! --------------------
			РазрешитьСерый = Контрагент.Экспорт или Контрагент.ВыгружатьСерыйСписокВTerminal;  //10.10.2018 --- серый список!
			РазрешитьЧерный= Контрагент.Экспорт или Контрагент.ВыгружатьЧерныйСписокВTerminal; //10.10.2018 --- черный список
			НомВар = ?(РазрешитьСерый,10,0)+?(РазрешитьЧерный,1,0);
			Если НомВар = 0 тогда 
			Запрос.Текст = стрЗаменить(Запрос.Текст,	"//РазрешитьЧерныйСерыйСписки", "
			//4) нет нет--------------стоп-лист Запрещен----------------------------------------------------
			               | И НЕ спр.Ссылка В (ВЫБРАТЬ НоменклатураИмпорт.Номенклатура
			               |			ИЗ	РегистрСведений.НоменклатураИмпорт КАК НоменклатураИмпорт)
						   |");
			ИначеЕсли НомВар = 1 тогда 
			Запрос.Текст = стрЗаменить(Запрос.Текст,	"//РазрешитьЧерныйСерыйСписки", "
			//3) нет да-----------12.11.2018 ---- ещё вариант - разрешен только серый список!---------------
						   | И НЕ спр.Ссылка В (ВЫБРАТЬ НоменклатураИмпорт.Номенклатура
			               |			ИЗ	РегистрСведений.НоменклатураИмпорт КАК НоменклатураИмпорт
			               |			ГДЕ НоменклатураИмпорт.ЧерныйЛист = ЛОЖЬ)
						   |");
			иначеЕсли НомВар = 10 тогда
			Запрос.Текст = стрЗаменить(Запрос.Текст,	"//РазрешитьЧерныйСерыйСписки", "
			 //2) да нет  ----------------------------------------------------------------------------------
			               |  И НЕ спр.Ссылка В (ВЫБРАТЬ НоменклатураИмпорт.Номенклатура
			               |			ИЗ	РегистрСведений.НоменклатураИмпорт КАК НоменклатураИмпорт
			               |			ГДЕ НоменклатураИмпорт.ЧерныйЛист = ИСТИНА)
						   |");
		//	иначеЕсли НомВар = 11 тогда
		//   //1) да да  --------все можно!-----------------------------------------------------------------
		//		Запрос.Текст = стрЗаменить(Запрос.Текст,	"//РазрешитьЧерныйСерыйСписки", "");
			КонецЕсли;			   
   //+++)
			...
			Для каждого СтрМассива ИЗ МассивСтрок Цикл
				СтрСвОстаток = таблСвОст.Найти(СтрМассива.Номенклатура);
				Если СтрСвОстаток = Неопределено Тогда
					СтрМассива.КоличествоОстаток = 0;
					Success = Ложь;
				//	Ошибка = "Нет товаров в свободном остатке!"; //15-16.11.2018
				Иначе //есть такой товар
					Если СтрСвОстаток.СвободныйОстаток < СтрМассива.Количество Тогда
						Success = Ложь;
				//		Ошибка = "Не достаточно товаров в свободном остатке!"; //15-16.11.2018
					КонецЕсли;	
					СтрМассива.КоличествоОстаток = СтрСвОстаток.СвободныйОстаток;
				КонецЕсли;
			КонецЦикла;	
		КонецЕсли;	
	...
	//+++ 15-19-29.11.2018 ПОДРАЗДЕЛЕНИЕ ТОЛЬКО Филиалы!!!
	Запрос.УстановитьПараметр("Подразделение", ?(Подразделение=справочники.Подразделения.НайтиПоКоду("00005"), справочники.Подразделения.ПустаяСсылка(), Подразделение ) ); 
	Запрос.Текст = "ВЫБРАТЬ
	               |	спр.Ссылка
	               |ПОМЕСТИТЬ ВТ_Товары
	               |ИЗ
	               ...
	               |	И ОстаткиНоменклатурыКонтрагентов.Номенклатура В (ВЫБРАТЬ ВТ_товары.Ссылка ИЗ ВТ_товары)
	...
	Если Success Тогда
		...
		Для каждого СтрВремТЗ ИЗ ВремТЗД Цикл  //перебираем все подразделения заказа для проверки остатков
			...
			Если ПодразделениеЗаказа="00000" Тогда  //СезонныйЗаказПокупателя
				...
				Заказ.ДатаДействияПо = Заказ.Дата+120*86400;
				...
				//+++ 04-05.09.2018 в сезонном заказе НЕТ процента скидки нал
				//Если найти(Заказ.ДоговорКонтрагента.Наименование,"*")>0 тогда
					//Заказ.ПроцентСкидкиНал = Заказ.Контрагент.ПроцентСкидкиНал;
				//конецЕсли;				
				...
				Если НЕ Заказ.Товары.Количество() = 0 Тогда
					...
					Для каждого стр1 из Заказ.Товары цикл
						...
						//31.07.18 Смирнов
						найденнаяСтрокаТаб = ТаблицаЗаказа.Найти(стр1.Номенклатура.код, "Номенклатура");
						Если найденнаяСтрокаТаб<>Неопределено тогда
							 найденнаяСтрокаТаб.Цена = стр1.Цена;
						КонецЕсли;
						 //31.07.18 Смирнов
					КонецЦикла;
				...
				Если НЕ OrderNumber = "" Тогда
					Попытка
						ПереместитьФайл(ИмяФайла2, "D:\1C\Terminal\Заказы\Сезонный заказ "+ OrderNumber +".xml");
					Исключение
					КонецПопытки;
				КонецЕсли;
			...
			Если ПодразделениеЗаказа<>"00000" тогда // ЗаказПокупателя
				...
				иначе
					Если  NameOperation = "CreateOrder2" или NameOperation = "CreateOrder3"  тогда
						...
						Заказ.ДатаУстановкиВТерминалеКОтгрузке = ТекущаяДата(); //+ Лукьяненков 28.02.2018
					конецЕсли;
				КонецЕсли;
				
				Если CaseForLogistik = 22 или CaseForLogistik = 41 //или CaseForLogistik = 52  - //+++ 27.11.2018  для 52 нужен ShippingDay! и для Яр. и для Филиала 
					тогда
					Заказ.ДатаОтгрузки = Дата(DeliveryDate2); //текущая?!
				иначеЕсли CaseForLogistik = 22 и Поставщик<>"0" тогда 
					Заказ.ДатаОтгрузки = Дата(DeliveryDate2);
				иначеЕсли CaseForLogistik = 51 и ПодразделениеЗаказа<>"00005" тогда  //+++ 27.11.2018 наоборот! = замена на <>
					Заказ.ДатаОтгрузки = Дата(DeliveryDate2);
				конецесли;
				...
				Если Контрагент.Экспорт и Контрагент.ОсновнойДоговорКонтрагента.ВалютаВзаиморасчетов.Код<>"643" Тогда
					...
					КурсДокумента = константы.КурсUSD.Получить(); //62,0000  на 09.11.2018 -> ФИКСИРОВАННЫЙ КУРС! "Так Егор сказал"...
					
					//СтруктураВалюты    = РегистрыСведений.КурсыВалют.ПолучитьПоследнее(Заказ.Дата, Новый Структура("Валюта", Заказ.ВалютаДокумента));
					//КурсДокумента      = СтруктураВалюты.Курс;      // 66,2155  на 09.11.2018 !!!
				   //КратностьДокумента  = СтруктураВалюты.Кратность;
					КратностьДокумента = 1;
				...
				Иначе // как было	
					Заказ.ТипЦен = Справочники.ТипыЦенНоменклатуры.НайтиПоКоду("00005"); //Крупный опт
					Заказ.ВалютаДокумента = Справочники.Валюты.НайтиПоКоду("643"); //руб.
					Заказ.КурсВзаиморасчетов = 1;
					Заказ.КратностьВзаиморасчетов = 1;
				КонецЕсли;
				...
				Если Поставщик<>"0" Тогда      
					...
					|	СрокиДоставки.АвтоПодтверждение
					|ИЗ
					...
					Если результат.Количество()>0 тогда
						...
						//13.08.2018 - при проведении сразу "автоподтверждение"
						Если Результат[0].АвтоПодтверждение тогда
							Заказ.Проверен = Истина;
							Заказ.Согласован = ЛОЖЬ;
							Заказ.Статус = Перечисления.СтатусыЗаказов.ПустаяСсылка();
						иначе
				...
				//+++ 27.11.2018 заказ из Ярославля -> надо везти через Филиал -> Клиенту!
				Если (CaseForLogistik = 52 или CaseForLogistik = 51) и ПодразделениеЗаказа="00005" тогда  
					// Подразделение2 - как определить ???  по подразделению менеджера клиента / договора !
					Подразделение2 = Контрагент.ОсновнойМенеджерКонтрагента.ОсновноеПодразделение;
					Если НЕ Подразделение2.ОбособленноеПодразделение тогда 
					Подразделение2 = Заказ.ДоговорКонтрагента.ОтветственноеЛицо.ОсновноеПодразделение;
					КонецЕсли;
					Если Подразделение2.ОбособленноеПодразделение тогда 
						Заказ.ОтгрузкаСПодразделенияСклада = Истина;
						Заказ.Склад = Подразделение2.Склад;  
					Иначе //если "не повезло", смотрим так же как при самовывозе... 
						УстановитьСамовывозСоСкладаФилиала(Заказ);
					КонецЕсли;
				КонецЕсли;
				...
				//WayOfDelivery =0 - мы везем, WayOfDelivery =1 самовывоз
				 ...
				 Если Заказ.Самовывоз и Подразделение.Код = "00005" тогда // самовывоз с Ярославля
				...
				//27.08.18 Смирнов, уже не надо
				 ////+++ Шарафутдинов    18.07.2018
				...
				Если Заказ.Товары.Количество() <> 0 Тогда
					...
					Заказ.Записать(РежимЗаписиДокумента.Проведение);
					//Заказ.Записать(РежимЗаписиДокумента.Проведение); //+++ 13.08.2018 - Зачем 2 раза подряд???
				...
				Если НЕ OrderNumber = "" Тогда
					Попытка
						ПереместитьФайл(ИмяФайла2, "D:\1C\Terminal\Заказы\Заказ "+OrderNumber+".xml");
					Исключение
					КонецПопытки;
				КонецЕсли;
		...
		Для каждого СтрокаТЗ ИЗ ТаблицаЗаказа Цикл
			...
			//31.07.18 Смирнов
			Товар.Price = ?(ЗначениеЗаполнено(СтрокаТЗ.Цена),СтрокаТЗ.Цена,0);
			Товары.ProductsResult.Добавить(Товар);
		КонецЦикла;
		
	Иначе // ------------------------------------------------------------
		Для каждого СтрокаТЗ ИЗ ТаблицаЗаказа Цикл
		  Если СтрокаТЗ.Подразделение<>"00000" Тогда //только если есть
...
функция ПолучитьЗначРеквизитаЗаказа(ссылкаДок, НазвРеквизита="Номер")
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	ЗаказПокупателя."+НазвРеквизита+" как Знач1
	|ИЗ
	|	Документ.ЗаказПокупателя КАК ЗаказПокупателя
	|ГДЕ
	|	ЗаказПокупателя.Ссылка = &СсылкаДок";
...
Функция ChangeOrder(OrderGUID, Goods, Comment, isReserve, ShippingDay,TransportCompany, DeliveryInfo,DeliveryDay,IsStar,WayOfDelivery,AddressId,TkId)
	...
	Иначе//Если (НЕ ЗаказСсылка=Документы.ЗаказПокупателя.ПустаяСсылка()) и (НЕ ЗаказСсылка = Неопределено) Тогда
		...
		Если СтрДлина(Comment) > 500 Тогда
			Success = Ложь;
			Ошибка = "Комментарий к заказу слишком большой ("+строка(СтрДлина(Comment))+" символов). Комментарий не должен превышать 500 символов!";
		КонецЕсли;
		
		Если Success Тогда // Заказ покупателя - получаем весь ОБЪЕКТ
			...
			Иначе
				...
				попытка
					...
					//+++ 15.05.2018
					ЗаписатьВЖурналИзменийTerminal(ДокОбъект.Ссылка, "Дата отгрузки: "+формат(ДокОбъект.ДатаОтгрузки,"ДЛФ=D")
											+", Число дней оплаты: "+строка(ДокОбъект.ЧислоДнейСМоментаОтгрузки) );
				исключение
				КонецПопытки;	
				//+++)
			КонецЕсли;

		//+++( 14.11.2018 -- ошибка доступа к файлу: в один и тот же файл пишут!---
		//БЫЛО: ИмяФайла1 = "D:\1C\Terminal\Текущий.xml"; - 1 файл на всех!?
			мс = ТекущаяУниверсальнаяДатаВМиллисекундах();  
			мс = (мс/1000 - цел(мс/1000) )*1000; //название до миллиСек 
			датавремя1 = формат(ТекущаяДата(),"ДФ='dd.MM.yyyy hh-mm-ss'; ДЛФ=")+"."+формат(мс,"ЧЦ=3; ЧВН=");
			НомерЗаказа = ПолучитьЗначРеквизитаЗаказа(ЗаказСсылка,"Номер"); // изменение  заказа!
			ИмяФайла1 = "D:\1C\Terminal\Текущий"+НомерЗаказа+"("+датавремя1+").xml";
		//+++)
			ТипОбъектаXDTO = ФабрикаXDTO.Тип("http://37.1.84.50:8080/Terminal", "ArrayOfProducts");
			ЗаписьXML = Новый ЗаписьXML;
			ЗаписьXML.ОткрытьФайл(ИмяФайла1);
			ФабрикаXDTO.ЗаписатьXML(ЗаписьXML, Goods,,"http://37.1.84.50:8080/Terminal");
			ЗаписьXML.Закрыть();
			
			ЧтениеXML = Новый ЧтениеXML;
			ЧтениеXML.ОткрытьФайл(ИмяФайла1);
			...
			//поменяем договор, если требуется
			 ...
			 Если IsStar = ЛОЖЬ и ЕстьЗвезда <>0 тогда //подставим Договор без * 
					 Запрос = Новый Запрос;
					 Запрос.Текст = 
					 "ВЫБРАТЬ
					 |	ДоговорыКонтрагентов.Ссылка
					 ...
					 |ГДЕ
					 |	ДоговорыКонтрагентов.Владелец = &Владелец
					// |	И ДоговорыКонтрагентов.ТипДоговора = &ТипДоговора   //15.11.2018
					 |	И ДоговорыКонтрагентов.ТипДоговора в (&СписокТиповДоговоров)
					 |	И ДоговорыКонтрагентов.ОтветственноеЛицо = &ОтветственноеЛицо
					 ...
					 Запрос.УстановитьПараметр("ОтветственноеЛицо", ДокОбъект.ДоговорКонтрагента.ОтветственноеЛицо);
					  //+++( 15.11.2018
					 //БЫЛО: Запрос.УстановитьПараметр("ТипДоговора", ДокОбъект.ДоговорКонтрагента.ТипДоговора);
					 текТипДог =  ДокОбъект.ДоговорКонтрагента.ТипДоговора;
						 СписокТиповДоговоров = новый массив;  // "одинаковые" предоплатые... договоры
					 ...
					 Если СписокТиповДоговоров.Найти(текТипДог) = Неопределено Тогда 
						СписокТиповДоговоров = новый массив; // ТОЛЬКО по текущему типу текущего договора - ищем такой же БЕЗ *
					 	 СписокТиповДоговоров.Добавить( текТипДог ); 
					 КонецЕсли;	 
				 ...
				 ИначеЕсли IsStar = истина и ЕстьЗвезда =0 тогда //подставим договор со *
					 Запрос = Новый Запрос;
					 Запрос.Текст = 
					 "ВЫБРАТЬ
					 |	ДоговорыКонтрагентов.Ссылка
					 ...
					 текТипДог =  ДокОбъект.ДоговорКонтрагента.ТипДоговора;
						 ...
						 СписокТиповДоговоров.Добавить(справочники.ТипыДоговоров.НайтиПоКоду("00001") ); //Предоплата
					 Если СписокТиповДоговоров.Найти(текТипДог) = Неопределено Тогда 
						СписокТиповДоговоров = новый массив; // ТОЛЬКО по текущему типу текущего договора - ищем такой же со *
					 	 СписокТиповДоговоров.Добавить( текТипДог ); 
					 КонецЕсли;	 
					 Запрос.УстановитьПараметр("СписокТиповДоговоров", СписокТиповДоговоров);
					 //+++)
			...
			Если Success Тогда
				...
				//+++ 13-16.07.2018 -------------------------------------------------------
				 Если ДокОбъект.Подразделение.Код = "00005" тогда // самовывоз с Ярославля/Филиала
					 Если ДокОбъект.Самовывоз тогда
					 	УстановитьСамовывозСоСкладаФилиала(ДокОбъект);
					 //Иначе  //+++ 27.11.2018 - МОЖЕТ быть [v]293 при отгрузке с Ярославля через Филиал -> CaseForLogistik = 51 или 52!
					 //ДокОбъект.Склад = ДокОбъект.Подразделение.Склад;   //Ярославский
					 //ДокОбъект.ОтгрузкаСПодразделенияСклада = ЛОЖЬ;
				 	 ИначеЕсли НЕ ДокОбъект.ОтгрузкаСПодразделенияСклада тогда
					 	ДокОбъект.Склад = ДокОбъект.Подразделение.Склад;  //Ярославский
					 КонецЕсли;
				 КонецЕсли;
				...
				//+++( 15.05.2018
				коммент = строка( ДокОбъект.ВидАдреса )+": "+СокрЛП(ДокОбъект.АдресДоставки)+", Транспортная компания: "+строка(ДокОбъект.ТранспортнаяКомпания);
	 			ЗаписатьВЖурналИзменийTerminal(ДокОбъект.Ссылка, коммент, 2);
				//+++)
				 КонецЕсли;
								 
				 ДокОбъект.Записать(РежимЗаписиДокумента.Проведение);
				 
		//+++ 14.11.2018 - переименовываем файл после успешного изменения документа!
				 //при создании делается файл: "D:\1C\Terminal\Заказ ТК453213.xml";
				//1) здесь, при изменении - В ТОТ ЖЕ ФАЙЛ !!! 
				//ИмяФайла11 = "D:\1C\Terminal\Заказ "+СокрЛП(ДокОбъект.Номер)+".xml";
				//2) в НОВЫЙ файл с датой-временем,ПРИМЕР D:\1C\Terminal\Заказы\Заказ ТК453213(14.11.2018 12-08-01.072).xml";
 				ИмяФайла11 = стрЗаменить(ИмяФайла1,"Текущий","\Заказы\Заказ ");
				
				ПереместитьФайл(ИмяФайла1,ИмяФайла11);
			 КонецЕсли;//Success
			 
		 КонецЕсли; //Success
		
	КонецЕсли; // есть ЗаказСсылка
...
Процедура ЗаписатьВЖурналИзменийTerminal(ссылкаДок, Коммент="", Ном=1 )
	журналИзм = РегистрыСведений.ЖурналИзменений.СоздатьМенеджерЗаписи();
	журналИзм.Период = ОценкаПроизводительностиВызовСервераПолныеПрава.ДатаИВремяНаСервере();//+++ 18.12.2015 - СЕРВЕРНОЕ ВРЕМЯ
	журналИзм.Объект = ссылкаДок;
	
	журналИзм.Компьютер = "Terminal.yst.ru";
    журналИзм.Пользователь = справочники.Пользователи.НайтиПоКоду("YStoreAdmin");
	журналИзм.НомерЗаписи  = Ном;
	
	журналИзм.Событие = "Изменения из Terminal.YST.ru"; 
	журналИзм.Комментарий = Коммент;
	
	журналИзм.Записать(Истина); //перезапись
КонецПроцедуры		
...
Функция CreateSeasonOrder(Partner,PointId, Goods, Storage, ActionDate, Comment, SeasonOrderGUID)
	...
	//+++( 14.11.2018 -- ошибка доступа к файлу: в один и тот же файл пишут!---
//БЫЛО: ИмяФайла2 = "D:\1C\Terminal\Текущий.xml"; - 1 файл на всех!?
	мс = ТекущаяУниверсальнаяДатаВМиллисекундах();  
	мс = (мс/1000 - цел(мс/1000) )*1000; //название до миллиСек 
	датавремя1 = формат(ТекущаяДата(),"ДФ='dd.MM.yyyy hh-mm-ss'; ДЛФ=")+"."+формат(мс,"ЧЦ=3; ЧВН=");
	ИмяФайла2 = "D:\1C\Terminal\"+Partner+"_Текущий"+датавремя1+".xml";
	//+++)
	...
	Если Success Тогда
		...
		Для каждого СтрВремТЗ ИЗ ВремТЗД Цикл  //перебираем все подразделения заказа для проверки остатков
			...
			//+++ 15.06.2015 - валютные клиенты
				...
				Если НЕ OrderNumber = "" Тогда
					Попытка
						ПереместитьФайл(ИмяФайла2, "D:\1C\Terminal\Заказы\Сезоный заказ "+ OrderNumber +".xml");
					Исключение
					КонецПопытки;
				КонецЕсли;
...
Функция GetPrices(Partner, Products)
	...
	ДатаСреза = ?(ЗначениеЗаполнено(ДатаСреза),ДатаСреза,Неопределено);
	...
	Для каждого СтрокаТЗ ИЗ табЗнач1 Цикл
			...
			Если Предоплата Тогда
				Товар.Price=Окр(Число(СтрокаТЗ.ЦенаСоСкидкойПредоплаты));
			Иначе
				Товар.Price=Окр(Число(СтрокаТЗ.МинимальнаяЦена));
			КонецЕсли;
			Товары.Product.Добавить(Товар);
	КонецЦикла;
...
Функция CreatePurchaseReturn( PartnerId, PointId, Goods, isTest  )
	...
	Заявка.ПричинаВозврата = Перечисления.ПричиныВозвратаТовара.ОтказКлиентаНеАктуально; //+++03.05.2018 ОтказПокупателя>>ОтказКлиентаНеАктуально
	...
	Для каждого стр из ТаблицаЗаявки Цикл
		...
		Если НачалоДня(ДокРеализации.Дата) = Дата(стр.ДатаРеализации) и ДокРеализации.Контрагент = Контрагент Тогда
			...
			|	ВЫБОР
			|		КОГДА ВЫРАЗИТЬ(ПродажиОбороты.ДокументПродажи.Сделка КАК Документ.ЗаказПокупателя).БонусПрименен = ИСТИНА
			|			ТОГДА ВЫРАЗИТЬ(ПродажиОбороты.СтоимостьОборот * (100 - ВЫРАЗИТЬ(ПродажиОбороты.ДокументПродажи.Сделка КАК Документ.ЗаказПокупателя).ПроцентСкидкиНал) / 100 КАК ЧИСЛО(15, 0))
	...
	Если Success Тогда
		...
		Если ОператорКонтрагента = Справочники.Пользователи.ПустаяСсылка() Тогда
			Заявка.Ответственный = Менеджер1; //+++ 04.08.2017  по задаче 30681   //***устарела, осталась как заглушка
		Иначе
			Заявка.Ответственный = ОператорКонтрагента; //***2018.05.23 №47249 
		КонецЕсли;
		Заявка.Записать(РежимЗаписиДокумента.Проведение);
		
		//Создание задачи ответственному
		НоваяЗадача = Задачи.ЗадачиПользователя.СоздатьЗадачу();
		НоваяЗадача.Исполнитель = Заявка.Ответственный; 
		НоваяЗадача.Дата = ТекущаяДата();
		НоваяЗадача.Оповещение = Истина;
		НоваяЗадача.СрокОповещения = ТекущаяДата();
		...
		НоваяЗадача.Записать();
		////////////////////////// дубль менеджеру
		Если Заявка.Ответственный <> Менеджер1 тогда
			НоваяЗадача = Задачи.ЗадачиПользователя.СоздатьЗадачу();
...
функция ПолучитьДЗиПени( Клиент1 )
    ...
    запрос.Текст = "ВЫБРАТЬ
                   ...
                   |	КОНЕЦ, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаОтгрузки,
                   |	ЕстьNULL(ВЫБОР
                   |		КОГДА Дебиторка.Сделка ССЫЛКА Документ.ЗаказПокупателя
...
//11.10.2018 - ВСПОМОГАТЕЛЬНАЯ ФУНКЦИЯ по Задаче № 56199 
//Date  - пустая = текущия,  дата заказа или дата, когда товар уже будет перемещен на филиал
Функция GetDatesOfShipmentFromFilialOnDate(Partner, AddressId, FilialId, Date='00010101')
	
	URL = "http://37.1.84.50:8080/Terminal";
	Даты = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(URL, "ListOfDates"));
					 
//Partner, AddressId, FilialId, Date >> Контрагент, сАдрес, Подразделение, ДатаЗаказа 	
	Контрагент = Справочники.Контрагенты.НайтиПоКоду(Partner);
	
	Вид = Справочники.ВидыКонтактнойИнформации.НайтиПоКоду(AddressId);
	сАдрес = ПолучитьКонтактнуюИнформацию(Контрагент, Вид);
	
	//11.10.2018 - c конкретного филиала - смотрим ТОЛЬКО маршруты ЭТОГО филиала!
	номФилиала = формат(FilialId, "ЧЦ=5; ЧН=0; ЧВН=; ЧГ=0");
	Подразделение = справочники.Подразделения.НайтиПоКоду(номФилиала); //Контрагент.ПодразделениеЛогистики;
	Если Подразделение = Неопределено или Подразделение = Справочники.Подразделения.ПустаяСсылка() Тогда
		возврат Даты;
	КонецЕсли;
...
//=========================================================================================	
	РегионЭтоГород = (  найти(сАдрес.Регион,"Москва")>0 
			...
			или найти(сАдрес.Регион,"Байконур")>0 );
	...
	Запрос.УстановитьПараметр("Подразделение",Подразделение); // подразделение филиала
	Запрос.Текст = "ВЫБРАТЬ
	...
	Если Рез.Количество() = 0 Тогда //Ничего не нашли, попробуем по координатам!
	...
	ИначеЕсли Рез.Количество() >= 1 Тогда // есть маршруты по Городу!
		МаршрутНайден = Истина;
			...
			Для каждого стрДень из графикРаботыТекущий Цикл
				Если стрДень.Используется Тогда
					...
					если соответсвиеДней.Получить(стрДень.ДеньНедели) < ТекДень Тогда
						...
						НовСтрокаМаршрутДата.Дата = ДобавитьИнтервал(ДатаЗаказа,Перечисления.Периодичность.День, 7 - ТекДень + соответсвиеДней.Получить(стрДень.ДеньНедели));  //на следующей неделю!
	...
	Возврат Даты;
КонецФункции //GetDatesOfShipmentFromFilial()


//11.10.2018 - доставка из ПодразделениеЛогистики партнера / с Ярославля !
Функция GetDatesOfShipment(Partner, AddressId)
	...
	КонецЕсли;
	FilialId = число(Подразделение.Код);
	
	//URL = "http://37.1.84.50:8080/Terminal";
	//Даты = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(URL, "ListOfDates"));
	
	//ЕДИНАЯ ФУНКЦИЯ ДОСТАВКИ С ПОДРАЗДЕЛЕНИЯ ЛОГИСТИКИ
	Даты = GetDatesOfShipmentFromFilialOnDate(Partner, AddressId, FilialId, ДатаЗаказа);
	
	Возврат Даты;

КонецФункции //GetDatesOfShipment()

//11.10.2018 - Задача № 56199 п.1)  Date  - пустая = текущия,  дата заказа или дата, когда товар уже будет перемещен на филиал
Функция GetDatesOfShipmentFromFilial(Partner, AddressId, FilialId)
	
	ДатаЗаказа = ТекущаяДата();  // на тек.дату ПРЯМО из филиала
...
//11.10.2018 - Задача № 56199 п.2) Date (текущая дата или дата заказа)
Функция GetDatesOfShipmentFromYarAndFilial(Partner, AddressId, FilialId)
	
	ДатаЗаказа = ТекущаяДата(); // заказа из Ярославля
	...
	датаЗаказаИзФилиала = ПолучитьДатуРазгрузкиВФилиале( Подразделение,  ДатаЗаказа ); //яштПрочее >> Товар будет перемещен и выгружен в Филиале
	
	даты = GetDatesOfShipmentFromFilialOnDate(Partner, AddressId, FilialId, датаЗаказаИзФилиала);
		...
		Если Строки.Количество() > 0 Тогда			
			Для каждого СтрМассива ИЗ Строки Цикл
				Если НЕ СтрМассива.НоменклатураКод = "" И НЕ СтрМассива.НоменклатураКод = Неопределено Тогда
				   ...
				   //+++( 20.11.2018 BugFix: //<ClaimNumber>10 465</ClaimNumber>
				//БЫЛО:	Товар.ClaimNumber.Добавить(Строка(СокрЛП(Формат(Строка(СтрМассива.Ссылка.Номер), "ЧГ=")))); 
						Товар.ClaimNumber.Добавить( Формат(СтрМассива.Ссылка.Номер, "ЧРД=.; ЧГ=0") );
					//+++)		
...
процедура УстановитьСамовывозСоСкладаФилиала( Зак1 )
	...
	СкладПодр.Вставить("Тольятти", справочники.Подразделения.НайтиПоКоду("00172").Склад );  // 5-ый филиал!
...
Функция ChengeOrders(DeliveryDate,WayOfDelivery,AddressId,TkId,DayOfWeekToDeliver,OrderGuids)
	...
	//+++( 16.05.2018
	заказОбъект.ДатаУстановкиВТерминалеКОтгрузке = ТекущаяДата();	
	ЗаписатьВЖурналИзменийTerminal(заказОбъект.Ссылка, "Дата отгрузки: "+формат(заказОбъект.ДатаОтгрузки,"ДЛФ=D")
							+", Число дней оплаты: "+строка(заказОбъект.ЧислоДнейСМоментаОтгрузки)
							+", Дата оплаты: "+формат( заказОбъект.ДатаОплаты,"ДЛФ=D") );
	//+++)
   	...
   	//+++( 16.05.2018
	коммент = строка( заказОбъект.ВидАдреса )+": "+СокрЛП(заказОбъект.АдресДоставки)+", Транспортная компания: "+строка(заказОбъект.ТранспортнаяКомпания);
 	ЗаписатьВЖурналИзменийTerminal(заказОбъект.Ссылка, коммент, 2);
	//+++)
		конецЕсли;
...
Функция CreatePurchaseComission(PartnerId, PartnerPointId, BeginDate, EndDate, Goods)
	URL = "http://37.1.84.50:8080/Terminal";
	
	ТипXDTOРезультатОперации = ФабрикаXDTO.Тип(URL, "ResultOfPurchaseComission");
	РезультатОперации = ФабрикаXDTO.Создать(ТипXDTOРезультатОперации);
	Товары = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(URL, "ArrayOfProductsPurchaseComission"));
...
ОписаниеТиповЧ = Новый ОписаниеТипов(Массив, , ,КЧ);
	...
	табОтчета = Новый ТаблицаЗначений;
	ТабОтчета.Колонки.Добавить("Номенклатура",ОписаниеТиповС,,);
	ТабОтчета.Колонки.Добавить("Количество",ОписаниеТиповЧ,,);
	
	Для каждого Запись из Goods.Product цикл
		стр = ТабОтчета.Добавить();
		стр.номенклатура = Справочники.Номенклатура.НайтиПоКоду(Запись.Code);
		стр.Количество = Число(Запись.Quantity);
	конецЦикла;	
	...
	Запрос.Текст = 
		...
		|ВЫБРАТЬ
		|	ВТ.Номенклатура,
		|	ВТ.Количество - ЕСТЬNULL(ТоварыПереданныеОстатки.КоличествоОстаток, 0) КАК Нехватает,
		|	ЕСТЬNULL(ТоварыПереданныеОстатки.КоличествоОстаток, 0) КАК Остаток
		|ИЗ
		|	ВТ КАК ВТ
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыПереданные.Остатки(
		|				&Дата,
		|				ДоговорКонтрагента.ТипДоговора = ЗНАЧЕНИЕ(Справочник.типыДоговоров.ПеремещениеОТХ)
		|					И ДоговорКонтрагента.Владелец = &Контрагент) КАК ТоварыПереданныеОстатки
		|		ПО ВТ.Номенклатура = ТоварыПереданныеОстатки.Номенклатура
		|ГДЕ
		|	ВТ.Количество - ЕСТЬNULL(ТоварыПереданныеОстатки.КоличествоОстаток, 0) > 0";

	Запрос.УстановитьПараметр("Дата", конецДня(EndDate));
	Запрос.УстановитьПараметр("Таб",ТабОтчета);
	Запрос.УстановитьПараметр("Контрагент",Контрагент);
	...
	Если не Результат.Пустой() тогда 
		...
		Пока Выборка.Следующий() Цикл
			Товар = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(URL, "ProductPurchaseComission"));
			//Товар.Code.Добавить(Строка(Выборка.Номенклатура.Код));
			//Товар.Quantity.Добавить(Число(Выборка.Нехватает));
			Товар.Code=Строка(Выборка.Номенклатура.Код);
			Товар.Quantity=Число(Выборка.Остаток);
	...
	Если Success тогда	
		Отчет = Документы.ОтчетПоОТХ.СоздатьДокумент();
		Отчет.Дата = ТекущаяДата();
		Отчет.Контрагент = Контрагент;
		Отчет.ДатаС = BeginDate;
		Отчет.ДатаПо = EndDate;
		Отчет.Комментарий ="Terminal"; 	
		
		
		Для каждого товар из ТабОтчета цикл
			СтрокаТЗ = Отчет.Товары.Добавить();
			СтрокаТЗ.Номенклатура = товар.номенклатура;
			СтрокаТЗ.Количество = товар.количество;
		КонецЦикла;
		отчет.Записать(РежимЗаписиДокумента.Проведение);
		GUID = Строка(Отчет.Ссылка.УникальныйИдентификатор());
	    Number = Отчет.Номер;
	...
	РезультатОперации.Products		 = Товары;
	РезультатОперации.Success		 = Success;
	РезультатОперации.Guid = GUID;
	РезультатОперации.Number = Number;
	Возврат РезультатОперации;
	
КонецФункции

Функция GetRestsOnComission(PartnerId, EndDate)

	URL = "http://37.1.84.50:8080/Terminal";
	
	Товары = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(URL, "ArrayOfProductsOnComissionResult"));
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Период", КонецДня(EndDate));
	Запрос.УстановитьПараметр("Контрагент", Контрагент);
	Запрос.Текст = "ВЫБРАТЬ
	...
	Пока Результат.Следующий() Цикл
		Товар = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(URL, "ProductOnComissionResult"));
		Товар.ProductId.Добавить(Строка(Результат.Номенклатура.Код));
		Товар.ProductName.Добавить(Строка(Результат.Номенклатура.Наименование));
		Товар.Rest.Добавить(Число(Результат.КоличествоОстаток));
		Товары.ProductsOnComissionResult.Добавить(Товар);
	КонецЦикла;
