...
Функция CreatePurchaseComission(PartnerId, PartnerPointId, BeginDate, EndDate, Goods)
	...
	ТабОтчета.Колонки.Добавить("Номенклатура");
	ТабОтчета.Колонки.Добавить("Количество");
	...
	Запрос.Текст = 
		...
		|ВЫБРАТЬ
		|	ВТ.Номенклатура,
		|	ВТ.Количество - ТоварыПереданныеОстатки.КоличествоОстаток КАК Нехватает
		|ИЗ
		...
		|	ВТ КАК ВТ
		|ГДЕ
		|	ВТ.Количество - ТоварыПереданныеОстатки.КоличествоОстаток > 0";
