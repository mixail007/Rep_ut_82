...
Функция CreateOrder(Partner, Goods, Storage, Comment, isTestOrder, ShippingDay, isReserve)
	...
	//Если НЕ Аксессуары Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ спр.Ссылка 
		 ...
		 //+++( 09.11.2018 --- срочнок исправление! -----------------
			масТов_new = новый массив;	 // разрешение не на все товары, а только на часть резерва!
			...
			КонецЕсли;
		 	...
		 	Если КонтрРезерв.Пустая() тогда //+++ 09.11.2018 - новая схема резерва!
			...
			Если НЕ КонтрРезерв.Пустая() 
				...
				+"ВЫБРАТЬ
				 ...
				 //+++ 09.11.2018 -----------------
				 |	И Номенклатура В (&СписокТоваровРезерваИМ)
				...
				Запрос.УстановитьПараметр("СписокТоваровРезерваИМ", масТов_new); //+++ 09.11.2018
			КонецЕсли;
		    //) 23.01.2017 
	...
	Если Success Тогда
		...
		Если Найти(Comment,"KOLESATYT") > 0 и 	Partner ="П000053"   Тогда
			Заказ.ДоговорКонтрагента = Справочники.ДоговорыКонтрагентов.НайтиПоКоду("Е3431");
		КонецЕсли;
		...
		Если Success Тогда
			...
			Если Success Тогда
				...
				КонецЦикла; 
				 ...
				 попытка//+++ 10.05.2018 - надо видеть когда клиент что-то поменял!
					 ПараметрыСеанса.ТекущийПользователь = справочники.Пользователи.ПустаяСсылка();
					 обЗаписатьПротоколИзменений(ДокОбъект, ЛОЖЬ); 
				 исключение
				 КонецПопытки;
