...
Функция CreateOrder(Partner, Goods, Storage, Comment, isTestOrder, ShippingDay, isReserve)
	...
	//Если НЕ Аксессуары Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ спр.Ссылка 
			...
			Иначе //склады Ярославля - без подразделения всегда!
		 ...
		 //+++( 09.11.2018 --- срочнок исправление! -----------------
			...
			Если КонтрРезерв.Пустая() тогда 
				ТаблТов_new = ПолучитьТаблТовРезерваИМ( Контрагент, СписокНоменклатуры, Подразделение);   //+++ 12.11.2018 - ЕДИНАЯ ФУНКЦИЯ в модуле яштПрочее 
