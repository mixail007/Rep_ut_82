...
Функция  СформироватьРедактироватьЗаказ(Заказ, Ошибка, Customer, OrderProperties, Goods, Amenities)
	...
	ИначеЕсли customer.Type = "ТК" тогда //ДПД, подорожник и прочие тк
		...
		Если ЕстьПосредник = ложь тогда  //посредник не указан, заказ на ТК
			...
			Если СокрЛП(OrderProperties.TransportCompany)<>"" тогда
				...
				ИначеЕсли СокрЛП(OrderProperties.TransportCompany)="95768" тогда //Розничный покупатель ПИТСТОП 	
	...
	Если Success Тогда
		...
		//	ИначеЕсли СокрЛП(OrderProperties.TransportCompany)="95768" тогда //Розничный покупатель ПИТСТОП 	
		//		Заказ.ТранспортнаяКомпания = Справочники.Контрагенты.НайтиПоКоду("95768"); //Розничный покупатель ПИТСТОП //для самовывоза из питера
		//	КонецЕсли;
		//КонецЕсли;
		
		Если СокрЛП(OrderProperties.TransportCompany)<>"" тогда
			Заказ.ТранспортнаяКомпания = Справочники.Контрагенты.НайтиПоКоду(СокрЛП(OrderProperties.TransportCompany)); //Армадилло Бизнес Посылка
		КонецЕсли;
	...
	времяНачала=ТекущаяУниверсальнаяДатаВМиллисекундах();
			...
			Если Заказ.ТранспортнаяКомпания = Справочники.Контрагенты.НайтиПоКоду("95768") и Заказ.ИМСклиентомСогласовано тогда  //ПИТСТОП
				попытка
					//менеджерЗаписи = РегистрыСведений.КатегорииОбъектов.СоздатьМенеджерЗаписи();
					//менеджерЗаписи.Объект=Заказ.Ссылка;
					//менеджерЗаписи.Категория = Справочники.КатегорииОбъектов.НайтиПоКоду("00056"); //категория ПВ Филиал
					//менеджерЗаписи.Записать();
		...
		КонецЕсли;  //Заказ.Товары.Количество()>0
...
Процедура ЗаполнитьРеквизитыТК(Заказ,OrderProperties,Customer)
	...
	Если ТипЗНЧ(OrderProperties.Consignee)=Тип("Строка")тогда
		Если СокрЛП(OrderProperties.Consignee)="95768" тогда
...
Функция СформироватьОплатуПлатежнымиКартами(Заказ,Эквайер,ДоговорЭквайринга, TransactionDateTime, ЭтоВозврат = ложь, Контрагент, Сумма, Комментарий = "")
...
Функция setPaymet(OrderGUID, isReturn, Payed, TransactionNumber, TransactionDateTime, customerType, Customer, Summ, Comment)
	...
	Если  ЕстьЗаказ тогда
		попытка
			...
			ИначеЕсли customerType = "ФИЗ_Кредит" тогда
				...
				ДоговорЭквайринга = Справочники.ДоговорыЭквайринга.НайтиПоКоду("000000002");
			КонецЕсли;	
			Оплата = СформироватьОплатуПлатежнымиКартами(ЗаказСсылка,Эквайер, ДоговорЭквайринга, TransactionDateTime,isReturn,Справочники.Контрагенты.ПустаяСсылка(),0);
		исключение
			ОписаниеОшибки = ""+ОписаниеОшибки + " Ошибка при создании Оплаты платежными картами: "+ОписаниеОшибки();
			Success = Ложь;
		конецпопытки;
			...
			попытка 
				...
				Если Success тогда
					...
					ИначеЕсли customerType = "ФИЗ_Кредит" тогда
						...
						ДоговорЭквайринга = Справочники.ДоговорыЭквайринга.НайтиПоКоду("000000002");
					КонецЕсли;	
					Оплата = СформироватьОплатуПлатежнымиКартами(ЗаказСсылка,Эквайер, ДоговорЭквайринга, TransactionDateTime,isReturn,Контрагент,Summ, Comment);
				КонецЕсли;
