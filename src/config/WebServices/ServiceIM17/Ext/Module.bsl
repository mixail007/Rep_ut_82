...
Функция  СформироватьРедактироватьЗаказ(Заказ, Ошибка, Customer, OrderProperties, Goods, Amenities)
	...
	времяНачала=ТекущаяУниверсальнаяДатаВМиллисекундах();
			...
			Если Заказ.ТранспортнаяКомпания = Справочники.Контрагенты.НайтиПоКоду("П019371") и Заказ.ИМСклиентомСогласовано тогда  //ПИТСТОП
...
Функция СоздатьКонтрагента(Подразделение,Customer, МожноБезПредоплаты = ложь)
