...
Функция CreateOrder(Goods, Comment, OrderID, Point, INN)
	...
	Если НЕ OrderID = "" Тогда
		...
		Если Контрагент.Код = "П019142" тогда
			Контрагент = Справочники.Контрагенты.НайтиПоКоду("П017979");
		конецЕсли;	
	...
	Если Success И OrderID<>"" Тогда
		...
		Если INN = "760214780588" тогда//Скрипченко, номера точек другие
			...
			ИначеЕсли Point = 6 Тогда //Ленинградский
				...
				Заказ.НомерТорговойТочкиКонтрагента = 7476;
			КонецЕсли;
		иначе
			Если Point = 2 Тогда  //Мостецкая
				ДополнениеККомментарию = "Код склада: 00182";
