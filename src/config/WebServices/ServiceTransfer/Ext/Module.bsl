...
Функция ChangeOrder(Number="", ArrayOfProducts=НЕОПРЕДЕЛЕНО )
	...
	Попытка
		...
		преф = лев(Number,2);
		Если преф="00" тогда преф="ТК" КонецЕсли;//09.01.2018
		...
		Запрос.Текст ="ВЫБРАТЬ ПЕРВЫЕ 1
			...
			|	И ЗаданиеНаПеремещение.Номер = &Number
			|   И ЗаданиеНаПеремещение.Подразделение = &Подразделение
	   		|УПОРЯДОЧИТЬ ПО
			|	ЗаданиеНаПеремещение.Дата
			|АВТОУПОРЯДОЧИВАНИЕ";
		Запрос.УстановитьПараметр("Number", преф+формат(число(Number),"ЧЦ=5; ЧВН=; ЧГ=0")  ); //+++ 2017, 2019г. НОМЕР ЗАДАНИЯ НА ПЕРЕМЕЩЕНИЕ 5-ти значный!
		результат = Запрос.Выполнить();
		
		Если результат.Пустой() Тогда
			Запрос.УстановитьПараметр("Number", преф+формат(число(Number),"ЧЦ=6; ЧВН=; ЧГ=0")  ); //НОМЕР ЗАДАНИЯ НА ОТГРУЗКУ 6-ти значный!
			Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
