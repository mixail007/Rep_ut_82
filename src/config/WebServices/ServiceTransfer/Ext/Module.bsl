...
//--------------------Общий ЛОГ--------------------------------------	
	файлРез = новый ЗаписьТекста;
	файлРез.Открыть("D:\1C\Android\RESULT.txt", КодировкаТекста.UTF8, , Истина);
	файлРез.ЗаписатьСтроку( строка(ТекущаяДата())+" "+строка(Инвентаризация)+" ==> "+рез );
	файлРез.Закрыть();
...
//22.11.2016 =================ЛОГ файл ВСЕГДА! ==================	
	 ЗаписьXML = Новый ЗаписьXML;
	 ИмяФайлаЛОГА = "D:\1C\Android\insideMoved_"+строка(Number)+".xml";
	 ЗаписьXML.ОткрытьФайл(ИмяФайлаЛОГА);
...
//22.11.2016 =================ЛОГ файл ВСЕГДА! ==================			
	 ЗаписьXML = Новый ЗаписьXML;
	 ИмяФайлаЛОГА = "D:\1C\Android\Moved_"+строка(Number)+".xml";
	 ЗаписьXML.ОткрытьФайл(ИмяФайлаЛОГА);
	 ФабрикаXDTO.ЗаписатьXML(ЗаписьXML, ArrayOfCells);
	 ЗаписьXML.Закрыть();			
//==============================================================	
