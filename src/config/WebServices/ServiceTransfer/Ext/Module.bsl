...
Функция ChangeOrder(Number="", ArrayOfProducts=НЕОПРЕДЕЛЕНО )
	...
	ЗаданиеСсылка = ПолучитьЗадание(НачДата, КонДата, Number, "ЗаданиеНаПеремещение", 5);
	
	Если ЗаданиеСсылка = Неопределено Тогда
		ЗаданиеСсылка = ПолучитьЗадание(НачДата, КонДата, Number, "ЗаданиеНаОтгрузку", 6);
		Если ЗаданиеСсылка = Неопределено Тогда
...
//===============================================================
	...
	Исключение
		Возврат "Error on open Document "+ЗаданиеСсылка.Номер+" !!!
		         |"+ОписаниеОшибки();
	КонецПопытки;
...
//22.11.2016-------------доп.проверка----------------------
	...
	Попытка
		глТекущийПользователь = справочники.Пользователи.НайтиПоКоду("ClientRazumov");
...
//--------------------ТОВАРЫ из Заказа/1-го перемещения + Прихода/2-го перемещения с номером строки 1000+N-------------------
	...
	пока Выборка1.Следующий() Цикл	
		...
		Если Выборка1.КоличествоШК=1 тогда		
			Если НЕ ЗначениеЗаполнено(СокрЛП(Выборка1.barcode)) или стрДлина(СокрЛП(Выборка1.barcode))>13 тогда //+++ 10.05.2018
		...
		Иначе // 2 и более ШК
			...
			пока выборка2.Следующий() цикл
				...
				Если НЕ ЗначениеЗаполнено(ШК2) или стрДлина(ШК2)>13 тогда//+++ 10.05.2018
...
процедура ЗаполнитьШапкуПеремещенияПоПриходу(док, док0)
...
процедура ЗаполнениеЦенИВесаВПриходеПоЗаказу( ПриходОб, ЗаказСсылка )
	...
	для каждого стр1 из ПриходОб.Товары цикл
		...
		Если Тип = 0 тогда // ещё и цены из Заказа
			...
			//Стало
			Если ПриходОб.УчитыватьНДС тогда
				ЗаполнитьСтавкуНДСТабЧасти(стр1, ПриходОб);
			иначе
				стр1.СтавкаНДС = перечисления.СтавкиНДС.БезНДС;
			КонецЕсли;
			
			РассчитатьСуммуТабЧасти(стр1, ПриходОб);
			РассчитатьСуммуНДСТабЧасти(стр1, ПриходОб);
		КонецЕсли;	
	КонецЦикла;
...
//====================================================
	...
	Если НомерПрихода="" тогда // Автоматически создаём документ
		...
		Иначе
			...
			ЗаполнитьШапкуПеремещенияПоПриходу(ПриходОб, ЗаказСсылка); //+++ 24.01.2019
		КонецЕсли;	
	Иначе
		Если Type=0 тогда
...
функция CreateMoveGoods( ArrayOfCells )
	...
	//+++( 28.05.2018 явное указание параметров
	//ЗаполнитьШапкуДокумента(ДокПеремОб, глТекущийПользователь);
	ДокПеремОб.Организация = справочники.Организации.НайтиПоКоду("00001");
	ДокПеремОб.Подразделение = справочники.Подразделения.НайтиПоКоду("00005");
	ДокПеремОб.Ответственный = глТекущийПользователь;
	//+++)
	
	//+++ 06.02.2018  ---------------------------------------
	ДокПеремОб.ОтражатьВБухгалтерскомУчете = ЛОЖЬ;
	ДокПеремОб.ОтражатьВНалоговомУчете     = ЛОЖЬ;
	ДокПеремОб.Комментарий = "НЕ ПРОВОДИТЬ!  НЕ ПЕРЕНОСИТЬ В БУХГАЛТЕРИЮ!";
	...
	КонецЦикла;	
	ДокПеремОб.Комментарий = ДокПеремОб.Комментарий+"
	|>> через Android "+строка(ТекущаяДата());
...
//записывает данные в документ
Функция ChangeInventory(Number="", ArrayOfCells=НЕОПРЕДЕЛЕНО )
	type=1; // новые товары!	
	рез= "";
	//--------------проверка входных данных--------------------------	
	Если ArrayOfCells=НЕОПРЕДЕЛЕНО Тогда
		возврат "Error. Array of Products is Null!";
	КонецЕсли;
	
//22.11.2016 =================ЛОГ файл ВСЕГДА! ==================			
	 ЗаписьXML = Новый ЗаписьXML;
	 ИмяФайлаЛОГА = "D:\1C\Android\inv_"+строка(Number)+".xml";
	 ЗаписьXML.ОткрытьФайл(ИмяФайлаЛОГА);
	 ФабрикаXDTO.ЗаписатьXML(ЗаписьXML, ArrayOfCells);
	 ЗаписьXML.Закрыть();			
...
//===============================================================
	Инвентаризация =НайтиИнвентаризациюПоНомеру(Number);
	Если Инвентаризация=неопределено тогда
		возврат "Error. NOT Found document "+Number+"!!!";
	КонецЕсли;
	
	попытка
		ИнвентаризацияОб = Инвентаризация.ПолучитьОбъект();
	исключение
		возврат "Error. Document - not open! "+ОписаниеОшибки();
	КонецПопытки;
	
	Если ИнвентаризацияОб.ПометкаУдаления Тогда
		рез="Error. Document - already deleted! Rewrite is not available.";
	КонецЕсли;
	Если ИнвентаризацияОб.Проведен Тогда
		рез="Error. Document - already done! Rewrite is not available.";
	КонецЕсли;
...
//=================Основной цикл==============================================
	Для Каждого Стр1 ИЗ ArrayOfCells.Cell Цикл
		...
		// определяем новая это строка или изменение в старой
		Если лев(ШК,2) = "99" тогда // это просто склад! без адреса >> Товары!
			СкладКод = сред(ШК, 3,5); 
			Склад = справочники.Склады.НайтиПоКоду(СкладКод);
			отборT = новый Структура;
			отборT.Вставить("Номенклатура", тов);
			...
			Если Инвентаризация.Товары.Количество()>0 тогда
				строкиТ = ИнвентаризацияОб.Товары.НайтиСтроки( отборT );
				Если строкиТ.Количество()>0 тогда
					стрТ = строкиТ[0];
				Иначе	
					стрТ = ИнвентаризацияОб.Товары.Добавить();
					ЗаполнитьЗначенияСвойств(стрТ, отборT);
				КонецЕсли;
			Иначе	
				стрТ = ИнвентаризацияОб.Товары.Добавить();
		...
		Иначе  //-------------------табл.адресов----------------------
			...
			отборА  = новый Структура("АдресХранения, Номенклатура", Адр1, тов);
			строкиА = ИнвентаризацияОб.ТоварыАдресноеХранение.НайтиСтроки( отборА );
			Если строкиА.Количество()>0 тогда
				стрА = строкиА[0];
			Иначе	
				стрА = 	ИнвентаризацияОб.ТоварыАдресноеХранение.Добавить();
				ЗаполнитьЗначенияСвойств(стрА, отборА);
			КонецЕсли;
			стрА.Количество = кол; // меняем количество!
			стрА.Склад 		= Склад;
...
//19.03.2019 =================ЛОГ файл ВСЕГДА! ==================	
	 Number1 = формат(ТекущаяДата(),"ДЛФ=DT"); Number1 = стрЗаменить(Number1,":","-");
	 ЗаписьXML = Новый ЗаписьXML;
	 ИмяФайлаЛОГА = "D:\1C\Android\inv_"+строка(Number1)+".xml";
	 ЗаписьXML.ОткрытьФайл(ИмяФайлаЛОГА);
	 ФабрикаXDTO.ЗаписатьXML(ЗаписьXML, ArrayOfCells);
	 ЗаписьXML.Закрыть();
...
//------------ Запись инвентаризации --------------------------------
	Попытка
		ТекПользователь = справочники.Пользователи.НайтиПоКоду("Робот (магазин)");
		УстановитьПараметрыСеанса(ТекПользователь);
		ИнвентаризацияОб.Проведен = ЛОЖЬ; // НЕ проводим!
		ИнвентаризацияОб.Записать(РежимЗаписиДокумента.Запись);
		Number = ИнвентаризацияОб.Номер;
	Исключение
		рез = "Error. Not saved document "+Инвентаризация.Номер+"! 
		|"+ОписаниеОшибки();
	КонецПопытки;
	
	Если рез = "" Тогда 
		рез = "Ok. "+Number; //14.07.2017 
	КонецЕсли;
	
//--------------------Общий ЛОГ--------------------------------------	
	файлРез = новый ЗаписьТекста;
	файлРез.Открыть("D:\1C\Android\RESULT.txt", КодировкаТекста.UTF8, , Истина);
	файлРез.ЗаписатьСтроку(Number1+" "+строка(Инвентаризация)+" ==> "+рез );
	файлРез.Закрыть();
...
функция НайтиИнвентаризациюПоНомеру(Number="")
	Если Месяц(ТекущаяДата())=1 Тогда
		НачДата = НачалоМесяца(ТекущаяДата());  //29.12.2016 - для нового года!
	Иначе
		НачДата = НачалоДня(ТекущаяДата()) - 31*86400;  //не ранее 1 месяца
	КонецЕсли;
	КонДата = КонецДня(ТекущаяДата());
	...
	попытка 
		...
		преф="ТК";
		Number2 = Number;
	исключение	 //не число - значит есть префикс!
		Number2 = СокрЛП( прав(Number,стрДлина(Number)-2) ); // вырезаем префикс
	КонецПопытки;
	Ном2 = преф+формат(число(Number),"ЧЦ=6; ЧВН=; ЧГ=0");
	Запрос.УстановитьПараметр("Number", Ном2 ); //+++ ТК000056 - ВСЕГДА 6!
	...
	Запрос.Текст ="ВЫБРАТЬ ПЕРВЫЕ 1
		|	Инвентаризация.Ссылка как Док
		|ИЗ
		|	Документ.ИнвентаризацияТоваровНаСкладе КАК Инвентаризация
		|ГДЕ
		|	Инвентаризация.Дата МЕЖДУ &НачДата И &КонДата
		|	И Инвентаризация.Номер = &Number
		|   И Инвентаризация.Подразделение = &Подразделение
   		|УПОРЯДОЧИТЬ ПО
		|	Инвентаризация.Дата
		|АВТОУПОРЯДОЧИВАНИЕ";
	результат = Запрос.Выполнить();
...
функция ПолучитьЗадание(НачДата, КонДата, Number, назвДок="ЗаданиеНаОтгрузку", длинаКода=6)
	...
	попытка 
		префЧ=число(преф);
		преф="ТК";//числовой код => прибавляем ТК
	исключение	 //не число - значит есть префикс!
		Number = СокрЛП( прав(Number,стрДлина(Number)-2) ); // вырезаем префикс
	КонецПопытки;
	...
	Запрос.УстановитьПараметр("Number", преф+формат(число(Number),"ЧЦ="+строка(длинаКода)+"; ЧВН=; ЧГ=0")  ); //+++ 2017, 2019г. НОМЕР ЗАДАНИЯ НА ПЕРЕМЕЩЕНИЕ 5-ти значный!
	
	// Сакулина. Добавила условие на поиск перемещения по такому же номеру?!
	Запрос.Текст ="ВЫБРАТЬ ПЕРВЫЕ 1
		|	Задание.Ссылка
		|ИЗ
		|	Документ."+назвДок+" КАК Задание
		|ГДЕ
		|	Задание.Дата МЕЖДУ &НачДата И &КонДата
		|	И Задание.Номер = &Number
		|   И Задание.Подразделение = &Подразделение
   		|УПОРЯДОЧИТЬ ПО
		|	Задание.Дата
		|АВТОУПОРЯДОЧИВАНИЕ";
	результат = Запрос.Выполнить();
	...
	возврат ЗаданиеСсылка;
		
КонецФункции	

Функция CreateInsideMoved(ArrayOfCells2=НЕОПРЕДЕЛЕНО)
	рез = "Not ready!";
	Number = "<not saved>";
	//--------------проверка входных данных--------------------------	
	Если ArrayOfCells2=НЕОПРЕДЕЛЕНО Тогда
		возврат "Error. Array of Products is Null!";
	КонецЕсли;

	докОб = Документы.ВнутреннееПеремещениеТоваров.СоздатьДокумент();
	Склад = неопределено;
	Для Каждого Стр1 ИЗ ArrayOfCells2.Cell2 Цикл
		Тов = справочники.Номенклатура.НайтиПоКоду(	стр1.productid );
		Кол = стр1.quantity;
		
	    ячОтправитель = справочники.АдресаХранения.НайтиПоКоду(лев(стр1.cell_out,7));
	    ячПолучатель  = справочники.АдресаХранения.НайтиПоКоду(лев(стр1.cell_in,7));
		если Склад=неопределено тогда
			Склад = ячОтправитель.Владелец;
		ИначеЕсли Склад <> ячОтправитель.Владелец тогда
			рез="Error. Cells of different Stores! "+строка(Склад)+" <> "+строка(ячОтправитель.Владелец);
		КонецЕсли;	
		стрДок = докОб.Товары.Добавить();
		стрДок.Номенклатура = Тов;
	    стрДок.Количество   = Кол;
		стрДок.АдресОтправитель = ячОтправитель;
	    стрДок.АдресПолучатель  = ячПолучатель;
		
		ЗаполнитьДругиеПоляПеремещения(стрДок);
	КонецЦикла;
	
 	ЗаполнитьШапкуПеремещения(докОб, Склад, Склад);
	
//19.03.2019 =================ЛОГ файл ВСЕГДА! ==================	
	 Number1 = формат(ТекущаяДата(),"ДЛФ=DT"); Number1 = стрЗаменить(Number1,":","-");
	 ЗаписьXML = Новый ЗаписьXML;
	 ИмяФайлаЛОГА = "D:\1C\Android\insideMoved_"+строка(Number1)+".xml";
	 ЗаписьXML.ОткрытьФайл(ИмяФайлаЛОГА);
	 ФабрикаXDTO.ЗаписатьXML(ЗаписьXML, ArrayOfCells2);
	 ЗаписьXML.Закрыть();	
	 
	попытка
		ТекПользователь = справочники.Пользователи.НайтиПоКоду("Робот (магазин)");
		УстановитьПараметрыСеанса(ТекПользователь);
		докОб.Проведен = ЛОЖЬ; // НЕ проводим!
		докОб.Записать(РежимЗаписиДокумента.Запись);
		докОб.Записать();
		 рез = "";
		 Number = докОб.Номер;
	исключение	
		 рез = "Error. Not saved document! "+ОписаниеОшибки();
	КонецПопытки;	
	 
	 //--------------------19.03.2019---------------------------------------	
	файлРез = новый ЗаписьТекста;
	файлРез.Открыть("D:\1C\Android\RESULT.txt", КодировкаТекста.UTF8, , Истина);
	файлРез.ЗаписатьСтроку(Number1+" Внутреннее перемещение товаров "+?(рез="",Number,"")+" ==> "+рез );
	файлРез.Закрыть();
//==============================================================	
	Если рез = "" Тогда 
		рез = "Ok. "+Number; //14.07.2017 
...
Функция CreateMoved(ArrayOfCells=НЕОПРЕДЕЛЕНО)
	рез= "Not ready!";
	Number = "<not saved>";
	//--------------проверка входных данных--------------------------	
	Если ArrayOfCells=НЕОПРЕДЕЛЕНО Тогда
	...
	Для Каждого Стр1 ИЗ ArrayOfCells.Cell Цикл
		...
		Кол = стр1.quantityfact;
		ШК = стр1.stockcell;
		Если лев(ШК,2)="99" тогда // это склад
			код1   		 = сред(ШК,3,5);
			Склад1		 = справочники.Склады.НайтиПоКоду(код1);
			
			стрТов = ДокОб.Товары.Добавить(); // в таблицу Товары!
			стрТов.Номенклатура = Тов;
		    стрТов.Количество   = Кол;
			стрТов.КоличествоФакт = Кол;
			стрТов.Склад = Склад1;
			ЗаполнитьДругиеПоляПеремещения(стрТов);
			
		Иначе//ячейка
			яч = справочники.АдресаХранения.НайтиПоКоду(лев(ШК,7));
			Склад1 = яч.Владелец;
			Если яч.Пустая() тогда
				рез    = "Error. Not found Cell by code "+ШК;
				Склад1 = справочники.Склады.НайтиПоКоду("00904"); // 8000. адресного хранения
			КонецЕсли;	
			
			стрЯч = ДокОб.ТоварыАдресноеХранение.Добавить(); // в таблицу Адр.Хранения!
			стрЯч.Номенклатура = Тов;
		    стрЯч.Количество   = Кол;
			стрЯч.АдресХранения= яч;
			стряч.Склад		   = Склад1;
			
			//автозаполнение в Товары
			стрДокМас = ДокОб.Товары.НайтиСтроки( новый Структура("Номенклатура, Склад", Тов, Склад1) );
			если стрДокМас.Количество()=0 тогда
				стрТов = ДокОб.Товары.Добавить();
			иначе// должна быть только 1 строка! 
				стрТов = стрДокМас[0];
			КонецЕсли;	
		...
		Если СкладОтправитель = Неопределено тогда
			СкладОтправитель = Склад1; 
			СкладПолучатель  = Склад1;
		ИначеЕсли СкладОтправитель <> Склад1 тогда // МОЖНО с разных складов делать перемещение на 1 
			СкладПолучатель = справочники.Склады.ПустаяСсылка();
		КонецЕсли;
	КонецЦикла;
    ЗаполнитьШапкуПеремещения(ДокОб, СкладОтправитель, СкладПолучатель);
	
//19.03.2019 =================ЛОГ файл ВСЕГДА! ==================	
	 Number1 = формат(ТекущаяДата(),"ДЛФ=DT"); Number1 = стрЗаменить(Number1,":","-");
	 ЗаписьXML = Новый ЗаписьXML;
	 ИмяФайлаЛОГА = "D:\1C\Android\Moved_"+строка(Number1)+".xml";
	 ЗаписьXML.ОткрытьФайл(ИмяФайлаЛОГА);
	 ФабрикаXDTO.ЗаписатьXML(ЗаписьXML, ArrayOfCells);
	 ЗаписьXML.Закрыть();
...
//--------------------19.03.2019---------------------------------------	
	файлРез = новый ЗаписьТекста;
	файлРез.Открыть("D:\1C\Android\RESULT.txt", КодировкаТекста.UTF8, , Истина);
	файлРез.ЗаписатьСтроку( Number1+" Перемещение товаров "+?(рез="",Number,"")+" ==> "+рез );
	файлРез.Закрыть();
...
процедура ЗаполнитьДругиеПоляПеремещения(стрДок)
	стрДок.ЕдиницаИзмерения = стрДок.Номенклатура.ЕдиницаХраненияОстатков;
	стрДок.Коэффициент      = 1; //всегда в шт. 
	стрДок.Вес = 
//------не заполняется специально, но может быть выбрана впучную------------	
	//стрДок.ЕдиницаИзмеренияМест = стрДок.ЕдиницаИзмерения; 
	//стрДок.КоличествоМест       = стрДок.Количество; 
	
	стрДок.Качество	   = справочники.Качество.Новый;
	стрДок.СпособСписанияОстаткаТоваров = перечисления.СпособыСписанияОстаткаТоваров.СоСклада;
КонецПроцедуры

процедура ЗаполнитьШапкуПеремещения(докОб, СкладОтправитель, СкладПолучатель)
	докОб.Дата = ТекущаяДата();
	ДокОб.ВидОперации = Перечисления.ВидыОперацийПеремещениеТоваров.ТоварыПродукция;
	докОб.Организация = справочники.Организации.НайтиПоКоду("00001");
    докОб.ОтражатьВУправленческомУчете = истина;
	докОб.СкладОтправитель = СкладОтправитель;
	докОб.СкладПолучатель  = СкладПолучатель;
	докОб.Подразделение = ?(СкладОтправитель.Подразделение.Пустая(), справочники.Подразделения.НайтиПоКоду("00005"),СкладОтправитель.Подразделение);
	докОб.Комментарий	= "Android: "+строка(ТекущаяДата());
	
	глТекущийПользователь = справочники.Пользователи.НайтиПоКоду("Робот (Магазин)");
	ПараметрыСеанса.ТекущийПользователь = глТекущийПользователь;
 	докОб.Ответственный	= глТекущийПользователь;
КонецПроцедуры
