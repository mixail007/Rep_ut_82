...
Функция ChangeInventory(Number="", ArrayOfCells=НЕОПРЕДЕЛЕНО )
	...
	Если Number="" тогда
		рез="Error. Empty number!!!";
	КонецЕсли;	
	Если ArrayOfCells=НЕОПРЕДЕЛЕНО Тогда
		рез="Error. Array of Products is Null!";
	КонецЕсли;
	
//22.11.2016 =================ЛОГ файл ВСЕГДА! ==================			
	 ЗаписьXML = Новый ЗаписьXML;
	 ИмяФайлаЛОГА = "D:\1C\Android\inv_"+строка(Инвентаризация.Номер)+".xml";
	 ЗаписьXML.ОткрытьФайл(ИмяФайлаЛОГА);
	 ФабрикаXDTO.ЗаписатьXML(ЗаписьXML, ArrayOfCells);
	 ЗаписьXML.Закрыть();			
//===============================================================
...
функция НайтиИнвентаризациюПоНомеру(Number="")
	...
	КонецПопытки;
	Запрос.УстановитьПараметр("Number", преф+формат(число(Number),"ЧЦ=6; ЧВН=; ЧГ=0")  ); //+++ ТК000056 - ВСЕГДА 6!
