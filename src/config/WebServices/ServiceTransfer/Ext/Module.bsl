...
Функция CreateInsideMoved(ArrayOfCells2=НЕОПРЕДЕЛЕНО)
	...
	докОб.СкладОтправитель = Склад;
	докОб.СкладПолучатель  = Склад;
	докОб.Подразделение = ?(Склад.Подразделение.Пустая(), справочники.Подразделения.НайтиПоКоду("00005"),Склад.Подразделение);
	докОб.Комментарий	= "Android";
	//докОб.Ответственный	= справочники.Пользователи.НайтиПоКоду();
	попытка
		 докОб.Записать();
		 рез = "";
		 Number = докОб.Номер;
...
Функция CreateMoved(ArrayOfCells)
	рез= "Not ready!";
	//--------------проверка входных данных--------------------------	
	Если ArrayOfCells=НЕОПРЕДЕЛЕНО Тогда
		возврат "Error. Array of Products is Null!";
...
процедура ЗаполнитьДругиеПоляПеремещения(стрДок)
	...
	стрДок.КоличествоМест = ?(стрДок.ЕдиницаИзмерения.Коэффициент=0,стрДок.Количество, стрДок.Количество/стрДок.ЕдиницаИзмерения.Коэффициент);
