...
Функция СформироватьРеализации(PointId, Partner, Goods, Receipts, GuidYST, IsKTOrder, DateFrom, DateTo)
	...
	Если Success Тогда
		...
		Если ОтчетПоОТХ.Товары.Количество() <> 0 Тогда
			...
			ДокиСформированы = истина;
			Попытка
				ОтчетПоОТХ.СоздатьПакетДокументов(ДоговорРеализации);
			Исключение
				ДокиСформированы = ложь;
				Success = ложь;
				Ошибка = Ошибка + Символы.ПС+ОписаниеОшибки();
			КонецПопытки;
			
			Если Partner<>"П017979" тогда //не скрипченко-> заполним цены из реализаций
				Товары = ФабрикаXDTO.Создать(ТипXDTOТовары);
