...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	ЗаполнитьНачальныеНастройкиПоМетаданнымРегистра(СтруктураПредставлениеПолей, МассивОтбора, ЭтотОбъект, "Диаграмма");
...
Процедура СформироватьОтчет(Диаграмма, ЕстьОшибки = Ложь)	Экспорт
	...
	//	ПостроительОтчета.Параметры.Вставить("ДатаКон", ?(ЗначениеНеЗаполнено(ДатаКон), ДатаКон, НачалоДня(ДатаКон + 60*60*24)));
	//Иначе
	...
	ОбработкаПоказателейДобавитьВВыбранныеПоля(ИмяПоказателя, ПостроительОтчета);
...
Процедура ОбработкаРасшифровкиСтандартногоОтчета(Расшифровка, СтандартнаяОбработка, ЭтотОтчет) Экспорт
	...
	Если ТипЗнч(Расшифровка) = Тип("Структура") Тогда
		...
		Если Выбор.Значение = 0 Тогда
			...
			Если НЕ ЗначениеНеЗаполнено(ИмяОтчетаДляРасшифровкиПоДокументам) Тогда
