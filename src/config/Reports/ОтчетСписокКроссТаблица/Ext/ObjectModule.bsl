...
//+++ 24.01.2012 доп. параметр ПроверятьПрава
//для отчета Остатки на отв.Хранении - ПроверятьПрава=ЛОЖЬ
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь,  ЕстьОшибки = Ложь, УказыватьРасшифровки = Истина, ПроверятьПрава = Истина, ДатаВремя = Ложь) Экспорт

	Если ПроверятьПрава тогда //+++ 24.01.2012
		
		//+++ 03.07.2014 - 15.10.2014 - для группы АКБ - отбор по виду товара по всем клиентам и всем менеджерам... (по заявке от Киреева)
		//
		//группаПользователя = ПолучитьЗначениеПоУмолчанию(ПараметрыСеанса.ТекущийПользователь, "ГруппаПользователейДляРаспределенияЗаказов");
		//Если строка(группаПользователя)="АКБ" Тогда
		//	Если ПостроительОтчета.Отбор.Найти("НоменклатураВидТовара")=неопределено тогда
		//		ПостроительОтчета.Отбор.Добавить("Номенклатура.ВидТовара", "НоменклатураВидТовара", "Номенклатура.Вид товара");
		//	КонецЕсли;	
		//ПостроительОтчета.Отбор.НоменклатураВидТовара.ВидСравнения	= ВидСравнения.Равно;
		//  	ПостроительОтчета.Отбор.НоменклатураВидТовара.Значение	= перечисления.ВидыТоваров.АКБ;
		//ПостроительОтчета.Отбор.НоменклатураВидТовара.Использование	= Истина;
		//Иначе // как было
			УстановитьОтборПоУниверсальномуОтчету(ПостроительОтчета);
		//КонецЕсли;//+++) 
...
//					КонецЕсли;
	...
	ЗаголовокОтчета = СформироватьЗаголовок(МакетОтчета, КоличествоКолонок, ТолькоЗаголовок, ВыводитьДетальныеЗаписи);
	...
	Для Каждого Показатель Из Показатели Цикл
		Если Показатель.Использование Тогда
			...
			Попытка
				ОбработкаПоказателейДобавитьВВыбранныеПоля(Показатель.Имя, ПостроительОтчета);
			Исключение
			КонецПопытки;
	...
	Для Сч = 0 По ПостроительОтчета.ИзмеренияСтроки.Количество()-1 Цикл
		...
		Для Каждого Показатель Из Показатели Цикл
			...
			Если Показатель.Использование = Истина Тогда
				...
				//+++( 12.01.2012
				//ФорматПоказателя =мТаблицаПоказатели.Найти(Показатель.Имя).ФорматнаяСтрока;
				показатель1 = мТаблицаПоказатели.Найти(Показатель.Имя);
				Если не показатель1 = неопределено тогда
				ФорматПоказателя = показатель1.ФорматнаяСтрока;
				ВыводитьПоказатель = Истина;
                ВыводитьПоказательПоИерархии = Истина;
                 иначе
					 сообщить("Не найден показатель: "+Показатель.Имя);
                ФорматПоказателя = "ЧДЦ=2; ДЛФ=D";
	...
	Если ПостроительОтчета.ИзмеренияКолонки.Количество() = 0 Тогда
	    ...
	    // {--Лапенков 20060725
		//Если мНазваниеОтчета="Отчет по валовой прибыли" Тогда
		//	//	ДокументРезультат.Область(ДокументРезультат.ВысотаТаблицы-1, 2, ДокументРезультат.ВысотаТаблицы-1, 2).Текст="тест";
		//	//	ПостроительОтчета.ВыбранныеПоля.Количество()
		//	
		//	индРентабельностьПродаж=-1;
		//	индПроцентНаценки=-1;
		//	Для инд=0 по МассивПоказателей.Количество()-1 Цикл
		//		Если МассивПоказателей[инд]="РентабельностьПродаж" Тогда
		//			индРентабельностьПродаж=инд;
		//		ИначеЕсли МассивПоказателей[инд]="ПроцентНаценки" Тогда
		//			индПроцентНаценки=инд;
		//		КонецЕсли;
		//	КонецЦикла;	
		//	Рентабельность=0;
		//	ПроцентПрибыли=0;
		//	Рез=ПостроительОтчета.Результат;
		//	ВыбИтоги=Рез.Выбрать();
		//	Если ВыбИтоги.Следующий() Тогда
		//		Если  (ВыбИтоги.СебеСтоимость<>NULL) Тогда 
		//			Если (ВыбИтоги.Стоимость>0) и  (ВыбИтоги.Себестоимость>0) Тогда
		//				Рентабельность= 100*(ВыбИтоги.Стоимость-ВыбИтоги.Себестоимость)/ВыбИтоги.Стоимость;
		//				ПроцентПрибыли=100*(ВыбИтоги.Стоимость-ВыбИтоги.Себестоимость)/ВыбИтоги.Себестоимость;
		//			КонецЕсли;	   
		//		КонецЕсли;	   
		//	КонецЕсли;	
		//	
		//	Если 	(индПроцентНаценки>0) и (ПроцентПрибыли<>0) Тогда
		//		ДокументРезультат.Область(ДокументРезультат.ВысотаТаблицы-1, ШиринаТаблицы , ДокументРезультат.ВысотаТаблицы-1, ШиринаТаблицы  ).Текст=Формат(ПроцентПрибыли,"ЧДЦ=2");
		//		ДокументРезультат.Область(ДокументРезультат.ВысотаТаблицы-1, ШиринаТаблицы -1, ДокументРезультат.ВысотаТаблицы-1, ШиринаТаблицы -1 ).Текст=Формат(Рентабельность,"ЧДЦ=2");
		//	ИначеЕсли (индПроцентНаценки=-1) и (индРентабельностьПродаж>0) Тогда
		//		ДокументРезультат.Область(ДокументРезультат.ВысотаТаблицы-1, ШиринаТаблицы, ДокументРезультат.ВысотаТаблицы-1, ШиринаТаблицы ).Текст=Формат(Рентабельность,"ЧДЦ=2");
		//	КонецЕсли;
