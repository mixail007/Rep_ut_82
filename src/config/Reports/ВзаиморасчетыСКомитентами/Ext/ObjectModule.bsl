...
Функция СформироватьТекстЗапроса(Источник,КопияГруппировкиОтчета,СтруктураДоступныхГруппировок,СтруктураДоступныхПоказателей)
	...
	Если Источник="ПартииТоваровНаСкладах.ОстаткиИОбороты" Тогда
		ТекстИсточника="РегистрНакопления."+Источник+"(&ДатаНач,&ДатаКон,,,СтатусПартии=&ТоварПринятыйКомиссия";
		
		Если Не ПустаяСтрока(ТекстФильтры) Тогда 
			ТекстИсточника = ТекстИсточника + Символы.ПС + " И "+ТекстФильтры;
		КонецЕсли;
		ТекстИсточника= ТекстИсточника + ") КАК ПартииТоваровКомпанииОстаткиИОбороты";
		...
		КонецЕсли;
	ИначеЕсли Источник="ПартииТоваровНаСкладах.Обороты" Тогда
		ТекстИсточника="РегистрНакопления.ПартииТоваровНаСкладах.Обороты(&ДатаНач,&ДатаКон,Регистратор,СтатусПартии=&ТоварПринятыйКомиссия";
		
		Если Не ПустаяСтрока(ТекстФильтры) Тогда 
			ТекстИсточника = ТекстИсточника + Символы.ПС + " И "+ТекстФильтры;
		КонецЕсли;
		ТекстИсточника= ТекстИсточника + ") КАК ПартииТоваровКомпанииОбороты";

        ТекстИсточника = ТекстИсточника + ТекстИсточникиСведений;
		
		Если Не ПустаяСтрока(ТекстОбщиеФильтры) Тогда
			ТекстИсточника = ТекстИсточника  + Символы.ПС +
			"ГДЕ (" + ТекстОбщиеФильтры + ") И (Регистратор ССЫЛКА Документ.ПеремещениеТоваров)";
		Иначе
			ТекстИсточника = ТекстИсточника  + Символы.ПС +
			"ГДЕ (Регистратор ССЫЛКА Документ.ПеремещениеТоваров)";
		КонецЕсли;
	...
	ИначеЕсли Источник="РеализованныеТовары.ОстаткиИОбороты" Тогда
		...
		КонецЕсли;
		ТекстИсточника= ТекстИсточника + ") КАК РеализованныеТоварыОстаткиИОбороты

		|ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|	ЗаказПокупателя КАК ДокументПродажи,
		|	Номенклатура,
		|	ХарактеристикаНоменклатуры,
		|	Выбор Когда КоличествоОборот <> 0 Тогда СтоимостьОборот / КоличествоОборот Иначе 0 Конец КАК Цена
		|		ИЗ РегистрНакопления.ПродажиСебестоимость.Обороты) КАК ПродажиКомпанииОбороты
	...
	ИначеЕсли Источник="ВзаиморасчетыСКонтрагентами" Тогда
		ТекстИсточника="РегистрНакопления."+Источник+" КАК КонтрагентыВзаиморасчетыКомпании"+ТекстИсточникиСведений+"
		|ГДЕ
		|КонтрагентыВзаиморасчетыКомпании.ДоговорКонтрагента В (&МассивКомитентов)
		|И КонтрагентыВзаиморасчетыКомпании.Активность = Истина 
		|И КонтрагентыВзаиморасчетыКомпании.ВидДвижения = &Приход";

		Если (ДатаНач<>'00010101000000') и (ДатаКон<>'00010101000000') Тогда
			ТекстИсточника=ТекстИсточника+"
			| И (КонтрагентыВзаиморасчетыКомпании.Период МЕЖДУ &ДатаНач И &ДатаКон)";
		ИначеЕсли ДатаНач<>'00010101000000' Тогда
			ТекстИсточника=ТекстИсточника+"
			| И (КонтрагентыВзаиморасчетыКомпании.Период >= &ДатаНач)";
		ИначеЕсли ДатаКон<>'00010101000000' Тогда
			ТекстИсточника=ТекстИсточника+"
			| И (КонтрагентыВзаиморасчетыКомпании.Период <= &ДатаКон)";
		КонецЕсли;
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь,  ЕстьОшибки = Ложь) Экспорт
	...
	// По регистру "ПартииТоваровКомпании" (принятые на комиссию): остатки и обороты.
	СтруктураДоступныхГруппировок.Вставить("Контрагент",							"ПартииТоваровКомпанииОстаткиИОбороты.ДокументОприходования.ДоговорКонтрагента.Владелец");
	СтруктураДоступныхГруппировок.Вставить("ДоговорКонтрагента",					"ПартииТоваровКомпанииОстаткиИОбороты.ДокументОприходования.ДоговорКонтрагента");
	СтруктураДоступныхГруппировок.Вставить("Организация",							"ПартииТоваровКомпанииОстаткиИОбороты.ДокументОприходования.ДоговорКонтрагента.Организация");
	СтруктураДоступныхГруппировок.Вставить("ДокументПоставки",						"ПартииТоваровКомпанииОстаткиИОбороты.ДокументОприходования");
	СтруктураДоступныхГруппировок.Вставить("ДокументОтчета",						"&НеВыводить");
	СтруктураДоступныхГруппировок.Вставить("ДокументОплаты",						"&НеВыводить");
	СтруктураДоступныхГруппировок.Вставить("Номенклатура",							"ПартииТоваровКомпанииОстаткиИОбороты.Номенклатура");
	СтруктураДоступныхГруппировок.Вставить("ХарактеристикаНоменклатуры",			"ПартииТоваровКомпанииОстаткиИОбороты.ХарактеристикаНоменклатуры");
	СтруктураДоступныхГруппировок.Вставить("СерияНоменклатуры",						"ПартииТоваровКомпанииОстаткиИОбороты.ХарактеристикаНоменклатуры");

	СтруктураДоступныхПоказателей.Вставить("КоличествоНачальныйОстаток",			"ПартииТоваровКомпанииОстаткиИОбороты.КоличествоНачальныйОстаток");
	СтруктураДоступныхПоказателей.Вставить("КоличествоКонечныйОстаток",				"ПартииТоваровКомпанииОстаткиИОбороты.КоличествоКонечныйОстаток");
	СтруктураДоступныхПоказателей.Вставить("КоличествоПриход",						"ПартииТоваровКомпанииОстаткиИОбороты.КоличествоПриход");
	СтруктураДоступныхПоказателей.Вставить("КоличествоЕдиницДляОтчетовНачальныйОстаток",			"ПартииТоваровКомпанииОстаткиИОбороты.КоличествоНачальныйОстаток*ПартииТоваровКомпанииОстаткиИОбороты.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент/ПартииТоваровКомпанииОстаткиИОбороты.Номенклатура.ЕдиницаДляОтчетов.Коэффициент");
	СтруктураДоступныхПоказателей.Вставить("КоличествоЕдиницДляОтчетовКонечныйОстаток",				"ПартииТоваровКомпанииОстаткиИОбороты.КоличествоКонечныйОстаток*ПартииТоваровКомпанииОстаткиИОбороты.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент/ПартииТоваровКомпанииОстаткиИОбороты.Номенклатура.ЕдиницаДляОтчетов.Коэффициент");
	СтруктураДоступныхПоказателей.Вставить("КоличествоЕдиницДляОтчетовПриход",						"ПартииТоваровКомпанииОстаткиИОбороты.КоличествоПриход*ПартииТоваровКомпанииОстаткиИОбороты.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент/ПартииТоваровКомпанииОстаткиИОбороты.Номенклатура.ЕдиницаДляОтчетов.Коэффициент");
	СтруктураДоступныхПоказателей.Вставить("СуммаНачальныйОстаток",					"ПартииТоваровКомпанииОстаткиИОбороты.СтоимостьНачальныйОстаток");
	СтруктураДоступныхПоказателей.Вставить("СуммаКонечныйОстаток",					"ПартииТоваровКомпанииОстаткиИОбороты.СтоимостьКонечныйОстаток");
	СтруктураДоступныхПоказателей.Вставить("СуммаПриход",							"ПартииТоваровКомпанииОстаткиИОбороты.СтоимостьПриход");
	...
	ТекстЗапроса=ТекстЗапроса+СформироватьТекстЗапроса("ПартииТоваровНаСкладах.ОстаткиИОбороты",
	КопияГруппировкиОтчета,СтруктураДоступныхГруппировок,СтруктураДоступныхПоказателей);
	ТекстЗапроса=ТекстЗапроса+"
	|ОБЪЕДИНИТЬ ВСЕ  //выкидываем перемещения 
	|";
	// По регистру "ПартииТоваровКомпании" (принятые на комиссию): остатки и обороты.
	СтруктураДоступныхГруппировок.Вставить("Контрагент",							"ПартииТоваровКомпанииОбороты.ДокументОприходования.ДоговорКонтрагента.Владелец");
	СтруктураДоступныхГруппировок.Вставить("ДоговорКонтрагента",					"ПартииТоваровКомпанииОбороты.ДокументОприходования.ДоговорКонтрагента");
	СтруктураДоступныхГруппировок.Вставить("Организация",							"ПартииТоваровКомпанииОбороты.ДокументОприходования.ДоговорКонтрагента.Организация");
	СтруктураДоступныхГруппировок.Вставить("ДокументПоставки",						"ПартииТоваровКомпанииОбороты.ДокументОприходования");
	СтруктураДоступныхГруппировок.Вставить("ДокументОтчета",						"&НеВыводить");
	СтруктураДоступныхГруппировок.Вставить("ДокументОплаты",						"&НеВыводить");
	СтруктураДоступныхГруппировок.Вставить("Номенклатура",							"ПартииТоваровКомпанииОбороты.Номенклатура");
	СтруктураДоступныхГруппировок.Вставить("ХарактеристикаНоменклатуры",			"ПартииТоваровКомпанииОбороты.ХарактеристикаНоменклатуры");
	СтруктураДоступныхГруппировок.Вставить("СерияНоменклатуры",						"ПартииТоваровКомпанииОбороты.ХарактеристикаНоменклатуры");
	...
	СтруктураДоступныхПоказателей.Вставить("КоличествоПриход",						"(-1)*ПартииТоваровКомпанииОбороты.КоличествоПриход");
	...
	СтруктураДоступныхПоказателей.Вставить("КоличествоЕдиницДляОтчетовПриход",						"(-1)*ПартииТоваровКомпанииОбороты.КоличествоПриход*ПартииТоваровКомпанииОбороты.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент/ПартииТоваровКомпанииОбороты.Номенклатура.ЕдиницаДляОтчетов.Коэффициент");
	...
	СтруктураДоступныхПоказателей.Вставить("СуммаПриход",							"(-1)*ПартииТоваровКомпанииОбороты.СтоимостьПриход");
	СтруктураДоступныхПоказателей.Вставить("КоличествоПриходРеализовано",			"0");
	СтруктураДоступныхПоказателей.Вставить("КоличествоЕдиницДляОтчетовПриходРеализовано",			"0");
	СтруктураДоступныхПоказателей.Вставить("СуммаПриходРеализовано",				"0");
	СтруктураДоступныхПоказателей.Вставить("КоличествоРасходРеализовано",			"0");
	...
	КопияГруппировкиОтчета,СтруктураДоступныхГруппировок,СтруктураДоступныхПоказателей);
	ТекстЗапроса=ТекстЗапроса+"
	|ОБЪЕДИНИТЬ ВСЕ  //Запрос по задолженности 
	|";

	// По регистру "КонтрагентыВзаиморасчетыКомпании": остатки и обороты.
	СтруктураДоступныхГруппировок.Вставить("Контрагент",							"КонтрагентыВзаиморасчетыКомпанииОстаткиИОбороты.ДоговорКонтрагента.Владелец");
	СтруктураДоступныхГруппировок.Вставить("ДоговорКонтрагента",					"КонтрагентыВзаиморасчетыКомпанииОстаткиИОбороты.ДоговорКонтрагента");
	СтруктураДоступныхГруппировок.Вставить("Организация",							"КонтрагентыВзаиморасчетыКомпанииОстаткиИОбороты.ДоговорКонтрагента.Организация");
	СтруктураДоступныхГруппировок.Вставить("ДокументПоставки",						"&НеВыводить");
	СтруктураДоступныхГруппировок.Вставить("ДокументОтчета",						"&НеВыводить");
	СтруктураДоступныхГруппировок.Вставить("ДокументОплаты",						"&НеВыводить");
	...
	СтруктураДоступныхПоказателей.Вставить("КоличествоЕдиницДляОтчетовРасходРеализовано",			"0");
	СтруктураДоступныхПоказателей.Вставить("СуммаРасходРеализовано",				"0");
	СтруктураДоступныхПоказателей.Вставить("СуммаНачальныйДолг",					"КонтрагентыВзаиморасчетыКомпанииОстаткиИОбороты.СуммаВзаиморасчетовНачальныйОстаток");
	СтруктураДоступныхПоказателей.Вставить("СуммаКонечныйДолг",						"КонтрагентыВзаиморасчетыКомпанииОстаткиИОбороты.СуммаВзаиморасчетовКонечныйОстаток");
	СтруктураДоступныхПоказателей.Вставить("СуммаОплата",							"0");
	СтруктураДоступныхПоказателей.Вставить("СуммаВознаграждение",					"0");
	СтруктураДоступныхПоказателей.Вставить("КоличествоНачальныйОстатокРеализовано",	"0");
	СтруктураДоступныхПоказателей.Вставить("КоличествоЕдиницДляОтчетовНачальныйОстатокРеализовано",	"0");
	...
	КопияГруппировкиОтчета,СтруктураДоступныхГруппировок,СтруктураДоступныхПоказателей);
	ТекстЗапроса=ТекстЗапроса+"
	|ОБЪЕДИНИТЬ ВСЕ   // Запрос по реализованным товарам
	|";

	// По регистру "РеализованныеТовары": остатки и обороты. Стоимость реализованных товаров берется из документов реализации
	// в пересчете по курсу упр. учета на дату реализации.
	...
	СтруктураДоступныхГруппировок.Вставить("ДокументПоставки",						"РеализованныеТоварыОстаткиИОбороты.ДокументПоставки");
	СтруктураДоступныхГруппировок.Вставить("ДокументОтчета",						"&НеВыводить");
	СтруктураДоступныхГруппировок.Вставить("ДокументОплаты",						"&НеВыводить");
	...
	СтруктураДоступныхПоказателей.Вставить("СуммаОплата",							"0");
	СтруктураДоступныхПоказателей.Вставить("СуммаВознаграждение",					"0");
	СтруктураДоступныхПоказателей.Вставить("КоличествоНачальныйОстатокРеализовано",	"РеализованныеТоварыОстаткиИОбороты.КоличествоНачальныйОстаток");
	СтруктураДоступныхПоказателей.Вставить("КоличествоЕдиницДляОтчетовНачальныйОстатокРеализовано",	"РеализованныеТоварыОстаткиИОбороты.КоличествоНачальныйОстаток*РеализованныеТоварыОстаткиИОбороты.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент/РеализованныеТоварыОстаткиИОбороты.Номенклатура.ЕдиницаДляОтчетов.Коэффициент");
    СтруктураДоступныхПоказателей.Вставить("СуммаНачальныйОстатокРеализовано",		"РеализованныеТоварыОстаткиИОбороты.КоличествоНачальныйОстаток*ПродажиКомпанииОбороты.Цена");
	СтруктураДоступныхПоказателей.Вставить("КоличествоКонечныйОстатокРеализовано",	"РеализованныеТоварыОстаткиИОбороты.КоличествоКонечныйОстаток");
	СтруктураДоступныхПоказателей.Вставить("КоличествоЕдиницДляОтчетовКонечныйОстатокРеализовано",	"РеализованныеТоварыОстаткиИОбороты.КоличествоКонечныйОстаток*РеализованныеТоварыОстаткиИОбороты.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент/РеализованныеТоварыОстаткиИОбороты.Номенклатура.ЕдиницаДляОтчетов.Коэффициент");
	СтруктураДоступныхПоказателей.Вставить("СуммаКонечныйОстатокРеализовано",		"РеализованныеТоварыОстаткиИОбороты.КоличествоКонечныйОстаток*ПродажиКомпанииОбороты.Цена");
	ТекстЗапроса=ТекстЗапроса+СформироватьТекстЗапроса("РеализованныеТовары.ОстаткиИОбороты",
	КопияГруппировкиОтчета,СтруктураДоступныхГруппировок,СтруктураДоступныхПоказателей);
	...
	// По документу "ОтчетКомитентуОПродажах": Товары.
	...
	СтруктураДоступныхГруппировок.Вставить("ДокументПоставки",						"ОтчетКомитентуОПродажахТовары.ДокументПоступления");
	СтруктураДоступныхГруппировок.Вставить("ДокументОтчета",						"ОтчетКомитентуОПродажахТовары.Ссылка");
	СтруктураДоступныхГруппировок.Вставить("ДокументОплаты",						"&НеВыводить");
	...
	// По регистру "КонтрагентыВзаиморасчетыКомпании": основная таблица.
	СтруктураДоступныхГруппировок.Вставить("Контрагент",							"КонтрагентыВзаиморасчетыКомпании.ДоговорКонтрагента.Владелец");
	СтруктураДоступныхГруппировок.Вставить("ДоговорКонтрагента",					"КонтрагентыВзаиморасчетыКомпании.ДоговорКонтрагента");
	СтруктураДоступныхГруппировок.Вставить("Организация",							"КонтрагентыВзаиморасчетыКомпании.ДоговорКонтрагента.Организация");
	СтруктураДоступныхГруппировок.Вставить("ДокументПоставки",						"&НеВыводить");
	СтруктураДоступныхГруппировок.Вставить("ДокументОтчета",						"&НеВыводить");
	СтруктураДоступныхГруппировок.Вставить("ДокументОплаты",						"КонтрагентыВзаиморасчетыКомпании.Регистратор");
	СтруктураДоступныхГруппировок.Вставить("Номенклатура",							"&НеВыводить");
	СтруктураДоступныхГруппировок.Вставить("ХарактеристикаНоменклатуры",			"&НеВыводить");
	СтруктураДоступныхГруппировок.Вставить("СерияНоменклатуры",						"&НеВыводить");
	...
	СтруктураДоступныхПоказателей.Вставить("СуммаНачальныйДолг",					"0");
	СтруктураДоступныхПоказателей.Вставить("СуммаКонечныйДолг",						"0");
	СтруктураДоступныхПоказателей.Вставить("СуммаОплата",							"КонтрагентыВзаиморасчетыКомпании.СуммаВзаиморасчетов");
	СтруктураДоступныхПоказателей.Вставить("СуммаВознаграждение",					"0");
	СтруктураДоступныхПоказателей.Вставить("КоличествоНачальныйОстатокРеализовано",	"0");
	СтруктураДоступныхПоказателей.Вставить("КоличествоЕдиницДляОтчетовНачальныйОстатокРеализовано",	"0");
	СтруктураДоступныхПоказателей.Вставить("СуммаНачальныйОстатокРеализовано",		"0");
