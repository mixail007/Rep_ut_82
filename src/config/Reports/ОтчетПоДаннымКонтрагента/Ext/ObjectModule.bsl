...
Функция ПолучитьАВСКласс()
	...
	Если ТаблицаЗапроса.Количество() = 0 Тогда
		Возврат "Не задан";
	Иначе
		Если ЗначениеНеЗаполнено(ТаблицаЗапроса[0].ABCКласс) Тогда
			Возврат "Не задан";
		Иначе
...
Функция ПолучитьСтадиюВзаимоотношений()
	...
	Если ТаблицаЗапроса.Количество() = 0 Тогда
		Возврат "Не задана";
	Иначе
		Если ЗначениеНеЗаполнено(ТаблицаЗапроса[0].Стадия) Тогда
			Возврат "Не задана";
		Иначе
			Возврат (Строка(ТаблицаЗапроса[0].Стадия) + ?(НЕ ЗначениеНеЗаполнено(ТаблицаЗапроса[0].КлассПостоянногоПокупателя), (", " + Строка(ТаблицаЗапроса[0].КлассПостоянногоПокупателя)), ""));
		КонецЕсли; 
	КонецЕсли; 
...
Функция ОпределитьНадежностьПоставщика()
	...
	Если ТаблицаЗапроса.Количество() = 0 Тогда
		Возврат "Не задана";
	Иначе
		Если ЗначениеНеЗаполнено(ТаблицаЗапроса[0].Надежность) Тогда
			Возврат "Не задана";
		Иначе
...
Процедура ВывестиДанные(Таб, ПорядковыйНомер, Имя)
	...
	Если Имя = "ОбщиеДанные" Тогда
		...
		Если НЕ ЗначениеНеЗаполнено(КонтрагентОтчета.ЮрФизЛицо) Тогда
		...
		Если НЕ ЗначениеНеЗаполнено(КонтрагентОтчета.ГоловнойКонтрагент) Тогда
			СтрокаГоловногоКонтрагента = Строка(КонтрагентОтчета.ГоловнойКонтрагент);
			Если НЕ ЗначениеНеЗаполнено(КонтрагентОтчета.ГоловнойКонтрагент.ЮрФизЛицо) Тогда
				Если КонтрагентОтчета.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо Тогда
		...
		Если НЕ ЗначениеНеЗаполнено(КонтрагентОтчета.ОсновнойБанковскийСчет) тогда
			...
			Если Не ЗначениеНеЗаполнено(КонтрагентОтчета.ОсновнойБанковскийСчет.Банк) Тогда
	...
	ИначеЕсли Имя = "КонтЛица" Тогда
		...
		Для каждого СтрокаКЛ Из ТаблицаКЛ Цикл
			...
			КонецЕсли; 
			Если НЕ ЗначениеНеЗаполнено(СтрокаКЛ.Роль) Тогда
	...
	ИначеЕсли Имя = "ПараметрыПоставщикаПокупателя" Тогда
		...
		Если КонтрагентОтчета.Поставщик Тогда
			...
			Секция.Параметры.ВидДоставки          = КонтрагентОтчета.ВидДоставки;
		...
		Если КонтрагентОтчета.Покупатель Тогда
			...
			Секция.Параметры.ИсточникИнформации    = ?(ЗначениеНеЗаполнено(КонтрагентОтчета.ИсточникИнформацииПриОбращении), "Не указан", КонтрагентОтчета.ИсточникИнформацииПриОбращении);
			...
			Если ВыводитьДопКонтИнф И НЕ ЗначениеНеЗаполнено(КонтрагентОтчета.ОсновнойМенеджерПокупателя) И НЕ ЗначениеНеЗаполнено(КонтрагентОтчета.ОсновнойМенеджерПокупателя.ФизЛицо) Тогда
...
Процедура СформироватьОтчет(Таб) Экспорт
	...
	Если ЗначениеНеЗаполнено(КонтрагентОтчета) Тогда
