...
Функция ПолучитьПроизводныйОтборОтДокументаУсловий(ИмяПоляТаблицы, Запрос)
	...
	Для каждого ЭлементОтбора Из Отбор Цикл
		...
		Если ОтборПоДокументуУсловий.ВидСравнения = ВидСравнения.Равно Тогда
			...
			|И
			|" + ПолучитьСтрокуОтбора(ВидСравнения.Равно, "&ЗначениеОтбораДоговорДокументаУсловий", ИмяПоляТаблицы) + "
			|";
		ИначеЕсли ОтборПоДокументуУсловий.ВидСравнения = ВидСравнения.НеРавно Тогда
			Запрос.УстановитьПараметр("ЗначениеОтбораДоговорДокументаУсловий", ОтборПоДокументуУсловий.Значение.ДоговорКонтрагента);
			СтрокаВозврата = СтрокаВозврата + "
			|И
			|" + ПолучитьСтрокуОтбора(ВидСравнения.НеРавно, "&ЗначениеОтбораДоговорДокументаУсловий", ИмяПоляТаблицы) + "
			|";
		ИначеЕсли ОтборПоДокументуУсловий.ВидСравнения = ВидСравнения.ВСписке
			...
			СтрокаВозврата = СтрокаВозврата + "
			|И
			|" + ПолучитьСтрокуОтбора(ВидСравнения.ВСписке, "&ЗначениеОтбораДоговорДокументаУсловий", ИмяПоляТаблицы) + "
			|";
		ИначеЕсли ОтборПоДокументуУсловий.ВидСравнения = ВидСравнения.ВСписке
			  ИЛИ ОтборПоДокументуУсловий.ВидСравнения = ВидСравнения.ВСпискеПоИерархии
			  ИЛИ ОтборПоДокументуУсловий.ВидСравнения = ВидСравнения.ВИерархии Тогда
			...
			СтрокаВозврата = СтрокаВозврата + "
			|И
			|" + ПолучитьСтрокуОтбора(ВидСравнения.НеВСписке, "&ЗначениеОтбораДоговорДокументаУсловий", ИмяПоляТаблицы) + "
			|";
		КонецЕсли;
...
Функция ОтборПоДатам(ИмяПоляТаблицы, Запрос)
	...
	Если ЗначениеНеЗаполнено(ДатаНачала) И ЗначениеНеЗаполнено(ДатаОкончания) Тогда
		Возврат "ИСТИНА = ИСТИНА";
	ИначеЕсли ЗначениеНеЗаполнено(ДатаНачала) И НЕ ЗначениеНеЗаполнено(ДатаОкончания) Тогда
		Запрос.УстановитьПараметр("ДатаОкончания", КонецДня(ДатаОкончания));
		Возврат ("" + ИмяПоляТаблицы + " <= &ДатаОкончания");
	ИначеЕсли НЕ ЗначениеНеЗаполнено(ДатаНачала) И ЗначениеНеЗаполнено(ДатаОкончания) Тогда
...
Функция СформироватьОтборы(СтруктураОтборов, Запрос, УказыватьПервоеИ = Истина)
	...
	Для каждого ЭлементСтруктуры Из СтруктураОтборов Цикл
		...
		Для каждого ЭлементОтбора Из Отбор Цикл
			...
			СтрокаВозврата = СтрокаВозврата + "
			|" + ПолучитьСтрокуОтбора(НайденныйОтбор.ВидСравнения, ("&ЗначениеОтбора" + ЭлементСтруктуры.Ключ), ЭлементСтруктуры.Значение, ("&ЗначениеОтбора" + ЭлементСтруктуры.Ключ + "С"), ("&ЗначениеОтбора" + ЭлементСтруктуры.Ключ + "По"), НайденныйОтбор.Значение, НайденныйОтбор.ЗначениеС, НайденныйОтбор.ЗначениеПо) + "
			|";
...
Функция СобратьТекстЗапросаУсловий(ЗапросУсловий)
	...
	|	УсловияПоставокПоДоговорамКонтрагентовОбщие.Регистратор.НоменклатураДоговора        КАК НоменклатураУсловий,
	...
	|	УсловияПоставокПоДоговорамКонтрагентовОбщиеСрезПоследних.Регистратор.НоменклатураДоговора        КАК НоменклатураУсловий,
...
Процедура ЗаполнитьДокументыВыполненияУсловий(СтрокаДерева, ТаблицаЗапроса, ИндексСтроки, СписокГруппировок, ТаблицаЗапросаВыполненияУсловий, Знач ТаблицаДатВыполненияУсловийНоменклатуры, ТаблицаЗапросаВыполненияУсловийБезДокумента, ФлагВыводитьСтроки)
	...
	Если ФлагВыводитьСтроки Тогда
		...
		Если ТипЗнч(СтрокаДерева.ЗначениеГруппировки) = Тип("СправочникСсылка.Номенклатура") И НЕ ЗначениеНеЗаполнено(СтрокаДерева.ЗначениеГруппировки) Тогда
...
Процедура ЗаполнитьПериодыДоговораВзаиморасчетов(СтрокаДерева, ТаблицаЗапроса, ИндексСтроки, СписокГруппировок, ТаблицаЗапросаВыполненияУсловий, ТаблицаЗапросаВыполненияУсловийНоменклатуры, ТаблицаЗапросаВыполненияУсловийБезДокумента, ТаблицаЗапросаВыполненияУсловийНоменклатурыБезДокумента)
	...
	Для каждого СтрокаТаблицы Из ТаблицаЗапросаОбщихУсловийДоговора Цикл
		...
		Если ЗначениеНеЗаполнено(СтрокаТаблицы.Периодичность) Тогда
			а = 1;
		...
		Иначе
			Пока 1 = 1 Цикл
				Если ДатаНачалаДоговора >= ДатаКонечнойГраницы Тогда
					Прервать;
				КонецЕсли; 
				ДатаОкончанияДоговора = ОпределитьВторуюГраницуПериода(СтрокаТаблицы.Периодичность, Ложь, ДатаНачалаДоговора);
				ЗаписыватьСтроку = Ложь;
				Если ДатаНачалаДоговора >= ДатаНачальнойГраницы Тогда
					Если ТипЗнч(СтрокаДерева.ЗначениеГруппировки) = Тип("ДокументСсылка.УсловияПоставокПоДоговорамКонтрагентов") Тогда
						Если СтрокаДерева.ЗначениеГруппировки = СтрокаТаблицы.ДокументУсловий Тогда
...
Процедура СформироватьОтчет(Таб) Экспорт
	...
	|	" + СформироватьОтборы(Новый Структура("Контрагент, ДоговорКонтрагента, Номенклатура, ХарактеристикаНоменклатуры", "Продажи.ДоговорКонтрагента.Владелец", "Продажи.ДоговорКонтрагента", "Продажи.Номенклатура", "Продажи.ХарактеристикаНоменклатуры"), ЗапросВыполненияУсловий) + "
	...
	ДеревоУсловий.Колонки.Вставить(1, "СуммаОбязательств"         , ПолучитьОписаниеТиповЧисла(15,2));
	ДеревоУсловий.Колонки.Вставить(2, "СуммаВыполнения"           , ПолучитьОписаниеТиповЧисла(15,2));
	ДеревоУсловий.Колонки.Вставить(3, "КоличествоОбязательств"    , ПолучитьОписаниеТиповЧисла(20,3));
	ДеревоУсловий.Колонки.Вставить(4, "КоличествоВыполнения"      , ПолучитьОписаниеТиповЧисла(20,3));
	...
	СтрокаОтборов = СформироватьСтрокуОтборов(Отбор);
	...
	Секция.Параметры.ПредставлениеСумма = "Сумма, " + СокрЛП(Константы.ВалютаУправленческогоУчета.Получить());
	Таб.Вывести(Секция);
...
Процедура ВывестиСтроки(Таб, Макет, СтрокаПрошлогоУровня, ИндексГруппировки, СписокГруппировок)
	...
	Для каждого СтрокаТекущегоУровня Из СтрокаПрошлогоУровня.Строки Цикл
		...
		Если ЗначениеНеЗаполнено(СтрокаТекущегоУровня.ЗначениеГруппировки) Тогда
		...
		Иначе
			...
			ИначеЕсли ТипЗнч(СтрокаТекущегоУровня.ЗначениеГруппировки) = Тип("СправочникСсылка.Номенклатура") И СтрокаТекущегоУровня.ЗначениеГруппировки.ВестиУчетПоХарактеристикам Тогда
				СтрокаГруппировки = СтрокаГруппировки + " (" + ?(ЗначениеНеЗаполнено(СтрокаТекущегоУровня.ХарактеристикаНоменклатуры), "Характеристика не указана", СтрокаТекущегоУровня.ХарактеристикаНоменклатуры) + ")";
			КонецЕсли; 
...
// Формируем список обяхательных отборов
