...
Функция СформироватьТекстЗапроса(Источник,КопияГруппировкиОтчета,СтруктураДоступныхГруппировок,СтруктураДоступныхПоказателей)
	...
	Если Источник="ТоварыПереданные.ОстаткиИОбороты" Тогда
		...
		ТекстИсточника="
			...
			|РегистрНакопления."+Источник+"(&ДатаНач,&ДатаКон,,,";


		Если ТолькоПросроченные Тогда
			ТекстИсточника=ТекстИсточника+"(ДоговорКонтрагента В (&МассивКомиссионеров))";
		КонецЕсли;

		Если Не ПустаяСтрока(ТекстФильтры) Тогда 
			ТекстИсточника = ТекстИсточника + Символы.ПС + ?(ТолькоПросроченные," И ","")+ТекстФильтры;
		КонецЕсли;
	...
	ИначеЕсли Источник="ТоварыПереданные.Обороты" Тогда
		ТекстИсточника="
			|РегистрСведений.КурсыВалют.СрезПоследних(&ДатаКон, Валюта = &ВалютаУпрУчета) КАК УпрВалюты,
			|РегистрНакопления."+Источник+"(&ДатаНач,&ДатаКон,";

		Если (КопияГруппировкиОтчета.Найти("Регистратор", "ИмяГруппировки") = Неопределено)
			И (таблицаИсточниковСвойства.Найти("ТоварыПереданныеОбороты.Регистратор", "ИмяПоляОбъект") = Неопределено) Тогда

			ТекстИсточника = ТекстИсточника + ",";

		Иначе

			ТекстИсточника = ТекстИсточника + " Регистратор,";
		...
		Если Не ПустаяСтрока(ТекстФильтры) Тогда 
			ТекстИсточника = ТекстИсточника + Символы.ПС + ?(ТолькоПросроченные," И ","")+ТекстФильтры;
		КонецЕсли;
		ТекстИсточника= ТекстИсточника + ") КАК ТоварыПереданныеОбороты
			|ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&ДатаКон) КАК КурсыВалютСрезПоследних
			|ПО ТоварыПереданныеОбороты.ДоговорКонтрагента.ВалютаВзаиморасчетов = КурсыВалютСрезПоследних.Валюта";
	...
	ИначеЕсли Источник="ВзаиморасчетыСКонтрагентами" Тогда
		ТекстИсточника="РегистрНакопления."+Источник+" КАК ВзаиморасчетыСКонтрагентами
		|"+ТекстИсточникиСведений+"
		|ГДЕ
		|ВзаиморасчетыСКонтрагентами.ДоговорКонтрагента В (&МассивКомиссионеров)
		|И ВзаиморасчетыСКонтрагентами.Активность = Истина 
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь,  ЕстьОшибки = Ложь) Экспорт
	...
	СтруктураДоступныхГруппировок.Вставить("Контрагент",                "ТоварыПереданныеОстаткиИОбороты.ДоговорКонтрагента.Владелец");
	...
	СтруктураДоступныхГруппировок.Вставить("Организация",               "ТоварыПереданныеОстаткиИОбороты.ДоговорКонтрагента.Организация");
	СтруктураДоступныхГруппировок.Вставить("ДокументПередачи",          "ТоварыПереданныеОстаткиИОбороты.ДокументПередачи");
	...
	// По регистру "ТоварыПереданные": обороты, для доставания регистраторов...
	СтруктураДоступныхГруппировок.Вставить("Контрагент",                "ТоварыПереданныеОбороты.ДоговорКонтрагента.Владелец");
	СтруктураДоступныхГруппировок.Вставить("ДоговорКонтрагента",       "ТоварыПереданныеОбороты.ДоговорКонтрагента");
	СтруктураДоступныхГруппировок.Вставить("Сделка",                    "ТоварыПереданныеОбороты.Сделка");
	СтруктураДоступныхГруппировок.Вставить("Организация",               "ТоварыПереданныеОбороты.ДоговорКонтрагента.Организация");
	СтруктураДоступныхГруппировок.Вставить("ДокументПередачи",          "ТоварыПереданныеОбороты.ДокументПередачи");
	СтруктураДоступныхГруппировок.Вставить("Регистратор",               "ТоварыПереданныеОбороты.Регистратор");
	СтруктураДоступныхГруппировок.Вставить("ДокументОтчета",            "&НеВыводить");
	СтруктураДоступныхГруппировок.Вставить("ДокументОплаты",            "&НеВыводить");
	СтруктураДоступныхГруппировок.Вставить("Номенклатура",              "ТоварыПереданныеОбороты.Номенклатура");
	СтруктураДоступныхГруппировок.Вставить("ХарактеристикаНоменклатуры","ТоварыПереданныеОбороты.ХарактеристикаНоменклатуры");
	СтруктураДоступныхГруппировок.Вставить("СерияНоменклатуры",         "ТоварыПереданныеОбороты.СерияНоменклатуры");
	...
	СтруктураДоступныхПоказателей.Вставить("КоличествоПриход",          "ТоварыПереданныеОбороты.КоличествоПриход");
	...
	СтруктураДоступныхПоказателей.Вставить("КоличествоЕдиницДляОтчетовПриход",          "ТоварыПереданныеОбороты.КоличествоПриход*ТоварыПереданныеОбороты.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент/ТоварыПереданныеОбороты.Номенклатура.ЕдиницаДляОтчетов.Коэффициент");
	...
	СтруктураДоступныхПоказателей.Вставить("СуммаПриход",               "ТоварыПереданныеОбороты.СуммаВзаиморасчетовПриход
	                                                                    |*(КурсыВалютСрезПоследних.Курс/КурсыВалютСрезПоследних.Кратность)
	                                                                    |/(УпрВалюты.Курс/УпрВалюты.Кратность)");
	СтруктураДоступныхПоказателей.Вставить("КоличествоРасход",          "0");
	СтруктураДоступныхПоказателей.Вставить("КоличествоЕдиницДляОтчетовРасход",          "0");
	...
	ТекстЗапроса=ТекстЗапроса+СформироватьТекстЗапроса("ТоварыПереданные.Обороты",
	КопияГруппировкиОтчета,СтруктураДоступныхГруппировок,СтруктураДоступныхПоказателей);
	ТекстЗапроса=ТекстЗапроса+"
	|ОБЪЕДИНИТЬ ВСЕ
	|";
	...
	СтруктураДоступныхГруппировок.Вставить("Сделка",                    "ВзаиморасчетыСКонтрагентамиОстаткиИОбороты.Сделка");
	...
	СтруктураДоступныхГруппировок.Вставить("ДокументПередачи",          "&НеВыводить");
	СтруктураДоступныхГруппировок.Вставить("Регистратор",               "&НеВыводить");
//	СтруктураДоступныхГруппировок.Вставить("Запись",                    "&НеВыводить");
	СтруктураДоступныхГруппировок.Вставить("ДокументОтчета",            "ВзаиморасчетыСКонтрагентамиОстаткиИОбороты.Сделка");
	СтруктураДоступныхГруппировок.Вставить("ДокументОплаты",            "&НеВыводить");
	...
	//СтруктураДоступныхПоказателей.Вставить("СуммаКонечныйДолг",         "ВЫБОР
	//                                                                    |	КОГДА Сделка В (&МассивПросроченныхСделок) Тогда
	...
	СтруктураДоступныхГруппировок.Вставить("Сделка",                    "ОтчетКомиссионераОПродажахТовары.Ссылка.Сделка");
	...
	СтруктураДоступныхГруппировок.Вставить("ДокументПередачи",          "ОтчетКомиссионераОПродажахТовары.ДокументПередачи");
	СтруктураДоступныхГруппировок.Вставить("Регистратор",               "ОтчетКомиссионераОПродажахТовары.Ссылка");
//	СтруктураДоступныхГруппировок.Вставить("Запись",                    "&НеВыводить");
	СтруктураДоступныхГруппировок.Вставить("ДокументОтчета",            "ОтчетКомиссионераОПродажахТовары.Ссылка");
	...
	СтруктураДоступныхПоказателей.Вставить("СуммаРасход",               ШаблонТекстаЗапросаПересчетСуммыВВалютуУпрУчета("ОтчетКомиссионераОПродажахТовары","Сумма"));
	СтруктураДоступныхПоказателей.Вставить("СуммаНачальныйДолг",        "0");
	СтруктураДоступныхПоказателей.Вставить("СуммаКонечныйДолг",         "0");
	СтруктураДоступныхПоказателей.Вставить("СуммаОплата",               "0");
	СтруктураДоступныхПоказателей.Вставить("СуммаВознаграждение",       ШаблонТекстаЗапросаПересчетСуммыВВалютуУпрУчета("ОтчетКомиссионераОПродажахТовары","СуммаВознаграждения"));
	...
	СтруктураДоступныхГруппировок.Вставить("Регистратор",               "&НеВыводить");
	СтруктураДоступныхГруппировок.Вставить("ДокументОтчета",            "&НеВыводить");
	СтруктураДоступныхГруппировок.Вставить("ДокументОплаты",            "ВзаиморасчетыСКонтрагентами.Регистратор");
	СтруктураДоступныхГруппировок.Вставить("Номенклатура",              "&НеВыводить");
	СтруктураДоступныхГруппировок.Вставить("ХарактеристикаНоменклатуры","&НеВыводить");
	СтруктураДоступныхГруппировок.Вставить("СерияНоменклатуры",         "&НеВыводить");
	...
	СтруктураДоступныхПоказателей.Вставить("СуммаНачальныйДолг",        "0");
	СтруктураДоступныхПоказателей.Вставить("СуммаКонечныйДолг",         "0");
	СтруктураДоступныхПоказателей.Вставить("СуммаОплата",               "ВзаиморасчетыСКонтрагентами.СуммаВзаиморасчетов");
	СтруктураДоступныхПоказателей.Вставить("СуммаВознаграждение",       "0");
	...
	Для Каждого Строка Из РезультатЗапроса Цикл
		...
		Если Договор.ВедениеВзаиморасчетов = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоРасчетнымДокументам
