...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	ТекстЗапроса = "
		...
		|			ТОГДА 100*(-РублевыеЦеныКомпании.РублеваяЦена + 
		|				  ЦеныНоменклатуры.Цена*КурсыВалютСрезПоследних.Курс
		|					/КурсыВалютСрезПоследних.Кратность
		|					/ЦеныНоменклатуры.ЕдиницаИзмерения.Коэффициент)/
		...
		|		/КурсыВалютДляЦенКомпании.Кратность      КАК РублеваяЦена,
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ТолькоЗаголовок = Ложь) Экспорт
	...
	ДатаКурса = ?(ЗначениеНеЗаполнено(ДатаОтчета), ТекущаяДата(), ДатаОтчета);
...
//	возврат;
	...
	// Ширина таблицы:
	ШиринаТаблицы = 1+КоличествоКолонок + СтруктураПараметров.КоличествоКолонокПоказателей;//* КоличествоПоказателей;
