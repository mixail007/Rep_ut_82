...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	|	//СВОЙСТВА
	|}
	|ИЗ РегистрНакопления.ТоварыАдресноеХранение.Остатки(&ДатаКон,{Склад.* КАК Склад,
	|				Номенклатура.* КАК Номенклатура,
	|				АдресХранения.* КАК АдресХранения}) КАК ТаблицаРегистра
	|//СОЕДИНЕНИЯ
	|//СГРУППИРОВАТЬПО
 	|{УПОРЯДОЧИТЬ ПО 
	|	Склад.* КАК Склад,
	...
	|	СУММА(Количество)
	|ПО ОБЩИЕ ,
	|	Склад КАК Склад, Номенклатура  КАК Номенклатура, АдресХранения как АдресХранения"; //+++ 29.08.2014 АдресХранения  в итоги
	...
	"Склад",
	"Номенклатура",
	"Адрес хранения"); //Адрес >> Адрес хранения
	...
	Если ОбщийОтчет.Показатели.Найти("Количество", "Имя") = Неопределено Тогда
		ОбщийОтчет.ЗаполнитьПоказатели("Количество", "Количество ", Истина, "ЧЦ=15; ЧДЦ=3");
	КонецЕсли;
		
	МассивОтбора.Добавить("Номенклатура");
	МассивОтбора.Добавить("Склад");
	МассивОтбора.Добавить("АдресХранения"); //+++ 29.08.2014
...
ОбщийОтчет.ИмяРегистра = "ТоварыАдресноеХранение";
ОбщийОтчет.мНазваниеОтчета = "Остатки товаров адресное хранение";
