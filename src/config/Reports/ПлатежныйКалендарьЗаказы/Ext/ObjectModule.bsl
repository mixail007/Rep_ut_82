...
Процедура СформироватьНастройкиОтчета(МассивОтбора,СтруктураПредставлениеПолей) Экспорт
	...
	|	0
	|ИЗ
	|	РегистрНакопления.РасчетыСКонтрагентами.Остатки(,(НЕ Сделка=Неопределено) И (Сделка ССЫЛКА Документ.ЗаказПокупателя) И (Сделка.ДатаОплаты МЕЖДУ &ДатаНач И &ДатаКон) И НЕ(Сделка.СтруктурнаяЕдиница = НЕОПРЕДЕЛЕНО)) КАК РасчетыСКонтрагентамиОстатки
	...
	|		0
	|	ИЗ
	|		РегистрНакопления.РасчетыСКонтрагентами.Остатки(&ДатаОстаток,(НЕ Сделка=Неопределено) И (Сделка ССЫЛКА Документ.ЗаказПокупателя) И НЕ(Сделка.СтруктурнаяЕдиница = НЕОПРЕДЕЛЕНО)) КАК РасчетыСКонтрагентамиОстатки1
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&ДатаНач) КАК КурсыОплаты
	...
	|	0 
	|ИЗ
	|	РегистрНакопления.РасчетыСКонтрагентами.Остатки(, (НЕ Сделка=Неопределено) И (Сделка ССЫЛКА Документ.ЗаказПоставщику) И (Сделка.ДатаОплаты МЕЖДУ &ДатаНач И &ДатаКон) И НЕ(Сделка.СтруктурнаяЕдиница = НЕОПРЕДЕЛЕНО)) КАК РасчетыСКонтрагентамиОстатки
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&ДатаНач) КАК КурсыОплаты
	|		ПО КурсыОплаты.Валюта = РасчетыСКонтрагентамиОстатки.Сделка.СтруктурнаяЕдиница.ВалютаДенежныхСредств
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&ДатаНач) КАК КурсыРасчетов
	|		ПО КурсыРасчетов.Валюта = РасчетыСКонтрагентамиОстатки.ДоговорКонтрагента.ВалютаВзаиморасчетов
	...
	|					/ (КурсыОплаты.Курс * КурсыРасчетов.Кратность) ИНАЧЕ 0 КОНЕЦ КАК ЧИСЛО(15, 2))
	|ИЗ
	|	РегистрНакопления.РасчетыСКонтрагентами.Остатки(&ДатаОстаток, (НЕ Сделка=Неопределено) И (Сделка ССЫЛКА Документ.ЗаказПоставщику) И НЕ(Сделка.СтруктурнаяЕдиница = НЕОПРЕДЕЛЕНО)) КАК РасчетыСКонтрагентамиОстатки1
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&ДатаНач) КАК КурсыОплаты
