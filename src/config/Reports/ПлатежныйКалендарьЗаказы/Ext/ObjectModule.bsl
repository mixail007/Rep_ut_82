...
Процедура СформироватьНастройкиОтчета(МассивОтбора,СтруктураПредставлениеПолей) Экспорт
	...
	Периодичность=?(ДетализацияПоПериодам.Пустая(),"День",ПолучитьИмяЭлементаПеречисленияПоЗначению(ДетализацияПоПериодам));	
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|	ВЫБРАТЬ
	|	НАЧАЛОПЕРИОДА(Сделка.ДатаОплаты,"+Периодичность+"),
	|	&ПараметрПоступление КАК ВидДвиженияПриходРасход,
	|	&ПараметрПоступлениеЗаказ КАК ВидДвиженияДокПлан,
	|	ВЫБОР КОГДА РасчетыСКонтрагентамиОстатки.Сделка.СтруктурнаяЕдиница ССЫЛКА Справочник.БанковскиеСчета ТОГДА &Безналичные КОГДА РасчетыСКонтрагентамиОстатки.Сделка.СтруктурнаяЕдиница ССЫЛКА Справочник.Кассы ТОГДА &Наличные ИНАЧЕ &ПустаяФормаОплаты КОНЕЦ,
	|	РасчетыСКонтрагентамиОстатки.Сделка.СтруктурнаяЕдиница,
	|	РасчетыСКонтрагентамиОстатки.Сделка.СтруктурнаяЕдиница.ВалютаДенежныхСредств,
	|	ВЫРАЗИТЬ(РасчетыСКонтрагентамиОстатки.Сделка КАК Документ.ЗаказПоставщику),
	|	0 КАК СуммаПолучениеДокумент,
	|	0 КАК СуммаПолучениеПлан,
	|	ВЫРАЗИТЬ(ВЫБОР КОГДА РасчетыСКонтрагентамиОстатки.Сделка.СтруктурнаяЕдиница.ВалютаДенежныхСредств = РасчетыСКонтрагентамиОстатки.ДоговорКонтрагента.ВалютаВзаиморасчетов 
	|				ТОГДА РасчетыСКонтрагентамиОстатки.СуммаВзаиморасчетовОстаток 
	|				КОГДА РасчетыСКонтрагентамиОстатки.Сделка.СтруктурнаяЕдиница.ВалютаДенежныхСредств <> РасчетыСКонтрагентамиОстатки.ДоговорКонтрагента.ВалютаВзаиморасчетов 
	...
	|	РегистрНакопления.ВзаимоРасчетыСКонтрагентами.Остатки(,(НЕ Сделка=Неопределено) И (Сделка ССЫЛКА Документ.ЗаказПокупателя) И (Сделка.ДатаОплаты МЕЖДУ &ДатаНач И &ДатаКон) И НЕ(Сделка.СтруктурнаяЕдиница = НЕОПРЕДЕЛЕНО)) КАК РасчетыСКонтрагентамиОстатки
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&ДатаНач) КАК КурсыОплаты
	|		ПО КурсыОплаты.Валюта = РасчетыСКонтрагентамиОстатки.Сделка.СтруктурнаяЕдиница.ВалютаДенежныхСредств
	...
	|		&ПараметрПоступлениеЗаказ КАК ВидДвиженияДокПлан,
	|		ВЫБОР
	|			Когда Сделка.СтруктурнаяЕдиница Ссылка Справочник.БанковскиеСчета Тогда &Безналичные
	|			Когда Сделка.СтруктурнаяЕдиница Ссылка Справочник.Кассы Тогда &Наличные
	|			Иначе &ПустаяФормаОплаты Конец,
	|		Сделка.СтруктурнаяЕдиница,
	|		Сделка.СтруктурнаяЕдиница.ВалютаДенежныхСредств,
	|		ВЫРАЗИТЬ(Сделка КАК Документ.ЗаказПоставщику),
	|		0,
	|		0,
	|		0,
	|		0,
	...
	|		ВЫРАЗИТЬ 
	|		(ВЫБОР 
	|		КОГДА Сделка.СтруктурнаяЕдиница.ВалютаДенежныхСредств=РасчетыСКонтрагентамиОстатки1.ДоговорКонтрагента.ВалютаВзаиморасчетов ТОГДА СуммаВзаиморасчетовОстаток
	|		КОГДА Сделка.СтруктурнаяЕдиница.ВалютаДенежныхСредств<>РасчетыСКонтрагентамиОстатки1.ДоговорКонтрагента.ВалютаВзаиморасчетов 
	...
	|	ИЗ
	//01.04.2019
	|		РегистрНакопления.ВзаимоРасчетыСКонтрагентами.Остатки(&ДатаОстаток,(НЕ Сделка=Неопределено) И (Сделка ССЫЛКА Документ.ЗаказПокупателя) И НЕ(Сделка.СтруктурнаяЕдиница = НЕОПРЕДЕЛЕНО)) КАК РасчетыСКонтрагентамиОстатки1
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&ДатаНач) КАК КурсыОплаты
	|	ПО  КурсыОплаты.Валюта=Сделка.СтруктурнаяЕдиница.ВалютаДенежныхСредств
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&ДатаНач) КАК КурсыРасчетов
	...
	|	ВЫБОР КОГДА РасчетыСКонтрагентамиОстатки.Сделка.СтруктурнаяЕдиница ССЫЛКА Справочник.БанковскиеСчета ТОГДА &Безналичные 
	|		КОГДА РасчетыСКонтрагентамиОстатки.Сделка.СтруктурнаяЕдиница ССЫЛКА Справочник.Кассы ТОГДА &Наличные ИНАЧЕ &ПустаяФормаОплаты КОНЕЦ,
	|	РасчетыСКонтрагентамиОстатки.Сделка.СтруктурнаяЕдиница,
	|	РасчетыСКонтрагентамиОстатки.Сделка.СтруктурнаяЕдиница.ВалютаДенежныхСредств,
	|	ВЫРАЗИТЬ(РасчетыСКонтрагентамиОстатки.Сделка КАК Документ.ЗаказПоставщику),
	|	0,
	...
	|ИЗ
	//01.04.2019
	|	РегистрНакопления.ВзаимоРасчетыСКонтрагентами.Остатки(, (НЕ Сделка=Неопределено) И (Сделка ССЫЛКА Документ.ЗаказПоставщику) И (Сделка.ДатаОплаты МЕЖДУ &ДатаНач И &ДатаКон) И НЕ(Сделка.СтруктурнаяЕдиница = НЕОПРЕДЕЛЕНО)) КАК РасчетыСКонтрагентамиОстатки
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&ДатаНач) КАК КурсыОплаты
	|		ПО КурсыОплаты.Валюта = РасчетыСКонтрагентамиОстатки.Сделка.СтруктурнаяЕдиница.ВалютаДенежныхСредств
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&ДатаНач) КАК КурсыРасчетов
	|		ПО КурсыРасчетов.Валюта = РасчетыСКонтрагентамиОстатки.ДоговорКонтрагента.ВалютаВзаиморасчетов
	...
	|	&ПараметрРасход КАК ВидДвиженияПриходРасход,
	|	&ПараметрРасходЗаказ КАК ВидДвиженияДокПлан,
	|	ВЫБОР КОГДА РасчетыСКонтрагентамиОстатки1.Сделка.СтруктурнаяЕдиница ССЫЛКА Справочник.БанковскиеСчета ТОГДА &Безналичные 
	|		КОГДА РасчетыСКонтрагентамиОстатки1.Сделка.СтруктурнаяЕдиница ССЫЛКА Справочник.Кассы ТОГДА &Наличные ИНАЧЕ &ПустаяФормаОплаты КОНЕЦ,
	|	РасчетыСКонтрагентамиОстатки1.Сделка.СтруктурнаяЕдиница,
	|	РасчетыСКонтрагентамиОстатки1.Сделка.СтруктурнаяЕдиница.ВалютаДенежныхСредств,
	...
	|	0,
	|	0,
	|	ВЫРАЗИТЬ(ВЫБОР КОГДА РасчетыСКонтрагентамиОстатки1.Сделка.СтруктурнаяЕдиница.ВалютаДенежныхСредств = РасчетыСКонтрагентамиОстатки1.ДоговорКонтрагента.ВалютаВзаиморасчетов 
	|			ТОГДА РасчетыСКонтрагентамиОстатки1.СуммаВзаиморасчетовОстаток*(-1)
	|			КОГДА РасчетыСКонтрагентамиОстатки1.Сделка.СтруктурнаяЕдиница.ВалютаДенежныхСредств <> РасчетыСКонтрагентамиОстатки1.ДоговорКонтрагента.ВалютаВзаиморасчетов 
	|				И КурсыОплаты.Курс <> 0 И КурсыРасчетов.Курс <> 0 
	|				ТОГДА РасчетыСКонтрагентамиОстатки1.СуммаВзаиморасчетовОстаток*(-1) * КурсыРасчетов.Курс * КурсыОплаты.Кратность 
	|					/ (КурсыОплаты.Курс * КурсыРасчетов.Кратность) ИНАЧЕ 0 КОНЕЦ КАК ЧИСЛО(15, 2))
	|ИЗ
	//01.04.2019 ВзаимоРасчетыСКонтрагентами
	|	РегистрНакопления.ВзаимоРасчетыСКонтрагентами.Остатки(&ДатаОстаток, (НЕ Сделка=Неопределено) И (Сделка ССЫЛКА Документ.ЗаказПоставщику) И НЕ(Сделка.СтруктурнаяЕдиница = НЕОПРЕДЕЛЕНО)) КАК РасчетыСКонтрагентамиОстатки1
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&ДатаНач) КАК КурсыОплаты
	|		ПО КурсыОплаты.Валюта = РасчетыСКонтрагентамиОстатки1.Сделка.СтруктурнаяЕдиница.ВалютаДенежныхСредств
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&ДатаНач) КАК КурсыРасчетов
	|		ПО КурсыРасчетов.Валюта = РасчетыСКонтрагентамиОстатки1.ДоговорКонтрагента.ВалютаВзаиморасчетов
	|
	|ГДЕ
...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	Если ЗначениеНеЗаполнено(ДетализацияПоПериодам) Тогда
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок = Ложь, ВысотаЗаголовка = 0, ТолькоЗаголовок = Ложь, РапортРуководителю=Ложь) Экспорт
	...
	ТекстЗапроса="ВЫБРАТЬ "+ТекстГруппировки+"
	|СуммаОстаток
	...
	СтруктураПараметров.Вставить("Периодичность",ПолучитьИмяЭлементаПеречисленияПоЗначению(ДетализацияПоПериодам));
...
Процедура ВывестиСтрокуШапки(ВыборкаГоризонтальная, СтруктураПараметров, Номер)
	...
	Если Найти(ИмяГруппировки,"Период")>0 Тогда
		...
		ЗначениеТекущейГруппировки=ПолучитьПериодСтрокой(ВыборкаГоризонтальная[ИмяГруппировки],?(ИмяГруппировки="Период",Периодичность,СтрЗаменить(ИмяГруппировки,"Период","")));
	Иначе
...
Процедура ВывестиСтроку(Выборка, СтруктураПараметров, Номер, МассивРасшифровки, ТипЗначения)
	...
	Если ТипЗаписиВыборки = ТипЗаписиЗапроса.ИтогПоГруппировке Тогда
		...
		Если Найти(ИмяГруппировки,"Период")>0 Тогда
			ЗначениеТекущейГруппировки=ПолучитьПериодСтрокой(Выборка[ИмяГруппировки],?(ИмяГруппировки="Период",Периодичность,СтрЗаменить(ИмяГруппировки,"Период","")));
	...
	Если ОсновнаяГруппировка Тогда
		...
		КопияРасшифровкиСтрока1=СкопироватьУниверсальнуюКоллекцию(ЗначениеРасшифровкиСтрока);
		...
		КопияРасшифровкиСтрока2=СкопироватьУниверсальнуюКоллекцию(ЗначениеРасшифровкиСтрока);
		...
		КопияРасшифровкиСтрока3=СкопироватьУниверсальнуюКоллекцию(ЗначениеРасшифровкиСтрока);
		...
		КопияРасшифровкиСтрока4=СкопироватьУниверсальнуюКоллекцию(ЗначениеРасшифровкиСтрока);
		...
		КопияРасшифровкиСтрока5=СкопироватьУниверсальнуюКоллекцию(ЗначениеРасшифровкиСтрока);
		...
		КопияРасшифровкиСтрока6=СкопироватьУниверсальнуюКоллекцию(ЗначениеРасшифровкиСтрока);
