...
Процедура ЗаполнитьТаблицуКлассовОбъектов(Выборка, ТаблицаКлассовОбъектов)
	...
	ТаблицаКлассовОбъектов.Сортировать(ПараметрАнализа + " УБЫВ");
...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	|	СУММА(ПродажиТаблица.СтоимостьОборот) КАК СуммаВыручки,
	...
	|		ПродажиТаблица.СтоимостьОборот
	|	ИНАЧЕ
	|		ПродажиТаблица.СтоимостьОборот - (ПродажиСебестоимость.СтоимостьОборот / ПродажиСебестоимость.КоличествоОборот) * ПродажиТаблица.КоличествоОборот
	|	КОНЕЦ)                          КАК СуммаВаловойПрибыли,
	|	СУММА(ПродажиТаблица.КоличествоОборот) КАК КоличествоПроданныхТоваров
	|
	|	{ВЫБРАТЬ
	|		ПродажиСебестоимость.Номенклатура.*,
	|		ПродажиТаблица.ДокументПродажи.*,
	...
	|	ИЗ
	|
	|		РегистрНакопления.Продажи.Обороты(&ДатаНачала, &ДатаОкончания, , (ДоговорКонтрагента <> &ПустойДоговор И Номенклатура <> &ПустаяНоменклатура)) КАК Продажи
	|
	|	) КАК Продажи
	|//СОЕДИНЕНИЯ
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрНакопления.Продажи.Обороты(&ДатаНачала, &ДатаОкончания, , (ДоговорКонтрагента <> &ПустойДоговор И Номенклатура <> &ПустаяНоменклатура)) КАК ПродажиТаблица
	|
	|	ПО
	|		ПродажиТаблица.Номенклатура = Продажи.Номенклатура
	|		И
	|		ПродажиТаблица.ДоговорКонтрагента.Владелец = Продажи.Контрагент
	|		И
	|		ПродажиТаблица.ДоговорКонтрагента.Владелец.ОсновнойМенеджерПокупателя = Продажи.ОсновнойМенеджерПокупателя
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|
	|	(
	...
	|
	|	ИЗ
	|		РегистрНакопления.ПродажиСебестоимость.Обороты(&ДатаНачала, &ДатаОкончания, , Номенклатура <> &ПустаяНоменклатура) КАК ПродажиСебестоимость
	|
	|	СГРУППИРОВАТЬ ПО
	|		ПродажиСебестоимость.Номенклатура,
	...
	|	Продажи.Контрагент.* КАК Контрагент,
	|	Продажи.ОсновнойМенеджерПокупателя.* КАК ОсновнойМенеджерПокупателя,
	|	ПродажиТаблица.ДокументПродажи.* КАК ДокументПродажи
	|	//СВОЙСТВА
	|	}
	|
	|";
...
Процедура СформироватьОтчет(Таб) Экспорт
	...
	Если СуммаПроцентов <> 100 Тогда
		Предупреждение("Сумма процентов критериев распределения по АВС-классам должна быть равна 100%, а у вас " + Строка(СуммаПроцентов) + "%.");
		Возврат;
	КонецЕсли; 
