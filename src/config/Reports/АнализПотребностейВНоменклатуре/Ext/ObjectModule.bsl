...
Функция СформироватьСтрокуПериода()
	...
	Если Отбор.ДатаПотребности.Использование Тогда
		...
		ИначеЕсли Отбор.ДатаПотребности.ВидСравнения = ВидСравнения.Интервал
			...
			Если Отбор.ДатаПотребности.ЗначениеС > Отбор.ДатаПотребности.ЗначениеПо И НЕ ЗначениеНеЗаполнено(Отбор.ДатаПотребности.ЗначениеПо) Тогда
...
Процедура СформироватьОтчет(Таб) Экспорт
	...
	Если Группировки.Найти("", "Имя") <> Неопределено ИЛИ Группировки.Найти("", "Представление") <> Неопределено Тогда
		СообщитьОбОшибке("В настройках отчета не должны присутствовать пустые группировки.",, "Отчет не сформирован!");
		Возврат;
	КонецЕсли; 
	...
	Если Отбор.ДатаПотребности.Использование Тогда
		Запрос.Текст = Запрос.Текст + ПолучитьСтрокуОтбора(Отбор.ДатаПотребности.ВидСравнения, "&ДатаПотребности", "КалендарныеПотребностиВНоменклатуреСрезПоследних.ДатаПотребности", "&ДатаПотребностиНач", "&ДатаПотребностиКон", Отбор.ДатаПотребности.Значение, Отбор.ДатаПотребности.ЗначениеС, Отбор.ДатаПотребности.ЗначениеПо);
	...
	Если Отбор.Номенклатура.Использование Тогда
		Запрос.Текст = Запрос.Текст + " И " + ПолучитьСтрокуОтбора(Отбор.Номенклатура.ВидСравнения, "&ОтборНоменклатура", "КалендарныеПотребностиВНоменклатуреСрезПоследних.Номенклатура", "&ОтборНоменклатураС", "&ОтборНоменклатураПо");
	...
	Если ТаблицаПотребности.Количество() > 0 Тогда
		...
		Пока ИндексИсходнойТаблицы <= ТаблицаПотребности.Количество() - 1 Цикл
			...
			|ИЗ
			|	РегистрНакопления.ЗаказыПоставщикам.Остатки(&ДатаОтчета, ЕСТЬNULL(Подразделение.ВЭД, ЛОЖЬ) = ЛОЖЬ) КАК ЗаказыПоставщикамОстатки
			|
			|ЛЕВОЕ СОЕДИНЕНИЕ
			...
			ТемповаяСтрока = "";
			Если Отбор.Номенклатура.Использование Тогда
				ТемповаяСтрока = ТемповаяСтрока + ПолучитьСтрокуОтбора(Отбор.Номенклатура.ВидСравнения, "&ОтборНоменклатура", "ЗаказыПоставщикамОстатки.Номенклатура", "&ОтборНоменклатураС", "&ОтборНоменклатураПо");
				Запрос.УстановитьПараметр("ОтборНоменклатура"  , Отбор.Номенклатура.Значение);
				Запрос.УстановитьПараметр("ОтборНоменклатураС" , Отбор.Номенклатура.ЗначениеС);
				Запрос.УстановитьПараметр("ОтборНоменклатураПо", Отбор.Номенклатура.ЗначениеПо);
			КонецЕсли; 
			
			Если Отбор.ДатаПотребности.Использование  И НЕ (Отбор.ДатаПотребности.ВидСравнения = ВидСравнения.Равно И ЗначениеНеЗаполнено(Отбор.ДатаПотребности.Значение)) Тогда
				Если НЕ ПустаяСтрока(ТемповаяСтрока) Тогда
					ТемповаяСтрока = ТемповаяСтрока + " И ";
				КонецЕсли; 
				ТемповаяСтрока = ТемповаяСтрока + ПолучитьСтрокуОтбора(Отбор.ДатаПотребности.ВидСравнения, "&ДатаПотребности", "ЗаказыПоставщикамОстатки.ЗаказПоставщику.ДатаПоступления", "&ДатаПотребностиНач", "&ДатаПотребностиКон", Отбор.ДатаПотребности.Значение, Отбор.ДатаПотребности.ЗначениеС, Отбор.ДатаПотребности.ЗначениеПо);
				Запрос.УстановитьПараметр("ДатаПотребности"   , Отбор.ДатаПотребности.Значение);
				Запрос.УстановитьПараметр("ДатаПотребностиНач", НачалоДня(Отбор.ДатаПотребности.ЗначениеС));
				Запрос.УстановитьПараметр("ДатаПотребностиКон", КонецДня(Отбор.ДатаПотребности.ЗначениеПо));
			КонецЕсли; 
	...
	Иначе
		...
		ТемповаяСтрока = "";
		Если Отбор.Номенклатура.Использование Тогда
			ТемповаяСтрока = ПолучитьСтрокуОтбора(Отбор.Номенклатура.ВидСравнения, "&ОтборНоменклатура", "ЗаказыПоставщикамОстатки.Номенклатура", "&ОтборНоменклатураС", "&ОтборНоменклатураПо");
			Запрос.УстановитьПараметр("ОтборНоменклатура"  , Отбор.Номенклатура.Значение);
			Запрос.УстановитьПараметр("ОтборНоменклатураС" , Отбор.Номенклатура.ЗначениеС);
			Запрос.УстановитьПараметр("ОтборНоменклатураПо", Отбор.Номенклатура.ЗначениеПо);
		КонецЕсли; 
	...
	СтруктураТаблиц = ПолучитьСтруктуруРаспределенныхТаблиц(ТаблицаПотребности, ТаблицаНоменклатурыЗаказов);
	...
	Дерево = ПолучитьДеревоРаспределенияПотребностей(ТаблицаПотребности.Скопировать(), ТаблицаЗаказовПоставщикам, СтруктураЗначенийГруппировок, КопияГруппировок, Истина);
