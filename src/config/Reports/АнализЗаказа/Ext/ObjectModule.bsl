...
Процедура СформироватьОтчет(ДокументРезультат) Экспорт
	...
	ИначеЕсли Заказ.Контрагент = Справочники.Контрагенты.НайтиПоКоду("94072") и ЗначениеЗаполнено(Заказ.Основание) тогда
		Запрос.Текст="ВЫБРАТЬ
		             ...
		             |	ЕСТЬNULL(втЗаказПоставщикуФакт.Количество, 0) КАК КоличествоФакт,
		             |	ЕСТЬNULL(ЗакупкиОбороты.КоличествоОборот, 0) КАК Отгружено,
		             |	втЗаказПлан.Количество - ЕСТЬNULL(втЗаказПоставщикуФакт.Количество, 0) КАК ОсталосьОтгрузить,
		             |	0 КАК Резерв,
		             |	0 КАК Отменено,
		             ...
		             |		ЛЕВОЕ СОЕДИНЕНИЕ втЗаказПоставщикуФакт КАК втЗаказПоставщикуФакт
		             ...
		             |		ПО втЗаказПлан.Номенклатура = втЗаказПоставщикуФакт.Номенклатура
		             |			И втЗаказПлан.ЗаказПоставщикуПлан = втЗаказПоставщикуФакт.ЗаказПоставщикуПлан
		             |			И втЗаказПлан.ХарактеристикаНоменклатуры = втЗаказПоставщикуФакт.ХарактеристикаНоменклатуры
		             |ИТОГИ
		             |	СУММА(Запланировано),
