...
Процедура СформироватьОтчет(ДокументРезультат) Экспорт
	...
	ИначеЕсли Заказ.Контрагент = Справочники.Контрагенты.НайтиПоКоду("94072") и ЗначениеЗаполнено(Заказ.Основание) тогда
		Запрос.Текст="ВЫБРАТЬ
		             ...
		             |				ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.Закупки.Обороты(
		             |						,
		             |						,
		             |						Авто,
		             |						ЗаказПоставщику В
		             |								(ВЫБРАТЬ
		             |									втЗаказПоставщикуФакт.ЗаказПоставщикуФакт
		             |								ИЗ
		             |									втЗаказПоставщикуФакт)
					 //Алия попросила только текущий заказ, условие на все заказы из сезонного пока оставляю, мало ли понадобится
