...
Процедура СформироватьОтчет(Таб) Экспорт
	...
	Иначе
		ПредставлениеПараметра = "Параметр распределения: " + ПараметрАВСРаспределения + ", в валюте упр.учета (" + СокрЛП(Константы.ВалютаУправленческогоУчета.Получить().Наименование) + ")";
	КонецЕсли; 
...
//  ТаблицаЗапросаПрошлыхКлассов - ТаблицаЗначений,  прошлые занчения АВС-классов контрагентов
// 
// Возвращаемое значение
//  НЕТ
