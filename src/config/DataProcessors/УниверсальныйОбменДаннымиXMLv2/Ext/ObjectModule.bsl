...
Процедура ПрочитатьСвойстваПоискаИзФайла(СвойстваПоиска, СвойстваПоискаНеЗамещать, ИнформацияОТипах, ПоискПоДатеНаРавенство = Ложь)
...
Процедура ПрочитатьИнформациюОСвойствахПоиска(ТипОбъекта, СвойстваПоиска, СвойстваПоискаНеЗамещать, ПоискПоДатеНаРавенство = Ложь)
...
Функция ВыполнитьВыгрузкуИзмененныхДанныхДляУзлаОбмена(УзелОбмена, МассивПравилКонвертации, СтруктураДляУдаленияРегистрацииИзменений)
	...
	//***очевидно, ошибка разрабочтиков (исправлено в 10.3)
	//Возврат Отказ;
	Возврат НЕ Отказ;
	
КонецФункции

Функция ОбработатьВыгрузкуДляПлановОбмена(СоответсвиеУзловИПравилВыгрузки, СтруктураДляУдаленияРегистрацииИзменений)
...
Процедура ВыполнитьДействияПослеЗавершенияЧтенияДанных() Экспорт
	...
	#Если Клиент Тогда		
		Если ЭтоИнтерактивныйРежим Тогда			
			...
			//Предупреждение("Загрузка данных завершена!");//+++ блокировок чтобы не было 27.07.2016
			Сообщить(строка(ТекущаяДата())+" Загрузка данных завершена!", СтатусСообщения.Информация);
 		КонецЕсли;
	#КонецЕсли
...
Процедура ВыполнитьВыгрузку() Экспорт
	...
	Попытка
		...
		УдачноВыгруженоПоПланамОбмена = ОбработатьВыгрузкуДляПлановОбмена(СоответсвиеУзловИПравилВыгрузки, СтруктураДляУдаленияРегистрацииИзменений);
	...
	#Если Клиент Тогда
		Если ЭтоИнтерактивныйРежим Тогда
			ОткрытьДанныеПротоколовОбменаПриНеобходимости();
			//Предупреждение("Выгрузка данных завершена!");		//+++ 27.07.2016
			Сообщить(строка(ТекущаяДата())+" Загрузка данных завершена!", СтатусСообщения.Информация);
		КонецЕсли;	
	#КонецЕсли
...
Функция ВыполнитьПодключениеКИБПриемнику() Экспорт
	...
	Если ОбъектПодключения.Метаданные.Обработки.Найти("УниверсальныйОбменДаннымиXMLv2") = Неопределено Тогда
		НаименованиеОбработки="УниверсальныйОбменДаннымиXML";
	КонецЕсли;	
	
	Попытка
		//РезультатПодключения = ОбъектПодключения.Обработки.УниверсальныйОбменДаннымиXMLv2.Создать();
		РезультатПодключения = ОбъектПодключения.Обработки[НаименованиеОбработки].Создать();
	Исключение
		#Если Клиент тогда
