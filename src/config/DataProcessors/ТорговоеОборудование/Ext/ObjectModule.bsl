...
Функция ПолучитьВладельцаПоШтрихкоду(Штрихкод, ТаблицаВладельцев, ВидВладельца) Экспорт
	...
	Если длинаШК<>7 и ВидВладельца="Номенклатура" тогда    //+++ 10.05.2018 есть "короткие" ШК = 7
		Штрихкод = УдалитьПрефиксИзШтрихКода(ШтрихКод);
	КонецЕсли;

	//+++ 13.03.2012
	попытка
		Если длинаШК>0 и длинаШК<13 и длинаШК<>7 тогда //+++ 10.05.2018 есть "короткие" ШК = 7
