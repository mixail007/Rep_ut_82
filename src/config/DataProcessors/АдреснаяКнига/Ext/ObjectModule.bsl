...
Процедура ЗаполнитьСписокГруппРассылки(ГруппыРассылкиСписок) Экспорт
	...
	Иначе
		СписокУчетныхЗаписей = ПолучитьДоступныеУчетныеЗаписи(глТекущийПользователь).Отправка.Скопировать();
	КонецЕсли;
...
Процедура Объект_ПриОткрытии(ЭлементыФормы) Экспорт
	...
	ЭлементыФормы.КонтактныеЛицаКонтрагентовСписок.Значение.Отбор.ВидКонтактногоЛица.Использование = Истина;
	ЭлементыФормы.КонтактныеЛицаКонтрагентовСписок.Значение.Отбор.ВидКонтактногоЛица.Значение      = Перечисления.ВидыКонтактныхЛиц.КонтактноеЛицоКонтрагента;
	ЭлементыФормы.КонтактныеЛицаКонтрагентовСписок.Значение.Отбор.ОбъектВладелец.Использование     = Истина;
	ЭлементыФормы.КонтактныеЛицаКонтрагентовСписок.Значение.Отбор.ОбъектВладелец.Значение          = Справочники.Контрагенты.ПустаяСсылка();
	
	ЭлементыФормы.ЛичныеКонтактыСписок.Значение.Отбор.ВидКонтактногоЛица.Использование = Истина;
	ЭлементыФормы.ЛичныеКонтактыСписок.Значение.Отбор.ВидКонтактногоЛица.Значение      = Перечисления.ВидыКонтактныхЛиц.ЛичныйКонтакт;
	
	ЭлементыФормы.ПрочиеКонтактныеЛицаСписок.Значение.Отбор.ВидКонтактногоЛица.Использование = Истина;
	ЭлементыФормы.ПрочиеКонтактныеЛицаСписок.Значение.Отбор.ВидКонтактногоЛица.Значение      = Перечисления.ВидыКонтактныхЛиц.ПрочееКонтактноеЛицо;
	...
	Если ЗначениеНеЗаполнено(ВидОбщегоОбъекта) Тогда
...
Процедура ДобавитьЭлементы(ЭлементыФормы, СписокДанныхТекущейСтроки, ЭлементФормы, ЭлементУправленияАдрес, СтандартнаяОбработка = Неопределено, ИдентификаторСправочника = Неопределено, Множественный = Ложь) Экспорт
	...
	Если НЕ Множественный Тогда
		...
		Если ВыбранныеСтроки.Количество() = 1 И НЕ ЗначениеНеЗаполнено(ВыбранныеСтроки[0]) Тогда
			...
			Иначе
				Если ПодборОбъектов Тогда
					СписокЭлементов.Добавить(ВыбранныеСтроки[0]);
				Иначе
					Если Адрес = "E-mail не задан" ИЛИ ЗначениеНеЗаполнено(Адрес) Тогда
						Возврат;
					Иначе
		...
		ИначеЕсли ВыбранныеСтроки.Количество() > 0 Тогда
			Для каждого СтрокаТаблицы Из ВыбранныеСтроки Цикл
				Если ЗначениеНеЗаполнено(СтрокаТаблицы) Тогда
					Продолжить;
				КонецЕсли; 
	...
	Иначе
		...
		Если ИдентификаторСправочника = "КонтактныеЛицаКонтрагентов" Тогда
			...
			СтруктураПараметровЗапроса.Вставить("ВидКонтактногоЛица", Перечисления.ВидыКонтактныхЛиц.КонтактноеЛицоКонтрагента);
			
			Если ЭлементФормы.ТекущаяСтрока = Неопределено Тогда
				Если ЭлементыФормы.КонтрагентыСписок.ТекущаяСтрока <> Неопределено И НЕ ЗначениеНеЗаполнено(ЭлементыФормы.КонтрагентыСписок.ТекущаяСтрока.Ссылка) Тогда
			...
			Иначе
				...
				|	КонтактныеЛица.ОбъектВладелец = &ВыбКонтрагент
				|";
				СтруктураПараметровЗапроса.Вставить("ВыбКонтрагент", ЭлементФормы.ТекущаяСтрока.Ссылка.ОбъектВладелец);
			КонецЕсли; 
			
		ИначеЕсли ИдентификаторСправочника = "ЛичныеКонтакты" ИЛИ ИдентификаторСправочника = "ПрочиеКонтактныеЛица" Тогда
			...
			Если ИдентификаторСправочника = "ЛичныеКонтакты" Тогда
				СтруктураПараметровЗапроса.Вставить("ВидКонтактногоЛица", Перечисления.ВидыКонтактныхЛиц.ЛичныйКонтакт);
			Иначе
				СтруктураПараметровЗапроса.Вставить("ВидКонтактногоЛица", Перечисления.ВидыКонтактныхЛиц.ПрочееКонтактноеЛицо);
			КонецЕсли; 
	...
	Если ПодборОбъектов Тогда
		...
		Если ТипЗнч(СписокЭлементов) = Тип("СписокЗначений") Тогда
			...
			Если ИдентификаторСправочника = "ЛичныеКонтакты" ИЛИ ИдентификаторСправочника = "КонтактныеЛицаКонтаргентов" ИЛИ ИдентификаторСправочника = "ПрочиеКонтактныеЛица" Тогда
				УсловиеПоКонтактнымЛицам = "
				|	И
				|	ТаблицаСправочника.ВидКонтактногоЛица = &ВыбВидКонтактногоЛица
				|";
				ИмяТаблицыСправочника = "КонтактныеЛица";
				Если ИдентификаторСправочника = "ЛичныеКонтакты" Тогда
					Запрос.УстановитьПараметр("ВыбВидКонтактногоЛица", Перечисления.ВидыКонтактныхЛиц.ЛичныйКонтакт);
				ИначеЕсли ИдентификаторСправочника = "КонтактныеЛицаКонтаргентов" Тогда
					Запрос.УстановитьПараметр("ВыбВидКонтактногоЛица", Перечисления.ВидыКонтактныхЛиц.КонтактноеЛицоКонтрагента);
				ИначеЕсли ИдентификаторСправочника = "ПрочиеКонтактныеЛица" Тогда
					Запрос.УстановитьПараметр("ВыбВидКонтактногоЛица", Перечисления.ВидыКонтактныхЛиц.ПрочееКонтактноеЛицо);
				КонецЕсли;
			Иначе
	...
	Иначе
		...
		Если НЕ ЗначениеНеЗаполнено(ВыбЭлемент) Тогда
		...
		Иначе
			...
			Если ТипЗнч(СписокЭлементов) = Тип("СписокЗначений") Тогда
				...
				Если ИдентификаторСправочника = "ЛичныеКонтакты" ИЛИ ИдентификаторСправочника = "КонтактныеЛицаКонтаргентов" ИЛИ ИдентификаторСправочника = "ПрочиеКонтактныеЛица" Тогда
					УсловиеПоКонтактнымЛицам = "
					|	И
					|	КонтактнаяИнформация.Объект.ВидКонтактногоЛица = &ВыбВидКонтактногоЛица
					|";
					ИмяТаблицыСправочника = "КонтактныеЛица";
					Если ИдентификаторСправочника = "ЛичныеКонтакты" Тогда
						Запрос.УстановитьПараметр("ВыбВидКонтактногоЛица", Перечисления.ВидыКонтактныхЛиц.ЛичныйКонтакт);
			...
			Пока Выборка.Следующий() Цикл
				...
				Пока ВыборкаДетальныхЗаписей.Следующий() Цикл
					Если ЗначениеНеЗаполнено(ВыборкаДетальныхЗаписей.АдресЭлектроннойПочты) Тогда
						Продолжить;
					КонецЕсли; 
	...
	Иначе
		...
		Если Множественный Тогда
			...
			Для каждого СтрокаТаблицы Из РезультатыПоиска Цикл
				...
				СтрокаТЧ = ТекущаяТЧ.Добавить();
				СтрокаТЧ.Объект = СтрокаТаблицы.Объект;
				Если НЕ ЗначениеНеЗаполнено(СтрокаТаблицы.ЗначениеПоУмолчанию) Тогда
		...
		Иначе
			...
			Иначе
				...
				Для каждого СтрокаТаблицы Из ЭлементыФормы.РезультатыПоиска.ВыделенныеСтроки Цикл
					...
					Если ЗначениеНеЗаполнено(СтрокаТаблицы.ЗначениеПоУмолчанию) И СтрокаТаблицы.ДоступныеАдреса.Количество() = 0 Тогда
						Продолжить;
					КонецЕсли; 
...
Процедура Объект_РезультатыПоискаПриАктивизацииСтроки(Элемент, ЭлементыФормы) Экспорт
	...
	Иначе
		...
		Иначе
			ЭлементФормы.ТолькоПросмотр = Ложь;
			Если ЗначениеНеЗаполнено(Элемент.ТекущиеДанные.ЗначениеПоУмолчанию) Тогда
				ЭлементФормы.Значение = ЭлементФормы.СписокВыбора[0].Значение;
			Иначе
...
Процедура Объект_ГруппыРассылкиСписокПередНачаломИзменения(Элемент, Отказ, ЭтаФорма) Экспорт
	...
	Если Элемент.ТекущиеДанные <> Неопределено И НЕ ЗначениеНеЗаполнено(Элемент.ТекущиеДанные.ГруппаРассылки) Тогда
...
Процедура Объект_КонтрагентыСписокПриАктивизацииСтроки(Элемент, КонтактныеЛицаКонтрагентовСписок) Экспорт
	...
	Если Элемент.ТекущиеДанные <> Неопределено И НЕ Элемент.ТекущиеДанные.ЭтоГруппа Тогда
		КонтактныеЛицаКонтрагентовСписок.Отбор.ОбъектВладелец.Значение = Элемент.ТекущиеДанные.Ссылка;
	Иначе
		КонтактныеЛицаКонтрагентовСписок.Отбор.ОбъектВладелец.Значение = Справочники.Контрагенты.ПустаяСсылка();
	КонецЕсли; 
...
Процедура Объект_СоставГруппыРассылкиПередНачаломИзменения(Элемент, Отказ) Экспорт
	...
	Если НЕ ЗначениеНеЗаполнено(Элемент.ТекущиеДанные.ОбъектРассылки) Тогда
