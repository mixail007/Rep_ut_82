...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ВРег(ИсточникВыбора.ИмяФормы) = ВРег("Справочник.КлючевыеОперации.Форма.ФормаВыбора") Тогда
		...
		ПредопределенныйЭлемент = ПредопределенноеЗначение("Справочник.КлючевыеОперации.ОбщаяПроизводительностьСистемы");
...
Процедура ПроизводительностьВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Лев(Поле.Имя, 18) <> "Производительность" Или
		СтрокаТЧ.КлючеваяОперация = ПредопределенноеЗначение("Справочник.КлючевыеОперации.ОбщаяПроизводительностьСистемы")
		Тогда
			Возврат;
	КонецЕсли;
	СтандартнаяОбработка = Ложь;
	...
	ОткрытьФорму("Обработка.ОценкаПроизводительности.Форма.ИсторияВыполнения", ПараметрыОткрытия, ЭтаФорма);
...
Процедура ПереместитьВверх(Команда)
	...
	Если Врем.Количество() <= 1 ИЛИ 
		ТекущийИндекс = 0 ИЛИ 
		ТД.КлючеваяОперация = ПредопределенноеЗначение("Справочник.КлючевыеОперации.ОбщаяПроизводительностьСистемы")
		Тогда
			Возврат;
	КонецЕсли;
...
Процедура ПереместитьВниз(Команда)
	...
	Если Врем.Количество() <= 1 Или
		...
		Врем[ТекущийИндекс + 1].КлючеваяОперация = ПредопределенноеЗначение("Справочник.КлючевыеОперации.ОбщаяПроизводительностьСистемы") Или
		ТД.КлючеваяОперация = ПредопределенноеЗначение("Справочник.КлючевыеОперации.ОбщаяПроизводительностьСистемы")
		Тогда
			Возврат;
	КонецЕсли;
...
Процедура Настройка(Команда)
	...
	ОткрытьФорму("Обработка.ОценкаПроизводительности.Форма.АвтоматическийЭкспортЗамеровПроизводительности", , ЭтаФорма);
...
Процедура УстановитьОтбор(Команда)
	...
	ОткрытьФорму("Обработка.ОценкаПроизводительности.Форма.ФормаОтбора", , ЭтаФорма);
...
Процедура ОбновитьПоказатели(ЗначенияОтбора = Неопределено)
	...
	Сообщить("Период установлен не верно.");
		Возврат;
	КонецЕсли;
...
Процедура УстановитьУсловноеОформлениеТЧ(ПоляОтбора, ПоляОформления, УсловноеОформление);
	...
	ЭлементОтбора.ПравоеЗначение = Справочники.КлючевыеОперации.ОбщаяПроизводительностьСистемы;
	// Оформляемое поле
...
Функция ПодготовитьПараметрыЭкспорта()
	...
	ОбщаяПроизводительность = ПредопределенноеЗначение("Справочник.КлючевыеОперации.ОбщаяПроизводительностьСистемы");
...
Процедура ОткрытьФормуВыбора()
	...
	ОткрытьФорму("Справочник.КлючевыеОперации.ФормаВыбора", ПараметрыФормы, ЭтаФорма);
...
Функция НастройкаВыполнена()
	...
	Для Каждого СтрокаТЧ Из Объект.Производительность Цикл
		...
		Если СтрокаТЧ.ЦелевоеВремя = 0 
			И СтрокаТЧ.КлючеваяОперация <> Справочники.КлючевыеОперации.ОбщаяПроизводительностьСистемы 
		Тогда
...
Функция ЗагрузитьКлючевыеОперации()
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ОбщаяПроизводительностьСистемы", Справочники.КлючевыеОперации.ОбщаяПроизводительностьСистемы);
