...
Процедура ЗаполнитьСделки() Экспорт
	...
	|ИЗ
	//+++ 01.04.2019
	|	РегистрНакопления.ВзаимоРасчетыСКонтрагентами.Остатки(&ТекущаяДата) КАК КонтрагентыВзаиморасчетыКомпании
	|
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	(
	|	ВЫБРАТЬ
	...
	|	ИЗ
	//+++ 01.04.2019
	|		РегистрНакопления.ВзаимоРасчетыСКонтрагентами КАК РасчетыСКонтрагентамиВсяТаблица
	|	СГРУППИРОВАТЬ ПО
	...
	|		ИЛИ
	|		ЗаказыПоставщикамВсяТаблица.Регистратор ССЫЛКА Документ.КорректировкаЗаказаПокупателя
	|		ИЛИ
...
МассивЗначенийОтбора.Добавить(глТекущийПользователь);
