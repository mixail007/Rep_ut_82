		...
		Для каждого СтрокаДерева Из ДеревоАнкет.Строки Цикл  // загружаем из каталога
			...
			Для индФлага = 1 по 2 Цикл
				Если индФлага = 1 и СтрокаДерева.ЗагружатьВСправочник тогда
					...
					Если ЗначениеНеЗаполнено(АнкетаИзФайла) тогда
						Продолжить;
					КонецЕсли;
					Если ЗначениеНеЗаполнено(ОбъектДляЗагрузки) тогда
	...
	Если ЕстьФизическиеЛица И Анкета.ВидСправочникаДляЗагрузки = Перечисления.ВидыОбъектовЗагружаемыхИзОпроса.ФизическиеЛица тогда
		Пока ИсточникXMLФайл.Прочитать() Цикл
			...
			Если НЕ ЗначениеНеЗаполнено(Фамилия) и 
				...
				НЕ ЗначениеНеЗаполнено(ДатаРождения) тогда
			...
			Если ИсточникXMLФайл.Имя = "Вопрос" и ИсточникXMLФайл.ТипУзла = ТипУзлаXML.НачалоЭлемента тогда
				...
				Пока НЕ(ИсточникXMLФайл.ТипУзла = ТипУзлаXML.КонецЭлемента и ИсточникXMLФайл.Имя = "Вопрос") Цикл
					...
					Если ИсточникXMLФайл.Имя = "Значение" тогда
						ИсточникXMLФайл.Прочитать();
						Значение	= ИсточникXMLФайл.Значение;
						Если Ид = ИдНаименованиеКонтрагента И ЗначениеНеЗаполнено(НаименованиеКонтрагента) тогда
							НаименованиеКонтрагента = Значение;
						КонецЕсли;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
			Если НЕ ЗначениеНеЗаполнено(НаименованиеКонтрагента) тогда
...
Функция НайтиОбъектДляЗагрузки(ИсточникXMLФайл) Экспорт
	...
	Если НЕ ЗначениеНеЗаполнено(IDЗагружаемогоОбъекта) тогда
	...
	ИначеЕсли ХМЛ.Имя = "ВариантОтвета" Тогда
		...
		Пока НЕ ПрочиталиВесьВопрос Цикл
			...
			Если ХМЛ.ТипУзла = ТипУзлаXML.НачалоЭлемента тогда
				...
				КонецЕсли;
				Если ТипЗнч(ПотокВывода) = Тип("ТаблицаЗначений") Тогда
					Если ЗначениеНеЗаполнено(ТабличныйВопрос) тогда
...
Процедура ПрочитатьВопросы(ХМЛ, ПотокВывода, ВОбъект = Ложь, ДатаЗаполненияАнкеты = Неопределено, Анкета = Неопределено)
	...
	Если НЕ ЗначениеНеЗаполнено(Анкета) тогда
...
Процедура ЗаписатьПредопределенныйТабличныйВопрос(ТабличныйВопрос, ПотокВывода, ОтветыТаблицы) Экспорт;
	...
	Для каждого ОтветИзТаблицы из ОтветыТаблицы Цикл
		Если НЕ ЗначениеНеЗаполнено(ОтветИзТаблицы.ТиповойОтвет) тогда
			ПустаяТаблица = Ложь;
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПоместитьВопросыВОбъектЗагрузки(ХМЛ, ПотокВывода, ДатаЗаписи, Анкета, ТабличныйВопрос = Неопределено, НомерСтрокиВтаблице = 0)
	...
	Для каждого СтрокаОтвет из ОтветыТаблицы Цикл
		...
		Если ЕстьКонтактныеЛица И Анкета.ВидСправочникаДляЗагрузки = Перечисления.ВидыОбъектовЗагружаемыхИзОпроса.КонтактныеЛица и
			...
			ЗначениеНеЗаполнено(ПотокВывода.Фамилия + ПотокВывода.Имя + ПотокВывода.Отчество) тогда
				...
				ПотокВывода.ДатаРождения 	= ДатаРождения;
				ПотокВывода.Наименование	= Фамилия + " " + Имя + " " + Отчество;
				Если НЕ ЗначениеНеЗаполнено(КонтрагентДляЗаписи) тогда
		...
		// Контрагент
		Если (ЕстьКонтрагенты И НЕ ЗначениеНеЗаполнено(ИНН) и ТипЗнч(ПотокВывода) = Тип("СправочникОбъект.Контрагенты")) ИЛИ
			 (ЕстьКонтрагенты И НЕ ЗначениеНеЗаполнено(НаименованиеКонтрагента) и ТипЗнч(ПотокВывода) = Тип("СправочникОбъект.Контрагенты")) тогда
		...
		Если ЕстьКонтактныеЛица И НЕ ЗначениеНеЗаполнено(Фамилия) и 
			 НЕ ЗначениеНеЗаполнено(Имя) и 
			 НЕ ЗначениеНеЗаполнено(Отчество) и 
			 НЕ ЗначениеНеЗаполнено(ДатаРождения) и  
		...
		Если ЕстьКонтрагенты И НЕ ЗначениеНеЗаполнено(Контрагент) тогда
	...
	// гражданство
	Если ЕстьФизическиеЛица И НЕ ЗначениеНеЗаполнено(Гражданство) тогда
	...
	// Семейное положение
	Если ЕстьФизическиеЛица И НЕ ЗначениеНеЗаполнено(СемейноеПоложение) тогда
	...
	// воинский учет
	Если ЕстьФизическиеЛица И НЕ ЗначениеНеЗаполнено(ОтношениеКВоинскойОбязанности) или
		...
		НЕ ЗначениеНеЗаполнено(ВУС) или
		НЕ ЗначениеНеЗаполнено(КатегорияЗапаса) или
		НЕ ЗначениеНеЗаполнено(Состав) или
		НЕ ЗначениеНеЗаполнено(НомерКомандыПартии) тогда
	...
	Если ЕстьКонтрагенты И ЗначениеНеЗаполнено(ПотокВывода) и НЕ ЗначениеНеЗаполнено(Контрагент) тогда
...
Процедура ЗаписатьДанныеОбОбразовании(Физлицо, ВидОбразования, УчебноеЗаведение, Специальность, ГодОкончания);
	Если НЕ ЗначениеНеЗаполнено(ВидОбразования) или
		НЕ ЗначениеНеЗаполнено(УчебноеЗаведение) или
		НЕ ЗначениеНеЗаполнено(Специальность) или
		НЕ ЗначениеНеЗаполнено(ГодОкончания) тогда
		НоваяСтрокаОбразования = Физлицо.Образование.Добавить();
		НоваяСтрокаОбразования.ВидОбразования = ВидОбразования;
		НоваяСтрокаОбразования.УчебноеЗаведение = УчебноеЗаведение;
		НоваяСтрокаОбразования.Специальность = Специальность;
		НоваяСтрокаОбразования.ГодОкончания  = ГодОкончания;
	КонецЕсли;
КонецПроцедуры

Процедура ЗаписатьДанныеОСоставеСемьи(Физлицо, СоставСемьиСтепеньРодства, СоставСемьиРодственник, СоставСемьиГодРождения);
	Если НЕ ЗначениеНеЗаполнено(СоставСемьиСтепеньРодства) или
		НЕ ЗначениеНеЗаполнено(СоставСемьиРодственник) или
		НЕ ЗначениеНеЗаполнено(СоставСемьиГодРождения) тогда
		НоваяСтрокаСоставСемьи = Физлицо.СоставСемьи.Добавить();
		НоваяСтрокаСоставСемьи.СтепеньРодства = СоставСемьиСтепеньРодства;
		НоваяСтрокаСоставСемьи.Родственник  = СоставСемьиРодственник;
		НоваяСтрокаСоставСемьи.ГодРождения  = СоставСемьиГодРождения;
	КонецЕсли;
КонецПроцедуры

Процедура ЗаписатьДанныеОТрудовойДеятельности(Физлицо, ТрудоваяДеятельностьОрганизация, ТрудоваяДеятельностьНачалоРаботы, ТрудоваяДеятельностьОкончаниеРаботы, ТрудоваяДеятельностьДолжность);    
	Если НЕ ЗначениеНеЗаполнено(ТрудоваяДеятельностьОрганизация) или
		НЕ ЗначениеНеЗаполнено(ТрудоваяДеятельностьНачалоРаботы) или
		НЕ ЗначениеНеЗаполнено(ТрудоваяДеятельностьОкончаниеРаботы) или
		НЕ ЗначениеНеЗаполнено(ТрудоваяДеятельностьДолжность) тогда
		НоваяСтрокаТрудоваяДеятельность = Физлицо.ТрудоваяДеятельность.Добавить();
		НоваяСтрокаТрудоваяДеятельность.Организация   = ТрудоваяДеятельностьОрганизация;
		НоваяСтрокаТрудоваяДеятельность.НачалоРаботы  = ТрудоваяДеятельностьНачалоРаботы;
		НоваяСтрокаТрудоваяДеятельность.ОкончаниеРаботы  = ТрудоваяДеятельностьОкончаниеРаботы;
		НоваяСтрокаТрудоваяДеятельность.Должность   = ТрудоваяДеятельностьДолжность;
	КонецЕсли;
КонецПроцедуры

Процедура ЗаписатьДанныеОЗнанииЯзыков(Физлицо, Язык, СтепеньЗнанияЯзыка);
	Если НЕ ЗначениеНеЗаполнено(Язык) или
		НЕ ЗначениеНеЗаполнено(СтепеньЗнанияЯзыка) тогда
		НоваяСтрокаЗнаниеЯзыков = Физлицо.ЗнаниеЯзыков.Добавить();
		НоваяСтрокаЗнаниеЯзыков.Язык    = Язык;
		НоваяСтрокаЗнаниеЯзыков.СтепеньЗнанияЯзыка = СтепеньЗнанияЯзыка;
	КонецЕсли;
КонецПроцедуры
