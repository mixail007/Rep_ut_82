	...
	Пока ПравилаОбмена.Прочитать() Цикл
		...
		ИначеЕсли ИмяУзла = "ПоискПоТабличнымЧастям" Тогда
			...
			Для Номер = 1 По СтрЧислоСтрок(Значение) Цикл
				...
				СтрокаТаблицы.ПоляПоискаТЧ = РазложитьСтрокуВМассивПодстрок1(СтрокаПоиска);
...
Процедура ВыгрузитьОбработчикиСобытий(Отказ) Экспорт
	...
	#Если Клиент Тогда
		Если ЭтоИнтерактивныйРежим Тогда
			...
			Если ФлагОшибки Тогда
				сообщить("При выгрузке обработчиков были обнаружены ошибки.",СтатусСообщения.Внимание);  //01.04.2019
				
			Иначе
				
				сообщить("Обработчики успешно выгружены.", СтатусСообщения.Информация);
			КонецЕсли;
...
//
Функция ПолучитьИмяФайлаОбмена1(ИмяКаталога, ИмяФайла) Экспорт
...
Функция РаспаковатьZipФайл(ИмяФайлаДляРаспаковки)
	...
	Попытка
		...
		Если Архиватор.Элементы.Количество() > 0 Тогда
			...
			ИмяРаспакованногоФайла = ПолучитьИмяФайлаОбмена1(КаталогДляРаспаковки, Архиватор.Элементы[0].Имя);
...
Процедура ВыполнитьВыгрузку() Экспорт
	...
	#Если Клиент Тогда
		Если ЭтоИнтерактивныйРежим Тогда
			ОткрытьДанныеПротоколовОбменаПриНеобходимости();
			сообщить(строка(ТекущаяДата())+" Выгрузка данных завершена.", СтатусСообщения.Информация);		//01.04.2019
		КонецЕсли;	
	#КонецЕсли
