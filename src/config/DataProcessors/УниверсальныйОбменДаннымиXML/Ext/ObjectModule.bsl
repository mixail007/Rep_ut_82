...
Функция одПустое(Значение, ЭтоNULL=Ложь)
	...
	Иначе
		...
		Попытка
			Результат = ЗначениеНеЗаполнено(Значение);
			Возврат Результат;
		Исключение
			Возврат Ложь;
		КонецПопытки;
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		ИначеЕсли (ИмяУзла = "Свойство" ИЛИ ИмяУзла = "ЗначениеПараметра") И (ФайлОбмена.ТипУзла = одТипУзлаXML_КонецЭлемента) Тогда
			...
			Если Не НаличиеСвойств
				И НЕ ЗначениеНеЗаполнено(Тип) Тогда
		...
		Если СсылкаНаОбъект <> Неопределено
			...
			ИначеЕсли ИмяТипаОбъекта = "Документ" 
				...
				Если РежимПоискаОсновногоОбъекта Тогда
					...
					ДатаПустая = ЗначениеНеЗаполнено(СвойстваПоиска["Дата"]);
...
Процедура ДобавитьСложныйПараметрПриНеобходимости(ПараметрыДанных, ИмяВеткиПараметров, НомерСтроки, ИмяПараметра, ЗначениеПараметра)
	...
	Если ТекущиеДанныеПараметра = Неопределено Тогда
		...
		//ТекущиеДанныеПараметра.Индексы.Добавить("НомерСтроки"); //8.1
...
Процедура ПровестиГенерациюКодаНомераПриНеобходимости(ГенерироватьНовыйНомерИлиКодЕслиНеУказан, Объект, ИмяТипаОбъекта, НужноЗаписатьОбъект, 
	...
	Если ИмяТипаОбъекта = "Документ"
		...
		Если ЗначениеНеЗаполнено(Объект.Номер) Тогда
	...
	ИначеЕсли ИмяТипаОбъекта = "Справочник"
		...
		Если ЗначениеНеЗаполнено(Объект.Код) Тогда
			...
			ИначеЕсли ИмяТипаОбъекта <> "Перечисление" Тогда
				...
				Если ИмяТипаОбъекта = "РегистрСведений" Тогда
					...
					Если Периодический Тогда
						...
						Если ЗначениеНеЗаполнено(Объект.Период) Тогда
...
Процедура ВыполнитьДействияПослеЗавершенияЧтенияДанных() Экспорт
	...
	#Если Клиент Тогда		
		...
		//	Предупреждение("Загрузка данных завершена!", 60);	//+++ 27.07.2016		
			Сообщить("Выгрузка данных завершена!", СтатусСообщения.Информация);  
		КонецЕсли;
	#КонецЕсли
...
Процедура ВыполнитьВыгрузку() Экспорт
	...
	//мСтекВызововВыгрузкиДанных.Индексы.Добавить("Ссылка"); //8.1
	...
	#Если Клиент Тогда
		Если ЭтоИнтерактивныйРежим Тогда
			ОткрытьДанныеПротоколовОбменаПриНеобходимости();
		//+++ 27.07.2016	
		//	Предупреждение("Выгрузка данных завершена!", 5); // 5-10 сек. 
			Сообщить("Выгрузка данных завершена!", СтатусСообщения.Информация);  
		КонецЕсли;	
	#КонецЕсли
