...
Функция ОпределитьИмяТаблицыОбъекта() Экспорт
	...
	Если ТипОбъекта = "КонтактныеЛицаКонтрагентов" ИЛИ ТипОбъекта = "ЛичныеКонтакты" ИЛИ ТипОбъекта = "ПрочиеКонтактныеЛица" Тогда
...
Процедура ЗаполнитьТаблицуПолей()
	...
	мФормаИндикации.КомментарийОбработкиДанных = "Выполняется заполнение таблицы полей ...";
	...
	Если НаименованиеПрограммы = "OutlookExpress" Тогда
		...
		мФормаИндикации.МаксимальноеЗначение = СписокПолей.Количество();
		мФормаИндикации.Открыть();
		
		ИндПоля = 0;
		Для каждого ЭлементСписка Из СписокПолей Цикл

			мФормаИндикации.Значение = Число(СписокПолей.Индекс(ЭлементСписка) + 1);
	...
	ИначеЕсли НаименованиеПрограммы = "MSOutlook" Тогда
		...
		Для каждого ЭлементСписка Из СписокПолей Цикл
			...
			мФормаИндикации.Значение = СписокПолей.Индекс(ЭлементСписка) + 1;
...
Процедура ЗаполнитьТаблицуПолейВыгрузки()
	...
	мФормаИндикации.КомментарийОбработкиДанных = "Заполняется таблица полей выгрузки ...";
	мФормаИндикации.МаксимальноеЗначение = мСписокРеквизитовОбъекта.Количество();
	мФормаИндикации.Значение = 0;
	мФормаИндикации.Открыть();
	...
	Для каждого Поле Из мСписокРеквизитовОбъекта Цикл
		мФормаИндикации.Значение = мСписокРеквизитовОбъекта.Индекс(Поле) + 1;
...
Функция ЗаполнитьТаблицуДанныхФайла()
	...
	мФормаИндикации.КомментарийОбработкиДанных = "Выполняется анализ данных файла ...";
	мФормаИндикации.МаксимальноеЗначение = ТекстФайла.КоличествоСтрок();
	мФормаИндикации.Значение = 2;
	мФормаИндикации.Открыть();
...
Процедура ПроверитьТаблицуОбъектовВыгрузки() Экспорт
	...
	мФормаИндикации.КомментарийОбработкиДанных = "Проверяется таблица объектов выгрузки ...";
	мФормаИндикации.МаксимальноеЗначение = НастройкаВыгрузкиОбъектов.Количество();
	мФормаИндикации.Значение = 0;
	мФормаИндикации.Открыть();
...
Процедура ЗаписатьВыгружаемуюИнформацию() Экспорт
	...
	мФормаИндикации.КомментарийОбработкиДанных = "Формируется выходной файл ...";
	мФормаИндикации.Значение = 0;
	мФормаИндикации.Открыть();
	...
	НомерПоляКИ = 0;
	мФормаИндикации.МаксимальноеЗначение = НастройкаПолейВыгрузки.Количество();
	Для каждого СтрокаТаблицы Из НастройкаПолейВыгрузки Цикл
	
		мФормаИндикации.Значение = НастройкаПолейВыгрузки.Индекс(СтрокаТаблицы) + 1;
...
Процедура ЗаписатьЗагружаемуюИнформацию(РодительДляЗагрузки = Неопределено) Экспорт
	...
	мФормаИндикации.КомментарийОбработкиДанных = "Загрузка данных ...";
	...
	Для каждого СтрокаТаблицы Из НастройкаЗагрузкиЗначений Цикл
		...
		мФормаИндикации.Значение = НастройкаЗагрузкиЗначений.Индекс(СтрокаТаблицы) + 1;
		...
		Если СтрокаТаблицы.Объект1С = Неопределено ИЛИ СтрокаТаблицы.Объект1С.Пустая() Тогда
			...
			Если НЕ ЗначениеНеЗаполнено(РодительДляЗагрузки) Тогда
				Объект1С.Родитель = РодительДляЗагрузки;
			КонецЕсли; 
			
			Если ТипОбъекта = "ПрочиеКонтактныеЛица" Тогда
				Объект1С.ОбъектВладелец = СтрокаТаблицы.Владелец;
				Объект1С.ВидКонтактногоЛица = Перечисления.ВидыКонтактныхЛиц.ПрочееКонтактноеЛицо;
			ИначеЕсли ТипОбъекта = "КонтактныеЛицаКонтрагентов" Тогда
				Объект1С.ОбъектВладелец = СтрокаТаблицы.Владелец;
				Объект1С.ВидКонтактногоЛица = Перечисления.ВидыКонтактныхЛиц.КонтактноеЛицоКонтрагента;
			ИначеЕсли ТипОбъекта = "ЛичныеКонтакты" Тогда
				Объект1С.ОбъектВладелец = глТекущийПользователь;
				Объект1С.ВидКонтактногоЛица = Перечисления.ВидыКонтактныхЛиц.ЛичныйКонтакт;
			...
			ИначеЕсли СтрокаПолей.ТипРеквизита1С = "ВидКИ" Тогда
				...
				Если СтрЧислоВхождений(СтрокаПолей.ИндПоля, ";") = 0 Тогда
					...
					Если ТипКонтактнойИнформации = Перечисления.ТипыКонтактнойИнформации.Телефон Тогда
						...
						СписокПолейТелефона = РазложитьТелефонПоПолям(СтрокаФайла[СтрокаПолей.ИндПоля]);

						ЗаписьРегистраКИ.Поле3 = СписокПолейТелефона.НайтиПоЗначению("СамТелефон").Представление;
						ЗаписьРегистраКИ.Поле1 = СписокПолейТелефона.НайтиПоЗначению("КодСтраны").Представление;
						ЗаписьРегистраКИ.Поле2 = СписокПолейТелефона.НайтиПоЗначению("КодГорода").Представление;
				...
				Иначе
					...
					Если СписокКонечныхЗначений.Количество() > 0 Тогда
						...
						Если ЕстьНеПустые Тогда
							...
							ЗаписьРегистраКИ.Представление = ПолучитьПредставлениеАдреса(ЗаписьРегистраКИ);
							ЗаписьРегистраКИ.Комментарий = "Перенсено из " + СокрЛП(НаименованиеПрограммы);
