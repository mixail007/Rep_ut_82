...
Процедура ПередЗаписью(Отказ)
	...
	Если ТипЗнч(Объект) = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваров") и не Перенаправлена Тогда
		...
		Если КолДа>0 тогда // согласовали хоть одну позицию  - проверяем и перенаправляем на руководителей
			
			//по просьбе Егора исключить его из цепочки утверждений
			рукАКБ = справочники.НоменклатурныеГруппы.НайтиПоКоду("00060").Ответственный;
			
			Если Товары[0].Номенклатура.ВидТовара = Перечисления.ВидыТоваров.АКБ 
				и Товары.Итог("Количество")>=10 и Исполнитель <> рукАКБ Тогда
			...
			ИначеЕсли Товары.Итог("Сумма")>=100000 
				  и (Исполнитель = Справочники.Пользователи.НайтиПоКоду("Серков") 
				    или Исполнитель = Справочники.Пользователи.НайтиПоКоду("Бондаренко Е.Д. (снабжение)")
				    или Исполнитель = Справочники.Пользователи.НайтиПоКоду("Филатова С.В.")) Тогда	
				выполнена = Ложь;  Согласованно = Ложь;
				Описание = Описание+"
				|"+строка(Исполнитель)+" согласованно "+?(КолДа=Товары.Количество(),"полностью","ЧАСТИЧНО"); 
				
				Если Инициатор.ОсновноеПодразделение.ОбособленноеПодразделение тогда  
					
					Исполнитель = Инициатор.ОсновноеПодразделение.Руководитель; // Ростов - "Лигута А.С.";   // Питер  - "Красюк Н.В.";
...
процедура ИзменитьСтатусыВЗаявке() Экспорт
	 ДокОбъект = Объект.ПолучитьОбъект();
	 Для каждого стр из Товары Цикл
		 Если (найти(Наименование,"УТВЕРЖДЕНЫ")=0 и найти(Наименование,"СОГЛАСОВАНЫ")=0) 
...
Процедура СоздатьЗадачу(НаКорректировку = Ложь) Экспорт
	...
	Если НаКорректировку Тогда    
		...
		КонецЕсли;
	Иначе
		Фролов = Справочники.Пользователи.НайтиПоКоду("Фролов");
		Если Инициатор <> Фролов и ПараметрыСеанса.ТекущийПользователь = Фролов тогда /// задача от Фролова 
			//Задача.Исполнитель = Справочники.Пользователи.НайтиПоКоду("Пелевина Ирина");		
			Задача.Исполнитель = Справочники.Пользователи.НайтиПоКоду("Брызгалова А.");				
		иначе  //
			Задача.Исполнитель = Бухгалтер;			
		КонецЕсли;
	...
	Если  Объект.РаботаетПоОбратнойПродаже тогда
		...
		Если НаКорректировку Тогда 
			...
			Для Каждого ЭлементМассива Из Мссв Цикл
				...
				ИначеЕсли ЭлементМассива.Пользователь.Имя="Пименова Н.И." Тогда
					Задача.Исполнитель = Справочники.Пользователи.НайтиПоКоду("Пименова Н.И.");
				Иначе
					Задача.Исполнитель = Справочники.Пользователи.НайтиПоКоду("Невежина И.Н.");
				КонецЕсли;
			КонецЦикла;
