...
//+++12.01.2018 и для Бонусов и для ОТК тоже!		
		Для каждого стр из Товары Цикл
			Если стр.Статус = Перечисления.СтатусыСтрокЗаказа.Подтвержден тогда 	 
				...
				//Подтвержден = Истина;
			иначе
				ДокОбъект.Бонусы[стр.НомерСтрокиОснования-1].статус = Перечисления.СтатусыСтрокЗаказа.Отменен;
				//	//Отменен = Истина;
			конецЕсли; 
		конецЦикла;
...
Процедура ИзменитьСтатусыВВыплатеБонусов() Экспорт
	...
	Если Исполнитель <> Инициатор тогда //Менеджер или Пятницкая должны закрыть задачу!
		...
		Если ДокОбъект.СпособОплаты="выплаты представителю" тогда
			Исполнитель = справочники.Пользователи.НайтиПоКоду("Пятницкая");
			Наименование = "Задание на Выплату  - "+Наименование;
		Иначе	
			Исполнитель = Инициатор; //31.01.2018 - ОБРАТНО!
		КонецЕсли;
		Выполнена   = ЛОЖЬ;
...
Процедура СоздатьЗадачу(НаКорректировку = Ложь) Экспорт
	...
	Иначе
		...
		задача.Наименование = "Выставление притензии перевозчику по недогрузу контрагенту " + строка(Объект.Контрагент.НаименованиеПолное); 
