...
Процедура ПроверитьРезервы(Объект, Отказ, таблПост=неопределено) Экспорт
	...
	флИндекс = (табл.Количество()>=100); // 100х100 = 10 000
	...
	+?(разрешенСерый и разрешенЧерный,"", " ГДЕ НЕ вт2.Номенклатура В ( ВЫБРАТЬ стопЛист.Номенклатура
		|			ИЗ РегистрСведений.НоменклатураИмпорт КАК стопЛист 
	   	|"  +?(разрешенСерый и не разрешенЧерный, "ГДЕ стопЛист.ЧерныйЛист", 
	   		?(не разрешенСерый и разрешенЧерный, "ГДЕ стопЛист.ЧерныйЛист = ЛОЖЬ", ""))+")") 
	//-------------------------------------------------
