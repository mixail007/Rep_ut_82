...
Процедура ПроверитьРезервы(Объект, Отказ, таблПост=неопределено, ЭтоПроведениеИзЗаказа=ЛОЖЬ) Экспорт
	...
	разрешенЧерный = (ОбъектКонтрагент.Экспорт или Объект.РазрешитьГрузитьТоварыИзСтопЛиста или ОбъектКонтрагент = Справочники.Контрагенты.НайтиПоКоду("П004703")); //ТОЛЬКО в заказах  или Резерв для ИМ
