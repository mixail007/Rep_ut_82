...
// 4)-----------------------   договор *  - проверяем только если нет договора поручительства ----------------------------
	Если ТипЗнч(Объект) = Тип("СписокЗначений") Тогда 
		...
		Если ЭтоЗаказИлиРеализация тогда //+++ 21.11.2018 - ТУТ ТОЖЕ НАДО! 
			питСтоп = Справочники.Контрагенты.НайтиПоКоду("П019371");	// Объект1 всегда Ссылка!
			флТрКомпанияПитСтоп = ПроверитьЗначениеРеквизитаДокумента(Объект1, "ТранспортнаяКомпания", питСтоп );  //общ.модуль: УправлениеДенежнымиСредствами
		КонецЕсли;	
 ...
 //21.11.2018 - ВСЕ условия проверки предоплаты в 1 функцию >> 
		флТрКомпанияПитСтоп = ЛОЖЬ;
 		Если ЭтоЗаказИлиРеализация тогда 
			Если ТипЗнч(Объект)=Тип("ДокументОбъект.ЗаказПокупателя") // ИЛИ ТипЗнч(Объект)=Тип("ДокументОбъект.РеализацияТоваровУслуг") 
				Тогда ОбъектСсылка = Объект.ссылка; 
				Иначе ОбъектСсылка = Объект; 
			КонецЕсли;
			питСтоп = Справочники.Контрагенты.НайтиПоКоду("П019371"); //13.11.18 Смирнов для питстопа - карточники можно без предоплаты  Задача 5886	
...
//13-21.11.2018 (УЖЕ учтено через флТрКомпанияПитСтоп ) Смирнов для питстопа - карточники можно без предоплаты  Задача 58865
        //
		//Если ТипЗнч(Объект)=Тип("ДокументСсылка.ЗаказПокупателя") 
		//	ИЛИ ТипЗнч(Объект)=Тип("ДокументОбъект.ЗаказПокупателя") тогда
...
Функция ПолучитьУсловиеБезПредоплаты(УКлиентаЕстьДоговорПоручительства, ОбъектКонтрагент, ЭтоЗаказИлиРеализация, дог1, флТрКомпанияПитСтоп=ЛОЖЬ)
	...
	//+++ 20-21.11.2018 - ВКЛючено отдельное условие для ИП - проверка предоплаты - нужна по всем договорам Предоплаты (без *)
	...
	//13.11.18 Смирнов для питстопа - карточники можно без предоплаты  Задача 58865 
	Если ЭтоЗаказИлиРеализация и флТрКомпанияПитСтоп тогда  //только для питСтопа!
		рез = (Дог1.ОтветственноеЛицо.НаправлениеПродаж = Справочники.НаправленияПродаж.НайтиПоКоду("16")       //колесатут
			   и Дог1.Владелец.КатегорияПокупателя = Справочники.КатегорииПокупателей.КонечныйПотребительФЛ );
     КонецЕсли;
