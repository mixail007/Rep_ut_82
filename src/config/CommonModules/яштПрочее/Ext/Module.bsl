...
Процедура ПроверитьРезервы(Объект, Отказ, таблПост=неопределено) Экспорт
	...
	//03.06.18 Смирнов Если Экспорт, то разрешено все
	|"+?(Объект.Контрагент.Экспорт или Объект.РазрешитьГрузитьТоварыИзСтопЛиста,"",?(Объект.Контрагент.РазрешитьГрузитьТоварыИзСтопЛиста,"И НЕ Номенклатура В
	|			( ВЫБРАТЬ НоменклатураИмпорт.Номенклатура
	|			ИЗ РегистрСведений.НоменклатураИмпорт КАК НоменклатураИмпорт где НоменклатураИмпорт.ЧерныйЛист = истина)",
	" И НЕ Номенклатура В
	|			( ВЫБРАТЬ НоменклатураИмпорт.Номенклатура
	|			ИЗ РегистрСведений.НоменклатураИмпорт КАК НоменклатураИмпорт)
	|"))+"
