...
Процедура ПроверитьРезервы(Объект, Отказ, таблПост=неопределено) Экспорт
	...
	флИндекс = (табл.Количество()>=100); // 32х32 = 1024 итераций...  100х100 = 10 000
	...
	//ТУТ НЕ НАДО! только для внутреннего отбора!   
	...
	|ИЗ
	| ( ВЫБРАТЬ вт2.Номенклатура из ВТ_ЗаказТовары как вт2 ) как ВТЗаказТовары						   
    |	ЛЕВОЕ СОЕДИНЕНИЕ
