...
//24.07.2018  неявный - лучше!	пратформа сама делает ВНУТРЕННЕЕ соединение по нужным заказам
	...
	//--------------надо делать ограничение вглубину! не попадает в индекс!?---------
...
//18.07.2018 ---- U-блокировка на ВСЕ заказы! по этим товарам!  делает блокировку и выкидывает любой из этих заказов в очередь
...
//24.07.2018  неявный - лучше!	пратформа сама делает ВНУТРЕННЕЕ соединение по нужным заказам
	...
	//|	И ЗаказПокупателя.Дата>&МинДатаЗаказов
 	//---------только по товарам---уменьшает выборку и время с 2,5 до 0,7-0,9 сек, но делает внутренне соединение со всеми Товарами! ----------
	//на реальной базе до 20 - 21" на ТК504568, а без неё снижается до 8-9"
	//|		И ЗаказПокупателя.Товары.Номенклатура В (ВЫБРАТЬ вт.Номенклатура ИЗ ВТ_ЗаказТовары КАК вт "+?(флСтоп, "ГДЕ НЕ вт.Номенклатура в (&масСтоп)","")+")
 	//+++)
