...
Процедура ПроверитьРезервы(Объект, Отказ, таблПост=неопределено) Экспорт
	...
	//---30.05.2018 ЕДИНОЕ условие для всех остатков (ОСТ, ОТХ, РезерваИМ)! --------
	|ГДЕ НЕ вт2.Номенклатура В ( ВЫБРАТЬ стопЛист.Номенклатура
	|			ИЗ РегистрСведений.НоменклатураИмпорт КАК стопЛист "
	+?(Объект.Контрагент.Экспорт или Объект.РазрешитьГрузитьТоварыИзСтопЛиста,"",
	   ?(Объект.Контрагент.РазрешитьГрузитьТоварыИзСтопЛиста,"ГДЕ стопЛист.ЧерныйЛист = Истина",""))
	                       +")
	//-------------------------------------------------
