...
//+++ 12.10.2017 --------------остатки Заказанного по 1 или списку Заказов -----------------------------
	...
	//МинДата = ДобавитьМесяц(НачалоДня(ТекущаяДата()), -3); // в заданиях на отгрузку или перемещение Заказы не более 3х мес. давности!
	//Запрос1.УстановитьПараметр("МинДата",МинДата);
	запрос1.Текст = "
					//|ВЫБРАТЬ
					//|	РезервНаСкладахОстатки.ЗаказПокупателя
					//|ПОМЕСТИТЬ ВТ_ЗакВЗаданиях
					//|ИЗ
					//|	РегистрНакопления.РезервНаСкладах.Остатки( ,
					//|		НЕ ЗаказПокупателя В (&СписокЗаказов)
					//|			И ЗаказПокупателя.Проведен
					////|			И ЗаказПокупателя.Проверен
					//|			И ЗаказПокупателя.Дата>&МинДата
					//|			И ЗаказПокупателя.Контрагент = &Контрагент ) КАК РезервНаСкладахОстатки
					//|;
					//|
					//|////////////////////////////////////////////////////////////////////////////////
	                |ВЫБРАТЬ
	                ...
	                |ИЗ
	                |	РегистрНакопления.ЗаказыПокупателей.Остатки( ,
	                |			(   ЗаказПокупателя.ВМаршрут
					|        	   И ЗаказПокупателя.Проверен
			|					    
			//|					 ИЛИ ЗаказПокупателя В (ВЫБРАТЬ вт.ЗаказПокупателя
			//|											ИЗ ВТ_ЗакВЗаданиях КАК вт)
					|			  ) 
					|		И ДоговорКонтрагента В (&массивДоговоров)
	     			|		И НЕ ЗаказПокупателя В (&СписокЗаказов)) КАК ЗаказыПокупателейОстатки
	                |ГДЕ
	                |	ЗаказыПокупателейОстатки.КоличествоОстаток > 0
	                //|;УНИЧТОЖИТЬ ВТ_ЗакВЗаданиях
					|";
		...
		Если НЕ СтрТЗ.СуммаКонтракта = 0 Тогда
			...
			Если СуммаОтгрузокПоДоговору > СтрТЗ.СуммаКонтракта Тогда
				...
				#Если Клиент Тогда
					...
					|Отгружено по контракту на сумму " + Строка(СуммаОтгрузокПоДоговору) + " " + СтрТЗ.ДоговорКонтрагента.ВалютаВзаиморасчетов.Наименование);
				#КонецЕсли	
			КонецЕсли;
		КонецЕсли;
...
Процедура ПроверитьРезервы(Объект, Отказ, таблПост=неопределено) Экспорт
	...
	ИспользоватьОстатокПостащиков = ?(Объект.ДоговорКонтрагента.ОтветственноеЛицо.НаправлениеПродаж = Справочники.НаправленияПродаж.НайтиПоКоду("16"), ложь, Константы.ИспользоватьДележкуЗаказовВМинус.Получить());
	...
	|		ЕстьNull(ВТСклады.КоличествоОстаток,0) КАК КоличествоОстаток,
	|		0 КАК КоличествоНаОТХ
	
	|	ИЗ  ВТ_ЗаказТовары как ВТЗаказТовары
	|	Левое Соединение
	|(ВЫБРАТЬ
	|	Склады.Номенклатура,
	...
	//+++ 14.11.2017 отбор по запрещенным дискам только если нет разрешения!
	|"+?(Объект.Контрагент.РазрешитьГрузитьТоварыИзСтопЛиста,"", " И НЕ Номенклатура В
	|			( ВЫБРАТЬ НоменклатураИмпорт.Номенклатура
	|			ИЗ РегистрСведений.НоменклатураИмпорт КАК НоменклатураИмпорт)
	|")+"
	////ВМЕСТО
	////(Плотников 04.07.2017 новый отбор по запрещенным дискам 
	//|   И ВЫБОР КОГДА &РазрешитьДопДиски = ИСТИНА
	//|	ТОГДА ИСТИНА
	...
	|	ГДЕ Склады.КоличествоОстаток>0
	
	|) КАК ВТСклады
	
	|ПО ВТЗаказТовары.Номенклатура = ВТСклады.Номенклатура
	...
	//07.11.2016 для правильного расчета товаров в ожидании... надо делать до даты заказа!
	// "+?(ИспользоватьОстатокПостащиков,"&ДатаЗаказа","")+"
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗаказыПокупателей.Остатки(, Номенклатура В (ВЫБРАТЬ ВТ_ЗаказТовары.Номенклатура ИЗ ВТ_ЗаказТовары)
	|					И ЗаказПокупателя <> &Ссылка
	|					И ЗаказПокупателя.Проверен
	
	//26.05.2017 - уменьшает выборку заказов почти в 10 раз, а процент в общем запросе с 63% до 24% - в 3 раза!
	|					И ЗаказПокупателя.проведен
	...
	|					ОстНомКонтр.Номенклатура В (ВЫБРАТЬ ВТ_ЗаказТовары.Номенклатура ИЗ ВТ_ЗаказТовары)
	...
	|		Номенклатура В (ВЫБРАТЬ ВТ_ЗаказТовары.Номенклатура ИЗ ВТ_ЗаказТовары)
	|		И ЗаказПокупателя.Проверен
	...
	//ЗапросКБазе.УстановитьПараметр("РазрешитьДопДиски", Объект.Контрагент.РазрешитьГрузитьТоварыИзСтопЛиста);
	...
	Если //НЕ Объект.Ссылка.Контрагент.КонтрагентДляРезерваИМ.Пустая() или //123Заменить
		...
		|	ЕСТЬNULL(РезервДляИМ.КоличествоОстаток, 0),
		|	0
		|ИЗ
		|	РегистрНакопления.РезервДляИМ.Остатки( , КонтрагентДляРезерваИМ = &КлиентИМ
		|				И Подразделение = &ПодразделениеИМ
		|				И Номенклатура В (ВЫБРАТЬ ВТ_ЗаказТовары.Номенклатура ИЗ ВТ_ЗаказТовары)
		|											) КАК РезервДляИМ
		//+++ 30.05.2016   ТОЛЬКО  + резерв прибавляем !
		|ГДЕ
		|	ЕСТЬNULL(РезервДляИМ.КоличествоОстаток, 0) > 0
		|"); //активируем!
...
Процедура ОтправитьПисьмоОНедогрузе(ТекстОповещения, Заказ) Экспорт;
	//========================Проверим уч.запись======================
	УЗ =  ПолучитьЗначениеПоУмолчанию(ПараметрыСеанса.ТекущийПользователь,"ОсновнаяУчетнаяЗапись");
	...
	Для каждого Стр из ТекстОповещения.Товары Цикл
		ТекстПисьма = "" + Стр.Номенклатура.НаименованиеПолное + " - в количестве: " + Стр.Количество + " " + Стр.ЕдиницаИзмерения + Символы.ПС;
	КонецЦикла;
	СписокФайловВложений=Новый СписокЗначений;
	
	ТекстСообщения0 ="   Добрый день,
	|
	|Высылаем Вам информацию о недогрузе товаров по заказу №"+Заказ.Номер+"
...
Функция ЗапросПоСтатусуКонтрагентаФНС(Контрагенты) Экспорт
	...
	Запрос.УстановитьПараметр("Значение", Перечисления.СтатусКонтрагентаФНС.Статус_0);
	Запрос.УстановитьПараметр("Объект", Контрагенты);
	Запрос.УстановитьПараметр("Свойство", ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоКоду("90270"));
...
Функция ПолучитьБонусы(ПолучитьТаблицу = ложь) экспорт
	...
	Запрос.Текст ="ВЫБРАТЬ РАЗЛИЧНЫЕ
	              ...
	              |ГДЕ
	              //|	ПравилаЦенообразованияВход.Бонус <> 0 и
  ...
  Иначе
	  ...
	  ТаблицаБонусов.Колонки.Добавить("Номенклатура");
	  ТаблицаБонусов.Колонки.Добавить("Бонус");
	  ТаблицаБонусов.Колонки.Добавить("Период");
  КонецЕсли;
