	...
	Если Не пДокументОбъект.ОбменДанными.Загрузка Тогда
		...
		Если ЕстьРеквизитДокумента("Проверен", пДокументОбъект.Метаданные()) Тогда
			...
			Если (пДокументОбъект.Проверен Или пДокументОбъект.Ссылка.Проверен)
				И Не ПолучитьЗначениеПоУмолчанию(ПараметрыСеанса.ТекущийПользователь, "РазрешитьРедактироватьПроверенныеДокументы")
				И НЕ ПолучитьЗначениеПоУмолчанию(ПараметрыСеанса.ТекущийПользователь, "РазрешитьРедактироватьНоменклатурныеГруппыВДокументахПриОтраженииВЗатратах") //***Задача № 56734 поэтому условию разрешено редактировать только ном.группы затрат
				Тогда
...
Процедура яштФормаДокументаПриОткрытии(пДокументОбъект, пФормаДокумента) Экспорт
	...
	Если РольДоступна("МенеджерФормулы") и ТипЗнч(пДокументОбъект) = Тип("ДокументОбъект.РеализацияТоваровУслуг")  тогда
		пФормаДокумента.ТолькоПросмотр = Истина;
	КонецЕсли;
...
//+++ 22.11.2018 - ТУТ ТОЖЕ НАДО - ТОЛЬКО для колесатут -> ВСЕ условия проверки предоплаты в 1 функцию
		флТрКомпанияПитСтоп = ЛОЖЬ;
		Если ЭтоЗаказИлиРеализация 
			и Дог1.ОтветственноеЛицо.НаправлениеПродаж = Справочники.НаправленияПродаж.НайтиПоКоду("16")  //колесатут
			и Дог1.Владелец.КатегорияПокупателя = Справочники.КатегорииПокупателей.КонечныйПотребительФЛ тогда
			//>> ТУТ УЖЕ Объект1 - ДокументСсылка
			Если      ТипЗнч(Объект1)=Тип("ДокументСсылка.ЗаказПокупателя") 	   тогда ОбъектСсылка = Объект1;
			ИначеЕсли ТипЗнч(Объект1)=Тип("ДокументСсылка.РеализацияТоваровУслуг") тогда ОбъектСсылка = Объект1.Сделка;
			КонецЕсли;
			питСтоп = Справочники.Контрагенты.НайтиПоКоду("П019371"); 
			Если ЗначениеЗаполнено(ОбъектСсылка) и ТипЗнч(ОбъектСсылка) = Тип("ДокументСсылка.ЗаказПокупателя") тогда 
				флТрКомпанияПитСтоп = ПроверитьЗначениеРеквизитаДокумента(ОбъектСсылка, "ТранспортнаяКомпания", питСтоп );  //общ.модуль: УпрвлениеДенежнымиСредствами
			КонецЕсли;	
		КонецЕсли;	
		УсловиеБезПредоплаты = ПолучитьУсловиеБезПредоплаты(УКлиентаЕстьДоговорПоручительства, ОбъектКонтрагент, ЭтоЗаказИлиРеализация, дог1, флТрКомпанияПитСтоп);
		
		//-----собираем только договоры Без поручительства и Без предоплаты----------------------							 
			Если НЕ УсловиеБезПредоплаты тогда
				...
				иначе	//реализация или что-то ещё
					 Объект1 = списокЗак[i].Значение;
	...
	//=================================объект не списокЗначений =====================================================
		...
		ЭтоЗаказИлиРеализация = (   ТипЗнч(Объект)=Тип("ДокументСсылка.ЗаказПокупателя") 
								ИЛИ ТипЗнч(Объект)=Тип("ДокументОбъект.ЗаказПокупателя")
									ИЛИ ТипЗнч(Объект)=Тип("ДокументСсылка.РеализацияТоваровУслуг")
			                        ИЛИ ТипЗнч(Объект)=Тип("ДокументОбъект.РеализацияТоваровУслуг")  // у Колеса тут может быть такое?
								);
 ...
 //<< 13.11.18 -  Смирнов для питстопа - карточники можно без предоплаты  Задача 5886 (см. условие ниже )	
 //>> 22.11.2018 - ВСЕ условия проверки предоплаты в 1 функцию
		флТрКомпанияПитСтоп = ЛОЖЬ;
		Если ЭтоЗаказИлиРеализация 
			и Дог1.ОтветственноеЛицо.НаправлениеПродаж = Справочники.НаправленияПродаж.НайтиПоКоду("16")  //колесатут
			и Дог1.Владелец.КатегорияПокупателя = Справочники.КатегорииПокупателей.КонечныйПотребительФЛ тогда
			//надо получить именно из Объект -> ДокументСсылка.ЗаказПокупателя
			Если ТипЗнч(Объект)=Тип("ДокументСсылка.ЗаказПокупателя") 		тогда  ОбъектСсылка = Объект;
			ИначеЕсли ТипЗнч(Объект)=Тип("ДокументОбъект.ЗаказПокупателя")  тогда  ОбъектСсылка = Объект.Ссылка;
			ИначеЕсли ТипЗнч(Объект)=Тип("ДокументОбъект.РеализацияТоваровУслуг") 
				  ИЛИ ТипЗнч(Объект)=Тип("ДокументСсылка.РеализацияТоваровУслуг") тогда  ОбъектСсылка = Объект.Сделка;
			КонецЕсли;
			питСтоп = Справочники.Контрагенты.НайтиПоКоду("П019371"); 
			Если ЗначениеЗаполнено(ОбъектСсылка) и ТипЗнч(ОбъектСсылка) = Тип("ДокументСсылка.ЗаказПокупателя") тогда 
				флТрКомпанияПитСтоп = ПроверитьЗначениеРеквизитаДокумента(ОбъектСсылка, "ТранспортнаяКомпания", питСтоп );  //общ.модуль: УпрвлениеДенежнымиСредствами
...
//13.11.2018 ( УЖЕ учтено выше через флТрКомпанияПитСтоп ) Смирнов для питстопа - карточники можно без предоплаты  Задача 58865
        //
		//Если ТипЗнч(Объект)=Тип("ДокументСсылка.ЗаказПокупателя") 
		//	ИЛИ ТипЗнч(Объект)=Тип("ДокументОбъект.ЗаказПокупателя") тогда
		//	Если Дог1.ОтветственноеЛицо.НаправлениеПродаж = Справочники.НаправленияПродаж.НайтиПоКоду("16")  //колесатут
		//		и Дог1.Владелец.КатегорияПокупателя = Справочники.КатегорииПокупателей.КонечныйПотребительФЛ
		//		и Объект.ТранспортнаяКомпания = Справочники.Контрагенты.НайтиПоКоду("П019371") //питстоп
		//		тогда
		//		УсловиеБезПредоплаты = истина;
		//	КонецЕсли;
		//ИначеЕсли ТипЗнч(Объект)=Тип("ДокументСсылка.РеализацияТоваровУслуг") 
		//	ИЛИ ТипЗнч(Объект)=Тип("ДокументОбъект.РеализацияТоваровУслуг") тогда
		//	Если Дог1.ОтветственноеЛицо.НаправлениеПродаж = Справочники.НаправленияПродаж.НайтиПоКоду("16")  //колесатут
		//		и Дог1.Владелец.КатегорияПокупателя = Справочники.КатегорииПокупателей.КонечныйПотребительФЛ
		//		и Объект.Сделка.ТранспортнаяКомпания = Справочники.Контрагенты.НайтиПоКоду("П019371") //питстоп
		//		тогда
		//		УсловиеБезПредоплаты = истина;
		//	КонецЕсли;
		//КонецЕсли;
...
//5) ---- Проверка ПРЕДОПЛАТЫ -----------------------------------------------
	Если НЕ Отказ И ТабДог.Количество()>0 Тогда 
			...
			Запрос2.Текст = "ВЫБРАТЬ
			               ...
			               |	ВзаиморасчетыСКонтрагентамиОстатки.ДоговорКонтрагента.Код как ДоговорКод,
						   |	ВзаиморасчетыСКонтрагентамиОстатки.ДоговорКонтрагента.Владелец как ДоговорВладелец,
					...
					Если Выборка.ДоговорКонтрагента = ТабДог[ii].Договор Тогда
						...
						если  delta <=0 тогда
							...
							//Сообщить("По договору № "+строка(ТабДог[ii].Договор.Код)+" для контрагента '" + строка(ТабДог[ii].Договор.Владелец)+"' - Есть полная предоплата для отгруки на сумму: "+строка(ТабДог[ii].Сумма)+"р. Отгрузка - разрешена.", СтатусСообщения.Информация);
							  Сообщить("Для контрагента " + строка(Выборка.ДоговорВладелец)+" - есть полная Предоплата по договору № "+строка(Выборка.ДоговорКод)+" для отгрузки на сумму: "+строка(ТабДог[ii].Сумма)+"р. Отгрузка - разрешена.", СтатусСообщения.Информация);
							#КонецЕсли
						...
						иначе 
							...
							Сообщить("Для контрагента " + строка(Выборка.ДоговорВладелец) + " по договору № "+строка(Выборка.ДоговорКод)+" - частичная Предоплата: "+строка(-выборка.СуммаВзаиморасчетовОстаток)+"р."
							+?(ТабДог[ii].Сумма>0,"("+формат(-выборка.СуммаВзаиморасчетовОстаток/ТабДог[ii].Сумма*100,"ЧДЦ=0")+"%)","")  // % предоплаты
							+" меньше суммы: "+строка(ТабДог[ii].Сумма)+"р. на "+строка(delta)+"р.!", СтатусСообщения.Внимание);
							#КонецЕсли
						конецЕсли;
					КонецЕсли;	
				КонецЦикла;
			...
			//11.02.19 Смирнов, задача №61398, для юриков ИМ можно отгружать закз, при условии, что сумма заказа не превышает Контрагент.ДопустимаяСуммаЗадолженностиИМ
//-----------------------------------------------
			Если отказ и ЭтоЗаказИлиРеализация 
				и Дог1.ОтветственноеЛицо.НаправлениеПродаж = Справочники.НаправленияПродаж.НайтиПоКоду("16")  //колесатут
				и Дог1.Владелец.КатегорияПокупателя = Справочники.КатегорииПокупателей.КонечныйПотребитель тогда
				//>> ТУТ УЖЕ Объект1 - ДокументСсылка
				...
				ИначеЕсли ТипЗнч(Объект)=Тип("ДокументСсылка.РеализацияТоваровУслуг") или ТипЗнч(Объект)=Тип("ДокументОбъект.РеализацияТоваровУслуг") тогда 
					ОбъектСсылка = Объект.Сделка;
				КонецЕсли;
	
				Если ЗначениеЗаполнено(ОбъектСсылка) и ТипЗнч(ОбъектСсылка) = Тип("ДокументСсылка.ЗаказПокупателя") тогда 
					ДопустимаяСуммаЗадолженностиИМ = Дог1.Владелец.ДопустимаяСуммаЗадолженностиИМ;
					Если ДопустимаяСуммаЗадолженностиИМ>=ОбъектСсылка.СуммаДокумента тогда
						Отказ = ложь;
						#Если Клиент тогда
							Сообщить("Для контрагента " + строка(Выборка.ДоговорВладелец) + " установлен лимит отгрузки без предоплаты ИМ: "+ ДопустимаяСуммаЗадолженностиИМ+"р. Отгрузка - разрешена.", СтатусСообщения.Информация);
						#КонецЕсли
					КонецЕсли;
				КонецЕсли;	
			КонецЕсли;
...
СуммаТекущихОтгрузокПоСкладам = 0;
	...
	//11.02.19 Смирнов
	флДобавитьДопустимуюСуммуЗадолженностиИМ = ложь;
	Попытка 
		Если ЭтоЗаказИлиРеализация тогда
			Если ТипЗнч(Объект)=Тип("ДокументСсылка.ЗаказПокупателя") тогда 
				ОбъектСсылка = Объект;
			ИначеЕсли ТипЗнч(Объект)=Тип("ДокументОбъект.ЗаказПокупателя") тогда 
				ОбъектСсылка = Объект.Ссылка;	
			...
			Если ЗначениеЗаполнено(ОбъектСсылка) и ТипЗнч(ОбъектСсылка) = Тип("ДокументСсылка.ЗаказПокупателя") тогда 
				Если ОбъектСсылка.ДоговорКонтрагента.ОтветственноеЛицо.НаправлениеПродаж = Справочники.НаправленияПродаж.НайтиПоКоду("16")  //колесатут
					и ОбъектСсылка.Контрагент.КатегорияПокупателя = Справочники.КатегорииПокупателей.КонечныйПотребитель тогда
					флДобавитьДопустимуюСуммуЗадолженностиИМ = истина;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;				
	Исключение
	...
	Если ТекущаяЗадолженность > ОбъектКонтрагент.ДопустимаяСуммаЗадолженности 
		//11.02.19 Смирнов
		+ ?(флДобавитьДопустимуюСуммуЗадолженностиИМ, ОбъектКонтрагент.ДопустимаяСуммаЗадолженностиИМ, 0) тогда //+++ 19.03.2014  не ноль - тогда что то скажем... или по предоплате!
...
Функция ПолучитьУсловиеБезПредоплаты(УКлиентаЕстьДоговорПоручительства, ОбъектКонтрагент, ЭтоЗаказИлиРеализация, дог1, флТрКомпанияПитСтоп=ЛОЖЬ)
	
	//2 запроса вместо 4х
	дог1Номер = дог1.Номер;
	дог1ТипДоговора = дог1.ТипДоговора;
	СоЗвездой = (Найти(дог1.Наименование,"*")>0); 
	рез =(  
		...
		//+++ 20-21.11.2018 - ВКЛючено обратно
		...
		//5. или тип =  Предоплата *  - не проверяем предоплату!!!
				ИЛИ ( дог1ТипДоговора = Справочники.ТипыДоговоров.НайтиПоКоду("00001") и СоЗвездой) 
				
		//6. или это ИП 21.11.2018 - можно без предоплаты
				 ИЛИ ( стрДлина(сокрЛП(ОбъектКонтрагент.ИНН))=12 )
		  );
		  
//+++ 26.11.2018 - ВКЛючено условие проверка предоплаты - ДЛЯ ВСЕХ клиентов по всем договорам Предоплаты (без *) и не Отсрочка/отсрочки до 14 дней
	//если не включена ЕстьПодписаннаяСлужебнаяЗаписка
	Если не ОбъектКонтрагент.ЕстьПодписаннаяСлужебнаяЗаписка тогда
		
		//++++ 26-27.11.2018 тип: предоплата, но в названии: Отсрочка 14 дн. / Договор отсрочки до 14 дней
		 дог1назв = нрег(дог1.Наименование);
		дог1Отсрочка  = Найти( дог1назв,"отсрочк" )>0 и Найти(дог1назв,"14")>0; 

	    Если (  (дог1ТипДоговора = Справочники.ТипыДоговоров.НайтиПоКоду("00001") и не дог1Отсрочка и не СоЗвездой)
	    	или дог1ТипДоговора = Справочники.ТипыДоговоров.ПредоплатаПоСчетам
	    	или дог1ТипДоговора = Справочники.ТипыДоговоров.ФакторингПредоплата
	 		  )
	    Тогда   рез = ЛОЖЬ;//НЕЛЬЗЯ без предоплаты!
		КонецЕсли;
		
	КонецЕсли;	 
	
	//13-22.11.2018 для питстопа - карточники можно без предоплаты  Задача 58865
	Если флТрКомпанияПитСтоп тогда  //только для питСтопа!
		рез = Истина;
     КонецЕсли;
...
Процедура ПроверитьРезервы(Объект, Отказ, таблПост=неопределено, ЭтоПроведениеИзЗаказа=ЛОЖЬ) Экспорт
	...
	ЗапросКБазе.УстановитьПараметр( "Транзит",  Транзит );//+++ 11.09.2014 - 13.10.2018
	ГоловноеЯрославль=Справочники.Подразделения.НайтиПоКоду("00005");
	ЗапросКБазе.УстановитьПараметр( "ПодразделениеЯр", ГоловноеЯрославль); //11.10.2018 - так лучше отбор
	ЗапросКБазе.УстановитьПараметр( "Подразделение", ?(Транзит, Объект.Подразделение, Справочники.Подразделения.ПустаяСсылка()) );//+++ 04.04.2016
	Если Транзит и (Объект.Подразделение.Пустая() или Объект.Подразделение=ГоловноеЯрославль) тогда // транзит на Склады с подразделением Ярославль?!
	...
	ИспользоватьОстатокПостащиков = ?(Объект.ДоговорКонтрагента.ОтветственноеЛицо.НаправлениеПродаж = Справочники.НаправленияПродаж.НайтиПоКоду("16") 
	                                  и НЕИспользоватьОстатокПостащиковДляКолесаТут, Ложь, Константы.ИспользоватьДележкуЗаказовВМинус.Получить() );
									  
	//+++ 15.06-07 вкл, 07-10.08 выкл с 10.08.2018 - вкл заново! для транзитных заказов/реализаций - НЕ ИСПОЛЬЗУЮТСЯ остатки сторонних поставщиков!?
	//используется остатки для стоп-листа!
	Если Транзит тогда  
		ИспользоватьОстатокПостащиков = ЛОЖЬ;
	КонецЕсли;
 	
	флИндекс = (табл.Количество()>=1000); //индексация МЕШАЕТ?! 1000 х 1000 = 1 млн. -не бывает столько строк! максимум 100-200
	
	ОбъектКонтрагент =Объект.Контрагент;//10.10.2018
	
	//+++ 07.02.2019 Задача № 63220 - Автокомплектация должна срабатывать, но в Маршрут ставить БЕЗ галки - нельзя!!!
	РазрешенСтопЛистВЗаказе = ((Объект.ТипЗаказа=1) и не Объект.ВМаршрут) или Объект.РазрешитьГрузитьТоварыИзСтопЛиста;
	
	разрешенСерый  = РазрешенСтопЛистВЗаказе или ОбъектКонтрагент.Экспорт или ОбъектКонтрагент.ВыгружатьСерыйСписокВTerminal; //в контрагенте
	разрешенЧерный = РазрешенСтопЛистВЗаказе или ОбъектКонтрагент.Экспорт или ОбъектКонтрагент.ВыгружатьЧерныйСписокВTerminal; //ТОЛЬКО в заказах  или Резерв для ИМ
	                                                                                            
//+++ 20.06.2018 стоп-лист крайне редко попадается... делаем списком и доп.флаг
масСтоп = новый Массив;
флСтоп = ложь;
Если НЕ разрешенСерый или НЕ разрешенЧерный тогда
	запросСтоп = новый Запрос;
	Спис = Табл.ВыгрузитьКолонку("Номенклатура");
	запросСтоп.УстановитьПараметр("Спис",Спис );
	запросСтоп.Текст ="ВЫБРАТЬ стопЛист.Номенклатура
		|	ИЗ РегистрСведений.НоменклатураИмпорт КАК стопЛист 
		|	 ГДЕ стопЛист.Номенклатура в (&Спис)
		...
		|";
	результатСтоп = запросСтоп.Выполнить(); 
	если не результатСтоп.Пустой() тогда
		таблСтоп = результатСтоп.Выгрузить();
		масСтоп = таблСтоп.ВыгрузитьКолонку("Номенклатура");
		флСтоп = Истина;
	ЗапросКБазе.УстановитьПараметр("масСтоп",масСтоп); 	
	КонецЕсли;
КонецЕсли;
//+++)
	...
	//+++ 29.05.2018 - для больших таблиц (больше 30-100 строк - индексация нужна!
	...
	|ИЗ
	| ВТ_ЗаказТовары как ВТЗаказТовары						   
    |	ЛЕВОЕ СОЕДИНЕНИЕ
//=============================Остатки + ОТХ + РезервИМ===========================	
	|(ВЫБРАТЬ
	|	ост.Номенклатура,
	...
	//+++ 04.06.2018 все условия стоп листа в ВТ!--------------------------------------------------
	|	Номенклатура В (выбрать вт.Номенклатура из ВТ_ЗаказТовары как вт "+?(флСтоп, "ГДЕ НЕ вт.Номенклатура в (&масСтоп)","")+")	
	//+++)
	| 	И Склад.ЗапретитьИспользование = ЛОЖЬ
	|	И Склад.Транзитный    = &Транзит
	//13.10.2018 --- для Яр - пустое ?! ---------------------------------------
	|	И Склад.Подразделение = &Подразделение
	|) КАК ост
	//15.06|	ГДЕ ост.КоличествоОстаток>0
	|		
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|
	|   ) КАК ОстОТХрезервы
	|ПО ВТЗаказТовары.Номенклатура = ОстОТХрезервы.Номенклатура
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТЗаказТовары.Номенклатура
	|
	//+++20.06.2018----- для больших таблиц (больше 30-100 строк - индексация нужна!----------------
	|"+?(ФлИндекс, "ИНДЕКСИРОВАТЬ ПО Номенклатура","")+"
	|;
	
|////////////////////////////////////////////////////////////////////////////////
//+++ всё-таки лучше через ВТ +++
|ВЫБРАТЬ
|	ЗаказыПокупателейОст.Номенклатура,
|	ЗаказыПокупателейОст.КоличествоОстаток как КоличествоОстаток 
|ПОМЕСТИТЬ ВТ_ДругиеЗаказы
|ИЗ
|	РегистрНакопления.ЗаказыПокупателей.Остатки( ,
|			 Номенклатура В (ВЫБРАТЬ вт.Номенклатура ИЗ ВТ_ЗаказТовары КАК вт 
//------------все товары!--------------------
//|"+?(флСтоп, "ГДЕ НЕ вт.Номенклатура в (&масСтоп)","")+"
|			)
//24.07.2018   неявный - лучше! пратформа делает ВНУТРЕННЕЕ соединение по нужным заказам (без блокировки!)
//НЕЯВНЫЙ ЗАПРОС - 0,3-0,8 сек  
...
// 11.10.2018 -------------------------------------------------
//по Дате - уменьшает выборку до 4-5 тыс. документов! и время с 3,5 - 5 сек	 до 1 - 1,5
	|		И ЗаказПокупателя.Дата > &МинДатаЗаказов
//11.10.2018 - отбор по заказам не транзит -добавлен Ярославль!   а поле Транзит - индексация не возможна!
	//|		И ЗаказПокупателя.Транзит  "+?(Транзит, "= Истина", "=ЛОЖЬ")+"
	|		И ЗаказПокупателя.Подразделение = "+?(Транзит, "&Подразделение", "&ПодразделениеЯр")+" 
	|	) КАК ЗаказыПокупателейОст

|ГДЕ ЗаказыПокупателейОст.КоличествоОстаток > 0
//18.07.2018 ---- U-блокировка 26.07.2018 ТОЛЬКО на Остатки Заказов по этим товарам! 
|"+ ?(ЭтоПроведениеИзЗаказа," ДЛЯ ИЗМЕНЕНИЯ РегистрНакопления.ЗаказыПокупателей.Остатки","")+"

//после группировки - индексация не нужны! 
|"+?(ФлИндекс, "ИНДЕКСИРОВАТЬ ПО ЗаказыПокупателейОст.Номенклатура","")+"
|;

//+++( 15.06.2018 ------------вынесена в ВТ----------------------------------------
|"+?(ИспользоватьОстатокПостащиков=ЛОЖЬ,"","
	| //////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТоварыВОжиданииОтПоставщика.Номенклатура,
	|	ТоварыВОжиданииОтПоставщика.КоличествоОборот КАК Количество
	|Поместить ВТ_ТоварыВОжидании
	|ИЗ
	//======================== отборы по заказам такие же как в ЗаказахПокупателей! ============================
	|	РегистрНакопления.ТоварыВОжиданииОтПоставщика.Обороты( , , , 
	//------------по всем товарам, !!!включая!!! стоп-лист!------------------------------------
	|		Номенклатура В (ВЫБРАТЬ вт.Номенклатура ИЗ ВТ_ЗаказТовары как вт)
	
//24.07.2018  неявный - лучше!	пратформа делает ВНУТРЕННЕЕ соединение по ВСЕМ нужным заказам
	|		И ЗаказПокупателя.Проведен 
	|		И ЗаказПокупателя.Проверен  
	|		И ЗаказПокупателя.ссылка <> &Ссылка
//11.10.2018 по Дате - уменьшает выборку до 4 - 5 тыс. и время под-запроса с 4-5 сек до 1 - 1,5
	|		И ЗаказПокупателя.Дата > &МинДатаЗаказов
	//11.10.2018 - отбор по заказам не транзит -добавлен Ярославль!   а поле Транзит - индексация не даёт ничего!
	//|		И ЗаказПокупателя.Транзит  "+?(Транзит, "= Истина", "=ЛОЖЬ")+"
	//индексное поле!
	|И ЗаказПокупателя.Подразделение = "+?(Транзит, "&Подразделение", "&ПодразделениеЯр")+" 
//-----------тормозит-------------------
//|		И ЗаказПокупателя.Товары.Номенклатура в (ВЫБРАТЬ вт.Номенклатура ИЗ ВТ_ЗаказТовары КАК вт)
...
|///////////////////////////////////////////////////////////////////////
|ВЫБРАТЬ
	|			ОстСторПоставщиков.Номенклатура КАК Номенклатура,
	|			ОстСторПоставщиков.Остаток КАК Остаток,
 //---------НЕ может быть несколько производителей для 1 товара!?--------------
	|			СрокиДоставки.СрокДоставки КАК СрокДоставки,
//+++ 07.08.2018 	
	|			СрокиДоставки.АвтоПодтверждение КАК АвтоПодтверждение,
	|			СрокиДоставки.КонтролироватьКоличество КАК КонтролироватьКоличество,
	|			СрокиДоставки.Поставщик КАК Контрагент
	
	|ПОМЕСТИТЬ ВТ_ОстаткиСторПоставщиков
	
	|		ИЗ РегистрСведений.ОстаткиНоменклатурыКонтрагентов КАК ОстСторПоставщиков
	
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СрокиДоставки КАК СрокиДоставки
 	|				ПО ОстСторПоставщиков.Контрагент = СрокиДоставки.Поставщик 
	
	|				ГДЕ
	|					ОстСторПоставщиков.Номенклатура В (выбрать вт.Номенклатура из ВТ_ЗаказТовары как вт 
	|					) 
	//13.10.2018 - пустое для Ярославля
	|И ОстСторПоставщиков.Подразделение = &Подразделение
...
//+++)	
	...
	//16.06.2018
	|	ВТ_ЗаказТовары.Номенклатура.Код как НоменклатураКод,
	|	Представление(ВТ_ЗаказТовары.Номенклатура) как ПредставлениеНоменклатура,
	...
	|	ВТ_ОстаткиНаСкладахИОТХ.КоличествоОстаток КАК КоличествоОстаток,
	|	ВТ_ОстаткиНаСкладахИОТХ.КоличествоНаОТХ КАК КоличествоНаОТХ,
	...
	|	- ВТ_ОстаткиНаСкладахИОТХ.КоличествоОстаток - ВТ_ОстаткиНаСкладахИОТХ.КоличествоНаОТХ
	...
	//+++ 07.08.2018 ----доп.поля передаются в Заказ----------
	|	ОстаткиСтороннихПоставщиков.АвтоПодтверждение КАК АвтоПодтверждение,
	|	ОстаткиСтороннихПоставщиков.КонтролироватьКоличество КАК КонтролироватьКоличество,
	
	|	ОстаткиСтороннихПоставщиков.Контрагент КАК Контрагент,
		
	//+++( 07.08.2018 --- Если у Стор.Поставщика [v]КонтролироватьКоличество,
	//				 то в другой заказ - нельзя заказать уже подтвержденный товар!  
	//т.е. если сначала на нападало заказов, то при пожтверждении - будет контроль по количеству!
	//если же "прайс" обновится, то КолПост может уйти в "---" и уже подтвержденные заказы не контролируются!?
	|	ВЫБОР КОГДА ОстаткиСтороннихПоставщиков.КонтролироватьКоличество=Истина
	|			ТОГДА ЕСТЬNULL(ОстаткиСтороннихПоставщиков.Остаток, 0) - ЕСТЬNULL(ТоварыВОжидании.Количество,0)
	|			ИНАЧЕ 	ЕСТЬNULL(ОстаткиСтороннихПоставщиков.Остаток, 0) КОНЕЦ
	//+++)
	|	КАК КолПост",
	//------------------------------------------------------------------
	"
	...
	//+++ 15.06.2018 --- через ВТ - лучше на больших заказах!! ------------------------
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ДругиеЗаказы как ЗаказыПокупателей
	|		ПО ВТ_ЗаказТовары.Номенклатура = ЗаказыПокупателей.Номенклатура
	|	
	//07.11.2016 +++++++++++++++++ сторонние поставщики++++++++++++++++++++++++++++++++++++++++++++
	|"+?(ИспользоватьОстатокПостащиков, "
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ОстаткиСторПоставщиков как ОстаткиСтороннихПоставщиков
	|ПО ВТ_ЗаказТовары.Номенклатура = ОстаткиСтороннихПоставщиков.Номенклатура
	|
	//------------------учет "отрицательных" остатков в ожидании по другим заказам----------------------------
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ТоварыВОжидании как ТоварыВОжидании
	|ПО ВТ_ЗаказТовары.Номенклатура = ТоварыВОжидании.Номенклатура
	|","")+"
	//+++++++++++++++++ сторонние поставщики++++++++++++++++++++++++++++++++++++++++++++
	
	|ГДЕ
	|	ВТ_ЗаказТовары.Количество + ЕСТЬNULL(ЗаказыПокупателей.КоличествоОстаток, 0) - ВТ_ОстаткиНаСкладахИОТХ.КоличествоОстаток > 0
	
	//+++20.07.2018 --- "долго" удаляются, НО чтобы tempDb не рос - надо явно удалять!!!
	|;
	|УНИЧТОЖИТЬ ВТ_ЗаказТовары;
	|УНИЧТОЖИТЬ ВТ_ОстаткиНаСкладахИОТХ;
	|УНИЧТОЖИТЬ ВТ_ДругиеЗаказы;
	|"+?(ИспользоватьОстатокПостащиков,"УНИЧТОЖИТЬ ВТ_ОстаткиСторПоставщиков; УНИЧТОЖИТЬ ВТ_ТоварыВОжидании;","")+"
	|";
	
	//26-27.06.2018 - внутри 1 запроса вместо 2х не 
	...
	//20.07.2018 - максимум заказы "висят" от 3 до 9 месяцев! (константа.МаксимальныйСрокСМоментаОтгрузкиДоОплаты.Получить() = 250 дней = 8,33 мес.) 
	//есть ещё резервные заказы ИМ... которые не закрываются?!
	МинДатаЗаказов  = '20180320';// 3-4 000 всего!  с Заказ покупателя ТК394326 от 26.03.2018 11:27:39 - резерв ИМ !
	ЗапросКБазе.УстановитьПараметр("МинДатаЗаказов", МинДатаЗаказов  ); 
	
	//+++ 15.01.2016 - не надо брать заказы КонтрагентаИМ - тогда св.остаток Увеличится!
	//Если НЕ Объект.ДоговорКонтрагента.КонтрагентДляРезерваИМ.Пустая()
	Если Объект.ДоговорКонтрагента.КонтрагентДляРезерваИМ = Справочники.Контрагенты.НайтиПоКоду("П004703")  //Резерв Для ИМ
		тогда
		...
		|				Номенклатура В 
	//+++ 04.06.2018 все условия стоп листа в ВТ!--------------------------------------------------
	|	(выбрать вт.Номенклатура из ВТ_ЗаказТовары как вт "+?(флСтоп, "ГДЕ НЕ вт.Номенклатура в (&масСтоп)","")+" )	
	//+++)
	...
	иначеЕсли ЕстьРезервРазрешения(Объект) тогда //+++ 31.10.2018 -------по Клиенту и Подразделению -----------------------
		
		ЗапросКБазе.текст = стрЗаменить(ЗапросКБазе.текст ,"//ОстаткиРезерваИМ", " ОБЪЕДИНИТЬ ВСЕ
		|"+"ВЫБРАТЬ
		   |	РезервДляИМ.Номенклатура,
		 //если несколько разных вариантов - берут одинаковые резервы - МАКСИМУМ !
		   ...
		   |	ИЗ
		   |		РегистрСведений.РезервРазрешения.СрезПоследних(&ДатаСрезаРезерва,
		   |				Контрагент = &Клиент
		   |					И Подразделение = &ПодразделениеИМ) КАК РезервРазрешенияСрезПоследних
		   |	ГДЕ
		   |		РезервРазрешенияСрезПоследних.ДатаОкончания = ДАТАВРЕМЯ(1, 1, 1) )КАК РазрешенияДляКлиента
		   
		   |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.РезервДляИМ.Остатки( ,
		   |				Номенклатура В (ВЫБРАТЬ вт.Номенклатура ИЗ ВТ_ЗаказТовары КАК вт  
		   |								//флСтопЛист
		   |								)
		   |					И Подразделение = &ПодразделениеИМ) КАК РезервДляИМ
		   ...
		   |			И (РазрешенияДляКлиента.НоменклатурнаяГруппа = РезервДляИМ.Номенклатура.НоменклатурнаяГруппа
		   	   //+++ 12.11.2018 - 15:06! все диски!
		   |			или РазрешенияДляКлиента.НоменклатурнаяГруппа = Значение(Справочник.НоменклатурныеГруппы.ПустаяСсылка) )
  		   |   	
		   |СГРУППИРОВАТЬ ПО
		   |	РезервДляИМ.Номенклатура
		   |Имеющие Сумма(РезервДляИМ.КоличествоОстаток)>0 "); //активируем!
		                                                                              
		Если флСтоп тогда //12.11.2018
		ЗапросКБазе.Текст = стрЗаменить(ЗапросКБазе.Текст,"//флСтопЛист","ГДЕ НЕ вт.Номенклатура в (&масСтоп)");
		КонецЕсли;
		
		ЗапросКБазе.УстановитьПараметр("ДатаСрезаРезерва", ?(Объект.Дата='00010101', ТекущаяДата(),Объект.Дата)  );
		ЗапросКБазе.УстановитьПараметр("Клиент",	Объект.Контрагент );
		ЗапросКБазе.УстановитьПараметр("ПодразделениеИМ", ?(Транзит, Объект.Подразделение, Справочники.Подразделения.найтиПоКоду("00005") ) ); //+++ 07.04.2016
	...
	//+++ 07.08.2018 - для Чёрного списка
	таблПост.Колонки.Добавить("АвтоПодтверждение");
	таблПост.Колонки.Добавить("КонтролироватьКоличество");
	КолПост = 0;
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.КоличествоНеХватает>0 Тогда
			...
			КолПост = Выборка.КолПост;  //"заглушка" на "---" остаток поставщика...не нужна
			...
			Если ИспользоватьОстатокПостащиков //константы.ИспользоватьОстаткиСтороннихПоставщиков.Получить() 
				...
				//19.02.18 Смирнов
				фл16 = (Объект.ДоговорКонтрагента.ОтветственноеЛицо.НаправлениеПродаж = Справочники.НаправленияПродаж.НайтиПоКоду("16")); 
				Если фл16 тогда //заказ в производство поставщику
					КолПост = КолНеХватает;
				КонецЕсли;
				
				Если КолПост>0 и КолНеХватает<=КолПост и КолНеХватает>0 тогда
					стрПост = таблПост.Добавить();
					ЗаполнитьЗначенияСвойств(стрПост, Выборка);  //+++ 07.08.2018 - все поля из рс.СрокиДоставки
				...
				#Если Клиент тогда
					текстСообщенияПользователю = "Свободного остатка "+Строка(Выборка.НоменклатураКод)+" . "+Строка(Выборка.ПредставлениеНоменклатура) //26.06.2018
		...
		ИначеЕсли Выборка.КоличествоОстаток<Выборка.КоличествоВЗаказе тогда  
			#Если Клиент тогда
				...
				иначе
					Сообщить( Строка(Выборка.НоменклатураКод)+" . "+Строка(Выборка.ПредставлениеНоменклатура) + " товаров для отгрузки не хватает "+строка(дельта)+" шт. "
...
функция ЕстьРезервРазрешения(ЗакОбъект)
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	РезервРазрешенияСрезПоследних.КонтрагентРезерв,
	|	РезервРазрешенияСрезПоследних.ВариантКод,
	|	РезервРазрешенияСрезПоследних.ДатаОкончания
	|ИЗ
	|	РегистрСведений.РезервРазрешения.СрезПоследних(&ДатаСрезаРезерва, 
	|				Контрагент = &Клиент
	|				И Подразделение = &Подразделение) КАК РезервРазрешенияСрезПоследних
	|ГДЕ
	...
	|";
	Запрос.УстановитьПараметр("Клиент", ЗакОбъект.Контрагент);
	Запрос.УстановитьПараметр("Подразделение", ЗакОбъект.Подразделение); 
	Запрос.УстановитьПараметр("ДатаСрезаРезерва", ЗакОбъект.Дата);
...
Процедура ПроверитьДоговорВЗаказеПокупателя(Объект, Отказ, Запретить=ЛОЖЬ, ДатаДок=неопределено) Экспорт
		...
		Если ТипЗнч(Объект)=Тип("ДокументОбъект.РеализацияТоваровУслуг") тогда
			...
			КонецЕсли;
		ИначеЕсли ТипЗнч(Объект)=Тип("ДокументОбъект.ЗаказПокупателя") тогда
			ОбъектКонтрагент = Объект.Контрагент;//29.11.2018 - 1 запрос вместо 6 
			Если ОбъектКонтрагент.ЗапретитьВводЗаказаПокупателя Тогда
				#Если Клиент тогда
					Сообщить("Контрагенту '"+ ОбъектКонтрагент.Наименование +"' - ЗАПРЕЩЕН Ввод новых 'Заказов покупателя'!",СтатусСообщения.Важное);
				#КонецЕсли
			...
			//перед циклом получим параметры запрета
			ЗапретДиски = ОбъектКонтрагент.ЗапретитьДиски;
			ЗапретШина  = ОбъектКонтрагент.ЗапретитьШины;
			ЗапретАКБ   = ОбъектКонтрагент.ЗапретитьАКБ;
		    ЗапретАксы  = ОбъектКонтрагент.ЗапретитьАксы;
			ЕстьЗапреты = ЗапретДиски или ЗапретШина или ЗапретАКБ или ЗапретАксы;
			Если ЕстьЗапреты тогда //99% нет запретов!
				
			для каждого стр из товарыТабл Цикл
				
				Если ЗапретДиски и стр.Номенклатура.ВидТовара = Перечисления.ВидыТоваров.Диски Тогда
					Отказ=Истина;
					#Если Клиент тогда
						Сообщить("Вам запрещено создавать заказы с дисками!",СтатусСообщения.Важное);
					#КонецЕсли
					возврат;
				ИначеЕсли ЗапретШина и стр.Номенклатура.ВидТовара = Перечисления.ВидыТоваров.Шины Тогда
					Отказ=Истина;
					#Если Клиент тогда
						Сообщить("Вам запрещено создавать заказы с шинами!",СтатусСообщения.Важное);
					#КонецЕсли
					возврат;
				ИначеЕсли ЗапретАКБ и стр.Номенклатура.ВидТовара = Перечисления.ВидыТоваров.АКБ Тогда
					Отказ=Истина;
					#Если Клиент тогда
						Сообщить("Вам запрещено создавать заказы с аккумуляторами!",СтатусСообщения.Важное);
					#КонецЕсли
					возврат;
				ИначеЕсли ЗапретАксы и стр.Номенклатура.ВидТовара = Перечисления.ВидыТоваров.Аксессуары Тогда
					Отказ=Истина;
					#Если Клиент тогда
		...
		Если ЕстьРеквизитДокумента("ДоговорКонтрагента",Объект.Метаданные()) Тогда
			ДоговорКонтрагента=Объект.ДоговорКонтрагента;
			ДоговорКонтрагентаТипДоговора = ДоговорКонтрагента.ТипДоговора; //+++ 29.11.2018 - 1 запрос Без Точки! вместо 7
			...
			Если Константы.ОплатыРаспределяютМенеджеры.Получить() Тогда
							...
							Если НЕ ПолучитьЗначениеПоУмолчанию(ПараметрыСеанса.ТекущийПользователь,"РазрешеноСозданиеДокументовЗаДругогоМенеджера") Тогда
								...
								иначе
									#Если Клиент тогда
										...
										Если СписокОснМенРегиона.НайтиПоЗначению(ДоговорКонтрагента.ОтветственноеЛицо) = Неопределено и СписокОснМенРегиона.НайтиПоЗначению(ДоговорКонтрагента.Владелец.ОсновнойМенеджерКонтрагента) = Неопределено Тогда 																					
				...
				Если (ДоговорКонтрагентаТипДоговора=Справочники.ТипыДоговоров.Пени) 
					ИЛИ (ДоговорКонтрагентаТипДоговора=Справочники.ТипыДоговоров.НД) Тогда
					#Если Клиент тогда
			...
			Если 
				//+++ 30.01.2017 - исключение ТОЛЬКО для клиентов КолесаТУТ / ФА / ФА+ / ШТ  !
				ДоговорКонтрагента.КонтрагентДляРезерваИМ <> справочники.Контрагенты.НайтиПоКоду("П004703") //Резерв для ИМ
				и ДоговорКонтрагентаТипДоговора <> справочники.ТипыДоговоров.ШинтрейдЯрославль     //ШТ
				и ДоговорКонтрагентаТипДоговора <> справочники.ТипыДоговоров.ФормулаАвтоПлюс       //ФА+
				и ДоговорКонтрагентаТипДоговора <> справочники.ТипыДоговоров.НайтиПоКоду("Я0004")  //ФА
				
	//+++ 29.11.2018 Задача № 59476 - ОБЯЗАТЕЛЬНАЯ проверка всех! Временных договоров! НЕ нужна!
				и ДоговорКонтрагентаТипДоговора <> справочники.ТипыДоговоров.ПредоплатаПоСчетам    //"00015"
//				и ( ( ДоговорКонтрагента.ТипДоговора <> справочники.ТипыДоговоров.ПредоплатаПоСчетам    //"00015"
				и не ( ДоговорКонтрагентаТипДоговора = Справочники.ТипыДоговоров.НайтиПоКоду("00001") и Найти(ДоговорКонтрагента.Наименование,"*")<>0 ) // предоплата *
//				    ) или ДоговорКонтрагента.СостояниеДоговора = справочники.СостояниеДокументов.НайтиПоКоду("13") 
//                )
		 	Тогда	 
				ДоговорКонтрагентаДатаОкончанияДействия  = ДоговорКонтрагента.ДатаОкончанияДействия; //+++ 29.11.2018 - 1 запрос Без Точки! вместо 3+2
				Если ДоговорКонтрагентаДатаОкончанияДействия>'00010101' Тогда
					Если НачалоДня(ДатаДок)>НачалоДня(ДоговорКонтрагентаДатаОкончанияДействия) Тогда
						#Если Клиент тогда
							Сообщить("ВНИМАНИЕ! НЕЛЬЗЯ ОТГРУЖАТЬ Контрагента: "+строка(ДоговорКонтрагента.Владелец)
							...
							+ " - Срок действия договора УЖЕ истёк "+Формат(ДоговорКонтрагентаДатаОкончанияДействия,"ДФ=dd.MM.yyyy")+"г.!!!",СтатусСообщения.ОченьВажное);
						#КонецЕсли
				...
				ДоговорКонтрагентаСостояниеДоговора = ДоговорКонтрагента.СостояниеДоговора;	//+++ 29.11.2018 - 1 запрос Без Точки! а не 7 раз подряд с точкой
				Если  ДоговорКонтрагентаСостояниеДоговора <> справочники.СостояниеДокументов.НайтиПоКоду("1")  // подписан последний лист
					и ДоговорКонтрагентаСостояниеДоговора <> справочники.СостояниеДокументов.НайтиПоКоду("2") // Есть печать и подпись
					                                                                                   //"3") //  Есть копия факсимиле-скан - запрещен!
					и ДоговорКонтрагентаСостояниеДоговора <> справочники.СостояниеДокументов.НайтиПоКоду("4") //Оригинал передан юристам 
					
					и ДоговорКонтрагентаСостояниеДоговора <> справочники.СостояниеДокументов.НайтиПоКоду("12")//Копия (Филиал)
					и ДоговорКонтрагентаСостояниеДоговора <> справочники.СостояниеДокументов.НайтиПоКоду("13")//Временный договор!?
					и ДоговорКонтрагентаСостояниеДоговора <> справочники.СостояниеДокументов.НайтиПоКоду("14")//Внутренний договор
				Тогда
					#Если Клиент тогда
...
ЕСЛИ НЕ ТолькоТабЧасть ТОГДА //+++( 07.12.2011  игнорируем изменение реквизитов
	...
	Если Найти(Объект.Метаданные().ПолноеИмя(),"Документ.") Тогда
		ОбъектБДНомер = ОбъектБД.Номер;
		ОбъектНомер   = Объект.Номер; // 28.06.2018 - 1 раз запрос к базе
		Если НЕ ОбъектБДНомер = ОбъектНомер Тогда
			Если флОднойЗаписью тогда
				   текстЗаписиВТранзакции = текстЗаписиВТранзакции+"Номер: "+ОбъектБДНомер+">>"+ОбъектНомер+РазделительЗаписей1;
			иначе	   
				...
				ДобавитьИзменение(ТаблицаИзменений, "Изменен номер документа.", Объект.Метаданные(), Объект.Ссылка, "Был: "+ОбъектБДНомер+", стал: " +ОбъектНомер, НомерЗаписиВТранзакции);
...
////+++ 19.01.2015 -  08.10.2018 по оборотам - быстрее на сервере!
//Запрос.Текст="ВЫБРАТЬ ПЕРВЫЕ 1
//			 |	ЗаказыПокупателей.Регистратор КАК Регистратор
//			 |ИЗ
//			 |	РегистрНакопления.ЗаказыПокупателей.Обороты(&ЗаказПокупателяДата, , Регистратор,
//			 |					ЗаказПокупателя = &ЗаказПокупателя) КАК ЗаказыПокупателей
//			 |ГДЕ
...
//			 |		ИЛИ ЗаказыПокупателей.Регистратор ССЫЛКА Документ.ЧекККМ
//			 |		ИЛИ ЗаказыПокупателей.Регистратор ССЫЛКА Документ.ОтчетОРозничныхПродажах
//			 |	   )
//			 |";
//+++ 08.10.2018 анализ Взаимо-блокировок показывает, что нельзя использовать ЗаказыПокупателей.Обороты
 Запрос.Текст="ВЫБРАТЬ ПЕРВЫЕ 1
             |	ПродажиОбороты.Регистратор КАК Регистратор
             |ИЗ
             |	РегистрНакопления.Продажи.Обороты(&ЗаказПокупателяДата, , Регистратор,
			 |					ЗаказПокупателя = &ЗаказПокупателя) КАК ПродажиОбороты
             |ГДЕ
...
СделкаДатаОтгрузки = Сделка.ДатаОтгрузки;
СделкаДата = Сделка.Дата;
если СделкаДатаОтгрузки>'00010101' тогда //дата отгрузки ставится при проведении Реализации, иначе смотрим не более  - 7 дней ,иначе заказ закрывается (в Ярославле)
	Запрос.УстановитьПараметр("ЗаказПокупателяДата",НачалоДня(мин(СделкаДата, СделкаДатаОтгрузки))-7*86400 ); 	
иначе 
 	Запрос.УстановитьПараметр("ЗаказПокупателяДата",НачалоДня(СделкаДата) ); //18.01.2016 реализация или корректировка - не ранее недели от Заказа!
КонецЕсли;
 Если Сделка.ВводитьТолькоПеремещение тогда  //+++ 09.10.2018 - перемещение закрывает заказы для Филиалов или перемещение на складОТХ
Запрос.Текст="ВЫБРАТЬ ПЕРВЫЕ 1
             |	ВЫРАЗИТЬ(ЗаказыПокупателей.Регистратор КАК Документ.ПеремещениеТоваров) КАК Регистратор
             |ИЗ
             |	РегистрНакопления.ЗаказыПокупателей.Обороты(&ЗаказПокупателяДата, , Регистратор, ЗаказПокупателя = &ЗаказПокупателя) КАК ЗаказыПокупателей
             |ГДЕ
             |	(ЗаказыПокупателей.КоличествоРасход > 0
             |			ИЛИ ЗаказыПокупателей.КоличествоПриход < 0)
             |	И ЗаказыПокупателей.Регистратор ССЫЛКА Документ.ПеремещениеТоваров";
 КонецЕсли;
...
запрос1 = новый Запрос;
запрос1.Текст = "выбрать Ед.Вес из справочник.ЕдиницыИзмерения как Ед
				|где ед.Владелец = &Тов";
запрос1.УстановитьПараметр("Тов", Номенклатура);
рез1 = запрос1.Выполнить();
выб1 = рез1.Выбрать();
Если выб1.Следующий() тогда
	Вес=выб1.Вес;
КонецЕсли;
//+++)
...
Процедура ДвиженияРегистровГрузчикиИКладовщики(ДокументОбъект, ДельтаПеремещения=0.5, МножительПеремещения=1 )  Экспорт
	...
	//09.01.2019
	|ВЫРАЗИТЬ (А.ВесАксы*А.КТУ/А.КТУПоДокументу КАК ЧИСЛО(10,3)) ВесАксы,
	...
	|ТОГДА Вес * 1 иначе 0 конец ВесДиски,
	
//+++ 09.01.2019 ---  ВесАксы отдельно!
	|выбор КОГДА Номенклатура.ВидТовара=&Аксессуары
//+++ 27.08.2018 - по согласованию с Фроловым - крепеж (4 папки) тоже как Аксы и АКБ!
	|	ИЛИ	Номенклатура.Родитель в (&СписБолтыГайки)
	|ТОГДА Вес * 9 
//+++ 06.08.2018 - ещё в 2 раза, был коэффициент 2,5  стал  5 !
	...
	//+++( 27.08.2018
	СписБолтыГайки = новый СписокЗначений;
	СписБолтыГайки.Добавить(справочники.Номенклатура.НайтиПоКоду("0000702") );//Болты
	СписБолтыГайки.Добавить(справочники.Номенклатура.НайтиПоКоду("9190192") );//Вентили
	СписБолтыГайки.Добавить(справочники.Номенклатура.НайтиПоКоду("0000713") );//Гайки
	СписБолтыГайки.Добавить(справочники.Номенклатура.НайтиПоКоду("0000726") );//Кольца
	Запрос.УстановитьПараметр("СписБолтыГайки", СписБолтыГайки);
	//+++)	 
	...
	//при перемещении на ангарИ - ДельтаПеремещения=1 и все делится/2
		ДельтаДиски		   = 1   *2*ДельтаПеремещения*(2-МножительПеремещения);	//+++ 09.01.2019 - для перемещения = 0!
		ДельтаАКБАксессуары= 1   *2*ДельтаПеремещения; 	
		ДельтаИмпорт 	   = 0.2 *2*ДельтаПеремещения*(2-МножительПеремещения);	//+++ 09.01.2019 - для перемещения = 0!  иначе 1
        ДельтаВыходные= ?(ПризнакПереработки, 1, 0)*2*ДельтаПеремещения*МножительПеремещения;//09.01.2019    2х1х1=2 или 2х0.5х2=2  иначе 2х0.5х1 = 1;
	весПоГрузчикам=0;	
	Пока Выборка.Следующий() Цикл
			...
			Движ.ВесДиски		=(Выборка.ВесДиски*ДельтаДиски + Выборка.ВесАксы*ДельтаАКБАксессуары); //09.01.2019 - разделено!
			...
			Движ.ВесПеремещения = ?(ТипЗнч(ДокументОбъект.Ссылка)=Тип("ДокументСсылка.ПеремещениеТоваров")
			                         , Выборка.ВесНетто * ДельтаПеремещения *МножительПеремещения  // +1x1 для Ангар-И, 0.5х2 для Остальных, 1.5 для перемещения на Товары в пути...
									 , 0); // для реализаций и поступлений - 0
			
			Движ.Вес		=Движ.ВесНетто+Движ.ВесДиски+Движ.ВесИмпорт+Движ.ВесВыходные+Движ.ВесПеремещения;
			
			весПоГрузчикам = весПоГрузчикам + Движ.Вес;
	КонецЦикла;	
	
	////------------ тест. разности в весе ----------------
	#Если Клиент тогда
		весРеальный    = ДокументОбъект.Товары.Итог("Вес");
		  сообщить("Средний коэффициент по весу для грузчиков: " + формат( ?(весРеальный=0,0, весПоГрузчикам / весРеальный), "ЧДЦ=1") ); 
	#КонецЕсли
...
функция ПолучитьСписокПапокГрузШин() 
	
	СписокПапокГрузШин=НОВЫЙ СписокЗначений;
	СписокПапокГрузШин.Добавить( справочники.Номенклатура.НайтиПоКоду("0001748") ); //Грузовые
	СписокПапокГрузШин.Добавить( справочники.Номенклатура.НайтиПоКоду("9178094") );    // Востановленные KaMaRetread		
	
	СписокПапокГрузШин.Добавить( справочники.Номенклатура.НайтиПоКоду("0080632") ); //Спецтехника
	СписокПапокГрузШин.Добавить( справочники.Номенклатура.НайтиПоКоду("9109641") ); 	// Шины для вилочных погрузчиков
	СписокПапокГрузШин.Добавить( справочники.Номенклатура.НайтиПоКоду("9109642") );    // Диагон. шины для самосвалов и погрузчиков
	СписокПапокГрузШин.Добавить( справочники.Номенклатура.НайтиПоКоду("9109643") );    // Шины для дор.-строит. техники
	СписокПапокГрузШин.Добавить( справочники.Номенклатура.НайтиПоКоду("9109644") );    // Шины радиальные для Д-С Техники
			
	СписокПапокГрузШин.Добавить( справочники.Номенклатура.НайтиПоКоду("0001750") ); //Сельскохозяйственные
	СписокПапокГрузШин.Добавить( справочники.Номенклатура.НайтиПоКоду("0080631") ); //Грузовые и спецтехника
		
	возврат СписокПапокГрузШин;
КонецФункции

//массив товаров!
функция ПолучитьМасГрузШин() экспорт
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	...
	|ГДЕ
	|	Номенклатура.Родитель В(&списГрузШин)";
	Запрос.УстановитьПараметр("списГрузШин", ПолучитьСписокПапокГрузШин() );
	Результат = Запрос.Выполнить();
	табл = результат.Выгрузить();
	масГрузШин = табл.ВыгрузитьКолонку("Номенклатура");
	возврат масГрузШин;
КонецФункции

// ПоказатьОбъем() -> аналогична РаботаСДиалогами.ПоказатьКодАртикул()
//
...
Процедура ПоказатьОбъем(КолонкиТабПоля, ЯчейкиСтрокиТабПоля, Номенклатура) Экспорт
	...
	КонецЕсли;
КонецПроцедуры	
Процедура ПоказатьОбъемШПодвал(КолонкиТабПоля, ТЧТовары, ТочностьРез=3, ИмяКоличество="Количество", масГрузШин=неопределено) Экспорт
		Если КолонкиТабПоля.Объем.Видимость Тогда
		итогОбъем = 0;
		Для каждого стр1 из ТЧТовары Цикл
			Коэф=1;
			Если масГрузШин.Найти(стр1.Номенклатура)<>Неопределено тогда
				Коэф=1.3;
			КонецЕсли;	
		итогОбъем = итогОбъем + стр1[ИмяКоличество] * Коэф * ВычислитьОбъемНоменклатуры(стр1.Номенклатура); 
		КонецЦикла;
		КолонкиТабПоля.Объем.ТекстПодвала = формат(окр(итогОбъем,ТочностьРез),"ЧДЦ="+строка(ТочностьРез)+"; ЧН=-");
	КонецЕсли;
КонецПроцедуры

//06.02.2019 - для Заявки на возврат товаров
процедура ПоказатьВес(КолонкиТабПоля, ЯчейкиСтрокиТабПоля, Номенклатура,ТочностьРез=3, ИмяКол="Количество") Экспорт
	Вес = 0;
	Если КолонкиТабПоля.Вес.Видимость Тогда
		Количество = ЯчейкиСтрокиТабПоля[ИмяКол].Значение;
		Если Номенклатура <> Неопределено и Количество>0 Тогда
 			Вес = Количество * ПолучитьВесНетто(Номенклатура);
		КонецЕсли;
	ЯчейкиСтрокиТабПоля.Вес.УстановитьТекст( Формат(Вес, "ЧДЦ="++строка(ТочностьРез)+"; ЧН=-") );	// до 0.001 кг.
	КонецЕсли;	
КонецПроцедуры
процедура ПоказатьВесПодвал(мКолонкиТовары, ТЧТовары,ТочностьРез=3, ИмяКоличество="Количество") Экспорт
	Если мКолонкиТовары.Вес.Видимость Тогда
		итогВес = 0;
		Для каждого стр1 из ТЧТовары Цикл
		итогВес = итогВес + стр1[ИмяКоличество] * ПолучитьВесНетто(стр1.Номенклатура); 
		КонецЦикла;
		мКолонкиТовары.Вес.ТекстПодвала = формат(окр(итогВес,ТочностьРез),"ЧДЦ="+строка(ТочностьРез)+"; ЧН=-");
	КонецЕсли;
КонецПроцедуры	

//окс - Объемный коэффициент скругления ( процентКвадратности )
функция получитьОКС( процентКвадратности = 0 )
	пи4 = 0.7854; // не может быть менее круглой! пи/4 = 0,7853981634...
	максКоэф = (1/пи4-1); //                  максКоэф = 0,2732365674...
	кк = 1 + максКоэф*процентКвадратности/100;  //Коэф.Квадратности: 72% -> 1,19673
	окс = Окр(пи4*кк,3); // 0,939912 >> 0,940   
	возврат окс;
КонецФункции
...
Функция ВычислитьОбъемНоменклатуры(Номенклатура, ТочностьРез=6) экспорт
	...
	Если Номенклатура = неопределено 
		ИЛИ НЕ ЗначениеЗаполнено(Номенклатура) тогда // пустая ссылка!
		Возврат Рез;
	КонецЕсли;	
	
	Если ТипЗнч(Номенклатура)=Тип("СправочникСсылка.Номенклатура") тогда
		
		//+++ 06.02.2019 - сразу объем по Ед.изм, и только если там 0, то по типоразмеру
...
попытка
		...
		d = Диаметр;	//посадочный диаметр
		Ширина = ?(тр.Ширина="", 0, число(тр.Ширина));
		Ш = Ширина;//  Ш – ширина диска/шины 
		// Высота профиля шины
...
//--------------добавлены проверки и сообщения на 0---------------------------
	...
	иначеЕсли видТовара = видШины тогда
		...
		процентКвадратности = 72; // 72%  окс=0.940
		окс = получитьОКС(процентКвадратности); //Объемный коэффициент скругления от 0.7854 до 1  
		рез = D*D*(Ш*мм)*окс;
...
функция ПолучитьТипДокументаПоБукве( ВидПФ )
	...
	СоответствиеВидаТипуДок.Вставить("V", "ВозвратТоваровПоставщику");
	СоответствиеВидаТипуДок.Вставить("M", "РеализацияТоваровУслуг"); //"M" - eng, МХ-1
	СоответствиеВидаТипуДок.Вставить("N", "ВозвратТоваровОтПокупателя"); //"Т" - eng, МХ-3
 //в русской раскладке
...
//21.07.2017
 функция ПолучитьВидПечатнойФормы( ВидПФ ) экспорт
	ВидПечатнойФормы = Перечисления.ВидыПечатныхФорм.ПустаяСсылка();
		...
		ИначеЕсли ВидПФ = "M" Тогда
			ВидПечатнойФормы = Перечисления.ВидыПечатныхФорм.МХ1;
		ИначеЕсли ВидПФ = "N" Тогда
			ВидПечатнойФормы = Перечисления.ВидыПечатныхФорм.МХ3;
...
функция ПолучитьРуководителя(Пользователь=неопределено) Экспорт
	...
	Иначе
		...
		Если Пользователь.НаправлениеПродаж.Пустая() тогда
			...
			рук = справочники.пользователи.НайтиПоКоду("Бондаренко Е.Д. (снабжение)");
		Иначе	
...
услПредоплаты = // константы.СогласованиеПоСкидкамКлиентов.Получить() и  //10.09.2015 ВСЕГДА по договору
 ...
 //+++ 26.09.2017 - нужна колонка:  табЗнач1.колонки.Удалить("ЦенаСоСкидкойПредоплаты");
  ...
  // Код, Номенклатура, Цена, 
  // доп.поля: Приоритет, ЦенаПоТипуЦен, СкидкаПредоплаты
   товКод0 = ""; Приоритет0=99000999;  i=0; //+++ 17.07.2017  дубли строк! приоритет до 999 999
...
//24.12.2015--------------удаление нулевых цен-------------------
	...
	ном = 0;
  	для каждого Стр1 Из МассивНулевыхЦен Цикл
		ном = ном + 1;
  		#Если Клиент тогда
		сообщить(строка(ном)+") Удаляется товар "+Стр1.Код+" с нулевой ценой.");
		#КонецЕсли
...
//------------------------прослушка всех---------------------------------
   ...
   Рез.Сортировать("ДеньВремя Возр");
		...
		Для каждого стр из Рез Цикл
			Если стр.BillSec>0 тогда      //если продолжительность > 0
	...
	функция получитьВнутреннийТелефон(Объект1, КодВида="38840")  экспорт
...
Процедура ЗапуститьОбработкуНаполеон(РежимЗапуска) Экспорт
	...
	Если РежимЗапуска = 1 тогда
		...
		ТекСсылка=Справочники.ВнешниеОбработки.НайтиПоКоду("630");//Выгрузка наполеон 
...
Запрос = Новый Запрос;
	Запрос.Текст = 
		...
		|	ЗаявкаНаВозврат.Ссылка.Ответственный КАК СсылкаОтветственный,
		|	ЗаявкаНаВозврат.Ссылка.Ответственный.НаправлениеПродаж КАК СсылкаОтветственныйНаправлениеПродаж,
		|	ЗаявкаНаВозврат.Ссылка.Дата КАК СсылкаДата
		|ПОМЕСТИТЬ ВТ_ЗаявкиНаВозврат
		|ИЗ
		|	Документ.ЗаявкаНаВозвратТоваров.Товары КАК ЗаявкаНаВозврат
		|ГДЕ
		|	НЕ ЗаявкаНаВозврат.Реализация В(ВЫБРАТЬ ВТ.ДокументПартии ИЗ ВТ_Возвраты КАК ВТ)
		|	И ЗаявкаНаВозврат.Ссылка.Дата >= &ДатаНач
		|	И ЗаявкаНаВозврат.Ссылка.Проведен = ИСТИНА
		|	И ЗаявкаНаВозврат.Ссылка.ПометкаУдаления = ЛОЖЬ
		|	И ЗаявкаНаВозврат.Ссылка.нетДокументов = ИСТИНА
		|;
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|	ЗаявкаНаВозвратТоваровТовары.СсылкаОтветственный как Ответственный,
		
		|	РАЗНОСТЬДАТ(ЗаявкаНаВозвратТоваровТовары.СсылкаДата, &ДатаТек, ДЕНЬ) КАК ДнейСоздания,
		|	ЗадачиПользователя.Ссылка КАК Задача,
		|	ВЫБОР
		|		КОГДА РАЗНОСТЬДАТ(ЗаявкаНаВозвратТоваровТовары.СсылкаДата, &ДатаТек, ДЕНЬ) > 14
		|			ТОГДА НаправленияПродаж.Согласование
		|		ИНАЧЕ ВЫБОР
		|				КОГДА РАЗНОСТЬДАТ(ЗаявкаНаВозвратТоваровТовары.СсылкаДата, &ДатаТек, ДЕНЬ) > 7
		|					ТОГДА НаправленияПродаж.Руководитель
		|				ИНАЧЕ ЗаявкаНаВозвратТоваровТовары.СсылкаОтветственный
		|			КОНЕЦ
		...
		|	КОНЕЦ КАК Менеджер
		|ИЗ
		|	ВТ_ЗаявкиНаВозврат КАК ЗаявкаНаВозвратТоваровТовары
		
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НаправленияПродаж КАК НаправленияПродаж
		|		ПО ЗаявкаНаВозвратТоваровТовары.СсылкаОтветственныйНаправлениеПродаж = НаправленияПродаж.Ссылка
		...
		|ГДЕ
		|	ЕСТЬNULL(ЗадачиПользователя.ЗадачаПоПоискуДокументов, ИСТИНА) = ИСТИНА
		...
		|;
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТ_Возвраты;
		|УНИЧТОЖИТЬ ВТ_ЗаявкиНаВозврат;
		|";
    Запрос.УстановитьПараметр( "ДатаТек", ТекущаяДата());
	Запрос.УстановитьПараметр( "ДатаНач", ДобавитьМесяц(НачалоДня(ТекущаяДата()),-12) ); //03.04.2019 за ГОД >> 3 мес задаче...!
	Результат = Запрос.Выполнить();
	ВыборкаДетальныеЗаписи = Результат.Выбрать();

	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		
		ЗаголовокЗадачи = ?(ВыборкаДетальныеЗаписи.ДнейСоздания > 7,  
			"По менеджеру: " + ВыборкаДетальныеЗаписи.Ответственный + " отсутствуют печатные формы по заявке на возврат", 
			"Отсутствуют печатные формы по заявке на возврат"); 
		//03.04.2019 
		Если УжеЕщёЕстьТакаяЗадачаМенеджеру(ЗаголовокЗадачи, ВыборкаДетальныеЗаписи) тогда 
			Продолжить; 
		КонецЕсли;
			...
			Задача.Исполнитель = ВыборкаДетальныеЗаписи.Менеджер;
			задача.НаСогласование = Ложь; 
			задача.Наименование = ЗаголовокЗадачи;
			задача.Описание =   "Задача просрочена " 
			+?(ВыборкаДетальныеЗаписи.ДнейСоздания > 14 , "руководителем: "+ВыборкаДетальныеЗаписи.Руководитель , 
						?(ВыборкаДетальныеЗаписи.ДнейСоздания > 7, "менеджером: " + ВыборкаДетальныеЗаписи.Ответственный, ""
			  )           ) + "
			|Предоставьте документы в бухгалтерию для проведения возврата 
			|по документу: " + Строка(ВыборкаДетальныеЗаписи.Ссылка);								
...
//+++ 03.04.2019
функция УжеЕщёЕстьТакаяЗадачаМенеджеру(Заголовок, выборка1)
	 Запрос = Новый Запрос;
	 Запрос.Текст = "ВЫБРАТЬ
	 ...
	 |ГДЕ
	 |	ЗадачиПользователя.Наименование = &Заголовок
	 |	И ЗадачиПользователя.Объект = &Объект
	 |	И ЗадачиПользователя.Исполнитель = &Менеджер
	//// КАЖДЫЙ ДЕНЬ - НОВУЮ задачу ?
	// |	И ЗадачиПользователя.СрокИсполнения = &КонецСегоДня
	 
	 |	И ЗадачиПользователя.ЗадачаПоПоискуДокументов=Истина
	 |	И ЗадачиПользователя.Оповещение = истина";
	 Запрос.УстановитьПараметр("Заголовок", Заголовок);
	 Запрос.УстановитьПараметр("Объект", выборка1.Ссылка);
	 Запрос.УстановитьПараметр("Менеджер", выборка1.Менеджер);
...
процедура РаспределитьЗаказНаМаршрут(Заказ,ДатаЗаказа,РегионЭтоГород = Неопределено,сАдрес = неопределено, ВернутьМаршрут = неопределено,Подразделение = неопределено) Экспорт
	...
	Попытка
		сАдресКоординаты = СокрЛП(сАдрес.Координаты);//27.03.2019 меньше цифр стало!
		поз = Найти(сАдресКоординаты,",");  //yandex-координаты - сначала "Долгота,Широта"
		если сАдресКоординаты="" или поз = 0 тогда
			#Если Клиент тогда
			сообщить("Не заполнены Координаты в Адресе Заказа!", СтатусСообщения.Внимание);
			#КонецЕсли	
			Возврат;
		КонецЕсли;
	    ДолготаОсн = Число( Лев(сАдресКоординаты,поз-1) );
  	    ШиротаОсн  = Число(Прав(сАдресКоординаты,стрДлина(сАдресКоординаты)-поз)); //не посередине!
	Исключение
		#Если Клиент тогда
			сообщить("Не правильное значение в Адресе! Поле Координаты: '"+сАдресКоординаты+"'  - должно быть два числа, разделенных одной запятой: 'Широта,Долгота'", СтатусСообщения.Внимание);
		#КонецЕсли	
		Возврат;
		ШиротаОсн  = 0;
	    ДолготаОсн = 0;
	...
	Результат = Запрос.Выполнить();
	Рез = Результат.Выгрузить();
	Если Результат.Пустой() Тогда //Ничего не нашли, попробуем по другому
	...
	//+++ 11.08.2017 - НЕТ такой ТЧ:  Справочник.НовМаршруты.Зоны !!!
		...
		Для Индекс = -РезКоординаты.Количество() По -1 Цикл
		 	//здесь Google-координаты!  сначала "широта, долгота" (через ,_)
			уд = РезКоординаты.Получить(-Индекс-1);
			
			удКоордината1 = сокрЛП(уд.Координата1); //27.03.2019 меньше цифр стало!
			поз1 = Найти(удКоордината1,",");
			Широта1  = Число(Лев(удКоордината1,поз1-1));
			Долгота1 = Число(Прав(удКоордината1,стрДлина(удКоордината1)-поз1));//не посередине!
			
			удКоордината2 = сокрЛП(уд.Координата2);  //27.03.2019 меньше цифр стало!
			поз2 = Найти(удКоордината2,",");
			Широта2  = Число(Лев(удКоордината2,поз2-1));
			Долгота2 = Число(Прав(удКоордината2,стрДлина(удКоордината2)-поз2)); //не посередине!
			
			//если за границами Прямоугольника - удаляем такой адрес
			Если ШиротаОсн > Широта1 или ШиротаОсн < Широта2 
			или ДолготаОсн < Долгота1 или ДолготаОсн > Долгота2 Тогда
...
//====================================================================	
	Если НЕ МаршрутНайден Тогда
		Маршрут = Справочники.НовМаршруты.НайтиПоКоду("");
		датаМ = Дата(1,1,1);
	Иначе	
		НомерДня = Новый Соответствие; //соответсвиеДней >>НомерДня
		НомерДня.Вставить(Перечисления.ДниНедели.Понедельник,1);
		НомерДня.Вставить(Перечисления.ДниНедели.Вторник,2);
		НомерДня.Вставить(Перечисления.ДниНедели.Среда,  3);
		НомерДня.Вставить(Перечисления.ДниНедели.Четверг,4);
		НомерДня.Вставить(Перечисления.ДниНедели.Пятница,5);
		НомерДня.Вставить(Перечисления.ДниНедели.Суббота,6);
		НомерДня.Вставить(Перечисления.ДниНедели.Воскресенье,7);
		
		табМаршрутИДата = Новый ТаблицаЗначений;
		табМаршрутИДата.Колонки.Добавить("Маршрут");
		табМаршрутИДата.Колонки.Добавить("Приоритет");
		табМаршрутИДата.Колонки.Добавить("Дата");
		...
		Для каждого стрМаршрут из Поиск Цикл
			...
			Для каждого стрДень из графикРаботыТекущий Цикл
				Если НЕ стрДень.Используется Тогда
					Продолжить;
				КонецЕсли;
				
				ТекДень      = ДеньНедели(ДатаЗаказа);//число
				ДеньМаршрута = НомерДня.Получить(стрДень.ДеньНедели);
				  пДень = Перечисления.Периодичность.День;
				  
				Если ДеньМаршрута > ТекДень Тогда
					стрМаршрутИДата = табМаршрутИДата.Добавить();
					стрМаршрутИДата.Маршрут   = стрМаршрут.ссылка;
					стрМаршрутИДата.Приоритет = стрМаршрут.Приоритет; //1
					стрМаршрутИДата.Дата      = ДобавитьИнтервал(ДатаЗаказа, пДень, ДеньМаршрута - ТекДень);
					
				ИначеЕсли  ДеньМаршрута < ТекДень Тогда
					стрМаршрутИДата = табМаршрутИДата.Добавить();
					стрМаршрутИДата.Маршрут   = стрМаршрут.ссылка;
					стрМаршрутИДата.Приоритет = стрМаршрут.Приоритет; //1
					стрМаршрутИДата.Дата      = ДобавитьИнтервал(ДатаЗаказа, пДень, 7 + ДеньМаршрута - ТекДень);
					
				ИначеЕсли ДеньМаршрута = ТекДень Тогда  //текущий день!
					
					Если стрДень.ВремяНачала  = Дата(1,1,1) Тогда //без ограничения
						стрМаршрутИДата = табМаршрутИДата.Добавить();
						стрМаршрутИДата.Маршрут   = стрМаршрут.ссылка;
						стрМаршрутИДата.Приоритет = стрМаршрут.Приоритет;//1
						стрМаршрутИДата.Дата      = ДатаЗаказа;
					Иначе //отсечка по времени
						Если ДатаЗаказа < НачалоДня(ДатаЗаказа)+Час(стрДень.ВремяНачала)*3600 + Минута(стрДень.ВремяНачала)*60 Тогда
							стрМаршрутИДата = табМаршрутИДата.Добавить();
							стрМаршрутИДата.Маршрут   = стрМаршрут.ссылка;
							стрМаршрутИДата.Приоритет = стрМаршрут.Приоритет;
							стрМаршрутИДата.Дата      = ДатаЗаказа;
						иначе // на следующий неделю
							стрМаршрутИДата = табМаршрутИДата.Добавить();
							стрМаршрутИДата.Маршрут   = стрМаршрут.ссылка;
							стрМаршрутИДата.Приоритет = стрМаршрут.Приоритет;
							стрМаршрутИДата.Дата      = ДобавитьИнтервал(ДатаЗаказа, пДень, 7);
						КонецЕсли;	
					КонецЕсли;	
				КонецЕсли;
		...
		табМаршрутИДата.Сортировать("Приоритет Возр, Дата Возр"); //Приоритет одинаковый = 1, поэтому ближайший по дате Маршрута!
		Если табМаршрутИДата.Количество() > 0 Тогда
			Маршрут = табМаршрутИДата[0].Маршрут;
			ДатаМ   = табМаршрутИДата[0].Дата;
		Иначе
			Маршрут = Справочники.НовМаршруты.НайтиПоКоду("");
			датаМ = Дата(1,1,1);
		КонецЕсли;
	КонецЕсли;//МаршрутНайден		
	
	Если ВернутьМаршрут = Неопределено Тогда  // ЗаказыМаршруты
		...
		Если набор.Количество()=0 тогда
			...
			нов.Маршрут 		= Маршрут;
		иначе
			Если Маршрут <> Набор[0].Маршрут Тогда  //меняем маршрут!
				Набор[0].Дата    = ДатаМ;
...
//+++ 11.03.2019 +++ Адрес доставки 02, 03... 35
функция ПолучитьАдресДоставкиПервыйПопавшийся(Контрагент)
РезСтруктура =новый Структура("Вид, Адрес, Регион, Город, Улица, Дом, Координаты", 
                                 "","","","","");
Запрос = Новый Запрос;
Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
       ...
       |	КонтактнаяИнформация.Объект = &Контрагент
       |	И КонтактнаяИнформация.Тип = &Тип
       |	И КонтактнаяИнформация.Вид.Наименование ПОДОБНО &АдресДоставки
       |
       |УПОРЯДОЧИТЬ ПО
       |	КонтактнаяИнформация.Вид.Наименование
       |АВТОУПОРЯДОЧИВАНИЕ";
		Запрос.УстановитьПараметр("АдресДоставки", "Адрес доставки %"); //02... 35
		Запрос.УстановитьПараметр("Контрагент", Контрагент);
		Запрос.УстановитьПараметр("Тип", перечисления.ТипыКонтактнойИнформации.Адрес);
...
функция НайтиАдресПоУмолчанию(Контрагент)
	...
	Вид = справочники.ВидыКонтактнойИнформации.НайтиПоКоду("00021"); //+++ 16.10.2017  "Адрес доставки" 
	сАдрес = ПолучитьКонтактнуюИнформацию(Контрагент, Вид);
	
	Если СокрЛП(сАдрес.Адрес)="" Тогда //+++ 11.03.2019 Адрес доставки 02, 03... 35  по старой задаче от Горелова
		сАдрес = ПолучитьАдресДоставкиПервыйПопавшийся(Контрагент);
	КонецЕсли;
	
	Если СокрЛП(сАдрес.Адрес)="" Тогда
		Вид = справочники.ВидыКонтактнойИнформации.ФактАдресКонтрагента; // был сразу Юр!? >> факт.адрес 11.03.2019
		сАдрес = ПолучитьКонтактнуюИнформацию(Контрагент, Вид);
		
		Если СокрЛП(сАдрес.Адрес)="" Тогда
			Вид = справочники.ВидыКонтактнойИнформации.ЮрАдресКонтрагента; //Юр.адрес
			сАдрес = ПолучитьКонтактнуюИнформацию(Контрагент, Вид);
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьБонусы(ПолучитьТаблицу = ложь, начПериода = неопределено, конПериода = неопределено, СписНом = неопределено) экспорт
  ...
  Если СписНом<>неопределено тогда
	ТекстЗ = СтрЗаменить(ТекстЗ,"88 = 88"	, "НоменклатураСпр.Ссылка В(&СписНом)");
 КонецЕсли;

  Если ПолучитьТаблицу тогда
	  ТаблицаБонусов = ПолучитьНоменклатуруСБонусами(ТабПравил, СписНом);
  Иначе
...
Функция ПолучитьНоменклатуруСБонусами(ТабПравил, СписНом = неопределено)
	Запрос = новый Запрос;
	...
	Запрос.Текст="ВЫБРАТЬ
				 ...
				 Если СписНом<>Неопределено тогда
				 	Запрос.Текст=СтрЗаменить(Запрос.Текст,"88 = 88","НоменклатураСпр.Ссылка В(&СписНом)");
					Запрос.УстановитьПараметр("СписНом",СписНом);
				 КонецЕсли;
				 Возврат Запрос.Выполнить().Выгрузить();
...
//***2018.06.18 Задача № 49314
Процедура ГонконгОстаткиДенежныхСредствДействие() Экспорт

	глТекущийПользователь = ПараметрыСеанса.ТекущийПользователь;
	Если РольДоступна("ПравоЗавершенияРаботыПользователей") //ИТ
		ИЛИ СокрЛП(глТекущийПользователь.Код) = "Малышев Егор (WG)"
		ИЛИ СокрЛП(глТекущийПользователь.Код) = "Никитин (WG)"
		Тогда
		ОстаткиДенежныхСредств = Отчеты.ОстаткиДенежныхСредств.Создать();
		ОстаткиДенежныхСредств.ЗаполнитьНачальныеНастройки();
		ФормаОтчета = ОстаткиДенежныхСредств.ПолучитьОсновнуюФорму();
		ФормаОтчета.Открыть();
	Иначе
...
Процедура ГонконгДвиженияДенежныхСредствДействие() Экспорт
	...
	Если РольДоступна("ПравоЗавершенияРаботыПользователей") //ИТ
		...
		Тогда
			ИмяФайла = ПолучитьИмяВременногоФайла();
			файл = новый файл(ИмяФайла);
			обр = справочники.ВнешниеОбработки.НайтиПоКоду("723"); 
...
//29.06.18 Шарафутдинов
//Функция ОтправлятьЭД
//вход: Контрагент - ссылка на справочник Контрагенты
//Возвращает истина , если разрешено использование ЭДО и отправка УПД для данного контрагента
функция ОтправлятьЭД (Контрагент) экспорт
	
	Если Константы.ИспользоватьЭДО.Получить() тогда
		...
		Запрос.УстановитьПараметр("Свойство", ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоКоду("90272")); //ДиадокФормироватьУПДКонтрагент
...
//17.09.18 Смирнов
Процедура ОткрытьОтчетВознаграждениеФормулыЗаБезналы() экспорт
	ТекСсылка=Справочники.ВнешниеОбработки.НайтиПоКоду("738");
	ИмяФайла = ПолучитьИмяВременногоФайла(); 
	ДвоичныеДанные = ТекСсылка.ХранилищеВнешнейОбработки.Получить(); 
	ДвоичныеДанные.Записать(ИмяФайла); 
	Если ТекСсылка.ВидОбработки = Перечисления.ВидыДополнительныхВнешнихОбработок.Отчет Тогда 
...
Процедура ОткрытьОбработкуРаспределенияДС() экспорт
	ТекСсылка=Справочники.ВнешниеОбработки.НайтиПоКоду("470");
	ИмяФайла = ПолучитьИмяВременногоФайла(); 
	ДвоичныеДанные = ТекСсылка.ХранилищеВнешнейОбработки.Получить(); 
	ДвоичныеДанные.Записать(ИмяФайла); 
	Если ТекСсылка.ВидОбработки = Перечисления.ВидыДополнительныхВнешнихОбработок.Отчет Тогда 
...
//11-15.10.2018 --- вспомогательная функция ----
//Код - Код подразделения "00112"....
//ДеньЗаказа    - тип Дата, понедельник, 11-00 = '01.01.0001 00:00:00' + 0*86400 + 11*3600
//ДеньРазгрузки - тип ЧИСЛО, Вторник, 11-00     = + 1*86400 + 9*3600
//Сезон - Истина - есть отгрузки в субботу!
//
функция ПолучитьТаблРазгрузкиВФилиалах(Сезон=Истина)
	день0 = '00010101';  час = 3600; день=86400;
	табл = новый ТаблицаЗначений;
	табл.Колонки.Добавить("КодФилиала");
	табл.Колонки.Добавить("ДеньЗаказа");   // тип дата
	табл.Колонки.Добавить("ДеньРазгрузки");// тип число (секунд)
	
	для дн=1 по 9 цикл // отгрузка в понедельник/вторник!
		
	Если Сезон и дн=7 тогда //в воскресенье НЕТ отгрузок!
		продолжить;
	Иначе
		Если дн=6 или дн=7 тогда // НЕ ГРУЗЯТ в выходные!?
			продолжить;
		КонецЕсли;	
	КонецЕсли;	
	
// для правильного сложения дата + секунды -> понедельник: дн=1, ii=0
		ii = дн-1; 
//---------------- Питер --------------
	КодФилиала = "00112";    //5 раз
	Если дн=6 тогда впути=2;
	иначеЕсли (НЕ Сезон и дн=5) тогда впути=3;//пятница - прибытие не в сезон! в следующий понедельник
	иначе впути=1; 
	КонецЕсли;
	стр1 = табл.Добавить(); 
	стр1.КодФилиала		= КодФилиала; 
	стр1.ДеньЗаказа		= день0 + ii*день + 11*час;
	стр1.ДеньРазгрузки	= (ii+впути)*день + 9*час;
	
	стр1 = табл.Добавить();  //2-я машина
	стр1.КодФилиала		= КодФилиала; 
	стр1.ДеньЗаказа		= день0 + ii*день + 13*час;
	стр1.ДеньРазгрузки	= (ii+впути)*день + 15*час;
	
//---------------- РнД --------------
	КодФилиала = "00106";   //2 раза 
	Если (не Сезон и (дн=1 или дн=5 или дн=8) )
		или (Сезон и (дн=1 или дн=2 или дн=5 или дн=6 или дн=8) )тогда
		
	Если дн=1 или дн=8 тогда впути=2; // только с понедельника - прибытие в среду!
	иначе впути=3;  //обычно 3 
	КонецЕсли;	
		
	стр1 = табл.Добавить(); 
	стр1.КодФилиала		= КодФилиала; 
	стр1.ДеньЗаказа		= день0 + ii*день + 9*час;
	стр1.ДеньРазгрузки	= (ii+впути)*день + 13*час;
	КонецЕсли;
	
//---------------- Ект --------------
	КодФилиала = "00138";   //2 раза 
	Если (не Сезон и (дн=1 или дн=4 или дн=8))
		или (Сезон и (дн=1 или дн=2 или дн=4 или дн=5 или дн=8)) тогда
		
	Если дн=4 или дн=5 тогда впути=4;  //пятница - прибытие в следующий ВТОРНИК	
	иначе впути=3;  
	КонецЕсли;
	...
	стр1.КодФилиала 	= КодФилиала; 
	стр1.ДеньЗаказа		= день0 + ii*день + 9*час;
	стр1.ДеньРазгрузки	= (ii+впути)*день + 14*час;
	КонецЕсли;
	
//---------------- Тл --------------
	КодФилиала = "00172";   //1 раз во втроник или во вторник через неделю 
	Если (НЕ Сезон и (дн=2 или дн=9))
		или (Сезон и (дн=2 или дн=4 или дн=9)) тогда 
	впути=?(дн=4, 1, 2); //четверг -прибытие и отгрузка в пятницу	 

	стр1 = табл.Добавить(); 
	стр1.КодФилиала 	= КодФилиала; 
	стр1.ДеньЗаказа    = день0 + ii*день + 9*час;
	стр1.ДеньРазгрузки = (ii+впути)*день + ?(дн=4,17,13)*час;
	КонецЕсли;
	
//---------------Москва - на следующий день!--------------
	КодФилиала = "00133";    //5 раз
	Если НЕ Сезон и дн=5 тогда впути=3;
	иначеЕсли Сезон и дн=6 тогда впути=2;
	иначе впути= 1; //пятница - прибытие в следующий понедельник	 
	КонецЕсли;	
	стр1 = табл.Добавить(); 
	стр1.КодФилиала		= КодФилиала; 
	стр1.ДеньЗаказа		= день0 + ii*день + 16*час;  //до 4х все заказы!
	стр1.ДеньРазгрузки	= (ii+впути)*день + 9*час;   // утром уже в москва
...
//11.10.2018 --- вспомогательная функция ----
функция найтиБлижайщуюДатуРазгрузкиНаФилиале(ДатаЗаказа, Подразделение)
	
	Если Подразделение.ОбособленноеПодразделение = ЛОЖЬ тогда  // это Ярославль!
		ДатаРазгрузки = НачалоДня(ДатаЗаказа)+18*3600; //СРАЗУ всё есть... но на складе отгружают до 18-00 
		возврат ДатаРазгрузки;
	КонецЕсли;
	
	табл1 = ПолучитьТаблРазгрузкиВФилиалах();
	ДатаРазгрузки = '00010101'; // пустая дата!	
	ДеньЗаказа = ДатаРазгрузки + (ДатаЗаказа - НачалоНедели(ДатаЗаказа)); // понедельник - 0 день
	КодФилиала = Подразделение.Код;
		
	для каждого стр1 из табл1 цикл
		Если стр1.КодФилиала = КодФилиала и ДеньЗаказа < стр1.ДеньЗаказа тогда
			ДатаРазгрузки = НачалоНедели(ДатаЗаказа) + стр1.ДеньРазгрузки;
			прервать; // первый 
		КонецЕсли;
	КонецЦикла;	
	
	Если ДатаРазгрузки = '00010101' тогда // не нашли ни одной отгрузки... только для новых/старых филиалов!?
		ДатаРазгрузки = ДобавитьМесяц(ДатаЗаказа, 6); // через пол-года !?
	КонецЕсли;
...
//11.10.2018 --- Задача № 56199 учет графика:
// Подразделение - справочник подразделений
//	датаЗаказа   - текущая или дата заказа
//Если датаЗаказа < ближайшей даты-времени Приема заказов в Ярославле
//то возвращает: 
// ДатуЗаказаИзФилиала = дата приема + время выезда из Ярославля + время в пути + время разгрузки в филиале
// т.е. сокращаем число параметров до 2х: 
// ДатуЗаказаИзФилиала = Дата перемещения на Филиал + числоДнейВПути + Время Разгрузки в Филиале
//
// для web-сервиса:  ServiceTerminal - GetDatesOfShipmentFromYarAndFilial()
//
функция ПолучитьДатуРазгрузкиВФилиале( Подразделение,  ДатаЗаказа='00010101' ) Экспорт
	
	Если НЕ ЗначениеЗаполнено(ДатаЗаказа) тогда
		ДатаЗаказа = ТекущаяДата();
	КонецЕсли;	
	
	ДатуЗаказаИзФилиала = найтиБлижайщуюДатуРазгрузкиНаФилиале(ДатаЗаказа, Подразделение);
	
	возврат ДатуЗаказаИзФилиала;
	
КонецФункции	

//+++ 23.10.2018 +++ 
функция НельзяОбъединятьНеПодтвержденныйЗаказВКоторомЕстьТоварыСтопЛиста( Зак1 , РазрешенныйЗаказ=ЛОЖЬ) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	ЗаказПокупателяТовары.Номенклатура,
	|	НоменклатураИмпорт.Номенклатура КАК Номенклатура1
	|ИЗ
	|	Документ.ЗаказПокупателя.Товары КАК ЗаказПокупателяТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НоменклатураИмпорт КАК НоменклатураИмпорт
	|		ПО ЗаказПокупателяТовары.Номенклатура = НоменклатураИмпорт.Номенклатура
	|ГДЕ
	|	ЗаказПокупателяТовары.Ссылка = &Зак1
	//условия для отказа - автоматическое внутреннее соединение по ссылка
	|	И ЗаказПокупателяТовары.Ссылка.РазрешитьГрузитьТоварыИзСтопЛиста = &РазрешенныйЗаказ
	|	И НЕ (НоменклатураИмпорт.Номенклатура есть Null)";
	
	Запрос.УстановитьПараметр("РазрешенныйЗаказ", РазрешенныйЗаказ);
...
//+++ 24.10.2018 +++ 
функция ЕстьТоварыСтопЛиста( Зак1 ) Экспорт
...
//+++ 06.11.2018
// на Дату документа - смотрим наличие резерва по Контрагенту и Подразделению
//СВЕРТКА по ТЧ . ТОВАРЫ
//ВариантВозврата=0 => // Булево
//ВариантВозврата=1 => // КонтрагентРезерв
//ВариантВозврата=2 => // ТаблЗнач
//------поля: ---------
//КонтрагентРезерв,
//ВариантКод,
//Номенклатура,
//Количество
//
функция ЕстьРезервИМ(Док, ВариантВозврата=0) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ТЧ.Номенклатура,
	               |	ТЧ.Количество
	               |ПОМЕСТИТЬ ВТ_Товары_ТЧ
	               |ИЗ
	               |	&Табл КАК ТЧ
	               |;
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ВТ_ТоварыB.Номенклатура,
				   //09.11.2018---в реализации ЕСТЬ дубли строк-------------
				   |	СУММА(ВТ_ТоварыB.Количество) КАК Количество,
	               |	Ном.ВидТовара,
	               |	Ном.НоменклатурнаяГруппа
	               |ПОМЕСТИТЬ ВТ_Товары
	               |ИЗ
	               |	ВТ_Товары_ТЧ КАК ВТ_ТоварыB
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК Ном
	               |		ПО ВТ_ТоварыB.Номенклатура = Ном.Ссылка
	               |СГРУППИРОВАТЬ ПО
	               |	ВТ_ТоварыB.Номенклатура,
	               |	Ном.ВидТовара,
	               |	Ном.НоменклатурнаяГруппа
	               ...
	               |				И Подразделение = &Подразделение) КАК РезервРазрешенияСрезПоследних
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_Товары КАК ТЧТовары
	               |		ПО РезервРазрешенияСрезПоследних.ВидТовара = ТЧТовары.ВидТовара
	               |			И (РезервРазрешенияСрезПоследних.НоменклатурнаяГруппа = ТЧТовары.НоменклатурнаяГруппа
	               |				ИЛИ РезервРазрешенияСрезПоследних.НоменклатурнаяГруппа = ЗНАЧЕНИЕ(Справочник.НоменклатурныеГруппы.ПустаяСсылка))
	               |ГДЕ
	               |	РезервРазрешенияСрезПоследних.ДатаОкончания = ДАТАВРЕМЯ(1, 1, 1)
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	РезервРазрешенияСрезПоследних.КонтрагентРезерв.Код
	               |АВТОУПОРЯДОЧИВАНИЕ
	               |;
	               |////////////////////////////////////////////////////////////////////////////////
	               |УНИЧТОЖИТЬ ВТ_Товары_ТЧ;
	               |УНИЧТОЖИТЬ ВТ_Товары";
	...
	табл = Док.Товары.Выгрузить(,"Номенклатура, Количество");
	Запрос.УстановитьПараметр("табл",табл);
	
	Результат = Запрос.Выполнить();

	Если ВариантВозврата=0 тогда      // Булево
		рез = НЕ Результат.Пустой();
		
	иначеЕсли ВариантВозврата=1 тогда // Контрагент
		Если Результат.Пустой() тогда
		рез = справочники.Контрагенты.ПустаяСсылка();
		иначе
		выборка = Результат.Выбрать();
		выборка.Следующий();
		рез = выборка.КонтрагентРезерв;
	КонецЕсли;
	
	иначеЕсли ВариантВозврата=2 тогда // ТаблЗнач
		Рез = Результат.Выгрузить();
	КонецЕсли;
...
//+++ 12.11.2018 - единая функция для Web-сервисов
функция ПолучитьТаблТовРезерваИМ( Контрагент, СписоКодов, Подразделение=неопределено ) Экспорт
	
	табл_New = новый ТаблицаЗначений;
	
		Подразделение=?(Подразделение=неопределено, справочники.Подразделения.НайтиПоКоду("00005"), Подразделение);
		запрос_new = новый Запрос;
		запрос_new.Текст = "ВЫБРАТЬ
           |	спр.Ссылка КАК Номенклатура,
           |	спр.ВидТовара,
           |	спр.НоменклатурнаяГруппа
           |ПОМЕСТИТЬ ВТ_Товары_New
           |ИЗ
           |	Справочник.Номенклатура КАК спр
           |ГДЕ
           |	спр.Код В (&СписоКодов)
           |;
           |////////////////////////////////////////////////////////////////////////////////
           |ВЫБРАТЬ РАЗЛИЧНЫЕ
           |	РезервNew.КонтрагентРезерв,
           |	ВТ.Номенклатура
           |ИЗ
           |	РегистрСведений.РезервРазрешения.СрезПоследних(,
           |			Контрагент = &Контрагент
           |				И Подразделение = &Подразделение) КАК РезервNew
		   |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Товары_New КАК ВТ
		   |		ПО РезервNew.ВидТовара = ВТ.ВидТовара
           |			И (РезервNew.НоменклатурнаяГруппа = ВТ.НоменклатурнаяГруппа
           |				ИЛИ РезервNew.НоменклатурнаяГруппа = ЗНАЧЕНИЕ(Справочник.НоменклатурныеГруппы.ПустаяСсылка)
		   |			   )
		   |ГДЕ РезервNew.ДатаОкончания = ДатаВремя(1,1,1)
		   |; 
		   |////////////////////////////////////////////////////////////////////////////////
  		   |Уничтожить ВТ_Товары_New
		   |";
		запрос_new.УстановитьПараметр("Контрагент",Контрагент);		   
		запрос_new.УстановитьПараметр("СписоКодов", СписоКодов);
	   	запрос_new.УстановитьПараметр("Подразделение",Подразделение);	
		результат_new = запрос_new.Выполнить();
	    Если НЕ результат_new.Пустой() тогда
	    	табл_New = результат_new.Выгрузить();
		Конецесли;
		
	возврат табл_New;
КонецФункции	

Функция ДобавитьДоговорыФормулыОТХ(СписокДоступныхДоговоров) экспорт
	Запрос = новый Запрос;
	Запрос.Текст="ВЫБРАТЬ
	             |	ДоговорыКонтрагентов.Ссылка
	             |ИЗ
	             |	Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
	             |ГДЕ
	             |	ДоговорыКонтрагентов.Владелец = &Скрипченко
	             |	И (ДоговорыКонтрагентов.ТипДоговора = ЗНАЧЕНИЕ(Справочник.ТипыДоговоров.ПеремещениеОТХ)
	             |			ИЛИ ДоговорыКонтрагентов.ТипДоговора = ЗНАЧЕНИЕ(Справочник.ТипыДоговоров.ОтсрочкаОТХ))";
				 Запрос.УстановитьПараметр("Скрипченко", Справочники.Контрагенты.НайтиПоКоду("П017979"));
				 Рез = Запрос.Выполнить().Выбрать();
				 
				 Пока Рез.Следующий() Цикл
					СписокДоступныхДоговоров.Добавить(рез.Ссылка); 
				КонецЦикла;
				Возврат СписокДоступныхДоговоров;
			КонецФункции
			
Функция	ДобавитьКонтрагентовФормулыОТХ(СписокДоступныхКонтрагентов) экспорт
	СписокДоступныхКонтрагентов.Добавить(Справочники.Контрагенты.НайтиПоКоду("П017979"));
	Возврат СписокДоступныхКонтрагентов;
КонецФункции

функция ВыразитьКакЧисло(Стр) экспорт
возврат число(Лев(Стр,2));	
конецФункции	
