...
Процедура ПроверитьРезервы(Объект, Отказ, таблПост=неопределено, ЭтоПроведениеИзЗаказа=ЛОЖЬ) Экспорт
	...
	флИндекс = (табл.Количество()>=100); //индексация МЕШАЕТ?! 100 х 100 = 10 тыс.  1000х1000=1 млн.
...
//+++ 20.07.2018 ------Явный запрос по другим заказам ------------	
//|////////////////////////////////////////////////////////////////////////////////
//|	ВЫБРАТЬ 
//|		док.Ссылка КАК Заказ
//|	ПОМЕСТИТЬ вт_закДругие
//|		ИЗ Документ.ЗаказПокупателя КАК док
//|	ГДЕ 
//|		док.Проведен И док.Проверен  
//|		И док.ссылка <> &Ссылка
////--------------надо делать ограничение вглубину!---------
//|		И док.Дата>&МинДатаЗаказов

////---------только по товарам---уменьшает выборку и время с 2,5 до 0,7-0,9 сек, но делает внутренне соединение со всеми Товарами! ----------
//|		И док.Товары.Номенклатура В (ВЫБРАТЬ вт.Номенклатура ИЗ ВТ_ЗаказТовары КАК вт "+?(флСтоп, "ГДЕ НЕ вт.Номенклатура в (&масСтоп)","")+")

//|		И док.Транзит"+?(Транзит, " И док.Подразделение = &Подразделение", "=ЛОЖЬ")+" 
//S-блокировка всех заказов кроме Ссылки ?!
//|"+ ?(ЭтоПроведениеИзЗаказа," ДЛЯ ИЗМЕНЕНИЯ","")+"
//|;	
|////////////////////////////////////////////////////////////////////////////////
...
|	РегистрНакопления.ЗаказыПокупателей.Остатки( ,
|			 Номенклатура В (ВЫБРАТЬ вт.Номенклатура ИЗ ВТ_ЗаказТовары КАК вт "+?(флСтоп, "ГДЕ НЕ вт.Номенклатура в (&масСтоп)","")+")
//+++ 20.07.2018 ------неявный запрос >> явный------------	
//|		И ЗаказПокупателя В (ВЫБРАТЬ вт2.Заказ ИЗ вт_закДругие КАК вт2)	 

//24.07.2018  неявный - лучше!	пратформа сама делает ВНУТРЕННЕЕ соединение по нужным заказам
...
//18.07.2018 ---- U-блокировка на ВСЕ заказы! по этим товарам!
|"+ ?(ЭтоПроведениеИзЗаказа," ДЛЯ ИЗМЕНЕНИЯ","")+"
...
|ВЫБРАТЬ
 ...
 //---------НЕ может быть несколько производителей для 1 товара!?--------------
	...
	//|					И ОстСторПоставщиков.Остаток > 0
...
//+++)	
	...
	//24.07.2018 - неявное лучше! |УНИЧТОЖИТЬ вт_закДругие;
