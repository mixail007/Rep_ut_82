...
Процедура ПроверитьРезервы(Объект, Отказ, таблПост=неопределено, ЭтоПроведениеИзЗаказа=ЛОЖЬ) Экспорт
	...
	//10.10.2018
	ЗапросКБазе.УстановитьПараметр( "ПодразделениеЯрославль", ГоловноеЯрославль );
	...
	//+++ 10.10.2018 - Задача № 56309 и Черный и Серый список по [v] РазрешитьГрузитьТоварыИзСтопЛиста из Заказа!
	//разрешение серого списка было в Контрагенте!  с 11.10.2018 - ДОЛЖЕН БЫТЬ В ЗАКАЗЕ !
	разрешенСерый  = (ОбъектКонтрагент.Экспорт или ОбъектКонтрагент.ВыгружатьСерыйСписокВTerminal  или Объект.РазрешитьГрузитьТоварыИзСтопЛиста); 
	разрешенЧерный = (ОбъектКонтрагент.Экспорт или ОбъектКонтрагент.ВыгружатьЧерныйСписокВTerminal или Объект.РазрешитьГрузитьТоварыИзСтопЛиста); 
...
//-> если не проверен - пусть будет НЕ проведенным! тогда можно избавиться от поиска по индексу Проверен!
	|		И ЗаказПокупателя.ПометкаУдаления = ЛОЖЬ И ЗаказПокупателя.Проверен  
	|		И ЗаказПокупателя.ссылка <> &Ссылка
	|		И ЗаказПокупателя.Транзит"+?(Транзит, " И ЗаказПокупателя.Подразделение = &Подразделение", 
	
//+++ 10.10.2018 для Ярославля - тоже нужен отбор по Подразделению!, иначе все подразделения блокируются ?!
	"=ЛОЖЬ" +?(ЭтоПроведениеИзЗаказа," И ЗаказПокупателя.Подразделение = &ПодразделениеЯрославль","") )+" 
//уменьшает выборку в несколько раз
...
|"+?(ИспользоватьОстатокПостащиков=ЛОЖЬ,"","
	...
	|		Номенклатура В (ВЫБРАТЬ вт.Номенклатура ИЗ ВТ_ЗаказТовары как вт)
//24.07.2018  неявный - лучше!	пратформа делает ВНУТРЕННЕЕ соединение по нужным заказам
	|		И ЗаказПокупателя.Проведен И ЗаказПокупателя.Проверен  
	|		И ЗаказПокупателя.ссылка <> &Ссылка
	|		И ЗаказПокупателя.Транзит"+?(Транзит, " И ЗаказПокупателя.Подразделение = &Подразделение", "=ЛОЖЬ")+" 
//по Дате - нельзя делать ограничение, т.к. Проверен и Транзит - без доп.упорядочивания... и поиск в индексах идёт по Реквизит+Ссылка	
...
//+++)	
	...
	МинДатаЗаказов  = '20170704';//  с 2017.07.04 много заказов висит 
