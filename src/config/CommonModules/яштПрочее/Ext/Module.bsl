...
//ДеньЗаказа    - тип Дата, понедельник, 11-00 = '01.01.0001 00:00:00' + 1*86400 + 11*3600
//ДеньРазгрузки - тип Дата, Вторник, 11-00     = '02.01.0001 00:00:00' + 2*86400 + 9*3600
//
функция ПолучитьТаблРазгрузкиВФилиалах()
...
//---------------- Питер --------------
	...
	впути=?(дн=5, 3, 1); //пятница - прибытие в следующий понедельник	 
...
//---------------- РнД --------------
	КодФилиала = "00106";   //3 раза 
	Если дн=1 или дн=3 или дн=5 или дн=8 тогда
	впути=?(дн=5, 3, 2); //пятница - прибытие в следующий понедельник	 

	стр1 = табл.Добавить(); 
	стр1.КодФилиала		= КодФилиала; 
	стр1.ДеньЗаказа		= день0 + ii*день + 9*час;
	стр1.ДеньРазгрузки	= (ii+впути)*день + 17*час;
	КонецЕсли;
	
//---------------- Ект --------------
	КодФилиала = "00138";   //2 раза 
	Если дн=1 или дн=4 или дн=8 тогда
	впути=?(дн=5, 4, 3); //пятница - прибытие в следующий понедельник	 
