...
функция ПолучитьСписокГрузШин() экспорт
	
	масГруз=НОВЫЙ СписокЗначений;
	масГруз.Добавить( справочники.Номенклатура.НайтиПоКоду("0001748") ); //Грузовые
	масГруз.Добавить( справочники.Номенклатура.НайтиПоКоду("9178094") );    // Востановленные KaMaRetread		
	
	масГруз.Добавить( справочники.Номенклатура.НайтиПоКоду("0080632") ); //Спецтехника
	масГруз.Добавить( справочники.Номенклатура.НайтиПоКоду("9109641") ); 	// Шины для вилочных погрузчиков
	масГруз.Добавить( справочники.Номенклатура.НайтиПоКоду("9109642") );    // Диагон. шины для самосвалов и погрузчиков
	масГруз.Добавить( справочники.Номенклатура.НайтиПоКоду("9109643") );    // Шины для дор.-строит. техники
	масГруз.Добавить( справочники.Номенклатура.НайтиПоКоду("9109644") );    // Шины радиальные для Д-С Техники
			
	масГруз.Добавить( справочники.Номенклатура.НайтиПоКоду("0001750") ); //Сельскохозяйственные
	масГруз.Добавить( справочники.Номенклатура.НайтиПоКоду("0080631") ); //Грузовые и спецтехника
...
Процедура ПоказатьОбъем(КолонкиТабПоля, ЯчейкиСтрокиТабПоля, Номенклатура) Экспорт
	...
	КонецЕсли;
КонецПроцедуры	
процедура ПоказатьОбъемШ(КолонкиТабПоля, ЯчейкиСтрокиТабПоля, Номенклатура, ТочностьРез=3, ИмяКоличество="Количество" ) Экспорт
	СписокГрузШин = ПолучитьСписокГрузШин(); //11.12.2017
	ОбъемВычисленный = 0;
	Если КолонкиТабПоля.Объем.Видимость Тогда
		Количество = ЯчейкиСтрокиТабПоля[ИмяКоличество].Значение;
		Если Номенклатура <> Неопределено и Количество>0 Тогда
			//+++ 11.12.2017 только для груз.шин  по задаче № 37329
			Если СписокГрузШин.НайтиПоЗначению(Номенклатура.Родитель)<>неопределено 
		    	Тогда Коэф = 1.3; 
				Иначе Коэф = 1; 
			КонецЕсли;
			ОбъемВычисленный = Коэф * ВычислитьОбъемНоменклатуры(Номенклатура) * Количество;
		КонецЕсли;
	ЯчейкиСтрокиТабПоля.Объем.УстановитьТекст( Формат(окр(ОбъемВычисленный,ТочностьРез),"ЧДЦ="+строка(ТочностьРез)+";ЧН=-; ЧВН=") );
	КонецЕсли;
КонецПроцедуры
Процедура ПоказатьОбъемШПодвал(КолонкиТабПоля, ТЧТовары, ТочностьРез=3, ИмяКоличество="Количество") Экспорт
		Если КолонкиТабПоля.Объем.Видимость Тогда
		итогОбъем = 0;
		Для каждого стр1 из ТЧТовары Цикл
		итогОбъем = итогОбъем + стр1[ИмяКоличество] * ВычислитьОбъемНоменклатурыШ(стр1.Номенклатура); 
		КонецЦикла;
...
Функция ВычислитьОбъемНоменклатурыШ(Номенклатура, ТочностьРез=6) экспорт
	объем = 0;
	...
	Если Номенклатура <> Неопределено Тогда
		...
		Если СписокГрузШин.НайтиПоЗначению(Номенклатура.Родитель)<>неопределено 
			...
			Иначе Коэф = 1; 
		КонецЕсли;
		объем = Коэф * ВычислитьОбъемНоменклатуры(Номенклатура, ТочностьРез);
	КонецЕсли;
возврат Объем;			
КонецФункции
