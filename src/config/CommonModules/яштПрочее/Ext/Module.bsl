...
Процедура ПроверитьДоговорВЗаказеПокупателя(Объект, Отказ, Запретить=ЛОЖЬ, ДатаДок=неопределено) Экспорт
		...
		ИначеЕсли ТипЗнч(Объект)=Тип("ДокументОбъект.ЗаказПокупателя") тогда
			...
			ЗапретДиски = Объект.Контрагент.ЗапретитьДиски;
			ЗапретШина  = Объект.Контрагент.ЗапретитьШины;
			ЗапретАКБ   = Объект.Контрагент.ЗапретитьАКБ;
		    ЗапретАксы  = Объект.Контрагент.ЗапретитьАксы;
