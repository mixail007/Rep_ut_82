...
Процедура ПроверитьРезервы(Объект, Отказ, таблПост=неопределено) Экспорт
	...
	//+++ 29.05.2018 - для больших таблиц (больше 40-100 строк - сокращает запрос с 13-15 до 3-4 сек (4-5х)
	|"+?(табл.Количество()>40, "ИНДЕКСИРОВАТЬ ПО ЗаказПокупателяТовары.Номенклатура","")+"
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|"+?(Объект.Контрагент.Экспорт или Объект.РазрешитьГрузитьТоварыИзСтопЛиста,"",?(Объект.Контрагент.РазрешитьГрузитьТоварыИзСтопЛиста,"И НЕ Номенклатура В ( ВЫБРАТЬ НоменклатураИмпорт.Номенклатура
	...
	|	А.Номенклатура
	|
	//+++ 29.05.2018 - для больших таблиц (больше 40-100 строк - сокращает запрос с 13-15 до 2,5 - 4 сек (4-6х)
	|"+?(табл.Количество()>40, "ИНДЕКСИРОВАТЬ ПО А.Номенклатура","")+"
	...
	|	из РегистрНакопления.ЗаказыПокупателей.Остатки(, Номенклатура В (ВЫБРАТЬ ВТ_ЗаказТовары.Номенклатура ИЗ ВТ_ЗаказТовары)
