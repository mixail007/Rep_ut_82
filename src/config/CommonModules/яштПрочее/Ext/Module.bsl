...
функция ПолучитьТаблРазгрузкиВФилиалах()
	...
	табл.Колонки.Добавить("ДеньЗаказа");
	табл.Колонки.Добавить("ДеньРазгрузки");
...
//---------------- Питер --------------
	...
	стр1.ДеньРазгрузки	= день0 + (ii+впути)*день + 9*час;
	
	стр1 = табл.Добавить();  //2-я машина
	стр1.КодФилиала		= КодФилиала; 
	стр1.ДеньЗаказа		= день0 + ii*день + 13*час;
	стр1.ДеньРазгрузки	= день0 + (ii+впути)*день + 15*час; 
//---------------- РнД --------------
	...
	стр1.ДеньРазгрузки	= день0 + (ii+впути)*день + 17*час;
	КонецЕсли;
...
//---------------- Тл --------------
	кодФ = "00172";   //1 раз во втроник или во вторник через неделю 
...
функция найтиБлижайщуюДатуРазгрузкиНаФилиале(ДатаЗаказа, Подразделение)
	...
	ДеньЗаказа = ДатаЗаказа - НачалоНедели(ДатаЗаказа); // понедельник - 1 день
	КодФилиала = Подразделение.Код;
	
	ДатаРазгрузки = '00010101'; // пустая дата!	
	
	для каждого стр1 из табл1 цикл
		Если стр1.Код = КодФилиала и ДеньЗаказа < стр1.ДеньЗаказа тогда
			ДатаРазгрузки = НачалоНедели(ДатаЗаказа) + стр1.ДеньРазгрузки;
			прервать; // первый 
		КонецЕсли;
	КонецЦикла;	
	
	Если ДатаРазгрузки = '00010101' тогда // не нашли ни одной отгрузки...
...
//11.10.2018 --- Задача № 56199 учет графика отгрузки из Ярославля + доставка + разгрузка в филиале ---------------
