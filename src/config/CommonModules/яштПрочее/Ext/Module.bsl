...
Функция ПолучитьУсловиеБезПредоплаты(УКлиентаЕстьДоговорПоручительства, ОбъектКонтрагент, ЭтоЗаказИлиРеализация, дог1, флТрКомпанияПитСтоп=ЛОЖЬ)
	...
	рез =(  
		...
		//6. или это ИП 21.11.2018 - вернули назад
				 ИЛИ ( стрДлина(сокрЛП(ОбъектКонтрагент.ИНН))=12 )
		  );
		  
	//+++ 21-22.11.2018 - ВЫКЛючено отдельное условие для ИП - проверка предоплаты - нужна по всем договорам Предоплаты (без *)
	//Если ( стрДлина(сокрЛП(ОбъектКонтрагент.ИНН))=12 и не СоЗвездой //ИП - и договор без* можно?!
	//	//--------21.11.2018 - для ИП - надо проверять предоплату всегда!-----------
	//		и НЕ (  дог1ТипДоговора = Справочники.ТипыДоговоров.НайтиПоКоду("00001") 
