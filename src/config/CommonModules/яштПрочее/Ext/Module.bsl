...
функция ЗаказЗакрытИлиОтгруженПолностью( зак1, УчитыватьЧастичнуюОтгрузку = ЛОЖЬ )
	...
	Запрос = Новый Запрос;
	//+++ 04.06.2018 - сразу табл. товаров
	Табл1 = зак1.Товары.Выгрузить(,"Номенклатура, Количество");
	...
	Запрос.Текст = "ВЫБРАТЬ
					...
					|ИЗ
					|	&Табл1 КАК тов
					|"+?(Табл1.Количество()>30," Индексировать по Номенклатура","")+"
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
				 ...
				 //+++ 04.06.2018  - избыточный отбор! не нужен
...
//+++)
	...
	//+++ 26.06.2018  - 1 подзапрос вместо 3х  х  2
	|ВЫБРАТЬ спр.Ссылка КАК Склад
	|ПОМЕСТИТЬ ВТ_Склады
	|ИЗ справочник.Склады как спр
	|	ГДЕ спр.ЭтоГруппа = ЛОЖЬ 
	| 	И спр.ЗапретитьИспользование = ЛОЖЬ
	|	И спр.Транзитный "+?(Транзит, " И спр.Подразделение = &Подразделение", " = ЛОЖЬ")+"
    |;
	|////////////////////////////////////////////////////////////////////////////////
	|
	|ВЫБРАТЬ
...
//+++)	
	...
	//26.06.2018 - внутри 1 запроса вместо 2х
