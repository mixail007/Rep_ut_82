...
Процедура ПроверитьРезервы(Объект, Отказ, таблПост=неопределено) Экспорт
	...
	//28.06.2018 - вынесена проверка в модуль заказа ДО блокировки!
	//Если ЗаказЗакрытИлиОтгруженПолностью(Объект) тогда 
	//	Возврат;
	//КонецЕсли;	
...
|				И ЗаказПокупателя.Проверен
|				И ЗаказПокупателя.Транзит"+?(Транзит, " И ЗаказПокупателя.Подразделение = &Подразделение", "=ЛОЖЬ")+") КАК ЗаказыПокупателейОст
|ГДЕ
...
|"+?(ИспользоватьОстатокПостащиков=ЛОЖЬ,"","
	...
	|		И Номенклатура В (ВЫБРАТЬ вт.Номенклатура ИЗ ВТ_ЗаказТовары как вт)
	
	//26.05.2017 - уменьшает выборку заказов почти в 10 раз
	|		И ЗаказПокупателя.проведен
//// В ожидании ТОЛЬКО проверенные заказы и так!
	|		И ЗаказПокупателя.Проверен
	
//+++ 20.06.2018 - ТОЛЬКО не транзитные всегда!
...
//+++)	
	...
	//------------- "долго" удаляются, НО чтобы tempDb не рос - надо ---------------------
	//|;
	...
	ЗапросКБазе.УстановитьПараметр("ДатаЗаказа",Объект.Дата); //07.11.2016
...
Процедура ГонконгОстаткиДенежныхСредствДействие() Экспорт
	...
	Если РольДоступна("ПравоЗавершенияРаботыПользователей") //ИТ
		...
		ФормаОтчета = ОстаткиДенежныхСредств.ПолучитьФорму();
		ФормаОтчета.Открыть();
		
		ФормаОтчета.ЭлементыФормы.ФлажокНастройкиОрганизация.Значение   = Истина;
		ФормаОтчета.ЭлементыФормы.ПолеВидаСравненияОрганизация.Значение = ВидСравнения.Равно;
		ФормаОтчета.ЭлементыФормы.ПолеНастройкиОрганизация.Значение     = Справочники.Организации.НайтиПоКоду("00005"); //Wheels Group Limited
		
		ФормаОтчета.ЭлементыФормы.ФлажокНастройкиОрганизация.Доступность = Ложь;
		ФормаОтчета.ЭлементыФормы.ПолеВидаСравненияОрганизация.Доступность = Ложь;
		ФормаОтчета.ЭлементыФормы.ПолеНастройкиОрганизация.Доступность = Ложь;
		
		ФормаОтчета.ЭлементыФормы.КоманднаяПанельФормы.Кнопки.Настройка.Доступность = Ложь;
