...
Процедура ПроверитьРезервы(Объект, Отказ, таблПост=неопределено) Экспорт
	...
	флИндекс = (табл.Количество()>=1000); //индексация МЕШАЕТ?! 100-1000  1000х1000 = 1 млн.
	...
	|ВЫБРАТЬ вт1.Номенклатура 
	|Поместить ВТ_ЗаказТоварыБезСтопЛиста
	|из ВТ_ЗаказТовары как вт1
	//-------------все условия через СОЕДИНЕНИЕ быстрее!---------
	|"+?(разрешенСерый и разрешенЧерный,"",
		" ЛЕВОЕ СОЕДИНЕНИЕ
	|	( ВЫБРАТЬ стопЛист.Номенклатура
	|	ИЗ РегистрСведений.НоменклатураИмпорт КАК стопЛист 
	|	ГДЕ стопЛист.Номенклатура в (выбрать вт2.Номенклатура из ВТ_ЗаказТовары как вт2)
	|"  +?(разрешенСерый и не разрешенЧерный, "И стопЛист.ЧерныйЛист", 
	  		?(не разрешенСерый и разрешенЧерный, "И стопЛист.ЧерныйЛист = ЛОЖЬ", ""))+") как НомСтоп
	|ПО вт1.Номенклатура = НомСтоп.Номенклатура
		//-------- не в списке!--------------
	|ГДЕ НомСтоп.Номенклатура есть Null"
	   )+"
	//не надо! только для внутреннего отбора  
	//|"+?(ФлИндекс, "ИНДЕКСИРОВАТЬ ПО вт1.Номенклатура","")+"
	|;
...
//=============================Остатки + ОТХ + РезервИМ===========================	
	...
	|	Номенклатура В (выбрать вт.Номенклатура из ВТ_ЗаказТоварыБезСтопЛиста как вт)	
	//+++)
...
|				И ЗаказПокупателя.Проверен
|				И ЗаказПокупателя.Транзит"+?(Транзит, "=Истина И ЗаказПокупателя.Подразделение = &Подразделение", "=ЛОЖЬ")+") КАК ЗаказыПокупателейОст
|ГДЕ
...
|ВЫБРАТЬ
 ...
 //---------НЕ может быть несколько производителей для 1 товара!?--------------
	...
	|				ГДЕ
	|					ОстСторПоставщиков.Номенклатура В (выбрать вт.Номенклатура из ВТ_ЗаказТоварыБезСтопЛиста как вт)	
...
//+++)	
	...
	| ГДЕ ТоварыВОжиданииОтПоставщика.КоличествоОборот>0 ) как ТоварыВОжидании
	...
	//|УНИЧТОЖИТЬ ВТ_ЗаказТоварыБезСтопЛиста;
	//|УНИЧТОЖИТЬ ВТ_ОстаткиНаСкладахИОТХ;
	//|УНИЧТОЖИТЬ ВТ_ДругиеЗаказы;
	//|"+?(ИспользоватьОстатокПостащиков,"УНИЧТОЖИТЬ ВТ_ОстаткиСторПоставщиков;","")+"
	|";
	...
	Если //НЕ Объект.Ссылка.Контрагент.КонтрагентДляРезерваИМ.Пустая() или //123Заменить
		...
		|				Номенклатура В 
	//+++ 04.06.2018 все условия стоп листа в ВТ!--------------------------------------------------
	|	(выбрать вт.Номенклатура из ВТ_ЗаказТоварыБезСтопЛиста как вт)	
	//+++)
