...
Процедура ПроверитьРезервы(Объект, Отказ, таблПост=неопределено, ЭтоПроведениеИзЗаказа=ЛОЖЬ) Экспорт
	...
	//+++ 15.06.2018 - для транзитных заказов/реализаций - НЕ ИСПОЛЬЗУЮТСЯ остатки сторонних поставщиков!
	Если Транзит тогда  
...
//+++ 26.07.2018 ЯВНЫЙ запрос приводит в лишним блокировкам!?--------------
//|ВНУТРЕННЕЕ СОЕДИНЕНИЕ (выбрать заказПокупателя.Ссылка из Документ.ЗаказПокупателя как ЗаказПокупателя 
//|ГДЕ 
////если не проверен - пусть будет НЕ проведенным! тогда можно избавиться от поиска по индексу Проверен!
//	|		 ЗаказПокупателя.Проведен И ЗаказПокупателя.Проверен  
//	|		И ЗаказПокупателя.ссылка <> &Ссылка
//	|		И ЗаказПокупателя.Транзит"+?(Транзит, " И ЗаказПокупателя.Подразделение = &Подразделение", "=ЛОЖЬ")+" 
//	
...
////-----------ТОЛЬКО ЗАКАЗЫ с такими же товарами!-------------------
//|		И ЗаказПокупателя.Товары.Номенклатура в (ВЫБРАТЬ вт.Номенклатура ИЗ ВТ_ЗаказТовары КАК вт "+?(флСтоп, "ГДЕ НЕ вт.Номенклатура в (&масСтоп)","")+")
//|)как Док
//|ПО ЗаказыПокупателейОст.ЗаказПокупателя = док.Ссылка
//+++)
...
//+++ 07.08.2018 	
	|			СрокиДоставки.СрокДоставки КАК АвтоПодтверждение,
	|			СрокиДоставки.СрокДоставки КАК КонтролироватьКоличество,
	...
	|				ГДЕ
	|					ОстСторПоставщиков.Номенклатура В (выбрать вт.Номенклатура из ВТ_ЗаказТовары как вт "+?(флСтоп, "ГДЕ НЕ вт.Номенклатура в (&масСтоп)","")+")	
//---- не нужно контролировать остаток! |	И ОстСторПоставщиков.Остаток > 0
	//ТОЛЬКО НЕ ТРАНЗИТНЫЕ ОСТАТКИ!
...
//+++)	
	...
	|	ЕСТЬNULL(ОстаткиСтороннихПоставщиков.Остаток, 0)
	...
	|	- ВЫБОР КОГДА ОстаткиСтороннихПоставщиков.КонтролироватьКоличество
	|			ТОГДА ЕСТЬNULL(ТоварыВОжидании.Количество,0)
	|			ИНАЧЕ 0 КОНЕЦ
	//+++)
	|	КАК КолПост",
	//------------------------------------------------------------------
