...
Процедура ПроверкаПериодаЗаписей(НаборЗаписей, Отказ) Экспорт
	...
	Если Отказ 
		 или ( НачалоМесяца(ДатаМин)=НачалоМесяца(ДокументРегистратор.Дата)
			 и ДокументРегистратор.Дата > ДатаМин 
			 ) тогда
		Возврат;
	КонецЕсли;
	//+++)                                
...
//+++ 26.11.2014 - так было!
//-------------------берётся Период из ПОЛНОГО Набора записей Регистра...по регистратору???--------------
//-------------------который ВСЕГДА равен Дате документа!?----------------------		
		//Запрос.Текст = "
...
//+++ 26.11.2014 - берем Дату из Ссылка Документа-регистратора!----------------------		
		...
		Запрос.УстановитьПараметр("ДатаДок", ДокументРегистратор.Дата);	
		Если ЕстьОрганизация тогда
...
//Функция ПроверкаНастроенныхРазрешений(Объект, Новые = Ложь)

//	Запрос = Новый Запрос;
//	Запрос.Текст = 
//	"ВЫБРАТЬ
//	|	НастройкаПравДоступа.ВидДокументов,
//	|	НастройкаПравДоступа.ВыбранныйДокументВид,
//	|	НастройкаПравДоступа.ДатаНачалаОбъектов,
//	|	НастройкаПравДоступа.ДатаКонцаОбъектов,
//	|	НастройкаПравДоступа.ДатаОкончанияРазрешения,
//	|	НастройкаПравДоступа.Утвержден,
//	|	НастройкаПравДоступа.ПользовательРоль
//	|ИЗ
//	|	РегистрСведений.НастройкаПравДоступа КАК НастройкаПравДоступа
//	|ГДЕ
//	|	ВЫБОР
//	|			КОГДА НастройкаПравДоступа.ВидДокументов
//	|				ТОГДА &ДокументТип = НастройкаПравДоступа.ВыбранныйДокументВид
//	|						И (&ДокументДата МЕЖДУ НастройкаПравДоступа.ДатаНачалаОбъектов И НастройкаПравДоступа.ДатаКонцаОбъектов)
//	|			ИНАЧЕ НастройкаПравДоступа.ВыбранныйДокументВид = &Документ
//	|		КОНЕЦ
//	|	И ВЫБОР
//	|			КОГДА НастройкаПравДоступа.ПользовательРоль ССЫЛКА Справочник.Пользователи
//	|				ТОГДА НастройкаПравДоступа.ПользовательРоль = &ТекущийПользователь
//	|			ИНАЧЕ НастройкаПравДоступа.ПользовательРоль В (&РолиПользователя)
//	|		КОНЕЦ
//	|" + ?(Новые, "И НастройкаПравДоступа.РазрешитьНовые = ИСТИНА", "") + " 
//	|	И НастройкаПравДоступа.Утвержден = ИСТИНА
//	|	И НастройкаПравДоступа.ДатаОкончанияРазрешения >= &ТекущаяДата";
//	
//	//Объект = Документы.ЗаказПоставщику.ПустаяСсылка();
//	Запрос.УстановитьПараметр("Документ", Объект.Ссылка);
//	Запрос.УстановитьПараметр("ДокументДата", Объект.Дата);
//	Запрос.УстановитьПараметр("ТекущийПользователь", ПараметрыСеанса.ТекущийПользователь);
//	Запрос.УстановитьПараметр("ДокументТип", Объект.Метаданные().Имя);
...
//	Запрос.УстановитьПараметр("РолиПользователя", СписокРолейПользователя);
