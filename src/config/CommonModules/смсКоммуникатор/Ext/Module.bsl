...
//
// • WSСсылка «SMS4B»
// • WSСсылка «SMS4BРезерв»
// • Параметр сеанса «смсКодСессии»
// • Параметр сеанса «смсПользователь»
// • Параметр сеанса «смсПароль»
// • Параметр сеанса «смсОсновнойСервер»
// • Параметр сеанса «смсКорректировкаUTC»
...
// 
//	//+++ Замена Параметры на Константы
//		ИмяПользователя    = Константы.смсИмяПользователя.Получить();  
//		ПарольПользователя = Константы.смсПарольПользователя.Получить();
//
Процедура УстановитьИмяПользователяИПароль(ИмяПользователя, Пароль) Экспорт 
	//ПараметрыСеанса.смсПользователь = ИмяПользователя;
	//ПараметрыСеанса.смсПароль		= Пароль;
КонецПроцедуры
...
Функция Подключиться() Экспорт
	...
	Попытка
		//+++ Замена Параметров на Константы
		//ИмяПользователя = ПараметрыСеанса.смсПользователь;
		//ПарольПользователя = ПараметрыСеанса.смсПароль;
		ИмяПользователя    = Константы.смсИмяПользователя.Получить();
		ПарольПользователя = Константы.смсПарольПользователя.Получить();
	Исключение
	...
	ВерсияЯдра = смсКоммуникатор.ВернутьВерсиюЯдра();
	// Дополняем имя пользователя сведениями о конфигурации
	Пользователь = " S_" + ВерсияЯдра + "_" + Метаданные.Версия + "_" + ИмяКонфигурации + "_" + СисИнфо.ВерсияПриложения + " " + ИмяПользователя;
	//Пользователь	=  ИмяПользователя;
	Пароль 		 = ПарольПользователя;
	
	// Всегда пробуем сначала подключиться на основной сервер
	ПараметрыСеанса.смсОсновнойСервер = Истина;
	...
	Если НЕ РезультатПодключения = Неопределено Тогда // Успешно
		...
		Если РезультатПодключения < 1 Тогда // Есть ошибки
			ВывестиСообщение(ОписаниеОшибокВебСервиса(РезультатПодключения), СтатусСообщения.Важное, 
		...
		Иначе // Ошибок нет
			...
			ПараметрыСеанса.смсНомерСессии = РезультатПодключения;
			
			// Вызываем функцию для установки параметра сеанса "смсКорректировкаUTC"
...
Функция ПолучитьПараметрыСессии(Параметры = Неопределено) Экспорт
	...
	Если НЕ ПараметрыСессии = Неопределено Тогда
		...
		Если КодОшибки < 0 Тогда 
			...
			ВывестиСообщение(ОписаниеОшибокВебСервиса(КодОшибки), СтатусСообщения.Важное, 
...
// и код ошибки < 0, если функция выполнена не успешно.
//
Функция ОтправитьСообщения(СписокСообщений, НомерОтправителя, ТекстСообщенияПоУмолчанию = "", _ДатаОтправки, _Актуальность = '00010101', НачалоПериодаЗапрета = '00010101', 
										КонецПериодаЗапрета = '00010101', РавномернаяРассылка = Ложь) Экспорт
	...
	Для Индекс = 1 По КолвоОтправок Цикл
		...
		Для Каждого Сообщение Из СписокСообщений Цикл
			...
			Если СчетчикПакетаСообщений = РазмерПакета ИЛИ СчетчикПакетаСообщений = КолвоСообщений Тогда
				...
				Если НЕ РезультатОтправки = Неопределено Тогда
					...
					Для Счетчик = 1 По КолвоОтправленных Цикл
						...
						КонецЕсли;
						//Орлов ++
						Если ТекСообщение.КодОшибки >= 1 Тогда 
							НаборЗаписей = РегистрыСведений.SMSРассылка.СоздатьНаборЗаписей();
							НаборЗаписей.Отбор.GUID.Установить(ТекСообщение.GUID);
							НаборЗаписей.Прочитать();
							Если НаборЗаписей.Выбран() И НаборЗаписей.Количество() > 0 Тогда
								НаборЗаписей[0].ДатаОтправления = _ДатаОтправки;
			...
			КонецЕсли;
			//Орлов++
			//Если КодОшибки = 1 - все ОК запишем дату отправки в регистр "SMSРассылка"
			//Если КодОшибки >= 1 Тогда
			//	НаборЗаписей = РегистрыСведений.SMSРассылка.СоздатьНаборЗаписей();
			//	НаборЗаписей.Отбор.GUID.Установить(Сообщение.GUID);
			//	НаборЗаписей.Прочитать();
			//	Если НаборЗаписей.Выбран() И НаборЗаписей.Количество() > 0 Тогда
			//		НаборЗаписей[0].ДатаОтправления = _ДатаОтправки;
			//		НаборЗаписей.Записать();
...
Функция ПодключитьВебСервис()
	...
	Попытка
		Если ОсновнойСервер Тогда
			// Основной сервер
			ВебСервис = WSСсылки.SMS4B.СоздатьWSПрокси("SMS client", "WSSM", "WSSMSoap12",10); // 10 сек.
		Иначе
			// Резервный сервер
			ВебСервис = WSСсылки.SMS4BРезерв.СоздатьWSПрокси("SMS client", "WSSM", "WSSMSoap12",30);//30 сек.
		КонецЕсли; 
	Исключение
		Возврат Неопределено;
	КонецПопытки; 
...
//Функция подключающияся к другому серверу 
...
Функция ПоменятьСервер(КодОшибкиПодкл)
	...
	// Подключаемся
	КодОшибкиПодкл = Подключиться();
	Возврат НЕ ОсновнойСервер = ПараметрыСеанса.смсОсновнойСервер;
...
//
Процедура ВывестиСообщение(Сообщение, Статус, Заголовок, Уровень)
...
//
Функция ДесятичноеВШестнадцатеричное(Знач _Число) // DecToHex
	База = 16;
	Результат = "";
	Пока _Число <> 0 Цикл
		Поз =_Число % База;
...
//
Функция ШестнадцатеричноеЧислоВДесятичное(Знач _Hex) // HexToDec
	База = 16;
	_Hex = СокрЛП(_Hex);
