...
функция ПолучитьСписТов( Папка1 )
Запрос = Новый Запрос;
Запрос.Текст = "ВЫБРАТЬ
|	Номенклатура.Ссылка
|ИЗ
...
Процедура БергЗагрузкаЦенИОстатков(Печатать = Ложь, ПроверитьЦены = Ложь)
...
ФайлРезультата = "c:\berg_yar.mxl";
   ...
   масТов = ПолучитьСписТов( Папка1 );

  Табл2 = новый ТаблицаЗначений;
   Табл2.Колонки.Добавить("Номенклатура");
   Табл2.Колонки.Добавить("Количество");
   Табл2.Колонки.Добавить("Цена");
   
   Табл2.Колонки.Добавить("Артикул");
   Табл2.Колонки.Добавить("Производитель"); //для установки цен
...
для i=0 по масТов.Количество()-1 цикл
	...
	стр2.Производитель = строка(товар.Производитель);
	
	рез = загрузитьТоварБерг( стр2.Артикул , стр2.Производитель);
...
иначе
	...
	для каждого стр1 из табл1 цикл
		...
		стр2.Производитель =  справочники.Производители.НайтиПоНаименованию(стр1.Производитель);
	КонецЦикла;
...
КонецЕсли;
	...
	Если Печатать Тогда
		...
		Для каждого стр11 из табл2 Цикл
	...
	//----------------нужен код поставщика?-----------------	
	  ЗагрузитьОстаткиВРегистр(Табл2, Берг);
	  ЗагрузитьЦеныВРегистр(Табл2, Берг);
	  
	  //---------------нужен производитель--------------------  
	//  ЗагрузитьЦеныВДокумент(Табл2, Берг);
    КонецЕсли;
...
функция загрузитьТоварБерг(Артикул="1100", Брэнд="MANNOL")
   ...
   стр2 = новый Структура("Склад, Остаток, Цена", "", 0, 0);
...
пока чтение.Прочитать() цикл
	...
	Если Чтение.ТипУзла=ТипУзлаXML.НачалоЭлемента и  Чтение.Имя="offer" тогда
		стр1 = Табл1.Добавить();
...
Процедура ЕжедневнаяЗагрузкаЦенИОстатков() экспорт
	...
	Если ЗагружатьОстаткиИЦеныПоставщика(Справочники.Контрагенты.НайтиПоКоду("П000727")) Тогда //14.05.2018 - Берг Холдинг
		БергЗагрузкаЦенИОстатков(ЛОЖЬ,ЛОЖЬ);
	КонецЕсли;	
	
	Если ЗагружатьОстаткиИЦеныПоставщика(Справочники.Контрагенты.НайтиПоКоду("92532")) Тогда //14.05.2018 - Берг Холдинг
		ТестовыйЗагрузкаЦенИОстатков();
	КонецЕсли;	
...
Процедура ПроверитьНоменклатуруФайлПоставщика(Поставщик) Экспорт
	...
	ИначеЕсли Поставщик = Справочники.Контрагенты.НайтиПоКоду("П000727") Тогда //14.05.2018 - Берг Холдинг
		
		БергЗагрузкаЦенИОстатков(Истина,ЛОЖЬ);
...
Процедура ПроверитьЦеныФайлПоставщика(Поставщик) Экспорт
	...
	ИначеЕсли Поставщик = Справочники.Контрагенты.НайтиПоКоду("П000727") Тогда //14.05.2018 - Берг Холдинг
		...
		БергЗагрузкаЦенИОстатков(ЛОЖЬ,Истина);
	...
	Для i=0 по N-1 цикл
		...
		пока ii<Nn цикл //--------------------цикл по заказам-----------------------
			...
			//ищем такую строку.... 
				...
				Иначе //не может быть разных строк! только 1-я найденная
...
//02.04.2015 не проделенный или Резервный заказ - можно только посчитать R13
	...
	Если ОбДок.Товары.Итог("Количество")<4 тогда
		НетДругихЗаказов = НайтиЗаказыВЭтойЖЕТочке(ОбДок); //+++ 10.05.2018 Задача № 46148 п.2
		Если НетДругихЗаказов тогда
			Текст1 ="Заказ "+обДок.Номер+" - слишком маленький! Количество <4 шт. Других заказов на тот же Адрес доставки - НЕТ.";
			Отказ = Истина;
		иначе
			Текст1 ="Есть другие Заказы в мершруте, поэтому "+обДок.Номер+" - слишком маленький! Количество <4 шт. Поставлен в Маршрут!";
		КонецЕсли;
	ИначеЕсли ОбДок.Товары.Итог("Количество")>4 и ОбДок.СуммаДокумента<10000 тогда	//06.04.2018
		НетДругихЗаказов = НайтиЗаказыВЭтойЖЕТочке(ОбДок); //+++ 10.05.2018 Задача № 46148 п.2
		Если НетДругихЗаказов тогда
		Текст1 ="Заказ "+обДок.Номер+" - слишком маленький! Количество: "+строка(ОбДок.Товары.Итог("Количество"))+", а Сумма <10 тыс. руб. Других заказов на тот же Адрес доставки - НЕТ.";
		Отказ = Истина;
		иначе
			Текст1 ="Есть другие Заказы в мершруте, поэтому "+обДок.Номер+" - слишком маленький! Количество: "+строка(ОбДок.Товары.Итог("Количество"))+", а Сумма <10 тыс. руб. Поставлен в Маршрут!";
		КонецЕсли;
	КонецЕсли;	
...
функция НайтиЗаказыВЭтойЖЕТочке(ЗакОб)
	нетЗаказовНаТуЖеТочку = Истина;
	 Запрос = Новый Запрос;
	 Запрос.Текст = "ВЫБРАТЬ
	                |	ЗаказыПокупателей.ЗаказПокупателя,
	 ...
	 Результат = Запрос.Выполнить();
	 нетЗаказовНаТуЖеТочку = результат.Пустой();
	 
	возврат нетЗаказовНаТуЖеТочку;
КонецФункции	
