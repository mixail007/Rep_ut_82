...
функция ПолучитьЧерныйСписок(КонтрЧерныйСписок)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	черныйСписок.Номенклатура,
	               |	черныйСписок.Артикул КАК Артикул,
	               |	черныйСписок.Производитель КАК Производитель,
	               |	ТоварыНаСкладахОстатки.Склад.Подразделение КАК Подразделение,
				   //свободный остаток черного списка!
	               |	СУММА(ТоварыНаСкладахОстатки.КоличествоОстаток - ЕСТЬNULL(ЗаказыПокупателейОстатки.КоличествоОстаток, 0)) КАК Остаток
	               |ИЗ
	               |	ВТ_ЧернСписок КАК черныйСписок
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыНаСкладах.Остатки(,
	               |				Склад.ЗапретитьИспользование = ЛОЖЬ
	               |					И Номенклатура В (ВЫБРАТЬ вт.Номенклатура ИЗ ВТ_ЧерныйСписок КАК вт)) КАК ТоварыНаСкладахОстатки
	               |			ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗаказыПокупателей.Остатки(,
				   //только "нормальные" проделенные заказы без поставщика!
	               |					ЗаказПокупателя.Поставщик = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	               ...
	               |				И выбор когда ТоварыНаСкладахОстатки.Склад.Подразделение = Значение(справочник.Подразделения.ПустаяСсылка)
				   |					тогда ЗаказыПокупателейОстатки.ЗаказПокупателя.транзит=ЛОЖь
				   |					иначе ТоварыНаСкладахОстатки.Склад.Подразделение =ЗаказыПокупателейОстатки.ЗаказПокупателя.Подразделение
				   |				 Конец
	               |		ПО черныйСписок.Номенклатура = ТоварыНаСкладахОстатки.Номенклатура
	               |ГДЕ
	               |	ТоварыНаСкладахОстатки.КоличествоОстаток > 0
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	черныйСписок.Номенклатура,
	               |	черныйСписок.Артикул,
	               |	черныйСписок.Производитель,
...
Процедура ЗагрузитьОстаткиЧерныйСписок(Печатать = Ложь, ПроверитьЦены = Ложь) экспорт
	...
	Иначе
	//----------------нужен код поставщика и Производитель !?-----------------	
	//  ЗагрузитьОстаткиВРегистр(Табл2 , КонтрЧерныйСписок, ЛОЖЬ);
...
КонецПроцедуры	//Черный список

//+++ 07.08.2018 --- уже автоПодтвержденные заказы ---
функция получитьЗаказыЧернСписок(КонтрЧерныйСписок)
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	               ...
	               |	И ЕстьNull(НоменклатураИмпорт.ЧерныйЛист, ЛОЖЬ) = ЛОЖЬ
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	ЗаказПокупателя
	               |АВТОУПОРЯДОЧИВАНИЕ";
 ...
 Процедура ПроверкаЗаказовЧерныйСписок() Экспорт
	КодПоставщика = "П018280";
	КонтрЧерныйСписок = справочники.Контрагенты.НайтиПоКоду(КодПоставщика);
	масЗаказов = получитьЗаказыЧернСписок(КонтрЧерныйСписок);
	...
	для ii=0 по масЗаказов.Количество()-1 цикл
		...
		закОб.Статус = перечисления.СтатусыЗаказов.Получен; //из Подтвержден >> Все товары есть на складе
        //нельзя чистить статус, так как заново закажется у поставщика, если из остатков не убрано!
		...
		//сами удаляем товары в ожидании по Заказу
...
//		Артикул: 95599405   Брэнд: «GENERAL MOTORS»  (через пробел)   //ID: 185318929 
//		Артикул: 95599405   Брэнд: «GENERALMOTORS» (слитно!)		  //ID: 183832837
...
Запрос.Текст = "ВЫБРАТЬ
               ...
               |	И КомплектующиеНоменклатуры.Номенклатура.ПутьКфайламСПрограммой = """"
               |
               |СГРУППИРОВАТЬ ПО
               |	НоменклатураИмпорт.Номенклатура";
...
Процедура ЕжедневнаяЗагрузкаЦенИОстатков() экспорт
	...
	//АВС регион
	если ЗагружатьОстаткиИЦеныПоставщика(Справочники.Контрагенты.НайтиПоКоду("95695")) Тогда      //17.07.2018 П005499>>95695
		Попытка
			AVSЦены(Ложь,Ложь);
		Исключение
		КонецПопытки;
	...
	Если ЗагружатьОстаткиИЦеныПоставщика(Справочники.Контрагенты.НайтиПоКоду("92532")) Тогда //14.05.2018 - Тестовый (Vissol кованные)
	...
	//"Диски на проверке" 07.08.2018 - чёрный список
	если ЗагружатьОстаткиИЦеныПоставщика(Справочники.Контрагенты.НайтиПоКоду("П018280")) Тогда
		Попытка
			ЗагрузитьОстаткиЧерныйСписок(Ложь,Ложь);
		Исключение
		КонецПопытки;
	КонецЕсли;
...
Процедура ЗагрузитьОстаткиЛингЛонг (Печатать = Ложь, ПроверитьЦены = Ложь) Экспорт
	...
	//яч = 12;
	//для кк=КолонкаКоличества по 40 цикл
...
Процедура ЗагрузитьОстаткиПирелли(Печатать = Ложь, ПроверитьЦены = Ложь) Экспорт	
	...
	//
	//получаем файл:
	ФайлПриемник = "C:\Остатки_склада_Пирелли.xls"; 
	///
...
Процедура AVSЦены(Печатать = Ложь, ПроверитьЦены = Ложь)
	...
	Контрагент = Справочники.Контрагенты.НайтиПоКоду("95695"); //17.07.2018 АВС регион П005499 >> Кайлас 95695
...
Процедура ПроверитьНоменклатуруФайлПоставщика(Поставщик) Экспорт
	...
	ИначеЕсли Поставщик = Справочники.Контрагенты.НайтиПоКоду("95695") Тогда  //17.07.2018 AVS  П005499 >> 95695
