...
Запрос.Текст = "ВЫБРАТЬ
               ...
               |	ВложенныйЗапрос.Номенклатура,
               |	СУММА(ВложенныйЗапрос.Цена * ЕСТЬNULL(ПравилаУстановкиБонусовПоставщиковСрезПоследних.ПроцентБонуса, 0) / 100) КАК Бонус,
               |	МАКСИМУМ(ВложенныйЗапрос.Цена) КАК Цена,
               |	СУММА(ЕСТЬNULL(ПравилаУстановкиБонусовПоставщиковСрезПоследних.ПроцентБонуса, 0)) КАК ПроцентБонуса
               |ИЗ
               |	(ВЫБРАТЬ
               ...
               |		ИЗ
               |			РегистрСведений.ПравилаУстановкиБонусовПоставщиков.СрезПоследних(&ДатаДок, //УсловиеОтбора
			   |				) КАК ПравилаУстановкиБонусов
               |		ГДЕ
			   //----------------------- 0 как выключатель -----------------------
               ...
               |СГРУППИРОВАТЬ ПО
               |	ВложенныйЗапрос.Номенклатура
			   |"+?(ТолькоНеНулевые, "ИМЕЮЩИЕ
			   |	Сумма( ЕстьNull(ПравилаУстановкиБонусовПоставщиковСрезПоследних.ПроцентБонуса,0) ) <> 0","");
	...
	Если ТипЗнч(ДокСсылка) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "//УсловиеОтбора", "БонусОтПродаж=ЛОЖЬ И Контрагент = &Контрагент");
		Запрос.УстановитьПараметр("Контрагент", ДокСсылка.Контрагент);
	Иначе //Если ТипЗнч(ДокСсылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг") тогда //без ограничений по ИНН или первые 3 цифры
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "//УсловиеОтбора", "БонусОтПродаж=Истина И (ИНН = """" или ИНН = &ИНН) ");
