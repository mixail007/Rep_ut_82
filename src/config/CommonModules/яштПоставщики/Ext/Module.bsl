...
Запрос.Текст = "ВЫБРАТЬ
               ...
               |		ИЗ
               |			РегистрСведений.ПравилаУстановкиБонусовПоставщиков.СрезПоследних(&ДатаДок, БонусОтПродаж//УсловиеОтбора
	...
	Если ТипЗнч(ДокСсылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг") тогда //без ограничений по ИНН или первые 3 цифры
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "//УсловиеОтбора", " И (ИНН=&ИНН или ИНН="""") "); // необязательный параметр!
	...
	Иначе//Если ТипЗнч(ДокСсылка) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "//УсловиеОтбора", "=ЛОЖЬ И Контрагент=&Контрагент");
