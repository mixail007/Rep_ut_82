...
Если Печатать = Ложь и ПроверитьЦены = Ложь  тогда 
	для i=0 по масТов.Количество()-1 цикл
		...
		стр2.Производитель = товар.Производитель;
...
функция получитьЗаказыБерга()
	...
	|				И ЗаказПокупателя.Статус = &Статус) КАК ЗаказыПокупателейОстатки";
    Берг = справочники.Контрагенты.НайтиПоКоду("П000727");
...
процедура ПроверкаЗаказовБерг()
	масЗак = получитьЗаказыБерга();
	
	для ii=0 по масЗак.Количество()-1 цикл
		зак1 = масЗак[ii];
...
резОтвет = "";
 ...
 ДатаОтгрузки = ?(зак1.ДатаОтгрузки='00010101', КонецДня(ТекущаяДата())+1, зак1.ДатаОтгрузки);
 
 dispatch_at = формат(Год(ДатаОтгрузки),"ЧГ=0")+"-"+формат(месяц(ДатаОтгрузки),"ЧЦ=2; ЧВН=")+"-"+формат(день(ДатаОтгрузки),"ЧЦ=2; ЧВН=");
 dispatch_time=1;    //1-до 15-00, 2 - после 15-00

 shipment_address_id="90255";   //# 1:  телефоны: 89201280590/89201276067
 person = "Robot1C"; 	// "Наталья/Валерий"
Коммент = "TEST_order: "+строка(Зак1.Номер);  
is_test ="0";  //+++ В ЖИВУЮ +++ !
...
//+++ 22.05.2018 - грузим с розницы... пока нет других данных
		//Файл = "/RU_TC.csv";
