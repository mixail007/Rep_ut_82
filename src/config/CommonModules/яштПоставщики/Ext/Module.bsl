...
//==========================Последовательность разделения - ФИКСИРОВАННАЯ ============================			
			...
			Если ДеньНедели( ТекущаяДата() ) >= 5 тогда //07.04.2018  - в пятницу и в субботу! заказы могут разделится на 2 части: на тек.день и на понедельник
...
//---- проверка необходимости согласования литых дисков с Егором---------------------
	Если НЕ Отказ тогда 
		...
		Запрос.Текст = "ВЫБРАТЬ
					   ...
					   |		ПО ЗаказыПокупателейОстатки.Номенклатура = ВТ.Номенклатура
					   |ГДЕ
					   |	ВТ.Номенклатура ЕСТЬ NULL 
					   |
					   |СГРУППИРОВАТЬ ПО
					   |	ЗаказыПокупателейОстатки.Номенклатура";
