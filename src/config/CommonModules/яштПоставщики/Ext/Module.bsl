...
Процедура ЗагрузитьОстаткиМишлен(Печатать = Ложь, ПроверитьЦены = Ложь) Экспорт
	...
	Исключение//01.02.2019
		ВызватьИсключение строка(Мишлен)+": Не существует файла: "+Файл1+" на FTP://shop.tyreplus.ru";
	КонецПопытки;
...
функция СуммаЗаказовВЭтойЖЕТочке(ЗакОб)
	СуммаДрЗаказов = 0;
	 ...
	 Запрос.Текст = "ВЫБРАТЬ
	                ...
	                |	СУММА(ЗаказыПокупателей.КоличествоОстаток) как Количество,
	 ...
	 Запрос.УстановитьПараметр("АдресДоставки", лев(ЗакОб.АдресДоставки,500) ); //адрес в точности совпадает...не более 500 символов
	 ...
	 Если НЕ результат.Пустой() тогда
		табл = Результат.Выгрузить();
			//02.04.2019 - бывает Cумма=Null? BugFix: Преобразование значения к типу Число не может быть выполнено
	        СуммаДрЗаказов = ?(ЗначениеЗаполнено(табл[0].Сумма), табл[0].Сумма, 0);
	 КонецЕсли;
