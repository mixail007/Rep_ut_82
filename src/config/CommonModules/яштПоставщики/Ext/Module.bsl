...
Процедура АвтоОбъединениеЗаказовПокупателей() Экспорт
    ...
    выборкаДоговор = ПолучитьЗаказыДляОбъединения(списКонтр);
				...
				попытка //могут быть разные типы
					...
					Иначе //20.02.2018 - другой адрес... в следующую строку
						...
						Если стрТабл1 = неопределено тогда
							...
							стрТабл1.ВидАдресаДоставки = ВидАдресаДоставки0;
...
Процедура РазделитьТаблицуПоСезоннымЗаказам( таблСписЗак )
	...
	Для i=0 по N-1 цикл
		...
		пока ii<Nn цикл //--------------------цикл по заказам-----------------------
			...
			//ищем такую строку.... 
				стрТаблСтроки = таблСписЗак.НайтиСтроки( Новый Структура("ВидАдресаДоставки, СезЗакТек", стр1.ВидАдресаДоставки, СезЗакТек) );
				...
				стр1.СписЗак.Удалить(зак1); //удаляем из тек.строки
                Nn=Nn-1;
				продолжить;
			КонецЕсли;	
