...
функция ПолучитьЧерныйСписок(КонтрЧерныйСписок)
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   //есть остатки в Ростове! иначе - пустое!
	               |	ТоварыНаСкладахОстатки.Склад.Подразделение КАК Подразделение
	               |ИЗ
	               |	РегистрСведений.НоменклатураИмпорт КАК НоменклатураИмпорт
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыНаСкладах.Остатки(, Склад.ЗапретитьИспользование = ЛОЖЬ) КАК ТоварыНаСкладахОстатки
	               |		ПО НоменклатураИмпорт.Номенклатура = ТоварыНаСкладахОстатки.Номенклатура
	               |ГДЕ
	               ...
	               |СГРУППИРОВАТЬ ПО
	               |	НоменклатураИмпорт.Номенклатура,
	               |	НоменклатураИмпорт.Номенклатура.Код,
	               |	НоменклатураИмпорт.Номенклатура.Производитель,
