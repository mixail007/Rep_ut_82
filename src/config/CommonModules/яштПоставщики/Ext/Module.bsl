...
//Пауэр
//11.11.2016 Смирнов
Процедура ПауэрЗагрузкаОстатков() Экспорт
...
Функция ЗагрузитьЦеныПауэр() экспорт
	Пауэр = Справочники.Контрагенты.НайтиПоРеквизиту("ИНН", "7703247653");
...
Процедура ТулеЗагрузкаЦенИОстатков() 
	...
	ИмяФайлаИсточника = "c:\Thule.xml";
    ЧтениеXML = Новый ЧтениеXML(); 
	ЧтениеXML.ОткрытьФайл(ИмяФайлаИсточника); 
	
	тзЦены=новый ТаблицаЗначений;
	тзЦены.Колонки.Добавить("Артикул",Новый ОписаниеТипов("Строка",,,,Новый КвалификаторыСтроки(20)));
	тзЦены.Колонки.Добавить("Наименование",Новый ОписаниеТипов("Строка",,,,Новый КвалификаторыСтроки(200)));
	...
	Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
		попытка
			Если ЧтениеXML.Имя = "table:table-cell" Тогда // шапка
				если ЧтениеXML.ЗначениеАтрибута("table:style-name")="ce9" тогда
					ЧтениеXML.Прочитать();   ЧтениеXML.Прочитать();
					нстр = тзЦены.Добавить();
					нстр.Артикул = ЧтениеXML.Значение;
				КонецЕсли;
				если ЧтениеXML.ЗначениеАтрибута("table:style-name")="ce10" тогда
					ЧтениеXML.Прочитать();   ЧтениеXML.Прочитать();
					нстр.Наименование = ЧтениеXML.Значение;
				КонецЕсли;
			
				если ЧтениеXML.ЗначениеАтрибута("table:style-name")="ce25" тогда
				ценаСтр =ЧтениеXML.ЗначениеАтрибута("office:value");
				нстр.Цена = Число(ценаСтр);
				КонецЕсли;
			
				если ЧтениеXML.ЗначениеАтрибута("table:style-name")="ce32" тогда
				нстр.ШтрихКод     = ЧтениеXML.ЗначениеАтрибута("office:value");
				КонецЕсли;
			
				если ЧтениеXML.ЗначениеАтрибута("table:style-name")="ce34" тогда
				КолСтр = ЧтениеXML.ЗначениеАтрибута("office:value");
				нстр.Количество   = число(КолСтр);
				КонецЕсли;
			КонецЕсли;
		исключение
			#Если клиент тогда
...
#Если Клиент тогда
	сообщить("Получена таблица из "+строка(тзЦены.Количество())+" строк."); 
 #КонецЕсли	 
...
//--------------------загрузка в регистры-----------------
 ...
 Запрос.Текст = "ВЫБРАТЬ
                ...
                |
				///////////////////// ДОПОЛНИТЕЛЬНАЯ ТАБЛИЦА СООТВЕТСТВИЯ /////////////////////
				//|	ОБЪЕДИНИТЬ ВСЕ
				//|	
				//|	ВЫБРАТЬ
				//|		номКлиента.АртикулНоменклатурыКонтрагента,
				//|		номКлиента.Номенклатура
				//|	ИЗ
				//|		РегистрСведений.НоменклатураКонтрагентов КАК номКлиента
				//|	ГДЕ
				//|		номКлиента.Контрагент = &Контр
				|) КАК спрНоменклатура
 ...
 Если табл2.Количество()>0 тогда
	 ...
	 #КонецЕсли	 
	//--------------------загрузка цен------------
	регСв = РегистрыСведений.ЦеныНоменклатурыПоставщиков.СоздатьНаборЗаписей();
	регСв.Отбор.Контрагент.ВидСравнения = ВидСравнения.Равно;
	регСв.Отбор.Контрагент.Значение = Туле;
	регСв.Отбор.Контрагент.Использование = истина;
	регСв.Записать(); //чистим старое
	регСв.Загрузить( табл2 );
	регСв.Записать();
	//------------------загрузка остатков------------
	регСв = РегистрыСведений.ОстаткиНоменклатурыКонтрагентов.СоздатьНаборЗаписей();
	регСв.Отбор.Контрагент.ВидСравнения = ВидСравнения.Равно;
	регСв.Отбор.Контрагент.Значение = Туле;
	регСв.Отбор.Контрагент.Использование = истина;
	регСв.Записать(); //чистим старое
...
Процедура AVSЦены(Печатать = Ложь, ПроверитьЦены = Ложь)
	...
	иначе
		...
		//Для каждого СтрокаТЗ ИЗ ТабЦеныОстатки Цикл
		//	//Цены номенклатуры поставщиков
		//		МенеджерЗаписи = РегистрыСведений.ЦеныНоменклатурыПоставщиков.СоздатьМенеджерЗаписи();
		//		МенеджерЗаписи.ДатаУстановки = ТекущаяДата();
		...
		//
		//
		//НаборЗаписей.Загрузить( ТабЦеныОстатки );
		//НаборЗаписей.Записать();  //новые данные	
...
Процедура АлькорЦены(Печатать = Ложь, ПроверитьЦены = Ложь) Экспорт
	...
	иначе	
		...
		ЗагрузитьОстаткиВРегистр(ТЗ, Алькор);
...
Процедура ЗагрузитьЦеныВРегистр(ТЗ,Контрагент)
	...
	Если ТЗ.Количество()>0 Тогда		
		...
		//НаборОстатки.Отбор.Подразделение.Установить(Справочники.Подразделения.НайтиПоКоду("00005"));
...
Процедура ЗагрузитьЦеныВДокумент(ТабЦеныОстатки,Контрагент)
	...
	ТЗЦены.Свернуть("Номенклатура,Цена");
	...
	НовЦена.ТипЦен = Справочники.ТипыЦенНоменклатуры.НайтиПоКоду("00001");
	
	НовЦена = НовыйДокумент.ТипыЦен.Добавить();
	НовЦена.ТипЦен = Справочники.ТипыЦенНоменклатуры.НайтиПоКоду("00005");
	
	Для каждого стр из ТЗЦены Цикл
		
		НаборНаценки = РегистрыСведений.НаценкаПоПроизводителямДляАксов.СоздатьНаборЗаписей();
		НаборНаценки.Отбор.Контрагент.Установить(Контрагент);
		НаборНаценки.Отбор.Производитель.Установить(Стр.Номенклатура.Производитель);
		НаборНаценки.Прочитать();
		
		если НаборНаценки.Количество()>0 Тогда
			...
			НовТовар.Цена = Окр(НаборНаценки[0].Процент * стр.Цена / 100 + стр.Цена);
