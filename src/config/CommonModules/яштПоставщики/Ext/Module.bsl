...
Процедура ЗагрузитьОстаткиТуле(Печатать = Ложь, ПроверитьЦены = Ложь)
	...
	Если НЕ файл1.Существует() тогда
		ВызватьИсключение "Пирелли: Не существует файла "+ИмяФайла;
	КонецЕсли;
...
Процедура ЗагрузитьОстаткиПирелли(Печатать = Ложь, ПроверитьЦены = Ложь) Экспорт	
	...
	Если списокФайлов.Количество()=0 тогда  
		...
		ИначеЕсли ДеньНедели(ТекущаяДата()) = 1 тогда //в понедельник - пятничный
			КолДней = 3;
		КонецЕсли;	
		текДата = текДата - КолДней*86400;
		
		дд = формат(день(текДата),"ЧЦ=2; ЧВН="); //вчерашний прайс!
		мм = формат(месяц(текДата),"ЧЦ=2; ЧВН=");
		гг = прав( формат(год(текДата),"ЧЦ=4; ЧГ=0"), 2);
		// ИмяФайлаЗагрузки = "FREE_Stock_Pirelli "+дд+"_"+мм+"_"+гг+"_Summer_Winter - RUS.XLSX";
...
//-----------требуется конвертация!-----------------		
		...
		попытка
			...
			ЗапуститьПриложение("D:\1c\Pirelli\Pirelli_Convert.bat");
			//ИмяФайлаЗагрузки = "-Unlicensed-Stock_Pirelli.xls";
		исключение
		    ВызватьИсключение "Ошибка при Конвертации файла Пирелли XLSX !";
		КонецПопытки;
	КонецЕсли;	
