...
Процедура АвтоОбъединениеЗаказовПокупателей() Экспорт
    ...
    ДатаОтгрузки7 = НачалоДня(ДатаОтгрузки) - 7*86400;
...
Процедура АвтоПостановкаЗаказовВМаршрут() Экспорт
	...
	Пока выборкаЗак.Следующий() Цикл //послеовательно по клиенту
		...
		ОбЗак.Дата = ТекущаяДата(); //+++ 22.03.2018 сдвиг документа на тек.дату и проверка тоже на тек.дату!
		ОбЗак.ДатаОтгрузки = ТекущаяДата();  //ставим на сегодня!
		
		Если ПроверитьЗаказПередУстановкойВМаршрут(ОбЗак) Тогда
			#Если Клиент тогда
				Сообщить(" Нельзя устанавливать в маршрут заказ "+обЗак.Номер+" ("+строка(обЗак.Контрагент)+")", СтатусСообщения.Внимание );
			#КонецЕсли	
			продолжить;
		Иначе
...
Функция ПолучитьЗаказыДляПостановкиВМаршрут(списКонтр, Подразделение=Неопределено)
	 ...
	 Запрос.Текст = "ВЫБРАТЬ
					...
					//+++ 22.03.2018 - пред.даты тоже надо, но не "старше" 2х недель
	                |		И ЗаказПокупателя.ДатаОтгрузки <= &ДатаОтгрузки
					...
					//Клиент или менеджер ОБЯЗАТЕЛЬНО должен поставить [v] к отгрузке!
...
Процедура ВыполнитьОбъединениеЗаказовПокупателей(СписокЗаказов)
	...
	докОбщийЗаказ.ДатаОтгрузки = макс( НачалоДня(ТекущаяДата()), НачалоДня(ДатаОтгрузки1) );
	докОбщийЗаказ.ДатаОплаты   = макс( НачалоДня(ТекущаяДата()), НачалоДня(ДатаОплаты1) );
