...
Функция ПроверитьЗаказПередУстановкойВМаршрут(обДок) 
	...
	Если не ОбДок.ПоставитьНаОтгрузку и ОбДок.ТипЗаказа = 8 тогда
		#Если Клиент Тогда 
		сообщить("Для Заказа "+обДок.Номер+" (с сайта) выключена [ ] На отгрузку!", СтатусСообщения.Внимание);
		#КонецЕсли
		Возврат Истина;
	КонецЕсли;
	...
	Если  обДок.Подразделение.ОбособленноеПодразделение Тогда
		#Если Клиент Тогда 
		сообщить("Для Заказа "+обДок.Номер+" Автопостановка в Маршрут - включена только по Ярославским заказам!", СтатусСообщения.Внимание);
		#КонецЕсли
		Возврат истина;
	КонецЕсли;
	...
	Если обДок.Резерв или обДок.ДатаОтгрузки='00010101' Тогда 
		...
		#КонецЕсли
       Возврат истина;//01.12.2016 запрет!
	КонецЕсли;
