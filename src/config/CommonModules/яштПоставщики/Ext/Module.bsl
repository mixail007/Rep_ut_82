...
//==========================Последовательность разделения - ФИКСИРОВАННАЯ ============================			
			...
			//08.05.2018  - в пятницу и в субботу! заказы могут разделится на 2 части: на тек.день и на понедельник
...
//02.04.2015 не проделенный или Резервный заказ - можно только посчитать R13
	...
	Если ОбДок.Товары.Итог("Количество")<4 тогда
		НетДругихЗаказов = НайтиЗаказыВМаршруте(ОбДок.Контагент, ОбДок.Подразделение); //+++ 10.05.2018 Задача № 46148 п.2
...
функция НайтиЗаказыВМаршруте(Контр, Подр)
	нетЗаказовВМаршруте = Истина;
	 Запрос = Новый Запрос;
	 Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
	                |	ЗаказыПокупателей.ЗаказПокупателя,
	 ...
	 Запрос.УстановитьПараметр("ДатаОтгрузки", ДатаОтгрузки );
	 Результат = Запрос.Выполнить();
	 нетЗаказовВМаршруте = результат.Пустой();
	 
	возврат нетЗаказовВМаршруте;
КонецФункции	
