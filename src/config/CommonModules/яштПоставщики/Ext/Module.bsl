...
Процедура АвтоОбъединениеЗаказовПокупателей() Экспорт
  ...
  Пока счДней<=ЧислоДнейВперед цикл
	...
	Пока выборкаДоговор.Следующий() Цикл
		...
		//28.03.2018 - разделяем и объединяем заказы по СрокуОплаты
		//ИначеЕсли выборкаДоговор.СрокОплаты>1 тогда
		//	текстОшибки1 = "Нельзя объединить "+строка( выборкаДоговор.ЗаказПокупателя )+" заказа ("+строка(Контр1)+") по "+строка(выборкаДоговор.СрокОплаты)+" различным срокам оплаты!"
			
		//28.03.2018 - разделяем и объединяем 4 различных Способа = Самовывоз*1 + по293*2 	
		//ИначеЕсли выборкаДоговор.СпособОтгрузки>1 тогда
		//	ТекстОшибки1 = "Нельзя объединить "+строка( выборкаДоговор.ЗаказПокупателя )+" заказа по "+строка(выборкаДоговор.СпособОтгрузки)+" способам отгрузки (Самовывоз нельзя смешивать с Обычными отргузками)!";
		КонецЕсли;	
...
//==========================Последовательность разделения - ФИКСИРОВАННАЯ ============================			
			...
			Если выборкаДоговор.ДоговорКонтрагента.Владелец.НеОбъединятьРазныеТовГруппы тогда
				...
				//5+1 колонки:  ВидАдресаДоставки, СезЗак, СпособОтгрузки, СрокОплаты, ВидТовара; списЗак ========================
				флЕстьТовары = Истина;
			//Иначе	
				//4+1 колонки:  ВидАдресаДоставки, СезЗак, СпособОтгрузки, СрокОплаты; списЗак ========================
			КонецЕсли;	

			Если ДеньНедели( ТекущаяДата() ) = 5 тогда //07.04.2018  - ТОЛЬКО в пятницу! заказы могут разделится на 2 части
				РазделитьТаблицуПоСвойствуДняОтгрузки( таблСписЗак );
			КонецЕсли;
			
		//20.02.2018 ---- п.8 - цикл по адресам ---------------------------
			для каждого стр1 из таблСписЗак цикл
				
				списЗак = стр1.списЗак; //09.04.2018
				...
				ИначеЕсли списЗак.Количество()=1 тогда // один заказ НЕ НАДО объединять!
					...
					+?(флЕстьТовары, ", Только "+строка(стр1.ВидТовара), "")
				...
				ВыполнитьОбъединениеЗаказовПокупателей(стр1);
			КонецЦикла;
...
Процедура РазделитьТаблицуПоСвойствуДняОтгрузки( таблСписЗак )
	...
	Для i=0 по N-1 цикл
		...
		Nn = стр1.СписЗак.Количество();
		Выборка1 = ПолучитьЗаказыПоСвойству( стр1.СписЗак );
		
		выборка1.Следующий();
		стр1.Понедельник = выборка1.Значение;
			если выборка1.Количество()>1 тогда
				
				стр1.СписЗак.Очистить();
			    выборка2 = выборка1.выбрать();
				пока выборка2.следующий() цикл
				стр1.СписЗак.добавить( выборка2.зак );
				КонецЦикла;
			
			    выборка1.Следующий();  //всего 2 значения!
				стр2 = таблСписЗак.Добавить();
				ЗаполнитьЗначенияСвойств(стр2, стр1);
				   стр2.СписЗак.Очистить();
			    выборка2 = выборка1.выбрать();
				пока выборка2.следующий() цикл
...
табл.Колонки.Добавить("Зак");
для i=0 по СписЗак1.Количество()-1 цикл
	стр1 = табл.Добавить();
	стр1.Зак = СписЗак1[i].Значение;
КонецЦикла;	
...
Запрос.Текст = "ВЫБРАТЬ
               ...
               |ВЫБРАТЬ
               |	табл.Зак КАК Зак,
               |	ЕСТЬNULL(Свва.Значение, ЛОЖЬ) КАК Значение
               |ИЗ
               |	ВТ_табл КАК табл
               |		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
               |			ЗначенияСвойствОбъектов.Объект КАК зак,
               |			ЗначенияСвойствОбъектов.Значение КАК Значение
               |		ИЗ
               |			РегистрСведений.ЗначенияСвойствОбъектов КАК ЗначенияСвойствОбъектов
               |		ГДЕ
               |			ЗначенияСвойствОбъектов.Свойство = &Свойство
               |			И ЗначенияСвойствОбъектов.Объект В(&СписЗак1) )КАК Свва
               |		ПО табл.Зак = Свва.зак
               |ИТОГИ
               |	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Значение)
               |ПО
               |	Значение";
свво = ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоКоду("90222");  //Понедельник
Запрос.УстановитьПараметр("свво", свво );
...
Процедура АвтоПостановкаЗаказовВМаршрут() Экспорт
	...
	Пока выборкаЗак.Следующий() Цикл //послеовательно по клиенту
		...
		//ОбЗак.ДатаОтгрузки = ТекущаяДата();  //ставим на сегодня или завтра !!!
...
//- аналог 246 
Процедура ВыполнитьОбъединениеЗаказовПокупателей(стр1)
	
		СписокЗаказов = новый СписокЗначений;
		СписокЗаказов.ЗагрузитьЗначения( стр1.списЗак.ВыгрузитьЗначения() ); //через значения!
...
//02.04.2015 не проделенный или Резервный заказ - можно только посчитать R13
	...
	Если ОбДок.Товары.Итог("Количество")<4 тогда
		Текст1 ="Заказ "+обДок.Номер+" - слишком маленький! Количество <4 шт.";
		Отказ = Истина;
	ИначеЕсли ОбДок.Товары.Итог("Количество")>4 и ОбДок.СуммаДокумента<10000 тогда	//06.04.2018
		Текст1 ="Заказ "+обДок.Номер+" - слишком маленький! Количество: "+строка(ОбДок.Товары.Итог("Количество"))+", а Сумма <10 тыс. руб.";
		Отказ = Истина;
	КонецЕсли;	
...
КонецЕсли;
	...
	Если НЕ Отказ Тогда 
		...
		Если ЛитыеДиски и РезАдреса.Количество()>0 и не обДок.РазрешитьГрузитьДиски Тогда
			Текст1 = "Согласуйте заказ "+ОбДок.Номер+" у Егора Малышева!!!";
			Отказ = истина;
		КонецЕсли;
	КонецЕсли;
	...
	Если Отказ И Текст1<>"" тогда //+++ 27.03.2018 - единая функция 
		#Если Клиент Тогда
			Сообщить(Текст1, СтатусСообщения.Внимание);
        #КонецЕсли
