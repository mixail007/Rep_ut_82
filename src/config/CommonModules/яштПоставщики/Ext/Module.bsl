...
Процедура ЗагрузитьОстаткиБерг(Печатать = Ложь, ПроверитьЦены = Ложь) 
...
//+++ 19.11.2014+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Процедура ЗагрузитьОстаткиАйсАвто(ЗагрузкаСFTP=Истина) Экспорт
...
Процедура ЕжедневнаяЗагрузкаЦенИОстатков() экспорт
	...
	для i=0 по СписКонтр.Количество()-1 цикл
		...
		Если флНадоГрузить тогда
			...
			Исключение
				текстИскл = "ОШИБКА Загрузки цен и Остаток для контрагента: "+СписКонтр[i].Представление+" : "+ОписаниеОшибки(); 
				#Если Клиент тогда
...
Процедура ПроверитьНоменклатуруФайлПоставщика(Поставщик) Экспорт
	...
	ИначеЕсли Поставщик = Справочники.Контрагенты.НайтиПоКоду("95695") Тогда  //17.07.2018 AVS  П005499 >> 95695
		...
		ЗагрузитьОстаткиКайлас(Истина,Ложь);// AVSЦены(Истина,Ложь);
		
	ИначеЕсли Поставщик = Справочники.Контрагенты.НайтиПоКоду("П000727") Тогда //14.05.2018 - Берг Холдинг
		
		ЗагрузитьОстаткиБерг(Истина,ЛОЖЬ); 
	
	ИначеЕсли Поставщик = Справочники.Контрагенты.НайтиПоКоду("П003301") Тогда     //Алькор
		
		ЗагрузитьОстаткиАлькор(Истина,Ложь);//АлькорЦены(Истина,Ложь);
...
Процедура ПроверитьЦеныФайлПоставщика(Поставщик) Экспорт
	...
	ИначеЕсли Поставщик = Справочники.Контрагенты.НайтиПоКоду("95695") Тогда  //17.07.2018 AVS П005499 >> 95695
		...
		ЗагрузитьОстаткиКайлас(Ложь,Истина); //AVSЦены(Ложь,Истина);
			
	ИначеЕсли Поставщик = Справочники.Контрагенты.НайтиПоКоду("П000727") Тогда //14.05.2018 - Берг Холдинг
		
		ЗагрузитьОстаткиБерг(ЛОЖЬ,Истина);
		
	ИначеЕсли Поставщик = Справочники.Контрагенты.НайтиПоКоду("П003301") Тогда     //Алькор
		
		ЗагрузитьОстаткиАлькор(Ложь,Истина);
