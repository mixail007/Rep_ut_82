...
Процедура ВыполнитьОбъединениеЗаказовПокупателей(стрТабл1, сдвигДатыОтгрузки=0)
	...
	Попытка		
		...
		Если докОбщийЗаказ.Конрагент.НеОбъединятьРазныеТовГруппы тогда //11.05.2018
...
функция НайтиЗаказыВЭтойЖЕТочке(ЗакОб)
	нетЗаказовНаТуЖеТочку = Истина;
	 ...
	 Запрос.Текст = "ВЫБРАТЬ
					...
					|				   ИЛИ ЗаказПокупателя.АдресДоставки = &АдресДоставки)
	 ...
	 Запрос.УстановитьПараметр("АдресДоставки", ЗакОб.АдресДоставки ); //адрес в точности совпадает... скопировали...
