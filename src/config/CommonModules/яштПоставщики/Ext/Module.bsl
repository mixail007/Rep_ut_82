...
Процедура ЗагрузитьОстаткиПирелли(Печатать = Ложь, ПроверитьЦены = Ложь) Экспорт	
	...
	// 22.01.2019 ---  загрузка прайса текущего прайса - через почту... 
	текДата = НачалоДня(ТекущаяДата()); // последний параметр - чисое имя!
	списокФайлов = яштЗаказыСПочты.ПолучитьФайлыСПочты("suppliers@yst.ru", "\\sigma\c$\", "ContactCenter.Russia@pirelli.com", ЛОЖЬ, текДата, КонецДня(текДата), ЛОЖЬ ); 
	
	Если списокФайлов.Количество()=0 тогда //вчерашний прайс! 
		дд = формат(день(текДата)-1,"ЧЦ=2; ЧВН=");
	...
	КонецЕсли;	
	МаксНомЛиста=2;
	//ИмяФайла = "Остатки_склада_Пирелли.xls"; 
	//МаксНомЛиста=1;
	ФайлПриемник = "C:\"+ИмяФайла;
