...
Процедура ЗагрузитьОстаткиЛингЛонг(Печатать = Ложь, ПроверитьЦены = Ложь) Экспорт
	...
	Если ДеньНедели(ТекущаяДата())=1 тогда  //+++ 28.01.2019 - в понедельник - загрузка не нужна!
		возврат 
	КонецЕсли;	
	предДень = НачалоДня(ТекущаяДата())- 1; 
...
Процедура ЗагрузитьОстаткиПирелли(Печатать = Ложь, ПроверитьЦены = Ложь) Экспорт	
	...
	Если списокФайлов.Количество()=0 тогда  
		возврат;  //+++ 28.01.2019 - загрузка предыдущих прайсов - не нужна!
		
		дд = формат(день(текДата)-1,"ЧЦ=2; ЧВН="); //вчерашний прайс!
