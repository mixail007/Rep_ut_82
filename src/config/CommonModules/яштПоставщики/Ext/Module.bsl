...
Процедура СписаниеДенегСЗакрытыхСезонныхЗаказов() Экспорт
	...
	Запрос.Текст = 
		...
		|								Сделка В (ВЫБРАТЬ ВТ.Сделка ИЗ ВТ КАК ВТ)     ) КАК ВзаиморасчетыСКонтрагентамиОбороты
	...
	Пока ВыборкаДоговорКонтрагента.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			НовСтр.КурсВзаиморасчетов = Окр(?(Выборка.ВалютаПриход=0, 1, Выборка.РублиПриход/Выборка.ВалютаПриход),4);
			НовСтр.СуммаУпр = Выборка.СуммаВзаиморасчетовОстаток * НовСтр.КурсВзаиморасчетов;
