...
Процедура ЗагрузитьОстаткиПирелли(Печатать = Ложь, ПроверитьЦены = Ложь) Экспорт	
    ...
    //яштЗаказыСПочты.ПолучитьФайлыСПочты("suppliers@yst.ru", "\\sigma\c$\", "contactcenter.russia@pirelli.com", ЛОЖЬ, неопределено, неопределено);
	// ИмяФайла = FREE_Stock_Pirelli_29 05_18_Summer_Winter - RUS.XLSX
	//
	//получаем файл:
	ИмяФайла = "C:\Остатки_склада_Пирелли.xls"; 
	///
