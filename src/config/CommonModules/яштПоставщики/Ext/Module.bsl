  ...
  Пока счДней<=ЧислоДнейВперед цикл
	...
	Пока выборкаДоговор.Следующий() Цикл
		...
		ИначеЕсли выборкаДоговор.СрокОплаты>1 тогда
			текстОшибки1 = "Нельзя объединить "+строка( выборкаДоговор.ЗаказПокупателя )+" заказа по "+строка(выборкаДоговор.СрокОплаты)+" различным срокам оплаты!"
		...
		//20.02.2018 ---- п.8 - цикл по адресам ---------------------------
			для каждого стр1 из таблСписЗак цикл 
				...
				ИначеЕсли списЗак.Количество()=1 тогда // один заказ НЕ НАДО объединять!
					текстОшибки1 = строка(стр1.ВидАдресаДоставки)+": Один заказ "+строка(списЗак[0].Значение)+" - НЕ НАДО объединять!";
				...
				Если текстОшибки1<>"" тогда //+++ 27.03.2018
					...
					Сообщить(текстОшибки1,СтатусСообщения.Внимание);
					#КонецЕсли
					ЗаписатьЛОГвЖурналИзменений(Контр1, Событие1, ТекстОшибки1, счДней );
					продолжить;
				КонецЕсли;			
...
//02.04.2015 не проделенный или Резервный заказ - можно только посчитать R13
	...
	//+++27.03.2018
	Если ОбДок.Товары.Итог("Количество")<4  или ОбДок.СуммаДокумента<10000 тогда
		Текст1 ="Заказ "+обДок.Номер+" - слишком маленький! Количество <4 или Сумма <10 тыс. руб.";
		Отказ = Истина;
	КонецЕсли;	
...
КонецЕсли;
	...
	Если Отказ И Текст1<>"" тогда //+++ 27.03.2018 - единая функция 
		#Если Клиент Тогда
			Сообщить(Текст1, СтатусСообщения.Внимание);
        #КонецЕсли
		ЗаписатьЛОГвЖурналИзменений( обДок, "АвтоПостановкаВМаршрут", Текст1 );
	КонецЕсли;
...
процедура ЗаписатьЛОГвЖурналИзменений(Контр1, Событие1="", текст1 = "", НомЗаписи1 = 0 )
