...
функция ПолучитьЧерныйСписок(КонтрЧерныйСписок)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |				ЗаказПокупателя.Подразделение = &Ярославль
	...
	Запрос.УстановитьПараметр("Ярославль", справочники.Контрагенты.НайтиПоКоду("00005") );
	Результат = Запрос.Выполнить();
...
функция получитьЗаказыЧернСписок(КонтрЧерныйСписок, списТов=неопределено)
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	               ...
	               |	И НоменклатураИмпорт.ЧерныйЛист = ЛОЖЬ
   				   |//тов  И НоменклатураИмпорт.Номенклатура в (&СписТов)
 					|
	               |УПОРЯДОЧИТЬ ПО
	               |	ЗаказПокупателя
	               |АВТОУПОРЯДОЧИВАНИЕ";
