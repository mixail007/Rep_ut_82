...
функция СуммаЗаказовВЭтойЖЕТочке(ЗакОб)
	СуммаДрЗаказов = 0;
	 ...
	 Запрос.Текст = "ВЫБРАТЬ
					...
					//02.04.2019 - бывает?! пустая сумма BugFix: Преобразование значения к типу Число не может быть выполнено
	                |	СУММА( ЕстьNull(ЗаказыПокупателей.СуммаУпрОстаток,0) ) КАК Сумма
	                |ИЗ
	                |	РегистрНакопления.ЗаказыПокупателей.Остатки(,
