...
//ДЛЯ ПЛАНА ОБМЕНА УТ-ФА+ БП
Процедура ЗарегистрироватьОбъектФАП(Объект, Отказ, ЗаписьИзФормы, ПерезаписатьОбъект=Ложь) Экспорт
	...
	Если НЕ Метаданные.Документы.Найти(Объект.Метаданные().Имя) = Неопределено Тогда
		...
		Если Регистрировать Тогда
			...
			Запрос.Текст = 
			"ВЫБРАТЬ
			|	ПланОбменаУТ_БП_ФАП.Ссылка
			|ИЗ
			|	ПланОбмена.ПланОбменаУТ_БП_ФАП КАК ПланОбменаУТ_БП_ФАП
			|ГДЕ
			|	ПланОбменаУТ_БП_ФАП.Ссылка <> &ЭтотУзел"; 
			
			Запрос.УстановитьПараметр("ЭтотУзел", ПланыОбмена.ПланОбменаУТ_БП_ФАП.ЭтотУзел());
