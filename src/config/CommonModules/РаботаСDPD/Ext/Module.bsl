...
процедура ОформитьЗаявкуВDPD(Заказ) Экспорт
	...
	Если Данные.Получить("DPDЗабираетПоАдресу") тогда
		КодУслуги="Д";
		Если СокрЛП(Данные.Получить("ПоставщикГород"))="" или СокрЛП(Данные.Получить("ПоставщикУлица"))="" тогда
...
Функция ПодготовитьШапкуXML(Выборка,ДатаОтправки,аббр,ЗаказИМ = ложь,Заказ, тзПараметрыТК)
	...
	если  Выборка.Получить("ПоставщикАббрУлицы")<>"" тогда
		ЗаписьXML.ЗаписатьНачалоЭлемента("streetAbbr"); ЗаписьXML.ЗаписатьТекст(""+СтрЗаменить(ЗаписьXML.Получить("ПоставщикАббрУлицы"),".","")); ЗаписьXML.ЗаписатьКонецЭлемента();
