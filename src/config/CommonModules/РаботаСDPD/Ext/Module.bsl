...
Функция ПодготовитьДанные(Заказ,Данные,ЗаказИМ,Префикс)
	Данные.Вставить("Заказ через посредника", ложь);
	Если ЗаказИМ тогда
	...
	Если ЗаказИМ тогда
		...
		тек = ПолучитьЗначениеРеквизитаТК(табинфо,"Заказ через посредника");
		Данные.Вставить("Заказ через посредника", ?(ЗначениеЗаполнено(тек.Значение),тек.Значение,ложь));
...
Функция ДобавитьЗаказВDPD(Запись,Выборка,КодУслуги,аббр,Тариф,тзПараметрыТК,Заказ,Префикс = "YST",ОтправлятьДоки=ложь)
	...
	//Если Префикс = "KTY" тогда
		...
		Если  Выборка.получить("ОплаченБН") или Выборка.получить("Заказ через посредника") 
			
			//09.04.2019 Смирнов 
			//https://corp.yst.ru/task/1750/show если перемещение ОТХ то без НПП
			или Заказ.ДоговорКонтрагента.ТипДоговора = Справочники.ТипыДоговоров.ПеремещениеОТХ тогда
		иначе	
	...
	//09.04.2019 Смирнов 
	//https://corp.yst.ru/task/1750/show если перемещение ОТХ то можно частичный возврат
	Если  Заказ.ДоговорКонтрагента.ТипДоговора = Справочники.ТипыДоговоров.ПеремещениеОТХ тогда
		Запись.ЗаписатьНачалоЭлемента("extraService");
		Запись.ЗаписатьНачалоЭлемента("esCode");Запись.ЗаписатьТекст("ЧСТ"); Запись.ЗаписатьКонецЭлемента();
		//Запись.ЗаписатьНачалоЭлемента("param");
		//Запись.ЗаписатьНачалоЭлемента("name");Запись.ЗаписатьТекст("sum_npp"); Запись.ЗаписатьКонецЭлемента();
		//Запись.ЗаписатьНачалоЭлемента("value");Запись.ЗаписатьТекст(СтрЗаменить(СтрЗаменить(""+Заказ.СуммаДокумента," ",""),",",".")); Запись.ЗаписатьКонецЭлемента();
		//Запись.ЗаписатьКонецЭлемента();//param
	...
	//12.02.19 Смирнов Задача 63394 надо в каждом заказе добавлять двд(доставку в выходной день)
	Если Префикс="KTY" тогда
		Запись.ЗаписатьНачалоЭлемента("extraService");
		Запись.ЗаписатьНачалоЭлемента("esCode");Запись.ЗаписатьТекст("ДВД"); Запись.ЗаписатьКонецЭлемента();
		//Запись.ЗаписатьНачалоЭлемента("param");
		//Запись.ЗаписатьНачалоЭлемента("name");Запись.ЗаписатьТекст("phone"); Запись.ЗаписатьКонецЭлемента();
		//Запись.ЗаписатьНачалоЭлемента("value");Запись.ЗаписатьТекст(Выборка.Получить("ТелефонКонтрагента")); Запись.ЗаписатьКонецЭлемента();
		//Запись.ЗаписатьКонецЭлемента();//param
	...
	//unitLoad для ФЗ 54
	//Если Префикс = "KTY" тогда
	Если  Выборка.получить("ОплаченБН") или Выборка.получить("Заказ через посредника") тогда
	иначе
...
Функция ПолучитьСоставПосылки(Заказ,Префикс)Экспорт
	...
	тБонусы = Заказ.Бонусы.Выгрузить();
	...
	КонецЦикла;
	//03.10.18 Смирнов Бонусы в аксы
	Для каждого стр из тБонусы цикл
		Состав=состав+"("+СокрЛП(стр.Номенклатура.Код)+") "+стр.Номенклатура+" - "+стр.Количество+"# "+Символ(10);
		нстр=составтз.Добавить();
		нстр.Номенклатура = стр.номенклатура;
		нстр.ЕдиницаИзмерения = стр.номенклатура.ЕдиницаХраненияОстатков;
...
Процедура ОбновитьСтатусыЗаказов(ДатаН=неопределено,ДатаК=неопределено) Экспорт
	...
	Запрос.Текст="ВЫБРАТЬ РАЗЛИЧНЫЕ
	             ...
	             |	И ВЫБОР
	             |			КОГДА ЗаказПокупателя.ДоговорКонтрагента.ОтветственноеЛицо.НаправлениеПродаж = &КолесаТут
	             |				ТОГДА ЗаказПокупателя.ТипЗаказа <> 6
	             |			ИНАЧЕ ИСТИНА
	             |		КОНЕЦ
	             |;
	             |
...
Процедура СогласоватьЗаказСПИТСТОП(Заказ) Экспорт
	СоответствиеПоставщиковКодДпд = ЗаполнитьСоответствиеПоставщиков();
	
	табРеквизитыТК=новый таблицаЗначений;
	табРеквизитыТК.Колонки.Добавить("Поле");
	табРеквизитыТК.Колонки.Добавить("Значение");
	...
	ОтправлятьДоки=ложь;
	//Если Заказ.Контрагент.КатегорияПокупателя <> Справочники.КатегорииПокупателей.ПосредникТК тогда 
	//	ОтправлятьДоки=истина;
	//КонецЕсли;
	...
	Для каждого стр из Заказ.Бонусы цикл
		КоличествоМ = КоличествоМ+стр.Количество;
		Содержимое 	= Содержимое+"("+СокрЛП(стр.Номенклатура.Код)+") "+стр.Номенклатура+" - "+стр.Количество+"#"+Символы.ПС;
		Если Найти(ШК,"VAKSV")=0 тогда
			ШК = ШК+Префикс+СокрЛП(Выборка.Получить("НомерЗаказа"))+"V"+СоответствиеПоставщиковКодДпд.Получить(Заказ.Подразделение)+"V"+"AKS"+"V1"+";";
	...
	нстр = табРеквизитыТК.Добавить();
	нстр.Поле = "Номер заказа перевозчика";
	нстр.Значение = "ПИТСТОП";
	
	ЗаказОб=Заказ.ПолучитьОбъект();
	
	ЗаказОб.СтатусДПД  = Перечисления.СтатусыЗаказовДПД.ДПДЗаказПодтвержден;
	ЗаказОб.СтатусПеревозчика  = "ОК";
	ЗаказОб.НомерЗаказаТК  = "ПИТСТОП";
	
	ТабЧастьПараметрыТК=ЗаказОб.РеквизитыЗаказаТК;
	Для каждого стрЗ из табРеквизитыТК цикл
		ОбновитьРеквизитТК(ТабЧастьПараметрыТК,стрЗ.Поле, стрЗ.Значение);
	КонецЦикла;
	
	ЗаказОб.Комментарий = "Подтвержден; ПИТСТОП; "+ЗаказОб.Комментарий;
...
Функция ПодготовитьШапкуЗаказаПодорожник(Заказ,табРеквизитыТК) Экспорт
	...
	ЛогинПодорожникЛО    = "super_client_2576";
	ПарольПодорожникЛО   = "d3e8ea";
	...
	ИначеЕсли Найти(нрег(Область),"ленинградская")>0 или Найти(нрег(Область2),"ленинградская")>0 или  Найти(нрег(Область),"петербург")>0 или Найти(нрег(Область2),"петербург")>0 тогда  
		Логин  = ЛогинПодорожникЛО;
		Пароль = ПарольПодорожникЛО;	
	Иначе 
	...
	Если Логин  = ЛогинПодорожникЛО тогда
	...
	Для каждого стр из тзТовары Цикл
		...
		ОбщийВес 	= ОбщийВес+стр.ЕдиницаИзмерения.Вес * стр.Количество;
		//ШК       	= ШК+СокрЛП(стр.Номенклатура.Код)+Символы.ПС;
		КоличествоМ = КоличествоМ+стр.Количество;
		Содержимое 	= Содержимое+"("+СокрЛП(стр.Номенклатура.Код)+") "+стр.Номенклатура+" - "+стр.Количество+"#"+Символы.ПС;
		ОбъявленнаяЦенность = ОбъявленнаяЦенность+стр.Сумма;
...
Функция ПодготовитьЗапросСтоимостиДоставкиДПД(Заказ)
	...
	ДоставкаДоТерминала = не Данные.Получить("DPDВезетНаАдрес");
