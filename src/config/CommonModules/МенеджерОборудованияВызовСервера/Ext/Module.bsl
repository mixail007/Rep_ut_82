...
Функция ПолучитьXMLПакетДляФискализацияЧека(ОбщиеПараметры, ВыходныеПараметры, СуммаЧека, ШиринаСтроки = 32, НовыйФормат = Ложь) Экспорт
	...
	Если ОбщиеПараметры.Свойство("КассирИНН") и Не ПустаяСтрока(ОбщиеПараметры.КассирИНН) Тогда 
		...
		Если ОбщиеПараметры.ПозицииЧека <> Неопределено Тогда
			...
			Для ИндексМассива = 0 По ОбщиеПараметры.ПозицииЧека.Количество() - 1 Цикл
				...
				Если ТекущаяПозиция.Свойство("ФискальнаяСтрока") Тогда
						...
						Если ТекущаяПозиция.Свойство("ДанныеАгента") Тогда 
							ЗаписатьДанныеАгента(ЗаписьXML, ТекущаяПозиция.ДанныеАгента);
						КонецЕсли;
						Если ТекущаяПозиция.Свойство("ДанныеПоставщика") Тогда 
	...
	Если Не РасчетыСПокупателем Тогда
		Для Каждого СтрокаПлатежа Из Ссылка.РасшифровкаПлатежа Цикл
		  ...
		  //СтрокаЧека.ПризнакПредметаРасчета = МенеджерОборудованияВызовСервера.ОпределитьПризнакПредметаРасчета(СтрокаТовара.Номенклатура);
		    СтрокаЧека.ПризнакСпособаРасчета = ПредопределенноеЗначение("Перечисление.ПризнакиСпособаРасчета.ПередачаСПолнойОплатой");
			    //СтрокаЧека.ПризнакПредметаРасчета = МенеджерОборудованияВызовСервера.ОпределитьПризнакПредметаРасчета(СтрокаТовара.Номенклатура);
			СтрокаЧека.ПризнакПредметаРасчета = ПредопределенноеЗначение("Перечисление.ПризнакиПредметаРасчета.Товар");
...
Процедура ЗаполнитьСтрокуЧека(ОбщиеПараметры, СтрокаЧека, СтрокаПозиции, КурсОплаты, ИндексСтрокиСоСкидкой, СуммаПлатежаПоСтавке,Организация)
	...
	СтрокаЧека.СтавкаНДС = МенеджерОборудованияКлиентСервер.ПолучитьСтавкуНДСДляФискализацииЧека(СтрокаПозиции.СтавкаНДС,организация,СтрокаПозиции.Услуга);
	// Дополнительные реквизиты начиная с ФФД 1.1
	СтрокаЧека.КодПризнакаСпособаРасчета = 4;  
...
Функция ПолучитьXMLПакетДляОперации(ОбщиеПараметры) Экспорт
	...
	ЗаписьXML.ЗаписатьНачалоЭлемента("Parameters");
	ЗаписьXML.ЗаписатьАтрибут("CashierName", ?(ОбщиеПараметры <> Неопределено И ОбщиеПараметры.Свойство("Кассир"), XMLСтрока(ОбщиеПараметры.Кассир), НСтр("ru='Администратор'")));
	ЗаписьXML.ЗаписатьАтрибут("CashierVATIN", ?(ОбщиеПараметры <> Неопределено И ОбщиеПараметры.Свойство("КассирИНН"), XMLСтрока(ОбщиеПараметры.КассирИНН), ""));
	ЗаписьXML.ЗаписатьКонецЭлемента();
	ЗаписьXML.ЗаписатьКонецЭлемента();
