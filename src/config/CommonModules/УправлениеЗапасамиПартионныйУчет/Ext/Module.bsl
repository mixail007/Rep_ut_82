...
Процедура СообщитьОНехваткеПартииУпр(СтрокаДокумента, СтруктураПараметров, РегистрУчета, КоличествоОсталосьПогасить)
	...
	Если СтрокаДокумента.ОтражатьВУправленческомУчете Тогда
		...
		СтрокаДокументаНоменклатура = СтрокаДокумента.Номенклатура; //13.11.2018
		Если РегистрУчета = "НаСкладах" Тогда
			СтрокаСообщения = СтрокаСообщения +
			"Документ " + СтрокаДокумента.Регистратор + ", строка :" + СтрокаДокумента.НомерСтроки + Символы.ПС
			+ "Не списано по партиям " + КоличествоОсталосьПогасить + " " + СтрокаДокументаНоменклатура.ЕдиницаХраненияОстатков 
			+ " товара " + "(" + СтрокаДокументаНоменклатура.Код + ") " + СтрокаДокументаНоменклатура
			+ ?(СтрокаДокументаНоменклатура.ВестиУчетПоХарактеристикам, ", х-ка: " + СтрокаДокумента.ХарактеристикаНоменклатуры, "")
			+ ?(СтрокаДокументаНоменклатура.ВестиУчетПоСериям, ", серия: " + СтрокаДокумента.СерияНоменклатуры, "")
			...
			Если ПараметрыСеанса.ПроведениеПоПартиямДатаПервойОшибки = Дата(1,1,1) Тогда
				ПараметрыСеанса.ПроведениеПоПартиямДатаПервойОшибки =  ПолучитьДатуДок(СтрокаДокумента.Регистратор); // = СтрокаДокумента.Регистратор.Дата; 13.11.2018
			КонецЕсли;
...
Процедура ПоступлениеВозвратОтПокупателя(СтрокаДокумента, СтруктураПараметров)
	...
	Если Метаданные.РегистрыНакопления.ПартииТоваровНаСкладах.Измерения.ДокументОприходования.Тип.СодержитТип(ТипЗнч(ДокументПартии)) Тогда
		...
		Иначе
			  СтрокаДокументаНоменклатура = СтрокаДокумента.Номенклатура;  //1 запрос
			СообщитьОбОшибке("Управленческий учет. Документ " + СтрокаДокумента.Регистратор + ", строка :" + СтрокаДокумента.НомерСтроки + Символы.ПС
			+ "Не найдено поступление " + СтрокаДокумента.Количество + " " +СтрокаДокументаНоменклатура.ЕдиницаХраненияОстатков 
			+ " товара " + СтрокаДокументаНоменклатура
			+ ?(СтрокаДокументаНоменклатура.ВестиУчетПоХарактеристикам, ", х-ка: " + СтрокаДокумента.ХарактеристикаНоменклатуры, "")
			+ ?(СтрокаДокументаНоменклатура.ВестиУчетПоСериям, ", серия: " + СтрокаДокумента.СерияНоменклатуры, "")
	...
	Иначе
		...
		Для Каждого Строка Из НайденныеСтроки Цикл
			...
			// Вспомогательные поля для списания
			Движение.ДокументОприходованияДата	=  ПолучитьДатуДок(СтрокаДокумента.Регистратор); // = СтрокаДокумента.Регистратор.Дата; 13.11.2018
		...
		Если КоличествоОсталосьПогасить > 0 И НЕ (Учет="Нал" ) Тогда
			СтрокаДокументаНоменклатура = СтрокаДокумента.Номенклатура; //13.11.2018 - 1 раз
			СообщитьОбОшибке("Управленческий учет. Документ " + строка(СтрокаДокумента.Регистратор) + ", строка :" + СтрокаДокумента.НомерСтроки + Символы.ПС
			+ "Не найдено " + КоличествоОсталосьПогасить + " " + строка(СтрокаДокументаНоменклатура.ЕдиницаХраненияОстатков)
			+ " товара " + строка(СтрокаДокументаНоменклатура)
			+ ?(СтрокаДокументаНоменклатура.ВестиУчетПоХарактеристикам, ", х-ка: " + СтрокаДокумента.ХарактеристикаНоменклатуры, "")
			+ ?(СтрокаДокументаНоменклатура.ВестиУчетПоСериям, ", серия: " + СтрокаДокумента.СерияНоменклатуры, "")
		...
		Если ТаблицаЗаписейПереданных.Количество() > 0 Тогда
			...
			Для Каждого Строка Из НайденныеСтроки Цикл
				...
				// Свойства
				ЗаполнитьЗначенияСвойств( Движение, СтрокаДокумента); //13.11.2018 - одной операцией 9 строк
				//Движение.Период      = СтрокаДокумента.Период;
				//Движение.Регистратор = СтрокаДокумента.Регистратор;
				//Движение.Номенклатура 			    = СтрокаДокумента.Номенклатура;			
				//Движение.ХарактеристикаНоменклатуры   = СтрокаДокумента.ХарактеристикаНоменклатуры;			
			...
			СтрокаДокументаНоменклатура = СтрокаДокумента.Номенклатура;	//13.11.2018 - 1 а не 5 запросов 
			Если КоличествоОсталосьПогасить > 0	
				//+++ 29.09.2016 - крышки - можно без партий... для Всех! и ничего не пишем!
				И СтрокаДокументаНоменклатура.ВидТовара <> перечисления.ВидыТоваров.КрышкиНаклейки 
				Тогда
					СообщитьОбОшибке("Управленческий учет. Документ " + СтрокаДокумента.Регистратор + ", строка :" + СтрокаДокумента.НомерСтроки + Символы.ПС
				+ "Не списано по партиям " + КоличествоОсталосьПогасить + " " + СтрокаДокументаНоменклатура.ЕдиницаХраненияОстатков 
				+ " товара " + "(" + СтрокаДокументаНоменклатура.Код + ") " + СтрокаДокументаНоменклатура
...
//13.11.2018 - для уменьшения взаимоблокировок - только Дата и Только 1 типа документов!
функция ПолучитьДатуДок( регистратор1 )
	ТипДок = регистратор1.Метаданные().Имя;
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	РеализацияТоваровУслуг.Дата
	|ИЗ Документ."+ТипДок+" КАК РеализацияТоваровУслуг
	|ГДЕ РеализацияТоваровУслуг.Ссылка = &регистратор1";
	Запрос.УстановитьПараметр("регистратор1", регистратор1);
