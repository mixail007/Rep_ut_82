...
функция SIM_ПолучитьЗаданияНаОтгрузку(сРеал, сЗак) 
	...
	ii=0;
	Пока ii<сРеал.Количество()-1 цикл
	...
	ii=0;
	Пока ii<сЗак.Количество()-1 цикл
		Если не ЗначениеЗаполнено(сЗак[ii].Значение) тогда
			   сЗак.Удалить(ii);
	   ИначеЕсли сЗак[ii].Значение.Подразделение.ОбособленноеПодразделение = Истина тогда
			   сЗак.Удалить(ii);
		   иначе 
			   ii=ii+1;
		   КонецЕсли;
