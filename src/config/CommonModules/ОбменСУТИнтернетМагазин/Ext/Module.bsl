...
//
//+++ 30.10.2018 - замена на другую функцию!!! >>>> SIM_ВыгрузкаРезервов_NEW()
//
//процедура SIM_ВыгрузкаРезервов(путь="", флFTP=ЛОЖЬ, НадоПерепривязыватьРезервыПоМенеджеру = Ложь) Экспорт
//	
//	//  пока выключена !!!
...
//					   |	РегистрНакопления.ТоварыНаСкладах.Остатки(,
//					   |			Склад.ЗапретитьИспользование = ЛОЖЬ
//					 //16.10.2018 - подразделение лучше отбор делать!  
//					  // |				И Склад.Транзитный = ЛОЖЬ
...
//					   //+++ 11.10.2018 сразу только "+" остатки!
//					   |ГДЕ
...
//					   
//					   //+++ с 10.10.2018 - выгружаем и черный и/или серый списки!
//					   |	выбор когда спрКонтрагенты.ВыгружатьСерыйСписокВTerminal тогда ОстаткиСтопЛист.Остаток иначе 0 Конец
//					   |	+ выбор когда спрКонтрагенты.ВыгружатьЧерныйСписокВTerminal тогда ОстаткиСтопЛист.ОстатокЧерныйЛист  иначе 0 Конец
//					   |ИЗ
//					   |	(ВЫБРАТЬ
//					   |		спр.Код КАК Код,
//					   |		спр.ВыгружатьСерыйСписокВTerminal,
//					   |		спр.ВыгружатьЧерныйСписокВTerminal
//					   |	ИЗ
//					   |		Справочник.Контрагенты КАК спр
//					   |	ГДЕ
//					   //10.10.2018 - РазрешитьГрузитьТоварыИзСтопЛиста
//					   |		(спр.ВыгружатьСерыйСписокВTerminal или спр.ВыгружатьЧерныйСписокВTerminal)
//					   |		И спр.Экспорт = ЛОЖЬ) КАК спрКонтрагенты
//					   |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ОстаткиСтопЛист КАК ОстаткиСтопЛист
//					   //10.10.2018 1, 2 или оба сразу
//					   |		ПО  ОстаткиСтопЛист.ЧерныйЛист = спрКонтрагенты.ВыгружатьЧерныйСписокВTerminal
//					   |		или НЕ ОстаткиСтопЛист.ЧерныйЛист = спрКонтрагенты.ВыгружатьСерыйСписокВTerminal
//					   //+++ 11.10.2018 - уже сделан отбор выше! 
...
//	текстФайл.УстановитьТекст(текстФайла2);
//	
//	ИмяФайла1 = "RestsOfPartners.xml";
//	текстФайл.Записать(путь+ ИмяФайла1); //06.11.0215
//	
//	Если флFTP Тогда 	// в потоке - не работает с 2019г.
//		SIM_ВыгрузитьФайлНаСервер(путь+ ИмяФайла1, "importrestsofpartners", Истина, "terminal.yst.ru");   //terminal
//	КонецЕсли;
//	
//КонецПроцедуры	//SIM_ВыгрузкаРезервов()
...
КонецЦикла;
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |						ИЗ
	               |							вт_ЗадачиПоЗаказамСтопЛиста КАК ВТзадачи)) КАК ЗаказыПокупателейОбороты
	               |ГДЕ
