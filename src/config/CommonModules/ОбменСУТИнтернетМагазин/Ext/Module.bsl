...
Запрос.Текст = "ВЫБРАТЬ
			   ...
			   //05.01.2019 булево преобразуется в число СРАЗУ! и берётся только [v]
               |		ИНАЧЕ выбор когда ЗначенияСвойствОбъектов.Значение=Истина тогда 1
			   |					когда ЗначенияСвойствОбъектов.Значение=ЛОЖЬ тогда 0
			   |				иначе ЗначенияСвойствОбъектов.Значение Конец
               |	КОНЕЦ как Значение1
...
//+++ 18.04.2017 ==========единый список всех свойств!===========================================
	...
	//21.02.2017 --- свва АКБ----------------------
	  СписокСвойств.Добавить("90167", "Ток1"); // Пусковой ток
	  СписокСвойств.Добавить("00132","Знач1"); // Диаметр/Емкость
	  СписокСвойств.Добавить("90225", "Тип1"); // Тип АКБ
	  
	//2. В файл ProductProperties выгружать данные по Дискам и Аксам
	  СписокСвойств.Добавить("90205", "maxload");    //+++ 07.07.2017  - Максимальная нагрузка (данные производителя)
	  СписокСвойств.Добавить("90251", "itemsinbox"); //+++ 19.07.2017  - Количество штук в упаковке
	  СписокСвойств.Добавить("90356", "new");  //+++ 05.01.2019 Новинка
	  СписокСвойств.Добавить("90357", "hit");  //+++ 05.01.2019 Хит продаж
	  
	Запрос.УстановитьПараметр("СписокСвойств",СписокСвойств);
    // каждое св-во отдельно!	  
	  для i=0 по СписокСвойств.Количество()-1 цикл
		  назв1 = СокрЛП(СписокСвойств[i].Представление);
		  знач1 = ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоКоду(СписокСвойств[i].Значение);
