...
процедура ОповещениеПоЗаказамСтопЛиста() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		//Если Выборка.ЭтоКлиентИМ тогда
	 	    СоздатьЗадачуОповещениеПоСтопЛисту( Выборка.ЗаказПокупателя, Исполнитель1, Тема1, ТекстЗадачи1 ); 
...
процедура СоздатьЗадачуОповещениеПоСтопЛисту(ЗаказПокупателя, Исполнитель, Тема="Надо РАЗРЕШИТЬ Стоп-лист по Заказу ИМ !", ТекстЗадачи="") Экспорт
	...
	Задача.Постановщик = Исполнитель; //или никого не надо
	 ...
	 задача.РеквизитДляСогласования = "РазрешитьГрузитьТоварыИзСтопЛиста";
	 ...
	 попытка
		 задача.Записать();
		 #Если Клиент тогда
			 Сообщить("Создана Задача по Стоп-листу для: "+ строка(Исполнитель), СтатусСообщения.Информация);
		 #КонецЕсли	 
	 исключение
		Вызватьисключение "При создании Задача по Стоп-листу для: "+ строка(Исполнитель)+"
						   |"+ОписаниеОшибки();
	 КонецПопытки;
