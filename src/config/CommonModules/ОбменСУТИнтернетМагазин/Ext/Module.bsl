...
//+++ 15.01.2018
функция SIM_ПолучитьЗаданияНаОтгрузку(сРеал, таблЗак) 
	
	//+++21.03.2018 - убираем заказы и реализации Филиалов и пустые
	ii=0;
	Пока ii<сРеал.Количество() цикл
		Если не ЗначениеЗаполнено(сРеал[ii].Значение) тогда
			   сРеал.Удалить(ii);
	   ИначеЕсли сРеал[ii].Значение.Подразделение.ОбособленноеПодразделение = Истина тогда
			   сРеал.Удалить(ii);
		   иначе 
			   ii=ii+1;
		   КонецЕсли;
	...
	ii=0;
	Пока ii<таблЗак.Количество() цикл
		Если не ЗначениеЗаполнено(таблЗак[ii].Заказ) тогда //это табл
			   таблЗак.Удалить(ii);
	   ИначеЕсли таблЗак[ii].Заказ.Подразделение.ОбособленноеПодразделение = Истина тогда
			   таблЗак.Удалить(ii);
		   иначе 
			   ii=ii+1;
		   КонецЕсли;
   ...
   //+++)
	...
	Запрос.УстановитьПараметр("сЗак", таблЗак.ВыгрузитьКолонку("Заказ") );
...
//	</JobForShipment>
//</JobForShipments>
Процедура SIM_ВыгрузитьСписокЗаданийНаОтгрузкуВФайлXML(СписокЗаданий=Неопределено, ИмяФайла="", ИмяУзла="JobForShipment" ) ЭКСПОРТ
