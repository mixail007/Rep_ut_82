...
//клиенты каждого сочетания групп: 
	//4(1)= 7,11,с,ч, 
	//6(2)=117-,11с,11ч, 7с,7ч, сч 
	//4(3)=117с-; 117ч-; 11сч; 7сч:  
	//1(4)=117сч-
...
Выборка = Результат.Выбрать();
	...
	пока Выборка.Следующий() цикл
	текстФайл.ДобавитьСтроку("<Item PartnerId="+k+выборка.РезервКод+k+" ProductId="+k+выборка.НоменклатураКод+k+" Rest="+k+формат(Выборка.Остаток,"ЧДЦ=0; ЧН=0; ЧГ=0")+k+">");
	КонецЦикла;
...
выборкаКлиенты	= Результат.Выбрать();
	...
	текстФайл.ДобавитьСтроку("<Item PartnerIdGroup="+k+выборкаКлиенты.РезервКод+k+" PartnerId="+k+ СокрЛП(выборка.КонтрагентКод)+k+" >");
	КонецЦикла;
	текстФайл.ДобавитьСтроку("</LinksOfPartners>");
