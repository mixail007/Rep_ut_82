...
функция ПредварительнаяАвтокомплектацияДляVIPконтрагентов()
		...
		ВОЗВРАТ ТаблицаПриоритетныхКонтрагентов;
КонецФункции
...
ТаблицаПриоритетныхКонтрагентов = ПредварительнаяАвтокомплектацияДляVIPконтрагентов();

//+++ 06.04.2018 +++ Задача № 40782 - Диски - распределение по долям между отделами/направлениями продаж
//ДележкаДисковПоНаправлениямПродаж();   - еще не готова, тестируется
...
//+++ 06-09.04.2018 --- основная функция по Задаче № 40782 ----
процедура ДележкаДисковПоНаправлениямПродаж() Экспорт 
	...
	ТаблНадо = ПолучитьТаблНадоПоНаправлениямПродаж();
...
//Надо
//
функция ПолучитьТаблНадоПоНаправлениямПродаж()
запрос = новый запрос;
запрос.Текст = "ВЫБРАТЬ
               |	ВЫБОР
               ...
               |ГДЕ
               |	Выразить(ЗаказыПокупателейСезонныеОстатки.КоличествоОстаток/4-0.5 как число(10,0) ) > 0
               |
               |УПОРЯДОЧИТЬ ПО
               |	ДатаСезЗаказа
               |АВТОУПОРЯДОЧИВАНИЕ";
...
функция ПолучитьТаблЕстьПоНаправлениямПродаж()
	...
	Запрос.Текст = "ВЫБРАТЬ
		...
		|	НаправленияПродаж.ПроцентДележки / 100 КАК Коэффициент
		|ПОМЕСТИТЬ ВТ
		|ИЗ
		...
		|	ЕСТЬNULL(ОстаткиТоваров.КоличествоОстаток, 0) - ЕСТЬNULL(ЗаказыПокупателей.КоличествоОстаток, 0) КАК СвободныйОстаток,
		|	ВЫРАЗИТЬ((ЕСТЬNULL(ОстаткиТоваров.КоличествоОстаток, 0) - ЕСТЬNULL(ЗаказыПокупателей.КоличествоОстаток, 0)) * ВТ.Коэффициент КАК ЧИСЛО(15, 0)) КАК Доля,
		...
		|		ПО (ЗаказыПокупателей.Номенклатура = ОстаткиТоваров.Номенклатура)
		|ГДЕ
		|	ЕСТЬNULL(ОстаткиТоваров.КоличествоОстаток, 0) - ЕСТЬNULL(ЗаказыПокупателей.КоличествоОстаток, 0) >= 4
		|
		|УПОРЯДОЧИТЬ ПО
		|	Процент УБЫВ
		|ИТОГИ
		...
		//====================ПЕРЕНОРМИРОВКА=======================
		сумПроц = 0;
		пока выборка.Следующий() цикл // ТОЛЬКО по ном.группам доли делим на сумПроц = 100%
			сумПроц = сумПроц + выборка.Процент;
		КонецЦикла;
		
		выборка.Сбросить();
		ТаблЕсть = новый ТаблицаЗначений;
		ТаблЕсть.Колонки.Добавить("НаправлениеПродаж");ТаблЕсть.Колонки.Добавить("Номенклатура");
		ТаблЕсть.Колонки.Добавить("СвободныйОстаток");ТаблЕсть.Колонки.Добавить("Доля"); ТаблЕсть.Колонки.Добавить("Процент");
		
		пока выборка.Следующий() цикл
			Коэф = выборка.Процент/сумПроц; // из того, что есть, Коэфф>1 
			выборкаТов = выборка.Выбрать();
			
			//сделаем кратно 4 и  Доля = Св.Ост х Коэф
			пока выборкаТов.Следующий() цикл
			    стр = ТаблЕсть.Добавить();
			    ЗаполнитьЗначенияСвойств(стр, выборкаТов);
				стр.Процент = ОКР(100*стр.Процент/сумПроц,4); //пересчет коэффициента до 4х знаков!
				стр.Доля = Цел( стр.СвободныйОстаток * Коэф ); // без обрезания до 4х!
			КонецЦикла;
...
функция ПолучитьРезультирующуюТаблицуЗаказовПоНаправлениямПродаж(таблЕсть, таблНадо)
	...
	Запрос = Новый Запрос;
	 Запрос.Текст = "ВЫБРАТЬ
	                |	тНадо.НаправлениеПродаж, тНадо.Номенклатура, 
					|	тНадо.ДатаСезЗаказа, тНадо.СезонныйЗаказ, тНадо.Контрагент, тНадо.ДоговорКонтрагента,
	                |	тНадо.Надо
	                |ПОМЕСТИТЬ ВТ_Надо
	                |ИЗ
	                |	&таблНадо КАК тНадо	;
	                |////////////////////////////////////////////////////////////////////////////////
	                |ВЫБРАТЬ
	                |	тЕсть.НаправлениеПродаж, тЕсть.Номенклатура, 
					|	тЕсть.СвободныйОстаток, тЕсть.Доля, тЕсть.Процент
	                |ПОМЕСТИТЬ ВТ_Есть
	                |ИЗ
	                |	&таблЕсть КАК тЕсть;
	                |////////////////////////////////////////////////////////////////////////////////
	                ...
	                |	ЕстьNull(тЕсть.Доля,0) КАК Доля,
	                |	ЕстьNull(тЕсть.Процент,0) КАК Процент
					
					//|,	ВЫБОР
					//|		КОГДА тНадо.Надо > ЕСТЬNULL(тЕсть.Доля, 0)
					//|			ТОГДА тНадо.Надо
					//|		ИНАЧЕ ЕСТЬNULL(тЕсть.Доля, 0)
					//|	КОНЕЦ КАК НадоИзДоли
	                |ИЗ
	                |	ВТ_Надо КАК тНадо
	                ...
	                |ГДЕ
					//--------------отбор по св.остатку!----------------------------
	                |	ВЫБОР КОГДА тНадо.Надо > ЕСТЬNULL(тЕсть.СвободныйОстаток, 0)
	                |			ТОГДА тНадо.Надо
	                |			ИНАЧЕ ЕСТЬNULL(тЕсть.СвободныйОстаток, 0)
	                |		КОНЕЦ > 0
	                |
	                |УПОРЯДОЧИТЬ ПО
	                |	НаправлениеПродаж,
	                ...
	                |АВТОУПОРЯДОЧИВАНИЕ
					|////////////////////////////////////////////////////////
					|; УНИЧТОЖИТЬ ВТ_Надо; УНИЧТОЖИТЬ ВТ_Есть";
	 ...
	 //============= Нормировка по сумме процентов =============
	 таблНорм = табл1.Скопировать(,"НаправлениеПродаж, Процент");
	 таблНорм.свернуть("НаправлениеПродаж, Процент");
	 сумПроц = таблНорм.итог("Процент");	 
	 
	//============= Обрезание по НадоИзДоли не более того, что есть в Доле -- по каждому направлению и номенклатуре =================
	// основное поле: НадоИзДоли
	напр0 = неопределено; тов0 = Неопределено; СумНадо = 0;
	для каждого стр1 из табл1 цикл
		...
		Если напр0<>напр1 тогда
			...
			СумНадоНапр = надо1; // первый сезонник...
		...
		дельта1 = Доля1 - СумНадоНапр;
		Если  дельта1>0  тогда
			дельта2 = Доля1 - СумНадоТов;
			Если дельта2>0 тогда //надо 8, дельта2=4 -> минимум
				стр1.НадоИзДоли = Цел( мин(Надо1, Дельта2) /4 ) * 4;
				СумНадоТов = СумНадоТов + стр1.НадоИзДоли;
			иначе
				стр1.НадоИзДоли = 0;
			КонецЕсли;	
			СумНадоНапр = СумНадоНапр + стр1.НадоИзДоли;
		иначе
			стр1.НадоИзДоли = 0;
		КонецЕсли;	
	КонецЦикла;
...
пока ВыборкаЗак.Следующий() цикл
	...
	НовЗаказ.ДоговорКонтрагента = ВыборкаЗак.ДоговорКонтрагента; // ОПРЕДЕЛЯЕТСЯ ЗАРАНЕЕ!!!
	//НовЗаказ.ДоговорКонтрагента = найтиДоговорФакторингаПоОтветственному(НовЗаказ); // Если пустой - ищет договор
	Если НовЗаказ.ДоговорКонтрагента.пустая() Тогда
		Продолжить; // без договора - ничего не делаем!
	КонецЕсли;
	...
	Пока ВыборкаВ.Следующий() Цикл
		Если ВыборкаВ.Доля >0 Тогда
			...
			Если ВыборкаВ.Надо<ВыборкаВ.Доля Тогда
				новСтр.Количество = ВыборкаВ.Надо;
			иначе
				новСтр.Количество = ВыборкаВ.Доля;
			КонецЕсли;
			новСтр.Номенклатура = ВыборкаВ.Номенклатура;
			НовСтр.КоличествоМест             = новСтр.Количество;
			НовСтр.Коэффициент                = 1;
			НовСтр.ЕдиницаИзмерения           = ВыборкаВ.ЕдиницаХраненияОстатков;
			НовСтр.ЕдиницаИзмеренияМест       = ВыборкаВ.ЕдиницаХраненияОстатков;
			НовСтр.СтавкаНДС                  = Перечисления.СтавкиНДС.НДС18;
			НовСтр.Цена			= ВыборкаВ.Цена;
...
процедура SIM_ВыгрузитьАдресаКлиентовStore(путь="") 
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   //+++ 10.04.2018 - удаленные точки НЕ выгружаем (и их не видно в карточке клиента)
				    |ГДЕ Точки.ПометкаУдаления = ЛОЖЬ
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
