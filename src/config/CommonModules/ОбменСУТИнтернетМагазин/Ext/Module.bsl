...
Процедура ВключитьРегистрациюОбъектаОИМ(ЭтотОбъект1,ВыгружатьВКоннект24=Ложь) экспорт
	...
	Если ТипЗнч(ЭтотОбъект1)=Тип("ДокументОбъект.ЗаказПокупателя")
		или ТипЗнч(ЭтотОбъект1)=Тип("ДокументСсылка.ЗаказПокупателя") Тогда
...
процедура  ПроверитьВложенныеЗаказыДляРегистрации(ДокументСсылка)
 Для каждого стр из ДокументСсылка.Заказы Цикл
	 ПланыОбмена.ЗарегистрироватьИзменения(ПланыОбмена.ПланОбмена_ОИМ.НайтиПоКоду("sim"), стр.заказПокупателя);
	 ПроверитьВложенныеЗаказыДляРегистрации(стр.заказПокупателя); //рекурентно
 КонецЦикла;
Конецпроцедуры
...
|					как Число(15,0)) как МинимальнаяЦена,
...
|								    *(1 + ЕстьNull(ВТ_Товары.СкидкаПредоплаты,0)/100) как Число(15,0)) как ЦенаСоСкидкойПредоплаты
|
|
|ИЗ ВТ_Товары
|
...
Если ТекущаяПолитикаПравила=Неопределено Тогда
	...
	Если Константы.ЦенообразованиеПоКатегориям.Получить() Тогда 
		...
		Запрос.УстановитьПараметр("ТипЦенB2B", справочники.ТипыЦенНоменклатуры.НайтиПоКоду("00031")); 
...
процедура АвтоКомплектацияЗаказовПоСезонным() экспорт
	// предварительная для VIP контрагентов
		...
		Строка = ТаблицаПриоритетныхКонтрагентов.Добавить();
		Строка.Приоритет = 2;
		строка.Контрагент = Справочники.Контрагенты.НайтиПоКоду("92228"); //Контаер
		Строка.Доля = 0.7;
		Строка = ТаблицаПриоритетныхКонтрагентов.Добавить();
		Строка.Приоритет = 3;
		строка.Контрагент = Справочники.Контрагенты.НайтиПоКоду("94036"); //Екатеринбург
		Строка.Доля = 0.1;
		Строка = ТаблицаПриоритетныхКонтрагентов.Добавить();
		Строка.Приоритет = 4;
		строка.Контрагент = Справочники.Контрагенты.НайтиПоКоду("93187"); //Ростов
		Строка.Доля = 0.1;
		Строка = ТаблицаПриоритетныхКонтрагентов.Добавить();
		Строка.Приоритет = 5;
		строка.Контрагент = Справочники.Контрагенты.НайтиПоКоду("П000835"); //Питер
		Строка.Доля = 0.1;
		...
		Для каждого Контр из ТаблицаПриоритетныхКонтрагентов Цикл   
				...
				списНом.ЗагрузитьЗначения( Табном.ВыгрузитьКолонку("Номенклатура") );
					...
					Если НовЗаказ.Товары.Итог("Количество")>0 Тогда
						...
						НовЗаказ.ДатаОтгрузки     = ?(ТекущаяДата() - НачалоДня( ТекущаяДата() )<15*3600, НачалоДня( ТекущаяДата() ), НачалоДня( ТекущаяДата() )+86400);
						...
						днейДоОплаты));
						// =ДатеОтгрузки  или из Договора +ДопустимоеЧислоДнейЗадолженности 
						НовЗаказ.ДатаОплаты       = НовЗаказ.ДатаОтгрузки + НовЗаказ.ЧислоДнейСМоментаОтгрузки*86400; 
						
						//НовЗаказ.ИспользоватьПлановуюСебестоимость = Выборка.ссылка.ИспользоватьПлановуюСебестоимость;
						//ИтогПлановаяСебестоимость       = Выборка.ссылка.ИтогПлановаяСебестоимость;
						НовЗаказ.КратностьВзаиморасчетов           = 1;
						//НовЗаказ.КонтактноеЛицо     = Выборка.ссылка.КонтактноеЛицо;
						...
						//НовЗаказ.Склад              = Выборка.ссылка.Склад;
						//НовЗаказ.СтруктурнаяЕдиница = Выборка.ссылка.СтруктурнаяЕдиница;
						//СуммаДокумента     = Выборка.ссылка.СуммаДокумента;
						//НовЗаказ.ТипЦен             = Выборка.ссылка.ТипЦен;

						НовЗаказ.ТипЦен   =	справочники.ТипыЦенНоменклатуры.найтиПоКоду("00006");
...
Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
               ...
               |	РегистрНакопления.ЗаказыПокупателейСезонные.Остатки(
               |			,
               |			НЕ ЗаказПокупателяСезонный.Контрагент В (&СписокКонтрагентов)
...
Запрос.УстановитьПараметр("СписокКонтрагентов", ТаблицаПриоритетныхКонтрагентов.ВыгрузитьКолонку("Контрагент"));
Результат = Запрос.Выполнить();
Выборка = Результат.Выбрать();

глТекущийПользователь1 = Справочники.Пользователи.НайтиПоКоду("Робот (магазин)");
мВалютаРегламентированногоУчета1 = Справочники.Валюты.НайтиПоКоду("643"); //643 код
...
Пока Выборка.Следующий() Цикл
 	Основание  = выборка.Ссылка;
    	...
    	ЭтотОбъектЗакПок = документы.ЗаказПокупателя.СоздатьДокумент();
	
	ДобавитьДокументДляФилиалаПоЯШТ = Ложь; // 10.08.2015
	...
	// Заполнение шапки
		...
		//ЭтотОбъектЗакПок.Ответственный = Основание.Ответственный; // менеджер из сезонного заказа
		...
		//ЭтотОбъектЗакПок.НапомнитьОСобытии  = Основание.НапомнитьОСобытии; // из сезонного заказа
		//ЭтотОбъектЗакПок.ВремяНапоминания   = ТекущаяДата();
		...
		//ЭтотОбъектЗакПок.УстановитьДатуОплатыПоДоговору();
		//ЭтотОбъектЗакПок.ДатаОплаты = ТекущаяДата();
		//ЭтотОбъектЗакПок.ЧислоДнейСМоментаОтгрузки = 0;
		
		// отгрузка всегда с Ярославля!
		...
		ЭтотОбъектЗакПок.Резерв3 = Основание.Резерв3; // 29.02.2016
			...
			Если Основание.СУчетомАналогов Тогда
				...
				Запрос.Текст = "ВЫБРАТЬ
				               ...
				               |						И (&Транзит
				               |								И Склад.Транзитный
				               |								И НЕ Склад.ТипСклада = ЗНАЧЕНИЕ(Перечисление.ТипыСкладов.ВПути)
				               |								И Склад.Подразделение = &Подразделение
				               |							ИЛИ НЕ &Транзит
	...
	//Если Основание.Транзит Тогда // транзитный и автокомплектация >> для перемещения в филиал
		...
		// сразу делается заказ для перемещения  в Подразделение или на Склад!
		...
		ДобавитьДокументДляФилиалаПоЯШТ = Истина;
		
		//транзит и подразделение = ЯШТ - БРЕД!
		//Иначе
		//	#Если Клиент Тогда
		//		Сообщить("ОШИБКА в документе: "+строка(Основание)+" - Транзитный и с Ярославля...",СтатусСообщения.Внимание);
		//	#КонецЕсли	
		КонецЕсли;
		
	//Иначе //+++ 18.09.2014
	//	  //    не Основание.Транзит 
	//	Запрос.УстановитьПараметр("Транзит", Ложь); 
	//	Запрос.УстановитьПараметр("Подразделение", Основание.Подразделение); 
	//КонецЕсли;
	////+++)
...
КонецЕсли;
Если Основание.СучетомАналогов Тогда
ЕстьТовары = Автокомлектация_ЗаполнитьТабЧастьАналоги(ТекТабТовары,ЭтотОбъектЗакПок,ПодразделениеЯШТ,ЗаказыватьАналоги,Основание.ПоШтамповке);
иначе	
ЕстьТовары = Автокомлектация_ЗаполнитьТабЧасть(ТекТабТовары,ЭтотОбъектЗакПок,ПодразделениеЯШТ,Основание.ПоШтамповке);
КонецЕсли;
	...
	Иначе 
		#Если Клиент Тогда
			сообщить("  На основании: "+строка(основание)+" - Создание Заказа покупателя - НЕ требуется!");	
		#КонецЕсли	
	КонецЕсли;
...
Если ДобавитьДокументДляФилиалаПоЯШТ Тогда
	...
	ТекТабТоварыЕще=Запрос.Выполнить().Выгрузить();
	...
	Если ЭтотОбъектЗакПок.ЭтоНовый() Тогда  ///Если заказ по подразделению филиала не был записан, перезаполним по остаткам Ярославля
		ЭтотОбъектЗакПок.ВводитьТолькоПеремещение = Истина;
		ЭтотОбъектЗакПок.ОтгрузкаСПодразделенияСклада = Истина;
		...
		//ЕстьТовары = Автокомлектация_ЗаполнитьТабЧасть(ТекТабТоварыЕще,ЭтотОбъектЗакПок,ПодразделениеЯШТ);
		Если Основание.СучетомАналогов Тогда
		...
		КонецЕсли;
	Иначе
		НовыйОбъектЗакПок = ЭтотОбъектЗакПок.Скопировать(); 
		НовыйОбъектЗакПок.ВводитьТолькоПеремещение     = Истина;
		НовыйОбъектЗакПок.ОтгрузкаСПодразделенияСклада = Истина;
		НовыйОбъектЗакПок.Склад = основание.Подразделение.Склад;
		НовыйОбъектЗакПок.ДокументОснование = Основание;
		НовыйОбъектЗакПок.ТипЗаказа = 1; 
		НовыйОбъектЗакПок.Комментарий       = "#АвтоКомплектация по Cезонному заказу покупателя №"+строка(Основание.Номер)+" от "+Формат(Основание.Дата,"ДЛФ=D")+" # 
		|" +СокрЛП(Лев(Основание.Комментарий,100))+?(Основание.СучетомАналогов,"С учетом аналогов","");
		
		Если основание.ДоговорКонтрагента.Пустая() Тогда
			НовыйОбъектЗакПок.ДоговорКонтрагента =основание.Контрагент.ОсновнойДоговорКонтрагента;
			НовыйОбъектЗакПок.Ответственный = НовыйОбъектЗакПок.ДоговорКонтрагента.ОтветственноеЛицо; // Менеджер  - он же и ответственный
		Иначе//фикс.договор
			НовыйОбъектЗакПок.ДоговорКонтрагента =основание.ДоговорКонтрагента;
		КонецЕсли;	
		НовыйОбъектЗакПок.Транзит = Ложь;
		НовыйОбъектЗакПок.Подразделение = ПодразделениеЯШТ;
		НовыйОбъектЗакПок.Дата = ТекущаяДата(); //момент создания!
		// дата отгрузки = текущий день до 15-00, после этого - следующий
		НовыйОбъектЗакПок.ДатаОтгрузки     = ?(ТекущаяДата() - НачалоДня( ТекущаяДата() )<15*3600, НачалоДня( ТекущаяДата() ), НачалоДня( ТекущаяДата() )+86400);

		НовыйОбъектЗакПок.Товары.Очистить();
		//ЕстьТовары = Автокомлектация_ЗаполнитьТабЧасть(ТекТабТоварыЕще,НовыйОбъектЗакПок,ПодразделениеЯШТ);
		Если Основание.СучетомАналогов Тогда
		ЕстьТовары = Автокомлектация_ЗаполнитьТабЧастьАналоги(ТекТабТоварыЕще,НовыйОбъектЗакПок,ПодразделениеЯШТ,ЗаказыватьАналоги,Основание.ПоШтамповке);
		иначе	
		ЕстьТовары = Автокомлектация_ЗаполнитьТабЧасть(ТекТабТоварыЕще,НовыйОбъектЗакПок,ПодразделениеЯШТ,Основание.ПоШтамповке);
		КонецЕсли;
		
		Если ЕстьТовары Тогда
			Автокомплектация_ЗаписатьЗаказ(НовыйОбъектЗакПок,Основание);
...
Процедура SIM_ВыгрузитьСписокДокументовВФайлXML(СписокЗаказов=Неопределено, ИмяФайла="", ИмяУзла="Order" )Экспорт
	...
	для ii=0 по СписокЗаказов.Количество()-1 цикл
		...
		иначе
			...
			ОбъектXML.ЗаписатьАтрибут("NumberIn1S",СокрЛП(зак.Номер) );
			ОбъектXML.ЗаписатьАтрибут("PartnerId", СокрЛП(зак.Контрагент.Код) );
			ОбъектXML.ЗаписатьАтрибут("Total", формат(зак.СуммаДокумента,"ЧДЦ=2; ЧРД=.; ЧН=0; ЧГ=0") ); //С копейками !   
			...
			КонецЕсли;	
			кодПодразделения = ?(зак.Подразделение = справочники.Подразделения.ПустаяСсылка(), 5, число(зак.Подразделение.Код) );
			...
			//ОбъектXML.ЗаписатьНачалоЭлемента(ИмяУзла+"Details");
			
			//Для i=0 по Зак.Товары.Количество()-1 Цикл
			//	стр1 = Зак.Товары[i]; 
			//	
			//	#Если Клиент Тогда
			//		ОбработкаПрерыванияПользователя();
			//		Если стр1.НомерСтроки <> i+1 Тогда
			//			сообщить("Номер строки "+строка(стр1.НомерСтроки)+" <> "+строка(i+1) );
			//		КонецЕсли;
			//	#КонецЕсли
...
Функция SIM_ВыгрузитьФайлНаСервер(ИмяФайла="", ИмяПоТипуФайла="importorders", УдалитьФайлПослеУдачнойВыгрузки = Истина, сайт1="terminal.yst.ru",ПолныйАдрес=Ложь ) ЭКСПОРТ
...
Соединение = Новый HTTPСоединение(СтрокаПодключения, , login, password, , Истина);    //+++ 15.08.2017  https (SSL)!
...
функция ПолучитьЗапросВсехКлиентовТерминал()
	  ...
	  Запрос.Текст = "ВЫБРАТЬ
					 ...
					 //26.09.2017 - сразу в запросе определяем 4 параметра
					 |  Выбор когда Авторизация.Владелец.ОсновнойДоговорКонтрагента.ТипДоговора = &Предоплата
					 ...
					 |		Тогда 1 иначе 0 Конец как флагДоговорПредоплаты,
					 
			         |	Выбор когда Авторизация.Владелец.способОтгрузки = &Самовывоз Тогда 1 иначе 0 Конец как ОснСпособОтгрузки,
					 |	ДоговорыКонтр.флагЕстьДоговорПредоплаты,
					 |	ДоговорыКонтр.флагЕстьДоговорСоЗвездой,
					 
					 //27.09.2017 - все нужные поля
					 ...
					 |	Авторизация.Владелец.НаименованиеПолное как НаименованиеПолное,
					  ...
					  |	выбор когда Авторизация.Владелец.Экспорт 
			          |    и Авторизация.Владелец.основнойДоговорКонтрагента.ВалютаВзаиморасчетов.Код<>""643"" Тогда Истина
					   |   иначе ЛОЖЬ Конец как флЭкспортный,
					//+++ 11.01.2018   
					  |Выбор когда Авторизация.Владелец.ПодразделениеЛогистики.ОбособленноеПодразделение 
					  |	тогда Авторизация.Владелец.ПодразделениеЛогистики.Код
					  | иначе ""00005"" Конец КАК ПодразделениеЛогистики
					  
				//+++ 16.10.2017
				//	| , выбор когда Авторизация.Владелец.нал тогда 1 иначе 0 Конец как IsStar
				
					//+++ 31.10.2017
					 |	,выбор когда Авторизация.Владелец.основнойДоговорКонтрагента.Наименование подобно (""%*%"") 
					 |			тогда 1 иначе 0 Конец как IsStar
					 ...
					 //+++( 26.09.2017 ------группируем по макс. 2 параметра HasPrepay, HasStar ---------------
					 |		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
			        |	Договоры.Владелец,
			        |	МАКСИМУМ(ВЫБОР
					...
					////+++ 10.10.2017 - после совещания с Серковым - hasStar
					//|	МАКСИМУМ(ВЫБОР КОГДА Договоры.Наименование ПОДОБНО ""%*%""
					//|				ТОГДА 1 ИНАЧЕ 0 КОНЕЦ)
					| МАКСИМУМ(	выбор когда Договоры.Владелец.Нал=Истина тогда 1 иначе 0 Конец)
					
					|КАК флагЕстьДоговорСоЗвездой
...
процедура SIM_ВыгрузитьКлиентовStore(путь="") 
	...
	//	Для каждого клиент из СписокКлиентовМИМ Цикл
		для каждого стр0 из табл цикл //27.09.2017
			...
			////+++ 03.03.2017 - Формула ООО 2015 - для отражения остатков сторонних поставщиков PrePay = 1 всегда!
			//Если сокрЛП(стр1.КонтрагентКод)="93980" Тогда 
			//	стр1.флагДоговорПредоплаты = 1;
			//КонецЕсли;
			//
			//+++ 29.04.2015
			//флЭкспортный = Клиент.Значение.Экспорт 
			//				и Клиент.Значение.основнойДоговорКонтрагента.ВалютаВзаиморасчетов.Код<>"643";
			
			//Если стр0.ПроцентНаценкиТранспорта>0 Тогда
			//стр1.ПроцентНаценкиТранспорта = стр0.ПроцентНаценкиТранспорта;
			//КонецЕсли;
...
процедура SIM_ВыгрузитьМенеджеровStore(путь="") экспорт
	...
	//	|	ВЫБОР
	//	|		КОГДА ВТ.Менеджер.ФизЛицо.Представление ЕСТЬ NULL 
	//	|			ТОГДА ВТ.Менеджер.Наименование
	//	|		ИНАЧЕ ВТ.Менеджер.ФизЛицо.Представление
	//	|	КОНЕЦ КАК ФИО,
	//	|	NULL КАК Email,
	...
	//	|	NULL КАК Фото,
	//	|	NULL КАК icq,
	//	|	ВТ.Менеджер.ОсновноеПодразделение.Код КАК storage
	//	|ПОМЕСТИТЬ Свод
	//	|ИЗ
	//	|	ВТ КАК ВТ
	//	|
	...
	//	|		ПО ВТ.Менеджер.ФизЛицо = МобильныйП.Объект
	//	|
	//	|ОБЪЕДИНИТЬ ВСЕ
	//	|
	//	|ВЫБРАТЬ
	...
	//	|		ПО ВТ.Менеджер.ФизЛицо = ПочтаП.Объект
	//	|
	//	|ОБЪЕДИНИТЬ ВСЕ
	//	|
	//	|ВЫБРАТЬ
	...
	//	|		ПО ВТ.Менеджер.ФизЛицо = СкайпП.Объект
	//	|
	//	|ОБЪЕДИНИТЬ ВСЕ
	//	|
	//	|ВЫБРАТЬ
	...
	//	|		ПО ВТ.Менеджер.ФизЛицо = ВнутреннийП.Объект
	//	|
	//	|ОБЪЕДИНИТЬ ВСЕ
	//	|
	//	|ВЫБРАТЬ
	...
	//	|		ПО ВТ.Менеджер.ФизЛицо = ФотоП.Объект
	//	|
	//	|ОБЪЕДИНИТЬ ВСЕ
	//	|
	//	|ВЫБРАТЬ
	...
	//	|		ПО ВТ.Менеджер.ФизЛицо = АськаП.Объект;
	//	|
	//	|////////////////////////////////////////////////////////////////////////////////
	//	|ВЫБРАТЬ
	//	|	Свод.Менеджер,
...
процедура sim_ВыгрузитьRestOfSupplier(путь="",таблКодовSupplier, КодПодразделения="5")
 ...
 Запрос.Текст = "ВЫБРАТЬ
                ...
                |			СУММА(остаткиТотх.КоличествоОстаток) 
				
				//+++ 25.01.2017 --- свободный остаток! НЕ СУММА!  1 остаток на 1 товар! --- для ЛингЛонг очень актуально!
				|				- МАКСИМУМ( ЕстьNull(ЗаказыЯШТ.КоличествоОстаток,0) )
				...
				//+++( 25.01.2017 -------Заказы только Ярославля, проделенные!-------------------------------
                |	    ЛЕВОЕ СОЕДИНЕНИЕ
				|		(выбрать Зак.Номенклатура, Зак.КоличествоОстаток 
				|			из РегистрНакопления.ЗаказыПокупателей.Остатки(,  ЗаказПокупателя.Проверен и ЗаказПокупателя.Транзит = ЛОЖЬ) как Зак) как ЗаказыЯШТ
	...
	запрос.текст =  "ВЫБРАТЬ РАЗЛИЧНЫЕ
	 ...
	 пока выборка.Следующий() цикл
		...
		Если выборка.КоличествоШК=1 Тогда
		 	ШК = выборка.ШК;
			Если ШК="" или стрДлина(ШК)<>13 Тогда 
        ...
        Иначе // 2 и более ШК
			...
			пока выборка2.Следующий() цикл
			 	ШК2 = выборка2.ШК;
				Если ШК2="" или стрДлина(ШК2)<>13 Тогда 
