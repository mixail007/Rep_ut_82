...
//+++ 13.04 - выключена по # Бондаренко --- ДележкаДисковПоНаправлениямПродаж(масVIPКонтрагентов);   
...
пока ВыборкаЗак.Следующий() цикл
	...
	НовЗаказ.Комментарий        = "#АвтоКомплектация по Cезонному заказу покупателя №"+строка(Основание.Номер)+" от "+Формат(Основание.Дата,"ДЛФ=D")+" # 
				|" +СокрЛП(Лев(Основание.Комментарий,100))+?(Основание.СучетомАналогов," БЕЗ учета аналогов!","")+" (ДИСКИ)";
...
Процедура SIM_ВыгрузитьСписокДокументовВФайлXML(СписокЗаказов=Неопределено, ИмяФайла="", ИмяУзла="Order" )Экспорт
	...
	для ii=0 по СписокЗаказов.Количество()-1 цикл
	 ...
	 //+++( 17.04.2018 --- цены B2B ---------Задача № 45011----------------------
	таблB2B = новый ТаблицаЗначений;
	Если Найти(Зак.ДоговорКонтрагента.Наименование,"*")>0 тогда
		спис1 = Зак.товары.ВыгрузитьКолонку("Номенклатура");
	...
	//+++)
			...
			Для каждого стр1 из Зак.Товары Цикл
				...
				Попытка
					...
					стр1Цена = стр1.Цена;
					//+++( 17.04.2018 подмена на цену B2B +++
					Если таблB2B.Количество()>0 тогда
						стрB2B = таблB2B.Найти(стр1.Номенклатура, "Номенклатура");
						Если стрB2B<>неопределено тогда
							Если стр1Цена<стрB2B.Цена тогда
							стр1Цена = стрB2B.Цена;
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
					//+++)	
					ОбъектXML.ЗаписатьАтрибут("Price", формат(стр1Цена,"ЧДЦ=2; ЧРД=.; ЧН=0; ЧГ=0") ); //цена C копейками!?
