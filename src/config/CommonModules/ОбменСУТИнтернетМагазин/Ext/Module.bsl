...
//пересчет координат в км
функция получитьРасстояниеПоКоординатамНапрямую(предКоординаты,стр1Координаты, ВернутьКМ=Истина) экспорт
	
	КоэффициентУвеличения = 1.0;	// расстояние по прямой (гипотенуза) х этот коэффициент 1.4 - 1.6
	
	дRмin = 20; // --- на расстоянии 10 - 20 км. - это близко!
	R  = 6370; // средний радиус Земли в км. от 6378.1 экваториальный до 6356.8 полярный
	...
	Rш = R*cos( (Ш1+Ш2)/2 * pi180 ); // радиус на средней широте 50-60 градусов  (ш1+ш2)/2   * pi/180
	дШ = знакШ*(Ш2-Ш1)*pi180*Rш; // 1 градус = 56 км   (  ш2-ш1 < 0.2 )  
	
	дR = КоэффициентУвеличения * окр( sqrt(дШ*дШ + дД*дД),1); //пифагор - сразу 2 катета и доп.коэффициент как от гипотенузы   до 0.1 км
...
функция получитьРасстояниеПоКоординатам(Коорд1,Коорд2) экспорт   
	...
	КоэфОбъезда = 1.4; // от 1.3 до 2.0
	
	R12 = КоэфОбъезда * получитьРасстояниеПоКоординатамНапрямую(Коорд1,Коорд2); //21.03.2019
