...
//		попытка
	...
	//23.01.2017 см. следующую процедуру  - перепривязка клиентов к менеджеру - 3-ий параметр
	 ...
	 SIM_ВыгрузкаРезервов_NEW("C:\1c\", ЛОЖЬ); //не выгружаем, только в файл!
...
//---------------резервы по разрешениям клиентов---------------
	...
	пока Выборка.Следующий() цикл
		текстФайл.ДобавитьСтроку("	<Item RezerveId="+k+выборка.ВариантКод+k +" ProductId="+k+выборка.НоменклатураКод+k +" Rest="+k+формат(Выборка.КоличествоОстаток,"ЧДЦ=0; ЧН=0; ЧГ=0")+k+"/>");
	КонецЦикла;
	пока Выборка2.Следующий() цикл
		текстФайл.ДобавитьСтроку("	<Item RezerveId="+k+выборка2.ВариантКод+"."+выборка2.РезервКод+k+" ProductId="+k+выборка2.НоменклатураКод+k+" Rest="+k+формат(Выборка2.КоличествоОстаток,"ЧДЦ=0; ЧН=0; ЧГ=0")+k+"/>");
	КонецЦикла;
	текстФайл.ДобавитьСтроку("</RestOfRezerves>");
...
//---------------резервы по разрешениям клиентов---------------
	...
	текстФайл.ДобавитьСтроку("	<Item RezerveId="+k+выборкаКлиенты2.ВариантКод+"."+выборкаКлиенты2.РезервКод+k+" PartnerId="+k+ СокрЛП(выборкаКлиенты2.КонтрагентКод)+k+"/>");
	КонецЦикла;
текстФайл.ДобавитьСтроку("</LinksOfPartners>");
