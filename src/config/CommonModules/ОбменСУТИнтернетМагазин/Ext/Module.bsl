...
//+++ 29.10.2018 -------------- 2-я выгрузка ----------------------------
// выгружаем варианты резерва:
//
// <RestOfRezerve>
//	 <Item RezerveId="Д.Литые" ProductId="9165523" Rest="12"/>
//	 <Item RezerveId="Д.Штамп" ProductId="9154311" Rest="4"/>
//	 <Item RezerveId="Black" ProductId="91765432" Rest="16"/>
//	 <Item RezerveId="Gray" ProductId="9234653" Rest="8"/>
//....
// </RestOfRezerve>
//
//<LinksOfPartners>
//	<Item RezerveId="Д.Штамп" PartnerId="00128">
//	<Item RezerveId="Д.Литые" PartnerId="П007930">
//	<Item RezerveId="Д.Штамп" PartnerId="П007930">
//	<Item RezerveId="Gray" PartnerId="П007930">
//</LinksOfPartners>
//
процедура SIM_ВыгрузкаРезервов_NEW(путь="", флFTP=Истина)
	k = """"; //кавычка в тексте
	текстФайл = новый ТекстовыйДокумент;	
...
Запрос.Текст = "ВЫБРАТЬ
               ...
               |	ЕСТЬNULL(ОстаткиСвободные.СвободныйОстаток, 0) КАК Остаток
               |ИЗ
...
Результат = Запрос.Выполнить();
Выборка = Результат.Выбрать();
//------------1 или несколько Выборок!-------------------
//<Item PartnerId="П006249" ProductId="9165523" Rest="12"/>
пока Выборка.Следующий() цикл
текстФайл.ДобавитьСтроку("<Item RezerveId="+k+выборка.РезервКод+k+" ProductId="+k+выборка.НоменклатураКод+k+" Rest="+k+формат(Выборка.Остаток,"ЧДЦ=0; ЧН=0; ЧГ=0")+k+">");
КонецЦикла;
...
Запрос.Текст = "ВЫБРАТЬ
               ...
               |	""Gray"" КАК РезервКод
               |ИЗ
               |	Справочник.Авторизация КАК спр
               |ГДЕ
               |	спр.Владелец.ВыгружатьСерыйСписокВTerminal
               ...
               |	Справочник.Авторизация КАК спр
               |ГДЕ
               |	спр.Владелец.ВыгружатьЧерныйСписокВTerminal
			   |	И спр.Владелец.ПометкаУдаления = ЛОЖЬ
           		|";
			   
Результат = Запрос.Выполнить();
выборкаКлиенты	= Результат.Выбрать();
	текстФайл.ДобавитьСтроку("</RestOfRezerve>");
	
	текстФайл.ДобавитьСтроку("<LinksOfPartners>");
	
//-------------клиенты черного/серого списка-------------------------	
	пока выборкаКлиенты.Следующий() цикл
	текстФайл.ДобавитьСтроку("<Item RezerveId="+k+выборкаКлиенты.РезервКод+k+" PartnerId="+k+ СокрЛП(выборка.КонтрагентКод)+k+" >");
	КонецЦикла;
