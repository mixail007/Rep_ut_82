...
функция ПолучитьЗапросВсехКлиентовТерминал()
	  ...
	  Запрос.Текст = "ВЫБРАТЬ
					 ...
					 //+++ 10.10.2018 ВыгружатьСерыйСписокВTerminal ИЛИ ВыгружатьЧерныйСписокВTerminal
	                 |	(Авторизация.Владелец.ВыгружатьСерыйСписокВTerminal или Авторизация.Владелец.ВыгружатьЧерныйСписокВTerminal) КАК РазрешитьГрузитьТоварыИзСтопЛиста,
					 ...
					 |	   					ВТ_П КАК ВТ)
					 |		СГРУППИРОВАТЬ ПО
					 |Контрагенты.ссылка) КАК СоответствиеМенеджеровРегионам
