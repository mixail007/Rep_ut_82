...
функция ЕстьЦеныПовышенияПоКлиенту(Контр, ДатаЦены=Неопределено) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ИЗ
	               |	РегистрСведений.РезервРазрешения.СрезПоследних(//&ДатаЦены
				   |	, Контрагент = &Контрагент) КАК РезервРазрешенияСрезПоследних
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныПовышения.СрезПоследних(//&ДатаЦены
...
|	(выбрать различные
   ...
   |	ЦеныПовышенияСрезПоследних.Цена
   |ИЗ
   |	РегистрСведений.РезервРазрешения.СрезПоследних(//ДатаЦеныПовышения &ДатаЦеныПовышения
   |	, Контрагент = &Контрагент) КАК РезервРазрешенияСрезПоследних
   |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныПовышения.СрезПоследних(//ДатаЦеныПовышения &ДатаЦеныПовышения
   |	, ) КАК ЦеныПовышенияСрезПоследних
   |		ПО РезервРазрешенияСрезПоследних.ВариантКод = ЦеныПовышенияСрезПоследних.ВариантКод
   |ГДЕ
   |	РезервРазрешенияСрезПоследних.ДатаОкончания = ДАТАВРЕМЯ(1, 1, 1)
...
|");
	...
	//14.02.2019
	Если ЕстьЦеныПовышения и ДатаЦены <> неопределено тогда
		ПостроительОтчетаОтчет.Текст = стрЗаменить(Запрос.Текст,"//ДатаЦеныПовышения","");
