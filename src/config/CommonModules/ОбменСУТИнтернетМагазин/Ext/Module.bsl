...
функция ПолучитьстатусыЗаказов(списокЗаказов) Экспорт
	...
	Запрос.Текст ="ВЫБРАТЬ РАЗЛИЧНЫЕ
				 ...
				 //+++ 27.06.2018 - исключение удаленных документов! и ограничение заданий по дате - 14 дней!
				 ...
				 |						 где  ЗадНаОтгрузку.Ссылка.ПометкаУдаления=ЛОЖЬ и ЗадНаОтгрузку.Ссылка.Дата>&ДатаНачЗаданий ) КАК ЗаданиеНаОтгрузкуЗаказыПокупателей
				 ...
				 |						 где  ЗадНаПеремещение.Ссылка.ПометкаУдаления=ЛОЖЬ и ЗадНаПеремещение.ссылка.Дата>&ДатаНачЗаданий ) КАК ЗаданиеНаПеремещениеЗаказыПокупателей
				 //+++)
				...
				//+++( 27.06.2018 - ограничение по неудаленным и по дате задания на отгрузку 
	            //  |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаданиеНаОтгрузку.ЗаказыПокупателей КАК ЗаданиеНаОтгрузкуЗаказыПокупателей
				 |ЛЕВОЕ СОЕДИНЕНИЕ (выбрать * из Документ.ЗаданиеНаОтгрузку.ЗаказыПокупателей как ЗадНаОтгрузку 
				 |						 где  ЗадНаОтгрузку.Ссылка.ПометкаУдаления=ЛОЖЬ и ЗадНаОтгрузку.Ссылка.Дата>&ДатаНачЗаданий ) КАК ЗаданиеНаОтгрузкуЗаказыПокупателей
				 //+++)
	...
	Запрос.УстановитьПараметр("ДатаНачЗаданий", НачалоДня(ТекущаяДата())-14*86400 ); //+++ 27.06.2018 - Задания "висят" от 7 до 14 дней!
