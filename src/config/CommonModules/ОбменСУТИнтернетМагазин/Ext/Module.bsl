...
ИмяПоляXML.Вставить("РазрешитьСезоннуюОтсрочку", "НasSeasonAdjournment"); //22.03.2018
...
функция SIM_ПолучитьЗаданияНаОтгрузку(сРеал, таблЗак) 
   ...
   ////+++)
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	               ...
	               |	И ЗаданиеНаОтгрузкуЗаказыПокупателей.ЗаказПокупателя.Подразделение.ОбособленноеПодразделение = ЛОЖЬ
	               |	И ЗаданиеНаОтгрузкуЗаказыПокупателей.Реализация.Подразделение.ОбособленноеПодразделение = ЛОЖЬ";
