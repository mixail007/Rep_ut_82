...
функция ПолучитьТекстЦенЭкспорт()
	...
	ксНДС   = 1 + кСтавкаНДС/100;
	кБезНДС = " / "+формат( ксНДС, "ЧРД=.; ЧГ=0"); //  / 1.18 или 1.20
	текст = "ВЫБРАТЬ
|   Приоритет, 
|	Номенклатура,   
//| ТипЦен,
...
|
|Выразить(((Тов2.Цена+&Фрахт/ЕстьNull(Фрахт.Кол,1) + Тов2.Вес*6.27)*"+ксНДС+") как Число(15,2)) как МинимальнаяЦена,
|Выразить(((Тов2.Цена+&Фрахт/ЕстьNull(Фрахт.Кол,1) + Тов2.Вес*6.27)*"+ксНДС+") как Число(15,2)) как ЦенаСоСкидкойПредоплаты
|
|ИЗ (выбрать 
|Тов.Приоритет,
|Тов.Номенклатура,
...
//+++)
 ...
 Если безФрахта Тогда 
	...
	"Выразить(((Тов2.Цена+&Фрахт/ЕстьNull(Фрахт.Кол,1) + Тов2.Вес*6.27)*"+кcНДС+") как Число(15,2))",
