...
СписокТиповЦен=Новый СписокЗначений;
 ...
 #Если Клиент Тогда
	 Состояние(строка(ТекущаяДАта())+" Начат анализ "+строка(списНом.Количество())+" товаров по таблице цен для "+строка(масШин.Количество())+" шин...");
	 Сообщить(строка(ТекущаяДАта())+" Начат анализ "+строка(списНом.Количество())+" товаров по таблице цен для "+строка(масШин.Количество())+" шин...");
 #КонецЕсли 
...
контрТУТ = справочники.Контрагенты.НайтиПоКоду("94291"); //для ценообразования KOLESATYT
 ...
 ЗапросПоЦенамНаШины.Текст = "ВЫБРАТЬ
							 ...
							 |";
 #Если Клиент Тогда
 Состояние(строка(ТекущаяДата())+" Идет получение всех цен по списку "+строка(списНом.Количество())+" товаров");
 #КонецЕсли	 
