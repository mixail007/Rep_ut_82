...
Процедура ПереключениеПравилРезерва() Экспорт
	...
	ВыполнитьПереключениеПравилРезерва(); //-8000
КонецПроцедуры

//основная функция - вынесена отдельно для "ручного" запуска...
Процедура ВыполнитьПереключениеПравилРезерва() Экспорт
...
Дата1 = КонецДня( ТекущаяДата() )+1;  //завтрашним днём!
...
No=-8000; i=0;
	Пока Вариант.Следующий() цикл
		#Если Клиент тогда
		Сообщить( строка(i)+") Вариант: "+Вариант.ВариантКод+" для переключения: "+строка(Вариант.Номенклатура)+" товаров для "+строка(Вариант.Контрагент)+" клиентов."); 
		#КонецЕсли
		выборкаТов = Вариант.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		пока выборкаТов.Следующий() цикл
			Если УжеЕстьЗаписьЦенПовышения(выборкаТов, Дата1-1 ) тогда  //на 1 сек./день раньше
				...
				регСв.Удалить();
				#Если Клиент тогда
					сообщить("Запись по товару "+выборкаТов.Номенклатура.Код+" с ценой "+строка(выборкаТов.Цена)+"руб. - УЖЕ была.");
				#КонецЕсли
				продолжить;
			КонецЕсли;
			...
			#Если Клиент тогда
				сообщить("НОВАЯ Запись: "+выборкаТов.Номенклатура.Код+" цена: "+строка(выборкаТов.Цена)+"руб.");
			#КонецЕсли
...
функция ПолучитьВариантКлиентаИТоварыДляПовышенияЦен(товарыВРезервеИСвОст, таблЦен, КонтрРезерв, ПодразделениеРезерва)
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   //|	РезервРазрешения.ВидТовара,
				   //|	РезервРазрешения.НоменклатурнаяГруппа,
...
процедура ЗаполнитьДвиженияРН_ПрибыльОтПовышенияЦен(Движения, ЭтотОбъект1 , ДатаСреза , Сторно=1) Экспорт
	Если ОбменСУТИнтернетМагазин.ЕстьЦеныПовышенияПоКлиенту(ЭтотОбъект1.Контрагент, ДатаСреза) тогда
		масТов = ЭтотОбъект1.Товары.ВыгрузитьКолонку("Номенклатура");
	    ...
	    для каждого стр1 из ЭтотОбъект1.Товары цикл
			...
			Движение1.Цена         = стр1.Цена;  // цена продажи!
			...
			КонецЕсли;
			//Прибыль = (Движение1.Цена - Движение1.ЦенаКлиента)*Движение1.Количество * Сторно;
