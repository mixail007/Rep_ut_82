...
функция ПолучитьПравилаДляВключения(товарыВРезервеИСвОст, таблЦен, КонтрРезерв)
	...
	Запрос.Текст = "ВЫБРАТЬ
				  ...
				  //|	РезервыИОстатки.Код как НоменклатураКод,
	               ...
	               |	РезервыИОстатки.СвОстаток * ПравилаПереключения.N КАК СвОстатокN,
	               ...
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.РезервРазрешения КАК РезервРазрешения
	               ...
	               |	И РезервРазрешения.КонтрагентРезерв = &КонтрРезерв
				  ...
				  //----------------если есть хотя бы 1 товар!--------------------
				  // | ГДЕ	ТоварыДляПереключения.ТоварНадоПереключать=1
	               ...
	               |	Цены.Бонус1
	               |
				   //=================Интегральные правила ПО ВСЕМ товарам 1 правила!====================
				   |ИМЕЮЩИЕ
				   |			 (Сумма(ТоварыДляПереключения.СвОстатокN) = 0
