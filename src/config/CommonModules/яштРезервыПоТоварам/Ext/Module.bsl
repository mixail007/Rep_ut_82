...
Процедура ЗаполнениеДанныхОперацийОТХвЗаявкахНаБрак() Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	А.ДокументОснование.ДатаПриходаНаОТХ
	               |ИЗ
	               |	(ВЫБРАТЬ
	               ...
	               |	ВТ_ОТХ КАК ВТ_ОТХ
	               |ГДЕ
	               |	НАЧАЛОПЕРИОДА(ВТ_ОТХ.ОперацияОТХДата, День) <> НАЧАЛОПЕРИОДА(ВТ_ОТХ.СсылкаДатаПриходаНаОТХ, День)";
	Результат = Запрос.Выполнить().Выбрать();
