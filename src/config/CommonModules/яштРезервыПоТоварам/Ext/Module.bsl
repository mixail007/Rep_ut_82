...
//
Процедура ПереключениеПравилРезерва(Дата1) Экспорт
	Дата1 = КонецДня( ТекущаяДата() )+1;  //завтрашним днём!
	ВыполнитьПереключениеПравилРезерва(Дата1); //-8000
КонецПроцедуры
...
процедура ЗаполнитьДвиженияРН_ПрибыльОтПовышенияЦен(Движения, ЭтотОбъект1 , ДатаСреза , Сторно=1) Экспорт
	...
	Если таблЦенПовышения.Количество()>0 тогда
		...
		для каждого стр1 из ЭтотОбъект1.Товары цикл
			...
			Движение1.Прибыль = (стр1.Цена - Движение1.ЦенаКлиента)*Движение1.Количество *Сторно;
