...
Процедура ОповещениеОПриходеКонтейнера() Экспорт
	ТекСсылка=Справочники.ВнешниеОбработки.НайтиПоКоду("300");//РЗ_Оповестить о приходе контейнера 
...
Процедура ОповещениеОПриходеКонтейнеровВСледМесяце() Экспорт
	//Попытка
		ТекСсылка=Справочники.ВнешниеОбработки.НайтиПоКоду("301");//РЗ_Оповестить о приходе контейнеров в след месяце 
		ИмяФайла = ПолучитьИмяВременногоФайла(); 
		ДвоичныеДанные = ТекСсылка.ХранилищеВнешнейОбработки.Получить(); 
		ДвоичныеДанные.Записать(ИмяФайла); 
		Если ТекСсылка.ВидОбработки = Перечисления.ВидыДополнительныхВнешнихОбработок.Отчет Тогда 
...
//	попытка
		ТекСсылка=Справочники.ВнешниеОбработки.НайтиПоКоду("303");//РЗ_Оповестить о приходе контейнера по заданию на отгр 
		ИмяФайла = ПолучитьИмяВременногоФайла(); 
		ДвоичныеДанные = ТекСсылка.ХранилищеВнешнейОбработки.Получить(); 
		ДвоичныеДанные.Записать(ИмяФайла); 
		Если ТекСсылка.ВидОбработки = Перечисления.ВидыДополнительныхВнешнихОбработок.Отчет Тогда 
...
Процедура ЗагрузитьЗаявкиНаБракССайта() Экспорт
	//попытка
		//ТекСсылка=Справочники.ВнешниеОбработки.НайтиПоКоду("");//.НайтиПоНаименованию("Получить заявки на брак"); 
		//ИмяФайла = ПолучитьИмяВременногоФайла(); 
		//ДвоичныеДанные = ТекСсылка.ХранилищеВнешнейОбработки.Получить(); 
		//ДвоичныеДанные.Записать(ИмяФайла); 
		//Если ТекСсылка.ВидОбработки = Перечисления.ВидыДополнительныхВнешнихОбработок.Отчет Тогда 
...
//	попытка
		ТекСсылка=Справочники.ВнешниеОбработки.НайтиПоКоду("333");//Напоминание о сезонных заказах поставщикам 
		ИмяФайла = ПолучитьИмяВременногоФайла(); 
		ДвоичныеДанные = ТекСсылка.ХранилищеВнешнейОбработки.Получить(); 
		ДвоичныеДанные.Записать(ИмяФайла); 
		Если ТекСсылка.ВидОбработки = Перечисления.ВидыДополнительныхВнешнихОбработок.Отчет Тогда 
...
//+++ 27.02.2015 --- по программе Топаз АЗС -> регистр сведений ОстаткиТоплива 
 ...
 Процедура НапоминаниеДатаОплатыШинтрейд() Экспорт
	//	попытка
		ТекСсылка=Справочники.ВнешниеОбработки.НайтиПоКоду("358");//Напоминание об оплате Шинтрейд 
		ИмяФайла = ПолучитьИмяВременногоФайла(); 
		ДвоичныеДанные = ТекСсылка.ХранилищеВнешнейОбработки.Получить(); 
		ДвоичныеДанные.Записать(ИмяФайла); 
		Если ТекСсылка.ВидОбработки = Перечисления.ВидыДополнительныхВнешнихОбработок.Отчет Тогда 
...
Процедура СоздатьДокументОповещения(Менеджер,СтрокаЗаказов,ТаблицаЗаказов)
	...
	//***2019.03.18 Задача № 64740
	Если СокрЛП(Менеджер.Код) = "Филатова С.В."
		ИЛИ СокрЛП(Менеджер.Код) = "Фончинкова О.А."
		Тогда
		ДокСобытие = Документы.Событие.СоздатьДокумент();
		ДокСобытие.ТипСобытия			 = Перечисления.ВходящееИсходящееСобытие.Входящее;
		ДокСобытие.Важность				 = Перечисления.Важность.Высокая;
		...
		ДокСобытие.ОписаниеСобытия		 = "Закрытие заказов покупателей";
		ДокСобытие.СодержаниеСобытия	 = СтрокаЗаказов;
		ДокСобытие.Ответственный		 = Справочники.Пользователи.НайтиПоКоду("Соболева Л.");;
		ДокСобытие.ЗаказыПокупателей.Загрузить(ТаблицаЗаказов);
		ДокСобытие.Записать(РежимЗаписиДокумента.Проведение);
	КонецЕсли;
...
Процедура СозданиеЗадачиПользователямПоЗаявкамНаВозвратПоставщику() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Разница = 0 Тогда
			...
			// Делаем задачу операторам склада
			НоваяЗадача = Задачи.ЗадачиПользователя.СоздатьЗадачу();
			НоваяЗадача.Исполнитель = Справочники.Пользователи.НайтиПоКоду("Пименова Н.И.");
			НоваяЗадача.Дата = ТекущаяДата();                                                      
			НоваяЗадача.Оповещение = Истина;
			НоваяЗадача.СрокОповещения = ТекущаяДата();
			НоваяЗадача.Наименование = "Нужно сделать возврат по заявке на брак поставщику";
...
//	попытка
		ТекСсылка=Справочники.ВнешниеОбработки.НайтиПоКоду("602");//РЗ_Оповестить клиентов о приходе заказа
		ИмяФайла = ПолучитьИмяВременногоФайла(); 
		ДвоичныеДанные = ТекСсылка.ХранилищеВнешнейОбработки.Получить(); 
		ДвоичныеДанные.Записать(ИмяФайла); 
		Если ТекСсылка.ВидОбработки = Перечисления.ВидыДополнительныхВнешнихОбработок.Отчет Тогда 
...
Процедура СоздатьПоступленияВБазеФранчайзи() Экспорт
	//попытка
		ТекСсылка = Справочники.ВнешниеОбработки.НайтиПоКоду("699");//РЗ_Создать поступления в базе франчайзи
		ИмяФайла = ПолучитьИмяВременногоФайла(); 
		ДвоичныеДанные = ТекСсылка.ХранилищеВнешнейОбработки.Получить(); 
		ДвоичныеДанные.Записать(ИмяФайла); 
		Если ТекСсылка.ВидОбработки = Перечисления.ВидыДополнительныхВнешнихОбработок.Отчет Тогда 
...
Процедура АвтоматическоеСписаниеКрышек() Экспорт
	//попытка
		ТекСсылка=Справочники.ВнешниеОбработки.НайтиПоНаименованию("РЗ_Автоматическое списание крышек!",истина);
		ИмяФайла = ПолучитьИмяВременногоФайла(); 
		ДвоичныеДанные = ТекСсылка.ХранилищеВнешнейОбработки.Получить(); 
		ДвоичныеДанные.Записать(ИмяФайла); 
		Если ТекСсылка.ВидОбработки = Перечисления.ВидыДополнительныхВнешнихОбработок.Отчет Тогда 
...
//	попытка
		ТекСсылка=Справочники.ВнешниеОбработки.НайтиПоНаименованию("РЗ_Автоматическое перевыставление ПДЗ с клиента на перевозчика!",истина);
		ИмяФайла = ПолучитьИмяВременногоФайла(); 
		ДвоичныеДанные = ТекСсылка.ХранилищеВнешнейОбработки.Получить(); 
		ДвоичныеДанные.Записать(ИмяФайла); 
		Если ТекСсылка.ВидОбработки = Перечисления.ВидыДополнительныхВнешнихОбработок.Отчет Тогда 
...
функция ЕстьПриходыПоТоварамРаспродажи(Завтра)
	 ...
	 Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	                ...
	                |ИЗ
	                |	РегистрСведений.ЦеныДляРаспродажи.СрезПоследних(&Завтра,) КАК ЦеныДляРаспродажи
	 ...
	 ДатаПоследнейУстановки = ОбменСУТИнтернетМагазин.ПолучитьДатуЦеныРаспродажи( ЛОЖЬ, Завтра+1 ); 
...
//+++ 07-12.03.2019 вспомогательная функция для ВыполнитьУдалениеЦенРаспродажи()
функция УстановитьНовыеЦеныРаспродажи(Завтра)
	 ...
	 Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	                ...
	                |	ВЫБОР
	                |		КОГДА ЦеныДляРаспродажиСрезПоследних.ЗаписьАктивна
	                |			ТОГДА &USD
	                |		ИНАЧЕ &Руб
	                |	КОНЕЦ КАК Валюта,
	                ...
	                |ИЗ
	                |	(ВЫБРАТЬ
	                |		ЦеныДляРаспродажиСрезПоследних.Номенклатура КАК Номенклатура,
	                  |		ЦеныДляРаспродажиСрезПоследних.ЗаписьАктивна КАК ЗаписьАктивна,
	                |		ВЫБОР
	                |			КОГДА ЦеныДляРаспродажиСрезПоследних.Период = &ДатаПоследнейУстановки
	                |				ТОГДА ЦеныДляРаспродажиСрезПоследних.Цена
	                |			ИНАЧЕ 0
	                |		КОНЕЦ КАК Цена
	                |	ИЗ
	                |		РегистрСведений.ЦеныДляРаспродажи.СрезПоследних(&Завтра, ) КАК ЦеныДляРаспродажиСрезПоследних
	                |	ГДЕ
	                |		ЦеныДляРаспродажиСрезПоследних.Цена > 0) КАК ЦеныДляРаспродажиСрезПоследних
	                |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.Закупки.Обороты(&НачДата, &Завтра, , ) КАК ЗакупкиОбороты
	 ...
	 Запрос.УстановитьПараметр("Завтра", Завтра);//после последней установки! повторный вызов должен возвращать Результат.пустой()
	 Результат = Запрос.Выполнить();
 ...
 Если НЕ Результат.Пустой() тогда 
	...
	попытка
		...
		для каждого стр из табл Цикл
			...
			//12.03.2019 - список что исключили! Цена = 0
			Если стр.КоличествоЗакупки>0 тогда 
				Если найти(стрИскл, стр.НоменклатураКод+",")=0 тогда
					стрИскл = стрИскл + стр.НоменклатураКод+",";
				КонецЕсли;
				новСтр.Цена = 0; // так же зануляем!
			КонецЕсли;	
			
			Если новСтр.Цена>0 тогда //считаем товары...
				Если стр.ЗаписьАктивна тогда 
					КолUSD = КолUSD+1;	
				иначе 
					КолРуб=КолРуб+1;
				КонецЕсли;
		...
		Nискл = СтрЧислоВхождений(стрИскл,",");
		Док.Комментарий  = "Создан автоматически(Р) - установка "+строка(КолРуб)+"/"+строка(Док.Товары.Количество()) +" тов.(исключено "+строка( Nискл )+" тов."+?(Nискл<100,":"+стрИскл,"")+")";
		ДокВ.Комментарий = "Создан автоматически($) - установка "+строка(КолUSD)+"/"+строка(ДокВ.Товары.Количество())+" тов.(исключено "+строка( Nискл )+" тов."+?(Nискл<100,":"+стрИскл,"")+")";
	...
	//--- 12.03.2019 ---------убираем нулевые и искл. товары---------------------
		i=0; N= табл.Количество();
		пока i<N цикл
			если табл[i].Цена=0 или табл[i].КоличествоЗакупки>0 тогда
...
функция УжеЕстьЗаписьЦенПовышения(выборкаТов, Вчера) 
	 ...
	 Запрос.Текст = "ВЫБРАТЬ *
	                |ИЗ
	                |	РегистрСведений.ЦеныПовышения.СрезПоследних(&Вчера,
	                |			ВариантКод = &ВариантКод
	                |				И Номенклатура = &Номенклатура
	                |				) КАК ЦеныПовышенияСрезПоследних
					|ГДЕ Цена = &Цена";  //+++ 13.03.2019
...
//	попытка
		ТекСсылка=Справочники.ВнешниеОбработки.НайтиПоНаименованию("РЗ_Данные по ПДЗ по периодам",истина);
		ИмяФайла = ПолучитьИмяВременногоФайла(); 
		ДвоичныеДанные = ТекСсылка.ХранилищеВнешнейОбработки.Получить(); 
		ДвоичныеДанные.Записать(ИмяФайла); 
		Если ТекСсылка.ВидОбработки = Перечисления.ВидыДополнительныхВнешнихОбработок.Отчет Тогда 
...
Процедура ОтложенноеПроведениеУстановкиЦен() Экспорт
	//попытка
		ТекСсылка=Справочники.ВнешниеОбработки.НайтиПоНаименованию("РЗ_Отложенное проведение установки цен",истина);
		ИмяФайла = ПолучитьИмяВременногоФайла(); 
		ДвоичныеДанные = ТекСсылка.ХранилищеВнешнейОбработки.Получить(); 
		ДвоичныеДанные.Записать(ИмяФайла); 
		Если ТекСсылка.ВидОбработки = Перечисления.ВидыДополнительныхВнешнихОбработок.Отчет Тогда 
...
Процедура АвтоматическоеВыставлениеРеализацийНаУслугиИсправленияДисков() Экспорт
		...
		ТекСсылка = Справочники.ВнешниеОбработки.НайтиПоКоду("777");
		ИмяФайла = ПолучитьИмяВременногоФайла(); 
		ДвоичныеДанные = ТекСсылка.ХранилищеВнешнейОбработки.Получить(); 
		ДвоичныеДанные.Записать(ИмяФайла); 
		Обработка = ВнешниеОбработки.Создать(ИмяФайла,Ложь); 
...
КонецПроцедуры
Процедура АвтоПродлениеСезонныхЗаказовПокупателей() Экспорт
	
		ТекСсылка = Справочники.ВнешниеОбработки.НайтиПоКоду("778");
		ИмяФайла = ПолучитьИмяВременногоФайла(); 
		ДвоичныеДанные = ТекСсылка.ХранилищеВнешнейОбработки.Получить(); 
		ДвоичныеДанные.Записать(ИмяФайла); 
		Обработка = ВнешниеОбработки.Создать(ИмяФайла,Ложь); 
				
		Обработка.ПродлитьСезонныеЗаказы();
		
КонецПроцедуры
Процедура ПроверкаСтатусовДокументовЭДО() Экспорт
	//попытка
		...
		Обработка.ПолучитьСобытияИзмененийСтатусовДокументов();
	//исключение
	//	ЗаписьЖурналаРегистрации("Отложенное проведение установки цен",
	//	УровеньЖурналаРегистрации.Информация,
	//	,
