...
функция УстановитьНовыеЦеныРаспродажи(Завтра)
	 ...
	 Запрос = Новый Запрос;
	 Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	                |	ЦеныДляРаспродажиСрезПоследних.НоменклатураКод КАК НоменклатураКод,
	                ...
	                |		ИНАЧЕ &Руб
	                |	КОНЕЦ КАК Валюта,
	                |	ЦеныДляРаспродажиСрезПоследних.НоменклатураЕдиницаХраненияОстатков КАК ЕдиницаИзмерения,
	                ...
	                |		ЦеныДляРаспродажиСрезПоследних.Номенклатура.ЕдиницаХраненияОстатков КАК НоменклатураЕдиницаХраненияОстатков,
...
Процедура АвтоПродлениеСезонныхЗаказовПокупателей() Экспорт
	попытка
		ТекСсылка = Справочники.ВнешниеОбработки.НайтиПоНаименованию("Автоматическое продление сезонных заказов покупателей",истина);
		ИмяФайла = ПолучитьИмяВременногоФайла(); 
		ДвоичныеДанные = ТекСсылка.ХранилищеВнешнейОбработки.Получить(); 
		ДвоичныеДанные.Записать(ИмяФайла); 
		Если ТекСсылка.ВидОбработки = Перечисления.ВидыДополнительныхВнешнихОбработок.Отчет Тогда 
	...
	исключение
		ЗаписьЖурналаРегистрации("Автоматическое продление сезонных заказов покупателей",
		УровеньЖурналаРегистрации.Информация,
		,
		,
		ОписаниеОшибки(),
