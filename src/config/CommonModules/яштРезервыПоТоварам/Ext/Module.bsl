...
Процедура ЗаполнениеДанныхОперацийОТХвЗаявкахНаБрак() Экспорт
	...
	ДатаЗапрета = ДобавитьМесяц(НачалоДня(ТекущаяДата()),-3); // 3 месяца
	
	ЗапросОТХ = Новый Запрос;
	//+++ 24.01.2019 --- параметры сразу в запросе ---
	// в запросе используется >> ОперацияОТХ.Проведен и ОперацияОТХ.Дата ... поэтому нужен пустой документ
	ЗапросОТХ.УстановитьПараметр("ПустаяОперацияОТХ", Документы.ОперацияПоОтветственномуХранению.ПустаяСсылка() ); 
	ЗапросОТХ.УстановитьПараметр("ГоловноеПодразделение", справочники.Подразделения.НайтиПоКоду("00005"));
	//+++)
	ЗапросОТХ.УстановитьПараметр("Период", ДатаЗапрета );
	ЗапросОТХ.Текст = "ВЫБРАТЬ
	               |	ОперацияПоОтветственномуХранению.Ссылка,
	               |	ОперацияПоОтветственномуХранению.ДокументОснование
	               |ПОМЕСТИТЬ ВТ_ОперацииОТХ
	               |ИЗ
	               |	Документ.ОперацияПоОтветственномуХранению КАК ОперацияПоОтветственномуХранению
	               |ГДЕ
	               |	ОперацияПоОтветственномуХранению.Проведен
	               |	И ОперацияПоОтветственномуХранению.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоОтветственномуХранению.Поступление)
	               |	И ОперацияПоОтветственномуХранению.ДокументОснование ССЫЛКА Документ.ЗаявкаНаВозвратТоваров
				  //+++( 24.01.2019 - не филиал, значит = (Ярославль или Пустое)!
				  //|	И НЕ ОперацияПоОтветственномуХранению.Подразделение.ОбособленноеПодразделение
	               |	И (ОперацияПоОтветственномуХранению.Подразделение = &ГоловноеПодразделение
				   |		или ОперацияПоОтветственномуХранению.Подразделение = Значение(справочник.Подразделения.ПустаяСсылка)
	               |      )
				  //+++) 
				   |
	               |ОБЪЕДИНИТЬ
	               ...
	               |ГДЕ
	               |	ОперацияПоОтветственномуХранению.Проведен
				   |	И ОперацияПоОтветственномуХранению.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоОтветственномуХранению.ПеремещениеМеждуСкладами)
	               |	И ОперацияПоОтветственномуХранению.ДокументОснование ССЫЛКА Документ.ЗаявкаНаВозвратТоваров
				   //перемещение на склады Ярославля
				   |	И ОперацияПоОтветственномуХранению.СкладПолучатель.Подразделение = ЗНАЧЕНИЕ(справочник.Подразделения.ПустаяСсылка)
				   |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	А.ЗаявкаНаБрак,
				   |	МИНИМУМ(А.ОперацияОТХ.Дата) КАК ОперацияОТХДата,
	               |	А.ДатаПриходаОТХ,
				   //внизу есть условие и сортировка по ЗаявкаНаБракДата 
				   |	А.ЗаявкаНаБракДата
	               |ПОМЕСТИТЬ ВТ_ОТХ
	               |ИЗ
	               |	(ВЫБРАТЬ
	               |		ЗаявкаНаБрак.Ссылка КАК ЗаявкаНаБрак,
	               |		ЗаявкаНаБрак.Дата   КАК ЗаявкаНаБракДата,
				   |		ЗаявкаНаБрак.ДатаПриходаОТХ как ДатаПриходаОТХ,
	               |		ЗаявкаНаБрак.ДокументОснование КАК ДокументОснование,
	               |		ЕСТЬNULL(ОперацияПоОтветственномуХранению.Ссылка, &ПустаяОперацияОТХ ) КАК ОперацияОТХ
	               |	ИЗ
	               |		Документ.ЗаявкаНаБрак КАК ЗаявкаНаБрак
	               |			ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ОперацииОТХ КАК ОперацияПоОтветственномуХранению
	               |			ПО ЗаявкаНаБрак.ДокументОснование = ОперацияПоОтветственномуХранению.ДокументОснование
	               |	ГДЕ
	               |		ЗаявкаНаБрак.ДокументОснование <> Значение(Документ.ЗаявкаНаВозвратТоваров.ПустаяСсылка)
	               |		И ЗаявкаНаБрак.ДокументОснование ССЫЛКА Документ.ЗаявкаНаВозвратТоваров
				   ...
				   //+++ 24.01.2019 Сразу нижнее условие!  значительно сокращает выборку
				   |		И ЗаявкаНаБрак.Дата >= &Период
				   |		) КАК А
	               |ГДЕ
	               |	  А.ОперацияОТХ <> &ПустаяОперацияОТХ 
	               ...
	               |ВЫБРАТЬ
	               |	А.ДокументОснование,
	               |	МИНИМУМ(А.ОперацияОТХ.Дата),
	               |	А.ДокументОснованиеДатаПриходаОТХ,
				   |	А.ДокументОснованиеДата
	               |ИЗ
	               |	(ВЫБРАТЬ
	               |		ЗаявкаНаВозврат.Ссылка КАК Ссылка,
	               |		ЗаявкаНаВозврат.ДокументОснование КАК ДокументОснование,
				   |		ЗаявкаНаВозврат.ДокументОснование.ДатаПриходаОТХ как ДокументОснованиеДатаПриходаОТХ,
				   |		ЗаявкаНаВозврат.ДокументОснование.Дата как ДокументОснованиеДата,
 				   |		ЕСТЬNULL(ОперацияПоОтветственномуХранению.Ссылка, &ПустаяОперацияОТХ ) КАК ОперацияОТХ
	               |	ИЗ
	               ...
	               |	ГДЕ
	               |		ЗаявкаНаВозврат.ДокументОснование <> Значение(Документ.ЗаявкаНаБрак.ПустаяСсылка)
	               |		И ЗаявкаНаВозврат.ДокументОснование ССЫЛКА Документ.ЗаявкаНаБрак
	               |		И ЗаявкаНаВозврат.ДокументОснование.СостояниеЗаявки <> ЗНАЧЕНИЕ(Перечисление.СостоянияЗаявкиНаБрак.ПризнатьБрак)
	               |		И ЗаявкаНаВозврат.ДокументОснование.СостояниеЗаявки <> ЗНАЧЕНИЕ(Перечисление.СостоянияЗаявкиНаБрак.Отклонить)
				   //+++( 24.01.2019 Сразу нижние условие! значительно сокращает выборку
				   |		И выразить(ЗаявкаНаВозврат.ДокументОснование как Документ.ЗаявкаНаБрак).Дата >= &Период
				   |		И ЕСТЬNULL(ОперацияПоОтветственномуХранению.Ссылка, &ПустаяОперацияОТХ ) <> &ПустаяОперацияОТХ
				   //+++)
	           	   |	) КАК А
	               |
	               |СГРУППИРОВАТЬ ПО
	               ...
	               |ВЫБРАТЬ
	               |	ВТОТХ.ЗаявкаНаБрак,
	               |	ВТОТХ.ОперацияОТХДата
	               |ИЗ
	               |	ВТ_ОТХ КАК ВТОТХ
	               |ГДЕ
	               |	НАЧАЛОПЕРИОДА(ВТОТХ.ОперацияОТХДата, ДЕНЬ) <> НАЧАЛОПЕРИОДА(ВТОТХ.ДатаПриходаОТХ, ДЕНЬ)
	             // УЖЕ сделан отбор выше!  
				   //|	И ВТОТХ.ЗаявкаНаБракДата >= &Период
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	ВТОТХ.ЗаявкаНаБракДата
				   |;
				   |//////////////////////////////////////////////////////////////////////////////////
				   |УНИЧТОЖИТЬ ВТ_ОТХ;
				   |УНИЧТОЖИТЬ ВТ_ОперацииОТХ;
				   |";
	...
	Если ВыборкаРезультат.Количество()>0 Тогда
		...
		//сама обработка заявок
		Пока ВыборкаРезультат.Следующий() Цикл
			ЗаявкаНаБрак = ВыборкаРезультат.ЗаявкаНаБрак;
			Если ЗначениеЗаполнено(ЗаявкаНаБрак) тогда//+++ BugFix 24.01.2019 - не найдено поле ВыборкаРезультат.ЗаявкаНаБрак
				ДокОбъект 				 = ЗаявкаНаБрак.ПолучитьОбъект();  
				ДокОбъект.ДатаПриходаОТХ = ВыборкаРезультат.ОперацияОТХДата; 
