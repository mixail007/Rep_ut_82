...
Процедура ЗаполнениеДанныхОперацийОТХвЗаявкахНаБрак() Экспорт
	...
	Запрос.УстановитьПараметр("Период", ДобавитьМесяц(ТекущаяДата(),-3));
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ГДЕ
	               |	ОперацияПоОтветственномуХранению.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоОтветственномуХранению.Поступление)
	               ...
	               |	Документ.ОперацияПоОтветственномуХранению КАК ОперацияПоОтветственномуХранению
	               |ГДЕ
	               |	ОперацияПоОтветственномуХранению.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоОтветственномуХранению.ПеремещениеМеждуСкладами)
	               |	И ОперацияПоОтветственномуХранению.ДокументОснование ССЫЛКА Документ.ЗаявкаНаВозвратТоваров
	               |	И ОперацияПоОтветственномуХранению.СкладПолучатель.Подразделение = &ПустоеПодразделение
	               |	И ОперацияПоОтветственномуХранению.Проведен
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	А.Ссылка КАК ЗаявкаНаБрак,
	               |	МИНИМУМ(А.ОперацияОТХ.Дата) КАК ОперацияОТХДата,
	               |	А.Ссылка.ДатаПриходаОТХ
	               |ПОМЕСТИТЬ ВТ_ОТХ
	               |ИЗ
	               |	(ВЫБРАТЬ
	               ...
	               |		И ЗаявкаНаБрак.СостояниеЗаявки <> ЗНАЧЕНИЕ(Перечисление.СостоянияЗаявкиНаБрак.Отклонить)) КАК А
	               |ГДЕ
	               |	А.ОперацияОТХ <> 0
	               ...
	               |	А.ДокументОснование.ДатаПриходаОТХ
	               |ИЗ
	               |	(ВЫБРАТЬ
	               ...
	               |		И ЗаявкаНаВозврат.ДокументОснование.СостояниеЗаявки <> ЗНАЧЕНИЕ(Перечисление.СостоянияЗаявкиНаБрак.Отклонить)) КАК А
	               |ГДЕ
	               |	А.ОперацияОТХ <> 0
	               |
	               |СГРУППИРОВАТЬ ПО
	               ...
	               |	ВТ_ОТХ КАК ВТ_ОТХ
	               |ГДЕ
	               |	НАЧАЛОПЕРИОДА(ВТ_ОТХ.ОперацияОТХДата, ДЕНЬ) <> НАЧАЛОПЕРИОДА(ВТ_ОТХ.СсылкаДатаПриходаОТХ, ДЕНЬ)
	               |	И ВТ_ОТХ.ЗаявкаНаБрак.Дата >= &Период
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	ВТ_ОТХ.ЗаявкаНаБрак.Дата";
	Результат = Запрос.Выполнить().Выбрать();
	
	Если Результат.Количество() > 0 Тогда
		
		ДатаПервойЗаявки = Результат[0].ЗаявкаНаБрак.Дата;
		//ДатаЗапрета = ДобавитьМесяц(НачалоДня(ДатаПервойЗаявки), -1); // - 1 месяц на всякий случай
		//
		////открытие периода
		//Запрос = Новый Запрос;
		//Запрос.Текст = "ВЫБРАТЬ
		...
		//ВыборкаРолей = Запрос.Выполнить().Выгрузить();
		//		
		...
		//Для Каждого СтрТЗ ИЗ ВыборкаРолей Цикл
		...
		//Пока Результат.Следующий() Цикл
		//	ДокОбъект = Результат.ЗаявкаНаБрак.ПолучитьОбъект();
		//	ДокОбъект.ДатаПриходаОТХ = Результат.ОперацияОТХДата;
		//	Попытка 
