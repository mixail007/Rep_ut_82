...
Функция РассылкаСообщенийОПДЗ(АдресОтправки="", ИмяКаталога0="", ХранитьВБазе1С=ЛОЖЬ, КодКлиента="") Экспорт
	...
	УЗ = Справочники.УчетныеЗаписиЭлектроннойПочты.НайтиПоНаименованию(АдресОтправки);
 	КомуСоздаватьЗадачуОбОтсутвииЭлАдресов = "Серков";
...
Процедура ПереключениеПравилРезерва() Экспорт
	...
	МинДоляТоваровВОдномПравиле = 0.50;
	ВыполнитьПереключениеПравилРезерва(МинДоляТоваровВОдномПравиле);
	
КонецПроцедуры

//основная функция - вынесена отдельно для "ручного" запуска...
Процедура ВыполнитьПереключениеПравилРезерва(МинДоляТоваровВОдномПравиле=0.50) Экспорт
...
//1) для всех Клиентов (вообще!) находим особые правила от -7001 и выше
УдалитьВсеПравилаПошышения();

//2) находим товары в резерве>0 по клиенту "Резерв для ИМ"----
товарыВРезервеИСвОст = ПолучитьТаблВРезервеИСвОст(КлиентРезерв);
...
ПравилаДляВключения = ПолучитьПравилаДляВключения(товарыВРезервеИСвОст, таблЦен, КлиентРезерв); 

//в рег.св.ПравилаЦенообразования этих клиентов пишем в перс.правила: -8000 до -7001
	Для каждого правило	из ПравилаДляВключения цикл
		Если правило.КолТоваровНадоПереключать <= МинДоляТоваровВОдномПравиле*правило.КолТоваров тогда
			#Если Клиент тогда
			Сообщить(строка(правило.Приоритет)+") "+строка(правило.Контрагент)
			+" - товаров для переключения: "+строка(правило.КолТоваровНадоПереключать)+" < всех товаров "+строка(правило.КолТоваров)+" ("+формат(МинДоляТоваровВОдномПравиле,"ЧДЦ=2")+"%)" ); 
			#КонецЕсли
			продолжить;
		КонецЕсли;
		регСв = РегистрыСведений.ПравилаЦенообразования.СоздатьМенеджерЗаписи();
			ЗаписатьПравилоВЖурналИзменений(Правило, Истина);
			ЗаполнитьЗначенияСвойств(регСв, Правило); // только измерения!
		регСв.Записать(Истина);
	КонецЦикла;
  
КонецПроцедуры	

процедура УдалитьВсеПравилаПошышения()
Запрос = Новый Запрос;
Запрос.Текст = "ВЫБРАТЬ
               ...
               |ГДЕ
               |	ПравилаЦенообразования.Приоритет < -7000
               |
               |УПОРЯДОЧИТЬ ПО
               |	Контрагент,
               |	Категория
...
//ВЫКЛючаем все особые правила и пишем в рег.св. журнал изменений...правило -Удалено!
КолПравил = Выборка.Количество();
пока Выборка.Следующий() цикл
	регСв = РегистрыСведений.ПравилаЦенообразования.СоздатьМенеджерЗаписи();
	ЗаписатьПравилоВЖурналИзменений(Выборка, ЛОЖЬ);
	ЗаполнитьЗначенияСвойств(регСв, Выборка); // только измерения!
	регСв.Удалить();
КонецЦикла;
КонецПроцедуры

//вспомогательные функции для  ПереключениеПравилРезерва()
Процедура ЗаписатьПравилоВЖурналИзменений(регСв, Новое=Истина)
	записьЖурнала = РегистрыСведений.ЖурналИзменений.СоздатьМенеджерЗаписи();
		 записьЖурнала.НомерЗаписи = регСв.Приоритет;
		 записьЖурнала.Пользователь= справочники.Пользователи.НайтиПоКоду("Робот (магазин)");
		 записьЖурнала.Компьютер   = "регл.задание";
		 записьЖурнала.Объект	   = регСв.Контрагент;
		 ...
		 +?(Новое, "Добавлено" 
		 +"; Кат.:>>"+строка(регСв.Категория)
		 +"; Тип.цен:>>"+строка(регСв.ТипЦен)
		 +"; Скидка:>>"+строка(регСв.СкидкаНаценка)
		 +"; Предоплата:>>"+строка(регСв.СкидкаПредоплаты)
		       ," - Удалено!");
...
//СвОстаток
//
функция ПолучитьТаблВРезервеИСвОст(КонтрРезерв)
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ЗаказыПокупателейОстатки.Номенклатура,
	               ...
	               |						И Склад.Подразделение = Значение(Справочник.Подразделения.ПустаяСсылка)) КАК ост
				   |		
	               |		ОБЪЕДИНИТЬ ВСЕ
	               |		
	               |		ВЫБРАТЬ
...
функция ПолучитьПравилаДляВключения(товарыВРезервеИСвОст, таблЦен, КонтрРезерв)
	Запрос = Новый Запрос;
	...
	Запрос.Текст = "ВЫБРАТЬ
				  ...
				  //|	РезервыИОстатки.Код как НоменклатураКод,
	               |	РезервыИОстатки.ВРезерве,
	               |	РезервыИОстатки.СвОстаток,
	               |	ПравилаПереключения.N КАК СвОстатокN,
	               |	РезервыИОстатки.ВРезерве * ПравилаПереключения.k КАК СвОстатокK,
				   |	
				   |	выбор когда 
				   |			 (ПравилаПереключения.N = 0
				   |				ИЛИ ПравилаПереключения.N > 0
				   |					И РезервыИОстатки.СвОстаток < ПравилаПереключения.N )
				   |			И (ПравилаПереключения.k = 0
				   |				ИЛИ ПравилаПереключения.k > 0
				   |					И РезервыИОстатки.СвОстаток < ПравилаПереключения.k * РезервыИОстатки.ВРезерве )
 	   			   |тогда 1 иначе 0 Конец ТоварНадоПереключать,
				   |
				   //--------доп.привязка по 1 правилу--------
	               |	ПравилаПереключения.ID
				   |				   
				   ...
				   |из &таблЦен как таблЦен;
				   
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	таблЦен.Номенклатура КАК НоменклатураЦены,
	               |	таблЦен.МинимальнаяЦена КАК Цена,
	               |	таблЦен.ЦенаПоТипуЦен КАК ЦенаБазовая,
	               |	таблЦен.Приоритет,
	               ...
	               |	ПравилаЦенообразования.ОТК,
	               |	ПравилаЦенообразования.Коэф,
	               |	ПравилаЦенообразования.СкидкаПредоплаты,
	               |	ПравилаЦенообразования.ВидДефектаДляУценки,
	               |	ПравилаЦенообразования.Бонус1
	               |ПОМЕСТИТЬ ВТ_Цены
	               |ИЗ
	               |	ВТ_ТаблЦен КАК таблЦен
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ПравилаЦенообразования КАК ПравилаЦенообразования
	               |		ПО таблЦен.Приоритет = ПравилаЦенообразования.Приоритет
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.РезервРазрешения.СрезПоследних(, КонтрагентРезерв = &КонтрРезерв) КАК РезервРазрешения
	               |		ПО (ПравилаЦенообразования.ВидТовара = РезервРазрешения.ВидТовара)
	               |			И (ПравилаЦенообразования.НоменклатурнаяГруппа = РезервРазрешения.НоменклатурнаяГруппа
	               |				ИЛИ РезервРазрешения.НоменклатурнаяГруппа = ЗНАЧЕНИЕ(Справочник.НоменклатурныеГруппы.ПустаяСсылка))
	               |ГДЕ
	               |	ПравилаЦенообразования.Контрагент = &КонтрРезерв
	               |	И РезервРазрешения.ДатаОкончания = ДАТАВРЕМЯ(1, 1, 1)
	               |	И таблЦен.Приоритет >= 0
	               |	И таблЦен.Приоритет <= 1000
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
				   //-------------Основные измерения-----------------
	               |	Цены.Контрагент,
	               |	-8000 + Цены.Приоритет КАК Приоритет,
	               |	Цены.Категория,
				   
				  //================Перед установкой правил - надо сравнить...============================  
				   |	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Цены.НоменклатураЦены) КАК КолТоваров,
				   |    Сумма( ТоварыДляПереключения.ТоварНадоПереключать ) как КолТоваровНадоПереключать,
				   
					//================Лишние товары=======================   
	     		   //|	Среднее(Цены.Цена) КАК Цена,
	               //|	Среднее(Цены.ЦенаБазовая) КАК ЦенаБазовая,
				   // |	Сумма(ТоварыДляПереключения.ВРезерве) как ВРезерве,
				   
				   |	Сумма(ТоварыДляПереключения.СвОстаток)  как СвОстаток,
			       |	Сумма(ТоварыДляПереключения.СвОстатокN) как СвОстатокN,
	               |	Сумма(ТоварыДляПереключения.СвОстатокK) как СвОстатокK,
	               ...
	               |	Цены.ОТК,
	               |	Цены.Коэф,
	               |	Цены.СкидкаПредоплаты,
	               |	Цены.ВидДефектаДляУценки,
	               |	Цены.Бонус1
	               |ИЗ
	               |	ВТ_Цены КАК Цены
	               ...
	               |			И (ТоварыДляПереключения.ID = """" ИЛИ ТоварыДляПереключения.ID = Цены.ID)
	               ...
	               |	Цены.Бонус1
	               |
				   //=================Интегральные остатки ПО ВСЕМ товарам 1 правила!====================
				   |ИМЕЮЩИЕ 	 (Сумма(ТоварыДляПереключения.СвОстатокN) = 0
				   |				ИЛИ Сумма(ТоварыДляПереключения.СвОстатокN) > 0
				   |					И Сумма(ТоварыДляПереключения.СвОстаток) < Сумма(ТоварыДляПереключения.СвОстатокN) )
				   
				   |			И (Сумма(ТоварыДляПереключения.СвОстатокK) = 0
				   |				ИЛИ Сумма(ТоварыДляПереключения.СвОстатокK) > 0
				   |					И Сумма(ТоварыДляПереключения.СвОстаток) < Сумма(ТоварыДляПереключения.СвОстатокK) )
	               ...
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |УНИЧТОЖИТЬ ВТ_РезервыИОстаткиДляПереключения;
