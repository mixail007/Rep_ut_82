...
Процедура ПослатьЭлектронноеПисьмо(АдресПолучателя, СписокФайловВложений, УЗ, ТекстСообщения = "", Тема = "", Отправитель="",Копии = "", Уведомления = Ложь) Экспорт
	
	
	////30.01.2018 ==============================================
	Если УЗ.Наименование="1c@yst.ru" Тогда    //1c@yst.ru - англ.
		текстСписок = "";
	  ...
	  Если ДеньНедели(ТекущаяДата()) = 1 Тогда //По понедельникам рассылка 
		  ...
		  попытка
			 ...
			 // НачДата   = НачалоНедели( ТекущаяДата() ) - 7*86400; // начало пред.недели
			  ...
			  АдресПочты = "malyshev@yst.ru;ogorodnikov@yst.ru; novikova@yst.ru; ft@yst.ru; tigran@yst.ru";	 //
			  СписокФайловВложений = новый СписокЗначений;
...
Процедура РассылкаОтчетовДляПелевина() Экспорт
	...
	Попытка
		...
		АдресПочты = "malyshev@yst-group.com;Pelevin-s@mail.ru";
...
Процедура РассылкаПДЗИПредупредительногоПисьмаДолжнику() Экспорт
	...
	отКого = "no-reply@yst76.ru";
	ИмяКаталога = "\\sigma\Общая папка\Рассылка писем о ПДЗ\"; //только на SIGMA !
    ХранитьВБазе1С = ЛОЖЬ; //16.08.2017 - НЕ ХРАНИМ в 1С
	
	РассылкаПредупредительногоПисьмаДолжнику(отКого, ИмяКаталога, ХранитьВБазе1С);
 	колКлиентовПДЗ   = РассылкаСообщенийОПДЗ(отКого, ИмяКаталога, ХранитьВБазе1С);  //17.08.17 - сделана Функцией
		...
		Для каждого дог из Договоры Цикл
			...
			НачатьТранзакцию();
				...
				Для каждого зак из Заказы Цикл
					НовыйЗаказ.Комментарий = НовыйЗаказ.Комментарий+ Зак.ЗаказПокупателя.Номер+"; ";
...
Процедура СоздатьПоступленияВБазеФранчайзи() Экспорт
	попытка
		...
		Иначе 
			Обработка = ВнешниеОбработки.Создать(ИмяФайла,Ложь); 
		КонецЕсли;
		Обработка.ОтправитьПеремещения();
	исключение
