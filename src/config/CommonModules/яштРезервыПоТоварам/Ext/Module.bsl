...
Процедура ПодпискаНаСобытие1ПередЗаписью(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
		...
		Тогда // ДАТУ НЕ МЕНЯЕМ!
			...
			#Если Клиент Тогда
			сообщить("Документ уже был проведен "+строка(Источник.Дата)+"! Дату и время документа менять НЕЛЬЗЯ, поэтому проведение Перемщения № "+строка(Источник.Номер)+" будет выполнено НЕ оперативно, Дата не изменилась!", СтатусСообщения.Информация);
			#КонецЕсли	
