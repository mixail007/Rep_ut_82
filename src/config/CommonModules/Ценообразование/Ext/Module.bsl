...
Функция ПолучитьСуммуДокументаСНДС(ДокументОбъект, ИмяТабличнойЧасти = Неопределено, НеУчитыватьТару = Истина) Экспорт
	...
	Иначе
		Для каждого ТЧОбъекта Из ДокументОбъект.Метаданные().ТабличныеЧасти Цикл
			Если (НеУчитыватьТару и ТЧОбъекта.Имя = "ВозвратнаяТара")  или ТЧОбъекта.Имя = "ОплатаТруда" Тогда
				Продолжить;
			КонецЕсли;
...
//+++ 10.12.2018 - общая функция для всех
//
//ЧисловаяСтавкаНДС = ПолучитьСтавкуНДС( СтавкаНДСнаДату( дата1 ) );  // 18 или 20 с нового 2019 года!
//
функция СтавкаНДСнаДату(Дата1='00010101') ЭКСПОРТ
	СтавкаНДС = Перечисления.СтавкиНДС.НДС20;
	Если Дата1<'20190101' тогда // до 01.01.2019 00:00:00 - ставка была 18%
