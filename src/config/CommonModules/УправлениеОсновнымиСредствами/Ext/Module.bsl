...
Функция РасчетАмортизацииУпр(ДатаРасчета,ОсновноеСредство = Неопределено, ВыдаватьСообщения = Истина) Экспорт
	...
	ТаблицаАмортизации.Колонки.Добавить("НаправлениеАмортизации", Новый ОписаниеТипов("СправочникСсылка.СпособыОтраженияРасходовПоАмортизации"));
	ТаблицаАмортизации.Колонки.Добавить("ДатаПринятияКУчету", ПолучитьОписаниеТиповДаты(ЧастиДаты.Дата));
...
|				И (СостоянияОС.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияОС.ПринятоКУчету))";
	...
	Пока ВыборкаПоОС.Следующий() Цикл
		...
		СтрокаАмортизации.ДатаПринятияКУчету	 = ВыборкаПоОС.ДатаПринятияКУчету;
...
//	ТабЗатрат.Колонки.Добавить("СпособРаспределенияЗатратНаВыпуск", Новый ОписаниеТипов("СправочникСсылка.СпособыРаспределенияЗатратНаВыпуск"));
	...
	Для Каждого СтрокаТЗ Из ТабАмортизации Цикл
		...
		Для Каждого СтрокаНапр Из СтрокаТЗ.НаправлениеАмортизации.Способы Цикл
			...
			//*** Задача 47284 
			Если ДатаРасчета < '20180101000000' Тогда
				НоваяСтрока.Подразделение           = СтрокаНапр.Подразделение; 
			Иначе //начиная с документов 2018
				Если СтрокаТЗ.ДатаПринятияКУчету < '20180101000000' Тогда  //принятые до 2018 - на дирекцию
					НоваяСтрока.Подразделение           = Справочники.Подразделения.НайтиПоКоду("00004");//*** Дирекция
				Иначе //принятые после 2018 - на свое подразделение
					НоваяСтрока.Подразделение           = СтрокаНапр.Подразделение; 
				КонецЕсли;
			КонецЕсли;
