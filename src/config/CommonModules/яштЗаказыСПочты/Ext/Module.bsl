...
процедура ЗаполнитьШапкуЗаказаПокупателя( Заказ, Контрагент )
	...
	Заказ.ВыписыватьСчет       = Контрагент.ПечататьСчетИзРеализации;
...
процедура СоздатьУведомлениеОбОшибке(ИмяФайлаПолное, стр1, табЗначОтвет, ЗаказСсылка) экспорт
	...
	ЕстьОшибки = (табЗначОтвет.Итог("КоличествоОтказ")<>0); 
	флСоздаватьЗадачуМенеджеру = ЕстьОшибки; //только если ЕстьОшибки !
...
Если флСоздаватьЗадачуМенеджеру тогда
	...
	Задача1.Наименование =  ?(ЕстьОшибки,"УСПЕШНО","НЕ ПОЛНОСТЬЮ")+" Загружен с эл.почты @ файла Excel в Заказ для покупателя "+строка(стр1.Контрагент);
	Задача1.Дата = ТекущаяДата();
