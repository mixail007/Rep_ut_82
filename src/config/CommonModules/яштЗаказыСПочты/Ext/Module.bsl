...
Если СтрКонтрагент=неопределено тогда // не должно быть такого!
	...
	Запрос1.УстановитьПараметр("ЦенаExcel", ЦенаExcel);//число
	 Запрос1.текст = "ВЫБРАТЬ
					 ...
					 |						И ВЫБОР КОГДА Номенклатура.ВидТовара = &Диски
...
Функция НайтиТовар(Артикул="", Код="", ВидТовара1=неопределено, ЦенаExcel=0, стрКонтрагент=неопределено)	
	...
	Иначе //все "нормальные" виды... БЕЗ рекламной продукции
	...
	Если Код<>"" тогда //по коду точнее
		Запрос.Текст = стрЗаменить(запрос.Текст, "//И условие поиска по коду или артикулу", "И Номенклатура.Код = &Код");
	...
	Иначе
		...
		Запрос.Текст = стрЗаменить(Запрос.Текст, "//И условие поиска по коду или артикулу", "И (Номенклатура.Артикул = &Артикул ИЛИ Номенклатура.АртикулСокр = &Артикул)");
	...
	Если табл1.Количество()>0 тогда
		...
		табл2 = НайтиБлижайшийТоварПоЦенамКлиента(табл1, ЦенаExcel, стрКонтрагент); 
		тов1  = табл2[0].Товар;
		#Если Клиент тогда
		сообщить("Найден ближайщий по цене товар: "+строка(тов1.Код)+" по цене: "+строка(табл2[0].Цена)+"р. (цены Excel: "+строка(ЦенаExcel)+" р.)",  СтатусСообщения.Информация);
		#КонецЕсли	
	...
	Иначе
		...
		запрос.Текст = стрЗаменить(запросТекст0, "//И условие поиска по коду или артикулу", "  И (Номенклатура.Артикул подобно (&Арт1) 
		...
		Иначе
			...
			Если Код="" и стрДлина(Артикул)=7 тогда 
				...
				Если колЧисел=7 тогда//только в ЭТОМ случае - поиск по Коду!
					запрос.Текст = стрЗаменить(запросТекст0, "//И условие поиска по коду или артикулу", "И Номенклатура.Код = &Артикул");
					Результат = Запрос.Выполнить();
					табл1 = Результат.Выгрузить();  //+++ 26.03.2019 нужна таблицы для доп.анализа дублей!
