		...
		Попытка //возможна ошибка при записи 
			...
			имя1 = темаПисьма+Вложение.ИмяФайла;
				...
				Иначе
					...
					Если прав(имя1,4)=".xls"
						или прав(имя1,4)="xlsx" тогда  //+++ 17.05.2018
...
функция ПолучитьНастройкиЗагрузкиЗаказовИзExcel() экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               |	НастройкаЗагрузкиЗаказовИзExcel.*,
	              ...
	              // |	,КонтактнаяИнформация.Комментарий
	               ...
	               |						нн.Контрагент
	               |					ИЗ
	               |						РегистрСведений.НастройкаЗагрузкиЗаказовИзExcel КАК нн)
	               |			И КИ.Тип = &Тип
...
Функция НайтиТовар(Артикул="", Код="", ВидТовара1=неопределено)	
	...
	КонецЕсли;	
	 ...
	 Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	                ...
	                |					И Номенклатура.ВидТовара В (&ВидыТоваров)
	                |					И Номенклатура.Артикул = &Артикул ) КАК отх 
	...
	Если выборка.Следующий() тогда
		...
		Если Выборка.Количество()>1 тогда
			#Если Клиент тогда
			сообщить("Найдено "+строка(Выборка.Количество())+" товаров по " + ?(Код<>"","Коду: "+Код, "Артикулу: "+Артикул), СтатусСообщения.Внимание);
			#КонецЕсли	
		 //   тов1 = выборка.Ссылка; //первый попавшийся НЕЛЬЗЯ! могут быть разные типы... акс и шины / диски
		Иначе
		 	тов1 = выборка.Ссылка;
		КонецЕсли;
	...
	//+++ 07.05.2018 поиск по части артикула!
		...
		|) ");
	 	...
	 	Если Выборка.Количество()>1 тогда
	 		#Если Клиент тогда
			сообщить("Найдено "+строка(Выборка.Количество())+" товара по Артикулу: "+Артикул+" через знак = ", СтатусСообщения.Внимание);
			#КонецЕсли	
		
			Выборка.Следующий();
    	 	тов1 = выборка.Ссылка; //06.06.2018 первый попавшийся аналог подойдёт (это точно все шины!)
		...
		Иначе
			...
			Если Код="" и стрДлина(Артикул)=7 тогда 
				...
				Если колЧисел=7 тогда//только в ЭТОМ случае - поиск по Коду!
	 			запрос.Текст = стрЗаменить(запросТекст0, "Артикул = &Артикул", "Код = &Артикул");
					Результат = Запрос.Выполнить();
					Выборка = Результат.Выбрать();
					 
				 	Если Выборка.Количество()>1 тогда
				 		#Если Клиент тогда
						сообщить("Найдено "+строка(Выборка.Количество())+" товара по Коду: "+Артикул+" (Вместо Артикула)", СтатусСообщения.Внимание);
						#КонецЕсли	
					...
					Иначе
						#Если Клиент тогда
						сообщить("НЕ Найден товар по Коду: "+Артикул, СтатусСообщения.Внимание);
						#КонецЕсли	
					КонецЕсли;	
			
				Иначе
					#Если Клиент тогда
					сообщить("НЕ Найден товар по Артикулу: "+Артикул+" (через знак =)", СтатусСообщения.Внимание);
					#КонецЕсли	
				КонецЕсли;		
			Иначе
				#Если Клиент тогда
...
функция СоздатьЗаказыИзФайловExcel(стр1, Путь="", элПочтаОтКого="")
	...
	для i=0 по стр1.списокФайлов.Количество()-1 цикл
	  ...
	  табЗначОтвет = СоздатьЗаказПоТабЗнач(табЗнач, стр1.Контрагент, ИмяФайла, ЗаказСсылка, стр1.ОбъединятьЗаказы);
...
//+++( 06.06.2018 --------------------------------------------------------
 Если ЗаказСсылка.Пустая() тогда
	...
	Если ЭтоКонфликтБлокировок тогда
	  ИмяФайла2 = стрЗаменить(ИмяФайла,".xls","(БЛОКИРОКА).xls");
	  фл2 = новый Файл(Путь+ИмяФайла2);
	  Если не фл2.Существует() тогда 
	  	  ПереместитьФайл(Путь+ИмяФайла,Путь+ИмяФайла2);
	  Иначе // 2-я блокировка ?! тогда файл останется... и уже не обрабатывается!
	  КонецЕсли;	  
	  Кому=""; // НИЧЕГО не отправляем!
    КонецЕсли;
КонецЕсли;
//+++)

	  Если элПочтаОтКого<>"" и Кому<>"" тогда
	 	ОтправитьОтветОбратно(Путь, ИмяФайла, ЗаказСсылка, элПочтаОтКого, Кому); 
	  КонецЕсли;
...
//
функция СоздатьЗаказПоТабЗнач(табЗнач, Контр, ИмяФайла = "", ЗаказСсылка, ОбъединятьЗаказы=ЛОЖЬ) экспорт
...
//--------------------проверка цен клиента-----------------------------------------------------
		...
		Иначе	
			...
			если стр1.Цена < ценаКлиента тогда
                ...
                ЗакОб.ИсключитьИзАвтоОбъединения = не ОбъединятьЗаказы; //+++ 22.05.2018 - объединять с другими заказами ТОЖЕ нельзя!
...
НомерЛистаExcel = 1;	
	...
	для каждого стр2 из табЗначОтвет цикл
		...
		КонецЕсли;
		Если стр1.ПричинаОтказа>0 тогда   //25.05.2018 - не пише
