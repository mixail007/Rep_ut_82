...
Функция НайтиТовар(Артикул="", Код="", ВидТовара1=неопределено)	
	...
	Если Код<>"" тогда //по коду точнее
		Запрос.Текст = стрЗаменить(запрос.Текст, "//условие поиска по коду или артикулу", "Номенклатура.Код = &Код");
	...
	Иначе
		...
		Запрос.Текст = стрЗаменить(Запрос.Текст, "//условие поиска по коду или артикулу", "(Номенклатура.Артикул = &Артикул ИЛИ Номенклатура.АртикулСокр = &Артикул)");
	...
	Иначе
		...
		запрос.Текст = стрЗаменить(запросТекст0, "//условие поиска по коду или артикулу",
		...
		Иначе
			...
			Если Код="" и стрДлина(Артикул)=7 тогда 
				...
				Если колЧисел=7 тогда//только в ЭТОМ случае - поиск по Коду!
					запрос.Текст = стрЗаменить(запросТекст0, "//условие поиска по коду или артикулу", "Номенклатура.Код = &Артикул");
					Результат = Запрос.Выполнить();
					Выборка = Результат.Выбрать();
