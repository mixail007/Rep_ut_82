...
имяПочтЯщикаОтвет=""; //НЕ НАДО отправлять обратно (проверка дублей! 21.05.2018)
...
функция СоздатьЗаказПоТабЗнач(табЗнач, Контр, ИмяФайла = "", ЗаказСсылка) экспорт
	...
	Для каждого стрТЗ из табЗначОтвет цикл
		...
		Если стрТЗ.КоличествоОтказ>0 тогда // нет товара 
			продолжить;
		КонецЕсли;	
