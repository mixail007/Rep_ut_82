	...
	Если РежимВызова = "Склад" Тогда
		Если ЕстьРеквизитДокумента("СкладОрдер", МетаданныеДокумента) Тогда
			Если ДокументОбъект.ВидПоступления = Перечисления.ВидыПоступленияТоваров.НаСклад Тогда
			    ...
			    СтарыйСклад = ?(Склад=неопределено,ДокументОбъект.ссылка.СкладОрдер, Склад);       // BugFix: нет реквизита (ВидСклада)
			Иначе
				НовыйСклад = ДокументОбъект.СкладОрдер.Склад;
			    СтарыйСклад = ?(Склад=неопределено,ДокументОбъект.ссылка.СкладОрдер.Склад, Склад); // BugFix: нет реквизита (ВидСклада)
			КонецЕсли;
		Иначе
			НовыйСклад  = ДокументОбъект.Склад;
		    СтарыйСклад = ?(Склад=неопределено,ДокументОбъект.ссылка.Склад, Склад);                // BugFix: нет реквизита (ВидСклада)
		КонецЕсли;

		СтарыйВидСклада = СтарыйСклад.ВидСклада;// 20.03.2019 - BugFix: нет реквизита (ВидСклада)
...
Процедура НачалоВыбораЗначенияСделки(ДокументОбъект, ФормаДокумента, ЭлементФормы, ДоговорКонтрагента, Сделка,
		...
		Если СтрокаВидаДвижения = "Приход" Тогда
			...
			Если ДоговорКонтрагента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СПокупателем
			 Или ДоговорКонтрагента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомиссионером Тогда
                ...
                СписокТипов.Добавить(Метаданные.Документы["ЗаявкаНаБрак"].Имя,							    Метаданные.Документы["ЗаявкаНаБрак"].Представление());
	...
	иначеЕсли не ТипДокументаСделки = "ЗаявкаНаБрак" Тогда
	...
	Если ТипДокументаСделки = "ЗаявкаНаБрак" Тогда		
		ФормаВыбора.Отбор.Контрагент.Значение = ДоговорКонтрагента.Владелец;
		ФормаВыбора.Отбор.Контрагент.Использование = Истина;
	КонецЕсли;

	Если ТипДокументаСделки = "ЗаказПокупателя" Тогда
		ФормаВыбора.ВызываетсяДляРаспределенияОплат = Истина;
	КонецЕсли;
...
//
&НаКлиенте
Процедура ОткрытьСвойстваДокумента(ДокументОбъектСсылка, ФормаДокумента) Экспорт
	Если Найти ( нрег( Строка(ТипЗнч(ДокументОбъектСсылка)) ),"объект") > 0 Тогда //+++ 24.01.2019  "ссылка"=0  >> нрег(..."объект")>0
		Если ДокументОбъектСсылка.ЭтоНовый() Тогда
			...
			КонецЕсли;
		КонецЕсли;
		ДокументОбъект = ДокументОбъектСсылка; //это объект 99%
	Иначе
		ДокументОбъект = ДокументОбъектСсылка.ПолучитьОбъект(); //ДокументОбъектСсылка - это ссылка!
	КонецЕсли;	
...
Процедура НапечататьДокумент(ПечДокумент, КоличествоЭкземпляров = 1, НаПринтер = Ложь, Заголовок = "") Экспорт
	...
	Если ТипЗнч(ПечДокумент) <> Тип("ТабличныйДокумент") тогда //+++ 09.11.2018 BugFix: нельзя напечатать Excel или что-то ещё...
		Возврат;
	КонецЕсли;
...
Процедура ВидимостьРеквизитаПроект(ДокументОбъект = Неопределено, ЭлементыФормы, Знач Реквизиты) Экспорт
	...
	//+++ 01.05.2018 - проверка на наличие уч.политики!
	//ВедениеУчетаПоПроектам = ПолучитьПараметрыУчетнойПолитики(Дата, Неопределено)["ВедениеУчетаПоПроектам"]
	уч = ПолучитьПараметрыУчетнойПолитики(Дата, Неопределено);
	Если ЗначениеЗаполнено(уч) тогда 
		ВедениеУчетаПоПроектам = уч.ВедениеУчетаПоПроектам;
	Иначе
...
Процедура ПоказатьСтруктуруПодчиненностиДокумента(ДокументСсылка) Экспорт
	Если ПолучитьЗначениеПоУмолчанию(ПараметрыСеанса.ТекущийПользователь, "ОграниченныйИнтерфейс") = Истина и не рольДоступна("МенеджерФормулы") Тогда
       Возврат;
	конецЕсли;
