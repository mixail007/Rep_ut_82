...
Функция ФизлицаПрочитатьXML(ЧтениеXML)
	...
	ДанныеНаименование = ПрочитатьXML(ЧтениеXML);
	ДанныеДатаРождения = Дата(ПрочитатьXML(ЧтениеXML));
	ДанныеПометкаУдаления = ПрочитатьXML(ЧтениеXML);
	ДанныеGUID = ПрочитатьXML(ЧтениеXML);
	...
	Если ФизЛицоСсылка = Неопределено ИЛИ ФизЛицоСсылка = Справочники.ФизическиеЛица.ПустаяСсылка() Тогда
		...
		Идентификатор = Новый УникальныйИдентификатор(ДанныеGUID);
		ФЛСсылка = Справочники.ФизическиеЛица.ПолучитьСсылку(Идентификатор);
		ФизЛицо = ФЛСсылка.ПолучитьОбъект();
		Если ФизЛицо = Неопределено Тогда
			ФизЛицо = Справочники.ФизическиеЛица.СоздатьЭлемент();
			ФизЛицо.УстановитьСсылкуНового(ФЛСсылка);
		КонецЕсли;
		ФизЛицо = Справочники.ФизическиеЛица.СоздатьЭлемент();
		ФизЛицо.Код = ДанныеКод;
		ФизЛицо.Наименование = ДанныеНаименование;
		ФизЛицо.ДатаРождения = ДанныеДатаРождения;
		ФизЛицо.Комментарий = "Принято из ЗУП";
		Если ДанныеПометкаУдаления Тогда 
	...
	Иначе
		...
		Если ФизЛицо.Наименование <> ДанныеНаименование Тогда
			ФизЛицо.Наименование = ДанныеНаименование;
		КонецЕсли;
		Если ФизЛицо.ДатаРождения <> ДанныеДатаРождения Тогда
			ФизЛицо.ДатаРождения = ДанныеДатаРождения;
		КонецЕсли;
		Если ФизЛицо.ПометкаУдаления <> ДанныеПометкаУдаления Тогда
			ФизЛицо.ПометкаУдаления = ДанныеПометкаУдаления;
		КонецЕсли;
