...
Процедура ЗаписатьСообщениеСИзменениями() Экспорт
	...
	ИмяФайла = КаталогВременныхФайлов() + "\" + 
			   ПолучитьИмяФайлаОбменаФЛК(ПланыОбмена.ФизлицаКонтрагенты.ЭтотУзел(), Ссылка, "_FLK");
...
Функция ЗаписатьНовоеСообщение(ИмяФайла)
	...
	//	 КонецЕсли;
	//	 
	//	// Записываем данные в сообщение, если это контрагент и он не группа
	//	Если (СтрЧислоВхождений(Строка(ТипЗнч(Данные)),"Справочник объект: Контрагенты")>0) Тогда 
	//		ФизЛицо = НайтиФизЛицо(Данные.Ссылка);
	//		ФлагГруппы = ВходитВГруппуКонтрагентыДляЗУП(Данные.Ссылка);
	//		Если (ФизЛицо<>Неопределено и не Данные.ЭтоГруппа) Тогда 
...
Процедура ЗаписатьДанные(ЗаписьXML, Данные)
	...
	//Если (Не Удаление И ((ОбъектМетаданных = Метаданные.Справочники.Контрагенты) или (ОбъектМетаданных = Метаданные.Справочники.ФизическиеЛица))) Тогда
...
Функция ПрочитатьДанные(ЧтениеXML,Отправитель)
	...
	//	
	//	// Пытаемся прочесть значение справочника Номеклатура
	//	Возврат КонтрагентыПрочитатьXML(ЧтениеXML);
	//	
	//ИначеЕсли (ТипXML.ИмяТипа = "CatalogObject.Сотрудники.Вручную" 
	//   И ТипXML.URIПространстваИмен = "") Тогда
	//	
	//	// Пытаемся прочесть значение справочника Номеклатура
	//	Возврат СотрудникиПрочитатьXML(ЧтениеXML,Отправитель);
	...
	ИначеЕсли (ТипXML.ИмяТипа = "DeleteObject" 
	   И ТипXML.URIПространстваИмен = "") Тогда
		...
		// Если это контрагент или физ. лицо - очищаем справочник сотрудников.
...
Процедура КонтрагентыЗаписатьXML(ЗаписьXML, Данные)
	...
	Если (ФизЛицо = Неопределено) Тогда 
		ВызватьИсключение "Не найдено физ. лицо";
	КонецЕсли;
	ЗаписатьXML(ЗаписьXML, ФизЛицо, "NPref", НазначениеТипаXML.Явное);
...
Функция ФизлицаПрочитатьXML(ЧтениеXML)
	...
	ФизЛицо = ДанныеСсылка.ПолучитьОбъект();
	...
	Если ФизЛицо <> Неопределено Тогда
		...
		Если ФизЛицо.ЭтоГруппа <> ЭтоГруппа Тогда
	...
	Иначе
		...
		ФизЛицо.УстановитьСсылкуНового(ДанныеСсылка);
		
	КонецЕсли;
	
	// Код
	ФизЛицо.Код = ПрочитатьXML(ЧтениеXML);
	// Наименование
	ФизЛицо.Наименование = ПрочитатьXML(ЧтениеXML);
	// Датарождения
	ФизЛицо.ДатаРождения = Дата(ПрочитатьXML(ЧтениеXML));
	// Комментарий
...
Функция ФИОПрочитатьXML(ЧтениеXML,Отправитель)
	...
	// Ссылка на физлицо
	СсылкаФЛ = ПрочитатьXML(ЧтениеXML);
	// Дата
	Дата = Дата(ПрочитатьXML(ЧтениеXML));
	// Далее - читаем физ лицо
	Данные = ФизлицаПрочитатьXML(ЧтениеXML);	
	...
	Набор.Отбор.ФизЛицо.Значение = СсылкаФЛ;
	Набор.Отбор.ФизЛицо.Использование = Истина;
	Движение = Набор.Добавить();
	
	Движение.ФизЛицо 	= СсылкаФЛ;
...
Функция ПаспортныеДанныеПрочитатьXML(ЧтениеXML,Отправитель)
	...
	ДанныеСсылка = ПрочитатьXML(ЧтениеXML);
	Если (ТипЗнч(ДанныеСсылка) <> Тип("СправочникСсылка.ДокументыУдостоверяющиеЛичность")) Тогда
	...
	// Пытаемся создать объект по полученной ссылке
	ДокументУЛ = ДанныеСсылка.ПолучитьОбъект();
	...
	Если ДокументУЛ = Неопределено Тогда
		...
		// Устанавливаем значение ссылки для нового объекта
		ДокументУЛ.УстановитьСсылкуНового(ДанныеСсылка);
...
Функция ПолучитьИмяФайлаОбменаФЛК(УзелИсточник, УзелПриемник, Постфикс = "") Экспорт
	...
	Возврат "MessageKFL" + СокрЛП(УзелИсточник.Код) + "_" + СокрЛП(УзелПриемник.Код) + СокрЛП(Постфикс) + ".xml";
